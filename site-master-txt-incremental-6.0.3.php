...
$sqlquery="SELECT site_id,emp_code,site_name,address,city,location,sub_area,state,pin,contact_person,phone_no,site_reffered_by,
		...
		follow_up_date,expected_month_maturity,escalation_clause,auth_retailer_involved
...
$count=mysqli_num_rows($result);
	...
	if($count>0){
		...
		while($rowsite = mysqli_fetch_assoc($result))
		{
			...
			$contents  .= (($rowsite['remarks']!='')?$rowsite['remarks']: ' ')."^";
	...
	$url = APICALLLOGURL."/site-master-txt-incremental-6.0.3.php?nick_name=$nick_name&emp_code=$emp_code&last_update_time=$last_update_time&incremental_download=$incremental_download";
