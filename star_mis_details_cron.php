...
while($row_select_emp_details = mysqli_fetch_assoc($res_select_emp_details)){
	...
	for($i=1;$i<=3;$i++){
		...
		/*if($i == 1){
			...
			$res_get_tdy_present = mysqli_query($link,$sql_get_tdy_present);
			$emp_present = mysqli_num_rows($res_get_tdy_present);
		}*/
		if($i == 2)
		{
			...
			$res_get_mtd_present = mysqli_query($link,$sql_get_mtd_present);
			$emp_present = mysqli_num_rows($res_get_mtd_present);
		}
		//For YTD OR Year Today
		if($i == 3)
		{
			 ...
			 $res_get_ytd_present = mysqli_query($link,$sql_get_ytd_present);
			 $emp_present = mysqli_num_rows($res_get_ytd_present);
		}
		...
		$res_collection_customer = mysqli_query($link,$sql_collection_customer);
		while($row_collection_customer = mysqli_fetch_assoc($res_collection_customer)){
		...
		$res_emp_exist_check = mysqli_query($link,$sql_emp_exist_check);
		$emp_row_check = mysqli_num_rows($res_emp_exist_check);
		if($emp_row_check>0){
