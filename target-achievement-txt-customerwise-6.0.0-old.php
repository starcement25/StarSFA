...
if(providing_code=='yes')
{
	...
	{
	  $sqlquery="SELECT DISTINCT SCW.customer_code,CMA.customer_code AS customer_code_internal,CMA.customer_name,SCW.vertical,SCW.product_group_code,SCW.prod_code,SCW.jan_31_target,SCW.jan_31_achievement,SCW.feb_28_target,SCW.feb_28_achievement,
		...
		SCW.dec_31_achievement FROM self_appraisal_summary SCW,customer_route_emp_relation CM,customer_master CMA WHERE 
		SCW.customer_code = CMA.dns_customer_code AND CMA.customer_code=CM.customer_code AND  ".$emp_hierarchy_condition."";
	}
	else
	{
		$sqlquery="SELECT DISTINCT SCW.customer_code,CM.customer_name,SCW.vertical,SCW.product_group_code,SCW.prod_code,SCW.jan_31_target,SCW.jan_31_achievement,SCW.feb_28_target,SCW.feb_28_achievement,SCW.mar_31_target,
		...
		FROM self_appraisal_summary SCW,customer_master CM WHERE 
...
else
{
	if(modified_customer_emp_route=='yes')
	{
		$sqlquery="SELECT DISTINCT SCW.customer_code,CMA.customer_name,SCW.vertical,SCW.product_group_code,SCW.prod_code,SCW.jan_31_target,SCW.jan_31_achievement,SCW.feb_28_target,SCW.feb_28_achievement,SCW.mar_31_target,
		SCW.mar_31_achievement,SCW.apr_30_target,SCW.apr_30_achievement,SCW.may_31_target,SCW.may_31_achievement,SCW.jun_30_target,
		SCW.jun_30_achievement,SCW.jul_31_target,SCW.jul_31_achievement,SCW.aug_31_target,SCW.aug_31_achievement,SCW.sep_30_target,
		SCW.sep_30_achievement,SCW.oct_31_target,SCW.oct_31_achievement,SCW.nov_30_target,SCW.nov_30_achievement,SCW.dec_31_target,SCW.dec_31_achievement 
		FROM self_appraisal_summary SCW,customer_route_emp_relation CM,customer_master CMA WHERE 
...
$count=mysqli_num_rows($result);
	...
	if($count>0){
		...
		{
			$customer_code=$rowsappraisal['customer_code_internal'];
			...
			if(achievement_sale_value_wise=='yes')
			{
				...
				$sqlachivementval="SELECT SUM(amount) AS total_amount,SUBSTRING(invoice_date,6,2) As month
								  FROM 
								purchase_details WHERE  distributor_code='".$customer_code."' AND vertical_value='".$vertical."' 
								AND invoice_date 
				...
				while($rowachivementval=mysqli_fetch_assoc($rsachivementval))
				{
					...
					${achievement_val.$customer_code.$monthval}=$total_amount;
				}
			}
			for($i=1;$i<=12;$i++)
			{
				...
				else
				{
					...
					$column_achievement=${achievement_val.$customer_code.$i};
				}
	...
	$url = APICALLLOGURL."/target-achievement-txt-customerwise-6.0.0.php?nick_name=$nick_name&emp_code=$emp_code";
