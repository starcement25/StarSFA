...
if($nick_name=='STAR'){
	...
	if(joint_work=='yes' && !in_array('joint_work',$menu_access_array))
	{
		...
		AND emp_code IN(".$employee_hierarchy.") AND emp_code IN(SELECT emp_code FROM employee_master WHERE level='1' and acedns='Y') ".$login_condition;*/
		if(strtoupper($nick_name)=='PALSONS')
		{
			$sqlquery="SELECT *,DATE_FORMAT(visit_date,'%d-%m-%Y') AS visit_date FROM route_plan WHERE route_code <> '' 
						AND emp_code IN(".$employee_hierarchy.") AND route_code NOT 
						IN(select DISTINCT route_code FROM route_plan where emp_code!='".$emp_code."' and route_code='RT/3')";
		}
		else
		{
			$sqlquery="SELECT *,DATE_FORMAT(visit_date,'%d-%m-%Y') AS visit_date FROM route_plan WHERE route_code <> '' 
...
$count=mysqli_num_rows($result);
	...
	if($count>0){
		...
		while($rowrouteplan = mysqli_fetch_assoc($result))
		{
				...
				$working_with_data  = (($rowrouteplan['working_with']!='')?$rowrouteplan['working_with']: ' ');
				if($working_with_data=="null"){
				    $working_with_data = "";
				}
				if($working_with_data=="NULL"){
				    $working_with_data = "";
	...
	$url = APICALLLOGURL."/route-plan-master-txt-6.0.5.php?nick_name=$nick_name&emp_code=$emp_code&last_update_time=$last_update_time&data_download_time=$data_download_time&incremental_download=$incremental_download";
