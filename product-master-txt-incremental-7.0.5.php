...
else
{
	...
	if(state_wise_product=='yes')
	{
		...
		$state_code=$rowstatecode['state_code'];
		$sqlquery="SELECT DISTINCT PM.*
						FROM product_master PM WHERE PM.prod_desc <>'' AND FIND_IN_SET( PM.state_code,'".$state_code."') ".$condition_one." ".$login_condition." ORDER BY PM.prod_desc ASC";
	}
	else
	{
		$sqlquery="SELECT DISTINCT PM.* FROM product_master PM WHERE PM.prod_desc <>'' ".$condition_one." ".$login_condition." ORDER BY PM.prod_desc ASC";
...
$count=mysqli_num_rows($result);
	...
	if($count>0){
		...
		while($rowproduct = mysqli_fetch_assoc($result))
		{
			...
			if(trim($rowproduct['alias'])!=''){
				$prod_desc=trim($rowproduct['alias']);
			}
			else
			{
			...
			$contents  .= (($rowproduct['UOM4']!='')?$rowproduct['UOM4']: ' ')."^";
			$contents  .= (($rowproduct['UOM5']!='')?$rowproduct['UOM5']: ' ');
	...
	$url = DEV_BASE_URL."product-master-txt-incremental-7.0.5.php?nick_name=$nick_name&emp_code=$emp_code&last_update_time=$last_update_time&data_download_time=$data_download_time&incremental_download=$incremental_download";
