...
if($countbranches>1)
{
  /*$sqlquery="SELECT DISTINCT PM.*
	...
	{		
		$sqlquery="SELECT DISTINCT PM.*
					FROM product_master PM,employee_master EM
					WHERE FIND_IN_SET(PM.branch_code,EM.branch_code) AND PM.prod_desc <>''  
					AND EM.emp_code ='".$emp_code."' ".$condition_one." ".$login_condition." ORDER BY PM.prod_desc ASC";
	}
	else
	{
...
$count=mysqli_num_rows($result);
	...
	if($count>0){
		...
		while($rowproduct = mysqli_fetch_assoc($result))
		{
			...
			$contents  .= (($rowproduct['TD']!='')?$rowproduct['TD']: ' ')."^";
	...
	$url = "http://www.acedns.in/acednsproduct/product-master-txt-incremental-6.0.2.php?nick_name=$nick_name&emp_code=$emp_code&last_update_time=$last_update_time&data_download_time=$data_download_time&incremental_download=$incremental_download";
