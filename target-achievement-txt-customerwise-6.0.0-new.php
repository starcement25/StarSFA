  ...
  if(achievement_sale_value_wise=='yes')
   {
	...
	$sql_customer = "SELECT customer_code,customer_name,dns_customer_code FROM customer_master where 
					cust_type='D' AND customer_code IN(SELECT DISTINCT customer_code FROM customer_route_emp_relation  
	...
	if($count>0){
		...
		$customer_name = $row_customer['customer_name'];
			 foreach($vertical_array as $vertical_val)
			 {
				...
				$sqlqueryappraisal="SELECT SCW.product_group_code,SCW.prod_code,SCW.jan_31_target,SCW.jan_31_achievement,SCW.feb_28_target,SCW.feb_28_achievement,
				...
				$rowsappraisal=mysqli_fetch_assoc($rsqueryappraisal);
					...
					for($i=1;$i<=12;$i++)
					{
						...
						$coumn_achievement=strtolower($month_abrev).'_'.$days.'_achievement';
							...
							$column_achievement=${achievement_val.$dns_customer_code.$vertical_val.$i};
