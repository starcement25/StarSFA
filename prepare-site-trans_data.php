	...
	function excelDownload()
	{
		...
		if($from_date!='' && $to_date!='')
		{
		  $date_condition=" AND DATE_FORMAT(SUBSTRING(SO.survey_id,-14,8),'%Y-%m-%d') >='".$from_date."' AND 
			  ...
			  $sql_get_menu = "SELECT layout_name,menu_id,survey_sub_menu FROM survey_input WHERE type='menu' AND menu_id IN('RA001','RA002') ORDER BY display_order ASC";
			...
			for($i=0;$i<count($menu_id_array);$i++)
			{
				...
				${file_name.$menu_id_array[$i]}="$menu_name_array[$i]_$survey_sub_menu_array[$i]_Oct2022_$date$month$year$hour$minute$second.xls";
				
				//add files to the zip, passing file contents, not actual files
				//$zip->addFile($file_content1, $file_name1);
				$zip->addFile(${file_content.$menu_id_array[$i]}, ${file_name.$menu_id_array[$i]});
			}
			$finalzipfile='site-data.zip';
