		...
		if($count>0){
			...
			$contentsdatetime =$year.'-'.$month.'-'.$date.'€'.$hour.':'.$minute.':'.$second."\n";*/
				...
				for($i=1;$i<=12;$i++)
				{
					...
					${'target'.$emp_code_db.$route_code.$product_category.$i}=$column_target;
			...
			foreach($emp_roue_array as $emp_roue_val)
			{
				...
				for($m=1;$m<=12;$m++)
				{
					...
					$contents  = (($emp_code_fetched!='')?$emp_code_fetched: ' ')."^";
					$contents  .= ((${'emp_name'.$emp_code_fetched.$route_code}!='')?trim(preg_replace('/[\r\n]+/', '',${'emp_name'.$emp_code_fetched.$route_code})): ' ')."^";
					$contents  .= (($route_code!='')?$route_code: ' ')."^";
					$contents  .= ((${'route_name'.$emp_code_fetched.$route_code}!='')?trim(preg_replace('/[\r\n]+/', '',${'route_name'.$emp_code_fetched.$route_code})): ' ')."^";
					$contents  .= 'Area 1'."^";
					$contents  .= ' '."^";
					for($k=1;$k<=3;$k++)
					{
						...
						$contents  .= ((${'target'.$emp_code_fetched.$route_code.$product_category.$m}!='')?${'target'.$emp_code_fetched.$route_code.$product_category.$m}: ' ')."^";
						$contents  .= ((${'achievement'.$emp_code_fetched.$route_code.$product_category.$m}!='')?${'achievement'.$emp_code_fetched.$route_code.$product_category.$m}: ' ')."^";
					}
					$contents  .= $m;
					$linecontents  .= $contents."\n";
					$countappraisal++;
