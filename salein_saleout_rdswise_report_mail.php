<?php
	...
	define("BCCEMAIL","dipankarc@coral.in,acedns@coral.in");
		...
		$sqlbranch="SELECT branch_code,branch_name FROM branch_master WHERE branch_code IN 
					(SELECT DISTINCT branch_code FROM branch_rds_product_wise_stock) ORDER BY branch_name ASC";
        ...
        while($rowbranch=mysqli_fetch_assoc($resbranch))
        {
			...
			while($rowproduct=mysqli_fetch_assoc($resproduct))
			{
				...
				$sqlrdsemployee="SELECT rds_code,rds_name FROM rds_master WHERE emp_code IN($branch_employee) ORDER BY rds_name ASC";
				...
				while($rowrdsemployee=mysqli_fetch_assoc($resrdsemployee))
				{
					...
					$sql_branch_rds_product_stk="SELECT * FROM branch_rds_product_wise_stock WHERE branch_code='".$branch_code."' AND rds_code='".$rds_code."' 
					...
					$row_branch_rds_product_stk=mysqli_fetch_assoc($res_branch_rds_product_stk);
			...
			$mailsubj="RKBK - ".$branch_name." sale in sale out report RDS and ITEM wise";
			...
			$email='auto@rkbk.in,ak@coral.in';				
