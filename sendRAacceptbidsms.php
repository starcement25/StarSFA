<?php
	...
	$condition=" AND UNIX_TIMESTAMP(NOW()) > UNIX_TIMESTAMP(DATE_FORMAT(SUBSTRING(RAD.bid_id,-14,14),'%Y%-%m-%d %H:%i:%s') + INTERVAL 15 MINUTE)";
	$sqlquery="SELECT DISTINCT RAD.customer_code,RAD.bid_rate,CM.customer_name,PM.prod_desc,CM.phone_no,RAD.sms_done,RAD.prod_code,RAD.bid_id FROM RA_bid_rate_details RAD,customer_master CM,product_master PM WHERE RAD.bid_status='ACCEPT' AND RAD.counter_bid='N' AND RAD.prod_code=PM.dns_prod_code AND RAD.customer_code=CM.customer_code AND RAD.sms_done='0' AND SUBSTRING(RAD.bid_id,-14,8)='".$curentdatestring."'".$condition;
