...
class xml_customer_exists_header{
    var $emp_code, $trans_id,$prospect_date,$latt,$longi,$prospectiveheader_trans_id,$customer_code,$customer_name,$remarks,$check_in_time;
}
...
function contents($parser, $data){
    global $current_tag, $emp_code,$trans_id,$prospect_date,$latt,$longi,$prospectiveheader_trans_id,$customer_code,$customer_name,$remarks,$check_in_time,$counter,$prospective_customer_exists_array;
	//echo $current_tag.'<br />';
	//echo $data;
	if(substr($current_tag,0,26)=='*ROOT*PROSPECTIVE_CUSTOMER')
	{
		switch($current_tag){
			...
			case $customer_name:
				...
				break;		
			case $check_in_time:
				$prospective_customer_exists_array[$counter]->check_in_time = $data;
				break;
			case $remarks:
...
if(count($prospective_customer_exists_array)>0)
{
	...
	for($x=0;$x<count($prospective_customer_exists_array);$x++){
		...
		$check_in_time=$prospective_customer_exists_array[$x]->check_in_time;
		...
		if($countchkattlocation>0)
		{
			...
			$rsupdateorlocation=mysqli_query($link,$sqlupdateorlocation) or die(mysqli_error()." Error in update prospective customer exisists location: ".$sqlupdateorlocation);
			if($rsupdateorlocation)
			{
				$sqlupdateproheader="UPDATE prospective_customer_header SET check_in_time='".addslashes($check_in_time)."'
									WHERE trans_id='".$prospectiveheader_trans_id."'";
									//echo $sqlupdateproheader;exit();
				$rsuupdateproheader=mysqli_query($link,$sqlupdateproheader);					
		...
		else
		{
			...
			$sqlinsertproheader="INSERT INTO prospective_customer_header SET trans_id='".$prospectiveheader_trans_id."',
									...
									check_in_time='".addslashes($check_in_time)."',
									address='',
									pin ='',
									remarks='".addslashes($remarks)."'";
									//echo $sqlinsertproheader;exit();
