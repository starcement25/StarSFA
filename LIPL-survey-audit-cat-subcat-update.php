<?php
	...
	$sqlselallcatsubcat="SELECT row_id,value,survey_id FROM `survey_audit` WHERE row_id IN ('RA037', 'RA038', 'RA168', 'RA169')";
	...
	while($rowselallcatsubcat=mysqli_fetch_assoc($rsselallcatsubcat))
	{
		...
		if($row_id=='RA037' || $row_id=='RA168' )
		{
			...
			$sqlupdatesurveyoutputcat="UPDATE survey_audit SET value='".addslashes(${catstring.$survey_id})."' WHERE survey_id='".$survey_id."' 
									AND row_id='".$row_id."'";
		...
		if($row_id=='RA038' || $row_id=='RA169' )
		{
			...
			$sqlupdatesurveyoutputsubcat="UPDATE survey_audit SET value='".addslashes(${subcatstring.$survey_id})."' WHERE survey_id='".$survey_id."' 
									AND row_id='".$row_id."'";
