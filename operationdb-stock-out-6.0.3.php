...
/*$body="<?xml version='1.0' encoding='UTF-8'?><root><STOCK_OUT_DETAILS><location><emp_code><![CDATA[E0185]]></emp_code><trans_id><![CDATA[SOE018520190129182403]]></trans_id><latt><![CDATA[22.7279445]]></latt><longi><![CDATA[88.4906586]]></longi><date><![CDATA[2019-01-29 18:24:03]]></date></location><STOCK_OUT_DATA><STOCK_OUT_ID><![CDATA[SOE018520190129182403]]></STOCK_OUT_ID><PROD_CODE><![CDATA[12076]]></PROD_CODE><IMEI><![CDATA[]]></IMEI><STOCK_OUT_DATE><![CDATA[2019-01-29]]></STOCK_OUT_DATE><STOCK_OUT_QTY><![CDATA[5]]></STOCK_OUT_QTY></STOCK_OUT_DATA><STOCK_OUT_DATA><STOCK_OUT_ID><![CDATA[SOE018520190129182403]]></STOCK_OUT_ID><PROD_CODE><![CDATA[12077]]></PROD_CODE><IMEI><![CDATA[]]></IMEI><STOCK_OUT_DATE><![CDATA[2019-01-29]]></STOCK_OUT_DATE><STOCK_OUT_QTY><![CDATA[4]]></STOCK_OUT_QTY></STOCK_OUT_DATA></STOCK_OUT_DETAILS></root>";*/
...
$stock_out_customer_code="*ROOT*STOCK_OUT_DETAILS*STOCK_OUT_DATA*STOCK_OUT_CUSTOMER_CODE";
...
class xml_stock_out_details{
	var $stock_out_id,$prod_code,$IMEI,$stock_out_date,$stock_out_qty,$stock_out_customer_code;
}
...
function contents($parser, $data){
    global $current_tag,$counter,$counterstockout,$location_emp_code,$location_trans_id,$location_latt,$location_longi,$location_date,$stock_out_id,$prod_code,$IMEI,$stock_out_date,$stock_out_qty,$stock_out_customer_code,$stock_out_array,$stock_out_details_array;
	//echo $current_tag.'<br />';
	//echo $data.'<br />';
	...
	if(substr($current_tag,0,38)=='*ROOT*STOCK_OUT_DETAILS*STOCK_OUT_DATA')
	 {
			...
			switch($current_tag){
			   ...
			   case $stock_out_qty:
					...
					break;
			  case $stock_out_customer_code:
					$stock_out_details_array[$counterstockout]->stock_out_customer_code = $data;
					$counterstockout++;
					break;					
		 }
	}
	 ...
	 if(count($stock_out_details_array)>0)
		{
			for($i=0;$i<count($stock_out_details_array);$i++){
				...
				$stock_out_customer_code=$stock_out_details_array[$i]->stock_out_customer_code;
				$sqlcheckIMEI="SELECT IMEI FROM stock_out_details WHERE IMEI='".$IMEI."'";
				$rscheckIMEI=mysqli_query($link,$sqlcheckIMEI);
				$cntcheckIMEI=mysqli_num_rows($rscheckIMEI);
				if(!in_array($stock_out_id,$stock_out_array_trans_id) && $cntcheckIMEI==0)
				{
					...
					if(mysqli_query($link,$sqlinsertstockout))
					{
						...
						if($IMEI!='')
						{
						$sqlupdatestockout="UPDATE customer_product_billing SET stock_out_date='".$stock_out_date."',
											stock_out_customer_code='".$stock_out_customer_code."',download_time=CURRENT_TIMESTAMP() WHERE 
