	...
	if($countbranches>1)
	{
		if($nick_name=='EMAMI')
		{
			...
			$sqlquery="SELECT DISTINCT PGM.* FROM product_group_master PGM,employee_master EM,product_master PM WHERE 
					...
					AND EM.emp_code='".$emp_code."' ".$condition_branch.$condition_one." ".$login_condition." 
...
$count=mysqli_num_rows($result);
	...
	if($count>0){
		...
		while($rowproductgroup = mysqli_fetch_assoc($result))
		{
			...
			$contents  .= (($rowproductgroup['vertical_value']!='')?$rowproductgroup['vertical_value']: ' ')."^";
			$contents  .= (($rowproductgroup['stock_out_type']!='')?$rowproductgroup['stock_out_type']: ' ');
	...
	$url = APICALLLOGURL."/product-group-master-txt-incremental-6.0.2.php?nick_name=$nick_name&emp_code=$emp_code&last_update_time=$last_update_time&data_download_time=$data_download_time&incremental_download=$incremental_download";
