	...
	if(($versionCodecurrent-$user_db_version_code) > '.1' && $is_update==1 && $incremental_download=='yes' && $last_update_time!='1971-01-01 10:10:10')
		...
		if(in_array("product_master",$need_download_table_array))
		{
			if(branch_wise_product=='yes')
			{
				$sqlprodcnt="SELECT COUNT(PM.prod_code) AS total_product FROM product_master PM,employee_master EM
							WHERE FIND_IN_SET(PM.branch_code,EM.branch_code) AND EM.emp_code='".$emp_code."' ".$condition_verticle."";
		...
		if((mrp=='yes' || (sale_rate=='yes' && sale_rate_input_dropdown=='dropdown')) && in_array("mrp_master",$need_download_table_array)) {
		   if(branch_wise_mrp=='yes')
			{
			   $sqlmrpcnt="SELECT COUNT(MRP.mrp_code) AS total_mrp FROM mrp MRP,employee_master EM WHERE FIND_IN_SET(MRP.branch_code,EM.branch_code) AND EM.emp_code='".$emp_code."'";
...
if($successval=="1"){
	if($incremental_download=='no'){
		...
		if(mrp=='yes' || (sale_rate=='yes' && sale_rate_input_dropdown=='dropdown')){
		  if(branch_wise_mrp=='yes')
			{
			   $sqlmrpcnt="SELECT COUNT(MRP.mrp_code) AS total_mrp FROM mrp MRP,employee_master EM WHERE FIND_IN_SET(MRP.branch_code,EM.branch_code) 
	...
	else
	{
		...
		if(vertical_fields=='yes'){
			...
			$condition_verticle=' AND vertical_value IN ('.$emp_vertical_value.')';*/
			$sqlempvertical="SELECT vertical_value FROM employee_master WHERE emp_code='".$emp_code."'";
			$rsempvertical=mysqli_query($link,$sqlempvertical);
			$rowempvertical=mysqli_fetch_assoc($rsempvertical);
			$emp_vertical_value=$rowempvertical['vertical_value'];
		...
		if(in_array('route_master',$need_update_table_array))
		{
			...
			$sqlroutecnt="select RM.route_code from route_master RM,customer_master CM WHERE RM.route_code=CM.route_code AND 
						  UNIX_TIMESTAMP(RM.download_time) > UNIX_TIMESTAMP('".$last_update_time."') ".$emp_val_condition_route." GROUP BY RM.route_code";				
		...
		if(in_array('product_master',$need_update_table_array))
			...
			{
				$sqlprodcnt="SELECT COUNT(PM.prod_code) AS total_product FROM product_master PM,employee_master EM
							WHERE FIND_IN_SET(PM.branch_code,EM.branch_code) AND UNIX_TIMESTAMP(PM.download_time) > UNIX_TIMESTAMP('".$last_update_time."') 
							AND EM.emp_code='".$emp_code."' ".$condition_verticle."";
				$rsprodcnt=mysqli_query($link,$sqlprodcnt);
				$rowprodcnt=mysqli_fetch_assoc($rsprodcnt);
				$prodcnt=$rowprodcnt['total_product'];
		...
		if(mrp=='yes' || (sale_rate=='yes' && sale_rate_input_dropdown=='dropdown')){
			if(in_array('mrp',$need_update_table_array))
			{
				...
				if(branch_wise_mrp=='yes')
				{
				   $sqlmrpcnt="SELECT COUNT(MRP.mrp_code) AS total_mrp FROM mrp MRP,employee_master EM 
				              WHERE FIND_IN_SET(MRP.branch_code,EM.branch_code) AND EM.emp_code='".$emp_code."' AND UNIX_TIMESTAMP(MRP.download_time) > UNIX_TIMESTAMP('".$last_update_time."')";
					$rsmrpcnt=mysqli_query($link,$sqlmrpcnt);
					$rowmrpcnt=mysqli_fetch_assoc($rsmrpcnt);
					$mrpcnt=$rowmrpcnt['total_mrp'];
	...
	$datetime = gmdate('Y-m-d H:m:s',strtotime('+330 minute'));
	$url = "http://www.acedns.in/acednsproduct/datadownloaddictionary-6.0.3.php?nick_name=$nick_name&emp_code=$emp_code&device_id=$device_id&last_update_time=$last_update_time&incremental_download=$incremental_download";
	insertapilog($datetime,$emp_code,$url,$nick_name);
	/*$config = 'api_calllog.txt';
	$file=fopen($config,"r+");
	$date = date("F j, Y");
