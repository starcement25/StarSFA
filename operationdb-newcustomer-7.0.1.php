...
$TA_DA_mode="*ROOT*NEW_CUSTOMER*LOCATION*TA_DA_MODE";
...
class xml_new_customer{
	var $location_emp_code,$location_trans_id,$location_latt,$location_longi,$location_date,$TA_DA_mode,$new_customer_code,$new_customer_name,$phone_no,$pin_code,$area,$area_name,$rds_tag,$address,$landline_no,$owner_name,$owner_phone,$cust_class,$weekly_closing_day,$coverage_type,$TIN,$PAN,$email,$acedns,$branch_code,$base_latt,$base_longi,$image,$category_of_store,$instore_activity;	
}
...
function contents($parser, $data){
    global $current_tag,$counter,$location_emp_code,$location_trans_id,$location_latt,$location_longi,$location_date,$TA_DA_mode,$new_customer_code,$new_customer_name,$phone_no,$pin_code,$area,$area_name,$rds_tag,$address,$landline_no,$owner_name,$owner_phone,$cust_class,$weekly_closing_day,$coverage_type,$TIN,$PAN,
	...
	if(substr($current_tag,0,18)=='*ROOT*NEW_CUSTOMER')
	{
		...
		switch($current_tag){
			...
			case $location_date:
				...
				break;
			case $TA_DA_mode:
				$new_customer_array[$counter]->TA_DA_mode = $data;
				break;	
...
if(count($new_customer_array)>0)
{
	...
	for($x=0;$x<count($new_customer_array);$x++){
		...
		$TA_DA_mode=$new_customer_array[$x]->TA_DA_mode;
		...
		if(strtoupper($nick_name)=='ABDOS')
			{
				$TA_DA_condiion=" TA_DA_mode	='".$TA_DA_mode."',";
			}
			else
			{
				$TA_DA_condiion='';
		...
		$sqlinsertorlocation="INSERT INTO location SET emp_code='".$location_emp_code."',
									...
									latt='".$location_latt."',
									longi='".$location_longi."',
									date='".$location_date."',".$TA_DA_condiion."
	  ...
	  if(mysqli_query($link,$sqlinsertorlocation))
	   ...
	   /*$sqlinsertcustomerrelation = "INSERT INTO customer_route_emp_relation 
			...
			if(modified_customer_emp_route=='yes')
			{
			   if(strtoupper($nick_name)=='PALSONS')
			   {
				    $sqlinsertcustomer="INSERT INTO customer_master SET customer_code ='".$new_customer_code."',
								   ...
								   base_longi='0',
								   activated_by = '0',
								   appointment_date = '1970-01-01 10:00:00',
								   date_of_birth= '1970-01-01 10:00:00',
								   date_of_anniversary= '1970-01-01 10:00:00',
								   whatsapp_no='',
								   beneficiary_name='',
								   IFS_code='',
								   GST_type='0',
								   emp_code = '',
								   cust_class_update_by='',
			   ...
			   $sqlinsertcustomer="INSERT INTO customer_master SET customer_code ='".$new_customer_code."',
								   ...
								   state_code					='".$state."',
								    category_of_store			='".$category_of_store."',
								   instore_activity			='".$instore_activity."',
								   activated_datetime			='1970-01-01 10:00:00',
								   cust_class_update_time='1970-01-01 10:00:00',
								   emp_code					='".$location_emp_code."',
								   current_balance = '0',
								   credit_limit = '0',
								   monthly_potential='0',
								   sauda_limit='0',
		...
		$sqlinsertcustomer="INSERT INTO customer_master SET customer_code ='".$new_customer_code."',
						   ...
						   email						='".$email."',
						   state_code					='".$state."',
						   activated_datetime			='1970-01-20 10:00:00',
						   cust_class_update_time='1970-01-01 10:00:00',
						   credit_limit = '0',
						   monthly_potential='0',
						   sauda_limit='0',
	   ...
	   /*$sqlinsertcustomerrelation = "INSERT INTO customer_route_emp_relation 
		...
		//print_r($addcustomermailbody);
		if (!empty($addcustomer_email) ) {	
