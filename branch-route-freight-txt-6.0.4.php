...
else
{
	...
	if($count>0){
		...
		while($rowbranch = mysqli_fetch_assoc($result))
		{
			...
			$contents  .= (($rowbranch['transport_mode']!='')?strtoupper($rowbranch['transport_mode']): ' ')."^";
			$contents  .= (($rowbranch['vertical_value']!='')?$rowbranch['vertical_value']: ' ')."^";
			$contents  .= (($rowbranch['transit_time']!='')?$rowbranch['transit_time']: ' ');
	...
	$url = "http://salesmpower.acedns.in/branch-route-freight-txt-6.0.4.php?nick_name=$nick_name&emp_code=$emp_code&last_update_time=$last_update_time&data_download_time=$data_download_time&incremental_download=$incremental_download";
