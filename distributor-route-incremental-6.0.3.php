...
if($emp_code!='C0007'){
	$sqlquery="select distributor_code,route_code,emp_code,acedns,visit_day FROM distributor_route_relation WHERE 1 ".$emp_hierarchy_condition." ".$login_condition."";
}
else
{
	$sqlquery="select distributor_code,route_code,emp_code,acedns,visit_day FROM distributor_route_relation WHERE 1 ".$login_condition."";
}
...
$count=mysqli_num_rows($result);
	...
	if($count>0){
		...
		while($rowdistributorroute = mysqli_fetch_assoc($result))
		{
			...
			$contents  .= (($rowdistributorroute['acedns']!='')?$rowdistributorroute['acedns']: ' ')."^";
			$contents  .= (($rowdistributorroute['visit_day']!='')?$rowdistributorroute['visit_day']: ' ');
	...
	$url = APICALLLOGURL."/distributor-route-incremental-6.0.3.php?nick_name=$nick_name&emp_code=$emp_code&last_update_time=$last_update_time&data_download_time=$data_download_time&incremental_download=$incremental_download";
