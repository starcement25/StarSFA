...
$count=mysqli_num_rows($result);
	...
	if($count>0){
		...
		while($rowloyalty = mysqli_fetch_assoc($result))
		{
				...
				$contents  .= (($rowloyalty['vehicle_no']!='')?$rowloyalty['vehicle_no']: ' ')."^";
				$contents  .= (($rowloyalty['expiry_date']!='')?$rowloyalty['expiry_date']: ' ');
	...
	$url = "http://www.acedns.in/acednsproduct/loyalty-card-holder-txt-5.0.7.php?nick_name=$nick_name&emp_code=$emp_code&last_update_time=$last_update_time&data_download_time=$data_download_time&incremental_download=$incremental_download";
	...
	$date = date("F j, Y");
	$time = date("H:i:s");
	$newuser ="[$date $time]"."http://www.acedns.in/acednsproduct/loyalty-card-holder-txt-5.0.7.php?nick_name=$nick_name&emp_code=$emp_code&last_update_time=$last_update_time&data_download_time=$data_download_time&incremental_download=$incremental_download"."\r\n";
