				...
				$mrp=$recmrpdetails['mrp'];
					...
					if(!in_array($orderdatadetails_order_no,$order_array_trans_id))
					{
						...
						if(mysqli_query($link,$sqlinsertorderdetails))
						{
							...
							if(${orderdataheader_transaction_type.$orderdatadetails_order_no}=='BT' || ${orderdataheader_transaction_type.$orderdatadetails_order_no}=='ST' || ${orderdataheader_transaction_type.$orderdatadetails_order_no}=='SA')
							{
								...
								$sqlinsertgoodsintransit="INSERT INTO goods_in_transit SET grn_no='".$grn_no_goods_in_transit."',
														...
														sale_rate		='".$orderdatadetails_sale_rate."',
														status			='0',
														transaction_type ='".${orderdata_header_transaction_type.$orderdatadetails_order_no}."',
														download_time=CURRENT_TIMESTAMP(),
						...
						if(mrp=='yes' && TD=='yes' && TD_type=='sku wise'){
						 ...
						 if(${orderdata_header_transaction_type.$orderdatadetails_order_no}=='BT')
						 {
...
$url = "http://www.acedns.in/acednsproduct/operationdb-transaction-4.1.0.php?nick_name=$nick_name&emp_code=$emp_code&last_update_time=$last_update_time&last_git_master_update_time=$last_git_master_update_time";
insertapilog($datetime,$emp_code,$url,$nick_name);
	/*$config = 'api_calllog.txt';
	$file=fopen($config,"r+");
	$date = date("F j, Y");
	$time = date("H:i:s");
	$newuser ="[$date $time]"."http://www.acedns.in/acednsproduct/operationdb-transaction-4.1.0.php?nick_name=$nick_name&emp_code=$emp_code&last_update_time=$last_update_time&last_git_master_update_time=$last_git_master_update_time"."\r\n";
