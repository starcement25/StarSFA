...
$hint_remarks="*ROOT*NOTES_INFO*NOTES_INFO_DATA*HINT_REMARKS";
$uploaded_photo="*ROOT*NOTES_INFO*NOTES_INFO_DATA*UPLOADED_PHOTO";
...
class xml_notes_info{
	var $location_emp_code,$location_trans_id,$location_latt,$location_longi,$location_date,$notes_info_id,$feedback,$hint_remarks,$uploaded_photo;	
}
...
function contents($parser, $data){
	...
	$notes_info_array,$hint_remarks,$uploaded_photo;
	//echo $current_tag.'<br />';
	//echo $data.'<br />';
	if(substr($current_tag,0,16)=='*ROOT*NOTES_INFO')
	{
		...
		switch($current_tag){
			...
			case $feedback:
				...
				break;
			case $hint_remarks:
				$notes_info_array[$counter]->hint_remarks = $data;
				break;
			case $uploaded_photo:
				$notes_info_array[$counter]->uploaded_photo = $data;
				$counter++;
				break;		
		}
	}
...
if(count($notes_info_array)>0)
{
		for($x=0;$x<count($notes_info_array);$x++){
			...
			$hint_remarks=$notes_info_array[$x]->hint_remarks;
			$uploaded_photo=$notes_info_array[$x]->uploaded_photo;
		  ...
		  if(mysqli_query($link,$sqlinsertorlocation) && mysqli_query($link,$sqlinsertnotesinfo))
			{
				$flag=5;
			}
			else
