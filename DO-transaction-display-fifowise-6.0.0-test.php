...
foreach ($product_details_parts as $product_details_parts_val)
{
    ...
    $sqlquery="SELECT sauda_no,qty,sale_rate,freight_charge,sku_code FROM DO_master WHERE status='no' 
	...
	$Do_qty_distribute=0;
		...
		while($rowDOfifo = mysqli_fetch_assoc($result))
		{
			...
			$TD=$rowTD['TD'];
			if($TD=='') $TD=0;
			$bargain_rate=$bargain_rate+$freight_charge+$TD;
			
			if($product_details_sku_code!=$mapped_product_internal_code)
			{
				 $sqlquerychildrate="SELECT sale_rate,freight_charge FROM DO_master WHERE status='no' 
				...
				$sale_rate_child=$rowchildrate['sale_rate']+$rowchildrate['freight_charge'];
