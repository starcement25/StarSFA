...
if(sale=='yes')
{
	$sqlquerystockrefresh="SELECT sl_no,stock_transferred_by FROM stock_refresh_log 
	...
	while($rowstockrefresh=mysqli_fetch_assoc($resultstockrefresh))
	{
		$stock_transferred_by_value=$stock_transferred_by_value.$rowstockrefresh['stock_transferred_by'].',';
	}
...
function contents($parser, $data){
	...
	$orderdataheader_d_instruction,$orderdataheader_tag_distributor_code,$orderdataheader_cust_type,$orderdataheader_transaction_type,$orderdataheader_grn_no,$orderdataheader_customer_flag,$orderdatadetails_order_no,$orderdatadetails_sku_code,$orderdatadetails_qty,$orderdatadetails_TD,$orderdatadetails_sale_rate,$orderdatadetails_VAT,$orderdatadetails_mrp_code,
					...
					if(!in_array($orderdatadetails_order_no,$order_array_trans_id))
					{
						...
						if(mysqli_query($link,$sqlinsertorderdetails))
						{
							...
							if($orderdataheader_transaction_type=='BT' || $orderdataheader_transaction_type=='ST')
							{
								...
								if($orderdataheader_transaction_type=='BT')
								{
									$grn_no_goods_in_transit=$orderdataheader_grn_no;
...
$url = "http://www.acedns.in/acednsproduct/operationdb-transaction-4.0.7.php?nick_name=$nick_name&emp_code=$emp_code&last_update_time=$last_update_time&last_git_master_update_time=$last_git_master_update_time";
insertapilog($datetime,$emp_code,$url,$nick_name);
	/*$config = 'api_calllog.txt';
	$file=fopen($config,"r+");
	$date = date("F j, Y");
	$time = date("H:i:s");
	$newuser ="[$date $time]"."http://www.acedns.in/acednsproduct/operationdb-transaction-4.0.7.php?nick_name=$nick_name&emp_code=$emp_code&last_update_time=$last_update_time&last_git_master_update_time=$last_git_master_update_time"."\r\n";
