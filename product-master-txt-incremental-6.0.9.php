...
$count=mysqli_num_rows($result);
	...
	if($count>0){
		...
		while($rowproduct = mysqli_fetch_assoc($result))
		{
			if(state_branch_wise_TD=='yes')
			{
				...
				$cntselTD=mysqli_num_rows($rsselTD);
	...
	$url = "http://salesmpower.acedns.in/product-master-txt-incremental-6.0.9.php?nick_name=$nick_name&emp_code=$emp_code&last_update_time=$last_update_time&data_download_time=$data_download_time&incremental_download=$incremental_download";
