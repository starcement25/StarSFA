<?php
	...
	if($cntlastwindowtime >0)
	{
		...
		AND UNIX_TIMESTAMP(NOW()) > UNIX_TIMESTAMP('".$timeto_lastwindow."' + INTERVAL 60 MINUTE)";
			
		$sqlquery="SELECT DISTINCT RAD.bid_id,RAD.customer_code,RAD.qty,RAD.bid_rate,PM.prod_desc,CM.phone_no FROM RA_bid_rate_details RAD,
				customer_master CM,product_master PM WHERE  RAD.bid_status='REJECT' AND RAD.prod_code=PM.dns_prod_code 
				...
				//$customer_phone_no='9874450813';
				  /*if(strlen($smsstringfinal)> 160)
					...
					$sqlupdate="UPDATE RA_bid_rate_details SET sms_done='1' WHERE customer_code='".$customercodeval."' AND 
							DATE_FORMAT(SUBSTRING(bid_id,-14,8),'%Y%-%m-%d')='".$currentdate."' AND sms_done='0' AND bid_status='REJECT'";
