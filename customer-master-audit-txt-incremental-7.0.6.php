...
if($functionality=='DOS'){
	...
	if(modified_customer_emp_route=='yes')
	{
			...
			$sqlcustomer="SELECT  DISTINCT c1.customer_code,c1.route_code,c1.acedns,c1.emp_code,CM.customer_name,CM.emp_code,CM.current_balance,
							...
							CM.sauda_limit,CM.incoterms,CM.pending_qty,CM.loadability_ton,CM.transport_mode,CM.state_code FROM customer_master CM,
			...
			if($rowcustomer['customer_name']!=''){
					...
					$contentsrowcolumn=$countcustomer.'¥'.'34';
...
if($nick_name=='EMAMI' || $nick_name=='EMAMIT'){
	...
	{
		$customer_type_condition=" AND c1.cust_type='D' AND c1.sauda_type!='RA'";
	}
	else
	{
		//$customer_type_condition=" AND c1.cust_type IN('R','D')";
		$customer_type_condition=" AND c1.sauda_type!='RA'";
	}
}
else
{
...
$countbranches=mysqli_num_rows($rsbranches);
	...
	if(modified_customer_emp_route=='yes')
	{
		...
		if($countcustomerroute>0){
				...
				if(!in_array($customer_code,$customebr_code_array))
				{
					$sqlcustomer="SELECT  customer_name,emp_code,current_balance,credit_limit,black_list,acedns,
							TD,cust_type,rds_tag,sauda_validity_period,address,phone_no,pin,landline_no,owner_name,owner_phone,
							 ...
							 sauda_limit,incoterms,pending_qty,loadability_ton,transport_mode,state_code FROM customer_master WHERE customer_code='".$customer_code."'";
					$rscustomer=mysqli_query($link,$sqlcustomer);
					...
					if(!in_array($rowcustomer['rds_tag'],$customebr_code_array))
					{
						$sqlrdswisecustomer="SELECT  customer_name,route_code,current_balance,credit_limit,black_list,
											...
											transport_mode,state_code FROM customer_master WHERE customer_code='".$rowcustomer['rds_tag']."'";
			...
			$contentsrowcolumn=$customer_count.'¥'.'34';
			$datacontents = $contentsrowcolumn."\n".$contentsdatetime.str_replace("\r","",$linecontents);
		}
		else
		{
		 ...
		 else
		 {
			 ...
			 $sqlquery="SELECT DISTINCT c1.customer_code,c1.dns_customer_code, c1.customer_name, c1.route_code,c1.emp_code,c1.current_balance,c1.credit_limit,c1.black_list,c1.acedns,
						c1.TD,c1.cust_type,c1.rds_tag,c1.sauda_validity_period,c1.address,c1.phone_no,c1.pin,c1.landline_no,c1.owner_name,c1.owner_phone,
						 ...
						 c1.incoterms,c1.pending_qty,c1.loadability_ton,c1.transport_mode,c1.state_code FROM customer_master c1 WHERE ".$emp_hierarchy_condition." ".$login_condition." ".$customer_type_condition." 
						 ORDER BY c1.customer_name ASC";			
			 }
			 ...
			 else
			 {
				$sqlquery="SELECT DISTINCT c1.customer_code,c1.dns_customer_code,c1.customer_name, c1.route_code,c1.emp_code,c1.current_balance,c1.credit_limit,c1.black_list,c1.acedns,
						   c1.TD,c1.cust_type,c1.rds_tag,c1.sauda_validity_period,c1.address,c1.phone_no,c1.pin,c1.landline_no,c1.owner_name,c1.owner_phone,
						 c1.cust_class,c1.weekly_closing_day,c1.coverage_type,c1.TIN,c1.PAN,c1.minimum_stock,c1.branch_code,c1.visit_day,c1.email,c1.sauda_limit,c1.incoterms,
						 c1.pending_qty,c1.loadability_ton,c1.transport_mode,c1.state_code FROM customer_master c1 
...
$count=mysqli_num_rows($result);
	...
	$url = APICALLLOGURL."/customer-master-audit-txt-incremental-7.0.6.php?nick_name=$nick_name&emp_code=$emp_code&last_update_time=$last_update_time&data_download_time=$data_download_time&incremental_download=$incremental_download";
