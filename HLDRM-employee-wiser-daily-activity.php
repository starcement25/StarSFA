...
//For SO
$sqlselbaselevel="SELECT emp_code,emp_name,email,designation FROM employee_master WHERE level='3'";
...
while($rowbaselevel=mysqli_fetch_assoc($rsselbaselevel))
{
	...
	$sql_get_empdetails ="SELECT emp_code,emp_name FROM employee_master WHERE ".${emp_hierarchy_condition.$base_emp_code}." AND acedns='Y' 
						AND level='2' AND emp_code <>'".$base_emp_code."' ORDER BY emp_name ASC";					
	...
	while($row_get_empdetails = mysqli_fetch_assoc($res_get_empdetails)){
		...
		${employee_hierarchy.$emp_code_level1}=return_employee_hierarchy($emp_code_level1);
		...
		if($total_order_count_level1>0){
			...
			$sql_total_primary_secondary = "SELECT COUNT(DISTINCT order_no) AS order_header_count, COUNT(order_no) AS order_details_count,
											SUM(amount) AS amount FROM order_details WHERE SUBSTRING(order_no,2,5) = '".$emp_code_level1."' 
		...
		${table.$base_emp_code}.="<tr>
					...
					<td align='left'>".(count(explode(',',${employee_hierarchy.$emp_code_level1}))-1)."</td>
		...
		$count++;
	${total_employee_hierarchy.$base_emp_code} +=(count(explode(',',${employee_hierarchy.$emp_code_level1}))-1);		  

	}
	${table.$base_emp_code}.="<tr style='font-weight:bold;background:yellow;'>
			<td align='center' colspan='2'>Total</td>
			<td align='right'>".${total_employee_hierarchy.$base_emp_code}."</td>
