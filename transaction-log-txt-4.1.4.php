...
$contentsrowcolumn=$count.'¥'.'14';
	if($count>0){
		...
		while($rowtransactionlog = mysqli_fetch_assoc($result))
		{
			$branch_name='';
			$rds_name='';
	...
	$url = "http://www.acedns.in/acednsproduct/transaction-log-txt-4.1.4.php?nick_name=$nick_name&emp_code=$emp_code&last_update_time=$last_update_time&data_download_time=$data_download_time&incremental_download=$incremental_download";
	insertapilog($datetime,$emp_code,$url,$nick_name);
	/*$config = 'api_calllog.txt';
	$file=fopen($config,"r+");
	$date = date("F j, Y");
	$time = date("H:i:s");
	$newuser ="[$date $time]"."http://www.acedns.in/acednsproduct/transaction-log-txt-4.1.4.php?nick_name=$nick_name&emp_code=$emp_code&last_update_time=$last_update_time&data_download_time=$data_download_time&incremental_download=$incremental_download"."\r\n";
