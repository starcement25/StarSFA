...
$sqllocation = "SELECT emp_code,trans_id,date FROM location WHERE 
			(trans_id LIKE 'A%' OR trans_id LIKE 'CH%' OR trans_id LIKE 'MFE%' OR trans_id LIKE 'WO%' OR trans_id LIKE 'LR%') ".$emp_val_rds.$login_condition;
...
$contentsdatetime =$year.'-'.$month.'-'.$date.'€'.$hour.':'.$minute.':'.$second."\n";
	...
	$url = "https://sfa.starcement.co.in/attendance-checkout-txt-incremental-6.0.0.php?nick_name=$nick_name&emp_code=$emp_code&last_update_time=$last_update_time&incremental_download=$incremental_download";
