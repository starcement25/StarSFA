...
$finalzipfile="ICD-db-backup-".$date.$month.$year.$hour.$minute.$second.'.zip';
...
$linkbroker = mysqli_connect('localhost','acedns_dnsprod','dnsprod1234#'); 
mysqli_select_db("acedns_brokeraccount",$linkbroker) or die("could not connect the database");
		...
		//save file 
		${filename.$nick_name} = "ICD-db-backup-".$date.$month.$year.$hour.$minute.$second.'.sql'; 
	...
	$bodytext='ICD backup is attached below';
	$val='skguchait@kumarbrothers.co.in,icdrppl@gmail.com,dipankarc@coral.in,';
	$valpart='DIP';
	$subject='ICD backup'.$date.'-'.$month.'-'.$year.' '.$hour.':'.$minute.':'.$second;
	$mailer = new Swift_Mailer(new Swift_MailTransport());
	$message = Swift_Message::newInstance()
				 ->setSubject($subject) // Message subject
						   ->setTo(array('skguchait@kumarbrothers.co.in' => 'SK','icdrppl@gmail.com' => 'IC','dipankarc@coral.in' => 'DSC')) // Array of people to send to
						   //->setBcc(array('dipankarc@coral.in' => 'DSC'))
						   ->setFrom(array('info@salesmpower.acedns.in' => 'salempower')) // From:
						   ->setBody($bodytext, 'text/html') // Attach that HTML message from earlier
						   ->attach(Swift_Attachment::newInstance($zip->file(), "ICD-backup.zip", 'application/zip')); // Attach the generated PDF from earlier
