...
$body="<?xml version='1.0' encoding='UTF-8'?><root><order><location><emp_code><![CDATA[C0007]]></emp_code><trans_id><![CDATA[OC000720140822161904]]></trans_id><latt><![CDATA[0.0]]></latt><longi><![CDATA[0.0]]></longi><date><![CDATA[2014-08-22 16:19:04]]></date></location><orderdata><order_header><order_no><![CDATA[OC000720140822161904]]></order_no><customer_name><![CDATA[New Cust]]></customer_name><Phone_no><![CDATA[2588744557]]></Phone_no><address><![CDATA[]]></address><pin_code><![CDATA[258842]]></pin_code><area><![CDATA[NRT/C000720140822161817]]></area><area_name><![CDATA[new route]]></area_name><TD><![CDATA[0]]></TD><sale_type><![CDATA[COD]]></sale_type><order_value><![CDATA[0.00]]></order_value><d_instruction><![CDATA[]]></d_instruction><tag_distributor_code><![CDATA[C/0000401]]></tag_distributor_code><cust_type><![CDATA[D]]></cust_type><customer_flag><![CDATA[N]]></customer_flag><customer_code><![CDATA[NC000720140822161854]]></customer_code></order_header><order_details><Order_no><![CDATA[OC000720140822161904]]></Order_no><Sku_code><![CDATA[12010]]></Sku_code><qty><![CDATA[25.0]]></qty><TD><![CDATA[0]]></TD><sale_rate><![CDATA[]]></sale_rate><mrp_code><![CDATA[m010]]></mrp_code></order_details></orderdata></order></root>";*/
...
class xml_order{
	...
	$orderdataheader_phone_no,$orderdataheader_address,$orderdataheader_pin_code,$orderdataheader_area,$orderdataheader_area_name,$orderdataheader_TD,$orderdataheader_sale_type,$orderdataheader_d_instruction,$orderdataheader_tag_distributor_code,$orderdataheader_cust_type,$orderdataheader_transaction_type,$orderdataheader_customer_flag;	
}
class xml_order_details{
	var $orderdatadetails_order_no,$orderdatadetails_sku_code,$orderdatadetails_qty,$orderdatadetails_TD,$orderdatadetails_sale_rate,$orderdatadetails_VAT,
	$orderdatadetails_mrp_code;
}
class xml_payment{
...
function contents($parser, $data){
	...
	$orderdataheader_d_instruction,$orderdataheader_tag_distributor_code,$orderdataheader_cust_type,$orderdataheader_transaction_type,$orderdataheader_customer_flag,$orderdatadetails_order_no,$orderdatadetails_sku_code,$orderdatadetails_qty,$orderdatadetails_TD,$orderdatadetails_sale_rate,$orderdatadetails_VAT,$orderdatadetails_mrp_code,
			...
			if(mysqli_query($link,$sqlinsertattlocation) && mysqli_query($link,$sqlinsertattendance))
				{
					...
					if(mail(ATTENDANCEEMAILRECIPENTS, $attendanceemailsubj, $attendancemailbody, $headers,'-facedns@acedns.in'))
					{
						$flag=5;
					}
					else
		...
		else
		{
			...
			//Add new rds to rds master
				...
				if($orderdataheader_transaction_type=='PB')
					...
					{ 
						if($orderdataheader_transaction_type=='BT') $transaction_type_details='RDS Transfer';
						if($orderdataheader_transaction_type=='ST') $transaction_type_details='Stock Transfer to RDS';
						...
						if($orderdataheader_transaction_type=='BT' || $orderdataheader_transaction_type=='ST')
						{
							...
							$orderheader_TR="<th style='width:260px;min-height:21px;text-align:left'><strong><span style='font-size:10pt;font-family:Arial CE'>RDS Name</span></strong></th>";
					...
					if($orderdataheader_transaction_type=='SA')
					{ 
				...
				if(substr($order_trans_id,0,1)=='N')
				{
					...
					$headers .= "From: ".FROMTAG."<".FROMEMAIL."> \r\n" .
								...
								"Bcc: ".BCCEMAIL." \r\n".
								'X-Mailer: PHP/' . phpversion();
					if(mail(ORDEREMAILRECIPENTS, $noorderemailsubj, $noorderemailbody, $headers,'-facedns@acedns.in'))
					{
						$flag=5;
					}
					else
	...
	//print_r($order_details_array);
				...
				$mrp=$recmrpdetails['mrp'];
					...
					if(!in_array($orderdatadetails_order_no,$order_array_trans_id))
					{
						...
						if(mrp=='yes' && TD=='yes' && TD_type=='sku wise'){
						 ...
						 if(VAT=='yes')
						 {
							$orderemailbody_VAT_VAL="<td style='width:50px;text-align:right;min-height:21px;background-color:white'>
								<span style='font-family:Arial CE;font-size:10pt'>".number_format($orderdatadetails_VAT,2)."</span>&nbsp;</td>";
						 }
						 else
						 {
						...
						${a.$orderdatadetails_order_no} .="
								...
								<td style='width:50px;text-align:right;min-height:21px;background-color:white'>
								<span style='font-family:Arial CE;font-size:10pt'>".number_format($orderdatadetails_sale_rate,2)."</span>&nbsp;</td>".$orderemailbody_TD_VAL.$orderemailbody_VAT_VAL."
								<td style='width:50px;text-align:right;min-height:21px;background-color:white'>
								<span style='font-family:Arial CE;font-size:10pt'>".number_format(${totalamount.$orderdatadetails_sku_code},2)."</span>&nbsp;</td>
								</tr>";
					}
			}//End for loop
		}
		...
		if(count($orderheader_array_mailbody)>0)
		{
			for($countarr=0;$countarr<count($orderheader_array_mailbody);$countarr++)
			{
				...
				else if($orderdataheader_transaction_type_array[$countarr]=='SA' && $orderdataheader_sale_type_array[$countarr]==''){
				...
				$orderemailbody = $orderheader_array_mailbody[$countarr]."<br><table border=1 style=background-color:AliceBlue>
							...
							<th style='width:60px;min-height:21px;text-align:center'><strong>
							<span style='font-size:10pt;font-family:Arial CE'>".$sale_rate_TR."</span></strong></th>".$orderemailbody_TD_TH.$orderemailbody_VAT_TH." 	<th style='width:60px;min-height:21px;text-align:center'><strong>
				...
				if(mail($order_email, $orderemailsubj, $orderemailbody, $headers,'-facedns@acedns.in'))
				{
					$flag=5;
				}
				else
			...
			if(mysqli_query($link,$sqlinsertpaylocation) && mysqli_query($link,$sqlinsertpaymentheader))
				...
				if(substr($payment_trans_id,0,1)=='N')
				{
					...
					$headers .= "From: ".FROMTAG."<".FROMEMAIL."> \r\n" .
								...
								"Bcc: ".BCCEMAIL." \r\n" .
								'X-Mailer: PHP/' . phpversion();
					if(mail(PAYMENTEMAILRECIPENTS, $nopaymentemailsubj, $nopaymentemailbody, $headers,'-facedns@acedns.in'))
					{
						$flag=5;
					}
					else
			...
			for($countpayarr=0;$countpayarr<count($paymentheader_array_mailbody);$countpayarr++)
			{
				if($paymentdataheader_sale_type_array[$countpayarr]!='CASH'){
					...
					$headers .= "From: ".FROMTAG."<".FROMEMAIL."> \r\n" .
								...
								"Bcc: ".BCCEMAIL." \r\n".
								'X-Mailer: PHP/' . phpversion();
					if(mail(PAYMENTEMAILRECIPENTS, $paymentemailsubj, $paymentemailbody, $headers,'-facedns@acedns.in'))
					{
						$flag=5;
					}
					else
...
$url = "http://www.coralindia.com/dev/acednsproduct/operationdb-transaction-4.0.php?nick_name=$nick_name&emp_code=$emp_code&last_update_time=$last_update_time";
insertapilog($datetime,$emp_code,$url,$nick_name);
	/*$config = 'api_calllog.txt';
	$file=fopen($config,"r+");
	$date = date("F j, Y");
	$time = date("H:i:s");
	$newuser ="[$date $time]"."http://www.coralindia.com/dev/acednsproduct/operationdb-transaction-4.0.php?nick_name=$nick_name&emp_code=$emp_code&last_update_time=$last_update_time"."\r\n";
