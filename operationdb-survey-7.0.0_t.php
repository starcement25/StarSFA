...
$datetime = gmdate('Y-m-d H:m:s',strtotime('+330 minute'));
	$url = APICALLLOGURL."/operationdb-survey-7.0.0.php?nick_name=$nick_name&emp_code=$emp_code&last_update_time=$last_update_time&last_git_master_update_time=$last_git_master_update_time&last_loyalty_purchase_update_time=$last_loyalty_purchase_update_time";
...
if(count($survey_array)>0)
{
	for($x=0;$x<count($survey_array);$x++){
		...
		if(is_null($survey_latt)){
				...
				echo "is_null_check-".$survey_latt;
		...
		else
		{
			...
			$sqlmenuname="SELECT display_name FROM  survey_input_lead WHERE row_id='".$surveyheader_menu_name."'";
	...
	if(count($survey_details_array)>0)
	{
		for($i=0;$i<count($survey_details_array);$i++){
			...
			if($nick_name=='STAR' || $nick_name=='START'){
				...
				$survey_input_table='survey_input_lead';
			}
 ...
 if($last_operation_datetime==""){
	...
	$datetime = gmdate('Y-m-d H:m:s',strtotime('+330 minute'));
	$url = APICALLLOGURL."/acednsproduct/operationdb-survey-7.0.0.php?nick_name=$nick_name&emp_code=$emp_code&last_update_time=$last_update_time&last_git_master_update_time=$last_git_master_update_time&last_loyalty_purchase_update_time=$last_loyalty_purchase_update_time";
	insertapilog($datetime,$emp_code,$url,$nick_name);
