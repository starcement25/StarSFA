...
if($countselectuserdbversion>0)
{
	...
	if((($versionCodecurrent-$user_db_version_code)*10) > '1' && $is_update==1 && $incremental_download=='yes')
		...
		if(collection=='yes' && in_array("outstanding_master",$need_download_table_array))
		{
		...
		if((mrp=='yes' || (sale_rate=='yes' && sale_rate_input_dropdown=='dropdown')) && in_array("mrp",$need_download_table_array)) {
			$contents  .= 'mrp_master'."\n";
		}
		...
		$url = "http://www.acedns.in/acednsproduct/datadownloaddictionary-6.0.4.php?nick_name=$nick_name&emp_code=$emp_code&device_id=$device_id&last_update_time=$last_update_time&incremental_download=$incremental_download";
...
if($successval=="1"){
	if($incremental_download=='no'){
		...
		{
			$sqlpendingcontractcnt="SELECT COUNT(PC.branch_code) AS total_pending_contract FROM pending_contract PC,employee_master EM
								 WHERE FIND_IN_SET(PC.branch_code,EM.branch_code) AND EM.emp_code='".$emp_code."'";
	...
	else
	{
		...
		if(pending_contract=='yes')
		{
			if(in_array('pending_contract',$need_update_table_array))
			{
				$contents  .= 'pending_contract'."\n";
			}
			else
			 {
				$sqlpendingcontractcnt="SELECT COUNT(PC.branch_code) AS total_pending_contract FROM pending_contract PC,employee_master EM
									 WHERE FIND_IN_SET(PC.branch_code,EM.branch_code) AND EM.emp_code='".$emp_code."' AND 
									 UNIX_TIMESTAMP(PC.download_time) > UNIX_TIMESTAMP('".$last_update_time."')";
				$rspendingcontractcnt=mysqli_query($link,$sqlpendingcontractcnt);
				$rowpendingcontractcnt=mysqli_fetch_assoc($rspendingcontractcnt);
				$pendingcontractcnt=$rowpendingcontractcnt['total_pending_contract'];
