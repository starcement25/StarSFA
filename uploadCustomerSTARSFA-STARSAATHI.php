...
$linkone=mysqli_connect(SERVER,USER,PASSWORD,TRUE) or die("Database Connection Error.");
mysqli_select_db(DB,$linkone) or die("could not connect the database");

$sqlSFAcustomer="SELECT CM.dns_customer_code,CM.customer_code,CM.customer_name,CM.phone_no,CM.acedns,CM.cust_type,CM.address,CM.whatsapp_no,CM.rds_tag,RM.route_name,
RM.dns_route_code,BM.dns_branch_code FROM customer_master CM,route_master RM,branch_master BM WHERE CM.route_code=RM.route_code AND 
 CM.branch_code=BM.branch_code  ORDER BY dns_customer_code ASC";
$rsSFAcustomer=mysqli_query($link,$sqlSFAcustomer,$linkone);
while($rowSFAcustomer=mysqli_fetch_assoc($rsSFAcustomer))
{
			$customer_code=$rowSFAcustomer['customer_code'];
			$sqlchkcustomer="SELECT customer_code FROM customer_route_emp_relation WHERE acedns='Y' AND customer_code='".$customer_code."'";
			...
			$dns_customer_code=$rowSFAcustomer['dns_customer_code'];
			$customer_name=$rowSFAcustomer['customer_name'];
			$branch_code_name=$rowSFAcustomer['dns_branch_code'];
			$phone_no=$rowSFAcustomer['phone_no'];
			$dns_route_code=$rowSFAcustomer['dns_route_code'];
			$route_name=$rowSFAcustomer['route_name'];
			
			$customer_type=$rowSFAcustomer['cust_type'];
			$address=$rowSFAcustomer['address'];
			$whatsapp_no=$rowSFAcustomer['whatsapp_no'];
			$rds_tag=$rowSFAcustomer['rds_tag'];
			 if(strpos($dns_customer_code,',')===false && $acedns=='N'){
...
echo count($cust_in_final_array);
exit();		
	...
	foreach($cust_in_final_array as $array_value) {
	...
	foreach($dns_customer_code_array as $dns_customer_code_val){
			...
			else
			{
				...
				|| $address_db!=$address || $whatsapp_no_db!=$whatsapp_no || $rds_tag_db!=$rds_code || $acedns_db!=$acedns)
				{
					...
					$sqlinsert="INSERT INTO customer_master_temp SET customer_code='".$dns_customer_code."'";
