...
$contentsrowcolumn=$count.'¥'.'16';
	if($count>0){
		...
		while($rowmislog = mysqli_fetch_assoc($result))
		{
	...
	$url = "http://www.acedns.in/acednsproduct/mis-transaction-log-txt-5.0.3.php?nick_name=$nick_name&emp_code=$emp_code_value&last_update_time=$last_update_time&data_download_time=$data_download_time&incremental_download=$incremental_download";
	insertapilog($datetime,$emp_code,$url,$nick_name);
	/*$config = 'api_calllog.txt';
	$file=fopen($config,"r+");
	$date = date("F j, Y");
	$time = date("H:i:s");
	$newuser ="[$date $time]"."http://www.acedns.in/acednsproduct/mis-transaction-log-txt-5.0.3.php?nick_name=$nick_name&emp_code=$emp_code_value&last_update_time=$last_update_time&data_download_time=$data_download_time&incremental_download=$incremental_download"."\r\n";
