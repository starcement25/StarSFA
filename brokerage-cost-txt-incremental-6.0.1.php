...
if(strtoupper(substr($emp_code,0,1))=='B')
{
	$sqlquery="SELECT BC.broker_id,BC.oil_category,BC.pack_size,BC.UOM,BC.brokerage_cost,BC.acedns FROM 
			brokerage_cost BC
			WHERE BC.broker_id='".$emp_code."' ".$login_condition.""; 
}
if(strtoupper(substr($emp_code,0,1))=='C')
{
	$sqlquery="SELECT BC.broker_id,BC.oil_category,BC.pack_size,BC.UOM,BC.brokerage_cost,BC.acedns FROM 
			brokerage_cost BC,customer_broker_relation CBR 
			WHERE  CBR.broker_code=BC.broker_id 
			 AND CBR.acedns!='N' AND CBR.customer_code='".$emp_code."' ".$login_condition.""; 
}
...
$count=mysqli_num_rows($result);
	...
	$url = "http://www.acedns.in/acednsproduct/brokerage-cost-txt-incremental-6.0.1.php?nick_name=$nick_name&emp_code=$emp_code&last_update_time=$last_update_time&data_download_time=$data_download_time&incremental_download=$incremental_download";
