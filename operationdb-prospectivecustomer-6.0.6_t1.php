...
/*$body="<?xml version='1.0' encoding='UTF-8'?><root><prospective_customer><location><emp_code><![CDATA[E0005]]></emp_code><trans_id><![CDATA[DCE000520171226183042]]></trans_id><latt><![CDATA[22.5644413]]></latt><longi><![CDATA[88.3567865]]></longi><date><![CDATA[2017-12-26 18:30:42]]></date></location><prospective_customer_header><trans_id><![CDATA[DCE000520171226183042]]></trans_id><customer_code><![CDATA[DCE000520171226183042]]></customer_code><address><![CDATA[BANAMALIPUR]]></address><pin><![CDATA[700125]]></pin><area><![CDATA[RT/13]]></area><phone_no><![CDATA[9836389441]]></phone_no><remarks><![CDATA[test new]]></remarks><tagged_customer_code><![CDATA[C/0000037]]></tagged_customer_code><cust_type><![CDATA[R]]></cust_type><category_of_store><![CDATA[CARDIO]]></category_of_store><customer_name><![CDATA[Amit Pqul]]></customer_name></prospective_customer_header><prospective_customer_details><trans_id><![CDATA[DCE000520171226183042]]></trans_id><product_code><![CDATA[30845]]></product_code></prospective_customer_details></prospective_customer></root>";*/
...
$longi = "*ROOT*PROSPECTIVE_CUSTOMER*LOCATION*LONGI";
$TA_DA_mode="*ROOT*PROSPECTIVE_CUSTOMER*LOCATION*TA_DA_MODE";
...
$category_of_store="*ROOT*PROSPECTIVE_CUSTOMER*PROSPECTIVE_CUSTOMER_HEADER*CATEGORY_OF_STORE";
$check_in_time="*ROOT*PROSPECTIVE_CUSTOMER*PROSPECTIVE_CUSTOMER_HEADER*CHECK_IN_TIME";
...
class xml_customer_header{
    var $emp_code, $trans_id,$prospect_date,$latt,$longi,$TA_DA_mode,$prospectiveheader_trans_id,$prospectiveheader_customer_code,$customer_name,$address,$pin,$area,$phone_no,$remarks,$tagged_customer_code,$cust_type,$category_of_store,$check_in_time;
}
...
function contents($parser, $data){
    global $current_tag, $emp_code,$trans_id,$prospect_date,$latt,$longi,$TA_DA_mode,$prospectiveheader_trans_id,$prospectiveheader_customer_code,$customer_name,$address,$pin,$area,$phone_no,$remarks,$tagged_customer_code,$cust_type,$category_of_store,$check_in_time,$counter, $counterdetails,$prospectivedetails_trans_id,$prospectivedetails_product_code,$prospective_customer_array,$prospective_customer_details_array;
	//echo $current_tag.'<br />';
	//echo $data;
	if(substr($current_tag,0,26)=='*ROOT*PROSPECTIVE_CUSTOMER')
	{
		switch($current_tag){
			...
			case $longi:
				...
				break;
			case $TA_DA_mode:
				$prospective_customer_array[$counter]->TA_DA_mode = $data;
				break;	
			...
			case $category_of_store:
				$prospective_customer_array[$counter]->category_of_store = $data;
				break;
			case $check_in_time:
				$prospective_customer_array[$counter]->check_in_time = $data;
				break;	
			case $customer_name:
...
if(count($prospective_customer_array)>0)
{
	...
	for($x=0;$x<count($prospective_customer_array);$x++){
		...
		$TA_DA_mode=$prospective_customer_array[$x]->TA_DA_mode;
		...
		$category_of_store=$prospective_customer_array[$x]->category_of_store;
		$check_in_time=$prospective_customer_array[$x]->check_in_time;
		...
		else
		{
			...
			$sqlinsertprolocation="INSERT INTO location SET emp_code='".$emp_code."',
									...
									latt='".$latt."',
									longi='".$longi."',
									date='".$prospect_date."',".$TA_DA_condiion."
			...
			echo $sqlinsertproheader;
			if(mysqli_query($link,$sqlinsertprolocation) && mysqli_query($link,$sqlinsertproheader))
				{
				   $flag=5;
				   $sqlinsertprospectcustomer="INSERT INTO prospective_customer_master SET 
				...
				$sqlroutename="SELECT route_name FROM route_master WHERE route_code='".addslashes($area)."'";
				echo $sqlroutename;
