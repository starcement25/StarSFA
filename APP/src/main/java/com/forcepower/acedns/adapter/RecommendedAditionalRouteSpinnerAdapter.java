...
public class RecommendedAditionalRouteSpinnerAdapter extends BaseAdapter
{
    private Activity activity;
    private ArrayList<RoutePlanMasterDetails> participentListItemGlobal;

    public RecommendedAditionalRouteSpinnerAdapter(Activity activity, ArrayList<RoutePlanMasterDetails> participentListItemGlobal)
    {
    ...
    public View getView(int position, View convertView, ViewGroup parent)
    {
        ...
        String routeName = participentListItemGlobal.get(position).getRouteName();
        name.setText(routeName);
        if(routeName.equalsIgnoreCase("all")){
            name.setTextColor(Color.parseColor("#ffff8800"));
        }
        badge_tv.setText(participentListItemGlobal.get(position).getcustomerCount());
