...
public class ReportDetailsSalesAdapter extends ArrayAdapter<ReportDetails> {
    ...
    public ReportDetailsSalesAdapter(Context context, int resourceId, ArrayList<ReportDetails> nameValues) {
    ...
    public ReportDetailsSalesAdapter(Context context, int resourceId, ArrayList<ReportDetails> nameValues, boolean showDate) {
        super(context, resourceId, nameValues);
        this.context = context;
        this.nameValues = nameValues;
        this.resourceId = resourceId;
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (nameValues.get(position).getQty().equalsIgnoreCase("")
            ...
            if (!showDate) {
                ...
                String mainStr = defaultFormat.format(Double.parseDouble(nameValues.get(position).getAmount()));
                ...
                //String[] date = nameValues.get(position).getTD().split(" ");
                viewHolder.txtName.setText(nameValues.get(position).getProdCode() + "\n" + nameValues.get(position).getTD());
                viewHolder.txtAmt.setText(defaultFormat1.format(Double.parseDouble(nameValues.get(position).getQty())));
                String mainStr = defaultFormat.format(Double.parseDouble(nameValues.get(position).getAmount()));
                viewHolder.txtPayMode.setText("" + mainStr);
                viewHolder.col1.setVisibility(View.VISIBLE);
                viewHolder.col2.setVisibility(View.VISIBLE);
