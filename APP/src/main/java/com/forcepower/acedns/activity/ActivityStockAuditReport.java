...
public class ActivityStockAuditReport extends FragmentActivity implements OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mContext = ActivityStockAuditReport.this;
        ...
        mReportHandler = new Handler() {
            public void handleMessage(@NonNull Message msg) {
                ...
                ActivityStockAuditReport.this.runOnUiThread(() -> {
                    switch (job) {
                        case 1:
                            orderReportAdapter = new OrderReportAdapter(ActivityStockAuditReport.this, R.layout.sku_child, mOrderReportDetailsList, true);
                            mListViewList.setAdapter(orderReportAdapter);
                            TotalCalculation(mOrderReportDetailsList);
                            if(!Utils.isNumeric(mTotalQty)) {
                                mTotalQty="0.00";
                            }
    ...
    public String BuildQuery(int select) {
         return switch (select) {
             case 1 -> "substr(SA.transaction_id,-14,8)='" + mToday + "' ";
             case 2 -> "substr(SA.transaction_id,-14,4)='" + mCurrentYear + "' AND substr(SA.transaction_id,-10,2)='" + mCurrentMonth + "' ";
             case 3 -> "substr(SA.transaction_id,-14,8) BETWEEN '" + mStartDate + "' AND '" + mEndDate + "' ";
             default -> "";
         };
    }
                ...
                switch (whattodo) {
                    case 1:
                        if(Constants.nickName.equalsIgnoreCase("abdost") || Constants.nickName.equalsIgnoreCase("abdos")){
                            mOrderReportDetailsList = mAceDnsDatabaseHelper.GetStockCustomerDataAbdos(mQuery);
                        } else{
                            mOrderReportDetailsList = mAceDnsDatabaseHelper.GetStockCustomerData(mQuery);
                        }
                        break;
                    case 2:
                        if(Constants.nickName.equalsIgnoreCase("abdost") || Constants.nickName.equalsIgnoreCase("abdos")){
                            mOrderReportProductGroupDetailsList = mAceDnsDatabaseHelper.GetStockProductGroupDataStockAuditAbdos(mQuery,currentSelectedCustomerCode);
                        } else{
                            mOrderReportProductGroupDetailsList = mAceDnsDatabaseHelper.GetStockProductGroupDataStockAudit(mQuery,currentSelectedCustomerCode);
                        }
                        break;
                    case 3:
                        if(Constants.nickName.equalsIgnoreCase("abdost") || Constants.nickName.equalsIgnoreCase("abdos")){
                            mOrderReportProductSubGroupDetailsList = mAceDnsDatabaseHelper.GetStockProductSubGroupDataAbdos(mQuery, mProductGroupCode);
                        } else{
                    ...
                    case 5:
                        if(Constants.nickName.equalsIgnoreCase("abdost") || Constants.nickName.equalsIgnoreCase("abdos")){
                            mOrderReportProductDetailsList = mAceDnsDatabaseHelper.GetStockProductDataAbdos(mQuery, mProductGroupCode, mProductSubGroupCode, mProductBrandCode, mFilterNo, currentSelectedCustomerCode);
                        } else{
                            mOrderReportProductDetailsList = mAceDnsDatabaseHelper.GetStockProductData(mQuery, mProductGroupCode, mProductSubGroupCode, mProductBrandCode, mFilterNo, currentSelectedCustomerCode);
                        }
                        break;
                }
    ...
    public void ShowProductDetailsDialog(ArrayList<OrderReportDetails> orderReportDetailsList) {
        final Dialog mDetailsDialog = new Dialog(ActivityStockAuditReport.this, R.style.PauseDialog);
        mDetailsDialog.requestWindowFeature(Window.FEATURE_NO_TITLE);
        mDetailsDialog.setContentView(R.layout.dialog_order_chart_audit);
        ...
        final OrderReportAdapter orderreportAdapter = new OrderReportAdapter(ActivityStockAuditReport.this, R.layout.sku_child, orderReportDetailsList, true);
    ...
    public void ShowProductSubGroupDetailsDialog(final String titlename, ArrayList<OrderReportDetails> orderReportDetailsList) {
        ...
        if(Constants.weightage.toUpperCase().matches("YES")){
            ...
            FrameLayout layout1 = mDetailsDialog.findViewById(R.id.footerweightcol2);
            layout1.setVisibility(View.VISIBLE);
            textViewWtTotal.setText(mTotalWt);
        }else{
            FrameLayout layout = mDetailsDialog.findViewById(R.id.col4);
            layout.setVisibility(View.GONE);
            FrameLayout layout1 = mDetailsDialog.findViewById(R.id.footerweightcol2);
