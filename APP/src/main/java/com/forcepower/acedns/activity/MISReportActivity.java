...
public class MISReportActivity extends FragmentActivity implements OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mContext = MISReportActivity.this;
        ...
        mHandler = new Handler() {
            public void handleMessage(@NonNull Message msg) {
                ...
                if (aResponse.equalsIgnoreCase("JobDone")) {
                    ...
                    totalTransAmt = 0.00;
                    loader.cancel();
                    MISReportActivity.this.runOnUiThread(() -> {
        ...
        listReport.setOnItemClickListener((arg0, arg1, arg2, arg3) -> {
            if (!reportType.equalsIgnoreCase("STOCK")) {
                if (reportList.get(arg2).getTransId().substring(0, 2).equalsIgnoreCase("NO")) {
                    ...
                    Intent intent = new Intent(MISReportActivity.this, MISReportDetailsActivity.class);
                    ...
                    intent.putExtra("TotalMoney", Double.parseDouble(reportList.get(arg2).getAmount()));
            ...
            public void run() {
                ...
                if (reportType.equalsIgnoreCase("PURCHASE")) {
                    if (reportDuration.equalsIgnoreCase("TODAY")) {
                        dataList = transDataHelperObj.getMISReportList("PURCHASE", dateStr, selectedRDSCode, selectedGroupCode);
                    } else if (reportDuration.equalsIgnoreCase("MTD")) {
                        String timeStamp = Constants.dateString.substring(0, 6);
                        dataList = transDataHelperObj.getMISReportList("PURCHASE", timeStamp, selectedRDSCode, selectedGroupCode);
                    }
                } else if (reportType.equalsIgnoreCase("SALES")) {
                    if (reportDuration.equalsIgnoreCase("TODAY")) {
                        dataList = transDataHelperObj.getMISReportList("SALES", dateStr, selectedRDSCode, selectedGroupCode);
                    } else if (reportDuration.equalsIgnoreCase("MTD")) {
                        String timeStamp = Constants.dateString.substring(0, 6);
                        dataList = transDataHelperObj.getMISReportList("SALES", timeStamp, selectedRDSCode, selectedGroupCode);
                    }
                } else if (reportType.equalsIgnoreCase("STOCK TRANSFER")) {
                    if (reportDuration.equalsIgnoreCase("TODAY")) {
                        dataList = transDataHelperObj.getMISReportList("STOCK TRANSFER", dateStr, selectedRDSCode, selectedGroupCode);
                    } else if (reportDuration.equalsIgnoreCase("MTD")) {
                        String timeStamp = Constants.dateString.substring(0, 6);
                        dataList = transDataHelperObj.getMISReportList("STOCK TRANSFER", timeStamp, selectedRDSCode, selectedGroupCode);
                    }
                } else {
                    dataList = transDataHelperObj.getMISReportList("STOCK", "", selectedRDSCode, selectedGroupCode);
                }
    ...
    public void showCustomReport() {
        new Thread() {
            public void run() {
                if (reportType.equalsIgnoreCase("PURCHASE")) {
                    dataList = transDataHelperObj.getCustomMISReportList("PURCHASE", startDate, endDate, selectedRDSCode, selectedGroupCode);
                } else if (reportType.equalsIgnoreCase("SALES")) {
                    dataList = transDataHelperObj.getCustomMISReportList("SALES", startDate, endDate, selectedRDSCode, selectedGroupCode);
                } else if (reportType.equalsIgnoreCase("STOCK TRANSFER")) {
                    dataList = transDataHelperObj.getCustomMISReportList("STOCK TRANSFER", startDate, endDate, selectedRDSCode, selectedGroupCode);
                } else {
                    dataList = transDataHelperObj.getCustomMISReportList("STOCK", startDate, endDate, selectedRDSCode, selectedGroupCode);
                }
                Message msgObj = mHandler.obtainMessage();
                Bundle b = new Bundle();
                b.putString("message", "JobDone");
