...
public class MISReportGroupwiseActivity extends FragmentActivity implements OnClickListener {
    ...
    TextView col1Txt, col2Txt, col3Txt, startDatetxt, endDateTxt, txtTotal, txtTotalHeader, txtTotalQty;
    ListView listReport;
    Context mContext;
    ...
    @SuppressLint({"HandlerLeak", "SimpleDateFormat", "SetTextI18n"})
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mContext = MISReportGroupwiseActivity.this;
        ...
        mHandler = new Handler() {
            public void handleMessage(@NonNull Message msg) {
                ...
                if (aResponse.equalsIgnoreCase("JobDone")) {
                    ...
                    totalTransAmt = 0.00;
                    loader.cancel();
                    MISReportGroupwiseActivity.this.runOnUiThread(() -> {
        ...
        listReport.setOnItemClickListener((arg0, arg1, arg2, arg3) -> {
            if (!reportType.equalsIgnoreCase("STOCK")) {
                if (reportList.get(arg2).getTransId().substring(0, 2).equalsIgnoreCase("NO")) {
                    ...
                    Intent intent = new Intent(MISReportGroupwiseActivity.this, MISReportGroupRDSwiseActivity.class);
                    ...
                    intent.putExtra("GroupCode", "'" + reportList.get(arg2).getTransId() + "'");
            ...
            public void run() {
                ...
                if (reportType.equalsIgnoreCase("PURCHASE")) {
                    if (reportDuration.equalsIgnoreCase("TODAY")) {
                        dataList = transDataHelperObj.getMISReportListGroupWise("PURCHASE", dateStr, selectedRDSCode, selectedGroupCode);
                    } else if (reportDuration.equalsIgnoreCase("MTD")) {
                        String timeStamp = Constants.dateString.substring(0, 6);
                        dataList = transDataHelperObj.getMISReportListGroupWise("PURCHASE", timeStamp, selectedRDSCode, selectedGroupCode);
                    }
                } else if (reportType.equalsIgnoreCase("SALES")) {
                    if (reportDuration.equalsIgnoreCase("TODAY")) {
                        dataList = transDataHelperObj.getMISReportListGroupWise("SALES", dateStr, selectedRDSCode, selectedGroupCode);
                    } else if (reportDuration.equalsIgnoreCase("MTD")) {
                        String timeStamp = Constants.dateString.substring(0, 6);
                        dataList = transDataHelperObj.getMISReportListGroupWise("SALES", timeStamp, selectedRDSCode, selectedGroupCode);
                    }
                } else if (reportType.equalsIgnoreCase("STOCK TRANSFER")) {
                    if (reportDuration.equalsIgnoreCase("TODAY")) {
                        dataList = transDataHelperObj.getMISReportListGroupWise("STOCK TRANSFER", dateStr, selectedRDSCode, selectedGroupCode);
                    } else if (reportDuration.equalsIgnoreCase("MTD")) {
                        String timeStamp = Constants.dateString.substring(0, 6);
                        dataList = transDataHelperObj.getMISReportListGroupWise("STOCK TRANSFER", timeStamp, selectedRDSCode, selectedGroupCode);
                    }
                } else {
                    dataList = transDataHelperObj.getMISReportListGroupWise("STOCK", "", selectedRDSCode, selectedGroupCode);
                }
    ...
    public void showCustomReport() {
        new Thread() {
            public void run() {
                if (reportType.equalsIgnoreCase("PURCHASE")) {
                    dataList = transDataHelperObj.getCustomMISReportListGroupWise("PURCHASE", startDate, endDate, selectedRDSCode, selectedGroupCode);
                } else if (reportType.equalsIgnoreCase("SALES")) {
                    dataList = transDataHelperObj.getCustomMISReportListGroupWise("SALES", startDate, endDate, selectedRDSCode, selectedGroupCode);
                } else if (reportType.equalsIgnoreCase("STOCK TRANSFER")) {
                    dataList = transDataHelperObj.getCustomMISReportListGroupWise("STOCK TRANSFER", startDate, endDate, selectedRDSCode, selectedGroupCode);
                } else {
                    dataList = transDataHelperObj.getCustomMISReportListGroupWise("STOCK", startDate, endDate, selectedRDSCode, selectedGroupCode);
                }
                Message msgObj = mHandler.obtainMessage();
                Bundle b = new Bundle();
                b.putString("message", "JobDone");
