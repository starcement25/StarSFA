...
public class ReportActivityGrpWise extends FragmentActivity implements OnClickListener {

    ImageView imgLogo, showDurationLayout, hideDurationLayout;
    Button btnBack, btnStartDate, btnEndDate, btnDateDone;
    FrameLayout btnOrder, btnCollection, btnProspect, btnToday, btnMTD, btnCustom, col1Layout, col2Layout, col3Layout;
        ...
        mHandler = new Handler() {
            public void handleMessage(Message msg) {
                ...
                if (aResponse.equalsIgnoreCase("JobDone")) {
                    totalAmt = 0.00;
                    loader.cancel();
                    ReportActivityGrpWise.this.runOnUiThread(new Runnable() {
                        public void run() {
    ...
    public void initView() {
        ...
        adapter = new ReportAdapter(mContext, R.layout.report_list_child, reportList);
            ...
            public void onItemClick(AdapterView<?> arg0, View arg1, int arg2, long arg3) {
                if (reportType.equalsIgnoreCase("ORDER")) {
                    if (reportList.get(arg2).getTransId().substring(0, 2).equalsIgnoreCase("NO")) {
                        String remarks = transDataHelperObj.getReportRemarks("O", reportList.get(arg2).getTransId());
                        //Utils.showToast(mContext, "No Order was received. \n" + remarks);
                        AlertDialog.Builder alert = new AlertDialog.Builder(ReportActivityGrpWise.this);
                        alert.setMessage("No Order was received. \nRemarks : " + remarks).setCancelable(false);
                        ...
                        Intent intent = new Intent(ReportActivityGrpWise.this, ReportActivity.class);
                        intent.putExtra("CustomerCode", reportList.get(arg2).getTransId());
                    ...
                    if (reportList.get(arg2).getTransId().substring(0, 2).equalsIgnoreCase("NC")) {
                        String remarks = transDataHelperObj.getReportRemarks("P", reportList.get(arg2).getTransId());
                        //Utils.showToast(mContext, "No Payment was received.");
                        AlertDialog.Builder alert = new AlertDialog.Builder(ReportActivityGrpWise.this);
                    ...
                    Intent intent = new Intent(ReportActivityGrpWise.this, ReportDetailsActivity.class);
                    intent.putExtra("CustomerName", reportList.get(arg2).getCustomerName());
                    intent.putExtra("ReportType", reportType);
                    intent.putExtra("SearchKey", reportList.get(arg2).getTransId());
                    intent.putExtra("TotalMoney", totalAmt);
                    startActivity(intent);
                }
            }
        });
...
//		 endDateTxt.setText(dateFormat.format(new Date()));
        ...
        btnOrder = (FrameLayout) findViewById(R.id.btn_order);
    ...
    public void changeListViewBG(int selectedLayoutId) {
        switch (selectedLayoutId) {
            ...
            case 2:
                col1Txt.setText("Customer Name");
                col2Txt.setText("Amount (" + currency + ")");
                col3Txt.setText("Chq/C");
                col1Layout.setVisibility(View.VISIBLE);
            ...
            public void run() {
                if (reportType.equalsIgnoreCase("ORDER")) {
                    if (reportDuration.equalsIgnoreCase("TODAY")) {
                        ...
                        dataList = transDataHelperObj.getReportListGrpWise("O", timeStamp);
                    } else if (reportDuration.equalsIgnoreCase("MTD")) {
                    ...
                    if (reportDuration.equalsIgnoreCase("TODAY")) {
                        String timeStamp = Constants.dateString;
                        dataList = transDataHelperObj.getReportListGrpWise("P", timeStamp);
                    } else if (reportDuration.equalsIgnoreCase("MTD")) {
                        String timeStamp = Constants.dateString.substring(0, 6);
                        dataList = transDataHelperObj.getReportListGrpWise("P", timeStamp);
                    }
                    ...
                    if (reportDuration.equalsIgnoreCase("TODAY")) {
                        String timeStamp = Constants.dateString;
                        dataList = transDataHelperObj.getReportListGrpWise("DC", timeStamp);
                    } else if (reportDuration.equalsIgnoreCase("MTD")) {
                        String timeStamp = Constants.dateString.substring(0, 6);
                        dataList = transDataHelperObj.getReportListGrpWise("DC", timeStamp);
                    }
    ...
    public void showCustomReport() {
        new Thread() {
            public void run() {
                if (reportType.equalsIgnoreCase("ORDER")) {
                    dataList = transDataHelperObj.getCustomReportListGrpWise("O", startDate, endDate);
                } else if (reportType.equalsIgnoreCase("COLLECTION")) {
                    dataList = transDataHelperObj.getCustomReportListGrpWise("P", startDate, endDate);
                } else {
                    dataList = transDataHelperObj.getCustomReportListGrpWise("DC", startDate, endDate);
                }
                Message msgObj = mHandler.obtainMessage();
                Bundle b = new Bundle();
                b.putString("message", "JobDone");
