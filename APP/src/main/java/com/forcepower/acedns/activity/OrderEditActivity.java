...
import com.forcepower.acedns.backgroundTask.TRANS_SubmitOrderEditTransactionTask;

import com.forcepower.acedns.R;
import com.forcepower.acedns.adapter.ListAdapterGrn;
import com.forcepower.acedns.adapter.OrderEditAdapter;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        mHandlerPrepareSaudaData = new Handler() {
            public void handleMessage(Message msg) {
                ...
                OrderEditActivity.this.runOnUiThread(new Runnable() {
                    public void run() {
                        switch (jobToDo) {
                            case 1:

                                grnMasterListItemGlobal = mAceDnsDatabase.getOrderListForToday();
//                                grnmasterListItemDefault =new ArrayList<>(grnMasterListItemGlobal);
//                                Utils.writeDebugData("grnMasterListItemGlobal-"+grnMasterListItemGlobal.size(),mContext);
                                ...
                                if(grnMasterListItemGlobal.size() == 1)
                                {
                                    ...
                                    Utils.showToast(mContext,"No order to Edit.");
                                    finish();
                                }
...
//        Utils.writeDebugData("chosenOrderNot-"+chosenOrderNo,mContext);
        ...
        grnMasterSkuListItemGlobal = mAceDnsDatabase.getOrderListForTodayDetails(chosenOrderNo);
//        Utils.writeDebugData("grnMasterSkuListItemGlobal-"+grnMasterSkuListItemGlobal.size(),mContext);
        OrderEditAdapter lAdapter = new OrderEditAdapter((Activity)mContext);
...
//        showRemarksDialog();
    ...
    public void SaveDataToDatabase() {
        ...
        Boolean  isSuccessUpdateOrderDetailsTable ;
            ...
            isSuccessUpdateOrderDetailsTable = dataHelperObj.UpdateOrderDetails(chosenOrderNo);
        if (isSuccessUpdateOrderDetailsTable) {
            new TRANS_SubmitOrderEditTransactionTask(mContext, true,chosenOrderCustomerCode,chosenOrderNo,dataHelperObj).execute();
        } else {
