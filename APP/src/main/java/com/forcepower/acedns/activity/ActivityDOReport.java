...
public class ActivityDOReport extends AppCompatActivity implements OnClickListener {
    ...
    public static TextView textViewBargainOrDo = null;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mContext = ActivityDOReport.this;
        mAceDnsTransactionDatabase = new AceDnsTransactionDatabase(mContext);
        mAceDnsDatabaseHelper = new AceDnsDatabase(mContext);        Toolbar myToolbar = findViewById(R.id.my_toolbar);
        ...
        mReportHandler = new Handler() {
            public void handleMessage(Message msg) {
                ...
                ActivityDOReport.this.runOnUiThread(new Runnable() {
                    public void run() {
                        switch (job) {
                            case 1:
...
//            String key = (String)mapElement.getKey();
            ...
            if (orderReportDetailsList != null && orderReportDetailsList.size() > 0) {
                ...
                for (int count = 0; count < orderReportDetailsList.size(); count++) {
                    if (orderReportDetailsList.get(count).getAmount().trim().length() > 0) {
                        qty += Double.parseDouble(orderReportDetailsList.get(count).getAmount().trim());
                    }
                }
    ...
    public void InitializeView() {
        ...
        textViewBargainOrDo = (TextView) findViewById(R.id.textViewBargainOrDo);
                ...
                switch (whattodo) {
                    case 1:
                        if(HTTPUtils.isConnectionPossible(mContext)){
                            ...
                            new commonAsyncTaskMaster(mContext, "doReportDownload");
                            mOrderReportDetailsList=new ArrayList<>(mOrderReportDetailsListGlobal);
                        }
                        else{
                            mOrderReportDetailsList = mAceDnsDatabaseHelper.GetDOReportData(mQuery);
                        }
                    ...
                    case 2:
                        if(HTTPUtils.isConnectionPossible(mContext)){
                            ...
                            new commonAsyncTaskMaster(mContext, "doReportDownloadLevel2");
...
//                        mOrderReportDetailsListByCustomer = mAceDnsDatabaseHelper.GetDOReportDataByCustomer(mQuery,currentSelectedCustomerCode);
                            ArrayList<String> saudaTransList = mAceDnsDatabaseHelper.GetDOReportDataByCustomer(mQuery,currentSelectedCustomerCode);
                            for(int i=0;i<saudaTransList.size();i++)
                            {
                                ...
                                String ListItemHeader = "DO " + currentPos + "- " + dateInString;
...
//                            mOrderReportDetailsListByCustomer=mAceDnsDatabaseHelper.GetbargainReportDataBybargainId(saudaNo);
                                mOrderReportDetailsListByCustomer=mAceDnsDatabaseHelper.GetDOReportDataById(saudaNo);
    ...
    public String BuildQuery(int select) {
        ...
        switch (select) {
            case 1:
                ...
                condition = "substr(DO.DO_no,-14,8)='" + mToday + "'";
                dateRangeTV.setText("On " + Utils.changeDateFormat("yyyyMMdd", "dd/MM/yyyy", mToday));
                conditionForOpeningStock = "substr(DO.DO_no,-14,8) < '" + mToday + "'";
                break;
            case 2:
                Constants.mode="customermtd";

                condition = "substr(DO.DO_no,-14,4)='" + mCurrentYear + "' AND substr(DO.DO_no,-10,2)='" + mCurrentMonth + "'";
                dateRangeTV.setText("On " + mCurrentMonth + "/" + mCurrentYear);
                conditionForOpeningStock = "substr(DO.DO_no,-14,8) < '" + mCurrentYear + mCurrentMonth + "01'";
            ...
            case 3:
                ...
                condition = "substr(DO.DO_no,-14,8) BETWEEN '" + mStartDate + "' AND '" + mEndDate + "'";
                ...
                conditionForOpeningStock = "substr(DO.DO_no,-14,8) < '" + mStartDate + "'";
