...
public class TRANS_SubmitTDAllocation extends AsyncTask<String, Void, String> {
    ...
    Boolean isunSyncedDataPresent = true;

    public TRANS_SubmitTDAllocation(Context context, boolean finish) {
    ...
    protected String doInBackground(String... params) {
            ...
            if (HTTPUtils.isConnectionPossible(mContext) && !Constants.employeeDetailObject.getEmpCode().startsWith("C")) {
                try {
                    ...
                    String uri = BaseUrl.baseUrl +
                            AceDnsWebServiceURL.submitTDAllocation
                            + "?nick_name=" + Constants.nickName
                            + "&emp_code="
                            + Constants.employeeDetailObject.getEmpCode()
                            + "&last_update_time=" + mLastDownloadTime;

                    Log.d("_DOWNLOAD_", "_DOWNLOAD_ TRANS_SubmitTDAllocation: " +uri);
                    Log.d("_DOWNLOAD_", "_DOWNLOAD_ TRANS_SubmitTDAllocation value: " +mXmlData);
        ...
        Log.d("_DOWNLOAD_", "_DOWNLOAD_ TRANS_SubmitTDAllocation result: " +POST_result);
        return POST_result;
    }
    ...
    protected void onPostExecute(String result) {
        ...
        if (result.length() > 0) {
            if (result.equalsIgnoreCase("1") || result.equalsIgnoreCase("2")) {
                mAceDnsTransactionDatabase.UpdateTDAllocationLocationData();
                if (result.equalsIgnoreCase("2")) {
                    Constants.dataResfresh = true;
                }
//				Toast.makeText(mContext, "TD allocation submitted successfully", 15000)
//				.show();
            }
    ...
    private String XmlData() {
        ...
        for (int ii = 0; ii < unUploadedTransaction.size(); ii++) {
            ...
            String location = "<location>" +
                    ...
                    "</location>";
            if (currentLocation.getTransId().startsWith("TDA")) {
                isunSyncedDataPresent = true;
                xmlData += "<TD_ALLOCATION>";
                xmlData += location;
                ArrayList<TDAllocation> TDAllocationList = mAceDnsTransactionDatabase.GetUnuploadedTDAllocationData(currentLocation.getTransId());
                for (int jj = 0; jj < TDAllocationList.size(); jj++) {
                    TDAllocation obj = new TDAllocation();
                    obj = TDAllocationList.get(jj);
                    xmlData += "<TD_ALLOCATION_DETAILS>";
                    xmlData += "<allocation_id><![CDATA[" + obj.getAllocationId() + "]]></allocation_id>"
                            ...
                            + "<TD><![CDATA[" + obj.getTDAllocated() + "]]></TD>";
                    xmlData += "</TD_ALLOCATION_DETAILS>";
                }
                xmlData += "</TD_ALLOCATION>";
            }
