...
public class TRANS_LogintimePendingSAUDA extends AsyncTask<String, Void, String> {
    ...
    public TRANS_LogintimePendingSAUDA(Context context, boolean finish) {
    ...
    protected String doInBackground(String... params) {
        ...
        if (HTTPUtils.isConnectionPossible(mContext) ) {
            try {
                ...
                Log.d("_DOWNLOAD_", "_DOWNLOAD_ TRANS_LogintimePendingSAUDA: " +uri);
                Log.d("_DOWNLOAD_", "_DOWNLOAD_ TRANS_LogintimePendingSAUDA value: " +xmlData);
        ...
        Log.d("_DOWNLOAD_", "_DOWNLOAD_ TRANS_LogintimePendingSAUDA result: " +POST_result);
        return POST_result;
    }
    ...
    public String prepareXMLData() {
        ...
        for (int ii = 0; ii < unUploadedTransaction.size(); ii++) {
            ...
            if (currentLocation.getTransId().startsWith("FT")
                    || currentLocation.getTransId().startsWith("NFT")) {
                ...
                xmlData += "<sauda_header>"
                        + "<sauda_no><![CDATA[" + mSaudaHeader.getSaudaNo() + "]]></sauda_no>"
                        + "<TD><![CDATA[" + mSaudaHeader.getTD() + "]]></TD>"
                        + "<d_instruction><![CDATA[" + mSaudaHeader.getRemarks() + "]]></d_instruction>"
                        + "<broker_id><![CDATA[" + mSaudaHeader.getBrokerId() + "]]></broker_id>"
                        + "<transaction_type><![CDATA[" + mSaudaHeader.getTransactionType() + "]]></transaction_type>"
                        + "<vat><![CDATA[" + mSaudaHeader.getVat() + "]]></vat>"
                        + "<branch_code><![CDATA[" + mSaudaHeader.getBranchCode() + "]]></branch_code>"
                        + "<valid_from><![CDATA[" + mSaudaHeader.getSaudaValidity() + "]]></valid_from>"
                ...
                ArrayList<SaudaDetails> unUploadedSaudaDetails = dataHelperObj.GETSaudaDetails(mSaudaHeader.getSaudaNo());
                for (int jj = 0; jj < unUploadedSaudaDetails.size(); jj++) {
                    SaudaDetails saudadetails = unUploadedSaudaDetails.get(jj);
                    xmlData += "<sauda_details>"
                            + "<sauda_no><![CDATA[" + saudadetails.getSaudaNo() + "]]></sauda_no>"
                            + "<Sku_code><![CDATA[" + saudadetails.getSkuCode() + "]]></Sku_code>"
                            + "<qty><![CDATA[" + saudadetails.getQuantity() + "]]></qty>"
                            + "<TD><![CDATA[" + saudadetails.getTD() + "]]></TD>"
                            + "<premium><![CDATA[" + saudadetails.getPremium() + "]]></premium>"
                            + "<sale_rate><![CDATA[" + saudadetails.getSaleRate() + "]]></sale_rate>"
                            + "<VAT><![CDATA[" + saudadetails.getVat() + "]]></VAT>"
                            + "<amount><![CDATA[" + saudadetails.getAmount() + "]]></amount>"
