...
import static com.forcepower.acedns.activity.StockAuditEditActivity.grnMasterSkuListItemGlobal;

public class TRANS_SubmitStockAuditEditTransactionTask extends AsyncTask<String, Void, String> {
    Context mContext;
    ...
    public TRANS_SubmitStockAuditEditTransactionTask(Context context, boolean firstTimeTransaction, String customerCode, String orderNo, AceDnsTransactionDatabase dataHelperObj  ) {
        ...
        if (HTTPUtils.isConnectionPossible(mContext)) {
            try {
                ...
                String uri = BaseUrl.baseUrl +
                        AceDnsWebServiceURL.submitStockAuditEditTransactionURL
                        + "?nick_name=" + Constants.nickName
                        + "&emp_code="
                        + Constants.employeeDetailObject.getEmpCode();
                Log.d("_DOWNLOAD_", "_DOWNLOAD_ TRANS_SubmitSMS: " +uri);
                Log.d("_DOWNLOAD_", "_DOWNLOAD_ TRANS_SubmitSMS value: " +xmlData);
...
//                Double currentAmount = Double.parseDouble(sRate) * Double.parseDouble(newQty);
                xmlData += "<stock_audit_edit>"
                        + "<transaction_id><![CDATA[" + orderNo + "]]></transaction_id>"
