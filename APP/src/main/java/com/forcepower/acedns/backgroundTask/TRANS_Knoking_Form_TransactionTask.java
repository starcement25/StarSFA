...
public class TRANS_Knoking_Form_TransactionTask extends AsyncTask<String, Void, String>{
    ...
    public TRANS_Knoking_Form_TransactionTask(Context context, ArrayList<Location> unUploadedTransaction)
    {
    ...
    protected String doInBackground(String... params)
    {
        ...
        {
            String uri = BaseUrl.baseUrl + AceDnsWebServiceURL.submitKnockingFormURL + "?nick_name=" + Constants.nickName;// + "&emp_code=" + Constants.employeeDetailObject.getEmpCode();
            Log.d("_DOWNLOAD_", "_DOWNLOAD_ TRANS_Knoking_Form_TransactionTask: " +uri);
            Log.d("_DOWNLOAD_", "_DOWNLOAD_ TRANS_Knoking_Form_TransactionTask value: " +xmlData);
            POST_result=HttpCalling.httpPostCallWithXmlBodyJsonResponseDecrypted(uri,xmlData);
        }
        Log.d("_DOWNLOAD_", "_DOWNLOAD_ TRANS_Knoking_Form_TransactionTask result: " +POST_result);
        return POST_result;
    ...
    protected void onPostExecute(String result) {
        ...
        if (process.equalsIgnoreCase("YES") || result.equalsIgnoreCase("1") || result.equalsIgnoreCase("2"))
        {
    ...
    public void prepareXMLData()
    {
        ...
        for(int ii=0;ii<unUploadedTransaction.size();ii++)
        {
            ...
            ArrayList<KnokingFormDetails> unUploadedVisit = mAceDnsTransactionDatabase.getUnuploadedKnokingForm(currentLocation.getTransId());
            for(int jj=0;jj<unUploadedVisit.size();jj++)
            {
                KnokingFormDetails detailsObj = unUploadedVisit.get(jj);



                    json.put("knocking_form_id", detailsObj.getKnocking_form_id());
            ...
            ArrayList<TentProduct> unUploadedPro = mAceDnsTransactionDatabase.getUnuploadedKnockingFormProduct(currentLocation.getTransId(),mobile);
            for(int iii=0;iii<unUploadedPro.size();iii++)
            {
                TentProduct detailsObjP = unUploadedPro.get(iii);
                productdata = new JSONObject();
                productdata.put("knocking_form_id", ""+detailsObjP.getTent_form_id());
            ...
            String p ="";
            ArrayList<TentCust> unUploadedCus = mAceDnsTransactionDatabase.getUnuploadedKnockingFormCust(currentLocation.getTransId(),mobile);
            for(int iii=0;iii<unUploadedCus.size();iii++)
            {
                ...
                if((iii+1) == unUploadedCus.size()) {
                    customerdata.put("knocking_form_id", "" + detailsObjP.getTent_form_id());
