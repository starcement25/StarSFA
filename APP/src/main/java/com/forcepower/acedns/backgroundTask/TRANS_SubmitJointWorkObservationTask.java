...
public class TRANS_SubmitJointWorkObservationTask extends AsyncTask<String, Void, String> {
    ...
    public TRANS_SubmitJointWorkObservationTask(Context context, Boolean shouldFinish) {
    ...
    protected String doInBackground(String... params) {
        xmlData = UploadUnuploadedData.prepareXMLDataJointWorkObservation(mContext);//No data present for no data
        String POST_result = "";
            ...
            if (HTTPUtils.isConnectionPossible(mContext) && !Constants.employeeDetailObject.getEmpCode().startsWith("C")) {
                try {
                    String uri = BaseUrl.baseUrl +
                            AceDnsWebServiceURL.submitJointWOrkObservation
                            + "?nick_name=" + Constants.nickName
                            + "&emp_code="
                            + Constants.employeeDetailObject.getEmpCode();
                    Log.d("_DOWNLOAD_", "_DOWNLOAD_ TRANS_SubmitJointWorkObservationTask: " +uri);
                    Log.d("_DOWNLOAD_", "_DOWNLOAD_ TRANS_SubmitJointWorkObservationTask value: " +xmlData);
        ...
        Log.d("_DOWNLOAD_", "_DOWNLOAD_ TRANS_SubmitJointWorkObservationTask result: " +POST_result);
        return POST_result;
    }
