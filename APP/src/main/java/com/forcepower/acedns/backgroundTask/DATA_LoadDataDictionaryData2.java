...
public class DATA_LoadDataDictionaryData2 extends AsyncTask<String, Void, Void> {
    Context mContext;
    AceDnsDatabase dbHelper;
    String timeStamp = "";
    String lastUpdate = "2014-06-09 18:19:20"; //Just to know the format

    public DATA_LoadDataDictionaryData2(Context context) {
    ...
    protected void onPostExecute(Void result) {
        ...
        if (Constants.isFirstLoginOfApp) {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
                new UpdateDeviceIdAsynctask2(mContext).execute();
            } else {
    ...
    private void dataDownloadCommonProcess() {
        if (Constants.downloadTableList.size() > 0) { } else {
