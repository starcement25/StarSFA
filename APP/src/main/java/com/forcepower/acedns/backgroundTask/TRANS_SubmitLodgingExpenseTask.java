...
public class TRANS_SubmitLodgingExpenseTask extends AsyncTask<String, Void, String> {
    ...
    public TRANS_SubmitLodgingExpenseTask(Context context, boolean finish) {
    ...
    protected String doInBackground(String... params) {
        ...
        if (HTTPUtils.isConnectionPossible(mContext) && !Constants.employeeDetailObject.getEmpCode().startsWith("C")) {
            try {
                ...
                String uri = BaseUrl.baseUrl + AceDnsWebServiceURL.submitLodgeExpURL + "?nick_name=" + Constants.nickName + "&emp_code=" + Constants.employeeDetailObject.getEmpCode() + "&last_update_time=" + lastUpdate;
                Log.d("_DOWNLOAD_", "_DOWNLOAD_ TRANS_SubmitLodgingExpenseTask: " +uri);
                Log.d("_DOWNLOAD_", "_DOWNLOAD_ TRANS_SubmitLodgingExpenseTask value: " +xmlData);
        ...
        Log.d("_DOWNLOAD_", "_DOWNLOAD_ TRANS_SubmitLodgingExpenseTask result: " +POST_result);
        return POST_result;
    }
    ...
    protected void onPostExecute(String result) {
        ...
        if (result.equalsIgnoreCase("1") || result.equalsIgnoreCase("2")) {
            ...
            for (int ii = 0; ii < tourTransIdList.size(); ii++) {
                dataHelperObj.updateUnuploadedTourLodgingExp(tourTransIdList.get(ii));
                dataHelperObj.updateUnuploadedLocation(tourTransIdList.get(ii));
            }
            new TRANS_TravelFoodingLodgingAttachmentExportTask(mContext, "LODGING", result, finish).execute();
        } else {
            Utils.cancelProgressDialog();
    ...
    public String prepareXMLData() {
        ...
        for (int ii = 0; ii < unUploadedTransaction.size(); ii++) {
            ...
            String location = "<location>" +
                    ...
                    "</location>";
            if (currentLocation.getTransId().substring(0, 2).equalsIgnoreCase("TL")) {
                xmlData += "<lodge_expense>";
                xmlData += location;
                tourTransIdList.add(currentLocation.getTransId());
                String[] unUploadedTourExp = dataHelperObj.getUnuploadedLodgingExp(currentLocation.getTransId());
                xmlData += "<lodge_expense_details>"
                        + "<lodge_exp_trans_id><![CDATA[" + unUploadedTourExp[0] + "]]></lodge_exp_trans_id>"
                        + "<emp_code><![CDATA[" + unUploadedTourExp[1] + "]]></emp_code>"
                        + "<base_station><![CDATA[" + unUploadedTourExp[2] + "]]></base_station>"
                        + "<hotel_name><![CDATA[" + unUploadedTourExp[3] + "]]></hotel_name>"
                        + "<rent><![CDATA[" + unUploadedTourExp[4] + "]]></rent>"
                        + "<chkin_date><![CDATA[" + unUploadedTourExp[5] + "]]></chkin_date>"
                        + "<chkout_date><![CDATA[" + unUploadedTourExp[6] + "]]></chkout_date>"
                        + "<payment_amount><![CDATA[" + unUploadedTourExp[7] + "]]></payment_amount>"
                        + "<payment_mode><![CDATA[" + unUploadedTourExp[8] + "]]></payment_mode>"
                        + "<attachment_id><![CDATA[" + unUploadedTourExp[9] + "]]></attachment_id>" +
                        "</lodge_expense_details>";
                xmlData += "</lodge_expense>";
            }
        }
        xmlData += "</root>";
