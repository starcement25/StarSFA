		...
		if(count($order_details_array)>0)
		{
			for($i=0;$i<count($order_details_array);$i++){
				...
				if($nick_name=='DNV' || $nick_name=='HALDIRAM')
				{
					...
					if(no_of_filter==1){
						$sqlproductdetails="SELECT prod_desc,UOM1,UOM2,conversion_factor FROM product_master WHERE prod_code='".$orderdatadetails_sku_code."'";
					}
					if(no_of_filter==2){
						$sqlproductdetails="SELECT PGM.product_group_name,PM.prod_desc,PM.conversion_factor,PM.UOM1,PM.UOM2 
