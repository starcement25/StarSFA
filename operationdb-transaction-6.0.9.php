...
if($nick_name=='SKIPPER'  || $nick_name=='DNV' || $nick_name=='ASL' || $nick_name=='GOLDSTONE' || $nick_name=='EDIBLE' || strtoupper($nick_name)=='GOPE' || strtoupper($nick_name)=='ABDOS')
{
		...
		if(count($order_details_array)>0)
		{
			for($i=0;$i<count($order_details_array);$i++){
				...
				if($nick_name=='DNV' || $nick_name=='HALDIRAM')
				{
					...
					if(no_of_filter==1){
						$sqlproductdetails="SELECT prod_desc,UOM1,UOM2,conversion_factor,dns_prod_code,product_group_code FROM product_master WHERE prod_code='".$orderdatadetails_sku_code."'";
					}
					if(no_of_filter==2){
			     $sqlproductdetails="SELECT PGM.product_group_name,PM.prod_desc,PM.conversion_factor,PM.UOM1,PM.UOM2,PM.dns_prod_code,PM.product_group_code 
					...
					if(no_of_filter==3){
						$sqlproductdetails="SELECT PGM.product_group_name,PSGM.product_sub_group_name,PM.prod_desc,PM.dns_prod_code,PM.product_group_code FROM 
	   ...
	   $sqlproductdetails="SELECT PGM.product_group_name,PSGM.product_sub_group_name,PBM.product_brand_name,PM.prod_desc,PM.dns_prod_code,
					...
					if(!in_array($orderdatadetails_order_no,$order_array_trans_id))
					{
						...
						if(mysqli_query($link,$sqlinsertorderdetails))
						{
							...
							if($nick_name=='ARCHITA')
							{
								if(${cust_type.$orderdatadetails_order_no}=='R')
								{
									$sqlcustomerrds="SELECT rds_tag FROM customer_master WHERE customer_code='".$orderdataheader_customer_code."'";
									$rscustomerrds=mysqli_query($link,$sqlcustomerrds);
									$rowcustomerrds=mysqli_fetch_assoc($rscustomerrds);
									$rds_tag=$rowcustomerrds['rds_tag'];
									$sqlchkcustomerstk="SELECT last_stk_audit_date FROM customer_productwise_stock 
												WHERE customer_code='".$rds_tag."' AND prod_code='".$orderdatadetails_sku_code."'";
									...
									{
										$last_stk_audit_date=$rowchkcustomerstk['last_stk_audit_date'];
										if(strtotime($order_date) > strtotime($last_stk_audit_date))
										{
										$sqlupdateclstk="UPDATE customer_productwise_stock SET 
															sale_after_audit =(sale_after_audit+$orderdatadetails_qty),
															sale_date='".$order_date."',
															 cl_stk=((last_stk_audit_qty+purchase_after_audit)-sale_after_audit),
															download_time=CURRENT_TIMESTAMP() WHERE customer_code='".$rds_tag."' 
															AND prod_code='".$orderdatadetails_sku_code."'";
									...
									else
									{
										$sqlinsertclstk="INSERT INTO customer_productwise_stock SET
														 ...
														 sale_after_audit ='".$orderdatadetails_qty."',
														 sale_date='".$order_date."',
														 cl_stk=((last_stk_audit_qty+purchase_after_audit)-sale_after_audit),
														 download_time=CURRENT_TIMESTAMP() ";
								...
								{
								   $sqlchkcustomerstk="SELECT last_stk_audit_date FROM customer_productwise_stock 
									   WHERE customer_code='".$orderdataheader_customer_code."' AND prod_code='".$orderdatadetails_sku_code."'";
									$rschkcustomerstk=mysqli_query($link,$sqlchkcustomerstk);
									$countchkcustomerstk=mysqli_num_rows($rschkcustomerstk);
									$rowchkcustomerstk=mysqli_fetch_assoc($rschkcustomerstk);
									if($countchkcustomerstk>0)
									{
										...
										$sqlupdateclstk="UPDATE customer_productwise_stock SET 
															sale_after_audit =(sale_after_audit+$orderdatadetails_qty),
															sale_date='".$order_date."',
															cl_stk=((last_stk_audit_qty+purchase_after_audit)-sale_after_audit),															 															download_time=CURRENT_TIMESTAMP() WHERE customer_code='".$orderdataheader_customer_code."' 
															AND prod_code='".$orderdatadetails_sku_code."'";
										mysqli_query($link,$sqlupdateclstk);
										}
									}
