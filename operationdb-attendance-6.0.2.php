...
//$body_xml = mysqli_real_escape_string($link, $body);
if ($body == '') {
	$body = $_POST['xmldata'];
}
...
if (strtoupper($nick_name) == 'ABDOS' || strtoupper($nick_name) == 'RUPA' || strtoupper($nick_name) == 'DURO' || strtoupper($nick_name) == 'STAR') {
	...
	$stmt = $link->prepare("INSERT INTO xml_data (emp_code, xml, insertdate) VALUES (?, ?, CURRENT_TIMESTAMP())");
	$stmt->bind_param("ss", $emp_code, $body);
	$stmt->execute();
	$stmt->close();
}
...
//echo count($attendance_array);die;
...
if (count($attendance_array) > 0) {
	for ($x = 0; $x < count($attendance_array); $x++) {
		...
		if ($latt == '' || $latt == 'null' || $latt == 'NULL') $latt = 0;
		if ($longi == '' || $longi == 'null' || $longi == 'NULL') $longi = 0;
		$attendance_date = preg_replace('/\s*:\s*/', ':', $attendance_date);
		...
		//echo $sqlchkattlocationempdate;die;
		...
		if ($countchkattlocation > 0 || $countchkattlocationempdate > 0) {
			...
			$location_date = preg_replace('/\s*:\s*/', ':', $location_date);
			// echo $date.''.$hour;
			...
			// $attendance_date = (new DateTime($attendance_date))->format('Y-m-d H:i:s');
			// $location_date   = (new DateTime($location_date))->format('Y-m-d H:i:s');
			...
			if (mysqli_query($link, $sqlinsertattlocation) && mysqli_query($link, $sqlinsertattendance)) {
				...
				if (!empty($attendance_email)) {
					$mail_status = send_the_mail($attendance_email, $attendanceemailsubj, $attendancemailbody);
					if ($mail_status === "TRUE") {
						$flag = 5;
					} else {
