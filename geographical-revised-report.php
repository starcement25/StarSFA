...
$sql = "select ISNULL(SAT.zoneval,'NA') zoneval,ISNULL(SAT.SEGMENT,'NA') SEGMENT,ISNULL(SAT.MONTHVAL,'NA') MONTHVAL,SUM(SAT.QTYSHIPPED) AS tot_qty,SUM(SAT.TBASE1) AS tot_amount from ( select ID.*,(select ISNULL(VALUE,'NA') from ARCUSO where IDCUST=IH.CUSTOMER AND OPTFIELD='ARZONE') zoneval,(select ISNULL(VDESC,'NA') from CSOPTFD where value in(select value from ICITEMO where OPTFIELD='ITEMSEG' and ITEMNO=ID.ITEM) and OPTFIELD='ITEMSEG' ) SEGMENT,SUBSTRING(CAST(IH.INVDATE AS varchar(20)),1,6) AS MONTHVAL 
FROM OEINVD ID INNER JOIN OEINVH IH ON ID.INVUNIQ=IH.INVUNIQ   AND (IH.location LIKE '1%' or IH.location LIKE 'RUIYA%' or IH.location LIKE '3004%') 
UNION 
select ID.*,c.zone zoneval,(select ISNULL(VDESC,'NA') from CSOPTFD where value in(select value from ICITEMO where OPTFIELD='ITEMSEG' and ITEMNO=ID.ITEM) and OPTFIELD='ITEMSEG' ) SEGMENT,SUBSTRING(CAST(IH.INVDATE AS varchar(20)),1,6) AS MONTHVAL FROM OEINVD ID INNER JOIN OEINVH IH ON ID.INVUNIQ=IH.INVUNIQ INNER JOIN sales_hierarchy_new c ON c.SUBZONE=SUBSTRING(CAST(IH.location AS varchar(20)),1,4)   AND (IH.location LIKE '3%' and IH.location not LIKE '3004%') ) SAT group by SAT.zoneval,SAT.segment,SAT.monthval ORDER BY SAT.monthval,SAT.zoneval,SAT.segment";
