	...
	function excelDownload()
	{
		...
		if($from_date!='' && $to_date!='')
		{
		  $date_condition=" AND DATE_FORMAT(SUBSTRING(SO.survey_id,-14,8),'%Y-%m-%d') >='".$from_date."' AND 
			  ...
			  $sql_get_menu = "SELECT layout_name,menu_id,survey_sub_menu FROM survey_input WHERE type='menu' AND menu_id IN('RA061') ORDER BY display_order ASC";
				 ...
				 ${survey_id_array.$menu_id}=array();
				  ...
				  while($row_survey_output=mysqli_fetch_assoc($rs_survey_output))
				  {
					...
					if(strtoupper($nick_name)=='DURO')
					{
						...
						else if($row_survey_output['row_id']=='RA003')
						{
							if(!in_array('RA126',${row_id_array.$survey_id}))
							{
								//echo 'a';
								$value=$value."\t"."";
							}
							...
							if(!in_array('RA127',${row_id_array.$survey_id}))
							{
								//echo 'a';
								$value=$value."\t"."";
							}
						...
						else if($row_survey_output['row_id']=='RA099')
						{
							if(!in_array('RA125',${row_id_array.$survey_id}))
							{
								//echo 'a';
								$value=$value."\t"."";
							}
						...
						else if($row_survey_output['row_id']=='RA024')
						{
							if(!in_array('RA128',${row_id_array.$survey_id}))
							{
								//echo 'a';
								$value=$value."\t"."";
							}
							...
							if(!in_array('RA129',${row_id_array.$survey_id}))
							{
								//echo 'a';
								$value=$value."\t"."";
							}
						...
						else if($row_survey_output['row_id']=='RA107')
						{
							if(!in_array('RA124',${row_id_array.$survey_id}))
							{
								//echo 'a';
								$value=$value."\t"."";
							}
			...
			for($i=0;$i<count($menu_id_array);$i++)
			{
				...
				${file_name.$menu_id_array[$i]}="$menu_name_array[$i]_$survey_sub_menu_array[$i]_April2022_Oct2022_$date$month$year$hour$minute$second.xls";
				
				//add files to the zip, passing file contents, not actual files
				//$zip->addFile($file_content1, $file_name1);
				$zip->addFile(${file_content.$menu_id_array[$i]}, ${file_name.$menu_id_array[$i]});
			}
			$finalzipfile='customer-data.zip';
