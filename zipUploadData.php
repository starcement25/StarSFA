	...
	if(file_exists($_FILES['zip_file']['tmp_name']))
	{
		...
		if($successval==1)
		{
			...
			if(mail($mailto, $mailsub, $mailbody, $headers,'-facedns@coral.in'))
			{
				if(count($error_array)>0)
				{
					echo $GLOBALS['msg'] = 'Zip file extracted and data has been uploaded successfully with the following error(s).';
				}
				else{
					echo $GLOBALS['msg'] = 'Zip file extracted and data has been uploaded successfully';
				}
			}
			else 
			{
				echo $errmail = "Problem in mail sending.";
			}
