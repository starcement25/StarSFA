...
/** AWS S3 Bucket Name */
			...
			$dateprevious=date('dmY', strtotime("-1 days,$curdateserver "));
			foreach($files as $file) { 
			   ...
			   $file_parts=explode("-",$file_name_latest);
				...
				if(substr($file_parts[3],0,8)==$dateprevious)
				{
					$file_name_previous=$file_name_latest;
				}
				if(substr($file_parts[3],0,8)==$date.$month.$year)
				{
				$source = fopen("../smpower-bkup/$file_name_latest", 'r+');
