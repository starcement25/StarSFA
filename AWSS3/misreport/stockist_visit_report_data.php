...
<?php 
$count=1;
$sql_stockist = "SELECT CM.customer_name,DATE_FORMAT(SUBSTRING(SV.visit_trans_id,-14,8),'%d-%m-%Y') AS visit_date,EM.emp_name,
...
$res_stockist = mysql_query($sql_stockist);
$count_stockist=mysql_num_rows($res_stockist);
if($count_stockist >0){
while($row_stockist = mysql_fetch_array($res_stockist)){
	...
	$rsretailername=mysql_query($sqlretailername);
	$rowretailername=mysql_fetch_array($rsretailername);
