...
<center>
	<span style="font-weight:bold; font-size:14px;">TELECALLER ACHIEVED REPORT</span><br><br>
    <table class="border" width="40%" style="border-collapse:collapse;" cellpadding="6px">
      <tr class="TDHEAD_SUB">
        ...
        <?php
		$sql_emp = "SELECT emp_code, emp_name FROM employee_master WHERE acedns = 'Y' and designation='Telecaller' ".$emp_hierarchy_value_condition." ORDER BY emp_name ASC";
	...
	function display_result(){
		...
		GenericAjaxFunction('telecaller_achieved_report_data.php?employee='+employee+'&start_date='+start_date+'&end_date='+end_date,'display',0);
	}
	...
	function Popup(data) 
	{
		var mywindow = window.open('', 'TELECALLER ACHIEVED REPORT', 'height=400,width=600');
		mywindow.document.write('<html><head><title>TELECALLER ACHIEVED REPORT</title>');
	...
	function exporttocsv(divid)
	{
		...
		var table_div = document.getElementById('display_table');
		var table_html = table_div.outerHTML.replace(/ /g, '%20');
		a.href = data_type + ', ' + table_html;
		//setting the file name
		a.download = 'TELECALLER ACHIEVED REPORT' + postfix + '.xls';
