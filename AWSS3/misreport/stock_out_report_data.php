...
$sql_get_billed_details = "SELECT SUM(CASE WHEN CPB.invoice_date BETWEEN '".$start_date."' AND '".$end_date."' THEN 1 ELSE 0 END) AS billed_qty,
							SUM(CASE WHEN CPB.stock_out_date='0000-00-00' AND CPB.activation_date!='0000-00-00 00:00:00' 
							AND SUBSTRING(CPB.activation_date,1,10) BETWEEN '".$start_date."' AND '".$end_date."' THEN 1 ELSE 0 END) 
							AS stock_out_qty_unregistered,
							CPB.prod_code,CPB.customer_code,CM.customer_name,PM.prod_desc,DATE_FORMAT(CPB.invoice_date,'%d-%m-%Y') AS invoice_date,
CM.rds_tag FROM customer_product_billing CPB,customer_master CM,product_master PM
WHERE CPB.customer_code=CM.customer_code AND CPB.prod_code=PM.prod_code ".$customer_code_condition.$prod_code_condition." GROUP BY CPB.customer_code,CPB.prod_code ORDER BY CM.customer_name ASC";
$res_get_billed_details = mysql_query($sql_get_billed_details);
$total_rows = mysql_num_rows($res_get_billed_details);
if($total_rows>0){
	...
	echo "<tr class='TDHEAD'><td colspan='8' align='center'>Stock Out Report</td></tr>";
	echo "<tr class='TDHEAD_SUB' align=\"center\">
			<td width=\"8%\" >SI</td>
			<td width=\"16%\" align=\"center\">Party Name</td>
			<td width=\"15%\" align=\"center\">Model</td>
			<td width=\"12%\" align=\"center\">Opening Stock</td>
			<td width=\"12%\" align=\"center\">Billed Qty</td>
			<td width=\"12%\" align=\"center\">Stock Out Qty</td>
			<td width=\"13%\" align=\"center\">Stock Out Qty Unregistered</td>
			<td width=\"12%\" align=\"center\">Closing Stock</td>
		  </tr></thead></table></div><div class='tbl-content'><table class='BORDER datatable' cellpadding='0' cellspacing='0' border='1'><tbody>";
	while($row_get_billed_details = mysql_fetch_array($res_get_billed_details)){
		...
		$stock_out_qty_unregistered = $row_get_billed_details['stock_out_qty_unregistered'];
		...
		if($rds_tag=='')
		{			
			$stock_out_qty_unregistered_total=$stock_out_qty_unregistered;
			...
			$sqlstockoutqty="SELECT SUM(CASE WHEN stock_out_date!='0000-00-00' AND stock_out_date BETWEEN '".$start_date."' AND '".$end_date."' 
							THEN 1 ELSE 0 END) AS stock_out_qty  FROM customer_product_billing  
							WHERE stock_out_customer_code='".$customer_code."' AND prod_code='".$prod_code."'";	
			$rsstockoutqty=mysql_query($sqlstockoutqty);
			$rowstockoutqty=mysql_fetch_array($rsstockoutqty);
			...
			$sqlselopeningstock="SELECT (COUNT(IMEI)-(SUM(CASE WHEN stock_out_date!='0000-00-00' AND stock_out_date < '".$start_date."' THEN 1 ELSE 0 END)+SUM(CASE WHEN stock_out_date='0000-00-00' AND activation_date!='0000-00-00 00:00:00' AND SUBSTRING(activation_date,1,10) < '".$start_date."' 
			THEN 1 ELSE 0 END))) 
									AS opening_stock 
								FROM customer_product_billing  WHERE customer_code='".$customer_code."' AND prod_code='".$prod_code."' AND 
									invoice_date < '".$start_date."'";
			$rsselopeningstock=mysql_query($sqlselopeningstock);
			$rowselopeningstock=mysql_fetch_array($rsselopeningstock);
		...
		else
		{
		   $sqlbilling="SELECT COUNT(IMEI) as consolidated_billed_qty FROM customer_product_billing  
			...
			$rowbilling=mysql_fetch_array($rsbilling);
			$billed_qty=$rowbilling['consolidated_billed_qty'];
			$sqlstockoutunregisteredtot="SELECT SUM(CASE WHEN stock_out_date='0000-00-00' AND activation_date!='0000-00-00 00:00:00' THEN 1 ELSE 0 END) 
										AS stock_out_qty_unregistered_total FROM customer_product_billing  
									WHERE customer_code IN(SELECT customer_code FROM customer_master WHERE rds_tag='".$rds_tag."' AND acedns='Y') 
									AND prod_code='".$prod_code."' AND SUBSTRING(activation_date,1,10) BETWEEN '".$start_date."' AND '".$end_date."'";
			$rsstockoutunregisteredtot=mysql_query($sqlstockoutunregisteredtot);
			$rowstockoutunregisteredtot=mysql_fetch_array($rsstockoutunregisteredtot);
			$stock_out_qty_unregistered_total = $rowstockoutunregisteredtot['stock_out_qty_unregistered_total'];
			...
			$sqlstockoutqty="SELECT SUM(CASE WHEN stock_out_date!='0000-00-00' AND stock_out_date BETWEEN '".$start_date."' AND '".$end_date."' 
							THEN 1 ELSE 0 END) AS consolidated_stock_out_qty,SUM(CASE WHEN stock_out_date!='0000-00-00' AND 
							stock_out_date < '".$start_date."' THEN 1 ELSE 0 END) AS consolidated_stock_out_qty_opening  FROM 
			...
			$rsselopeningstockbilling=mysql_query($sqlselopeningstockbilling);
			$rowselopeningstockbilling=mysql_fetch_array($rsselopeningstockbilling);
			$opening_stock_billing=$rowselopeningstockbilling['opening_stock_billing'];
			if($opening_stock_billing==''){						
			   $opening_stock_billing=0;
			}
			$sqlselopeningstockunregistered="SELECT SUM(CASE WHEN stock_out_date='0000-00-00' AND activation_date!='0000-00-00 00:00:00' 
		   				THEN 1 ELSE 0 END) AS stock_out_qty_unregistered_opening FROM customer_product_billing  WHERE 
										customer_code IN(SELECT customer_code FROM customer_master WHERE rds_tag='".$rds_tag."' AND acedns='Y') 
										AND prod_code='".$prod_code."' AND SUBSTRING(activation_date,1,10) < '".$start_date."'";
			$rsselopeningstockunregistered=mysql_query($sqlselopeningstockunregistered);
			$rowselopeningstockunregistered=mysql_fetch_array($rsselopeningstockunregistered);
			$stock_out_qty_unregistered_opening=$rowselopeningstockunregistered['stock_out_qty_unregistered_opening'];
			if($stock_out_qty_unregistered_opening==''){						
			   $stock_out_qty_unregistered_opening=0;
			}
			$opening_stock=$opening_stock_billing-($stock_out_qty_opening+$stock_out_qty_unregistered_opening);
		}
		...
		$closing_stock=$opening_stock+$billed_qty-($stock_out_qty+$stock_out_qty_unregistered_total);
		if($closing_stock < 0)
		{
			$closing_stock=0;
		}
		 if($opening_stock >0 || $billed_qty > 0 || $stock_out_qty > 0 || $closing_stock >0 || $stock_out_qty_unregistered>0 ){
			if($rds_tag=='')
			{
				//echo $customer_code;
				//echo '<br />';
				//echo $stock_out_qty_unregistered;
				echo "<tr>
					<td width='8%'>".$count."</td>
					<td width='16%'>".$customer_name."</td>
					<td width='15%'>".$prod_desc."</td>
					<td align=\"right\" width='12%' style=\"text-align:right;\">".$opening_stock."</td>
					<td align=\"right\" width='12%' style=\"text-align:right;\">".$billed_qty."</td>
					<td align=\"right\" width='12%' style=\"text-align:right;\">".$stock_out_qty."</td>
					<td align=\"right\" width='13%' style=\"text-align:right;\">".$stock_out_qty_unregistered."</td>
					<td align=\"right\" width='12%' style=\"text-align:right;\">".$closing_stock."</td>
				  </tr>";
			 	$count++;
			}
			else
			{
				...
				{
				  //echo $rowselbranchoutlet['customer_code'];
				   //echo '<br />';
				   //echo $stock_out_qty_unregistered;
				   //echo '<br />';
				   ...
				   if(!in_array($concatenatestring,$branch_oulet_array))
					 {
						$sqlstockoutunregisteredbranchout="SELECT SUM(CASE WHEN stock_out_date='0000-00-00' AND activation_date!='0000-00-00 00:00:00' 
														THEN 1 ELSE 0 END) AS stock_out_qty_unregistered FROM customer_product_billing  
														WHERE customer_code ='".$rowselbranchoutlet['customer_code']."'
												AND prod_code='".$prod_code."' AND SUBSTRING(activation_date,1,10) BETWEEN '".$start_date."' AND '".$end_date."'";
						$rsstockoutunregisteredbranchout=mysql_query($sqlstockoutunregisteredbranchout);
						$rowstockoutunregisteredbranchout=mysql_fetch_array($rsstockoutunregisteredbranchout);
						$stock_out_qty_unregistered = $rowstockoutunregisteredbranchout['stock_out_qty_unregistered'];
						$sqlstockoutqtybranchout="SELECT SUM(CASE WHEN stock_out_date!='0000-00-00' AND stock_out_date BETWEEN '".$start_date."' 
														AND '".$end_date."' THEN 1 ELSE 0 END) AS stock_out_qty  FROM 
													customer_product_billing  
												WHERE stock_out_customer_code='".$rowselbranchoutlet['customer_code']."'AND prod_code='".$prod_code."'";
						$rsstockoutqtybranchout=mysql_query($sqlstockoutqtybranchout);
						$rowstockoutqtybranchout=mysql_fetch_array($rsstockoutqtybranchout);
						$stock_out_qty=$rowstockoutqtybranchout['stock_out_qty'];
						if($stock_out_qty== '') $stock_out_qty=0;
						 echo "<tr>
							<td width='8%'>".$count."</td>
							<td width='16%'>".$rowselbranchoutlet['customer_name']."</td>
							<td width='15%'>".$prod_desc."</td>
							<td align=\"right\" width='12%' style=\"text-align:right;\">".$opening_stock."</td>
							<td align=\"right\" width='12%' style=\"text-align:right;\">".$billed_qty."</td>
							<td align=\"right\" width='12%' style=\"text-align:right;\">".$stock_out_qty."</td>
