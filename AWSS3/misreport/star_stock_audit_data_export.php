...
while($row_cust_data = mysql_fetch_array($res_cust_data)){
	...
	if($customer_exist_check>0){
		...
		$table_data .= $count."\t".$date_selected."\t".$district."\t".$area_name."\t".$route_name."\t".$dns_customer_code."\t".$customer_name."\t".$dns_emp_code."\t".$emp_name."\t".$prod_group_name."\t".$prod_desc."\t".$visit_qty."\t".$prod_price."\t".($visit_qty*$prod_price)."\t".$cust_type."\t".$d_instruction."\n";
