...
$rsallfaciemp=mysql_query($sqlallfaciemp);
while($rowallfaciemp=mysql_fetch_array($rsallfaciemp))
{
...
$rsallfaciempcreation=mysql_query($sqlallfaciempcreation);
while($rowallfaciempcreation=mysql_fetch_array($rsallfaciempcreation))
{
...
foreach($emp_code_array as $emp_code_val){
	...
	$sqlfacilitatorlist="SELECT FM.f_code,FM.f_type,FM.facilitator_name,FM.firm_name,FM.mobile_no,FM.email_id,FM.f_area,
						FM.next_follow_up_date,DATE_FORMAT(SUBSTRING(FM.f_code,-14,8),'%d/%m/%Y') AS facilitator_entry_date,DATE_FORMAT(SUBSTRING(FM.last_visit_date,1,10),'%d/%m/%Y') AS last_visit_date,BM.branch_name
						FROM  facilitator_master FM,branch_master BM WHERE FM.branch_code=BM.branch_code AND FM.f_code IN (".${f_code_string.$emp_code_val}.") AND 
						FM.f_type IN(".$entity_val.") ORDER BY FM.f_type ASC";										
	$resfacilitatorlist = mysql_query($sqlfacilitatorlist);
	$cntfacilitatorlist=mysql_num_rows($resfacilitatorlist);					
	while($rowfacilitatorlist = mysql_fetch_array($resfacilitatorlist)){
		...
		/*$sqllastvisitfaci="SELECT DATE_FORMAT(SUBSTRING(survey_id,-14,8),'%d/%m/%Y') AS last_visit_date_facilitator FROM 
						...
						DESC LIMIT 0,1";				
		$rslastvisitfaci=mysql_query($sqllastvisitfaci);
		$rowlastvisitfaci=mysql_fetch_array($rslastvisitfaci);
	   ...
	   /*$sql_site_visit = "SELECT EM.emp_name,'SITES' AS entity_type,SM.site_name,SM.phone_no,SM.site_id,SM.address,
		...
		$total_rows = mysql_num_rows($res_site_visit);
			while($row_site_visit = mysql_fetch_array($res_site_visit)){
				...
				$rslastvisitsite=mysql_query($sqllastvisitsite);
				$rowlastvisitsite=mysql_fetch_array($rslastvisitsite);
