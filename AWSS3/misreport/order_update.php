...
$current_value=$qty.','.$sale_rate;

$sqlpreviousval="SELECT qty,sale_rate FROM order_details WHERE order_no = '" .$order_no."' AND sku_code='".$prod_code."'";
$rspreviousval=mysql_query($sqlpreviousval);
$rowpreviousval=mysql_fetch_array($rspreviousval);
$prev_qty = $_REQUEST['qty'];
$prev_sale_rate = $_REQUEST['sale_rate'];
$previous_value=$prev_qty.','.$prev_sale_rate;
$upd_sql="UPDATE order_details SET qty ='".addslashes($qty)."',
			 sale_rate='".addslashes($sale_rate)."',
			 amount='".($qty*$sale_rate)."'
			 WHERE order_no = '" .$order_no."' AND sku_code='".$prod_code."'";
$upd_prev_order="UPDATE prev_order_counting_master SET visit_qty='".addslashes($qty)."',
				 rate='".addslashes($sale_rate)."',
				 amount='".($qty*$sale_rate)."' WHERE order_no = '" .$order_no."' AND 	product_code='".$prod_code."'";			 
//mysql_query($upd_sql) or die(mysql_error()." Error in customer updation.");
if(mysql_query($upd_sql) && mysql_query($upd_prev_order)){
		$sqlinsertlog="INSERT INTO order_activity_log SET transaction_id='".$order_no."', transaction_date='".substr($order_no,-14,8)."', operation_done_by='".$_SESSION['admin_login']."',operation_type='edit',affected_field='".$prod_code."',previous_value='".$previous_value."',current_value='".$current_value."',operation_performed_ip='".$_SERVER['REMOTE_ADDR']."',update_datetime=current_timestamp,update_flag='0'";
	...
	$totalorderdetails = mysql_num_rows($resorderdetails);
	if($totalorderdetails >0){
		$count = 1;
		?>
        <table  id="display_table" style="border-collapse:collapse;"  width="100%">
          <tr>
          	<td colspan="13" align="center" class="ERR">Order Updated Successfully</td>
          </tr>
         </table>
         <br /> 
		...
		while($roworderdetails = mysql_fetch_array($resorderdetails)){
			...
			echo "<tr>
					...
					<td align=\"center\" width=\"10%\"><a href=\"javascript:access_add_edit('".$prod_code."','".$order_no."');\" title=\" Edit order \" style=\"color: #F00;\"><img src=\"images/edit_icon.gif\" alt=\"\" /></a></td>
				  </tr>";
			$count++;
		}
		?>
