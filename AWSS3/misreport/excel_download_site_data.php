				...
				(SELECT GROUP_CONCAT(customer_name SEPARATOR ';') FROM customer_master WHERE  FIND_IN_SET(customer_code,REPLACE(SM.sub_dealer_involved,';',','))) AS sub_dealer_involved_name
				 ...
				 SUBSTRING(SM.site_id,2,5)=EM.emp_code AND REPLACE(SM.site_id,'S','SU')=LO.trans_id ".$date_condition." ORDER BY DATE_FORMAT(SUBSTRING(SM.site_id,-14,8),'%Y-%m-%d') ASC ";
				  ...
				  while($row_survey_output=mysql_fetch_array($rs_survey_output))
				  {
					...
					$rsarchitectname=mysql_query($sqlarchitectname);
					$rowarchitectname=mysql_fetch_array($rsarchitectname);*/
					...
					$rscontractorname=mysql_query($sqlcontractorname);
					$rowcontractorname=mysql_fetch_array($rscontractorname);*/
					...
					$rsdealername=mysql_query($sqldealername);
					$rowdealername=mysql_fetch_array($rsdealername);*/
					...
					$rssubdealername=mysql_query($sqlsubdealername);
					$rowsubdealername=mysql_fetch_array($rssubdealername);*/
					...
					$rsauthretailername=mysql_query($sqlauthretailername);
					$rowauthretailername=mysql_fetch_array($rsauthretailername);*/
