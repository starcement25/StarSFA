...
function main(){
	...
	$sql_survey_type = "SELECT DISTINCT type FROM survey_output WHERE type <> 'Branding Verification'  ORDER BY type ASC";
	...
	$sql_technical_type = "SELECT display_name,row_id FROM survey_input WHERE type='menu' ORDER BY display_order ASC";
	$res_technical_type = mysql_query($sql_technical_type);
	while($row_technical_type = mysql_fetch_array($res_technical_type)){
	...
	function display_result(){
		...
		|| survey_type == 'Branding Verification'){
...
function insert_record($row_id)
{
	...
	while($row_survey_details = mysql_fetch_array($res_survey_details)){
		...
		$emp_code=substr($survey_id,2,5);
			...
			$rsmaxdnscustomercode=mysql_query($sqlmaxdnscustomercode);
			$rowmaxdnscustomercode=mysql_fetch_array($rsmaxdnscustomercode);
