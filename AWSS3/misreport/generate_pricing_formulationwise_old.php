...
<?php
	if($_REQUEST['pricing_mode']=='create_pricing')
	{
		...
		$tabledata='<form name="create_price" method="post" action="generate_pricing_formulationwise.php">
						...
						<table class="border" width="70%" border="1" style="border-collapse:collapse;" cellpadding="5px" align="center">
						  <tr>
							<td colspan="9" class="TDHEAD" align="left">Base Product Sale Rate </td>
						  </tr>
						  <tr class="TDHEAD_SUB">
							<td>SI</td>
							<td>Product Code</td>
			...
			foreach($formulation_prod_array as $formulation_prod_val)
			{
				...
				$sqlconversionfactor="SELECT conversion_factor,conversion_factor_two,prod_desc,UOM1 FROM product_master WHERE dns_prod_code='".$formulation_prod_val."'";
				...
				$sqlpackingprodwise="SELECT packing_cost,labour_cost,extra_cost FROM packing_master WHERE dns_prod_code='".$formulation_prod_val."' 
										AND plant_name='".$plant_name."' ORDER BY datetime DESC LIMIT 0,1";
				...
				{
					${loose_rate_case_prodwise.$formulation_prod_val}=${loosrate_total.$formulation_prod_val};
				}
				else
				{
					${loose_rate_case_prodwise.$formulation_prod_val}=round((${loosrate_total.$formulation_prod_val}*${conversion_factor_two.$formulation_prod_val}),2);
				}
				...
				${basic_rate_prodwise.$formulation_prod_val}=${loose_rate_case_prodwise.$formulation_prod_val}+${packing_cost.$formulation_prod_val}+${margin_cost.$formulation_prod_val}+${labour_cost.$formulation_prod_val}+${extra_cost.$formulation_prod_val};
				${basic_rate_prodwise.$formulation_prod_val}=round(${basic_rate_prodwise.$formulation_prod_val},2);
					...
					$tabledataval.="<input type=\"hidden\" name=\"prod_val[]\" value=".$formulation_prod_val.">
									...
									<input type=\"hidden\" name=\"plant_name[]\" value=".$plant_name.">
									<input type=\"hidden\" name=\"conversion_factor[]\" value=".${conversion_factor.$formulation_prod_val}.">
									...
									<tr id=\"tab\">
										...
										<td align=\"right\">".number_format(${packing_cost.$formulation_prod_val},2)."</td>
										<td align=\"right\">".number_format(${labour_cost.$formulation_prod_val},2)."</td>
										<td align=\"right\">".number_format(${extra_cost.$formulation_prod_val},2)."</td>
		...
		echo $tabledata.=$tabledataval."<tr><td colspan='4' align='right'>&nbsp;&nbsp;&nbsp;<input type=\"submit\" name=\"submit5\" value=\"Issued to Release\" /></td><td colspan='5' align='left'><input type='button' name='button5' value='Cancel' onclick=\"javascript:window.location='http://salesmpower.acedns.in/misreport/generate_pricing_formulationwise.php'\"/></td></tr></table></form>";
	}
	if($_REQUEST['mode']=='submit_pricing')
	{
		...
		if($flag==1){?>
		   <script language="JavaScript" type="text/javascript">alert('Base product rate release sucessfull.');</script>
           <?php
