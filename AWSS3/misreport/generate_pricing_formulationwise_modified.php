...
<form action="generate_pricing_formulationwise_modified.php" name="pricing_details" onSubmit="return validation();" method="post">
...
<table cellpadding="4px">
    ...
    <?php
		...
		$sqlselectformulation="SELECT base_oil,mandatory,lower_limit,upper_limit,sl_no FROM base_oil_master WHERE acedns='Y' ORDER BY sl_no ASC";										
		$rsselectformulation=mysql_query($sqlselectformulation);
		while($rowselectformulation=mysql_fetch_array($rsselectformulation))
		{
			$oils=$rowselectformulation['base_oil'];
			$mandateval=$rowselectformulation['mandatory'];
			$lower_limitval=$rowselectformulation['lower_limit'];
			$upper_limitval=$rowselectformulation['upper_limit'];
			/*if($oils=='CPO Declared')
			{
				...
				$rscrudeoilrate=mysql_query($sqlcrudeoilrate);
				$rowcrudeoilrate=mysql_fetch_array($rscrudeoilrate);
			...
			$rsprevoilrate=mysql_query($sqlprevoilrate); 
			$rowrsprevoilrate=mysql_fetch_array($rsprevoilrate);
			$text_value=$rowrsprevoilrate['oils_rate'];
			if($text_value=='') $text_value=0;
				
			//$text_value=$rowcrudeoilrate['crude_oil_rate'];
			$tabledata.='<tr><input type="hidden" name="mandate[]" id="mandate_"'.$oils.'" value="'.$mandateval.'"/><input type="hidden" name="lower_limit[]" id="lower_limit_"'.$oils.'" value="'.$lower_limitval.'"/><input type="hidden" name="upper_limit[]" id="upper_limit_"'.$oils.'" value="'.$upper_limitval.'"/> <td align="left">'.$oils.'(MT):</td><td align="left"><input type="text" name="loose_rate[]"  id="loose_rate_"'.$oils.'" style="height:20px;" value="'.$text_value.'"/><input type="hidden" name="oils_val[]" value="'.$oils.'" /></td></tr>';
		}
		echo $tabledata;
	?></table>
       </td>
...
function validation()
{
	...
	var y = document.getElementsByName("mandate[]");
	var m = document.getElementsByName("oils_val[]");
	var n = document.getElementsByName("lower_limit[]");
	var z = document.getElementsByName("upper_limit[]");
	var i;
	var flag=0;
	for (i = 0; i < x.length; i++) {
	  ...
	  if ((x[i].value ==0) && (y[i].value.toUpperCase() =='Y')) {
		 //alert(x[i].value);
		 alert(m[i].value+" rate value should be Greater than 0");
		 return false;
	  }
	  if(y[i].value.toUpperCase() =='Y')
	  {
		  //alert(x[i].value+'#'+z[i].value);
		  if ((x[i].value < parseFloat(n[i].value))===true) {
			 //alert(x[i].value);
			 alert(m[i].value+" rate range will be >="+n[i].value);
			 return false;
		  }
		  else if((x[i].value > parseFloat(z[i].value))===true)
		  {
			  alert(m[i].value+" rate range will be  <="+z[i].value);
			  return false;
		  }
	  }
	}
...
<?php
	if($_REQUEST['pricing_mode']=='create_pricing')
	{
		...
		$count=1;
		$tabledataval='';
		$tabledata='<form name="create_price" method="post" action="generate_pricing_formulationwise_modified.php">
						...
						<table class="border" width="70%" border="1" style="border-collapse:collapse;" cellpadding="5px" align="center">
						  ...
						  <tr class="TDHEAD_SUB">
							...
							<td>Material Cost(CASE)</td>
							<td>Material Cost(TON)</td>
							<td>Process Cost(CASE)</td>
							<td>Process Cost(TON)</td>
			...
			foreach($formulation_prod_array as $formulation_prod_val)
			{
				...
				else
				{
					...
					$tabledataval.="<input type=\"hidden\" name=\"prod_val[]\" value=".$formulation_prod_val.">
									...
									<input type=\"hidden\" name=\"loose_rate_ton[]\" value=".${loosrate_final_val.$formulation_prod_val}.">
									<input type=\"hidden\" name=\"process_cost_ton[]\" value=".${process_cost.$formulation_prod_val}.">
									<input type=\"hidden\" name=\"plant_name[]\" value=".$plant_name.">
									<input type=\"hidden\" name=\"conversion_factor[]\" value=".${conversion_factor_two.$formulation_prod_val}.">
									<input type=\"hidden\" name=\"UOM4[]\" value=".${UOM4.$formulation_prod_val}.">
									...
									<tr id=\"tab\">
										...
										<td align=\"right\">".number_format(${loosrate_final_val.$formulation_prod_val},2)."</td>
										<td align=\"right\">".number_format(${process_cost_case.$formulation_prod_val},2)."</td>
										<td align=\"right\">".number_format(${process_cost.$formulation_prod_val},2)."</td>
	...
	if($_REQUEST['mode']=='submit_pricing')
	{
		...
		$process_cost_ton=$_POST['process_cost_ton'];
		...
		for($m=0;$m<count($prod_val);$m++){
			...
			{*/
				...
				$sqlinsertmrpprodwise="INSERT INTO sauda_mrp SET mrp_code='".$max_mrp_code."',
										...
										basic_rate_ton='".$loose_rate_ton[$m]."',
										process_cost_ton='".$process_cost_ton[$m]."',
				...
				if(mysql_query($sqlinsertmrpprodwise))
				{
				  ...
				  while($rowfetchdependentprod=mysql_fetch_array($rsfetchdependentprod))
				  {
					...
					$sqlinsertmrpprodwisedepen="INSERT INTO sauda_mrp SET mrp_code='".$max_mrp_code."',
											...
											create_date='".$price_create_date."',
											primary_freight	='0',depot_cost='0',download_time=CURRENT_TIMESTAMP()";
					mysql_query($sqlinsertmrpprodwisedepen);
					...
					$rsmaxindustrialratecode=mysql_query($sqlmaxindustrialratecode);
					$rowmaxindustrialratecode=mysql_fetch_array($rsmaxindustrialratecode);
					...
					if(${pack_size.$dependent_prod_val}=='BP')
					{
						${industrial_rate_prodwise.$dependent_prod_val}=$sale_rate[$m]+$add_subtract_val;
						${industrial_rate_prodwise.$dependent_prod_val}=round(${industrial_rate_prodwise.$dependent_prod_val},0);
					}
					if(${pack_size.$dependent_prod_val}=='CP')
					{
						${industrial_rate_prodwise.$dependent_prod_val}=((($sale_rate[$m]/$UOM4[$m])*${UOM5.$dependent_prod_val})+$add_subtract_val)*${UOM4.$dependent_prod_val};
					...
					$sqlinsertindustrialrate="INSERT INTO industrial_rate SET mrp_code='".$max_industrial_code."'
											...
											branch_code='',product_code='".${prod_code.$dependent_prod_val}."',
											mapped_prod_code_rate='".$sale_rate[$m]."',
											UOM4_mapped_prod_code='".$UOM4[$m]."',
					...
					mysql_query($sqlinsertindustrialrate);
				  }
				}
			/*}
			else
		...
		if($flag==1){?>
		   <script language="JavaScript" type="text/javascript">window.location.href='generated_price_release_modified.php';</script>
           <?php
			...
			?><script language="JavaScript" type="text/javascript">alert('Base product rate release unsuccessful.');window.location.href='generate_pricing_formulationwise_modified.php';</script>
		<?php 
		}
	}
	/*if($_REQUEST['mode']=='submit_pricing_dependent')
	{
		...
		if($flag==2){?>
           ...
           <?php }else{?>
           <script language="JavaScript" type="text/javascript">alert('Dependent product rate release unsuccessful.');window.location.href='generate_pricing_formulationwise_modified.php';</script>
           <?php
	}
  }*/
}
