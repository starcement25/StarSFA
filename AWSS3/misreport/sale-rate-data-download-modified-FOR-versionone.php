...
$sql_product_details = "SELECT * FROM (SELECT DISTINCT PM.dns_prod_code,PM.prod_desc,PM.conversion_factor,PM.packing_realization,
						PM.conversion_factor_two,PM.product_group_code,MR.process_cost,MR.basic_rate,PM.pack_size,
						MR.sale_rate,PM.prod_code,MR.parent_child FROM product_master PM, sauda_mrp MR
...
if($total_rows >0){
    echo '<table class="border" width="98%" border="1"   align="center">
		<thead>
      <tr>
        <td colspan="18" class="TDHEAD" align="left" ><b>Sale Rate Report - '.$from_date.' - '.$product_group_name.' - '.$state_name.' - '.$route_name.' - '.$transport_mode.' - '.$capacityval.'</b></td>
      </tr>
      <tr class="TDHEAD_SUB">
      	<td width="4%">SI</td>
		<td width="6%">Route</td>
		<td width="6%">Capacity</td>
		<td width="6%">Load Distribution</td>
		<td width="6%">Actual Freight</td>
		<td width="6%">Oil group</td>
        <td width="6%">Material Code</td>
        <td width="6%">Material Description</td>
        <td width="5%">Material Cost</td>
		<td width="5%">Process Cost</td>
        <td width="5%">Packing Cost</td>
		<td width="5%">Labour Cost</td>
		<td width="5%">Extra Cost</td>
		<td width="6%">Packing Realization</td>
		<td width="5%">Margin cost</td>
		<td width="6%">Ex Plant Rate</td>
		<td width="6%">Plant to Route Freight</td>
		<td width="6%">Sale Rate - FOR Plant</td>
      </tr></thead>
      <tbody>';
	  	$count = 1;
...
foreach($routearray as $route)
{
	...
	while($row_product_details = mysql_fetch_array($res_product_details))
	{
		...
		else $ex_plant_rate=$basic_rate+$process_cost+$packing_realization+$margin_cost;
		...
		if($basic_rate==0) {$basic_rate='-';}
		else 			$basic_rate=number_format($basic_rate,2);
		...
		if($margin_cost==0) { $margin_cost='-';}
		else				$margin_cost=number_format($margin_cost,2);
		if($ex_plant_rate==0) {$ex_plant_rate='-';}
		else				$ex_plant_rate=number_format($ex_plant_rate,2);
		if($freight_charge_plant==0) {$freight_charge_plant='-';}
		else				$freight_charge_plant=number_format($freight_charge_plant,2);
		if($FOR_plant_rate==0) {$FOR_plant_rate='-';}
		else				$FOR_plant_rate=number_format($FOR_plant_rate,2);	
		...
		echo "<tr>
				...
				<td align=\"right\">".$margin_cost."</td>
				<td align=\"right\" style=\"background:#FFFF33;\">".$ex_plant_rate."</td>
				<td align=\"right\">".$freight_charge_plant."</td>
				<td align=\"right\" style=\"background:#00CC66;\">".$FOR_plant_rate."</td>
		</tr>";
		  $count++;
		}
	}
