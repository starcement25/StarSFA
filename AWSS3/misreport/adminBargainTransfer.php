...
function main()
{
 ...
 if($_REQUEST['mode']=='transferbargain'){
	...
	for($i=0;$i<=count($prod_val);$i++)
	{
	  ...
	  $rescommoditiy = mysql_query($sqlcommoditiy);
	  $rowcommoditiy = mysql_fetch_array($rescommoditiy);
    ...
    <?php
	  ...
	  else
	  {
		...
		for($i=0;$i<=count($prod_val);$i++)
		{
		  	$sqldnsprodcode="SELECT dns_prod_code FROM product_master WHERE prod_code='".$prod_val[$i]."'";
			...
			$rsparentexstchkblank=mysql_query($sqlparentexstchkblank);
			$cntparentexstchkblank=mysql_num_rows($rsparentexstchkblank);
		  ...
		  if($_REQUEST['saudaqty_'.$prod_val[$i]] >0)
		  {
			...
			if($cntparentexstchk >0)
			{
				$sqlupdateDO="UPDATE DO_master SET qty='".$_REQUEST['saudaqty_'.$prod_val[$i]]."',
					...
					mysql_query($sqlupdateDO);	 
			}
			else
			{
			  $sqlselchildprods="SELECT PM.prod_code,PM.dns_prod_code FROM product_master PM,product_unit_coversion_matrix PUCM WHERE PUCM.prod_code=PM.dns_prod_code AND PUCM.acedns='Y' AND PM.acedns='Y' AND PUCM.mapped_prod_code='".$dns_prod_code."'";
			...
			while($rowchildprods = mysql_fetch_array($rschildprods))
			{
				...
				if($child_prod_code==$prod_val[$i])
				{
					...
					$sqlrate="SELECT sale_rate FROM sauda_mrp WHERE  product_code='".$child_prod_code."' AND 
								release_date < '".$bargain_date."' ORDER BY release_date DESC LIMIT 0,1";
				}
				else
				{
					$qty=0;
				...
				$rsrate=mysql_query($sqlrate);
				$recrate=mysql_fetch_array($rsrate);
				$sale_rate_child=$recrate['sale_rate'];
				//exit();
				$mapped_prod_code=$dns_prod_code;
				//For child freight charge
				$sqlselcapacity="SELECT transport_mode,loadability_ton,route_code,branch_code,incoterms 
				...
				$branch_code=$rowselcapacity['branch_code'];
				$incoterms=$rowselcapacity['incoterms'];
				if(strtoupper($incoterms)=='FOR PLANT'){
				$sqlbranchroutefreight="SELECT freight FROM branch_route_freight WHERE 
										branch_code='".$branch_code."' AND route_code='".$route_code."' 
										AND transport_mode='".$transport_mode."' AND 
										capacity='".$loadability_ton."' AND acedns='Y' 
				...
				$sqlloaddistribution="SELECT qty_truck_load FROM load_distribution WHERE 
									transport_mode='".$transport_mode."' AND truck_load='".$loadability_ton."' 
									AND prod_code='".$child_dns_prod_code."' ORDER BY datetime DESC LIMIT 0,1";
...
<table width="80%" align="center" cellpadding="2" cellspacing="2" border="0">
	<tr>
		<td width="100%" align="left" valign="middle" style="padding:10px;"><strong> Administrator >>Bargain Transfer</strong></td>
	</tr>
	<tr>
                    ...
                    <tr class="TDHEAD_SUB"> 
                        ...
                        <?php
							...
							$sql_customer = "SELECT DISTINCT CM.customer_code,CM.customer_name FROM customer_master CM,DO_master DM 
											...
											AND DM.dns_sauda_no LIKE 'BR/%' ORDER BY CM.customer_name ASC";
    ...
    <input name="export" type="button" value="Export" id="btnExport" onClick="exporttocsv();" >
			   ...
			   function access_add_edit(prod_code,order_no)
                ...
                function customer_bargain(customer_code){
                    ...
                    GenericAjaxFunction('get_bargain_related_data.php?customer_code='+customer_code+'&opttype=bargaintransfer','bargain_select_div',0);
                }
			 ...
			 function display_result_transfer(){
				...
				GenericAjaxFunction('Bargain_transfer_data.php?customer_code='+customer_code+'&sauda_no='+sauda_no,'display',0);
