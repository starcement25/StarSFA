	...
	?>
     <tr class="TDHEAD">
      ...
      <?php
		...
		$rssurveyid=mysql_query($sqlsurveyid);
	    $count=1;
	    while($rowsurveyid = mysql_fetch_array($rssurveyid))
        {
			 ...
			 while($row_survey_output=mysql_fetch_array($rs_survey_output))
			  {
				...
				if($row_survey_output['row_id']=='RA006')
				{
					 $sqlsamplephoto="SELECT sample_photo FROM sample_master WHERE reference_no='".$row_survey_output['value']."'";
 					 $rssamplephoto=mysql_query($sqlsamplephoto);
					 $rowsamplephoto=mysql_fetch_array($rssamplephoto);
