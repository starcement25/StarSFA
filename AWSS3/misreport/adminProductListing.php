...
function main()
{
	...
	$sql_count = "SELECT COUNT(PM.prod_code) FROM product_master PM WHERE PM.acedns='Y'";
   ...
   $sql="SELECT PM.prod_desc,PM.prod_code,PM.dns_prod_code,PGM.product_group_name,PSGM.product_sub_group_name FROM 
			 product_master PM INNER JOIN product_group_master PGM ON PM.product_group_code=PGM.product_group_code
			 INNER JOIN product_sub_group_master PSGM ON PM.product_sub_group_code=PSGM.product_sub_group_code WHERE PM.acedns='Y'".$sql_condition."
						ORDER BY PSGM.product_sub_group_name ASC,PM.prod_desc ASC ";
...
<table width="100%" align="center" cellpadding="2" cellspacing="2" border="0">
	<tr>
		<td width="100%" align="left" valign="middle" style="padding:10px;"><strong> Administrator >> Manage Product Listing</strong></td>
	</tr>
    <tr>
		<td valign="top" >
		...
		<td valign="top" bgcolor="#FFFFFF">		
			...
			?>
				<tr class="TDHEAD_SUB"> 
                    ...
                    <td width="15%" align="left" style="padding-left:20px;">Product Code</td>
					<td width="26%" align="left" style="padding-left:20px;">Description</td>
                    <td width="" align="left" style="padding-left:20px;">Product Group</td>
                    <td width="30%" align="left" style="padding-left:20px;">Product Sub Group</td>
				...
				<tr onMouseOver="this.bgColor='<?=SCROLL_COLOR;?>'" onMouseOut="this.bgColor=''" class="body"> 
                    ...
                    <td align="left" valign="top" style="padding-left:20px;"><?=stripslashes($rec['dns_prod_code']);?></td>
                    ...
                    <td align="left" valign="top" style="padding-left:20px;"><?=stripslashes($rec['product_sub_group_name']);?></td>
					<td align="center"><a href="javascript:access_add_edit('<?=$prod_code;?>','<?=$GLOBALS[start]?>');" title=" Edit Product " style="color: #F00;">EDIT</a></td>
				</tr>
			<?php 
				} // end of while loop
			} // end of page count
	...
	<br>
	<form name="frm_opts" action="adminProductListing.php" method="post" >
		<input type="hidden" name="mode" value="<?=$_REQUEST['mode']?>">
        <input type="hidden" name="search_mode" value="<?=$_REQUEST['search_mode']?>">
		<input type="hidden" name="pageNo" value="<?=$_REQUEST[pageNo]?>">
		<input type="hidden" name="url" value="adminProductListing.php">
...
function access_add_edit($row_id)
{
	...
	$rsproduct=mysql_query($sqlproduct) or die(mysql_error()." Error in show customer: ".$sqlproduct);
	...
	$rsprodtype=mysql_query($sqlprodtype);
	$countprodtype=mysql_num_rows($rsprodtype);
	if($countprodtype > 0)
	{
		...
		$rsparentproduct=mysql_query($sqlparentproduct);
		$rowparentproduct=mysql_fetch_array($rsparentproduct);
		$prod_type= 'parent';
	...
	else
	{
	 ...
	 $sqlchildproduct="SELECT mapped_prod_code,is_flash,flash_name,add_subtract_val FROM product_unit_coversion_matrix 
	 					WHERE prod_code='".$dns_prod_code."' ORDER BY download_time DESC LIMIT 0,1";
	 $rschildproduct=mysql_query($sqlchildproduct);
	 $rowchildproduct=mysql_fetch_array($rschildproduct);
...
function state_route(state){
	...
	$(document).ready(function () {
		var xhrdowninwl;
		//alert('k');
		$(".process_cost_input").on('input', function() {
			document.getElementById('submitbutt').disabled = 'disabled';
		});
		$(".process_cost_input").blur(function(){  
	//$('body').on('blur','#productdet.process_cost_input',function(){
		//alert('kkkkk');
	var the_oils = $(this).attr("the_oils");
	//var the_oils=the_oils.replace(' ', '_');
	//alert(jQuery("#orderchnge_"+the_wish_id).val());
	//var process_val=$("#process_cost_"+the_oils).val();
	var process_val=document.getElementById("process_cost_"+the_oils).value;
	var img1 = '<img src="ajax-loader.gif">';
//jQuery("#imagedis").html(img1);
	$.ajax({
	url: 'price_change_process_cost.php',
	type: 'POST',
	dataType: 'html',
	data: "the_oils="+the_oils+"&process_val="+process_val,
	success: function(response){
		...
		//if (r == true) {
			$.ajax({
				url: 'send_mail_process_cost.php',
				type: 'POST',
				dataType: 'text',
				data: "the_oils="+the_oils+"&process_val="+process_val,
				success: function(response){
		  ...
		  document.getElementById("process_cost_"+the_oils).value=document.getElementById("process_cost_old_"+the_oils).value;
		  //document.getElementById('modval').value='cancelprocess';
...
<table width="100%" align="center" cellpadding="2" cellspacing="2" border="0" id="productdet">
	<tr>
			...
			<table width="70%" align="center" class="border" cellpadding="5" cellspacing="2">
                 ...
                 <tr>
					...
					<td align="left" valign="top"><select name="fg_rm" id="fg_rm" >
                            <option value="">SELECT</option>
                                ...
                                $resultqueryfgrm = mysql_query($sqlqueryfgrm);
                                $countqueryfgrm=mysql_num_rows($resultqueryfgrm);
                                if($countqueryfgrm>0){
                                while($rowqueryfgrm = mysql_fetch_array($resultqueryfgrm))
                                {
                                ?>
                ...
                <td align="left" width="20%"  valign="top" class="tbllogin">Oil Formulation<font color="#FF0000"><strong>*</strong></font>&nbsp;&nbsp;</td>
                  ...
                  <td align="left" valign="top"><div style="max-height:200px; overflow-y: scroll;">
                    <table cellpadding="5" cellspacing="2">
						...
						if($mod == 'change_mapping')
						{
							$sqloils="SELECT DISTINCT oils FROM loose_oilrate_formulation WHERE acedns='Y' ORDER BY oils ASC ";
							...
							while($rowoils=mysql_fetch_array($rsoils))
							{
								...
								<tr>
									...
									<td align="left">
                                    <input type="hidden" id="process_cost_old_<?php echo $rowoils['oils'];?>" value="<?php echo $process_cost;?>"/>
									 <input type="text" class="process_cost_input" name="process_cost_<?php echo $rowoils['oils'];?>" 
                                     id="process_cost_<?php echo $rowoils['oils'];?>" value="<?php echo $_POST['process_cost_'.$oil_val];?>" 
									 style="width:70px;height:15px;display:<?php if($prod_type=='parent' && $_POST['oils_'.$oil_val]==$rowoils['oils']){echo "";}else{?>none<?php }?>"  /><span id="imagedis" ></span>
									</td>
								 </tr>   
							<?php
							}
                        ...
                        while($rowoils=mysql_fetch_array($rsoils))
                        {
							$sqlprodoils="SELECT DISTINCT oils,formulation,percentile_calc from loose_oilrate_formulation WHERE 
										prod_code='".$dns_prod_code."' AND oils='".$rowoils['oils']."' and acedns='Y' ORDER BY datetime DESC LIMIT 0,1";
							$rsprodoils=mysql_query($sqlprodoils);
							$countprodoils=mysql_num_rows($rsprodoils);
							if($countprodoils >0)
							{
								$rowprodoils=mysql_fetch_array($rsprodoils);
								$prodoils=$rowprodoils['oils'];
								$prod_percentile_calc=$rowprodoils['percentile_calc'];
								$prodformulation=$rowprodoils['formulation'];
								if($rowprodoils['percentile_calc']=='N')
								{
								$sqlprocesscost="SELECT process_cost FROM process_cost WHERE oil_type='".$rowprodoils['oils']."' ORDER BY datetime DESC LIMIT 0,1";
							...
							{
								$prodoils='';
								$prod_percentile_calc='';
								$prodformulation='';
							}
						?>	
                                <td align="left">
                                    <input type="checkbox" name="oils_<?php echo $rowoils['oils'];?>" id="oils_<?php echo $rowoils['oils'];?>" value="<?php echo $rowoils['oils'];?>" onChange="javascript:populate_formulation(this.value);" 
									<?php if($prodoils==$rowoils['oils']){?>checked<?php }?>/><?php echo $rowoils['oils'];?>
                                    <input type="hidden" name="oils[]" value="<?php echo $rowoils['oils'];?>" />
                                </td>
                                <td align="left">
                                   <input type="text" name="formulation_<?php echo $rowoils['oils'];?>" id="formulation_<?php echo $rowoils['oils'];?>" 
                                   value="<?php echo $prodformulation;?>" 
                                    style="width:70px;height:15px;display:<?php if($prod_type=='parent' 
									&& $prodoils==$rowoils['oils']){echo "";}else{?>none<?php }?>"  />
                                </td>
                                <td align="left">
                                   <select name="percentile_calc_<?php echo $rowoils['oils'];?>" id="percentile_calc_<?php echo $rowoils['oils'];?>"  style="display:<?php if($prod_type=='parent' && $prodoils==$rowoils['oils']){echo "";}else{?>none<?php }?>" >
                                        <option value="">SELECT</option>
                                         <option value="Y" <?php if($prod_percentile_calc=='Y'){ echo 'selected';}?> >Y</option>
                                         <option value="N" <?php if($prod_percentile_calc=='N'){ echo 'selected';}?>>N</option>
                                    </select>
                                </td>
                                <td align="left">
                                <input type="hidden" id="process_cost_old_<?php echo $rowoils['oils'];?>" value="<?php echo $process_cost;?>"/>
                                 <input type="text" class="process_cost_input" name="process_cost_<?php echo $rowoils['oils'];?>" id="process_cost_<?php echo $rowoils['oils'];?>" value="<?php echo $process_cost;?>" the_oils="<?php echo $rowoils['oils'];?>"
                                 style="width:70px;height:15px;display:<?php if($prod_type=='parent' &&  $prodoils==$rowoils['oils']){echo "";}else{?>none<?php }?>"  /><span id="imagedis" ></span>
                                </td>
                             </tr>   
                        <?php
                    	}
              ...
              <tr id="parent_tr" style="display:<?php if($prod_type=='child'){echo "";}else{?>none<?php }?>">
                ...
                <tr>
					...
					<td>&nbsp;</td>
					<td>&nbsp;</td>
					<td><input type="submit" value=" Change " class="inplogin" id="submitbutt" > &nbsp;&nbsp;<input type="button" name="btn" value="Cancel" onClick="javascript:window.location='adminProductListing.php';" class="inplogin"></td>
				</tr>
			</table>
			</form>
		</td>
