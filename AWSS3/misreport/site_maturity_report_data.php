...
/*----> Total Site Visit <----*/
		/*$sql_site_visit = "SELECT EM.emp_name,EM.designation,EM.emp_code,EM.branch_code,SM.address,SM.sub_area,SM.city,SM.site_reffered_by,SM.contact_person,SM.phone_no,
						...
						ON SM.emp_code=EM.emp_code AND SUBSTRING(SM.expected_month_maturity,-4,4)='".$year."' AND 
						SUBSTRING(SM.expected_month_maturity,-7,2)='".$monthvalue."' AND SM.state='".$state."'";*/
		$sql_site_visit = "SELECT EM.emp_name,EM.designation,EM.emp_code,EM.branch_code,SM.address,SM.sub_area,SM.city,SM.site_reffered_by,SM.contact_person,SM.phone_no,
						SM.current_status,SM.product_info,SM.product_category,SM.expected_month_maturity,SM.site_id,SM.site_name
		 				FROM site_master SM INNER JOIN employee_master EM
						ON SM.emp_code=EM.emp_code AND SUBSTRING(SM.expected_month_maturity,-4,4)='".$year."' AND 
						SUBSTRING(SM.expected_month_maturity,-7,2)='".$monthvalue."' AND SM.emp_code IN(".$emp_code_string.")";				
		...
		if($total_rows>0){
			...
			while($row_site_visit = mysql_fetch_array($res_site_visit)){
				...
				$product_info=substr(str_replace(';',',',$row_site_visit['product_info']),0,-1);
