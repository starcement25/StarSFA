					...
					<td  align="left" valign="top"> 
                        <select name="approval_authority" id="approval_authority" onChange="javascript:sel_emp(this.value);">
                            <option value="">SELECT</option>
                                ...
                                $resultquerydesignation = mysql_query($sqlquerydesignation);
                                $countquerydesignation=mysql_num_rows($resultquerydesignation);
                                if($countquerydesignation >0){
                                while($rowquerydesignation= mysql_fetch_array($resultquerydesignation))
                                {
                                ?>
                 ...
                 <tr>
                    ...
                    onChange="javascript:sel_approval_sub_category(this.value);">
                            <option value="">SELECT</option>
                                ...
                                $resultqueryapprovalcat = mysql_query($sqlqueryapprovalcat);
                                $countqueryapprovalcat=mysql_num_rows($resultqueryapprovalcat);
                                if($countqueryapprovalcat>0){
                                while($rowqueryapprovalcat = mysql_fetch_array($resultqueryapprovalcat))
                                {
                                ?>
		...
		foreach($emp_code as $emp_code_val)
		{
			foreach($approval_sub_category as $approval_sub_category_val)
			{
				foreach($operation_type as $operation_type_val)
				{
					...
					mysql_query($sqlinsertappmatrix);
				}
			}
		}
