...
$sqldistributor="SELECT customer_name,cust_type,state_code FROM customer_master WHERE customer_code=".$dealer."";
...
$state_code_db=$rowdistributor['state_code'];

$sqlstatecode="SELECT state_code FROM state_master WHERE dns_state_code='".$state_code_db."'";
...
$rowlaststkaudit=mysql_fetch_array($rslaststkaudit);
...
$rowlaststkauditprevmonth=mysql_fetch_array($rslaststkauditprevmonth);
...
$cnt_product_details=mysql_num_rows($res_product_details);
if($cnt_product_details >0)
{
	...
	echo "<tr class='TDHEAD_SUB' align=\"center\">
			<td width='7%'>SL NO</td>
			<td width='20%'>SKU</td>
			<td width='10%'>Opening Stock Value</td>
			<td width='9%'>Sum of purchase value before $last_stock_audit_date_heading</td>
			<td width='9%'>Last stock audit value as on $last_stock_audit_date_heading</td>
			<td width='9%'>Sum of purchase value after $last_stock_audit_date_heading</td>
			<td width='9%'>Sale</td>
			<td width='9%'>Stock</td>
			<td width='9%'>UOM</td>
	...
	while($row_product_details=mysql_fetch_array($res_product_details)){
		...
		$row_purchase=mysql_fetch_array($rs_purchase);			
		...
		$rs1ststkaudit=mysql_query($sql1ststkaudit);	
		$row1ststkaudit=mysql_fetch_array($rs1ststkaudit);
		...
		if($invoice_date<=$last_stock_audit_date)
		{
	 ...
	 for($i=0;$i<count($prod_code_array);$i++)
	 {
	   ...
	   $sqlselrate="SELECT sale_rate,distributor_rate FROM mrp WHERE product_code='".$prod_code_array[$i]."' AND state_code='".$state_code."'";
	   $rsselrate=mysql_query($sqlselrate);	
	   $rowselrate=mysql_fetch_array($rsselrate);
	   $sale_rate=$rowselrate['sale_rate'];
	   $distributor_rate=$rowselrate['distributor_rate'];
	   ...
	   $sqllaststkauditprevmonth="SELECT quantity
		...
		$row_last_month_purchase=mysql_fetch_array($rs_last_month_purchase);
		...
		$rstotalsecondarysales=mysql_query($sqltotalsecondarysales);	
		$rowtotalsecondarysales=mysql_fetch_array($rstotalsecondarysales);
		...
		else					  						 $opening_stock=number_format(($opening_stock*$distributor_rate),0);
		if(${puchaseqtytilllatstkadt.$prod_code_array[$i]} ==0)	${puchaseqtytilllatstkadt.$prod_code_array[$i]}='-';
		else					  ${puchaseqtytilllatstkadt.$prod_code_array[$i]}=number_format((${puchaseqtytilllatstkadt.$prod_code_array[$i]}*$distributor_rate),0);
		if($stk_audit_last_array[$i] ==0)	$stk_audit_last_array[$i]='-';
		else					  $stk_audit_last_array[$i]=number_format(($stk_audit_last_array[$i]*$distributor_rate),0);
		if(${puchaseqtyafterlatstkadt.$prod_code_array[$i]} ==0)	${puchaseqtyafterlatstkadt.$prod_code_array[$i]}='-';
		else					  ${puchaseqtyafterlatstkadt.$prod_code_array[$i]}=number_format((${puchaseqtyafterlatstkadt.$prod_code_array[$i]}*$distributor_rate),0);
		if($sale ==0)			 $sale='-';
		else					  $sale=number_format(($sale*$distributor_rate),0);
		if($stock ==0)			$stock='-';
		else					  $stock=number_format(($stock*$distributor_rate),0);
		if($totalsecondarysales ==0)	$totalsecondarysales='-';
		else					  $totalsecondarysales=number_format($totalsecondarysales,0);
		/*echo "<td style=\"text-align:right\">".round((($totalsecondarysales/$conversion_factor_two_array[$i])*$sale_rate),2)."</td>";*/
		...
		echo "<tr>
				...
				<td style=\"text-align:right\">".round(($totalsecondarysales*$sale_rate),2)."</td>
			  </tr>";
		$count++;
		}
	 }
