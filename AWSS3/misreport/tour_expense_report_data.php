...
if($total_row_check>0){
	...
	while($row_emp_code = mysql_fetch_array($res_emp_code)){
		...
		$cnt_tour_details=mysql_num_rows($res_tour_details);				
		...
		while($row_tour_details = mysql_fetch_array($res_tour_details)){
			...
			$total_amount=($transport_other_charges+$fooding_charges+$hotel_charge+$local_conveyance+$other_expenses);
			${total_amount.$emp_code}=${transport_other_charges.$emp_code}+${fooding_charges.$emp_code}+${hotel_charge.$emp_code}+${local_conveyance.$emp_code}+${other_expenses.$emp_code};
