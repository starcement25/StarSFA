...
function customer_information_edit($row_id)
{
   $sqlcustomerdetails="SELECT customer_name,rds_tag,route_code,phone_no,emp_code
	...
	$rowcustomerdetails=mysql_fetch_array($rescustomerdetails);
					...
					<td  align="left" valign="top"> 
                        <select name="rds_name" id="rds_name" >
                            <option value="">SELECT</option>
                                ...
                                $resultquerydistributor = mysql_query($sqlquerydistributor);
                                $countquerydistributor=mysql_num_rows($resultquerydistributor);
                                if($countquerydistributor>0){
                                while($rowquerydistributor = mysql_fetch_array($resultquerydistributor))
                                {
                                ?>
					...
					<td  align="left" valign="top"> 
                        <select name="route_name" id="route_name" >
                            <option value="">SELECT</option>
                                ...
                                $resultqueryroute = mysql_query($sqlqueryroute);
                                $countqueryroute=mysql_num_rows($resultqueryroute);
                                if($countqueryroute>0){
                                while($rowqueryroute = mysql_fetch_array($resultqueryroute))
                                {
                                ?>
