...
$sql_prospect = "SELECT EM.emp_name, PCH.customer_name, PCH.phone_no, PCH.customer_code, PCH.area,PCH.pin,LO.latt, LO.longi FROM employee_master EM, prospective_customer_header PCH, location LO WHERE LO.trans_id=PCH.trans_id AND  EM.emp_code=substring(LO.trans_id,3,5) AND ".$condition." AND (LO.trans_id LIKE 'DC%' OR LO.trans_id LIKE 'DE%') AND EM.emp_code='$_GET[emp_code]'";
...

{
	...
	echo "<tr>
			...
			<td><a href=\"#\" id=\"pros$count\" onClick=\"GenericAjaxFunction('prospect_details.php?pcid=$row_prospect[customer_code]&route_name=$row_prospect[route_name]','prospect_info',0); changecolor(id);\" style=\"color:blue;\">$row_prospect[customer_name]</a></td>
