...
$rs_active_dealer_prev_month=mysql_query($sql_active_dealer_prev_month);
while($row_active_dealer_prev_month=mysql_fetch_array($rs_active_dealer_prev_month))
{
...
$sql_active_dealer_current_month="SELECT CM.branch_code,CM.customer_code,SACW.$columname AS achv_val FROM `self_appraisal_customer_wise` SACW,customer_master CM 
								WHERE CM.dns_customer_code=SACW.customer_code AND CM.cust_type='Dealer' ORDER BY CM.branch_code ASC";
$rs_active_dealer_current_month=mysql_query($sql_active_dealer_current_month);
while($row_active_dealer_current_month=mysql_fetch_array($rs_active_dealer_current_month))
{
...
$res_sis_emp = mysql_query($sql_sis_emp);
$sis_emp_check = mysql_num_rows($res_sis_emp);
	?>
	...
	while($row_sis_emp = mysql_fetch_array($res_sis_emp)){
		...
		$rsbranchvoltgtachv=mysql_query($sqlbranchvoltgtachv);
		$rowbranchvoltgtachv=mysql_fetch_array($rsbranchvoltgtachv);
		...
		$rslevelvisittgtachv=mysql_query($sqllevelvisittgtachv);
		$rowlevelvisittgtachv=mysql_fetch_array($rslevelvisittgtachv);
		...
		$rslevelincentive=mysql_query($sqllevelincentive);
		$rowlevelincentive=mysql_fetch_array($rslevelincentive);
		...
		$rslevelpenalty=mysql_query($sqllevelpenalty);
		$rowlevelpenalty=mysql_fetch_array($rslevelpenalty);
