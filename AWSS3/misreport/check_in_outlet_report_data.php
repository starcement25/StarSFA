...
$emp_date_checkout_array=array();
	...
	$emp_date_outlet_array=array();
		...
		$sqlcheckinoutlet="SELECT EM.emp_name,EM.emp_code,LO.trans_id,DATE_FORMAT(SUBSTRING(LO.trans_id,-14,8),'%Y%m%d') AS formatted_date,
						...
						DATE_FORMAT(SUBSTRING(LO.trans_id,-14,14),'%H:%i:%s') AS visit_time,LO.address,CM.customer_name
						FROM employee_master EM,location LO,order_header OH,customer_master CM 
						WHERE LO.emp_code=EM.emp_code AND LO.trans_id=OH.order_no AND OH.customer_code=CM.customer_code $emp_condition AND
		...
		$rscheckinoutlet=mysql_query($sqlcheckinoutlet) or die(mysql_error()." Error in select check in out details ".$sqlcheckinoutlet);
		$countcheckinoutlet=mysql_num_rows($rscheckinoutlet);
		if($countcheckinoutlet >0)
		{
			while($rowcheckinoutlet=mysql_fetch_array($rscheckinoutlet))
			{
