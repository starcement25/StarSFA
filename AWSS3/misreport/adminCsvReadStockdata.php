...
function main()
{
	if($_REQUEST['mode']=="submit_stock_allocation")
	{
		...
		$stock_array=$_POST['stock_array'];
		$prod_code_array=$_POST['prod_code_array'];
		
		$customer_code_array=array();
		//$registration_id_array=array();
		$customer_name_array=array();
		
		//$notificatiomessage="Hi,\nYour Today's allocations are\n";
		for($i=0;$i<count($prod_code_array);$i++)
		{								 
		  $from_date_val=date('Y-m-d',strtotime(str_replace('/','-',$from_date[$i])));
		  $to_date_val=date('Y-m-d',strtotime(str_replace('/','-',$to_date[$i])));
		  $sqlselcustomercode="SELECT customer_code,rds_tag FROM customer_master WHERE customer_name='".addslashes($customer_name[$i])."'";
		  ...
		  $rds_tag=$rowcustomercode['rds_tag'];
		  if($rds_tag=='')
		  {
			  $customer_code_consilated=$customer_code;
		  }
		  else
		  {
			   $customer_code_consilated=$rds_tag;
		  }
		  /*if(!in_array($customer_code,$customer_code_array))
			{
				array_push($customer_code_array,$customer_code);
			}*/
			$sqlproddesc="SELECT prod_desc FROM product_master WHERE prod_code='".$prod_code_array[$i]."'";
			$rsproddesc=mysql_query($sqlproddesc);
			$rowproddesc=mysql_fetch_array($rsproddesc);
			$prod_desc=$rowproddesc['prod_desc'];
			if($stock_array[$i] > 0)
			{
				${notificationmessage.$customer_code}.=$prod_desc.'-'.$stock_array[$i]."\n";
			}
			//End Push Notification
		  $allocation_id='CA'.str_replace('/','#',$customer_code).$contentsdatetime;
		  $allocation_id_consolidated='CA'.str_replace('/','#',$customer_code_consilated).$contentsdatetime;
		  if(!in_array($customer_code,$customer_code_array))
		  {
			  ...
			  mysql_query($sqlupdatestockbalance);
			  array_push($customer_code_array,$customer_code);
			  array_push($customer_name_array,$customer_name[$i]); 
		  }
		  $sqlinsert="INSERT INTO customer_product_allocation SET allocation_id='".$allocation_id."',
		  			allocation_id_consolidated='".$allocation_id_consolidated."',
		  			customer_code='".$customer_code."',
					from_date='".$from_date_val."',to_date='".$to_date_val."',acedns='Y',download_time=CURRENT_TIMESTAMP(),
					prod_code='".$prod_code_array[$i]."',qty='".$stock_array[$i]."'";
		  mysql_query($sqlinsert);
		  
		  $sqlinsertlog="INSERT INTO customer_product_allocation_log SET allocation_id='".$allocation_id."',
		  				allocation_id_consolidated='".$allocation_id_consolidated."',customer_code='".$customer_code."',
					from_date='".$from_date_val."',to_date='".$to_date_val."',acedns='Y',download_time=CURRENT_TIMESTAMP(),
					prod_code='".$prod_code_array[$i]."',qty='".$stock_array[$i]."'";
		  mysql_query($sqlinsertlog);
		  $sqlinsertstockbalance="INSERT INTO stock_balance_details SET allocation_id='".$allocation_id."',
		  						allocation_id_consolidated='".$allocation_id_consolidated."',customer_code='".$customer_code."',
		  							prod_code='".$prod_code_array[$i]."',allocation_qty='".$stock_array[$i]."',
								allocation_date=CURRENT_TIMESTAMP(),download_time=CURRENT_TIMESTAMP()";
		  mysql_query($sqlinsertstockbalance);				
	  }
	$date=gmdate('d',strtotime('+330 minute'));
	$month=gmdate('m',strtotime('+330 minute'));
	$year=gmdate('Y',strtotime('+330 minute'));
	...
	//$notification_id='PN'.strtoupper($_SESSION['admin_login']).$location_date;
	  for ($m=0;$m<count($customer_code_array);$m++)
		...
		$title = "";
		$message="Hi,\n.".$customer_name_array[$m]." Today's allocations are\n".${notificationmessage.$customer_code_array[$m]}." THANKS,\nVCONNECT";
		//echo $message="Hi,\nYour Today allocations are\nMi MAX 2-2\nMi A1-3\nMi Mix 2-11\nRedmi 5A (2+16)-3\nRedmi 5A (3+32)-0\n THANKS,\nVCONNECT";
		...
		while($rowempdetails=mysql_fetch_array($rsempdetails))
		{
			...
			$rsregdetailsreportingto=mysql_query($sqlregdetailsreportingto);
			$rowregdetailsreportingto=mysql_fetch_array($rsregdetailsreportingto);
	 ...
	 for ($m=0;$m<count($customer_code_array);$m++)
	   {
		 for($n=0;$n<count($prod_code_array);$n++)
		 {
			 ...
			 $rsselstockbalancedetails=mysql_query($sqlselstockbalancedetails);
			 $rowselstockbalancedetails=mysql_fetch_array($rsselstockbalancedetails);
	...
	<tr>
		<td valign="top" >
			<table width="70%" align="center" cellpadding="5" cellspacing="2">
            	 <tr> 
                    <td width="90%" align="center" class="ERR"><font size="+2"><u>Upload Stock Data</u></font></td>
            	</tr>
            </table>
          </td>
    </tr>       
	   ...
	   if(similar_file_exists("../csv/$folderName/stock allocation.csv")!=false)
	   {
		...
		$tabledata='<form name="depot_cost" method="post" action=""><table border="1" style="border-collapse:collapse;" class="border" width="70%" cellpadding="4" align="center" >
					  <tr class="TDHEAD" align="center" id="head_main">
					  	<td colspan="50" class="TDHEAD" align="center">Stock Allocation</td>
					  </tr>
	   ...
	   //$type=strtoupper(substr($_FILES['excel_file']['name'],(strrpos($_FILES['excel_file']['name'],".")+1)));
		...
		foreach($lines as $line)
		{
			   ...
			   if($rec_count==0)
				{
					...
					foreach($data as $key=>$value)
					{
						...
						{	
							...
							if($countprodcode==0)
							{
								array_push($error_array,"Error @Row (1) Invalid Column");
							}
							if($countprodcode >0){
								$rowprodcode=mysql_fetch_array($rsprodcode);
								$prod_code=$rowprodcode['prod_code'];
				...
				else
				{
				  ...
				  $sqlcustomer="SELECT customer_name FROM customer_master WHERE customer_name='".addslashes($customer_name)."' AND acedns='Y'";
				  $rscustomer=mysql_query($sqlcustomer);
				  $countcustomer=mysql_num_rows($rscustomer);
				  if($countcustomer==0)
					{
						array_push($error_array,"Error @Row (".$csv_row_count.") Column : (customer_name)");
					}
					$fromdatecheck=date('Y-m-d',strtotime(str_replace('/','-',$from_date)));
					$todatecheck=date('Y-m-d',strtotime(str_replace('/','-',$to_date)));
				  if(strtotime($todatecheck) < strtotime($current_date))
				  {
					  array_push($error_array,"Error @Row (".$csv_row_count.") Column : (To Date)");
				  }
				  ...
				  for($k=0;$k <count($prodarray);$k++)
				  {
					 ...
					 $tabledatacsv.="<input type=\"hidden\" name=\"customer_name[]\" value=\"$customer_name\">
								...
								<input type=\"hidden\" name=\"prod_code_array[]\" value=\"$prodarray[$k]\">
								<input type=\"hidden\" name=\"stock_array[]\" value=".$data[$k+3].">
								<td align=\"right\">".number_format($data[$k+3])."</td>";
				  }
				   $tabledatacsv.="</tr>";
				} 
			  $count++;	
			...
			if(count($error_array) >0){
			 echo "<tr> 
					<td width=\"90%\" align=\"center\"  colspan=\"50\"><font size=\"+2\"><u>Stock Allocation</u></font></td></tr><br />";
			   foreach($error_array as $error_val)
			   {
				   echo "<tr> 
					<td width=\"90%\" align=\"center\" class=\"ERR\" nowrap=\"nowrap\" colspan=\"20\"><font size=\"+1\">".$error_val."</font></td></tr>";
			   }
		   }
		   else
		   {
			echo $tabledatafinal=$tabledata.$tabledatacsv."<tr><td colspan='50' align='center'><input type='hidden' name='mode' value='submit_stock_allocation' /><input type='submit' name='submit1' value='Final Upload' /><input type='button' name='button3' value='Cancel' onclick=\"javascript:window.location='http://salesmpower.acedns.in/misreport/adminCsvReadStockdata.php'\"/></td></tr></table></form>";
			die;
			$successval=1;
		   }
		 }
		...
		foreach($lines as $line)
		{
			...
			{ 
				...
				//echo $value;
				 ...
				 $invoice_no=trim($data[2]);
				 $invoice_date=trim($data[3]);
				 $invoice_date_final=date('Y-m-d',strtotime(str_replace('/','-',$invoice_date)));
				 $prod_desc=trim($data[4]);
				 $IMEI=trim($data[5]);
				 //$refference_no=trim($data[5]);
				  ...
				  $rsselcustomercode=mysql_query($sqlselcustomercode);
				  $countselcustomercode=mysql_num_rows($rsselcustomercode); 
				  if($countselcustomercode==0)
					...
					{
						$rowselcustomercode=mysql_fetch_array($rsselcustomercode);
				  ...
				  $rsselcustomerdnscode=mysql_query($sqlselcustomerdnscode);
				  $countselcustomerdnscode=mysql_num_rows($rsselcustomerdnscode);
				 ...
				 /* $sqlprodcode="SELECT prod_code FROM product_master WHERE prod_desc='".rtrim(addslashes($prod_desc))."' and acedns='Y'";
				  ...
				  if($countprodcode==0)
					...
					$rschkactiveallocation=mysql_query($sqlchkactiveallocation);
				    $countchkactiveallocation=mysql_num_rows($rschkactiveallocation);
					...
					$sqlchkrequisitionqty="SELECT SUM(requisition_qty) AS total_requisition FROM stock_balance_details WHERE customer_code='".$customer_code."' 
											AND prod_code='".$prod_code."' AND active_flag='Y'  AND billed_qty =0";
					$rschkrequisitionqty=mysql_query($sqlchkrequisitionqty);
					$rowchkrequisitionqty=mysql_fetch_array($rschkrequisitionqty);
					...
					$sqlchkbilledqty="SELECT SUM(billed_qty) AS total_billed FROM stock_balance_details WHERE customer_code='".$customer_code."' 
											AND prod_code='".$prod_code."' AND active_flag='Y'";
					$rschkbilledqty=mysql_query($sqlchkbilledqty);
					$rowchkbilledqty=mysql_fetch_array($rschkbilledqty);
				    $total_billed=$rowchkbilledqty['total_billed']+1;
				  ...
				  $rsrequisition=mysql_query($sqlrequisition);
				  $countrequisition=mysql_num_rows($rsrequisition);
				  ...
				  $tabledatacsv.="<input type=\"hidden\" name=\"customer_name[]\" value=\"$customer_name\">
								...
								<td>".$customer_code."</td>
								<td>".$invoice_no."</td>
								<td>".$invoice_date."</td><td>".$prod_desc."</td>
								<td>".$IMEI."</td><td>".$refference_no."</td></tr>";	
				}
			 $rec_count++;
			 $count++;
		}		
