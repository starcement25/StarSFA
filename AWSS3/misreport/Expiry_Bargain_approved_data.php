...
$sqlexpirybargaindetails="SELECT EM.emp_name,PM.prod_desc,PM.UOM1,
				...
				DM.sku_code=PM.prod_code AND DM.is_approved='yes'  AND DM.qty >0 ORDER BY DATE_FORMAT(SUBSTRING(DM.sauda_no,-14,14),'%Y-%m-%d %H:%i:%s') ASC";	
//exit();					
...
if($totalexpirybargaindetails >0){
		$count = 1;
		?>
       <form name="frm_bargainapprove" method="post" action="adminExpiryBargainApproved.php" enctype="multipart/form-data" />
        <input type="hidden" name="mode" value="expiryapprovebargain"/>
		...
		<table border="1" id="display_table" style="border-collapse:collapse;" class="border" width="100%">
          <tr class="TDHEAD_SUB">
          	<td colspan="13" align="center">Expiry Bargain Information <!--of <?php //echo $customer_name;?> on <?php //echo $dns_sauda_no;?>--></td>
          </tr>
		  <tr class="TDHEAD" align="center">
			<td width="4%">SI</td>
            <td width="8%">Bargain Date</td>
            <td width="10%">Customer</td>
            <td width="10%">Bargain Done By</td>
            ...
            <td width="7%">Expired Date</td>
            <td width="7%">Increase Validity <br />(Days)</td>
		...
		while($rowbargaindetails = mysql_fetch_array($resexpirybargaindetails)){
			...
			if(strtotime($curr_date) > strtotime($valid_upto_format_chk))
			{
				...
				while($rowbargainqty=mysql_fetch_array($rsbargainqty))
				{
					...
					$sqltotalDOqty="SELECT SUM(DO_qty_MT) AS  DO_qty_MT FROM DO_transaction WHERE sauda_no='".$sauda_no."' AND sku_code	='".$child_sku_code."'";
