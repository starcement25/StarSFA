...
if($vertical=='all')
	...
	$rsdistinctproductgroup=mysql_query($sqldistinctproductgroup);
	$countdistinctproductgroup=mysql_num_rows($rsdistinctproductgroup);
	while($rowdistinctproductgroup=mysql_fetch_array($rsdistinctproductgroup))
	{
...
/*----> Total Calls & Days Present <----*/
	...
	$sql_closing_stock = "SELECT CM.customer_code,CM.customer_name,CM.dns_customer_code
		...
		$res_closing_stock = mysql_query($sql_closing_stock);
		$total_rows = mysql_num_rows($res_closing_stock);
		if($total_rows>0){
			...
			while($row_closing_stock = mysql_fetch_array($res_closing_stock)){
				...
				foreach($productgrouparray as $productgroupcodeval)
				{
					...
					$rsclstk=mysql_query($sqlclstk);
					$rowclosingstk=mysql_fetch_array($rsclstk);
