...
if($opt_type=='checkouttimeupdate')
{
	...
	$rschkattendance=mysql_query($sqlchkattendance);
	$cntchkattendance=mysql_num_rows($rschkattendance);
	if($cntchkattendance >0)
	{
		...
		$sqlinsertcheckout="INSERT INTO attendence SET emp_code='".$emp_code."',
							trans_id='".$trans_id."',
							date='".$checkout_date."'";	
		if(mysql_query($sqlinsertattlocation) && mysql_query($sqlinsertcheckout))
		{
		?>	
		<table  style="border-collapse:collapse;" class="border" width="60%">
		  <tr class="TDHEAD_SUB">
...
$total_rows = mysql_num_rows($res_emp_list);
if($total_rows>0){
	?>
	...
	while($row_emp_list = mysql_fetch_array($res_emp_list)){
		...
		$countatttime=mysql_num_rows($rsatttime);
