...

{
	...
	if($_REQUEST['mode']=='submit_opening_bargain')
	{
	  ...
	  $rsparentsku=mysql_query($sqlparentsku);

	  $rowparentsku=mysql_fetch_array($rsparentsku);
	 ...
	 if($dns_prod_code[$k]== $mapped_prod_code)
	 {
		...
		$rsselcapacity=mysql_query($sqlselcapacity);

		$rowselcapacity=mysql_fetch_array($rsselcapacity);
		...
		$sqlprodconversion="SELECT UOM1,UOM2,UOM3,conversion_factor,conversion_factor_two,UOM4 FROM product_master WHERE prod_code='".$prod_code[$k]."'";
		 ...
		 $rsprodconversion=mysql_query($sqlprodconversion);

		 $rowprodconversion=mysql_fetch_array($rsprodconversion);
		 ...
		 /*if($incoterms=='FOR PLANT')
			...
			$rsbranchroutefreight=mysql_query($sqlbranchroutefreight);

			$rowbranchroutefreight=mysql_fetch_array($rsbranchroutefreight);
			...
			while($rowchildprods = mysql_fetch_array($rschildprods))
			{
				...
				$rsloadistribution=mysql_query($sqlloaddistribution);

				$rowloaddistribution=mysql_fetch_array($rsloadistribution);
	...
	/* else

	 {
		...
		mysql_query($sqlinsertDomasterchild);
	...
	*/
...

		{
			...
			mysql_query($sqlinsertDomasterparent);
	...
	if($_REQUEST['mode']=='submit_load_distribution'){
		...
		{				
			...
			$cnthirecost=mysql_num_rows($rshirecost);

			if($cnthirecost > 0)

			{

			 while($rowshirecost=mysql_fetch_array($rshirecost))

			 {
			   ...
			   mysql_query($sqlinsertfreightcostloaddist);
	...
	if($_REQUEST['mode']=='submit_packing'){
		...
		{				
			...
			$sqlpacking .= "  dns_prod_code='".mysql_real_escape_string($dns_prod_code[$i])."'";

			$sqlpacking .= " , packing_cost='".mysql_real_escape_string($packing_cost[$i])."'";

			$sqlpacking .= " , labour_cost='".mysql_real_escape_string($labour_cost[$i])."'";

			$sqlpacking .= " , plant_name='".mysql_real_escape_string($plant_name[$i])."'";

			$sqlpacking .= " , extra_cost='".mysql_real_escape_string($extra_cost[$i])."'";
	...
	if($_REQUEST['mode']=='submit_formulation'){
		...
		$sqloilformulation .= "  plant_name='".mysql_real_escape_string($plant_name[$i])."'";

		$sqloilformulation .= " , prod_code='".mysql_real_escape_string($dns_prod_code[$i])."'";

		$sqloilformulation .= " , oils='".mysql_real_escape_string($oil_type[$i])."'";

		$sqloilformulation .= " , formulation='".mysql_real_escape_string($formulation[$i])."'";

		$sqloilformulation .= " , base_oil='".mysql_real_escape_string($base_oil[$i])."'";

		$sqloilformulation .= " , percentile_calc='".mysql_real_escape_string($percentile_calc[$i])."'";
	...
	if($_REQUEST['mode']=='submit_process'){
		...
		{				
			...
			$sqlprocess .= "  oil_type='".mysql_real_escape_string($oil_type[$i])."'";

			$sqlprocess .= " , oil_category='".mysql_real_escape_string($oil_category[$i])."'";

			$sqlprocess .= " , process_cost='".mysql_real_escape_string($process_cost[$i])."'";

			$sqlprocess .= " , plant_name='".mysql_real_escape_string($plant_name[$i])."'";
	...
	if($_REQUEST['mode']=='submit_packing_material'){
		...
		$rstruckloadval=mysql_query($sqltruckloadval);

		while($rowtruckloadval=mysql_fetch_array($rstruckloadval))
		...
		{				
			...
			$sqlpackingmaterial .= "  material_name='".mysql_real_escape_string($material_name[$i])."'";

			$sqlpackingmaterial .= " , UOM='".mysql_real_escape_string($UOM[$i])."'";

			$sqlpackingmaterial .= " , price='".mysql_real_escape_string($price[$i])."'";

			$sqlpackingmaterial .= " , weight='".mysql_real_escape_string($weight[$i])."'";

			$sqlpackingmaterial .= " , final_pack='".mysql_real_escape_string($final_pack[$i])."'";

			$sqlpackingmaterial .= " , height='".mysql_real_escape_string($height[$i])."'";

			$sqlpackingmaterial .= " , length='".mysql_real_escape_string($length[$i])."'";

			$sqlpackingmaterial .= " , breadth='".mysql_real_escape_string($breadth[$i])."'";
			...
			mysql_query($sqlpackingmaterial) or die(mysql_error().".Internal error occurrs @row $csv_row_count in Packing Material Master.csv.Please check.");

			$sqlprodlist="SELECT prod_code FROM packing_BOM WHERE material_name='".addslashes($material_name[$i])."'";

			$rsprodlist=mysql_query($sqlprodlist);

			while($rowprodlist=mysql_fetch_array($rsprodlist))
...

			{
				...
				$rsprodlistloaddist=mysql_query($sqlprodlistloaddist);

				while($rowprodlistloaddist=mysql_fetch_array($rsprodlistloaddist))
...

		{
			...
			while($row_BOM = mysql_fetch_array($res_BOM)){
				...
				$rsprodUOM=mysql_query($sqlprodUOM);

				$rowprodUOM=mysql_fetch_array($rsprodUOM);
			...
			$sqllatestlabourextra="SELECT labour_cost,extra_cost FROM packing_master WHERE 
								...
								dns_prod_code='".mysql_real_escape_string($prod_code_val)."' AND plant_name='Bhiwadi' ORDER BY datetime DESC LIMIT 0,1";

			$rslatestlabourextra=mysql_query($sqllatestlabourextra);

			$rowlatestlabourextra=mysql_fetch_array($rslatestlabourextra);
			...
			$sqlpacking .= "  dns_prod_code='".mysql_real_escape_string($prod_code_val)."'";

			$sqlpacking .= " , packing_cost='".mysql_real_escape_string(${total_packing.$prod_code_val})."'";

			$sqlpacking .= " , labour_cost='".mysql_real_escape_string($latest_labour_cost)."'";

			$sqlpacking .= " , plant_name='Bhiwadi'";

			$sqlpacking .= " , extra_cost='".mysql_real_escape_string($latest_extra_cost)."'";
			...
			$sqlupdateprodweight="UPDATE product_master SET gross_weight='".${gross_weight.$prod_code_val}."' WHERE dns_prod_code='".mysql_real_escape_string($prod_code_val)."'";

			mysql_query($sqlupdateprodweight);
	...
	if($_REQUEST['mode']=='submit_packing_bom'){
		...
		{				
			...
			$sqlpackingBOM .= "  material_name='".mysql_real_escape_string($material_name[$i])."'";

			$sqlpackingBOM .= " , prod_code='".mysql_real_escape_string($prod_code[$i])."'";

			$sqlpackingBOM .= " , usage_qty='".mysql_real_escape_string($usage_qty[$i])."'";

			$sqlpackingBOM .= " , UOM='".mysql_real_escape_string($UOM[$i])."'";

			$sqlpackingBOM .= " , multiple_single='".mysql_real_escape_string($multiple_single[$i])."'";
			...
			mysql_query($sqlpackingBOM) or die(mysql_error().".Internal error occurrs @row $csv_row_count in Packing BOM.csv.Please check.");
			...
			$sqlpackingmaterial="SELECT price,weight FROM packing_material_master WHERE material_name='".mysql_real_escape_string($material_name[$i])."' 

								ORDER BY datetime DESC LIMIT 0,1";

			$rspackingmaterial=mysql_query($sqlpackingmaterial);

			$rowpackingmaterial=mysql_fetch_array($rspackingmaterial);
...

		{
			...
			$sqlpacking .= " , packing_cost='".mysql_real_escape_string(${packing_cost.$prod_code_val})."'";
	...
	if($_REQUEST['mode']=='submit_labour_bom'){
		...
		{				
			...
			$sqllabourBOM .= "  prod_code='".mysql_real_escape_string($prod_code[$i])."'";

			$sqllabourBOM .= " , manufacturing='".mysql_real_escape_string($manufacturing[$i])."'";

			$sqllabourBOM .= " , unloading_top='".mysql_real_escape_string($unloading_top[$i])."'";

			$sqllabourBOM .= " , unloading='".mysql_real_escape_string($unloading[$i])."'";

			$sqllabourBOM .= " , empty_shifting='".mysql_real_escape_string($empty_shifting[$i])."'";

			$sqllabourBOM .= " , filling='".mysql_real_escape_string($filling[$i])."'";

			$sqllabourBOM .= " , labelling='".mysql_real_escape_string($labelling[$i])."'";

			$sqllabourBOM .= " , filed_shifting='".mysql_real_escape_string($filed_shifting[$i])."'";

			$sqllabourBOM .= " , loading='".mysql_real_escape_string($loading[$i])."'";

			$sqllabourBOM .= " , marking='".mysql_real_escape_string($marking[$i])."'";
			...
			mysql_query($sqllabourBOM) or die(mysql_error().".Internal error occurrs @row $csv_row_count in Labour BOM.csv.Please check.");
...

		{
			...
			$rslatestpackextra=mysql_query($sqllatestpackextra);

			$rowlatestpackextra=mysql_fetch_array($rslatestpackextra);
			...
			$sqlpacking .= " , packing_cost='".mysql_real_escape_string($latest_packing_cost)."'";

			$sqlpacking .= " , labour_cost='".mysql_real_escape_string(${labour_cost.$prod_code_val})."'";
	...
	if($_REQUEST['mode']=='submit_conversion'){
...

		{
			...
			$sqlconversion .= "  	prod_code='".mysql_real_escape_string($prod_code[$i])."'";

			$sqlconversion .= " , mapped_prod_code='".mysql_real_escape_string($mapped_prod_code[$i])."'";

			$sqlconversion .= " , is_flash='".mysql_real_escape_string($is_flash[$i])."'";

			$sqlconversion .= " , flash_name='".mysql_real_escape_string($flash_name[$i])."'";

			$sqlconversion .= " , add_subtract_val='".mysql_real_escape_string($add_subtract_val[$i])."'";
			...
			mysql_query($sqlconversion) or die(mysql_error().".Internal error occurrs @row $csv_row_count in conversion.csv.Please check.");
...

	{
		...
		if($countbranchdestinationfreight>=1 )

			{
				...
				$rsisplant=mysql_query($sqlisplant);

				$rowisplant=mysql_fetch_array($rsisplant);
	...
	if($_REQUEST['mode']=='submit_truck_load'){
...

		{
			...
			$sqltruckload .= "  load_qty='".mysql_real_escape_string($load_qty[$i])."'";

			$sqltruckload .= " , height='".mysql_real_escape_string($height[$i])."'";

			$sqltruckload .= " , length='".mysql_real_escape_string($length[$i])."'";

			$sqltruckload .= " , breadth='".mysql_real_escape_string($breadth[$i])."'";
			...
			mysql_query($sqltruckload) or die(mysql_error().".Internal error occurrs @row $csv_row_count in Truck load master.csv.Please check.");
	 ...
	 if($_REQUEST['mode']=='submit_base_oil'){
		...
		if(mysql_query($sqlupdatebaseoil)){
...

			{
				...
				$sqlbaseoil .= "  base_oil='".mysql_real_escape_string($base_oil[$i])."'";

				$sqlbaseoil .= " , mandatory='".mysql_real_escape_string($mandatory[$i])."'";

				$sqlbaseoil .= " , upper_limit='".mysql_real_escape_string($upper_limit[$i])."'";

				$sqlbaseoil .= " , lower_limit='".mysql_real_escape_string($lower_limit[$i])."'";
				...
				mysql_query($sqlbaseoil) or die(mysql_error().".Internal error occurrs @row $csv_row_count in Base Oil Master.csv.Please check.");
		...
		print_r($_POST);
...

			{
...

				{
					...
					$rsoilcategorycode=mysql_query($sqloilcategorycode);

					$rowoilcategorycode=mysql_fetch_array($rsoilcategorycode);

					$oil_category_code=$rowoilcategorycode['product_group_code'];

					$sqlbrokerage  = "insert into brokerage_cost SET ";

					$sqlbrokerage .= "  broker_id='".mysql_real_escape_string($broker_code[$i])."'";

					$sqlbrokerage .= " , broker_name='".mysql_real_escape_string($broker_name[$i])."'";

					$sqlbrokerage .= " , oil_category='".mysql_real_escape_string($oil_category_code)."'";

					$sqlbrokerage .= " , pack_size='".mysql_real_escape_string($pack_size[$i])."'";

					$sqlbrokerage .= " , UOM='".mysql_real_escape_string($UOM[$i])."'";

					$sqlbrokerage .= " , brokerage_cost='".mysql_real_escape_string($brokerage_cost[$i])."'";
					...
					mysql_query($sqlbrokerage) or die(mysql_error().".Internal error occurrs @row $csv_row_count in Brokerage Cost.csv.Please check.");
...

			{
				...
				{ 
				  ...
				  //print_r($data);
					...
					$rsplantvalue=mysql_query($sqlplantvalue);

					$countplantvalue=mysql_num_rows($rsplantvalue);
...

			{
				...
				{ 
				  ...
				  //print_r($data);
					...
					$rsmappedprodvalue=mysql_query($sqlmappedprodvalue);

					$countmappedprodvalue=mysql_num_rows($rsmappedprodvalue);
...

		{
			...
			{ 
			  ...
			  //print_r($data);
				...
				$rschkbroker=mysql_query($sqlchkbroker);

				$countchkbroker=mysql_num_rows($rschkbroker);
				...
				$rowchkbroker=mysql_fetch_array($rschkbroker);
...

		{
			...
			{ 
			  ...
			  //print_r($data);
				...
				$rschkprodcode=mysql_query($sqlchkprodcode);

				$countchkprodcode=mysql_num_rows($rschkprodcode);
				...
				$rowchkprodcode=mysql_fetch_array($rschkprodcode);
				...
				$rschkmappedprodcode=mysql_query($sqlchkmappedprodcode);

				$countchkmappedprodcode=mysql_num_rows($rschkmappedprodcode);
