...
if(rtrim($customer_info_val) == 'last_days_1'){
	...
	if($count>0){
		while($rowsorderquery = mysql_fetch_array($result))
		{
...
if(rtrim($customer_info_val) == 'target_ach'){
	...
	if($count>0){
		while($rowtarget = mysql_fetch_array($result))
		{
...
if(rtrim($customer_info_val) == 'proposed_sku'){
	...
	$rowquery = mysql_fetch_array($result);
			...
			$rsproposedsku=mysql_query($sqlproposedsku);
			$count=mysql_num_rows($rsproposedsku);
			if($count>0){
			while($rowproposedsku=mysql_fetch_array($rsproposedsku))
			{				
				...
				$resultprod = mysql_query($sqlqueryprod);
...
if(rtrim($customer_info_val) == 'selectretailerinfo')
  {
   ...
   /*$sqlretailerinfo="SELECT DISTINCT EM.emp_code,EM.emp_name,EM.reporting_to,RM.route_name,CM.customer_code,CM.rds_tag,CM.customer_name  FROM customer_master CM,employee_master EM,customer_route_emp_relation CRER,route_master RM  
	...
	$rsretailerinfo=mysql_query($sqlretailerinfo);
	
	$countretailerinfo=mysql_num_rows($rsretailerinfo);
	if($countretailerinfo > 0 && $phone!=''){
	$rowretailerinfo=mysql_fetch_array($rsretailerinfo);
