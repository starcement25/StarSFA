		...
		if($countbranches > 0 && strtoupper($_SESSION['nick_name'])!='PALSONS'){ //For the user of multiple branches
			$emp_code_chk_array=array();
				...
				while($rowemployeelist=mysql_fetch_array($resemployeelist))
				{
					...
					$rsinformationmonth=mysql_query($sqlinformationmonth);
					$countinformationmonth=mysql_num_rows($rsinformationmonth);
						...
						if($count==0)
						{
							...
							else
							{
								...
								else if(strtoupper($_SESSION['nick_name'])=='MAGIK')
								{
									...
									$resattactivity=mysql_query($sqlattactivity);
									$rowattactivity=mysql_fetch_array($resattactivity);
