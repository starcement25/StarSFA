...
$sql_product_details = "SELECT PM.dns_prod_code,PM.prod_desc,BM.branch_code,BM.dns_branch_code,BM.branch_name,PM.conversion_factor,
						PM.conversion_factor_two,BM.plant_name,BM.plant_code,PM.product_group_code,BM.branch_state,BM.dns_state_code 
						...
						WHERE PM.product_group_code IN(".$product_group_code.") AND PM.prod_code = MR.product_code 
						   ...
						   ORDER BY BM.plant_name,BM.branch_name,PM.prod_desc ASC";
//exit();						   
$res_product_details = mysql_query($sql_product_details);
$total_rows = mysql_num_rows($res_product_details);

if($total_rows >0){
    echo '<div class="tbl-header"><table class="border datatable" width="90%" border="0" style="border-collapse:collapse;" cellpadding="5px" align="center">
	<thead>
      <tr>
        <td colspan="15" class="TDHEAD" align="left"><b>Sale Rate Report - '.$from_date.'</b></td>
      </tr>
      <tr class="TDHEAD_SUB">
      	<td width="4%">SI</td>
		<td width="5%">Plant Code</td>
		<td width="8%">Plant Name</td>
        <td width="7%">Depot Code</td>
        <td width="6%">Depot Name</td>
		<td width="7%">Oil group</td>
        <td width="7%">Material Code</td>
        <td width="7%">Material Description</td>
        <td width="7%">Material Cost</td>
        <td width="7%">Primary Freight</td>
        <td width="7%">Packing Cost</td>
        <td width="7%">Depot Cost</td>
        <td width="7%">Margin</td>
        <td width="7%">Detention Cost</td>
        <td width="7%">Sale Rate</td>
      </tr></thead></table></div>
      <div class="tbl-content"><table class="BORDER datatable" cellpadding="0" cellspacing="0" border="1">
      <tbody>';
	$count = 1;
	$res_product_details = mysql_query($sql_product_details);
	$depot_array=array();
			...
			{					
				...
				if(strtoupper($vertical_value)=='SPECIALTY FATS'){
					$sqllooserate="SELECT oils_rate,price_generated FROM pricing_detials_formulation WHERE plant_name='".$plant_name."' 
									AND oils='".$rowprodwiseformulation['oils']."' AND vertical_value='".$_SESSION['vertical_value']."' 
				...
				$sqllooserate="SELECT oils_rate,price_generated FROM pricing_detials_formulation WHERE product_group_code='".$product_group_code."' AND 
						plant_name='".$plant_name."' AND oils='".$rowprodwiseformulation['oils']."'  
						AND vertical_value='".$_SESSION['vertical_value']."' AND SUBSTRING(datetime,1,10) ='".$from_date_check."' 
