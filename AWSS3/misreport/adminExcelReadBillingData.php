	...
	if(file_exists($_FILES['zip_file']['tmp_name']))
	{
	  ...
	  //For billing information xls
	   if(similar_file_exists("../csv/$folderName/billing information.xls")!=false)
		...
		$second=gmdate('s',strtotime('+330 minute'));
		$contentsdatetime =$year.$month.$date.$hour.$minute.$second;
			$data = new Spreadsheet_Excel_Reader("../csv/$folderName/billing information.xls");
               ...
               //echo "Sheet $i:<br /><br />Total rows in sheet $i  ".count($data->sheets[$i][cells])."<br />";
				...
				{ 
					...
					{ 
						...
						$customer_name = trim($data->sheets[$i][cells][$j][1]);
						$dns_customer_code = trim($data->sheets[$i][cells][$j][2]);
						$invoice_no = trim($data->sheets[$i][cells][$j][3]);
						$invoice_date = trim($data->sheets[$i][cells][$j][4]);
						$invoice_date_final=date('Y-m-d',strtotime(str_replace('/','-',$invoice_date)));
						$prod_desc = trim($data->sheets[$i][cells][$j][5]);
						$IMEI = trim($data->sheets[$i][cells][$j][6]);
