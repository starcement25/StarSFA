				...
				{ 
				  ...
				  $data[]=$value;
					...
					if(providing_code=='yes'){
						...
						$rschkphone=mysql_query($sqlchkphone);
						$cntchkphone=mysql_num_rows($rschkphone);
						if($cntchkphone > 0){
			  ...
			  foreach($lines as $line)
			  {
				...
				{ 
				   ...
				   $data[]=$value;
					...
					$cntstatecode=mysql_num_rows($rsstatecode);
					if($cntstatecode==0){
					...
					if($rds_tag!='')
					{
						...
						$rsrdscode=mysql_query($sqlrdscode);
						$cntrdscode=mysql_num_rows($rsrdscode);
						if($cntrdscode==0 ){
							  $GLOBALS['msg'] = "Improper rds tag @row $csv_row_count in Customer Master Haldiram.csv";
							  disphtml("main();");
							  exit;
					...
					$rschkchkcustomerphone=mysql_query($sqlchkcustomerphone);
					$cntchkchkcustomerphone=mysql_num_rows($rschkchkcustomerphone);
					...
					$rschkemplevel=mysql_query($sqlchkemplevel);
					$cntchkemplevel=mysql_num_rows($rschkemplevel);
					...
					if($cntchkemplevel==0){
						if($customer_type!='R'){
							if(providing_code=='yes'){
								...
								$rschkmultiplecust=mysql_query($sqlchkmultiplecust);
								$cntchkmultiplecust=mysql_num_rows($rschkmultiplecust);
								if($cntchkmultiplecust > 0)
								{
									$rowchkmultiplecust=mysql_fetch_array($rschkmultiplecust);
					...
					/*else
					{
						...
						$rschkempsaleaccess=mysql_query($sqlchkempsaleaccess);
						$rowchkempsaleaccess=mysql_fetch_array($rschkempsaleaccess);
					...
					else
					{
						...
						$rsfetchroute=mysql_query($sqlfetchroute);
						$countfetchroute=mysql_num_rows($rsfetchroute);
						...
						{
							$rowfetchroute=mysql_fetch_array($rsfetchroute);
					...
					else
					{
						...
						$rowselcustomerroute=mysql_fetch_array($rsselcustomerroute);
				...
				foreach($emp_code_name_array as $emp_code_name_value_next)
					{
						...
						$rsempnamechk=mysql_query($sqlempnamechk);
						$cntempcode=mysql_num_rows($rsempnamechk);
						if($cntempcode==0){
					...
					if($folderName=='HALDIRAM' || $folderName=='PRABHUJI')
					{
						...
						$rschkmultipledist=mysql_query($sqlchkmultipledist);
						$cntchkmultipledist=mysql_num_rows($rschkmultipledist);
						if($cntchkmultipledist > 0)
						{
							$rowchkmultipledist=mysql_fetch_array($rschkmultipledist);
