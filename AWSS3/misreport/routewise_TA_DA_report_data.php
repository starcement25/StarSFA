...
//if(!empty($date_array)){
	...
	$res_emp_beat = mysql_query($sql_emp_beat);
	$cnt_emp_beat=mysql_num_rows($res_emp_beat);
	if($cnt_emp_beat >0){
		$count=1;
	while($row_emp_beat = mysql_fetch_array($res_emp_beat)){
		...
		$rsTADA=mysql_query($sqlTADA);
		$rowTADA=mysql_fetch_array($rsTADA);
	...
	AND SUBSTRING(LO.date,1,10)='".$visit_date_format."' AND LO.emp_code='".$emp_code."'";
		...
		$row_total_calls=mysql_fetch_array($rs_total_calls);
