...
function login()
{
	...
	{	
		...
		else
		{
			...
			{
				$sql_emp_sale_access=mysql_fetch_array(mysql_query("SELECT sale_access FROM `employee_master` WHERE `emp_code`='".$_SESSION['admin_login']."'"));
				if(strpos(strtolower($sql_emp_sale_access['sale_access']),'vendor')!=false){
					$_SESSION['sale_access']=$sql_emp_sale_access['sale_access'];
					 header("Location: branding_verification_with_location_modified.php");
				}
				else
					header("Location: adminMain.php");
			}
			...
			{
				$sql_cust_type=mysql_fetch_array(mysql_query("SELECT cust_type FROM customer_master WHERE customer_code='".$_SESSION['admin_login']."'"));
