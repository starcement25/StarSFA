...
//if(!empty($date_array)){
    ...
    <tr class='TDHEAD'><td colspan='17' align='center'>Customer Visit & Productivity report Details</td></tr>
	 ...
	 $result = mysql_query("SELECT DISTINCT  SUBSTRING(`visit_date`,1,7) AS month FROM prev_order_counting_master WHERE DATE_FORMAT(SUBSTRING(order_no,-14,8),'%Y-%m-%d') BETWEEN '".$start_date."' AND '".$end_date."'");
    ...
    <?
	$sql_outlet_sku="SELECT DISTINCT EM.emp_name,EM.emp_code,CM.customer_name,CM.cust_class,CM.category_of_store,RM.route_name,DATE_FORMAT(SUBSTRING(POCM.order_no,-14,8),'%d') AS visit_date ,  IF( POCM.order_no LIKE 'O%', DATE_FORMAT(SUBSTRING(POCM.order_no,-14,8),'%d'), NULL ) AS productive_date, SUBSTRING(`visit_date`,1,7)  AS month,POCM.customer_code FROM customer_master CM,employee_master EM,prev_order_counting_master POCM,route_master RM WHERE POCM.customer_code=CM.customer_code AND CM.route_code=RM.route_code AND SUBSTRING(POCM.order_no,-19,5)=EM.emp_code AND DATE_FORMAT(SUBSTRING(POCM.order_no,-14,8),'%Y-%m-%d') > '2020-12-01' ".$emp_hierarchy_value_condition." AND DATE_FORMAT(SUBSTRING(POCM.order_no,-14,8),'%Y-%m-%d') BETWEEN '".$start_date."' AND '".$end_date."' ORDER BY EM.emp_name ASC,CM.customer_name ASC,MONTH(POCM.visit_date) ASC,POCM.visit_date ASC";
