                        ...
                        <tr>
                            ...
                            <table>
                                <tr>
                                    ...
                                    $cnt=0;
                                    while($rowzone=mysql_fetch_array($rszone))
                                    {
                                        $cnt++;
                                    ?>
                                        <tr>
			...
			if($survey_menu=='yes')
			{
			 ...
			 while($row_get_menu = mysql_fetch_array($res_get_menu))
			 {
				 ...
				 ${survey_id_array.$menu_id}=array();
				  ...
				  $rs_survey_existing_row_id=mysql_query($sql_survey_existing_row_id);
				   while($row_survey_existing_row_id=mysql_fetch_array($rs_survey_existing_row_id))
				   {
				  ...
				  while($row_survey_output=mysql_fetch_array($rs_survey_output))
				  {
					...
					if(strtoupper($_SESSION['nick_name'])=='DURO')
					{
						...
						else if($row_survey_output['row_id']=='RA047')
						{
							...
							$rsfacilitatortype=mysql_query($sqlfacilitatortype);
							$rowfacilitatortype=mysql_fetch_array($rsfacilitatortype);
						...
						else if($row_survey_output['row_id']=='RA085')
						{
							...
							if(strtolower($valueparts[0])!='office' && count($valueparts) > 0)
							{
								...
								$rsfacilitatorname=mysql_query($sqlfacilitatorname);
								$rowfacilitatorname=mysql_fetch_array($rsfacilitatorname);
						...
						else if($row_survey_output['row_id']=='RA090')
						{
							...
							$rstableview=mysql_query($sqltableview);
							$rowtableview=mysql_fetch_array($rstableview);
