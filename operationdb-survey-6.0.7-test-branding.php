...
if(count($survey_array)>0)
{
	for($x=0;$x<count($survey_array);$x++){
		...
		else
		{
			...
			$sqlmenuname="SELECT display_name FROM survey_input_test WHERE row_id='".$surveyheader_menu_name."'";
	...
	if(count($survey_details_array)>0)
	{
		for($i=0;$i<count($survey_details_array);$i++){
			...
			$survey_input_details="SELECT display_name,layout_name,insert_table_detail FROM survey_input_test WHERE row_id='".$row_id."' AND action_id='".trim($action_id)."'";
 ...
 /* -------------------------------------END QUERY FOR SURVEY------------------------------------------------------------------------------------*/
	...
	$url = "http://www.acedns.in/acednsproduct/operationdb-survey-6.0.7-test-branding.php?nick_name=$nick_name&emp_code=$emp_code&last_update_time=$last_update_time&last_git_master_update_time=$last_git_master_update_time&last_loyalty_purchase_update_time=$last_loyalty_purchase_update_time";
