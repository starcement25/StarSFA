...
if($successval=="1"){
	...
	if($incremental_download=='no'){
		...
		if(sale=='yes')
		{
			...
			$sqlquerygit="SELECT COUNT(GIT.grn_no) AS total_git FROM goods_in_transit GIT  WHERE GIT.rds_code='".$rds_code."'";
	...
	else
	{
		...
		if(sale=='yes')
		{
			...
			$login_condition=" AND UNIX_TIMESTAMP(GIT.download_time) > UNIX_TIMESTAMP('".$last_update_time."')";
			
			$sqlquerygit="SELECT COUNT(GIT.grn_no) AS total_git FROM goods_in_transit GIT  WHERE 
			 			GIT.transaction_type='ST' AND GIT.rds_code='".$rds_code."' AND UNIX_TIMESTAMP(GIT.download_time) > 
	...
	$url = "http://www.acedns.in/acednsproduct/datadownloaddictionary-4.0.7.php?nick_name=$nick_name&emp_code=$emp_code&device_id=$device_id&last_update_time=$last_update_time&incremental_download=$incremental_download";
	insertapilog($datetime,$emp_code,$url,$nick_name);
	/*$config = 'api_calllog.txt';
	$file=fopen($config,"r+");
	$date = date("F j, Y");
	$time = date("H:i:s");
	$newuser ="[$date $time]"."http://www.acedns.in/acednsproduct/datadownloaddictionary-4.0.7.php?nick_name=$nick_name&emp_code=$emp_code&device_id=$device_id&last_update_time=$last_update_time&incremental_download=$incremental_download"."\r\n";
