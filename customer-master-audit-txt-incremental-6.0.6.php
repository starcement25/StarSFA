...
$countbranches=mysqli_num_rows($rsbranches);
	...
	if(modified_customer_emp_route=='yes')
	{
		...
		$contentsrowcolumn=$countcustomerroute.'¥'.'24';
			...
			while($rowscustomerroute = mysqli_fetch_assoc($resultcustomerroute))
			{
				...
				$sqlcustomer="SELECT  customer_name,emp_code,current_balance,credit_limit,black_list,acedns,
						TD,cust_type,rds_tag,sauda_validity_period,address,phone_no,pin,landline_no,owner_name,owner_phone,
						 cust_class,weekly_closing_day,coverage_type,TIN,PAN FROM customer_master 
	...
	{
		 if($nick_name=='SELVEL' || $nick_name=='RKBKT'){
...
$count=mysqli_num_rows($result);
	...
	$url = "http://www.acedns.in/acednsproduct/customer-master-audit-txt-incremental-6.0.6.php?nick_name=$nick_name&emp_code=$emp_code&last_update_time=$last_update_time&data_download_time=$data_download_time&incremental_download=$incremental_download";
