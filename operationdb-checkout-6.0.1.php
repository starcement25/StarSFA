...
require_once("misreport/include/PHPMailer/src/PHPMailer.php");
require_once("misreport/include/PHPMailer/src/SMTP.php");
require_once("misreport/include/PHPMailer/src/Exception.php");
...
$checkout_TA_DA_mode = "*ROOT*CHECKOUT*LOCATION*TA_DA_MODE";
...
class xml_checkout
{
	var $checkout_emp_code, $checkout_trans_id, $checkout_latt, $checkout_longi, $checkout_date, $checkout_TA_DA_mode, $checkoutdata_emp_code, $checkoutdata_date;
}
...
{
	global $current_tag, $checkout_emp_code, $checkout_trans_id, $checkout_latt, $checkout_longi, $checkout_date, $checkout_TA_DA_mode, $checkoutdata_emp_code,
	...
	if (substr($current_tag, 0, 14) == '*ROOT*CHECKOUT') {
		switch ($current_tag) {
			...
			case $checkout_date:
				...
				break;
			case $checkout_TA_DA_mode:
				$checkout_array[$counter]->checkout_TA_DA_mode = $data;
				break;
...
// print_r(count($checkout_array));die;
...
if (count($checkout_array) > 0) {
	...
	for ($x = 0; $x < count($checkout_array); $x++) {
		...
		$checkout_TA_DA_mode = $checkout_array[$x]->checkout_TA_DA_mode;
		$checkout_date_DCR = substr($checkout_date, 0, 10);
		//print_r($checkout_emp_code);die;
		...
		//echo $countchkchecklocation;die;
		...
		if ($countchkchecklocation > 0) {
			...
			if (strtoupper($nick_name) == 'ABDOS') {
				$TA_DA_condiion = " TA_DA_mode	='" . $checkout_TA_DA_mode . "',";
			} else {
				$TA_DA_condiion = '';
			}
			...
			$sqlinsertchecklocation = "INSERT INTO location SET emp_code='" . $checkout_emp_code . "',
									...
									latt='" . $checkout_latt . "',
									longi='" . $checkout_longi . "',
									date='" . $checkout_date . "'," . $TA_DA_condiion . "
									updatetime='" . $location_date . "'";

			//For Insert into the checkout table for new trans id
			if ($checkoutdata_date == "") {
				$checkoutdata_date = date("Y-m-d H:i:s");
			}
			...
			if (mysqli_query($link, $sqlinsertchecklocation) && mysqli_query($link, $sqlinsertcheckout)) {
				...
				$sqlempname = "SELECT emp_name,vertical_value,branch_code,reporting_to,email,dns_emp_code FROM employee_master WHERE emp_code='" . $emp_code . "'";
				...
				if (strtoupper($nick_name) == 'DURO') {
					...
					$sqlupdateloc = "UPDATE location set address='" . addslashes($address) . "' WHERE trans_id='" . $checkout_trans_id . "'";
					$rsupdateloc = mysqli_query($link, $sqlupdateloc);
				}
				if (strtoupper($nick_name) == 'MAGIK') {
					$address = getReverseGeoAdd($checkout_latt, $checkout_longi);
					$sqlupdateloc = "UPDATE location set address='" . addslashes($address) . "' WHERE trans_id='" . $checkout_trans_id . "'";
					$rsupdateloc = mysqli_query($link, $sqlupdateloc);
				}
				if (strtoupper($nick_name) == 'STAR') {
					$sqlchkattinfo = "SELECT Emp_Id FROM  t_att_checkout_info WHERE Emp_Id='" . $rowempname['dns_emp_code'] . "' AND SUBSTRING(Entry_Date,1,10)='" . substr($checkout_date, 0, 10) . "'";
					$reschkattinfo = mysqli_query($link, $sqlchkattinfo);
					$countchkattinfo = mysqli_num_rows($reschkattinfo);
					if ($countchkattinfo > 0) {
						$sqlupattinfo = "UPDATE t_att_checkout_info SET CheckOUT='" . substr($checkout_date, 11, 8) . "' WHERE 
												Emp_Id='" . $rowempname['dns_emp_code'] . "' AND Emp_Name='" . $rowempname['emp_name'] . "' AND Entry_Date='" . substr($checkout_date, 0, 10) . "'";

						$rsupattinfo = mysqli_query($link, $sqlupattinfo);
					}
				}
				...
				$emp_own_email = $rowempname['email'];
				...
				if (!empty($checkout_email)) {
					$status = send_the_mail($checkout_email, $checkoutemailsubj, $checkoutmailbody, $filepath);
					if ($status === "TRUE") {
...
if ($flag == 5 || $flag == 6) {
	...
	if (need_DCR == 'yes' && DCR_checkout == 'yes' && in_array($curdateserver, $checkout_date_array)) {
		//For HALDIRAM
		if (strtoupper($nick_name) == 'HALDIRAM' || strtoupper($nick_name) == 'RUPA' ||  strtoupper($nick_name) == 'ARCHITA') {
			...
			if ($order_header_total_rows > 0) {
				...
				for ($cntdist = 0; $cntdist < count($distributor_array); $cntdist++) {
					...
					} else if (strtoupper($nick_name) == 'ARCHITA') {
						$email = $distributor_email_array[$cntdist] . ',' . $emp_own_email . ',sales@archita.in';
					} else {
...
$url = APICALLLOGURL . "/operationdb-checkout-6.0.1.php?nick_name=$nick_name&emp_code=$emp_code&last_update_time=$last_update_time";
