...
else
{
	...
	if(modified_customer_emp_route=='yes' && $nick_name!='OSHEA' && $nick_name!='HALDIRAM' && $nick_name!='PRAKASH' && $nick_name!='ARCHITA' && $nick_name!='PRABHUJI' && $nick_name!='PRABHUJI' && $nick_name!='AJANTA' && $nick_name!='EDIBLE' && $nick_name!='ASL' && $nick_name!='DNVFOODS' && $nick_name!='DNV')
	{
		...
		else if(strtoupper($nick_name)=='STAR' && ($sale_access_emp=='BRANDING VERIFICATION' || $sale_access_emp=='BRANDING VERIFICATION VENDOR'))
		{
			...
			$sqlquerycustomerroute="SELECT DISTINCT RM.route_code FROM customer_route_emp_relation RM,customer_master CM WHERE 
		...
		if($countcustomerroute>0){
			...
			if($nick_name=='SKIPPER' || $nick_name=='DNV' || $nick_name=='GANESH' || $nick_name=='PALSONS' || route_plan=='yes')
			{
				if($nick_name=='PALSONS')
				{
					$sqlrouteextra="SELECT  route_code,route_name,branch_code FROM route_master WHERE route_name IN('Office Visit','Leave Request','Meeting','Admin Work')";
					...
					$countcustomerroute=$countcustomerroute+4;
				}
				else if($nick_name=='SUPERSHAKTI')
				{
					$sqlrouteextra="SELECT  route_code,route_name,branch_code FROM route_master WHERE route_name IN('Office Visit','Leave Request','TRANSIT')";
					$rsrouteextra=mysqli_query($link,$sqlrouteextra);
					while($rowrouteextra=mysqli_fetch_assoc($rsrouteextra))
					{
						$contents  = (($rowrouteextra['route_code']!='')?$rowrouteextra['route_code']: ' ')."^";
					...
					$countcustomerroute=$countcustomerroute+3;
				}
				else
				{
		...
		|| $nick_name=='AJANTA' || $nick_name=='EDIBLE' || $nick_name=='ASL' || strtoupper($nick_name)=='DNVFOODS' || strtoupper($nick_name)=='DNV')
	{
		...
		$countdistributorroute=mysqli_num_rows($resultquerydistributorroute);
				...
				$sqlquerycustomerroute="SELECT DISTINCT RM.route_code FROM customer_route_emp_relation RM WHERE 
									route_code IN(SELECT route_code FROM route_master) AND acedns='Y' AND
									customer_code='".$emp_code."' ".$login_condition_one." ";
				$resultcustomerroute = mysqli_query($link,$sqlquerycustomerroute);
				$countcustomerroute=mysqli_num_rows($resultcustomerroute);
				}
				if(strtoupper(substr($emp_code,0,1))=='B')
				{
					$sqlquerycustomerroute="SELECT DISTINCT RM.route_code FROM customer_route_emp_relation RM,customer_broker_relation CBR WHERE 
										RM.route_code IN(SELECT route_code FROM route_master) AND RM.customer_code=CBR.customer_code AND 
										CBR.broker_code='".$emp_code."'  AND CBR.acedns='Y'  ".$login_condition_one." ";
					$resultcustomerroute = mysqli_query($link,$sqlquerycustomerroute);
					$countcustomerroute=mysqli_num_rows($resultcustomerroute);
