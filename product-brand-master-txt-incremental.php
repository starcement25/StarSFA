...
else
{
	if($nick_name=='RUPA')
	{
		$sqlquery="SELECT DISTINCT PBM.* FROM product_brand_master PBM WHERE PBM.is_download='yes'
				 ".$condition_one." ".$login_condition." ORDER BY PBM.product_brand_name ASC";
	}
	else
	{
		$sqlquery="SELECT DISTINCT PBM.* FROM product_brand_master PBM,product_master PM  WHERE 
				PM.product_brand_code=PBM.product_brand_code ".$condition_one." ".$login_condition." ORDER BY PBM.product_brand_name ASC";
	}
}
...
$count=mysqli_num_rows($result);
	...
	if($count>0){
		...
		while($rowproductbrand = mysqli_fetch_assoc($result))
		{
			/*$contents.="<data>";
			$contents .='
	...
	$url = "http://www.acedns.in/acednsproduct/product-brand-master-txt-incremental.php?nick_name=$nick_name&emp_code=$emp_code&last_update_time=$last_update_time&incremental_download=$incremental_download";
	...
	$date = date("F j, Y");
	$time = date("H:i:s");
	$newuser ="[$date $time]"."http://www.acedns.in/acednsproduct/product-brand-master-txt-incremental.php?nick_name=$nick_name&emp_code=$emp_code&last_update_time=$last_update_time&incremental_download=$incremental_download"."\r\n";
