...
function csv_upload(){
	...
	/*else
	{
		...
		//For Employee CSV
	if(similar_file_exists("csv/$folderName/Employee master krishi.csv")!=false)
	{
		$filename=similar_file_exists("csv/$folderName/Employee master krishi.csv");
		$rec_count = 0;
		$ins_count = 0;
		$err = "";
			$lines = file($filename);
			foreach($lines as $line)
			{
				...
				{ 
					...
					$employee_code=trim($data[1]);
					$reporting_to=trim($data[2]);
					
					$sqlempnamechk="SELECT emp_code FROM employee_master WHERE emp_code='".addslashes($employee_code)."'";
					...
					else
					{
						...
						$sqlupdate .= "  SET reporting_to='".$reporting_to."'";
						$sqlupdate .= " , emp_name='".$employee_name."'";
						$sqlupdate .= "  WHERE emp_code='".addslashes($employee_code)."'";
				...
				{ 
				  ...
				  //print_r($data);
					...
					else
					{
						...
						if(($credit_limit_db==$credit_limit) && ($route_code_db!=$route_code || $emp_code_db!=$emp_code || $current_balance_db!=$current_balance || $acedns_db!=$acedns || $black_list_db!=$black_list || $TD_db!=$TD || $customer_type_db!=$customer_type 
						|| $rds_tag_db!=$rds_tag || $branch_code_db!=$branch_code || $sauda_validity_period_db!= $sauda_validity_period))
						{
