<?php	
	...
	$rsselemphighlevel=mysqli_query($link,$sqlselemphighlevel,$link);
	while($rowselemphighlevel=mysqli_fetch_assoc($rsselemphighlevel))
	{
		...
		$rsregdetails=mysqli_query($link,$sqlregdetails,$link);
...
array('notification_id' =>$notification_id, 'notification_type' => $notification_type, 'sender_id' => 'CRONJOB', 'body' => $message); 
		...
		if($successval==1)
		 {
				...
				mysqli_query($link,$sqlnotificationmaster,$link) or die(mysqli_error()." Error in notificatin insertion.");
				...
				mysqli_query($link,$sqlnotification,$link) or die(mysqli_error()." Error in notification insertion.");
		 }
	}
	mysqli_close($link);
	...
	mysqli_select_db(DB,$linkone) or die("could not connect the database for invalid nick name");
	...
	echo $sqlselemphighlevel="SELECT emp_code,emp_name FROM employee_master WHERE emp_code IN(SELECT DISTINCT reporting_to FROM employee_master)";
	$rsselemphighlevel=mysqli_query($link,$sqlselemphighlevel,$linkone);
	while($rowselemphighlevel=mysqli_fetch_assoc($rsselemphighlevel))
	{
		$emp_code=$rowselemphighlevel['emp_code'];
		$emp_name=$rowselemphighlevel['emp_name'];
		...
		$collapseKey=rand();
		$sqlregdetails="SELECT registrationid FROM changepassword WHERE emp_code='".$emp_code."'";
		$rsregdetails=mysqli_query($link,$sqlregdetails,$linkone);
		$rowregdetails=mysqli_fetch_assoc($rsregdetails);
		$registrationid=$rowregdetails['registrationid'];
		//Title of the Notification.
		$title = "";
		...
		if($successval==1)
		 {
				...
				$sqlnotificationmaster .= " ,message='".$message."'";
				$sqlnotificationmaster .= " ,transferred='YES'";
				mysqli_query($link,$sqlnotificationmaster,$linkone) or die(mysqli_error()." Error in notificatin insertion.");
				$sqlnotification  = "INSERT INTO notification_ack_relation ";
				$sqlnotification .= " SET notification_id='".$notification_id."'";
				$sqlnotification .= " ,receiver_id='".$emp_code."'";
				mysqli_query($link,$sqlnotification,$linkone) or die(mysqli_error()." Error in notification insertion.");
		 }
	}*/
?>	
