...
$brand_image_url = $server_url."assets/img/common/logo.png";

$sql3 = "select * from $customer_master_STAR where `latitude`!='' and `longitude`!='' group by `dns_customer_code` order by `customer_name` asc";
...
.jplist-map-panel .jplist-marker-popup .arrow_on_st_dtls{
    ...
    <body>
        ...
        <div class="fcuk-header container-fluid" style="background-color:#eeeeee;">
        	<div class="row">
            	<div class="col-md-4 col-sm-12 logo" style="height: 85px; padding-top: 6px;" >
...
if($totres2>0){
	while($row3=mysql_fetch_assoc($res3)){
		$dns_customer_code = $row3["dns_customer_code"];
		$the_latitude = $row3["latitude"];
		$the_longitude = $row3["longitude"];			
		$customer_name = $row3["customer_name"];
		$address = $row3["address"] ? trim($row3["address"]) : "";
		$phone_no = $row3["phone_no"] ? trim($row3["phone_no"]) : "";
		if($phone_no!=""){
		$phone_no = 'Phone No: '.$phone_no;	
		}

		echo '<div class="store" data-latitude="'.$the_latitude.'" data-longitude="'.$the_longitude.'" style="position:relative;"><img src="'.$server_url.'img/arrow_image_for_brand_locator.png" class="arrow_on_st_dtls"/>
