...
if($countbranches>1 && vertical_fields=='yes')
{
	$branch_vertical_condition=' AND c1.branch_code=em.branch_code';
}
else if($countbranches>1 && vertical_fields=='no')
{
	$branch_vertical_condition=' AND c1.branch_code=em.branch_code';
}
else if($countbranches==0 && vertical_fields=='yes')
{
	$branch_vertical_condition='';
}
else
{
	$branch_vertical_condition='';
	 ...
	 if($emp_code!='C0007'){
	 $sqlquery="SELECT DISTINCT c1.customer_code, c1.customer_name, c1.route_code,c1.emp_code,c1.current_balance,
	 			c1.credit_limit,c1.black_list,c1.acedns,c1.TD 
...
$count=mysqli_num_rows($result);
	...
	$url = "http://www.acedns.in/acednsproduct/customer-master-txt-incremental.php?nick_name=$nick_name&emp_code=$emp_code&last_update_time=$last_update_time&incremental_download=$incremental_download";
	insertapilog($datetime,$emp_code,$url,$nick_name);
	/*$config = 'api_calllog.txt';
	$file=fopen($config,"r+");
	$date = date("F j, Y");
	$time = date("H:i:s");
	$newuser ="[$date $time]"."http://www.coralindia.com/dev/acednsproduct/customer-master-txt-incremental.php?nick_name=$nick_name&emp_code=$emp_code&last_update_time=$last_update_time&incremental_download=$incremental_download"."\r\n";
