...
$emp_code = urlencode(trim($_GET['emp_code']));
...
foreach ($data['data'] as $row) {
    ...
    $cur_target        =(int) trim($row['current_target']);
    $cur_ach           =(int) trim($row['current_ach']);
    $pre_target        =(int) trim($row['previous_target']);
    $pre_ach           =(int) trim($row['previous_ach']);
