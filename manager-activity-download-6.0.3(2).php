...
$count=mysql_num_rows($resultlocation);
	if($count>0){
		...
		while($rowlocation = mysql_fetch_array($resultlocation))
		{
		   ...
		   $rscustomervisit=mysql_query($sqlcustomervisit);
		   $rowcustomervisit=mysql_fetch_array($rscustomervisit);
			...
			if(strtoupper($nick_name)=='SYLVAN')
		   {
			  $sqlsurvey="SELECT COUNT(survey_id) AS tot_survey FROM survey_header WHERE 
			   ...
			   $rssurvey=mysql_query($sqlsurvey);
			   $rowsurvey=mysql_fetch_array($rssurvey);
	...
	$url = "http://salesmpower.acedns.in/manager-activity-download-6.0.2.php?nick_name=$nick_name&emp_code=$emp_code&dateval=$currentdate&timeval=$current_time";
