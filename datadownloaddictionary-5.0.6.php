...
if($successval=="1"){
	...
	{
		$emp_val_condition_audit=" AND CRER.emp_code='".$emp_code."'";
		if(employeewise_hierarchy=='yes'){
			$employee_hierarchy=return_employee_hierarchy($emp_code);
			$emp_val_rds=' AND (CRER.emp_code IN('.$employee_hierarchy.')';
			$emp_val_mis=' AND (emp_code IN('.$employee_hierarchy.')';
		}
		else
		{
			$emp_val_rds=" AND CRER.emp_code='".$emp_code."'";
			$emp_val_mis=" AND emp_code='".$emp_code."'";
		}
		if(sale=='yes')
		{
			...
			if($emp_code_list!='')
			{
				$emp_val_rds.=' OR CRER.emp_code IN('.$emp_code_list.'))';
				$emp_val_mis.=' OR emp_code IN('.$emp_code_list.'))';
			}
			else
			{
				$emp_val_rds.=')';
	...
	if($incremental_download=='no'){
		...
		if(stk_audit=='yes'){
			$sqlprevstkcnt="SELECT COUNT(PSCM.customer_code) AS total_prev_stk FROM prev_stock_counting_master PSCM,
							customer_master CM,customer_route_emp_relation CRER
							WHERE CM.customer_code=PSCM.customer_code  AND CM.customer_code=CRER.customer_code ".$emp_val_condition_audit."";
		...
		$sqlrds="SELECT COUNT(RD.rds_code) AS total_rds FROM rds_master RD,customer_route_emp_relation CRER 
				WHERE CRER.customer_code=RD.rds_code  ".$emp_val_rds."";
	...
	else
	{
		...
		{
			$emp_val_condition=" AND CRER.emp_code='".$emp_code."'";
		}
		...
		if(in_array('route_master',$need_update_table_array))
		{
			...
			$sqlroutecnt="SELECT COUNT(RM.route_code) AS total_route FROM route_master RM,customer_route_emp_relation CRER 
							WHERE CRER.route_code=RM.route_code AND UNIX_TIMESTAMP(RM.download_time) > UNIX_TIMESTAMP('".$last_update_time."') ".$emp_val_condition."";
		...
		if(in_array('customer_master',$need_update_table_array))
		{
			...
			$sqlcustomercnt="SELECT COUNT(CM.customer_code) AS total_customer FROM customer_master CM,customer_route_emp_relation CRER 
							WHERE CRER.customer_code=CM.customer_code AND UNIX_TIMESTAMP(CM.download_time) > UNIX_TIMESTAMP('".$last_update_time."') 
							".$emp_val_condition."";
		...
		if(credit_limit=='yes'){
			$sqlcustomercreditcnt="SELECT COUNT(CM.customer_code) AS total_customer_credit FROM customer_master CM,customer_route_emp_relation CRER 
								 WHERE CRER.customer_code=CM.customer_code AND UNIX_TIMESTAMP(CM.download_time_credit_limit) > 
								 UNIX_TIMESTAMP('".$last_update_time."') ".$emp_val_condition."";
		...
		if(stk_audit=='yes'){
			...
			$sqlprevstkcnt="SELECT COUNT(PSCM.customer_code) AS total_prev_stk FROM prev_stock_counting_master PSCM,
							customer_master CM,customer_route_emp_relation CRER 
							WHERE CM.customer_code=PSCM.customer_code AND CM.customer_code=CRER.customer_code
		...
		if(in_array('rds_master',$need_update_table_array))
			...
			$sqlrds="SELECT COUNT(RD.rds_code) AS total_rds FROM rds_master RD,customer_route_emp_relation CRER 
					WHERE CRER.customer_code=RD.rds_code AND 
					UNIX_TIMESTAMP(RD.download_time) > UNIX_TIMESTAMP('".$last_update_time."') ".$emp_val_rds."";
			$rsrds=mysqli_query($link,$sqlrds);
			$rowrds=mysqli_fetch_assoc($rsrds);
			$rdscnt=$rowrds['total_rds'];
	...
	$url = "http://www.acedns.in/acednsproduct/datadownloaddictionary-5.0.5.php?nick_name=$nick_name&emp_code=$emp_code&device_id=$device_id&last_update_time=$last_update_time&incremental_download=$incremental_download";
	insertapilog($datetime,$emp_code,$url,$nick_name);
	/*$config = 'api_calllog.txt';
	$file=fopen($config,"r+");
	$date = date("F j, Y");
	$time = date("H:i:s");
	$newuser ="[$date $time]"."http://www.acedns.in/acednsproduct/datadownloaddictionary-5.0.5.php?nick_name=$nick_name&emp_code=$emp_code&device_id=$device_id&last_update_time=$last_update_time&incremental_download=$incremental_download"."\r\n";
