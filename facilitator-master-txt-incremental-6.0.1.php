...
$sqlquery="SELECT f_code,emp_code,facilitator_name,f_type,firm_name,f_address,f_pin,f_area,f_sub_area,mobile_no,email_id,dob,
		annniversary,acedns,branch_code,check_in_date,f_category FROM facilitator_master WHERE branch_code='".$branch_code."'
			".$login_condition." ORDER BY facilitator_name ASC";			
$result = mysqli_query($link,$sqlquery);
$count=mysqli_num_rows($result);
	$cnt=1;
	$contentsrowcolumn  =$count.'¥'.'17';
	if($count>0){
	...
	$url = APICALLLOGURL."/facilitator-master-txt-incremental-6.0.1.php?nick_name=$nick_name&emp_code=$emp_code&last_update_time=$last_update_time&incremental_download=$incremental_download";
