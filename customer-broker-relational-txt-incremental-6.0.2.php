...
if($incremental_download=='no')
{
	...
	$login_condition_customer=" AND CBR.acedns='Y'";
}
else
{
	$login_condition=" AND UNIX_TIMESTAMP(CBR.download_time) > UNIX_TIMESTAMP('".$last_update_time."')";
	$login_condition_customer=" AND UNIX_TIMESTAMP(CBR.download_time) > UNIX_TIMESTAMP('".$last_update_time."')";
}
...
$sqlquery="SELECT  DISTINCT CBR.customer_code,CBR.broker_code,CBR.acedns,CBR.mapped_broker FROM customer_broker_relation CBR 
			WHERE CBR.broker_code='".$emp_code."' AND CBR.acedns!='N' ".$login_condition_customer.""; 
}
if(strtoupper(substr($emp_code,0,1))=='C')
{
$sqlquery="SELECT  DISTINCT CBR.customer_code,CBR.broker_code,CBR.acedns,CBR.mapped_broker FROM customer_broker_relation CBR 
			WHERE CBR.customer_code='".$emp_code."' AND CBR.acedns!='N' ".$login_condition_customer.""; 
}
