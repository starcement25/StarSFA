...
{
   $sqlquery="SELECT CM.customer_name,CM.customer_code,SH.sauda_no,DM.qty,DM.amount,PM.prod_desc,DM.sku_code,DM.is_approved FROM customer_master CM,sauda_header SH,DO_master DM,product_master PM where DM.sku_code=PM.prod_code and SH.customer_code=CM.customer_code AND SH.sauda_no=DM.sauda_no AND DM.qty > 0
   AND SH.sauda_no='".$sauda_no."' ORDER BY CM.customer_name ASC
";
}
...
if(strtoupper(substr($emp_code,0,1))=='B')
{
	...
	if($count>0){
		...
		while($rowDOtrans = mysqli_fetch_assoc($result))
		{
			...
			$prod_desc=$rowDOtrans['prod_desc'];
			$sku_code=$rowDOtrans['sku_code'];
			$is_approved=$rowDOtrans['is_approved'];
			if($is_approved =='yes') $is_approved='true';
			...
			$contents  .= (($is_approved!='')?$is_approved: ' ');
	...
	$url = APICALLLOGURL."/bargain-downlod-mode-sku-wise-6.0.0.php?nick_name=$nick_name&emp_code=$emp_code&mode=$mode&sauda_no=$sauda_no&from_date=$from_date&to_date=$to_date";

	insertapilog($datetime,$emp_code,$url,$nick_name);

	header("Content-type: application/text"); 

	header("Content-Disposition: attachment; filename=bargain_transaction_mode_sku_wise.txt");
