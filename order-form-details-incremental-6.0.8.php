...
$count=mysqli_num_rows($result);
	...
	if($count>0){
		...
		while($rowsorderformdetails = mysqli_fetch_assoc($result))
		{
				...
				$contents .='<order_form_id><![CDATA['.mb_convert_encoding($rowsorderformdetails['order_form_id'], 'UTF-8', 'UTF-8').']]></order_form_id>
							...
							<add_customer_branch><![CDATA['.mb_convert_encoding($rowsorderformdetails['add_customer_branch'], 'UTF-8', 'UTF-8').']]></add_customer_branch>
							';	
				$contents.="</data>";
				//echo $cnt++;
		}
	...
	$url = "http://www.acedns.in/acednsproduct/order-form-details-incremental-6.0.8.php?nick_name=$nick_name&last_update_time=$last_update_time&incremental_download=$incremental_download&mode=$mode";
