...
$countbranches=mysqli_num_rows($rsbranches);
 ...
 {
	 $sqlquery="SELECT DISTINCT ANY_VALUE(c1.customer_code) AS customer_code,ANY_VALUE(c1.credit_limit) AS credit_limit, ANY_VALUE(c1.customer_name) AS customer_name FROM customer_master c1 WHERE ".$emp_hierarchy_condition." ".$login_condition."  ORDER BY customer_name ASC"; 
 }
 else if($emp_code=='C0007')
 {
	$sqlquery="SELECT DISTINCT ANY_VALUE(c1.customer_code) AS customer_code,ANY_VALUE(c1.credit_limit) AS credit_limit, ANY_VALUE(c1.customer_name) AS customer_name FROM customer_master c1 WHERE 1  ".$login_condition." ORDER BY customer_name ASC";
 }
 else
 {
 	$sqlquery="SELECT DISTINCT ANY_VALUE(c1.customer_code) AS customer_code,ANY_VALUE(c1.credit_limit) AS credit_limit, ANY_VALUE(c1.customer_name) AS customer_name FROM customer_master c1 WHERE ".$emp_hierarchy_condition." ".$login_condition." ORDER BY customer_name ASC"; 
...
$count=mysqli_num_rows($result);
	...
	$url = APICALLLOGURL."/customer-master-credit-limit-incremental-6.0.0.php?nick_name=$nick_name&emp_code=$emp_code&last_update_time=$last_update_time&data_download_time=$data_download_time&incremental_download=$incremental_download";
	...
	$date = date("F j, Y");
	$time = date("H:i:s");
	$newuser ="[$date $time]"."http://www.acedns.in/acednsproduct/customer-master-credit-limit-incremental-6.0.0.php?nick_name=$nick_name&emp_code=$emp_code&last_update_time=$last_update_time&data_download_time=$data_download_time&incremental_download=$incremental_download"."\r\n";
