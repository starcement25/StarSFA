...
$emp_hierarchy_condition=' AND SUBSTRING(DT.DO_no,3,5) IN('.$employee_hierarchy.')';
$date_condition = "1"; 
if($mode=='customertoday')
{
	$date=date('Y-m-d');
	$date_condition = " DATE_FORMAT(SUBSTRING(DT.DO_no,-14,8),'%Y-%m-%d') = '".$date."' ";
}
else if($mode=='customermtd')
{
	$date_condition="  YEAR(DATE_FORMAT(SUBSTRING(DT.DO_no,-14,8),'%Y-%m-%d')) = YEAR(CURDATE()) AND MONTH(DATE_FORMAT(SUBSTRING(DT.DO_no,-14,8),'%Y-%m-%d')) = MONTH(CURDATE()) ";
}
else if($mode=='customercustom')
{
	$date_condition="  DATE_FORMAT(SUBSTRING(DT.DO_no,-14,8),'%Y%m%d') <='".$to_date."' 
						AND DATE_FORMAT(SUBSTRING(DT.DO_no,-14,8),'%Y%m%d') >='".$from_date."'";
}

if(strtoupper(substr($emp_code,0,1))=='E')
{
   $sqlquery="SELECT CM.customer_name,CM.customer_code,count(DISTINCT DT.DO_no) as DO_no_count,SUM(DT.DO_qty) AS qty, GROUP_CONCAT(DT.DO_amount) as amount FROM customer_master CM,DO_transaction DT where DT.customer_code=CM.customer_code AND ".$date_condition.$emp_hierarchy_condition." GROUP BY DT.customer_code ORDER BY CM.customer_name ASC
";
}
...
//echo"<pre>";print_r($sqlquery);die;
	...
	if($count>0){
		...
		{
			$DO_no_count=$rowDOtrans['DO_no_count'];
			$qty=$rowDOtrans['qty'];
			...
			$contents  .= (($DO_no_count!='')?$DO_no_count: ' ')."^";
			$contents  .= (($qty!='')?$qty: ' ')."^";
	...
	$url = APICALLLOGURL."/DO-transaction-downlod-mode-wise-6.0.0.php?nick_name=$nick_name&emp_code=$emp_code&mode=$mode&from_date=$from_date&to_date=$to_date";

	insertapilog($datetime,$emp_code,$url,$nick_name);

	header("Content-type: application/text"); 

	header("Content-Disposition: attachment; filename=DO_transaction_modewise.txt");
