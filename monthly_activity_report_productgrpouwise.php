...
if(tagged_distributor_for_order == 'yes'){
	$custtype_select_data = "<select name=\"cust_type\" id=\"cust_type\">
							  ...
							  <option value=\"secondary\">Both</option>
							</select>";
}
else{
	$custtype_select_data = "<select name=\"cust_type\" id=\"cust_type\">
...
function exporttocsv(divid)
{
	...
	a.download = 'SKU Wise Report' + postfix + '.xls';
...
<table width="40%" class="border" style="border-collapse:collapse;" border="1" cellpadding="4">
  <tr  class="TDHEAD">
  	<td colspan="2" align="center">SKU Wise Report</td>
  </tr>
  <tr class="TDHEAD_SUB">
    ...
    <td align="left">
		...
		FROM employee_master WHERE ".$emp_hierarchy_condition." AND SUBSTRING_INDEX( vertical_value, ',', -1 ) != '' 
		 AND vertical_value IN(SELECT DISTINCT vertical_value FROM employee_master 
  ...
  <!--tr class="TDHEAD_SUB">
    ...
    <option value="valuewise">Value Wise</option>
    <option value="volumewise">Volume Wise</option>
	</select>
    </td>
  </tr-->
   <tr>
...
function get_data(){
	...
	{
		alert("Start date and End date moth will be same");
		return false;
	}
	...
	GenericAjaxFunction('monthly_activity_data_productgroupwise.php?emp_code='+emp_code+'&start_date='+start_date+'&end_date='+end_date+'&vertical='+vertical+'&cust_type='+cust_type,'display',0);
}
</script>
<?php } ?>