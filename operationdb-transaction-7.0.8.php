...
if($nick_name=='ABDOS' || $nick_name=='ABDOST'  || $nick_name=='CORAL' || $nick_name=='PALSONS' ||  strtoupper($nick_name)=='STAR' || strtoupper($nick_name)=='SYLVAN' || strtoupper($nick_name)=='GOLDSTONET' || strtoupper($nick_name)=='SUPERSHAKTI')
{
...
if(count($order_array)>0)
{
	for($x=0;$x<count($order_array);$x++){
		...
		//echo 'sdsdsdsddsdsdsdsdsdsd'.$order_date=$order_array[$x]->order_date;
		...
		else
		{
			...
			if($orderdataheader_freight_component_value==""){
			    $orderdataheader_freight_component_value="0";
			}
			if($orderdataheader_TD==""){
			    $orderdataheader_TD="0";
			}
            if($orderdataheader_order_value==""){
			    $orderdataheader_order_value="0";
			}
			if($orderdataheader_VAT==""){
			    $orderdataheader_VAT="0.00";
			}
				  ...
				  if($nick_name=='STAR')
					{
						...
						$trans_type='NO';
						$trans_sub_type='';
						update_transaction_STAR($order_emp_code,$orderdataheader_order_no,$qty,$trans_type,$trans_sub_type,$link);
					}
		...
		if(count($order_details_array)>0)
		{
			for($i=0;$i<count($order_details_array);$i++){
				...
				if($nick_name=='DNV' || $nick_name=='HALDIRAM')
				{
					...
					if($orderdatadetails_weightage==""){
					    $orderdatadetails_weightage="0.00";
					}
					if($orderdatadetails_amount==""){
					    $orderdatadetails_amount="0.00";
					}
					...
					if(!in_array($orderdatadetails_order_no,$order_array_trans_id))
					{
						...
						if(mysqli_query($link,$sqlinsertorderdetails))
						{
							...
							if($nick_name=='STAR')
							{
								...
								$trans_type='O';
								$trans_sub_type='';
								update_transaction_STAR(substr($orderdatadetails_order_no,1,5),$orderdatadetails_order_no,$qty,$trans_type,$trans_sub_type,$link);
							}
...
if(count($payment_array)>0)
{
	for($x=0;$x<count($payment_array);$x++){
		...
		else
		{
			...
			if($paymentdataheader_date==""){
			    $paymentdataheader_date = "1971-01-01 00:00:01";
			}
