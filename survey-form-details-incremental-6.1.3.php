...
$count=mysqli_num_rows($result);
	...
	if($count>0){
		...
		while($rowssurveyformdetails = mysqli_fetch_assoc($result))
		{
			$contents.="<data>";
			$contents .='<survey_form_id><![CDATA['.mb_convert_encoding($rowssurveyformdetails['survey_form_id'], 'UTF-8', 'UTF-8').']]></survey_form_id>
						...
						<special_input_screen><![CDATA['.mb_convert_encoding($rowssurveyformdetails['special_input_screen'], 'UTF-8', 'UTF-8').']]></special_input_screen>
						<survey_menu_name><![CDATA['.mb_convert_encoding($rowssurveyformdetails['survey_menu_name'], 'UTF-8', 'UTF-8').']]></survey_menu_name>
						<menu_disp_sub_menu><![CDATA['.mb_convert_encoding($rowssurveyformdetails['menu_disp_sub_menu'], 'UTF-8', 'UTF-8').']]></menu_disp_sub_menu>
						<check_in_time_captured_menu><![CDATA['.mb_convert_encoding($rowssurveyformdetails['check_in_time_captured_menu'], 'UTF-8', 'UTF-8').']]></check_in_time_captured_menu>
						<follow_up_menu><![CDATA['.mb_convert_encoding($rowssurveyformdetails['follow_up_menu'], 'UTF-8', 'UTF-8').']]></follow_up_menu>
						';
			$contents.="</data>";
			//echo $cnt++;
		}
	...
	$url = APICALLLOGURL."/survey-form-details-incremental-6.1.3.php?nick_name=$nick_name&last_update_time=$last_update_time&incremental_download=$incremental_download&mode=$mode";
