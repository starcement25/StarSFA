...
$url = APICALLLOGURL."/operationdb-attendance-6.0.1.php?nick_name=$nick_name&emp_code=$emp_code&last_update_time=$last_update_time";
insertapilog($datetime,$emp_code,$url,$nick_name);

//For insertion of fetched xml
if(strtoupper($nick_name)=='ABDOS' || strtoupper($nick_name)=='RUPA' || strtoupper($nick_name)=='DURO')
{
...
$body="<?xml version='1.0' encoding='UTF-8'?><root><attendance><location><emp_code><![CDATA[E1430]]></emp_code><trans_id><![CDATA[AE143020230801212740]]></trans_id><latt><![CDATA[27.4691446]]></latt><longi><![CDATA[94.91709]]></longi><date><![CDATA[2023-08-01 21:27:40]]></date></location><attendancedata><emp_code><![CDATA[E1430]]></emp_code><date><![CDATA[2023-08-01]]></date></attendancedata></attendance></root>";
			...
			if(mysqli_query($link,$sqlinsertattlocation) && mysqli_query($link,$sqlinsertattendance))
				{
					...
					if($nick_name=='STAR')
					{
						...
						echo $sqlchkattinfo="SELECT Emp_Id FROM  t_att_checkout_info WHERE Emp_Id='".$rowempname['dns_emp_code']."' AND SUBSTRING(Entry_Date,1,10)='".substr($attendance_date,0,10)."'";
						$reschkattinfo = mysqli_query($link,$sqlchkattinfo); 

						$countchkattinfo=mysqli_num_rows($reschkattinfo);
						if($countchkattinfo ==0)
						{
							echo $sqlinsertattinfo="INSERT INTO t_att_checkout_info SET Emp_Id='".$rowempname['dns_emp_code']."',
