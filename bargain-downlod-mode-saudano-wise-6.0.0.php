...
{
   $sqlquery="SELECT distinct SH.sauda_no FROM customer_master CM,sauda_header SH,DO_master DM,product_master PM where DM.sku_code=PM.prod_code and SH.customer_code=CM.customer_code AND SH.sauda_no=DM.sauda_no AND SH.customer_code='".$customer_code."'  AND ".$date_condition." ORDER BY CM.customer_name ASC
";
}
...
if(strtoupper(substr($emp_code,0,1))=='B')
{
	...
	$url = APICALLLOGURL."/bargain-downlod-mode-saudano-wise-6.0.0.php?nick_name=$nick_name&emp_code=$emp_code&mode=$mode&customer_code=$customer_code&from_date=$from_date&to_date=$to_date";

	insertapilog($datetime,$emp_code,$url,$nick_name);

	header("Content-type: application/text"); 

	header("Content-Disposition: attachment; filename=bargain_transaction_mode_saudano_wise.txt");
