...
$surveyempstate_array_mailbody=array();
...
if(count($survey_array)>0)
{
	for($x=0;$x<count($survey_array);$x++){
		...
		{
			$sqlempname="SELECT emp_name,state FROM employee_master WHERE emp_code='".$survey_emp_code."'";
			$rsempname=mysqli_query($link,$sqlempname);
			$rowempname=mysqli_fetch_assoc($rsempname);
			$emp_name=$rowempname['emp_name'];
			...
			$sqlinsertsurveylocation="INSERT INTO location SET emp_code='".$survey_emp_code."',
									...
									date='".$survey_date."',
									updatetime='".$location_date."'";
			if(mysqli_query($link,$sqlinsertsurveylocation))
			{
				$flag=5;
			}
			else
			...
			{						
				...
				if(mysqli_query($link,$sql_insert_survey_header))
				{
					$flag=5;
				}
				else
			...
			array_push($surveyempstate_array_mailbody,$state);
	...
	if(count($survey_details_array)>0)
	{
		for($i=0;$i<count($survey_details_array);$i++){
			...
			if($nick_name=='TECPL'){
				if($row_id=='RA014' || $row_id=='RA015')
				{
					${no_of_transactions.$survey_id}=$value;
				}
				if($row_id=='RA005' || $row_id=='RA008')
				{
					${amount.$survey_id}=$value;
				}
				if($row_id=='RA004' || $row_id=='RA007' || $row_id=='RA010')
				{
					${kiosk_id.$survey_id}=$value;
				}
				if($row_id=='RA011')
				{
					${complaint_type.$survey_id}=$value;
				}
			}
			if($nick_name=='EMAMI' || $nick_name=='EMAMIT'){
				if($row_id=='RA001')
				{
					${customer_code.$survey_id}=$value;
				}
				if($row_id=='RA002')
				{
					${email_id.$survey_id}=$value;
				}
				if($row_id=='RA003')
				{
					${phone.$survey_id}=$value;
				}
				
				if($row_id=='RA005')
				{
					${customer_name.$survey_id}=$value;
				}
				if($row_id=='RA004')
				{
					${customer_state.$survey_id}=$value;
				}
				if($row_id=='RA007')
				{
					${customer_phone.$survey_id}=$value;
				}
				if($row_id=='RA018')
				{
					${customer_email.$survey_id}=$value;
				}
				if($row_id=='RA010')
				{
					${customer_route.$survey_id}=$value;
				}
				if($row_id=='RA019')
				{
					${customer_type.$survey_id}=strtoupper($value);
				}
				if($row_id=='RA020')
				{
					${customer_distributor.$survey_id}=$value;
				}
				if($row_id=='RA008')
				{
					${customer_district.$survey_id}=$value;
				}
			}
	...
	//End Insert into the Survey output table for new survey id
		for($countarr=0;$countarr<count($survey_array);$countarr++)
		{
			...
			if(($surveyempstate_array_mailbody[$countarr]=='NORTH BENGAL' || $surveyempstate_array_mailbody[$countarr]=='SOUTH BENGAL' || $surveyempstate_array_mailbody[$countarr]=='BIHAR') && 	($surveytype_array_mailbody[$countarr]=='Site Visit' || $surveytype_array_mailbody[$countarr]=='Technical Meets')){
				$survey_extra_mail='himanshushekhar22@starcement.co.in';
			}
				}
			//End for STAR mis data details survey count updation
			//Start for EMAMI customer email update
			if($nick_name=='EMAMI'){
				$sqlupdatecustomeremail="UPDATE customer_master SET email='".addslashes(${email_id.$surveytansid_array_mailbody[$countarr]})."',
										phone_no='".addslashes(${phone.$surveytansid_array_mailbody[$countarr]})."',download_time=CURRENT_TIMESTAMP() WHERE 
										customer_code='".addslashes(${customer_code.$surveytansid_array_mailbody[$countarr]})."'";
				if(mysqli_query($link,$sqlupdatecustomeremail))
					{
						$flag=5;
					}
					else
			...
			//End for EMAMI customer email update
			...
			//Start for TECPL KIOSK data updation
			if($nick_name=='TECPL')
			{
				$sqlkioskdetails="SELECT customer_name,route_code,district,dns_customer_code FROM customer_master WHERE customer_code='".${kiosk_id.$surveytansid_array_mailbody[$countarr]}."'";
				$rskioskdetails=mysqli_query($link,$sqlkioskdetails);
				$rowkioskdetails=mysqli_fetch_assoc($rskioskdetails);
				$customer_name=$rowkioskdetails['customer_name'];
				$route_code=$rowkioskdetails['route_code'];
				$district=$rowkioskdetails['district'];
				$dns_customer_code=$rowkioskdetails['dns_customer_code'];
				
				$sqlkioskdivisiondetails="SELECT route_name FROM route_master WHERE route_code='".$route_code."'";
				$rskioskdivisiondetails=mysqli_query($link,$sqlkioskdivisiondetails);
				$rowkioskdivisiondetails=mysqli_fetch_assoc($rskioskdivisiondetails);
				$route_name=$rowkioskdivisiondetails['route_name'];
				
				$sqlkioskzonedetails="SELECT zone FROM employee_master WHERE emp_code='".substr($surveytansid_array_mailbody[$countarr],2,5)."'";
				$rskioskzonedetails=mysqli_query($link,$sqlkioskzonedetails);
				$rowkioskzonedetails=mysqli_fetch_assoc($rskioskzonedetails);
				$zone=$rowkioskzonedetails['zone'];
				
				$sqlkiosktransaction="INSERT INTO kiosk_transaction_details SET 
									 survey_id='".$surveytansid_array_mailbody[$countarr]."',
									 kiosk_id='".$dns_customer_code."',
									 no_of_transactions='".${no_of_transactions.$surveytansid_array_mailbody[$countarr]}."',
									 amount='".${amount.$surveytansid_array_mailbody[$countarr]}."',
									 CCC='".$customer_name."',
									 division_id='".$route_code."',
									 division='".$route_name."',
									 region='".$district."',
									 zone='".$zone."',
									 type='".$surveytype_array_mailbody[$countarr]."'";
				if(mysqli_query($link,$sqlkiosktransaction))
				{
					$flag=5;
				}
				else
			...
			if($nick_name=='EMAMI' || $nick_name=='EMAMIT')
			{
				if(${customer_name.$surveytansid_array_mailbody[$countarr]}!=''){
					...
					$sqlmaxdnscustomercode="SELECT MAX(dns_customer_code) AS max_dns_customer_code FROM  customer_master WHERE dns_customer_code LIKE 'KYC%'";
					...
					$sqlstatecode="SELECT dns_state_code FROM state_master WHERE 
									state='".${customer_state.$surveytansid_array_mailbody[$countarr]}."'";
					...
					$sqlcreatecustomer="INSERT INTO customer_master SET 
										 customer_code='".$max_customer_code."',
										 dns_customer_code='".$max_dns_customer_code."',
										 customer_name='".${customer_name.$surveytansid_array_mailbody[$countarr]}."',
										 phone_no='".${customer_phone.$surveytansid_array_mailbody[$countarr]}."',
										 route_code='".${customer_route.$surveytansid_array_mailbody[$countarr]}."',
										 emp_code='".$emp_code."',
										 acedns='Y',
										 black_list='N',
										 cust_type='".${customer_type.$surveytansid_array_mailbody[$countarr]}."',
										 rds_tag='".${customer_distributor.$surveytansid_array_mailbody[$countarr]}."',
										 district='".${customer_district.$surveytansid_array_mailbody[$countarr]}."',
										 state_code='".$dns_state_code."'";
					if(mysqli_query($link,$sqlcreatecustomer))
					{
						$flag=5;
					}
					else
			...
			//End for TECPL KIOSK data updation 
			$survey_email=SURVEYEMAILRECIPENTS.','.$survey_extra_mail;
 ...
 /* -------------------------------------END QUERY FOR SURVEY------------------------------------------------------------------------------------*/
	...
	$url = APICALLLOGURL."/operationdb-survey-6.0.6.php?nick_name=$nick_name&emp_code=$emp_code&last_update_time=$last_update_time&last_git_master_update_time=$last_git_master_update_time&last_loyalty_purchase_update_time=$last_loyalty_purchase_update_time";
