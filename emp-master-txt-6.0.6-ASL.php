...
$sqlquery="SELECT emp_code,emp_name,sale_access,reporting_to,designation,vertical_value,branch_code,state,zone,acedns,lower_leaves,email 
			FROM employee_master WHERE branch_code IN($branch_code_list) ORDER BY emp_name ASC";
}
...
$sqlquery="SELECT emp_code,emp_name,sale_access,reporting_to,designation,vertical_value,branch_code,state,zone,acedns,lower_leaves,email 
	...
	if(strtoupper(substr($emp_code,0,1))=='B')
	{
		$sqlquery="SELECT broker_id,broker_name,'','','','','','','',acedns,'',mail_id 
				FROM broker_master WHERE 1 ".$login_condition." ORDER BY broker_name ASC";
	}
	if(strtoupper(substr($emp_code,0,1))=='C')
	{
		$sqlquery="SELECT DISTINCT CM.customer_code,CM.customer_name,'','','','',CM.branch_code,CM.state_code,CM.zone,CM.acedns,'',CM.email 
			...
			AND UNIX_TIMESTAMP(CRR.download_time) > UNIX_TIMESTAMP('".$last_update_time."') ORDER BY CM.customer_name ASC";
	}
}
...
$count=mysqli_num_rows($result);
	...
	$url = APICALLLOGURL."/emp-master-txt-6.0.6.php?nick_name=$nick_name&emp_code=$emp_code&last_update_time=$last_update_time&data_download_time=$data_download_time&incremental_download=$incremental_download";
