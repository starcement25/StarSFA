...
if($countselectuserdbversion>0)
{
	...
	if((($versionCodecurrent-$user_db_version_code)*10) > '1' && $is_update==1 && $incremental_download=='yes')
		...
		if(route_plan=='yes'){
			...
			{
				$sqlqueryrouteplan="SELECT COUNT(route_plan_trans_id) AS total_route_plan FROM route_plan WHERE 
				   emp_code='".$emp_code."' AND UNIX_TIMESTAMP(create_date) > UNIX_TIMESTAMP('".$last_update_time."')";			
				$rsqueryrouteplan=mysqli_query($link,$sqlqueryrouteplan);
				$rowqueryrouteplan=mysqli_fetch_assoc($rsqueryrouteplan);
		...
		$url = "http://www.acedns.in/acednsproduct/datadownloaddictionary-7.0.1.php?nick_name=$nick_name&emp_code=$emp_code&device_id=$device_id&last_update_time=$last_update_time&incremental_download=$incremental_download";
...
if($successval=="1"){
	if($incremental_download=='no'){
		...
		if(route_plan=='yes'){
			//$contents  .= 'route_plan'."\n";
			$sqlqueryrouteplan="SELECT COUNT(route_plan_trans_id) AS total_route_plan FROM route_plan WHERE emp_code='".$emp_code."'";			
			$rsqueryrouteplan=mysqli_query($link,$sqlqueryrouteplan);
			$rowqueryrouteplan=mysqli_fetch_assoc($rsqueryrouteplan);
			$routeplancnt=$rowqueryrouteplan['total_route_plan'];
	...
	else
	{
		...
		if(destination_price_list=='yes' || destination_ordertype_price_list=='yes' || destination=='yes')
		{
			...
			{
				$sqlquerydestination="SELECT COUNT(destination_master) AS total_destination FROM destination_master WHERE 
								UNIX_TIMESTAMP(download_time) > UNIX_TIMESTAMP('".$last_update_time."')";			
				$rsquerydestination=mysqli_query($link,$sqlquerydestination);
				$rowquerydestination=mysqli_fetch_assoc($rsquerydestination);
				$destinationcnt=$rowquerydestination['total_destination'];
	...
	$datetime = gmdate('Y-m-d H:m:s',strtotime('+330 minute'));
	$url = "http://www.acedns.in/acednsproduct/datadownloaddictionary-7.0.2.php?nick_name=$nick_name&emp_code=$emp_code&device_id=$device_id&last_update_time=$last_update_time&incremental_download=$incremental_download";
	insertapilog($datetime,$emp_code,$url,$nick_name);
	/*$config = 'api_calllog.txt';
	$file=fopen($config,"r+");
	$date = date("F j, Y");
