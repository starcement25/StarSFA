<?php
	...
	$sqlempname="SELECT emp_name,branch_code,vertical_value,emp_code FROM employee_master ";
	$rsempname=mysqli_query($link,$sqlempname);
	while($rowempname=mysqli_fetch_assoc($rsempname))
	{
	$emp_name=$rowempname['emp_name'];
	$branch_code=$rowempname['branch_code'];
	$emp_code=$rowempname['emp_code'];
	...
	if($days_first_login=='yes')
	{
		...
		$sqlpoint="(SELECT latt, longi,trans_id,DATE_FORMAT(date,'%H:%i:%s') as trantime FROM location WHERE emp_code ='".$emp_code."' AND 
					DATE LIKE '%".$dateprevious."%' AND  SUBSTRING(trans_id,1,1) IN('A','O','P','N','D') ORDER BY DATE_FORMAT(date,'%Y-%m-%d %H:%i:%s') ASC)";			
