...
$sqlempaccess="SELECT sale_access FROM employee_master WHERE emp_code='".$emp_code."'";
$rsempaccess=mysqli_query($link,$sqlempaccess);
$rowempaccess=mysqli_fetch_assoc($rsempaccess);
$emp_sale_access=$rowempaccess['sale_access'];
$countsis=0;
...
while($rowlevel=mysqli_fetch_assoc($rslevel))
{
	...
	if($countsis > 0)
		...
		{
			if(strtoupper($emp_sale_access)=='TECHNICAL')
			{
				$date=gmdate('d',strtotime('+330 minute'));
				$month=gmdate('m',strtotime('+330 minute'));
				$year=gmdate('Y',strtotime('+330 minute'));
				...
				$countcolumns='16';
				$contentsdatetime =$year.'-'.$month.'-'.$date.' '.$hour.':'.$minute.':'.$second;
				$res_data[] = array("emp_code"=>$emp_code,"emp_name"=>'',"year"=>$year,"month"=>$mnthval,"base_volume"=>0,"volume_achv"=>0,"achv_percent"=>0,"monthly_unique_visit_target"=>0,"monthly_unique_visit_achv"=>0,"dealer_appointment_tgt"=>0,"dealer_appointment_achv"=>0,"active_dealer_cur_month"=>0,"active_dealer_prev_month"=>0,"sis_earned"=>0,"penalty_deducted"=>0,"net_sis_earned"=>0);

			 $res_data_final = array("process_status"=>"YES","process_message"=>"Success.","countrows"=>1,"countcolumns"=>$countcolumns,"datetime"=>$contentsdatetime,"datavalue"=>$res_data );
			}
			else
			{
