...
$count=mysqli_num_rows($result);
	...
	if($count>0){
		...
		$url = "http://www.coralindia.com/dev/acednsproduct/user-details-incremental-5.0.0.php?nick_name=$nick_name&last_update_time=$last_update_time&incremental_download=$incremental_download&mode=$mode";
		insertapilog($datetime,$emp_code,$url,$nick_name);
		/*$config = 'api_calllog.txt';
		$file=fopen($config,"r+");
		$date = date("F j, Y");
		$time = date("H:i:s");
		$newuser ="[$date $time]"."http://www.coralindia.com/dev/acednsproduct/user-details-incremental-5.0.0.php?nick_name=$nick_name&last_update_time=$last_update_time&incremental_download=$incremental_download&mode=$mode"."\r\n";
