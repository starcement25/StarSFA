...
$changed_consignee_address = "*ROOT*ORDER_APPROVAL*ORDER_APPROVAL_DETAILS*CHANGED_CONSIGNEE_ADDRESS";
...
class xml_order_approval{
	var $location_emp_code,$location_trans_id,$location_latt,$location_longi,$location_date,$approval_id,$APPORDERNO,$customer_code,$changed_sub_dealer_code,$changed_dns_prod_code,$QTY_CHANGED,$changed_dns_destination_code,$changed_dump_code,$plant_name,$approval_status,$approval_done_by,$remarks,$changed_order_for,$changed_consignee_address;	
}
...
function contents($parser, $data){
	...
	$remarks,$changed_order_for,$changed_consignee_address,$order_approval_array;
	//echo $current_tag.'<br />';
	//echo $data.'<br />';
	if(substr($current_tag,0,20)=='*ROOT*ORDER_APPROVAL')
	{
		...
		switch($current_tag){
			...
			case $changed_order_for:
				...
				break;
			case $changed_consignee_address:
				$order_approval_array[$counter]->changed_consignee_address = $data;
				$counter++;
				break;											
		}
	}
...
if(count($order_approval_array)>0)
{
	for($x=0;$x<count($order_approval_array);$x++){
		...
		$changed_consignee_address=$order_approval_array[$x]->changed_consignee_address;
	  ...
	  if(mysqli_query($link,$sqlinsertorlocation))
		...
		else
		{
			...
			$sqlupdateorderapp="UPDATE T_APPERPDO_APPROVAL SET approval_id='".$approval_id."',
							   ...
							   changed_consignee_address	='".$changed_consignee_address."',
...
/*if($flag==1 || $flag==2)
{
	...
	for($k=0;$k<count($order_approval_array);$k++){
		...
		$changed_consignee_address=$order_approval_array[$k]->changed_consignee_address;
		...
		if($approval_status=='AUTHORIZE')
		{
		 ...
		 if($approval_status=='MODIFY')
		 {
			 ...
			 $sqlupdateorderappsaathii="UPDATE T_APPERPDO_TEMP  SET dns_prod_code ='".$changed_dns_prod_code."',
								...
								consignee_address		='".$changed_consignee_address."',
...
$url = APICALLLOGURL."/operationdb-order-approval-6.0.0.php?nick_name=$nick_name&emp_code=$emp_code&last_update_time=$last_update_time";
