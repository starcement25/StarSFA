...
if ($functionality == 'DOS') {
  ...
  from (SELECT customer_code,SUM( IF( SUBSTRING(visit_date,1,10) >='".$numericprevdatetotpurchase."' AND SUBSTRING(visit_date,1,10) <='".$todaydate."', visit_qty, 0 )) AS total_visit_prev,
			...
			//$sqlquerycustomerroute="SELECT DISTINCT CRER.route_code, RM.route_name, CRER.customer_code, CM.customer_name, CM.cust_type FROM customer_route_emp_relation CRER JOIN route_master RM ON RM.route_code = CRER.route_code JOIN customer_master CM ON CM.customer_code = CRER.customer_code WHERE CRER.emp_code IN ('E1239') AND CRER.acedns = 'Y' AND CM.cust_type IN ('Dealer', 'Sub Dealer', 'Exclusive Dealer');";
			//echo "rrrrr".$login_condition;
			//echo "ff".$emp_hierarchy_condition;
			//echo "ffff". $sqlquerycustomerroute;
			  echo"<pre>";print_r($sqlquerycustomerroute);
			$resultcustomerroute = mysqli_query($link, $sqlquerycustomerroute);
			$countcustomerroute = mysqli_num_rows($resultcustomerroute);
			 echo "count- ". $countcustomerroute;
			// echo"<pre-count>";echo '<pre>';print_r(mysqli_fetch_assoc($resultcustomerroute));die;
			...
			if ($countcustomerroute > 0) {
				...
				$contentsrowcolumn = $customer_count . '¥' . '58';
				$datacontents = $contentsrowcolumn . "\n" . $contentsdatetime . str_replace("\r", "", $linecontents);
				echo"<pre>";print_r($customebr_code_array);

				print_r($datacontents);die;
