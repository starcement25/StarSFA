...
if($countselectuserdbversion>0)
{
	...
	if((($versionCodecurrent-$user_db_version_code)*10) > '1' && $is_update==1 && $incremental_download=='yes')
		...
		if(in_array('emp_master',$need_download_table_array))
			...
			{
				$sqlqueryemp="SELECT COUNT(emp_code) AS total_emp FROM employee_mastrer WHERE 
								UNIX_TIMESTAMP(download_time) > UNIX_TIMESTAMP('".$last_update_time."') ".$emp_val_rds."";			
				$rsqueryemp=mysqli_query($link,$sqlqueryemp);
				$rowqueryemp=mysqli_fetch_assoc($rsqqueryemp);
		...
		$url = "http://www.acedns.in/acednsproduct/datadownloaddictionary-6.0.8.php?nick_name=$nick_name&emp_code=$emp_code&device_id=$device_id&last_update_time=$last_update_time&incremental_download=$incremental_download";
...
if($successval=="1"){
	if($incremental_download=='no'){
		...
		//$contents  .= 'emp_master'."\n";
		$sqlqueryemp="SELECT COUNT(emp_code) AS total_emp FROM employee_mastrer WHERE 1 ".$emp_val_rds."";			
		$rsqueryemp=mysqli_query($link,$sqlqueryemp);
		$rowqueryemp=mysqli_fetch_assoc($rsqqueryemp);
		$cntemp=$rowqueryemp['total_emp'];
