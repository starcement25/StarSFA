...
$url = APICALLLOGURL . "/updateApp-6.0.2_play_7.php?nick_name=$nick_name&emp_code=$emp_code&deviceId=$deviceId&versionCode=$versionCode";
...
if (vertical_fields == 'yes' && (strtoupper($_REQUEST['nick_name']) == 'EMAMI' || strtoupper($_REQUEST['nick_name']) == 'EMAMIT')) {
	$sqlselectappversion = "SELECT * FROM app_version_play_new WHERE vertical_value='" . $vertical_value . "'";
} else {
	$sqlselectappversion = "SELECT * FROM app_version_play_new";
}
...
TEAM - ACEdns</body></html>";
	...
	//echo $rowselect['version_code'];
	if ($rowselect['version_code'] == $versionCode && $rowselect['is_update'] == 1) {
		//echo "1-".$app_version_latest;
		//echo "4-".$app_version_latest;
		echo "0";
	} else if ($versionCode < $rowselect['version_code']) { 																																							//echo "1-".$app_version_latest;																
		echo "4/" . $app_version_latest;
	} else if ($versionCode < $app_version_latest) {
		echo "4/" . $app_version_latest;
	} else {
		//echo "--".$rowselect['version_code'];
		echo "0";
