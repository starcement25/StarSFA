...
$res_product_filter = mysqli_query($link,$sql_product_filter);
$row_product_filter = mysqli_fetch_assoc($res_product_filter);
...
$sql_order_exist_check = "SELECT OH.order_no, EM.emp_code, EM.emp_name, OH.status FROM order_header OH, employee_master EM WHERE SUBSTRING(OH.order_no,2,5) = EM.emp_code AND OH.order_no LIKE 'O%' AND OH.status IN('spl permission','hold') AND EM.HQ = (SELECT branch_name FROM branch_master WHERE branch_code = '".$branch_code."') AND SUBSTRING(OH.order_no,-14,8)>='20160901' ORDER BY SUBSTRING(OH.order_no,2,5) ASC, SUBSTRING(OH.order_no,-14,8) ASC";
$res_order_exist_check = mysqli_query($link,$sql_order_exist_check);
$total_row_check = mysqli_num_rows($res_order_exist_check);
...
<?php
	$res_order_exist_check = mysqli_query($link,$sql_order_exist_check);
	while($row_order_exist_check = mysqli_fetch_assoc($res_order_exist_check)){
		...
		$select_control = "<input type=\"button\" value=\"Confirm\" onclick=\"send_to_pending('$order_no');\">";
		...
		$res_product_details_count = mysqli_query($link,$sql_product_details_count);
		$row_product_details_count = mysqli_fetch_assoc($res_product_details_count);
		...
		while($row_product_details = mysqli_fetch_assoc($res_product_details)){
			...
			if($total_product>1){
				if(!in_array($order_no,$order_no_array_one)){
					...
					echo "<td rowspan=\"".$total_product."\" >".$status."</td>";
				}
			}
			...
			if($total_product>1){
				if(!in_array($order_no,$order_no_array)){
					...
					echo "<td rowspan=\"".$total_product."\" width=\"15%\" style=\"background:#EEE9BF;\" align=\"center\">$select_control</td>";
					echo "</tr>";
				}
				else{
					echo "</tr>";
				...
				echo "<td width=\"15%\" style=\"background:#EEE9BF;\" align=\"center\">$select_control</td>";
				echo "</tr>";
			}
		}
