...
if($_SESSION['admin_login']=="admin")
{
	...
	$emp_hierarchy_condition_one='WHERE EM.reporting_to IN(SELECT emp_code FROM employee_master WHERE reporting_to=(SELECT emp_code FROM employee_master WHERE reporting_to=\'\'))';
}
else
{
	$emp_hierarchy=return_employee_hierarchy($_SESSION['admin_login']);
	$emp_hierarchy_condition=' AND SAL.emp_code IN('.$emp_hierarchy.')';
	$emp_hierarchy_condition_one='WHERE EM.reporting_to IN(\''.$_SESSION['admin_login'].'\')';
}
...
while($row_count_product = mysqli_fetch_assoc($res_count_product))
{
	...
	while($row_emp = mysqli_fetch_assoc($res_emp))
	{
		...
		$res_sauda_allocation = mysqli_query($link,$sql_sauda_allocation);
		$row_sauda_allocation = mysqli_fetch_assoc($res_sauda_allocation);
		${$row_count_product[$field_name2].allocation} += $row_sauda_allocation['alloted'];
		$checkallocation += $row_sauda_allocation['alloted'];
		
		$sql_products = "SELECT PM.prod_desc, sum(SD.convert_qty_two) as mt_booked, sum(SD.qty) as case_booked FROM sauda_details SD, product_master PM WHERE SD.sku_code=PM.prod_code AND PM.product_group_code='$row_count_product[$field_name1]' AND SUBSTRING(SD.sauda_no,3,5)='$row_emp[emp_code]' ".$sauda_booked_condition." GROUP BY SD.sku_code" ;
		$res_product = mysqli_query($link,$sql_products);
...
//echo "-------";
print_r($product_booked);
//print_r($product_booked_case);
echo "</pre>";*/
if($checkallocation!='')
{
?>
...
{
	$color = "FF".rand(7000,9999);
	
	echo "<tr>
			<td colspan=\"3\" style=\"background:#$color; font-weight:bold;\" align=\"center\">".$productgroupindex."<div style=\"float:right;\" align=\"right\"> Alloted:".$productgroupvalue."</span></td>
		  </tr>
	...
	foreach($product_booked as $productbookedindex=>$productbookedvalue)
	{
		...
		if($productbookedindex == $productgroupindex)
		{
			...
			echo "<tr style=\"font-weight:bold;\">
					...
					<td align=\"right\">".$total_booked_quantity."</td>
...
echo "<tr style=\"font-weight:bold; background:#CFCFCF;\">
					...
					<td align=\"right\">".$total_booked_grand_total."</td>
