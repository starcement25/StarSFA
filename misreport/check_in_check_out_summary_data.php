...
//if(!empty($date_array)){
	...
	while($row_emp = mysqli_fetch_assoc($res_emp)){
		...
		//echo "yyyy-".${'outer_customer_code'.$emp_code};
				...
				foreach($date_array as $date_array_val)
				{
						...
						if($countcheckinout >0)
						{
							...
							while($rowcheckinout=mysqli_fetch_assoc($rscheckinout))
							{
								...
								${'time_difference_final'.$emp_code}=${'time_difference_final'.$emp_code}+$time_difference_final;
					...
					//For ILS employee wise Duration
					if(${'time_difference_final'.$emp_code} >=3600)
						{
							$hours = floor(${'time_difference_final'.$emp_code} / 3600);
							$minutes = floor((${'time_difference_final'.$emp_code} / 60) % 60);
							$seconds = ${'time_difference_final'.$emp_code} % 60;
							${'time_duration'.$emp_code}=$hours.' Hour(s) '.$minutes.' Minute(s) '.$seconds.' Second(s)';
						}
						else if(${'time_difference_final'.$emp_code} >=60 && ${'time_difference_final'.$emp_code}<3600)
						{
							$minutes = floor((${'time_difference_final'.$emp_code} / 60) % 60);
							$seconds = ${'time_difference_final'.$emp_code} % 60;
							${'time_duration'.$emp_code}=$minutes.' Minute(s) '.$seconds.' Second(s)';
						}
						else
						{
							$seconds = ${'time_difference_final'.$emp_code} % 60;
							${'time_duration'.$emp_code}=$seconds.' Second(s)';
						}
					echo "<tr><td  class = 'TDHEAD_SUB' align = 'center'></td>
					<td  class = 'TDHEAD_SUB' align = 'center'></td>
					<td  class = 'TDHEAD_SUB' align = 'center'></td>
					<td  class = 'TDHEAD_SUB' align = 'center'></td>
					<td  class = 'TDHEAD_SUB' align = 'center'></td>
					<td  class = 'TDHEAD_SUB' align = 'center'>Total Duration</td>
					<td  class = 'TDHEAD_SUB' align = 'center'>".${'time_duration'.$emp_code}."</td>
					<td  class = 'TDHEAD_SUB' align = 'center'></td>
					</tr>";
				}
			}
