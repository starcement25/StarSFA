...
<table width="100%" border="1" style="border-collapse:collapse;" cellpadding="5px">
  <tr>
  	<td colspan="<?php echo (($total_product*2)+2); ?>" class="TDHEAD" align="center"><b>Statewise <?php echo $sauda_duration; ?></b></td>
  </tr>
  <tr class="TDHEAD_SUB">
	...
	$res_count_product = mysqli_query($link,$sql_count_product);
	while($row_count_product = mysqli_fetch_assoc($res_count_product))
		echo "<td align=\"center\" colspan=\"2\" width=\"8%\"><b>$row_count_product[$field_name2]</b></td>";
	?>
  </tr>
  <tr class="TDHEAD_SUB">
    <td>&nbsp;</td>
...
{
  $sql_state = "SELECT EM.state as state_name FROM employee_master EM, sauda_transaction_log STL WHERE SUBSTRING(STL.sauda_no,3,5)='".$row_emp['emp_code']."' ".$sauda_booked_condition." AND EM.emp_code=SUBSTRING(STL.sauda_no,3,5) GROUP BY EM.state ORDER BY EM.state ASC";
  ...
  while($row_state = mysqli_fetch_assoc($res_state))	
  {
	  ...
	  while($row_product = mysqli_fetch_assoc($res_product))
	  {
		   ...
		   $sql_quantity_booked = "SELECT SUM(STL.convert_qty_two) as mt_booked, SUM(STL.qty) as case_booked FROM sauda_transaction_log STL, product_master PM, employee_master EM WHERE SUBSTRING(STL.sauda_no,3,5)='".$row_emp['emp_code']."' AND EM.emp_code=SUBSTRING(STL.sauda_no,3,5) AND STL.prod_code=PM.prod_code AND PM.$field_name1='$row_product[$field_name1]' AND EM.state LIKE '%".$state_name."%' ".$sauda_booked_condition;
		   ...
		   if($row_quantity_booked['case_booked'] != '')
			...
			$statewise[$state_name][$product_group_value] += $quantity_booked;
			$statewise_case_booked[$state_name][$product_group_value] += $quantity_booked_case;
...
$count=1;
foreach($statewise as $branchwiseindex=>$branchwisevalue)
{
	$branch_code = explode("^", $branchwiseindex);
	echo "<tr>
			<td width=\"2%\">$count</td>
			<td width=\"22%\" style=\"font-weight:bold;\">".$branchwiseindex."</td>";
	foreach($branchwisevalue as $index=>$val)
	{
		...
		{
		   echo "
				<td align=\"right\" style=\"background:$color; color:blue; cursor:pointer; font-weight:bold;\" onclick=\"show_statewise_data('$branchwiseindex','$start_date','$end_date','$value','$index');\" width=\"4%\">".number_format($val,3)."</td>";
		   echo "<td align=\"right\" style=\"background:$color;\" width=\"4%\" >".$statewise_case_booked[$branchwiseindex][$index]."</td>";
		}
		else
		{
		   echo "
				<td align=\"right\" style=\"background:$color;\" width=\"4%\">".$val."</td>";
		   echo "<td style=\"background:$color;\" width=\"4%\"></td>";
		}
	}
	echo "</tr>";
	$count++;
...
foreach($product_group_code as $key=>$val)
{
	...
	echo "<td align=\"right\" style=\"background:$color;\" width=\"4%\">".number_format($val,3)."</td>
		  <td align=\"right\" style=\"background:$color;\" width=\"4%\">".$case_booked[$key]."</td>";
}
