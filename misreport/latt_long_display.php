...
else if ($mode == 'editcustomer_multiple')
{
	$sel_customer_code=$_REQUEST['sel_customer_code'];
	foreach($sel_customer_code as $customer_code_val){
	$upd_sql="UPDATE customer_master SET 
			 base_latt='0',
			 base_longi='0',
			 need_location_update='yes',
			 download_time=CURRENT_TIMESTAMP()
			 WHERE customer_code = '" .$customer_code_val."'";
	mysqli_query($link,$upd_sql) or die(mysqli_error()." Error in customer updation.");
	
	$updcustomerroute="UPDATE customer_route_emp_relation SET download_time=CURRENT_TIMESTAMP() WHERE customer_code = '" .$customer_code_val."'";
	mysqli_query($link,$updcustomerroute) or die(mysqli_error()." Error in customer route updation.");
	
	$sqlinserlocationupdate="INSERT INTO customerwise_location_update_log SET customer_code = '" .$customer_code_val."',need_location_update='yes',update_time=CURRENT_TIMESTAMP()";
	mysqli_query($link,$sqlinserlocationupdate);
	}
	$GLOBALS['err_msg']="Selected Customer(s) location updated successfully.";
	disphtml("main();");
}
else {
disphtml("main();");
...
function main(){
   ...
   <script src="https://ajax.googleapis.com/ajax/libs/jquery/1.11.3/jquery.min.js"></script>
	...
	echo "<table width=\"80%\" align=\"center\" border=\"0\" cellpadding=\"5\" cellspacing=\"1\" >
                ...
                </table><span style=\"font-weight:bold; font-size:14px;\">Lattitude Longitude List</span><br><br>";
	...
	echo "</center>";
	?>
    <form name="frm_opts" action="customer_latt_long_edit.php" method="post" >
	...
	function display_result(){
		...
		GenericAjaxFunction('latt_long_display_data.php','display',0);
	...
	function Popup(data) 
	{
		var mywindow = window.open('', 'Latt Long Report', 'height=400,width=600');
