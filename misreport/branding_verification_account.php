...
function main(){
   ...
   <script src="https://ajax.googleapis.com/ajax/libs/jquery/1.11.3/jquery.min.js"></script>
	...
	function display_result(){
		...
		GenericAjaxFunction('branding_verification_account_data.php?employee='+employee+'&start_date='+start_date+'&end_date='+end_date+'&zone='+zone+'&state='+state+'&branch='+branch+'&department='+department,'display',0);
	...
	for($i=0;$i<=count($survey_id);$i++)
	{
	  ...
	  if($PO_val!=''){
		$upd_sql="UPDATE survey_header SET PO_no ='$PO_val',PO_submitted_by ='".$_SESSION['admin_login']."',PO_submitted_date =CURRENT_TIMESTAMP() WHERE survey_id = '".$survey_id_val."'";
