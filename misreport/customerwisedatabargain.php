...
$res_emp_name = mysqli_query($link,"SELECT emp_name FROM employee_master WHERE emp_code = '".$get_emp_code."'");
...
if($_GET['type'] == 'today'){
	...
	$value = 1;
	$start_date = str_replace("-","",$today);
	$end_date = str_replace("-","",$today);
}
	...
	$end_date = str_replace("-","",$current_date);
}
...
$sql_count_product = "SELECT  PGM.product_group_code,PGM.product_group_name FROM product_group_master PGM  
					...
					$vertical_val_condition_one  $sauda_booked_condition) ORDER BY PGM.product_group_name ASC";
$res_count_product = mysqli_query($link,$sql_count_product);
$total_product = mysqli_num_rows($res_count_product);

$count = 1;
$res_count_product = mysqli_query($link,$sql_count_product);
while($row_count_product = mysqli_fetch_assoc($res_count_product))
{
	...
	$sql_customer = "SELECT CM.customer_code, CM.customer_name, LO.latt, LO.longi, date_format(substring(LO.date,1,10),'%d-%m-%Y') as locate_date, 
	substring(LO.date,12,8) as locate_time FROM customer_master CM, sauda_details SD, sauda_header SH, location LO 
	WHERE SD.sauda_no=SH.sauda_no AND CM.customer_code=SH.customer_code AND SH.sauda_no = LO.trans_id 
	".$sauda_booked_condition." GROUP BY SH.customer_code";
	...
	while($row_customer = mysqli_fetch_assoc($res_customer))	
	{
	  ...
	  $sql_product="SELECT  PGM.product_group_code,PGM.product_group_name FROM product_group_master PGM   
					".$vertical_val_condition." AND PGM.acedns='Y' AND PGM.product_group_code IN
					(SELECT DISTINCT PM.product_group_code FROM product_master PM,sauda_details SD WHERE PM.prod_code=SD.sku_code 
					$vertical_val_condition_one  $sauda_booked_condition) ORDER BY PGM.product_group_name ASC";
	  ...
	  while($row_product = mysqli_fetch_assoc($res_product)){
		   ...
		   $sql_quantity_booked = "SELECT sum((SD.qty*PM.conversion_factor_two)) as mt_booked, sum(SD.qty) as case_booked,PM.UOM1 FROM sauda_details SD, 
		   product_master PM, sauda_header SH WHERE SD.sauda_no=SH.sauda_no AND SH.customer_code='".$row_customer['customer_code']."' 
		    AND SD.sku_code=PM.prod_code AND PM.$field_name1='".$row_product[$field_name1]."' 
		   ".$sauda_booked_condition;
		   $res_quantity_booked = mysqli_query($link,$sql_quantity_booked);
		   $row_quantity_booked = mysqli_fetch_assoc($res_quantity_booked);
		   $UOM1=$row_quantity_booked['UOM1'];
		  
		   if($UOM1=='Loose')
		   {
			$quantity_booked = $row_quantity_booked['case_booked']; 
			$quantity_booked_case = 0;
			$product_group_code[$product_group_value] += $row_quantity_booked['case_booked'];
		   }
		   else
		   {
...
if(!empty($customerwise_booked_mt)){
	?>
...
$count = 1;
	foreach($customerwise_booked_mt as $customerwiseindex=>$customerwiseval){
		...
		$sql_check_emp_customer = "SELECT customer_code FROM sauda_header WHERE customer_code = '".$customer_code_name[1]."' 
		AND (SUBSTRING(sauda_no,-14,8) BETWEEN ".$start_date." AND ".$end_date.")";
		$res_check_emp_customer = mysqli_query($link,$sql_check_emp_customer);
		$total_rows = mysqli_num_rows($res_check_emp_customer);
		if($total_rows>0){
			echo "<tr>
					<td>$count</td>
