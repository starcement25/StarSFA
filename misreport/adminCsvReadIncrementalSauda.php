					...
					if(no_of_filter > 3){
						...
						else
						{
								...
								|| $vertical_value_db!=$vertical_value || $conversion_factor_two_db!=$conversion_factor_two || $UOM3_db!=$UOM3 || $pack_size_db!=$pack_size || $TD_db!=$TD)
							{
								if($acedns_db!=$acedns)
								{
									if(strtoupper($acedns)=='N')
									{
										echo "Transaction exists for the product at row ".($csv_row_count+1).".Not allowed to change the status.";
										die;
									}
								}
				...
				{ 
				  ...
				  //print_r($data);
					...
					if($rds_tag!='' && $customer_type!='D')
					{
						...
						{
							if(providing_code=='yes'){
							  $sqlrdscode="SELECT customer_code FROM customer_master WHERE dns_customer_code='".addslashes($rds_tag_val)."' 
											AND emp_code='".$emp_code."' AND acedns='Y'";
							}
							else
							{
							  $sqlrdscode="SELECT customer_code FROM customer_master WHERE customer_name='".addslashes($rds_tag_val)."' 
											AND emp_code='".$emp_code."' AND acedns='Y'";
							}
					...
					if($countcustomernamechk<1 && $route_code!='')
					{
						...
						//$sql .= " ,	sauda_limit='".addslashes($sauda_limit)."'";
					...
					else
					{
						...
						|| $customer_name_db!=$customer_name || $dns_customer_code_db!=$dns_customer_code || $visit_day_db!=$visit_day || 
						$state_code_db!=$state || $email_db!=$email || $monthly_potential_db!=$monthly_potential || $sauda_limit_db!=$sauda_limit)
						{
							...
							echo $sqlupdated .= " , rds_tag='".$rds_code."',visit_day='".addslashes($visit_day)."',
		...
		$err = "";
			...
			foreach($lines as $line)
			{
				...
				//print_r($data);
					...
					$sqlinsertmrplog .= " , mrp='".mysqli_real_escape_string($mrp)."'";
