...
<table width="80%" align="center" cellpadding="2" cellspacing="2" border="0">
	...
	<tr>
		<td valign="top" bgcolor="#FFFFFF" >
                                ...
                                <td align="left" width="" style="vertical-align:top;" colspan="2">
                                   <?php $emp_code=$_REQUEST['emp_code'];?>
                                    ...
                                    <option value="0">SELECT</option>
                                     <?php 
                                     $sqlqueryemployee="SELECT emp_code,emp_name FROM employee_master WHERE 1 ".$emp_hierarchy_condition_one." 
									 					ORDER BY emp_name ASC";
                                     $resultemployee = mysqli_query($link,$sqlqueryemployee);
                                     $count=mysqli_num_rows($resultemployee);
                                     $cnt=1;
                                        if($count>0){
                ...
                style="display: '';height: 500px;overflow-y: scroll;display:block;width: 50%;" id="todayAttDisplay">
                    ...
                    <?php
						...
						while($rowrouteplan=mysqli_fetch_assoc($rsrouteplan))
						{
							...
							$rslatestroute=mysqli_query($link,$sqllatestroute);
							$rowlatestroute=mysqli_fetch_assoc($rslatestroute);
			...
			$cnt=0;
			while($rowroute=mysqli_fetch_assoc($rsroute))
			{
				$cnt++;
...
<?php 
   if($_REQUEST['mode']=='approve')
   {
	 ...
	 //exit();
	 if($success==1) {?><script language="JavaScript" type="text/javascript">alert('Route plan approved successfully');window.location.href='route_plan_approval.php';</script><?php }else{?><script language="JavaScript" type="text/javascript">alert('Route plan approval failure');window.location.href='route_plan_approval.php';</script><?php }
   }
}//End of main()?>