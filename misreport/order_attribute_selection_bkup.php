...
function attribute_selection($hidden,$get_control){
	...
	$table_data = "<div id=\"display_data\">	<form name=\"download_xls_data\" action=\"download_order_xls_data.php\" method=\"post\" >
 <table id=\"criteria_tab\" class=\"border\" width=\"45%\" style=\"border-collapse:collapse;border:1px solid #A92A61; padding:6px;\" >
	...
	if(vertical_fields == 'yes'){
		...
		EM ".$emp_hierarchy_value_condition;
		...
		if($total_rows>0){
			$res_attendance_verticalwise = mysqli_query($link,$sql_attendance_verticalwise);
			while($row_attendance_verticalwise = mysqli_fetch_assoc($res_attendance_verticalwise)){
		...
		$onclick = "vertical_zone(this.value);";
		$vertical_select_control = "<select name=\"vertical\" id=\"vertical\" onchange=\"".$onclick."\">
	...
	if($zone_total>0){
		...
		if(vertical_fields == 'yes'){
			$table_data .= "<div id=\"zone_select_div\"></div></td></tr>";
		}
		else
		{
			if($state_total>0)
	...
	if(vertical_field == 'yes' || $zone_total>0 || $state_total>0){
		...
		$table_data .= "<tr>
							...
							<td>";
		$emp_select_control = "<select name=\"employee\" id=\"employee_test\">";
		...
		while($row_emp = mysqli_fetch_assoc($res_emp)){
			...
			$emp_select_control_option .= "<option value=\"'".$emp_code."'\">".$emp_name."</option>";
		}
...
To:<input type=\"date\" name=\"end_date\" id=\"end_date\" style=\"height:15px;\" />
	...
	if(strtoupper($_SESSION['nick_name']) == 'RUPA')
	{
		echo $table_data."<tr><td colspan=\"2\" align=\"right\"><input type=\"button\" name=\"Submit\" value=\"Submit\" onclick=\"download_xls();\"></td></tr></table></div><div id=\"loader\" style=\"display:none\"><br/><center><img src=\"ajax-loader.gif\" /></center></div>";
	}
	else
	{
	echo $table_data."<tr><td colspan=\"2\" align=\"right\"><input type=\"submit\" name=\"Submit\" value=\"Submit\" onclick=\"display_result();\"></td></tr></table></form></div>";
	}
	?>
    <script>
	function vertical_zone(vertical){
		...
		GenericAjaxFunction('order_get_zone_related_data.php?vertical='+vertical+'&type=vertical','zone_select_div',0);
	}
	...
	function state_emp(state){
		...
		document.getElementById("emp_select_div").innerHTML = '<img src="ajax-loader.gif" id="ajaxloader">';
		GenericAjaxFunction('get_state_related_data.php?state='+state+'&vertical='+vertical+'&type=emp','emp_select_div',0);
	}
...
function attribute_selection_saletype($hidden,$get_control){
	?>
    <script type="text/javascript" src="../ajax1.js"></script>
    <?php
	if($_SESSION['admin_login']=="admin"){
 ...
 <table id=\"criteria_tab\" class=\"border\" width=\"45%\" style=\"border-collapse:collapse;border:1px solid #A92A61; padding:6px;\" >
					<tr class=\"TDHEAD\"><td colspan=\"2\" align=\"center\">Select Criteria</td></tr>";
	$table_data .= "<tr><td align=\"right\">Brand:</td><td>";
	$brand_select_control = "<select name=\"brand\" id=\"brand\">";
	$brand_select_control .= "<option value=\"\">Select</option>";
		$sql_brand = "SELECT product_group_code,product_group_name FROM product_group_master ORDER BY product_group_name ASC";
		$res_brand = mysqli_query($link,$sql_brand);
		while($row_brand = mysqli_fetch_assoc($res_brand)){
			$product_group_code = $row_brand['product_group_code'];
			$product_group_name = $row_brand['product_group_name'];
			$product_group_code_string .= "'".$product_group_code."',";
			$brand_select_control_option .= "<option value=\"'".$product_group_code."'\">".$product_group_name."</option>";
		}
		$product_group_code_string = rtrim($product_group_code_string,",");
		$brand_select_control .= "<option value=\"".$product_group_code_string."\">All</option>";
		$brand_select_control .=$brand_select_control_option;
		$brand_select_control .= "</select>";
		$table_data .= $brand_select_control;
		$table_data .= "</td></tr>";

	$table_data .= "<tr><td align=\"right\">Order Type:</td><td><select name=\"ordertype\" id=\"ordertype\">
						<option value=\"\">Select</option><option value=\"all\">All</option>
						<option value=\"primary\">Primary</option>
						<option value=\"secondary\">Secondary</option></td></tr>";
	$table_data.="<tr><td colspan=\"2\" align=\"center\"><div id=\"date_div\" $hidden >
From:<input type=\"date\" name=\"start_date\" id=\"start_date\" style=\"height:15px;\" />
To:<input type=\"date\" name=\"end_date\" id=\"end_date\" style=\"height:15px;\" />
</div></td></tr>";
