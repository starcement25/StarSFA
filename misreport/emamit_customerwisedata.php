...
$sql_emp = "SELECT EM.emp_code FROM employee_master EM WHERE ".$emp_condition;
$res_emp = mysqli_query($link,$sql_emp);
while($row_emp = mysqli_fetch_assoc($res_emp)){
	$get_emp_code = $row_emp['emp_code'];
	$sql_customer = "SELECT CM.customer_code, CM.customer_name, LO.latt, LO.longi, DATE_FORMAT(SUBSTRING(LO.date,1,10),'%d-%m-%Y') as locate_date, SUBSTRING(LO.date,12,8) as locate_time FROM customer_master CM, location LO, sauda_transaction_log STL WHERE CM.customer_code=STL.customer_code AND SUBSTRING(STL.sauda_no,3,5)='".$get_emp_code."' AND STL.sauda_no = LO.trans_id ".$sauda_booked_condition." GROUP BY STL.customer_code";
	...
	while($row_customer = mysqli_fetch_assoc($res_customer))	
	{
	  ...
	  while($row_product = mysqli_fetch_assoc($res_product)){
		   ...
		   $sql_quantity_booked = "SELECT SUM(STL.convert_qty_two) as mt_booked, SUM(STL.qty) as case_booked FROM sauda_transaction_log STL, product_master PM WHERE STL.customer_code='".$row_customer['customer_code']."' AND SUBSTRING(STL.sauda_no,3,5)='".$get_emp_code."' AND STL.prod_code=PM.prod_code AND PM.$field_name1='".$row_product[$field_name1]."' ".$sauda_booked_condition;
...
$count = 1;
	foreach($customerwise_booked_mt as $customerwiseindex=>$customerwiseval){
		...
		$sql_check_emp_customer = "SELECT customer_code FROM sauda_transaction_log WHERE customer_code = '".$customer_code_name[1]."' AND (SUBSTRING(sauda_no,-14,8) BETWEEN ".$start_date." AND ".$end_date.")";
