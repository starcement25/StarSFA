...
{
	?>
    <div id="curtain" style="height:100%; width:100%; background:#000000; left:0px; top:0px; position:fixed; opacity:0.5;" hidden></div>
    <?php
	$sql_select_survey_type = "SELECT survey_menu, survey_layer, survey_type, survey_type_details FROM acedns_acednsproduct.survey_form_details WHERE nick_name LIKE '%".$_SESSION['nick_name']."%'";
	$res_select_survey_type = mysqli_query($link,$sql_select_survey_type);
	$row_select_survey_type = mysqli_fetch_assoc($res_select_survey_type);
	$survey_menu = $row_select_survey_type['survey_menu'];
	$survey_layer = $row_select_survey_type['survey_layer'];
	...
	if($survey_type == 'yes'){
		...
		$hidden = 'hidden';
		$get_menu_data = "GenericAjaxFunction('get_menu.php','menu_input'+i,0);";
	}
	
	if($survey_menu == 'no' && $survey_type == 'no' && $survey_layer == 'yes'){
		$hide_survey_type = 'hidden';
		$hide_survey_menu = 'hidden';
		$get_layer_data = "GenericAjaxFunction('get_layer.php','layer_input'+i,0);";
	}
	else{
		$hide_survey_type = '';
		$hide_survey_menu = '';
...
{
	document.getElementById('tableview_'+k+'').innerHTML = '';
	...
	if(select_option == 'tableview')
	{
		document.getElementById('box_input_value'+k+'').disabled = false;
		document.getElementById('listview_div'+k+'').hidden = false;
		
		/*--------> Show Table View Input Form Data <---------*/
		var show_data = "<span style='border-radius:8px; width:100px; height:100px; margin-left:95%; margin-top:-5px; font-size:18px; cursor:pointer;' onclick='hide_info("+k+");'><b>X</b></span><table style='margin:20px;' cellpadding='4'><tr><td align='right'>Enter Type</td><td><select name='tableview_type'><option>Select</option><option>boolean</option><option>checkbox</option><option>radio</option></select></td></tr><tr><td align='right'>Enter Value</td><td><input type='textbox' name='tableview_value'></td></tr><tr><td align='right'>Dependent On</td><td><select name='tableview_dependenton'><option value=''>Select</option><?php $sql_dependent_row = "SELECT * FROM `survey_input` WHERE type != 'checkbox' AND type != 'radio' AND type != 'layer' ORDER BY display_name ASC"; $res_dependent_row = mysqli_query($link,$sql_dependent_row); while($row_dependent_row = mysqli_fetch_assoc($res_dependent_row)){ echo "<option value='".$row_dependent_row['row_id']."'>".$row_dependent_row['display_name']."</option>"; } ?></select></td></tr><tr><td align='right'>Dependent Value</td><td><input type='textbox' name='tableview_dependentvalue'></td></tr><tr><td align='right'>Action</td><td><input type='textbox' name='tableview_dependentaction'></td></tr><tr><td colspan='2' align='right'><input type='button' name='submit' value='Submit' onclick='hide_info("+k+");'></td></tr></table>";
		document.getElementById('tableview_'+k+'').innerHTML = show_data;
		document.getElementById('tableview_'+k+'').style.top = '200px';
		document.getElementById('tableview_'+k+'').style.left = '400px';
		document.getElementById('tableview_'+k+'').style.position = 'fixed';
		document.getElementById('tableview_'+k+'').style.background = '#FAFAFA';
		
		$('#curtain').fadeIn(800);
		$('#tableview_'+k).fadeIn(800);
	...
	/*if(select_option == 'tableview'){
		//document.getElementById('tableview_'+k+'').innerHTML = 'hello';
		var show_data = "<span style='border-radius:8px; width:100px; height:100px; margin-left:95%; margin-top:-5px; font-size:18px; cursor:pointer;' onclick='hide_info("+k+");'><b>X</b></span><table style='margin:20px;' cellpadding='4'><tr><td align='right'>Enter Type</td><td><select><option>Select</option></select></td></tr><tr><td align='right'>Enter Value</td><td><input type='textbox' name='textvalue'></td></tr><tr><td align='right'>Dependent On</td><td><select><option>Select</option></select></td></tr><tr><td align='right'>Dependent Value</td><td><input type='textbox' name='dependentvalue'></td></tr><tr><td align='right'>Action</td><td><input type='textbox' name='dependentaction'></td></tr><tr><td colspan='2' align='right'><input type='button' name='submit' value='Submit' onclick='hide_info("+k+");'></td></tr></table>";
		document.getElementById('tableview_'+k+'').innerHTML = show_data;
		document.getElementById('tableview_'+k+'').style.top = '200px';
		document.getElementById('tableview_'+k+'').style.left = '400px';
		document.getElementById('tableview_'+k+'').style.position = 'fixed';
...
function hide_info(k){
	$('#curtain').fadeOut(800);
	$('#tableview_'+k).fadeOut(800);
}
...
function post_data()
{
	...
	if($_REQUEST['mode']=='survey_post')
	{
		...
		$rs_row_id=mysqli_query($link,$sql_row_id);
		$rec_row_id=mysqli_fetch_assoc($rs_row_id);
		...
		for($count=0;$count <count($array_box);$count++)
		{
			...
			if($array_boxes_input[$count]=='tableview')
			{
		...
		foreach($survey_array_chunk as $key=>$index)
		{
			...
			//exit;							 
			$res_insert_survey_input = mysqli_query($link,$sql_insert_survey_input);
		}
		
		$sql_get_row_id = "SELECT MAX(row_id) FROM survey_input WHERE type = 'radio' OR type = 'checkbox'";
		$res_get_row_id = mysqli_query($link,$sql_get_row_id);
		$rows_count = mysqli_num_rows($res_get_row_id);
		if($rows_count>0){
			$res_get_row_id = mysqli_query($link,$sql_get_row_id);
			$row_get_row_id = mysqli_fetch_assoc($res_get_row_id);
			$get_row_id = $row_get_row_id['MAX(row_id)'];
			
			$sql_insert_tableview = "INSERT INTO table_view SET 
													 row_id = '".$get_row_id."', 
													   type = '".$_POST['tableview_type']."', 
													  value = '".$_POST['tableview_value']."', 
											   dependent_on = '".$_POST['tableview_dependenton']."', 
											dependent_value = '".$_POST['tableview_dependentvalue']."', 
													 action = '".$_POST['tableview_dependentaction']."'";
			
			$res_insert_tableview = mysqli_query($link,$sql_insert_tableview);
		}
