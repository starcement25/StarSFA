	...
	while($row_get_billed_details = mysqli_fetch_assoc($res_get_billed_details)){
		...
		else
		{
		   $sqlbilling="SELECT COUNT(IMEI) as consolidated_billed_qty FROM customer_product_billing  
			...
			$rsstockoutunregisteredtot=mysqli_query($link,$sqlstockoutunregisteredtot);
			$rowstockoutunregisteredtot=mysqli_fetch_assoc($rsstockoutunregisteredtot);
			...
			$rsselopeningstockunregistered=mysqli_query($link,$sqlselopeningstockunregistered);
			$rowselopeningstockunregistered=mysqli_fetch_assoc($rsselopeningstockunregistered);
				   ...
				   if(!in_array($concatenatestring,$branch_oulet_array))
					 {
						$sqlstockoutunregisteredbranchout="SELECT SUM(CASE WHEN stock_out_date='0000-00-00' AND activation_date!='0000-00-00 00:00:00' 
												...
												AND prod_code='".$prod_code."' AND SUBSTRING(activation_date,1,10) BETWEEN '".$start_date."' AND '".$end_date."'";
						$rsstockoutunregisteredbranchout=mysqli_query($link,$sqlstockoutunregisteredbranchout);
						$rowstockoutunregisteredbranchout=mysqli_fetch_assoc($rsstockoutunregisteredbranchout);
						...
						$rsstockoutqtybranchout=mysqli_query($link,$sqlstockoutqtybranchout);
						$rowstockoutqtybranchout=mysqli_fetch_assoc($rsstockoutqtybranchout);
