...
function main(){
	...
	{
		$sqlmenuaccess="SELECT not_accessible_menu FROM menu_access WHERE emp_code='".$_SESSION['admin_login']."' AND not_accessible_menu='app_order_approval'";
		$rsmenuaccess=mysqli_query($link,$sqlmenuaccess);
		$countmenuaccess=mysqli_num_rows($rsmenuaccess);
		if($countmenuaccess > 0)
		{
			echo "<center>";
	echo "<table width='50%' border=\"1\"><tr><td align='center' valign='top' ><b>You have no access to view the content of this menu</b></td></tr></table></center>";
		}
	}
	if($countmenuaccess > 0)
		{
   ...
   <script src="https://ajax.googleapis.com/ajax/libs/jquery/1.11.3/jquery.min.js"></script>
	...
	echo "<span style=\"font-weight:bold; font-size:14px;\">Order Authorize</span><br><br>";
	...
	function display_result(){
		...
		GenericAjaxFunction('star_order_authorize_data.php?employee='+employee+'&start_date='+start_date+'&end_date='+end_date+'&zone='+zone+'&state='+state+'&branch='+branch+'&department='+department,'display',0);
	...
	function exporttocsv(){
		...
		a.download = 'Order Authorize Report' + postfix + '.xls';
