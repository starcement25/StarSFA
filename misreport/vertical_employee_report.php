...
/*---------------------------------> Display vertical for ADMIN <--------------------------------*/
	if($_SESSION['admin_login'] == 'admin'){
		...
		if(strtoupper($_SESSION['nick_name']) == 'RUPA'){
			$vertical_condition=" AND EM.vertical_value NOT LIKE 'M%'";
		}
		...
		EM WHERE SUBSTRING_INDEX( EM.vertical_value, ',', -1 ) != ''".$vertical_condition;
		...
		if($total_rows>0){
			$res_attendance_verticalwise = mysqli_query($link,$sql_attendance_verticalwise);
			$vertical_select_control = "Vertical:<select name=\"vertical\" id=\"vertical\" >";
		...
		$vertical_name_array=array();
		$sql_get_vertical = "SELECT vertical_value FROM employee_master WHERE emp_code = '".$_SESSION['admin_login']."' ".$vertical_condition;
		$res_get_vertical = mysqli_query($link,$sql_get_vertical);
		$row_get_vertical = mysqli_fetch_assoc($res_get_vertical);
		...
		if(strpos($get_vertical,',') != false){
			...
			$vertical_select_control = "Vertical:<select name=\"vertical\" id=\"vertical\" >";
			$vertical_select_control .= "<option value=\"\">Select</option>";
			$vertical_select_control .= "<option value=\"'".$get_vertical_array."'\">".$get_vertical_array."</option>";
			echo $vertical_select_control .= "</select>";
			/*echo "<table width='60%' style='border-collapse:collapse;' cellpadding='6px'><tr>";
...
function show_data()
{
	...
	{
	  alert('To date will be after From date');
	  return false;
	}
	...
	//alert(diffDays);
	if(diffDays >30  || date1.getMonth()!=date2.getMonth())
	{
		alert("You can see maximum 30 days data");
	}
	else
	{
		document.getElementById("display").innerHTML = '<img src="ajax-loader.gif" id="ajaxloader">';
		GenericAjaxFunction('vertical_report_employee_data.php?vertical_name='+vertical_name+'&start_date='+start_date+'&end_date='+end_date,'display',0);
	}
}
...
function show_emp()
{
	...
	if(diffDays >=7)
	{
		alert("You can see maximum 7 days data");
	}
	else
	{
	document.getElementById("display").innerHTML = '<img src="ajax-loader.gif" id="ajaxloader">';
