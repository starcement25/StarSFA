...
define("DB","acedns_OSHEA");

mysqli_connect(SERVER,USER,PASSWORD);
mysqli_select_db(DB);

/*$sql_order_header = "SELECT order_no, SUBSTRING(order_no, -19, 5) AS emp_code, customer_code, d_instruction, DATE_FORMAT(SUBSTRING(order_no,-14,8),'%Y-%m-%d') AS date_selected, DATE_FORMAT(SUBSTRING(order_no,-14),'%H:%i:%s') AS time_selected, vertical_value
FROM `order_header`
WHERE DATE_FORMAT( SUBSTRING( order_no , -14, 8 ) , '%Y-%m-%d' ) >= '2017-03-15' AND DATE_FORMAT( 					SUBSTRING( order_no , -14, 8 ) , '%Y-%m-%d' ) <= '2017-03-15' ORDER BY DATE_FORMAT( SUBSTRING( order_no , -14, 8 ) , '%Y-%m-%d' ) DESC";*/
$sql_order_header = "SELECT order_no, SUBSTRING(order_no, -19, 5) AS emp_code, customer_code, d_instruction, DATE_FORMAT(SUBSTRING(order_no,-14,8),'%Y-%m-%d') AS date_selected, DATE_FORMAT(SUBSTRING(order_no,-14),'%H:%i:%s') AS time_selected, vertical_value
FROM `order_header` ORDER BY DATE_FORMAT( SUBSTRING( order_no , -14, 8 ) , '%Y-%m-%d' ) DESC";
...
while($row_order_header = mysqli_fetch_assoc($res_order_header)){
	...
	$row_cust_type = mysqli_fetch_assoc($res_cust_type);
