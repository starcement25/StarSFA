...
require ("attribute_selection_SIS.php");
...
function main(){
	...
	// $sql_month_selection = "SELECT DISTINCT SUBSTRING(date,1,7) AS distinct_datetime FROM location 
	// 						WHERE trans_id LIKE 'O%' AND date > '2021-04-01' ORDER BY date ASC";
	 $sql_month_selection = "
    SELECT DISTINCT 
        SUBSTRING(date,1,7) AS distinct_datetime,
        date AS full_date
    FROM location
...
";
	...
	echo "<center>";
	echo "<span style=\"font-weight:bold; font-size:14px;\">SIS Report NE</span><br><br>";
	...
	attribute_selection_NE($hidden,$get_control);
	...
	function display_result(){
		...
		GenericAjaxFunction('SIS_report_NE_data.php?employee='+employee+'&region='+region+'&month_data='+month_data+'&zone='+zone+'&state='+state+'&branch='+branch+'&department='+department,'display',0);
	...
	function Popup(data) 
	{
		var mywindow = window.open('', 'SIS Report NE', 'height=400,width=600');
		mywindow.document.write('<html><head><title>SIS Report NE</title>');
	...
	function exporttocsv(divid)
	{
		...
		a.download = 'SIS Report NE' + postfix + '.xls';
