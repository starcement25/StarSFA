...
{
  $sql_state = "SELECT EM.state as state_name FROM employee_master EM, sauda_details SD WHERE substring(SD.sauda_no,3,5)='".$row_emp['emp_code']."' ".$sauda_booked_condition." AND EM.emp_code=substring(SD.sauda_no,3,5) GROUP BY state_name ORDER BY state_name ASC";
  ...
  while($row_state = mysqli_fetch_assoc($res_state))	
  {
	  ...
	  while($row_product = mysqli_fetch_assoc($res_product))
	  {
		   ...
		   $sql_quantity_booked = "SELECT sum(SD.convert_qty_two) as mt_booked, sum(SD.qty) as case_booked FROM sauda_details SD, product_master PM, employee_master EM WHERE substring(SD.sauda_no,3,5)='".$row_emp['emp_code']."' AND EM.emp_code=substring(SD.sauda_no,3,5) AND SD.sku_code=PM.prod_code AND PM.$field_name1='$row_product[$field_name1]' AND EM.state LIKE '%".$state_name."%' ".$sauda_booked_condition;
...
foreach($statewise as $branchwiseindex=>$branchwisevalue)
{
	...
	foreach($branchwisevalue as $index=>$val)
	{
		...
		{
		   echo "
				<td align=\"right\" style=\"background:$color; color:blue; cursor:pointer; font-weight:bold;\" onclick=\"show_statewise_data('$branchwiseindex','$start_date','$end_date','$value','$index');\">".$val."</td>";
		   echo "<td align=\"right\" style=\"background:$color;\" >".$statewise_case_booked[$branchwiseindex][$index]."</td>";
		}
		else
		{
		   echo "
