...
$curdate=date('Y-m-d');
	...
	$res_sis_visibility = mysqli_query($link,$sql_sis_visibility);
	$countsis=mysqli_num_rows($res_sis_visibility);
	if($countsis >0){
		$count = 1;
		?>
		...
		<?php
		while($row_sis_visibility = mysqli_fetch_assoc($res_sis_visibility)){
