	...
	if(similar_file_exists("../csv/$folderName/SIS summary.csv")!=false)
	{
		...
		foreach($lines as $line)
		{
			...
			if($rec_count<1)
			{
				...
				$emp_code1=trim($data[0]);
				//echo $emp_code1;exit();
				$name=trim($data[1]);
				$month_year=trim($data[2]);
				$parameter_1=trim($data[3]);
				$sales_volume_TGT=trim($data[4]);
				$sales_volume_ACTUAL=trim($data[5]);
				$sis_slab_percent=trim($data[6]);
				$premium_sales_conversion_target =trim($data[7]);
				$sales_volume_SCORE_percent=trim($data[8]);
				$parameter_2=trim($data[9]);
				$monthly_unique_visit_TGT=trim($data[10]);
				$monthly_unique_visit_ACH=trim($data[11]);
				$activities_sis_slab_percent=trim($data[12]);
				$activities =trim($data[13]);
				$monthly_unique_visit_SCORE_percent=trim($data[14]);
				$parameter_3 =trim($data[15]);
				$dealer_appointment_ACTUAL=trim($data[16]);
				$dealer_appointment_sis_slab_percent=trim($data[17]);
				$influencer_registration=trim($data[18]);
				$dealer_appointment_SCORE_percent=trim($data[19]);
				$parameter_3_5=trim($data[20]);
				$parameter_4=trim($data[21]);
				$active_dealer_count_TGT=trim($data[22]);
				$active_dealer_count_ACH=trim($data[23]);
				$active_dealer_count_sis_slab_percent=trim($data[24]);
				$active_dealer_growth=trim($data[25]);
				$active_dealer_count_SCORE_percent=trim($data[26]);
				$paramiter_five=trim($data[27]);
				$five_TGT=trim($data[28]);
				$five_ACH=trim($data[29]);
				$five_sis_slab_percent=trim($data[30]);
				$active_influencer_growth=trim($data[31]);
				$five_SCORE_percent=trim($data[32]);
				
				$paramiter_six=trim($data[33]);
				$six_ACTUAL=trim($data[34]);
				$six_slab_percent=trim($data[35]);
				$six_SCORE_percent=trim($data[36]);
				$earning_score_percent=trim($data[37]);
				$penalty_percent=trim($data[38]);
				 $paramiter_seven=trim($data[39]);
				$seven_TGT=trim($data[40]);
				$seven_ACH=trim($data[41]);
				$seven_sis_slab_percent=trim($data[42]);
				$seven_WGT=trim($data[43]);
				$seven_SCORE_percent=trim($data[44]);

$final_score_percent=trim($data[45]);
				...
				$remarks=trim($data[50]);
				// $final_score_percent=trim($data[39]);
				...
				$header_id = "0";
				$sqlheader_id="SELECT MAX(header_id) as header_id FROM SIS_summary_header_new";
				$rsheader_id=mysqli_query($link,$sqlheader_id);
				$countheader_id=mysqli_num_rows($rsheader_id);
				$rowheader_id=mysqli_fetch_assoc($rsheader_id);
				if($countheader_id>0){
				    $header_id=$rowheader_id['header_id'];
				    if($header_id==null){
				        $header_id = "0";
				    }
				...
				$sqlinsis  = "insert into  SIS_summary_header_new  SET ";
				$sqlinsis .= "   header_id='".$header_id."'";
				
					$sqlinsis .= "   ,emp_code='".mysqli_real_escape_string($link,$emp_code1)."'";
					$sqlinsis .= "  ,name='".mysqli_real_escape_string($link,$name)."'";
					$sqlinsis .= "  , month_year='".addslashes($month_year)."'";
					$sqlinsis .= " , parameter_1='".mysqli_real_escape_string($link,$parameter_1)."'";
					$sqlinsis .= " , sales_volume_TGT='".mysqli_real_escape_string($link,$sales_volume_TGT)."'";
					$sqlinsis .= " , sales_volume_ACTUAL='".mysqli_real_escape_string($link,$sales_volume_ACTUAL)."'";
					$sqlinsis .= " , sis_slab_percent='".mysqli_real_escape_string($link,$sis_slab_percent)."'";
					$sqlinsis .= " , premium_sales_conversion_target='".mysqli_real_escape_string($link,$premium_sales_conversion_target)."'";
					$sqlinsis .= " , sales_volume_SCORE_percent='".mysqli_real_escape_string($link,$sales_volume_SCORE_percent)."'";
					$sqlinsis .= " , parameter_2='".mysqli_real_escape_string($link,$parameter_2)."'";
					...
					$sqlinsis .= " , activities_sis_slab_percent='".mysqli_real_escape_string($link,$activities_sis_slab_percent)."'";
					$sqlinsis .= " , activities='".mysqli_real_escape_string($link,$activities)."'";
					$sqlinsis .= " , monthly_unique_visit_SCORE_percent='".mysqli_real_escape_string($link,$monthly_unique_visit_SCORE_percent)."'";
					$sqlinsis .= " , parameter_3='".mysqli_real_escape_string($link,$parameter_3)."'";
					$sqlinsis .= " , dealer_appointment_ACTUAL='".mysqli_real_escape_string($link,$dealer_appointment_ACTUAL)."'";
					$sqlinsis .= " , dealer_appointment_sis_slab_percent='".mysqli_real_escape_string($link,$dealer_appointment_sis_slab_percent)."'";
					$sqlinsis .= " , influencer_registration='".mysqli_real_escape_string($link,$influencer_registration)."'";
					$sqlinsis .= " , dealer_appointment_SCORE_percent='".mysqli_real_escape_string($link,$dealer_appointment_SCORE_percent)."'";
					$sqlinsis .= " , parameter_3_5='".mysqli_real_escape_string($link,$parameter_3_5)."'";
					
					$sqlinsis .= " , parameter_4='".mysqli_real_escape_string($link,$parameter_4)."'";
					...
					$sqlinsis .= " , active_dealer_count_sis_slab_percent='".mysqli_real_escape_string($link,$active_dealer_count_sis_slab_percent)."'";
					$sqlinsis .= " , active_dealer_growth='".mysqli_real_escape_string($link,$active_dealer_growth)."'";
					...
					$sqlinsis .= " , five_sis_slab_percent='".mysqli_real_escape_string($link,$five_sis_slab_percent)."'";
					$sqlinsis .= " , active_influencer_growth='".mysqli_real_escape_string($link,$active_influencer_growth)."'";
					...
					$sqlinsis .= " , six_ACTUAL='".mysqli_real_escape_string($link,$six_ACTUAL)."'";
					$sqlinsis .= " , six_slab_percent='".mysqli_real_escape_string($link,$six_slab_percent)."'";
					...
					$sqlinsis .= " , parameter_seven='".mysqli_real_escape_string($link,$paramiter_seven)."'";
					$sqlinsis .= " , seven_TGT='".mysqli_real_escape_string($link,$seven_TGT)."'";
					$sqlinsis .= " , seven_ACH='".mysqli_real_escape_string($link,$seven_ACH)."'";
					$sqlinsis .= " , seven_sis_slab_percent='".mysqli_real_escape_string($link,$seven_sis_slab_percent)."'";
					$sqlinsis .= " , seven_WGT='".mysqli_real_escape_string($link,$seven_WGT)."'";
					$sqlinsis .= " , seven_SCORE_percent='".mysqli_real_escape_string($link,$seven_SCORE_percent)."'";

					$sqlinsis .= " , final_score_percent='".mysqli_real_escape_string($link,$final_score_percent)."'";
					
					$sqlinsis .= " , h6_l1='".mysqli_real_escape_string($link,$h6_l1)."'";
					$sqlinsis .= " , h6_l2='".mysqli_real_escape_string($link,$h6_l2)."'";
					$sqlinsis .= " , h6_l3='".mysqli_real_escape_string($link,$h6_l3)."'";
					$sqlinsis .= " , h6_l4='".mysqli_real_escape_string($link,$h6_l4)."'";
			...
			{ 
			  ...
			  $emp_code="";
				...
				$name=preg_replace('/\s+/', ' ', trim($data[1]));
				$month_year=trim($data[2]);
				...
				$seven_TGT=trim($data[40]);
				$seven_ACH=trim($data[41]);
				$seven_ACH_percent=trim($data[42]);
				$seven_WGT_percent=trim($data[43]);
				$seven_SCORE_percent=trim($data[44]);
				
				$earning_score_percent=trim($data[45]);
				$penalty_percent=trim($data[46]);
				$final_score_percent=trim($data[47]);
				$OTSI=trim($data[48]);
				$SIS_earning_month=trim($data[49]);
				$remarks=trim($data[50]);
				...
				if($countchkempcode ==0)
				{
					...
					$sqlinsis .= " , paramiter_seven='".mysqli_real_escape_string($link,$paramiter_seven)."'";
					$sqlinsis .= " , seven_TGT='".mysqli_real_escape_string($link,$seven_TGT)."'";
					$sqlinsis .= " , seven_ACH='".mysqli_real_escape_string($link,$seven_ACH)."'";
					$sqlinsis .= " , seven_ACH_percent='".mysqli_real_escape_string($link,$seven_ACH_percent)."'";
					$sqlinsis .= " , seven_WGT_percent='".mysqli_real_escape_string($link,$seven_WGT_percent)."'";
					$sqlinsis .= " , seven_SCORE_percent='".mysqli_real_escape_string($link,$seven_SCORE_percent)."'";
				...
				else
				{
					...
					$sqlupdate .= " , paramiter_seven='".mysqli_real_escape_string($link,$paramiter_seven)."'";
					$sqlupdate .= " , seven_TGT='".mysqli_real_escape_string($link,$seven_TGT)."'";
					$sqlupdate .= " , seven_ACH='".mysqli_real_escape_string($link,$seven_ACH)."'";
					$sqlupdate .= " , seven_ACH_percent='".mysqli_real_escape_string($link,$seven_ACH_percent)."'";
					$sqlupdate .= " , seven_WGT_percent='".mysqli_real_escape_string($link,$seven_WGT_percent)."'";
					$sqlupdate .= " , seven_SCORE_percent='".mysqli_real_escape_string($link,$seven_SCORE_percent)."'";
	...
	if(similar_file_exists("../csv/$folderName/BD SIS summary.csv")!=false)
	{
		$filename=similar_file_exists("../csv/$folderName/BD SIS summary.csv");
		$rec_count = 0;
		$ins_count = 0;
		$err = "";
		$header_id = "0";
		...
		foreach($lines as $line)
		{
			...
			if($rec_count<1)
			{
				...
				$influencer_registration=trim($data[18]);
				$dealer_appointment_SCORE_percent=trim($data[19]);
				$parameter_4=trim($data[20]);
				$active_dealer_count_TGT=trim($data[21]);
				$active_dealer_count_ACH=trim($data[22]);
				$active_dealer_count_sis_slab_percent=trim($data[23]);
				$active_dealer_growth=trim($data[24]);
				$active_dealer_count_SCORE_percent=trim($data[25]);
				$paramiter_five=trim($data[26]);
				$five_TGT=trim($data[27]);
				$five_ACH=trim($data[28]);
				$five_sis_slab_percent=trim($data[29]);
				$active_influencer_growth=trim($data[30]);
				$five_SCORE_percent=trim($data[31]);
				
				$paramiter_six=trim($data[32]);
				$six_ACTUAL=trim($data[33]);
				$six_slab_percent=trim($data[34]);
				$six_SCORE_percent=trim($data[35]);
				$earning_score_percent=trim($data[36]);
				$penalty_percent=trim($data[37]);
				$final_score_percent=trim($data[38]);
				$remarks=trim($data[39]);
				
				
				$header_id = "0";
				$sqlheader_id="SELECT MAX(header_id) as header_id FROM BD_SIS_summary_header";
				$rsheader_id=mysqli_query($link,$sqlheader_id);
				$countheader_id=mysqli_num_rows($rsheader_id);
				$rowheader_id=mysqli_fetch_assoc($rsheader_id);
				if($countheader_id>0){
				...
				$sqlinsis  = "insert into  BD_SIS_summary_header  SET ";
				$sqlinsis .= "   header_id='".$header_id."'";
					...
					mysqli_query($link,$sqlinsis) or array_push($error_array,"mysqli_error().Duplicate key @row $csv_row_count in BD sis summary.csv.Please check.");
			...
			{ 
			  ...
			  //print_r($data);
				...
				$sqlchkempcode="SELECT emp_code FROM  BD_SIS_summary WHERE emp_code='".addslashes($emp_code)."' AND month_year='".addslashes($month_year)."'";
				$rschkempcode=mysqli_query($link,$sqlchkempcode);
				$countchkempcode=mysqli_num_rows($rschkempcode);
				//$rowchkcustomercode=mysqli_fetch_assoc($rschkcustomercode);
				//$customer_code=$rowcustomercode['customer_code'];
				...
				if($countchkempcode ==0)
				{
					$sqlinsis  = "insert into  BD_SIS_summary  SET ";
					$sqlinsis .= "   emp_code='".mysqli_real_escape_string($link,$emp_code)."'";
					$sqlinsis .= "  ,name='".mysqli_real_escape_string($link,$name)."'";
					$sqlinsis .= "  , month_year='".addslashes($month_year)."'";
					$sqlinsis .= " , parameter_1='".mysqli_real_escape_string($link,$parameter_1)."'";
				...
				{
					$sqlupdate  = "UPDATE  BD_SIS_summary  SET ";
					$sqlupdate .= " parameter_1='".mysqli_real_escape_string($link,$parameter_1)."'";
					$sqlupdate .= " , sales_volume_TGT='".mysqli_real_escape_string($link,$sales_volume_TGT)."'";
					$sqlupdate .= " , sales_volume_ACTUAL='".mysqli_real_escape_string($link,$sales_volume_ACTUAL)."'";
					$sqlupdate .= " , sis_slab_percent='".mysqli_real_escape_string($link,$sis_slab_percent)."'";
					$sqlupdate .= " , premium_sales_conversion_target='".mysqli_real_escape_string($link,$premium_sales_conversion_target)."'";
					$sqlupdate .= " , sales_volume_SCORE_percent='".mysqli_real_escape_string($link,$sales_volume_SCORE_percent)."'";
					$sqlupdate .= " , parameter_2='".mysqli_real_escape_string($link,$parameter_2)."'";
					$sqlupdate .= " , monthly_unique_visit_TGT='".mysqli_real_escape_string($link,$monthly_unique_visit_TGT)."'";
					$sqlupdate .= " , monthly_unique_visit_ACH='".mysqli_real_escape_string($link,$monthly_unique_visit_ACH)."'";
					$sqlupdate .= " , activities_sis_slab_percent='".mysqli_real_escape_string($link,$activities_sis_slab_percent)."'";
					$sqlupdate .= " , activities='".mysqli_real_escape_string($link,$activities)."'";
					$sqlupdate .= " , monthly_unique_visit_SCORE_percent='".mysqli_real_escape_string($link,$monthly_unique_visit_SCORE_percent)."'";
					$sqlupdate .= " , parameter_3='".mysqli_real_escape_string($link,$parameter_3)."'";
					$sqlupdate .= " , dealer_appointment_ACTUAL='".mysqli_real_escape_string($link,$dealer_appointment_ACTUAL)."'";
					$sqlupdate .= " , dealer_appointment_sis_slab_percent='".mysqli_real_escape_string($link,$dealer_appointment_sis_slab_percent)."'";
					$sqlupdate .= " , influencer_registration='".mysqli_real_escape_string($link,$influencer_registration)."'";
					$sqlupdate .= " , dealer_appointment_SCORE_percent='".mysqli_real_escape_string($link,$dealer_appointment_SCORE_percent)."'";
					$sqlupdate .= " , parameter_4='".mysqli_real_escape_string($link,$parameter_4)."'";
					$sqlupdate .= " , active_dealer_count_TGT='".mysqli_real_escape_string($link,$active_dealer_count_TGT)."'";
					$sqlupdate .= " , active_dealer_count_ACH='".mysqli_real_escape_string($link,$active_dealer_count_ACH)."'";
					$sqlupdate .= " , active_dealer_count_sis_slab_percent='".mysqli_real_escape_string($link,$active_dealer_count_sis_slab_percent)."'";
					$sqlupdate .= " , active_dealer_growth='".mysqli_real_escape_string($link,$active_dealer_growth)."'";
					$sqlupdate .= " , active_dealer_count_SCORE_percent='".mysqli_real_escape_string($link,$active_dealer_count_SCORE_percent)."'";
					
					$sqlupdate .= " , paramiter_five='".mysqli_real_escape_string($link,$paramiter_five)."'";
					$sqlupdate .= " , five_TGT='".mysqli_real_escape_string($link,$five_TGT)."'";
					$sqlupdate .= " , five_ACH='".mysqli_real_escape_string($link,$five_ACH)."'";
					$sqlupdate .= " , five_sis_slab_percent='".mysqli_real_escape_string($link,$five_sis_slab_percent)."'";
					$sqlupdate .= " , active_influencer_growth='".mysqli_real_escape_string($link,$active_influencer_growth)."'";
					$sqlupdate .= " , five_SCORE_percent='".mysqli_real_escape_string($link,$five_SCORE_percent)."'";
					$sqlupdate .= " , paramiter_six='".mysqli_real_escape_string($link,$paramiter_six)."'";
					$sqlupdate .= " , six_ACTUAL='".mysqli_real_escape_string($link,$six_ACTUAL)."'";
					$sqlupdate .= " , six_slab_percent='".mysqli_real_escape_string($link,$six_slab_percent)."'";
					$sqlupdate .= " , six_SCORE_percent='".mysqli_real_escape_string($link,$six_SCORE_percent)."'";
					$sqlupdate .= " , earning_score_percent='".mysqli_real_escape_string($link,$earning_score_percent)."'";
					$sqlupdate .= " , penalty_percent='".mysqli_real_escape_string($link,$penalty_percent)."'";
					$sqlupdate .= " , final_score_percent='".mysqli_real_escape_string($link,$final_score_percent)."'";
					$sqlupdate .= " , remarks='".mysqli_real_escape_string($link,$remarks)."'";
					$sqlupdate .= "   ,header_id='".$header_id."'";
					$sqlupdate .= " , download_time=CURRENT_TIMESTAMP() WHERE emp_code='".addslashes($emp_code)."' AND month_year='".addslashes($month_year)."'";
					mysqli_query($link,$sqlupdate) or array_push($error_array,"mysqli_error().Internal error occurs @row $csv_row_count in BD sis summary.csv.Please check.");
				}
			}
			 $rec_count++;
		}
