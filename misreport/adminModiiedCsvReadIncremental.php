		...
		if(similar_file_exists("../csv/$folderName/Sku master.csv")!=false)
		{
			...
			foreach($lines as $line)
			{
				...
				{ 
					...
					$acedns=trim($data[7]);
					if($acedns != 'Y' && $acedns != 'N')
					{
						echo "Please provide proper value for Acedns column at row ".$csv_row_count;
						die;
					}
					
					$black_list=trim($data[8]);
					if($black_list != 'Y' && $black_list != 'N')
					{
						echo "Please provide proper value for Blacklist column at row ".$csv_row_count;
						die;
					}
				...
				{ 
				  ...
				  $data[]=$value;
					...
					if(providing_code == 'yes')
					{
						if($reporting_to!='')
						{
							$sql_check_emp_dns_code = "SELECT emp_code FROM employee_master WHERE dns_emp_code = '".$reporting_to."'";
							$res_check_emp_dns_code = mysqli_query($link,$sql_check_emp_dns_code);
							$total_rows_dns_emp_code = mysqli_num_rows($res_check_emp_dns_code);
							if($total_rows_dns_emp_code == 0)
							{
								echo "Please provide dns employee code in reporting to at row ".$csv_row_count;
								die;
							}
						}
					}
			...
			foreach($lines as $line)
			{
				...
				{ 
				  ...
				  //print_r($data);
					...
					/*if(providing_code == 'yes')
					{
						$sql_check_dns_code = "SELECT emp_code FROM employee_master WHERE dns_emp_code = '".$emp_code_name."'";
						$res_check_dns_code = mysqli_query($link,$sql_check_dns_code);
						$total_rows = mysqli_num_rows($res_check_dns_code);
						if($total_rows == 0)
						{
							echo "Please provide DNS Employee Code at row ".$csv_row_count;
							die;
						}
						
						if($route_name != '' && $dns_route_code == '')
						{
							echo "Please provide dns route code at row ".$csv_row_count;
						}
					}*/
					$acedns		  =trim($data[6]);
					if($acedns != 'Y' && $acedns != 'N')
					{
						echo "Please provide value for Acedns at row ".$csv_row_count;
						die;
					}
					...
					if($black_list != 'Y' && $black_list != 'N')
					{
						echo "Please provide value for Blacklist at row ".$csv_row_count;
						die;
					}
					...
					$sqlrouteempchk="SELECT emp_code FROM emp_route_relation WHERE route_code='".addslashes($route_code)."' AND emp_code='".$emp_code."'";
					$rsrouteempchk=mysqli_query($link,$sqlrouteempchk);
					$countrouteempchk=mysqli_num_rows($rsrouteempchk);
					if($countrouteempchk<1 )
					{
						$sqlemproute  = "insert into emp_route_relation ";
						$sqlemproute .= " SET route_code='".$route_code."'";
						$sqlemproute .= " ,emp_code='".$emp_code."'";
						$sqlemproute .= " , download_time=CURRENT_TIMESTAMP()";
						mysqli_query($link,$sqlemproute) or  array_push($error_array,"mysqli_error().Internal DATA execution problem on emp route relation table.PLease contact aceDNS admin.");
					}
					...
					else
					{
						...
						if(($credit_limit_db==$credit_limit) && ($route_code_db!=$route_code || $current_balance_db!=$current_balance || $acedns_db!=$acedns || $black_list_db!=$black_list || $TD_db!=$TD || $customer_type_db!=$customer_type || $rds_tag_db!=$rds_tag 
							|| $branch_code_db!=$branch_code || $credit_days_db!=$credit_days))
						{
							...
							$sqlupdated .= " , rds_tag='".$rds_code."',download_time=CURRENT_TIMESTAMP() 
											 WHERE customer_name='".addslashes($customer_name)."' AND route_code='".$route_code."'";
