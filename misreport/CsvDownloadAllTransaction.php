...
class ZipFile
{
 ...
 //For order header csv download
	...
	$sql_order_header_download = "SELECT OH.order_no,OH.customer_code,OH.d_instruction,OH.sale_type,LO.emp_code,OH.tag_distributor_code,
								DATE_FORMAT(LO.date,'%d/%m/%Y') AS order_date,OD.sku_code,OD.qty,OD.amount,OD.UOM,PM.prod_desc,PM.product_group_code,PM.conversion_factor,OD.sale_rate".$TD_condition."
	...
	{ 
		...
		$UOM_order=$rec_order_header_download['UOM'];
		$conversion_factor=$rec_order_header_download['conversion_factor'];	
		...
		//}
			...
			$rowproductgroup=mysqli_fetch_assoc($rsproductgroup);
	...
	$rs_payment_header_download = mysqli_query($link,$sql_payment_header_download) or die(mysqli_error()." Error in payment header download: ".$sql_payment_header_download);
	$linepaymentheader = ''; 
	while($rec_payment_header_download = mysqli_fetch_assoc($rs_payment_header_download)) 
	{ 
		...
		$rsupdatepaymentheader=mysqli_query($link,$sqlupdatepaymentheader)or die(mysqli_error()." Error in payment header update: ".$sqlupdatepaymentheader);*/
	} 
	...
	$rs_prospect_download = mysqli_query($link,$sql_prospect_download) or die(mysqli_error()." Error in prospect download: ".$sql_prospect_download);
	$lineprospect = ''; 
	while($rec_prospect_download = mysqli_fetch_assoc($rs_prospect_download)) 
	{ 
		...
		$rsupdateprospectheader=mysqli_query($link,$sqlupdateprospectheader)or die(mysqli_error()." Error in prospect header update: ".$sqlupdateprospectheader);*/
	...
	$arr_header = array('Visit Date','Employee code','Employee name','Joint work employee name','Customer remarks','Employee remarks');
	$headerjointwork = "";
	foreach($arr_header AS $header_value)
	{
	...
	header("Content-Disposition: attachment; filename=csv_files.zip");
	echo $zip->file();
	exit();*/
	header("Content-type: application/octet-stream");
	...
	fputs($file, $contents);
	fclose($file);*/
	//header("location: http://salesmpower.acedns.in/csvdownload/$folderName/csv_files.zip");
	//echo $zip->file();
	//echo $zip->file();*/
