...
if($mode=='producttent')
  {
    ...
    $sqlproduct="SELECT product,brand,life_of_product FROM  tent_form_product_details WHERE mobile_no='".$cust_mobile."'  ORDER BY product ASC";
		 ...
		 $life_of_product=$rowprod['life_of_product'];
		  ...
		  $select_control.='
                     ...
                     onMouseOver="this.bgColor='.SCROLL_COLOR.'" onMouseOut="this.bgColor=" class="body">
                        ...
                        <td align="left">'.$life_of_product.'</td>
                     </tr>   
                    '; 
	}
	}
	...
	{
		 $select_control.="<tr>";
          $select_control.="<td align='center' colspan=\"3\">No Records.</td>";
          $select_control.= "</tr>"; 
	}
  ...
  if($mode=='productselect')
  {
   ...
   $sqlproddesc="SELECT PM.prod_code,PM.dns_prod_code,PM.prod_desc,PM.product_group_code,PM.focus,MRP.sale_rate,PM.pack_size FROM product_master PM,mrp MRP 
	...
	$countproddesc=mysqli_num_rows($rsproddesc);
	$product_group_code_array=array();
  ...
  if($mode=='productgroupsel')
  {
	...
	$rsproddesc=mysqli_query($link,$sqlproddesc);
	$product_group_code_array=array();
	$res_product_group = mysqli_query($link,$sql_product_group);
 ...
 if($mode=='approvalsubcat')
  ...
  if($mode=='defocusing')
  {
	  ...
	  $sqlupdatefocus="UPDATE product_master SET focus='N',download_time=CURRENT_TIMESTAMP() WHERE prod_code='".$prod_code."'";
		if(mysqli_query($link,$sqlupdatefocus))
		{
