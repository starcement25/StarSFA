...
$header_string = "Zone:".$zone."&nbsp;&nbsp;State:".$state."&nbsp;&nbsp;Branch:".$branch."&nbsp;&nbsp;Department:".$department."&nbsp;&nbsp;Employee:".$new_emp_name."&nbsp;&nbsp;From:".date('d-m-Y',strtotime($start_date))."&nbsp;&nbsp;To:".date('d-m-Y',strtotime($end_date));
	...
	// 			ORDER BY SUBSTRING(trans_id,-14,14) DESC) AS SAT GROUP BY 1,2,3 ORDER BY SAT.emp_name ASC";
	  ...
	  $sql_customer = "SELECT * FROM(SELECT DISTINCT CVD.emp_code,DATE_FORMAT(SUBSTRING(CVD.trans_id,-14,8),'%Y-%m-%d') AS visit_date,
                SUBSTRING(CVD.trans_id,-14,14) AS visit_date_parts,CVD.customer_code,CVD.customer_name,CVD.route_name,CVD.route_code,
                CVD.cust_type,EM.emp_name,EM.dns_emp_code,CM.dns_customer_code,BM.branch_name,CVD.trans_id,CVD.hint_remarks, LO.purpose_of_visit, LO.date as ch_date, LO.updatetime as ch_o_date
	...
	if($total_customer>0){
		...
		while($row_customer = mysqli_fetch_assoc($res_customer)){
			...
			$ch_date = $row_customer['ch_date'];
			$ch_o_date = $row_customer['ch_o_date'];
			...
			/*if ($check_in_time == '') {
			$check_in_time = date("H:i:s", strtotime($ch_date));
		} else {
			$check_in_time = date("H:i:s", strtotime($check_in_time));
		}

		if ($check_out_time == '') {
			$check_out_time = date("H:i:s", strtotime($ch_o_date));
		} else {
			$check_out_time = date("H:i:s", strtotime($check_out_time));
		}*/

				//change on 18-02-26 repet data 
					// Reset variables first to avoid stale data
					$check_in_time = '';
					$check_out_time = '';

					// Build dynamic variable names
					$check_in_key  = 'check_in_time'  . $customer_code . $visit_date_formated . $emp_code;
					$check_out_key = 'check_out_time' . $customer_code . $visit_date_formated . $emp_code;

					// Only assign if the dynamic variable actually exists
					$check_in_time  = isset($$check_in_key)  ? $$check_in_key  : '';
					$check_out_time = isset($$check_out_key) ? $$check_out_key : '';
