...
if($reason_type == 'hold' || $reason_type == 'spl permission'){
	...
	mysqli_query($link,$sql_update_order_header);
	...
	while($row_order_details = mysqli_fetch_assoc($res_order_details)){
		...
		mysqli_query($link,$sql_insert_order_track);
	}
	...
	foreach($billed_qty_array as $billed_qty_val){
		...
		$res_update_order_details = mysqli_query($link,$sql_update_order_details);
		
		$sql_order_qty = "SELECT qty, billed_qty FROM order_details WHERE order_no = '".$order_no_get."' AND sku_code = '".$sku_code."'";
		$res_order_qty = mysqli_query($link,$sql_order_qty);
		$row_order_qty = mysqli_fetch_assoc($res_order_qty);
		...
		$sql_insert_order_track = "INSERT INTO order_tracking SET 
										...
										`invoice_amount` = '".$invoice_amt."',
												`status` = '".$set_status."'";
		$res_insert_order_track = mysqli_query($link,$sql_insert_order_track);
	}
	...
	$res_update_order_header = mysqli_query($link,$sql_update_order_header);
