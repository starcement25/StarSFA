...
if($_REQUEST['mode']=='swapping_emp')
{
	...
	$rsfetchcutomerroutetwo=mysqli_query($link,$sqlfetchcutomerroutetwo);
	while($rowfetchcutomerroutetwo=mysqli_fetch_assoc($rsfetchcutomerroutetwo))
	{
	...
	for($i=0;$i<count($customer_code_one_array);$i++)
	{
		...
		mysqli_query($link,$sqlupdatecustomerrouteone);
	...
	for($k=0;$k<count($customer_code_two_array);$k++)
	{
		...
		mysqli_query($link,$sqlupdatecustomerroutetwo);
