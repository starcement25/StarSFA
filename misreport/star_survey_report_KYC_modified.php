...
if($total_rows>0){
	...
	while($row_survey_ouput = mysqli_fetch_assoc($res_survey_output)){
		...
		while($row_survey_details = mysqli_fetch_assoc($res_survey_details)){
			...
			if($survey_row_id == 'RA004')
				...
				foreach($site_image_array as $image){
					...
					${'image_string'.$survey_id} .= "<a href=\"https://starcement1-sbinfo-upload.s3.ap-south-1.amazonaws.com/STAR/".$image."\" target=\"_blank\" style=\"color:brown;\">View</a><br>";
				}
			}
		...
		if((strtoupper($sub_category)=='DEALER' || strtoupper($sub_category)=='SUB DEALER') && $active=='inactive' && strtoupper($_SESSION['admin_login'])=='ADMIN'){
			...
			$rsdnscode=mysqli_query($link,$sqldnscode);
			$rowdnscode=mysqli_fetch_assoc($rsdnscode);
