...
/* ini_set('display_errors', 1);
 ...
 error_reporting(E_ALL);*/
	...
	$mode = $_REQUEST['mode'];
$row_id_value=$_REQUEST['row_id'];

	if($mode =='add' || $mode =='edit')				 disphtml("show_add_edit($row_id_value);");
	...
	elseif($mode =='access')							disphtml("access_add_edit('".$row_id_value."');");
	else    											disphtml("main();");
ob_end_flush();
...
function main()
{
	...
	if($GLOBALS['err_msg']==''){ $GLOBALS['err_msg']='';};
	?><head>
...
<script type="text/javascript" src="ajax1.js"></script>
	...
	if(strtoupper($_SESSION['nick_name'])=='RKBK')
	{
		...
		$sql_count ="SELECT COUNT(EM.emp_code) FROM employee_master EM,changepassword CH WHERE CH.emp_code=EM.emp_code AND
					EM.acedns='Y' ".$emp_hierarchy_condition_one."";
	}
	else
	{
	/*$sql_count = "SELECT COUNT(EM.emp_code) FROM employee_master EM,changepassword CH WHERE CH.emp_code=EM.emp_code
	 ...
	 $sql_count = "SELECT COUNT(EM.emp_code) FROM employee_master EM,changepassword CH WHERE CH.emp_code=EM.emp_code
				AND EM.acedns='Y' AND EM.emp_code<>'C0007'";
	}
	...
	if(strtoupper($_SESSION['nick_name'])=='RKBK')
	{
		...
		$sql="SELECT EM.* FROM employee_master EM,changepassword CH WHERE CH.emp_code=EM.emp_code AND EM.acedns='Y'
				".$emp_hierarchy_condition_one." ORDER BY EM.emp_code ASC";

		/*$row=mysqli_fetch_assoc(mysqli_query($link,"SELECT COUNT(EM.emp_code) FROM employee_master EM,changepassword CH WHERE CH.emp_code=EM.emp_code
		...
		$row=mysqli_fetch_assoc(mysqli_query($link,"SELECT COUNT(EM.emp_code) FROM employee_master EM,changepassword CH WHERE CH.emp_code=EM.emp_code
				AND EM.acedns='Y' ".$emp_hierarchy_condition_one." ORDER BY EM.emp_code ASC"));
	}
	else if(strtoupper($_SESSION['nick_name'])=='EMAMI'){
		$sql="SELECT EM.* FROM employee_master EM,changepassword CH WHERE CH.emp_code=EM.emp_code AND EM.acedns='Y' AND 
				EM.vertical_value='".$_SESSION['vertical_value']."' ".$emp_hierarchy_condition_one." ORDER BY EM.emp_code ASC";
		$row=mysqli_fetch_assoc(mysqli_query($link,"SELECT COUNT(EM.emp_code) FROM employee_master EM,changepassword CH WHERE CH.emp_code=EM.emp_code
				AND EM.acedns='Y' AND EM.vertical_value='".$_SESSION['vertical_value']."' ".$emp_hierarchy_condition_one." ORDER BY EM.emp_code ASC"));
	}
	else
	{
	 /*$sql="SELECT EM.* FROM employee_master EM,changepassword CH WHERE CH.emp_code=EM.emp_code AND
	...
	/*$row=mysqli_fetch_assoc(mysqli_query($link,"SELECT COUNT(EM.emp_code) FROM employee_master EM,changepassword CH WHERE CH.emp_code=EM.emp_code
	 ...
	 $row=mysqli_fetch_assoc(mysqli_query($link,"SELECT COUNT(EM.emp_code) as tot_emp FROM employee_master EM,changepassword CH WHERE CH.emp_code=EM.emp_code
		AND EM.acedns='Y' AND EM.emp_code<>'C0007' ORDER BY EM.emp_code ASC"));
	$sql="SELECT EM.* FROM employee_master EM,changepassword CH WHERE CH.emp_code=EM.emp_code AND EM.acedns='Y' AND EM.emp_code<>'C0007'
...
<table width="100%" align="center" cellpadding="2" cellspacing="2" border="0" id="display_table">
	...
	<tr>
		<td valign="top" bgcolor="#FFFFFF">
			...
			<table width="85%" align="center" border="0" cellpadding="5" cellspacing="2" class="border">
				<tr class="TDHEAD" >
					<td colspan="14">Employee Information</td>
				</tr>
			<?
			if($count == 0)
			{
			...
			?>
				<tr class="TDHEAD_SUB">
					...
					<td width="" align="left" style="padding-left:20px;">State</td>
                    <td width="15%" align="left" style="padding-left:20px;">DOJ</td>
				...
				while($rec=mysqli_fetch_assoc($rs))
				{
					...
					if($reporting_to == ''){
						...
						foreach($emp_array as $emp_val){
							...
							while($row_emp_name = mysqli_fetch_assoc($res_emp_name)){
					...
					$deviceid_res=mysqli_query($link,$deviceid_sql) or die(mysqli_error()." Error in fetch deviceid");
					$countdeviceid=mysqli_num_rows($deviceid_res);
					$row_device_id = mysqli_fetch_assoc($deviceid_res);
					...
					if(providing_code == 'yes'){
						...
						$res_dns_emp_code = mysqli_query($link,$sql_dns_emp_code);
						$row_dns_emp_code = mysqli_fetch_assoc($res_dns_emp_code);
						$dns_emp_code = $row_dns_emp_code['dns_emp_code'];
					}

					if($rec['DOJ']=='0000-00-00' || $rec['DOJ']=='1971-01-01') $DOJ='';
					else						$DOJ=date('d-m-Y',strtotime($rec['DOJ']));
				?>
				<tr onMouseOver="this.bgColor='<?=SCROLL_COLOR;?>'" onMouseOut="this.bgColor=''" class="body">
                    ...
                    <td align="left" valign="top" style="padding-left:20px;"><?=stripslashes($rec['state']);?></td>
                     <td align="left" valign="top" style="padding-left:20px;"><?=stripslashes($DOJ);?></td>
	...
	<br>
	<form name="frm_opts" action="adminEmployeeAccess.php" method="post" >
		<input type="hidden" name="mode" value="<?=$_REQUEST['mode']?>">
		<input type="hidden" name="pageNo" value="<?=$_REQUEST['pageNo']?>">
		<input type="hidden" name="url" value="adminEmployeeAccess.php">
...
function access_add_edit($row_id = '')
{
	require("include/dbcon.php");
		...
		$rsemp=mysqli_query($link,$sqlemp) or die(mysqli_error()." Error in show shop edit: ".$sqlemp);
		...
		$rsempPassword=mysqli_query($link,$sqlempPassword) or die(mysqli_error()." Error in Fetch employee change password details: ".$sqlempPassword);
		$countempPassword=mysqli_num_rows($rsempPassword);
		$rowempPassword=mysqli_fetch_assoc($rsempPassword);
...
<table width="100%" align="center" cellpadding="2" cellspacing="2" border="0">
	...
	<tr>
		<td valign="top" bgcolor="#FFFFFF">
			<br>
			<br>
			<form name="frmedit" method="post" action="adminEmployeeAccess.php" onSubmit="return check(this);">
			...
			<table width="50%" align="center" class="border" cellpadding="5" cellspacing="2">
				...
				<tr>
					<td>&nbsp;</td>
					<td>&nbsp;</td>
					<td><input type="submit" value=" Change " class="inplogin">&nbsp;&nbsp;<input type="button" name="btn" value="Cancel" onClick="javascript:window.location='adminEmployeeAccess.php';" class="inplogin"></td>
				</tr>
			</table>
			</form>
		</td>
...
function change_pwd()
{
	...
	$pwd_res=mysqli_query($link,$pwd_sql) or die(mysqli_error()." Error in fetch details");

	$pwd_sql_admin="SELECT * FROM admin_master WHERE  admin_login = '".$emp_code."'";
	$pwd_res_admin=mysqli_query($link,$pwd_sql_admin) or die(mysqli_error()." Error in fetch admin details");


	$sqlemp="SELECT emp_name FROM employee_master WHERE emp_code = '".$emp_code."'";
	$rsemp=mysqli_query($link,$sqlemp) or die(mysqli_error()." Error in show emp name: ".$sqlemp);
	$rowemp=mysqli_fetch_assoc($rsemp);
	$emp_name	=$rowemp['emp_name'] ;

	$sqlemail="SELECT admin_email FROM admin_master WHERE admin_id = 1";
	$rsemail=mysqli_query($link,$sqlemail) or die(mysqli_error()." Error in fetch admin email: ".$sqlemail);
	...
	if(mysqli_num_rows($pwd_res)>0)
	{
		...
		mysqli_query($link,$upd_sql) or die(mysqli_error()." Error in pasword updation.");
	...
	else
	{
		$ins_sql="INSERT INTO  changepassword SET
				  ...
				  oldpassword ='".trim($_POST['oldpassword'])."',
				  emp_code = '" .$emp_code."'";
		mysqli_query($link,$ins_sql) or die(mysqli_error()." Error in pasword insertion.");
	...
	if(mysqli_num_rows($pwd_res_admin)>0)
	{
		...
		mysqli_query($link,$upd_sql_admin) or die(mysqli_error()." Error in pasword updation admin.");
	}
...
function clear_allocation()
{
    ...
    mysqli_query($link,$upd_sql) or die(mysqli_error()." Error in device allocation updation.");
