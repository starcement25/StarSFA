...
class ZipFile
{
	...
	//For survey excel download
		...
		if($survey_type == 'no'){
			if($survey_menu=='yes')
			{
			  $sql_get_menu = "SELECT layout_name,menu_id,survey_sub_menu FROM survey_input_bkup WHERE type='menu' AND menu_id IN('RA253','RA254','RA280','RA281') ORDER BY display_order ASC";
			 ...
			 while($row_get_menu = mysqli_fetch_assoc($res_get_menu))
			 {
				 ...
				 ${survey_id_array.$menu_id}=array();
				   ...
				   $sql_get_display="SELECT display_name,row_id,action,display_order FROM survey_input_bkup WHERE  type!='menu' AND menu_id='".$menu_id."'  AND acedns='Y'
				  ...
				  while($row_survey_output=mysqli_fetch_assoc($rs_survey_output))
				  {
					...
					$sqlmasterview="SELECT type,display_table_name,insert_table_detail FROM survey_input_bkup WHERE  row_id='".$row_survey_output['row_id']."'";
					...
					if(strtoupper($_SESSION['nick_name'])=='DURO')
					{
					 ...
					 if(strtoupper($_SESSION['nick_name'])=='SAI')
						...
						{
							if($row_survey_output['row_id']=='RA291')
							{
							$valueparts=explode(";",$value);
							$value=$valueparts[0];
							}
			...
			if(strtoupper($_SESSION['nick_name'])=='SAI')
			{
			$sql_get_menu = "SELECT DISTINCT survey_sub_menu FROM survey_input_bkup WHERE survey_sub_menu!='KYC' ORDER BY survey_sub_menu ASC";
			$res_get_menu = mysqli_query($link,$sql_get_menu);
			$count_menu=mysqli_num_rows($res_get_menu);
			$menu_no=1;
			//$menu_id_array=array();
			...
			$survey_sub_menu_array=array();
			 while($row_get_menu = mysqli_fetch_assoc($res_get_menu))
			 {
				...
				// ${array_header.$menu_id}=array();
					//$sql_get_display="SELECT display_name,row_id,action FROM survey_input_bkup WHERE type!='menu' AND menu_id='".$menu_id."' ORDER BY display_order ASC";
					    $sql_get_display="SELECT display_name,row_id,action FROM survey_input_bkup WHERE 
			...
			{
			//$sql_get_menu = "SELECT layout_name,menu_id FROM survey_input_bkup WHERE type='menu' ORDER BY display_order ASC";
			$sql_get_menu = "SELECT DISTINCT survey_sub_menu FROM survey_input_bkup ORDER BY survey_sub_menu ASC";
			$res_get_menu = mysqli_query($link,$sql_get_menu);
			$count_menu=mysqli_num_rows($res_get_menu);
			$menu_no=1;
			//$menu_id_array=array();
			...
			$sql_get_menu = "SELECT layout_name,menu_id FROM survey_input_bkup WHERE type='menu' AND survey_type='".$survey_type."' ORDER BY display_order ASC";
			$res_get_menu = mysqli_query($link,$sql_get_menu);
			$count_menu=mysqli_num_rows($res_get_menu);
			$menu_no=1;
			$menu_id_array=array();
			$menu_name_array=array();
			 ...
			 while($row_get_menu = mysqli_fetch_assoc($res_get_menu))
			 {
				...
				// ${array_header.$menu_id}=array();
				$sql_layer_check = "SELECT row_id FROM survey_input_bkup WHERE type = 'layer'";
				$res_layer_check = mysqli_query($link,$sql_layer_check);
				$total_layer_check = mysqli_num_rows($res_layer_check);
				
				
				 $sql_get_layer = "SELECT layout_name,row_id FROM survey_input_bkup WHERE type='layer' AND 
				 ...
				 while($row_get_layer = mysqli_fetch_assoc($res_get_layer))
				 {
						...
						$sql_get_display="SELECT display_name,row_id,action FROM survey_input_bkup WHERE layout_name='".$layout_name."' 
