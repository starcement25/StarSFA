...
if($_GET['vertical_name'])
{
	...
	if($_GET['start_date'] != '' && $_GET['end_date'] != '')
	{
		...
		$primary_secondary_quantity_condition = "AND  (DATE_FORMAT(SUBSTRING(LO.trans_id,-14,8),'%Y-%m-%d') BETWEEN '".$start_date."' AND '".$end_date."')";
		if($_GET['start_date'] != $_GET['end_date'])
		{
			$table_columnname = 'No of days present';
			$count_transid_condition = " ,COUNT(LO.trans_id) as attendance ";
		}
		else
		{
			$table_columnname = 'Attendance';
			$count_transid_condition = " ,SUBSTRING(LO.date,12) as attendance";
		}
	}
}
...
else
{
	$table_columnname = 'Attendance';
	$condition = " AND SUBSTRING(LO.date,1,10)='".$start_date."' ";
	$primary_secondary_quantity_condition = "AND  DATE_FORMAT(SUBSTRING(LO.trans_id,-14,8),'%Y-%m-%d')='".$start_date."'";
}
...
$res_attendance_verticalwise = mysqli_query($link,$sql_attendance_verticalwise);
$total_rows = mysqli_num_rows($res_attendance_verticalwise);*/
...
$res_distinct_vertical_EM = mysqli_query($link,$sql_distinct_vertical_EM);
while($row_distinct_vertical_EM = mysqli_fetch_assoc($res_distinct_vertical_EM)){
...
$res_distinct_vertical_OH = mysqli_query($link,$sql_distinct_vertical_OH);
while($row_distinct_vertical_OH = mysqli_fetch_assoc($res_distinct_vertical_OH)){
...
{*/
	...
	echo "<tr class='TDHEAD'><td colspan='8' align='center'>Daily Activity Analysis - ".date('d-m-Y',strtotime($_GET['start_date'])).' To '.date('d-m-Y',strtotime($_GET['end_date']))." </td></tr>";
	  ...
	  $vertical_name  = $_GET['vertical_name'];
		...
		if($vertical_name != ''){
			if(!in_array($vertical_name,$vertical_array)){
				...
				echo "<tr style='color:$color; font-weight:bold; background:White;'><td colspan='8' align='center'>".$vertical_name."</td></tr>";
			}
		}
		...
		if($vertical_name != ''){
			if(strtoupper($_SESSION['nick_name']) == 'RUPA'){
			$sql_get_empdetails = "SELECT LO.emp_code,COUNT( DISTINCT (CASE WHEN POCM.order_no LIKE 'O%' THEN CONCAT(POCM.customer_code,'^',SUBSTRING(POCM.order_no,-14,8)) END )) AS productive_call, 
						COUNT( DISTINCT (CASE WHEN POCM.order_no LIKE 'NO%' THEN CONCAT(POCM.customer_code,'^',SUBSTRING(POCM.order_no,-14,8)) END )) AS non_productive_call,
						SUM(CASE WHEN POCM.cust_type='D' AND POCM.order_no LIKE 'O%' THEN (CASE WHEN OD.UOM=PM.UOM2 AND OD.UOM!='' AND PM.UOM2!='' THEN (POCM.visit_qty/PM.conversion_factor) ELSE POCM.visit_qty END) ELSE 0 END ) AS primary_quantity,
						SUM(CASE WHEN POCM.cust_type='R' AND POCM.order_no LIKE 'O%' THEN (CASE WHEN OD.UOM=PM.UOM2 AND OD.UOM!='' AND PM.UOM2!='' THEN (POCM.visit_qty/PM.conversion_factor) ELSE POCM.visit_qty END) ELSE 0 END ) AS secondary_quantity
						 FROM employee_master EM LEFT JOIN location LO ON EM.emp_code=LO.emp_code AND EM.vertical_value='".$_GET['vertical_name']."'
						LEFT JOIN `prev_order_counting_master` POCM ON POCM.order_no=LO.trans_id 
						LEFT JOIN order_details OD ON POCM.order_no=OD.order_no AND POCM.product_code=OD.sku_code 
						LEFT JOIN product_master PM ON OD.sku_code=PM.prod_code
						WHERE SUBSTRING(LO.trans_id,1,1) IN('A','N','O') 
			".$primary_secondary_quantity_condition.$emp_hierarchy_condition."  GROUP BY LO.emp_code ORDER BY EM.emp_name ASC";
			}
			else
			{
				$sql_get_empdetails = "SELECT LO.emp_code,COUNT( DISTINCT (CASE WHEN POCM.order_no LIKE 'O%' THEN POCM.customer_code END )) AS productive_call, 
			...
			while($row_get_empdetails = mysqli_fetch_assoc($res_get_empdetails)){
				...
				/*----> Get employee name <----*/
				$sql_emp_name = "SELECT EM.state,EM.emp_name ".$count_transid_condition ."  FROM employee_master EM,location LO 
								WHERE LO.emp_code=EM.emp_code AND EM.emp_code = '".$emp_code."' AND LO.trans_id like 'A%' ".$primary_secondary_quantity_condition;
				//$sql_emp_name = "SELECT emp_name,state FROM employee_master WHERE emp_code = '".$emp_code."'";				
			...
			<td align='right'>".number_format($total_primary_quantity,2)."</td>
	...
	exit();	
		...
		/*End of data organising-24-06-2019*/
		/*---------------------------------> Select employee <--------------------------------*/
		...
		$res_empname_time = mysqli_query($link,$sql_empname_time);
		while($row_empname_time = mysqli_fetch_assoc($res_empname_time))
		{
			...
			$row_non_productive_call = mysqli_fetch_assoc($res_non_productive_call);
			...
			$primary_quantity = '';
			/*---------------------------------> Total primary sales <--------------------------------*/
			/*$sql_total_primary = "SELECT OD.order_no, OD.sku_code, OD.qty, OD.UOM, PM.UOM1, PM.UOM2, PM.conversion_factor FROM order_details OD, 
			customer_master CM, order_header OH, product_master PM WHERE OH.order_no=OD.order_no 
			AND SUBSTRING(OD.order_no,2,5)='".$emp_code."' ".$primary_secondary_quantity_condition." AND OH.customer_code=CM.customer_code 
			AND CM.cust_type='D' AND OD.sku_code = PM.prod_code";
			$res_total_primary = mysqli_query($link,$sql_total_primary);
			while($row_total_primary = mysqli_fetch_assoc($res_total_primary)){
