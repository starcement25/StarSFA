...
while ($row_order_date = mysqli_fetch_assoc($res_order_date)) {
	...
	if (!in_array($outer_customer_code_val, $outer_customer_code_array)) {
		...
		//$outer_customer_code[$emp_code_fetched] .= "'".$customer_code."'".",";
...
//if(!empty($date_array)){
$header = "Date" . $dns_customer_code_string . "\t" . "Customer Name" . "\t" . "Route Name" . "\t" . "Cust Type" . "\t" . "Employee Code" . "\t" . "Employee Name" . "\t" . "Zone" . "\t" . "State" . "\t" . "Designation" . "\t" . "Order Qty" . "\t" . "Order Value" . "\t" . "Collection amount" . "\t" . "Check in/Att time" . "\t" . "Check out time" . "\t" . "Duration" . "\t"
	. "Remarks" . "\t" . "Lattitude" . "\t" . "Longitude";
	...
	if (${'outer_customer_code' . $emp_code} != '') {
		...
		if ($customer_row_check > 0) {
				...
				$order_rows = mysqli_num_rows($res_order_details);
						...
						if($countcheckinout >0)
						{
							...
							while($rowcheckinout=mysqli_fetch_assoc($rscheckinout))
							{
								...
								${'check_in_time_final'.$customer_code.$date_array_val.$emp_code}=${'check_in_time_final'.$customer_code.$date_array_val.$emp_code}.${'check_in_time'.$customer_code};
								${'check_out_time_final'.$customer_code.$date_array_val.$emp_code}=${'check_out_time_final'.$customer_code.$date_array_val.$emp_code}.${'check_out_time'.$customer_code};
								${'time_duration_final'.$customer_code.$date_array_val.$emp_code}=${'time_duration_final'.$customer_code.$date_array_val.$emp_code}.$time_duration;
								${'checkinout_remarks_final'.$customer_code.$date_array_val.$emp_code}=${';checkinout_remarks_final'.$customer_code.$date_array_val.$emp_code}.$checkinout_remarks;
						 	}
						}
						else
						{
						...
						{
							$checkinout_remarks='Telephonic Order'.''.$checkinout_remarks;
						}
			...
			foreach ($date_array as $date_array_val) {
				...
				$latt_att = $rowatttime['latt'];
				$longi_att = $rowatttime['longi'];
				if ($atttime != '') {
					$table_data .= date('d-m-Y', strtotime($date_array_val)) . "\t" . 'Attendance' . "\t" . '' . "\t" . '' . "\t" . $dns_emp_code . "\t" . $emp_name . "\t" . $zone . "\t" . $state . "\t" . $designation . "\t" . '' . "\t" . '' . "\t" . '' . "\t" . $atttime . "\t" . '' . "\t" . '' . "\t" . '' . "\t" . $latt_att . "\t" . $longi_att . "\n";
				}
				foreach (${'customer_code_array' . $emp_code} as $customer_code_val) {
				    /*echo "eeeee-".${'total_amount' . $customer_code_val . $date_array_val . $emp_code};*/
					if (${'customer_name' . $customer_code_val . $date_array_val . $emp_code} != '' && (${'total_qty' . $customer_code_val . $date_array_val . $emp_code} > 0 || ${'collection_amount' . $customer_code_val . $date_array_val . $emp_code} > 0 || ${'time_difference_final' . $customer_code_val . $date_array_val . $emp_code} > 0)) {
						...
						else  ${'total_amount' . $customer_code_val . $date_array_val . $emp_code} = ${'total_amount' . $customer_code_val . $date_array_val . $emp_code};/*number_format(${'total_amount' . $customer_code_val . $date_array_val . $emp_code}, 2);*/
						...
						else ${'collection_amount' . $customer_code_val . $date_array_val . $emp_code} = ${'collection_amount' . $customer_code_val . $date_array_val . $emp_code};/*number_format(${'collection_amount' . $customer_code_val . $date_array_val . $emp_code}, 2);*/
