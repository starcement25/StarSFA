...
$sql_menucard_name = "SELECT DISTINCT menucard_name FROM template_keyword_relation where company_id='".$_SESSION['company_id']."'
			...
			$res_menucard_name = mysqli_query($link,$sql_menucard_name);
			$row_menucard_name=mysqli_fetch_assoc($res_menucard_name);
...
$count = 1;
			/*$sql_menu_card = "SELECT DISTINCT menucard_id,menucard_name FROM template_keyword_relation where company_id='".$_SESSION['company_id']."' 
										AND active='yes' ";
			$res_menu_card= mysqli_query($link,$sql_menu_card);
			$countmenu_card=mysqli_num_rows($res_menu_card);
			//echo "<table border='1'>";
			while($row_menu_card=mysqli_fetch_assoc($res_menu_card))
			{
			...
			$res_template_keyword_details = mysqli_query($link,$sql_template_keyword_details);
			$counttemplatekeyword=mysqli_num_rows($res_template_keyword_details);
			//echo "<table border='1'>";
			$templateid_menuid_array=array();
			while($row_template_keyword_details=mysqli_fetch_assoc($res_template_keyword_details))
			{
				...
				$res_menu_keyword_details = mysqli_query($link,$sql_menu_keyword_details);
				//$countmenukeyword=mysqli_num_rows($res_menu_keyword_details);
				//echo "<table border='1'>";
				while($row_menu_keyword_details=mysqli_fetch_assoc($res_menu_keyword_details))
				{
					...
					if(in_array($menu_keyword_id,$final_keyword_array))
					{
						...
						$res_menu_details = mysqli_query($link,$sql_menu_details);
						$row_menu_details=mysqli_fetch_assoc($res_menu_details);
			...
			//print_r($templateid_menuid_array);
				foreach($templateid_menuid_array as $templateid_menuid_val)
				{
					...
					$templateid_menuid_parts=explode("#",$templateid_menuid_val);
						...
						$res_menu_card_name= mysqli_query($link,$sql_menu_card_name);
						$row_menu_card_name=mysqli_fetch_assoc($res_menu_card_name);
					...
					if(${menu_name.$templateid_menuid_parts[1].$templateid_menuid_parts[0].$templateid_menuid_parts[2]}!='' && ${keyword_name_string_whole.$templateid_menuid_parts[1].$templateid_menuid_parts[0].$templateid_menuid_parts[2]}!=''){
						...
						foreach(${keyword_name_string_match_array.$templateid_menuid_parts[1].$templateid_menuid_parts[0].$templateid_menuid_parts[2]} as $keywod_match_val){
							...
							$res_keyword_details = mysqli_query($link,$sql_keyword_details);
							$row_keyword_details=mysqli_fetch_assoc($res_keyword_details);
			...
			$sql_template_keyword_details_sub = "SELECT menucard_id,template,GROUP_CONCAT(keyword_id SEPARATOR ',') AS final_keyword FROM template_keyword_relation where company_id='".$_SESSION['company_id']."' AND menucard_id='".$templateid_menuid_parts[2]."' 
										...
										AND active='yes' GROUP BY  menucard_id,template ORDER BY menucard_id,template ASC";
			$res_template_keyword_details_sub = mysqli_query($link,$sql_template_keyword_details_sub);
			$counttemplatekeywordsub=mysqli_num_rows($res_template_keyword_details_sub);
			//echo "<table border='1'>";
			$templateid_menuid_sub_array=array();
			while($row_template_keyword_details_sub=mysqli_fetch_assoc($res_template_keyword_details_sub))
			{
				...
				$sql_menu_keyword_details_sub = "SELECT menu_id,keyword_id FROM menu_keyword_relation where company_id='".$_SESSION['company_id']."' 
										AND active='yes'   ORDER BY menu_id ASC ";
				$res_menu_keyword_details_sub = mysqli_query($link,$sql_menu_keyword_details_sub);
				//$countmenukeyword=mysqli_num_rows($res_menu_keyword_details);
				//echo "<table border='1'>";
				while($row_menu_keyword_details_sub=mysqli_fetch_assoc($res_menu_keyword_details_sub))
				{
					...
					if(in_array($menu_keyword_id_sub,$final_keyword_sub_array) && $menu_id_sub!=$templateid_menuid_parts[1])
					{
						...
						$res_menu_details_sub = mysqli_query($link,$sql_menu_details_sub);
						$row_menu_details_sub=mysqli_fetch_assoc($res_menu_details_sub);
			...
			//print_r($templateid_menuid_array);
				foreach($templateid_menuid_sub_array as $templateid_menuid_val_sub)
				{
					...
					$templateid_menuid_parts_sub=explode("#",$templateid_menuid_val_sub);
						...
						$res_menu_card_name_sub= mysqli_query($link,$sql_menu_card_name_sub);
						$row_menu_card_name_sub=mysqli_fetch_assoc($res_menu_card_name_sub);
					...
					if(${menu_name_sub.$templateid_menuid_parts_sub[1].$templateid_menuid_parts_sub[0].$templateid_menuid_parts_sub[2]}!='' && ${keyword_name_string_whole_sub.$templateid_menuid_parts_sub[1].$templateid_menuid_parts_sub[0].$templateid_menuid_parts_sub[2]}!=''){
						...
						foreach(${keyword_name_string_match_array_sub.$templateid_menuid_parts_sub[1].$templateid_menuid_parts_sub[0].$templateid_menuid_parts_sub[2]} as $keywod_match_val_sub){
							...
							$res_keyword_details_sub = mysqli_query($link,$sql_keyword_details_sub);
							$row_keyword_details_sub=mysqli_fetch_assoc($res_keyword_details_sub);
