...
if($start_date != '' && $end_date != ''){
	...
	$date_condition_one = " AND (SUBSTRING(OH.order_no,-14,8) BETWEEN '".str_replace("-","",$start_date)."' AND '".str_replace("-","",$end_date)."') ";
}
else{
	$date_condition = " AND SUBSTRING(OD.order_no,-14,8) = '".$today."' ";
	$date_condition_one = " AND SUBSTRING(OH.order_no,-14,8) = '".$today."' ";
}
...
else if($listing_type == 'arealist'){
	
	if(tagged_distributor_for_order == 'yes'){
		$sql_customer_code = "SELECT customer_code FROM customer_master WHERE route_code = '".$get_code."' AND cust_type = 'D'";
		$res_customer_code = mysqli_query($link,$sql_customer_code);
		$row_customer_code = mysqli_fetch_assoc($res_customer_code);
		$distributor_code = $row_customer_code['customer_code'];
		
		$listing_table = ", order_header OH ";
		$listing_condition = " AND OH.order_no = OD.order_no AND OH.tag_distributor_code = '".$distributor_code."' ";
	}
	else{
		$sql_customer_code = "SELECT customer_code FROM customer_master WHERE route_code = '".$get_code."'";
		$res_customer_code = mysqli_query($link,$sql_customer_code);
		while($row_customer_code = mysqli_fetch_assoc($res_customer_code)){
		...
		$listing_condition = " AND OH.order_no = OD.order_no AND OH.customer_code IN(".$customer_code_string.") ";
	}
}
...
$sql_order_header = "SELECT PM.product_group_code, SUM(OD.qty), SUM(OD.amount) FROM order_details OD, product_master PM ".$listing_table." WHERE OD.order_no LIKE 'O%' ".$date_condition.$emp_hierarchy_condition.$listing_condition." AND PM.prod_code = OD.sku_code GROUP BY PM.product_group_code";
...
if($total_row_check>0){
	...
	while($row_order_header = mysqli_fetch_assoc($res_order_header)){
		...
		$res_prod_groupname = mysqli_query($link,$sql_prod_groupname);
		$row_prod_groupname = mysqli_fetch_assoc($res_prod_groupname);
		$prod_group_name = $row_prod_groupname['product_group_name'];
		
		echo "<tr>
				<td>".$count."</td>
				<td><a href=\"#\" style=\"color:blue;\" onclick=\"show_next_level('".$page_href."','".$prod_group_code."','".$start_date."','".$end_date."','prod_group','".$get_code."','".$listing_type."');\">".$prod_group_name."</a></td>
