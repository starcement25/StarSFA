...
//if(!empty($date_array)){
	?>
    <table class="border" width="100%" style="border-collapse:collapse;table-layout:fixed;word-wrap:break-word;" border="1">
      <tr class="TDHEAD">
      	<td width="7%">Date</td>
        <td width="8%">Employee Code</td>
        <td width="12%">Employee Name</td>
         <td width="8%">Attendence</td>
         <td width="10%">PJP</td>
         <td width="8%">First Check in Time</td>
         <td width="8%">Last Check out Time</td>
         <td width="7%">Total Hours</td>
         <td width="8%">Count of Distributor Visit</td>
         <td width="8%">Count of Retailer Visit</td>
         <td width="8%">Total order amount</td>
         <td width="8%">Total collection amount</td>
      </tr>
    <?
	...
	while($row_emp = mysqli_fetch_assoc($res_emp)){
		...
		foreach($date_array as $date_array_val)
		{
			...
			if(${$att_time.$date_array_val.$emp_code} !='' && ${$pjp.$date_array_val.$emp_code}!=''){
				...
				${$order_amount.$date_array_val.$emp_code}= '0.00';
				}
				...
				${$collection_amount.$date_array_val.$emp_code}='0.00';
				}
				...
				if($countdistributorvisit==0) $countdistributorvisit='-';
				...
				if($countdealervisit==0) $countdealervisit='-';
			}
			else
			{
				if(strtoupper($_SESSION['nick_name'])=='NILESH')
				{
				//For Check in and Check out
				$sqlcheckinout="SELECT check_in_time,check_out_time FROM check_in_out_details 
			...
			if(${$att_time.$date_array_val.$emp_code} !='' ||  ${$pjp.$date_array_val.$emp_code}!=''){
				echo "<tr>
						...
						<td>".$dns_emp_code."</td>
						<td>".$emp_name."</td>
						<td>".${$att_time.$date_array_val.$emp_code}."</td>
						<td>".substr(${$pjp.$date_array_val.$emp_code},0,-1)."</td>
						<td>".${$first_checkintime.$date_array_val.$emp_code}."</td>
						<td>".${$last_checkouttime.$date_array_val.$emp_code}."</td>
						<td>".${$time_duration.$date_array_val.$emp_code}."</td>
						<td align=\"right\">".$countdistributorvisit."</td>
						<td align=\"right\">".$countdealervisit."</td>
						<td align='right'>".number_format((float)${$order_amount.$date_array_val.$emp_code},2)."</td>
						<td align='right'>".number_format((float)${$collection_amount.$date_array_val.$emp_code},2)."</td>
					  </tr>";
				}
		}
	}
