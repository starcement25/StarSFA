...
<table width='100%' class='border' border='1' style='border-collapse:collapse;' cellpadding='6px'>
		...
		//foreach($dateArray as $key=>$value){
				...
				$res_emp_login_logout_time = mysqli_query($link,$sql_emp_login_logout_time); 
				while($row_emp_login_logout_time = mysqli_fetch_assoc($res_emp_login_logout_time))
				{
					...
					//$sql_emp_logout_time=mysqli_fetch_assoc(mysqli_query($link,"SELECT date FROM `location` where emp_code='".$emp_code."' and trans_id LIKE 'CH%' AND SUBSTRING(trans_id,-14,8)='".$final_date."'"));
				...
				$res_prod_nonprod_call = mysqli_query($link,$sql_prod_nonprod_call); 
				while($row_prod_nonprod_call = mysqli_fetch_assoc($res_prod_nonprod_call))
				{
				...
				/*$sql_order_emp=mysqli_fetch_assoc(mysqli_query($link,"SELECT sum(amount) as AMT FROM `order_details` WHERE SUBSTRING(order_no,-19,5) = '".$emp_code."' and SUBSTRING(order_no,-14,8) ='".$final_date."'"));
