...
$sqlqueryemproute="SELECT DISTINCT RM.route_code,RM.route_name FROM customer_route_emp_relation CM,route_master RM 
	...
	if($countqueryemproute>0){
		...
		$res_exists_customer = mysqli_query($link,$sql_exists_customer);
		$customer_code_exists_array=array();
		while($row_exists_customer = mysqli_fetch_assoc($res_exists_customer))
		{
	...
	$res_activity = mysqli_query($link,$sql_activity);
	$cnt_activity=mysqli_num_rows($res_activity);
	if($cnt_activity >0){
		$count=1;
	while($row_activity = mysqli_fetch_assoc($res_activity)){
