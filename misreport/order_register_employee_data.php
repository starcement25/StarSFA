...
$total_rows = mysqli_num_rows($res_order_header);
...
<?php
	...
	while($row_order_header = mysqli_fetch_assoc($res_order_header)){
		...
		$sql_order_details = "SELECT SUM(OD.qty) as order_qty, SUM(OD.amount) as order_amt FROM order_details OD WHERE OD.order_no = '".$order_no."'";
