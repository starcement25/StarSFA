<?php
	...
	$prodplant=$_REQUEST['prodplant'];
	$prodplant_array=explode(',',$prodplant);
	$prodplant="'".implode("','", $prodplant_array)."'";
   ...
   $sqlselbranch="SELECT branch_code FROM branch_master WHERE acedns='Y' AND plant_name IN(".$prodplant.") ";
   $rsselbranch=mysqli_query($link,$sqlselbranch);
   while($rowselbranch=mysqli_fetch_assoc($rsselbranch))
   {
	   $prodbranch=$prodbranch."'".$rowselbranch['branch_code']."'".',';
   }
   $prodbranch=substr($prodbranch,0,-1);
