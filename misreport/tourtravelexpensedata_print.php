...
$res_tourexpense_emp = mysqli_query($link,$sql_tourexpense_emp);
$total_rows_tourexpense = mysqli_num_rows($res_tourexpense_emp);

if($total_rows_tourexpense>0){
	$res_tourexpense_emp = mysqli_query($link,$sql_tourexpense_emp);
	while($row_tourexpense_emp = mysqli_fetch_assoc($res_tourexpense_emp)){
		?>
        <div class="page">
        <table width="100%" border="1" style="border-collapse:collapse;" class="BORDER" id="touring_expenses">
          <tr class="TDHEAD">
            <td colspan="7" align="center" style="font-weight:bold;">Travelling Expenses</td>
          </tr>
		...
		$res_tourexpense = mysqli_query($link,$sql_tourexpense);
		while($row_tourexpense = mysqli_fetch_assoc($res_tourexpense)){
	...
	transport_mode_cat_id = '".$transport_cat_id."'";
			$res_transport_type = mysqli_query($link,$sql_transport_type);
			$row_transport_type = mysqli_fetch_assoc($res_transport_type);
...
$res_food_emp = mysqli_query($link,$sql_food_emp);
$total_fooding_rows = mysqli_num_rows($res_food_emp);

if($total_fooding_rows>0){
	
	$res_food_emp = mysqli_query($link,$sql_food_emp);
	while($row_food_emp = mysqli_fetch_assoc($res_food_emp)){
		?>
        <div class="page">
        <table width="100%" border="1" style="border-collapse:collapse;" class="BORDER" id="fooding_expenses">
          <tr class="TDHEAD">
            <td colspan="5" align="center" style="font-weight:bold;">Fooding Expenses</td>
          </tr>
          <tr class="TDHEAD_SUB" align="center">
            <td>Date</td>
		...
		$res_foodexpense = mysqli_query($link,$sql_foodexpense);
		while($row_foodexpense = mysqli_fetch_assoc($res_foodexpense))
		{
...
$total_lodge_rows = mysqli_num_rows($res_emp);
...
if($total_lodge_rows>0){
	...
	while($row_emp = mysqli_fetch_assoc($res_emp)){
        ...
        <table width="100%" border="1" style="border-collapse:collapse;" class="BORDER" id="lodging_expenses">
          <tr class="TDHEAD">
            <td colspan="6" align="center" style="font-weight:bold;">Lodging Expenses</td>
          </tr>
          <tr class="TDHEAD_SUB" align="center">
            <td>Entry Date</td>
            <td>Expenses Incurred</td>
		...
		$res_lodgeexpense = mysqli_query($link,$sql_lodgeexpense);
		while($row_lodgeexpense = mysqli_fetch_assoc($res_lodgeexpense)){
