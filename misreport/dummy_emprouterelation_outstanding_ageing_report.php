...
function main(){	
	...
	if(modified_customer_emp_route == 'yes'){
		if($_SESSION['admin_login'] == 'admin'){
			$sql_get_route = "SELECT DISTINCT ERR.route_code as route_code FROM emp_route_relation ERR, customer_master CM, outstanding_ageing OA WHERE ERR.route_code=CM.route_code AND CM.customer_code=OA.customer_code";
		}else{
			$sql_get_route = "SELECT DISTINCT ERR.route_code as route_code FROM emp_route_relation ERR, customer_master CM, outstanding_ageing OA WHERE ERR.emp_code IN(".$emp_hierarchy.") AND ERR.route_code=CM.route_code AND CM.customer_code=OA.customer_code";
		}
		$res_get_route = mysqli_query($link,$sql_get_route);
		while($row_get_route = mysqli_fetch_assoc($res_get_route)){
			$get_route = "'".$row_get_route['route_code']."',";
		}
		$get_route = rtrim($get_route,",");
		$condition = "emp_route_relation ERR WHERE ERR.emp_code=EM.emp_code AND ERR.route_code IN(".$get_route.") ORDER BY EM.emp_name ASC";
	}
	else{
		$condition = "outstanding_ageing OA, customer_master CM WHERE ".$emp_hierarchy_condition." AND OA.customer_code=CM.customer_code AND CM.emp_code=EM.emp_code ORDER BY EM.emp_name ASC";
	}
?><head>
    ...
    <table cellpadding="4">
      ...
      <tr class="TDHEAD_SUB">
        ...
        <select name="emp_name" id="emp_name" onChange="setblank('emp_name');">
                            ...
                            <?php
							$sql_select_emp = "SELECT DISTINCT EM.emp_code, EM.emp_name FROM employee_master EM,".$condition;
...
function submitdata(){
	...
	GenericAjaxFunction('dummy_emprouterelation_outstanding_ageing_data.php?emp_code='+emp_code+'&customer_name='+customer_name,'display',0);
}
