...
if($opt_type=='updateschemedate')
{
	...
	$res_scheme_PDF_val = mysqli_query($link,$sql_scheme_PDF_val);
	$row_scheme_PDF_val = mysqli_fetch_assoc($res_scheme_PDF_val);
...
$curdate=date('Y-m-d');
	...
	$res_branch_PDF = mysqli_query($link,$sql_branch_PDF);
	$countbranchPDF=mysqli_num_rows($res_branch_PDF);
	if($countbranchPDF >0){
		$count = 1;
		?>
		...
		<?php
		while($rowbranchPDF = mysqli_fetch_assoc($res_branch_PDF)){
