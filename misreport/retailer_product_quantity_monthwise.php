...
<center><br />
	<div>
    	Employee:<select name="emp_name" id="emp_name" onChange="show_customer_name(this.value);">
                            ...
                            <?php
							$sql_select_emp = "SELECT EM.emp_code, EM.emp_name FROM employee_master EM WHERE ".$emp_hierarchy_condition." AND EM.emp_code NOT LIKE 'C%' AND sale_access = 'secondary' ORDER BY EM.emp_name ASC";
         ...
         &nbsp;&nbsp;
         Retailer:<select name="cust_name" id="cust_name" >
         					<option value="" selected>Select</option>
                  </select>
         &nbsp;&nbsp;                
    ...
    <br />
    <div id="display" style="max-height: 500px; max-width:1000px; overflow-y: scroll; overflow-x: scroll;" align="center"></div>
    <br />
...
function show_customer_name(emp_code){
	//alert(emp_code);
	GenericAjaxFunction('retailer_product_quantity_monthwise_getcustomer.php?emp_code='+emp_code,'cust_name',0);
}
...
function submitdata(){
	...
	GenericAjaxFunction('retailer_prod_month_data.php?emp_code='+emp_code+'&cust_name='+cust_name,'display',0);
}

function show_data(){
	document.getElementById("display").innerHTML = '<img src="ajax-loader.gif" id="ajaxloader">';
	GenericAjaxFunction('retailer_prod_month_data.php','display',0)
}
</script>
<?php
}
