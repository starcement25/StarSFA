...
function show_saudawise()
{
	...
	if(type != 'custom')
		GenericAjaxFunction('zonewisedata.php?type='+type,'display',0);
	else 
	{
		if(response1 != 0 && response2 != 0)
			GenericAjaxFunction('zonewisedata.php?type='+type+'&start_date='+start_date+'&end_date='+end_date,'display',0);
	}
	
		
}

function show_zonewise_data(zone,start_date,end_date,condition_value,product_group_code)
{
	document.getElementById("product").innerHTML = '<img src="ajax-loader.gif" id="ajaxloader">';
	GenericAjaxFunction('zonewise_product_details.php?zone='+zone+'&product_group_code='+product_group_code+'&condition_value='+condition_value+'&start_date='+start_date+'&end_date='+end_date,'product',0);
}
...
function exporttocsv()
{
	...
	window.open('exporttocsvstatezone.php?start_date='+start_date+'&end_date='+end_date+'&condition_value='+type+'&mode=zone','mywindow');
}
