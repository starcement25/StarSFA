...
// 					AND SUBSTRING(survey_id,3,5) IN(".$employee.")  AND type = '".$t."'  AND (row_id='RA597' OR row_id='RA724')
// 					ORDER BY DATE_FORMAT(SUBSTRING(survey_id,-14,14),'%Y-%m-%d %H:%i:%s') DESC";
if(strtoupper(trim($_SESSION['admin_login'])) == 'E0658')
{
    $sql_distinct_date = "SELECT DISTINCT survey_id,value, SUBSTRING(survey_id,3,5) AS emp_code,
    DATE_FORMAT(SUBSTRING(survey_id,-14,14),'%d-%m-%Y %H:%i:%s') AS survey_date,
    DATE_FORMAT(SUBSTRING(survey_id,-14,14),'%H:%i:%s') AS survey_time 
    FROM survey_output WHERE 
    (SUBSTRING(survey_id,-14,8) BETWEEN '".str_replace("-","",$start_date)."' AND '".str_replace("-","",$end_date)."') 
    AND type = '".$t."'  
    AND (row_id='RA597' OR row_id='RA724')
    ORDER BY DATE_FORMAT(SUBSTRING(survey_id,-14,14),'%Y-%m-%d %H:%i:%s') DESC";
}
else
{
    ...
    FROM survey_output WHERE 
    (SUBSTRING(survey_id,-14,8) BETWEEN '".str_replace("-","",$start_date)."' AND '".str_replace("-","",$end_date)."') 
    AND SUBSTRING(survey_id,3,5) IN(".$employee.")  
    AND type = '".$t."'  
    AND (row_id='RA597' OR row_id='RA724')
    ORDER BY DATE_FORMAT(SUBSTRING(survey_id,-14,14),'%Y-%m-%d %H:%i:%s') DESC";
}
...
$menu_id = "";
$sql_survey_output_chk = "SELECT survey_id,row_id,value FROM survey_output where survey_id = '".$survey_id."' AND row_id = 'RA597'";
...
<!--<form action="star_survey_report_modified_one_lead_test.php" method="get">
    ...
    <table border="1" style="border-collapse:collapse;" class="border" width="100%">
 		...
 		<tr>
      	  <td colspan="55" class="TDHEAD_SUB"><?php echo $header_string; ?></td>
        </tr>
      ...
      <tr class="TDHEAD">
        ...
        <td width="3%">Region</td>
          ...
          $sql="SELECT * FROM `survey_input_mle` WHERE `menu_id` = 'RA723' AND acedns='Y' AND type<>'menu' ORDER BY display_order ASC";
          
          $res_head = mysqli_query($link,$sql);
          while($row_head = mysqli_fetch_assoc($res_head)){
		    $display_name = $row_head['display_name'];
		    if($display_name=="Cube Test11" || $display_name=="RA6333"){
		        ?>
		        <td class="id-col"><?php echo $display_name; ?></td>
		        <td class="id-col">Cube Test after 3 Days</td>
		        <td class="id-col">Cube Test after 7 Days</td>
		        <td class="id-col">Cube Test after 28 Days</td>
		        <td class="id-col">Test Done Date</td>
		        <td class="id-col">Remarks</td>
		        
		        <?php
		        
		    }else if($display_name=="RA8481"){
		        //
		    }else{
          ?>
		...
		$sql="SELECT * FROM `survey_input_mle` WHERE `menu_id` = '".$menu_id."' AND acedns='Y' AND type<>'menu' ORDER BY display_order ASC";
          ...
          while($row_head = mysqli_fetch_assoc($res_head)){
    		...
    		while($row_survey_details = mysqli_fetch_assoc($res_survey_details)){
    			...
    			if($row_id=="RA6331" || $row_id=="RA7591"){
    			    //if (strpos($survey_value, ':')) { 
                        $commaList = explode(':', $survey_value);
    			        $survey_value = $commaList[1];
                    //}else{
                        //$survey_value = $survey_value;
                    //}
    			    
    			    /*$survey_value1 = $commaList[1];
    			    $survey_value2 = $commaList[2];
    			    $survey_value3 = $commaList[3];
    			    $survey_value4 = $commaList[4];
    			    $survey_value5 = $commaList[5];
		            
		            echo "<td>".$survey_value."</td>";
		            echo "<td>".$survey_value1."</td>";
		            echo "<td>".$survey_value2."</td>";
		            echo "<td>".$survey_value3."</td>";
		            echo "<td>".$survey_value4."</td>";
		            echo "<td>".$survey_value5."</td>";*/
