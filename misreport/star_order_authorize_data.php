...
require("adminUtils.php");
	...
	$link=mysqli_connect(SERVERLOCAL,USERLOCAL,PASSWORDLOCAL,TRUE) or die("Database Connection Error.");
	mysqli_select_db(DBLOCAL,$link) or die("could not connect the database");
...
$header_string = "Zone:".$zone."&nbsp;&nbsp;State:".$state."&nbsp;&nbsp;Branch:".$branch."&nbsp;&nbsp;Department:".$department."&nbsp;&nbsp;Employee:".$new_emp_name."&nbsp;&nbsp;From:".date('d-m-Y',strtotime($start_date))."&nbsp;&nbsp;To:".date('d-m-Y',strtotime($end_date));
	...
	$res_authorize_data = mysqli_query($link,$sql_authorize_data,$link);
	$total_authorize_data = mysqli_num_rows($res_authorize_data);
	if($total_authorize_data>0){
		...
		<?php
		$res_authorize_data = mysqli_query($link,$sql_authorize_data);
		$customer_code_array=array();
		while($row_authorize_data = mysqli_fetch_assoc($res_authorize_data)){
			...
			$resprod = mysqli_query($link,$sqlprod,$link);
			...
			$rsdestcode=mysqli_query($link,$sqldestcode,$link);
			$rowdestcode=mysqli_fetch_assoc($rsdestcode);
			...
			if($changed_sub_dealer_code!='')
			{
				...
				$rssubdealercode=mysqli_query($link,$sqlsubdealercode,$link);
				$rowsubdealercode=mysqli_fetch_assoc($rssubdealercode);
			...
			$rsdumpname=mysqli_query($link,$sqldumpname,$link);
			...
			else
			{
				...
				$rsbrokername=mysqli_query($link,$sqlbrokername,$link);
				$rowbrokername=mysqli_fetch_assoc($rsbrokername);
			...
			$rssaathistatus=mysqli_query($link,$sqlsaathistatus,$linksaathi);
			$rowsaathistatus=mysqli_fetch_assoc($rssaathistatus);
