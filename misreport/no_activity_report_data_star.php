...
$sql_no_payment = "SELECT SUBSTRING(receipt_id,-14,8) AS no_payment_date FROM payment_header WHERE SUBSTRING(receipt_id,3,5) IN(".$employee.") AND (SUBSTRING(receipt_id,-14,8) BETWEEN '".str_replace("-","",$start_date)."' AND '".str_replace("-","",$end_date)."') AND receipt_id LIKE 'N%'";
...
$res_no_stock = mysqli_query($link,$sql_no_stock);
while($row_no_stock = mysqli_fetch_assoc($res_no_stock)){
	...
	while($row_emp = mysqli_fetch_assoc($res_emp)){
		...
		if($location_row_check>0){
			...
			foreach($date_array as $date_val){
				...
				$res_no_stock_details = mysqli_query($link,$sql_no_stock_details);
				$no_stock_rows = mysqli_num_rows($res_no_stock_details);
				if($no_stock_rows>0){
					$res_no_stock_details = mysqli_query($link,$sql_no_stock_details);
					while($row_no_stock_details = mysqli_fetch_assoc($res_no_stock_details)){
