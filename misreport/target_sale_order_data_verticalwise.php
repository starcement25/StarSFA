...
$rsdistdesig=mysqli_query($link,$sqldistdesig);
$countdistdesig=mysqli_num_rows($rsdistdesig);
...
AND PM.vertical_value=SAS.vertical GROUP BY CM.dns_customer_code,PM.vertical_value ORDER BY CM.customer_name ASC,SAS.vertical ASC*/
	  ...
	  //exit();
		...
		$res_total_target_sale = mysqli_query($link,$sql_total_target_sale);
		$total_rows = mysqli_num_rows($res_total_target_sale);
		if($total_rows>0){
			...
			while($row_total_target_sale = mysqli_fetch_assoc($res_total_target_sale)){
			...
			foreach($customer_code_array as $customer_code_val)
			{
				...
				$rstargetval=mysqli_query($link,$sqltargetval);
				while($rowtargetval=mysqli_fetch_assoc($rstargetval))
				{
