	...
	$sql_payment_header_customer = "SELECT SUBSTRING(receipt_id,-19,5) AS emp_code, customer_code, SUBSTRING(receipt_id,-14,8) 
		...
		if(check_in_out=='yes')
		{
			...
			if(mrp=='yes')
			{
				...
				{
					$sqltotalorder="SELECT OH.customer_code,OD.qty,OD.weightage,OD.amount,OD.UOM,PM.conversion_factor,OD.sku_code
							...
							AND PM.prod_code=OD.sku_code ".$date_condition." ORDER BY OH.customer_code,OD.amount";
					$rstotalorder=mysqli_query($link,$sqltotalorder) or die(mysqli_error()." Error in select order: ".$sqltotalorder);
					$total_order_received=0;
					$total_amount_received=0;
					$weightage_val=0;
					
					while($rowtotalorder=mysqli_fetch_assoc($rstotalorder))
					{
						//echo $emp_code.'----------------------';
						$UOM_order=$rowtotalorder['UOM'];
						$prod_code=$rowtotalorder['sku_code'];
						$customer_code=$rowtotalorder['customer_code'];
			...
			if(strtoupper($_SESSION['nick_name'])=='ABDOS')
			{
				...
				while($rownotransaction = mysqli_fetch_assoc($rsnotransaction)){
			...
			$rowval.="<tr> 
					...
					<td align=\"right\" valign=\"top\" style=\"padding-left:20px;BORDER: #A92A61 1px solid;\">".number_format($total_amount_received,2)."</td>".$tonnage_row."
