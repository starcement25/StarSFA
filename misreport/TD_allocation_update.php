...
$sqlimmediatebossallocation="SELECT TD,emp_code FROM TD_allocation WHERE 
								product_filter_code = '".$product_code."' AND emp_code =(SELECT reporting_to FROM employee_master 
...
$immediate_boss_allocation=$rowimmediatebossallocation['TD'];
...
$top_emp_code=$row_top_emp['emp_code'];
	   ...
	   if(mysqli_query($link,$sqlinsertlocation))
	   {
			if($total_rows>0)
			{
				...
				$res_update_TD_allocation  = mysqli_query($link,$sql_update_TD_allocation);
			}
			else
			{
			...
			$sql_insert_TD_log = "INSERT into TD_allocation_log SET 
									 ...
									 product_filter_code = '".$product_code."', 
									TD = '".$TD."'";
			$res_insert_TD_log = mysqli_query($link,$sql_insert_TD_log);
	   }
		
		if((strtoupper($_SESSION['admin_login'])=='ADMIN' || $_SESSION['admin_login']==$top_emp_code || $_SESSION['admin_login']=='E0042' || $_SESSION['admin_login']=='E0076') && $TD > $immediate_boss_allocation)
		{
			foreach($emp_upper_hierarchy_array as $emp_upper_hierarchy_val)
			{
				$emp_upper_hierarchy_leaves=return_employee_hierarchy($emp_upper_hierarchy_val);
				$emp_upper_hierarchy_leaves_condition='emp_code IN('.$emp_upper_hierarchy_leaves.')';
				
				$sqlselmaxallocation="SELECT MAX(TD) AS max_td FROM TD_allocation WHERE product_filter_code = '".$product_code."' AND ".$emp_upper_hierarchy_leaves_condition;
				$rsselmaxallocation=mysqli_query($link,$sqlselmaxallocation);
				$rowselmaxallocation=mysqli_fetch_assoc($rsselmaxallocation);
				...
				$res_check_upper_hierarchy_TD = mysqli_query($link,$sql_check_upper_hierarchy_TD);
				$total_check_upper_hierarchy_TD = mysqli_num_rows($res_check_upper_hierarchy_TD);
				...
				$sql_insert_TD_log_upper_hierarchy = "INSERT into TD_allocation_log SET
													     ...
													     product_filter_code = '".$product_code."', 
														 TD = '".$max_td."'";
				$res_insert_TD_log_upper_hierarchy = mysqli_query($link,$sql_insert_TD_log_upper_hierarchy);
