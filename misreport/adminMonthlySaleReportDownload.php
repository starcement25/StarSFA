...
//For date wise Employee attendance download
	function exceldownloaddata($search_mode)
	{
		...
		while($rowbranchlist=mysqli_fetch_assoc($rsbranchlist))
		{
			...
			while($rowemployeelist=mysqli_fetch_assoc($resemployeelist))
			{
				...
				for($count_number_days=1;$count_number_days<=$number_days;$count_number_days++)
				{
					...
					$resinformation=mysqli_query($link,$sqlinformation) or die(mysqli_error()." Error in select purchase or sales information: ".$sqlinformation);
