...
function show_add_edit($row_id = '')
{
	...
	$rsprem=mysqli_query($link,$sqlprem);
	$rowprem=mysqli_fetch_assoc($rsprem);
...
function change_status()
{
	...
	mysqli_query($link,$sqlupdate) or die(mysqli_error()." Error in status changed.");
...
{	
	...
	$reference_no= $_POST['reference_no'];
	 ...
	 $rsselectrefferenceno=mysqli_query($link,$sqlselectrefferenceno);
	 $cntselectrefferenceno=mysqli_num_rows($rsselectrefferenceno);
	 if($cntselectrefferenceno ==0){
	 $sql = "INSERT INTO sample_master   
			SET       
			reference_no 						= '".trim(mysqli_escape_string($reference_no))."',					
			acedns								= 'Y',
			creation_date						=CURRENT_TIMESTAMP()";
	$rs  = mysqli_query($link,$sql) or die(mysqli_error()." Error in sample insert: ".$sql);
	...
	{       					 
		...
		mysqli_query($link,$sql_update) or die(mysqli_error(). "Error in sample image insert: ".$sql_update);
	}	
