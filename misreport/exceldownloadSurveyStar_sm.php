...
?>
 ...
 <script src="https://ajax.googleapis.com/ajax/libs/jquery/1.11.3/jquery.min.js"></script>
	...
	echo "<span style=\"font-weight:bold; font-size:14px;\">Download Lead Generation Excel</span><br><br>";
...
function check()
{
		...
		else if(start_date<"2024-12-27"){
			alert("Start date cannot be less than 2024/12/27");
			return false;
		}
		return true;
...
class ZipFile
{
	...
	//For survey excel download
		...
		if($survey_type == 'no'){
			if($survey_menu=='yes')
			{
			  $sql_get_menu = "SELECT layout_name,menu_id,survey_sub_menu FROM survey_input_lead WHERE type='menu' AND menu_id IN('RA514','RA515') ORDER BY display_order ASC";
			 ...
			 while($row_get_menu = mysqli_fetch_assoc($res_get_menu))
			 {
				 ...
				 if(strtoupper($_SESSION['nick_name'])=='STAR' && ($menu_id=='RA514' || $menu_id=='RA515') )
				 			{
								${'excelsubheader'.$menu_id}='Sr. No'."\t".'Unique Store ID'."\t".'Emp Name'."\t".'Survey Date'."\t".'Survey Month'."\t".'Lattitude'."\t".'Longitude'."\t";
							}
				else{
				 ...
				 ${'survey_id_array'.$menu_id}=array();
				   ...
				   $sql_get_display="SELECT display_name,row_id,action,display_order FROM survey_input_lead WHERE type!='menu' AND menu_id='".$menu_id."' AND acedns='Y' ORDER BY display_order ASC";
					...
					while($row_get_display = mysqli_fetch_assoc($res_get_display))
					{
						...
						if(strtoupper($_SESSION['nick_name'])=='DURO' && $menu_id=='RA002')
						 ...
						 else if(strtoupper($_SESSION['nick_name'])=='STAR' && ($menu_id=='RA280' || $menu_id=='RA281') )
            			    {
            			        if($display_id =='RA321' || $display_id =='RA247'){
            			        ${'excelsubheader'.$menu_id}.=$display_name."\t";
            				    ${'excelsubheader'.$menu_id}.='Closed Date'."\t";
            			        }else{
            			            ${'excelsubheader'.$menu_id}.=$display_name."\t";
            			        }
            				}
            				else if(strtoupper($_SESSION['nick_name'])=='STAR' && ($menu_id=='RA514' || $menu_id=='RA515') )
            			    {
            			        if($display_id =='RA501' || $display_id =='RA531'){
            			        ${'excelsubheader'.$menu_id}.=$display_name."\t";
            				    ${'excelsubheader'.$menu_id}.='Action Taken'."\t";
            				    ${'excelsubheader'.$menu_id}.='Approved Price'."\t";
            				    
            			        }else if($display_id =='RA595' || $display_id =='RA594'){
            			            ${'excelsubheader'.$menu_id}.=$display_name."\t";
            				    ${'excelsubheader'.$menu_id}.='Sales Org'."\t";
            				    ${'excelsubheader'.$menu_id}.='Division'."\t";
            				    ${'excelsubheader'.$menu_id}.='Distribution Channel'."\t";
            				    ${'excelsubheader'.$menu_id}.='Document Type'."\t";
            				    ${'excelsubheader'.$menu_id}.='Customer Reference No'."\t";
            				    ${'excelsubheader'.$menu_id}.='Customer Reference Date'."\t";
            				    ${'excelsubheader'.$menu_id}.='Valid to Date'."\t";
            				    ${'excelsubheader'.$menu_id}.='material_number'."\t";
            				    ${'excelsubheader'.$menu_id}.='Sold to Party'."\t";
            				    ${'excelsubheader'.$menu_id}.='Ship to Party'."\t";
            				    ${'excelsubheader'.$menu_id}.='PO_method'."\t";
            				    ${'excelsubheader'.$menu_id}.='quotation_provided'."\t";
            				    ${'excelsubheader'.$menu_id}.='quotation_provided_date'."\t";
            				    ${'excelsubheader'.$menu_id}.='mis_submission_date'."\t";
            				    ${'excelsubheader'.$menu_id}.='hos_submission_date'."\t";
            			        }else{
            			            ${'excelsubheader'.$menu_id}.=$display_name."\t";
            			        }
            				}
				  ...
				  while($row_survey_output=mysqli_fetch_assoc($rs_survey_output))
				  {
						 ...
						 if(strtoupper($_SESSION['nick_name'])=='STAR' && ($menu_id=='RA514' || $menu_id=='RA515') )
				 			{
								...
								${'valueexcel'.$survey_id.$menu_id}.=$lg_date1."\t";
							}
					...
					$sqlmasterview="SELECT type,display_table_name,insert_table_detail FROM survey_input_lead WHERE  row_id='".$row_survey_output['row_id']."'";
					...
					if($type=='masterview')
					{
						...
						$dispaly_table_name_partstwo_0=addslashes($dispaly_table_name_partstwo[0]);
						$dispaly_table_name_partstwo_0=addslashes($dispaly_table_name_partstwo[0]);
						$sqlfetchval="SELECT GROUP_CONCAT($dispaly_table_name_partstwo[1] SEPARATOR ';') AS fetch_value 
								FROM $dispaly_table_name_partsone[0] WHERE FIND_IN_SET($dispaly_table_name_partstwo_0,'".$value."')";
							//echo	$sqlfetchval;
					...
					if(strtoupper($_SESSION['nick_name'])=='STAR')
					{
						...
						if(($row_survey_output['row_id']=='RA247' || $row_survey_output['row_id']=='RA321'))
						{
						    if($value=='CLOSED'){
						        $value=$value."\t".$survey_date;
						    }else{
						       $value=$value."\t".'';
						    }
						...
						if(($row_survey_output['row_id']=='RA486' || $row_survey_output['row_id']=='RA516'))
						{
						    $conversion_first_part=explode(";",$value);
						    $value=$conversion_first_part[0]."";
						    
						}
						
						
						if(($row_survey_output['row_id']=='RA501' || $row_survey_output['row_id']=='RA531'))
						{
						    
						    $sql14="SELECT value FROM `survey_output` WHERE survey_id='".$survey_id."' AND (row_id='RA486' OR row_id='RA516')";
						    //echo $sql14;
					        $res14 = mysqli_query($link,$sql14); 
					        $row14=mysqli_fetch_assoc($res14);
					        $action1=$row14['value'];
					        
						    $conversion_first_part1=explode(";",$action1);
						    
						    $sql12="SELECT action_on_lead,approved_price FROM lead_generation_master WHERE  lead_generation_id='".$conversion_first_part1[1]."'";
					        $res12 = mysqli_query($link,$sql12); 
					        $row12=mysqli_fetch_assoc($res12);
					        $action=$row12['action_on_lead'];
					        $approved_price = $row12['approved_price'];
					        
						    $value=$value."\t".$action."\t".$approved_price;
						    
						    
						}
						
						if(($row_survey_output['row_id']=='RA594' || $row_survey_output['row_id']=='RA595'))
						{
						    ...
						    $sql12="SELECT * FROM lead_generation_master WHERE  lead_generation_id='".$conversion_first_part1[1]."'";
						    //echo $sql12;
					        $res121 = mysqli_query($link,$sql12); 
					        $row121=mysqli_fetch_assoc($res121);
					        $action=$row121['action_on_lead'];
					        $approved_price = $row121['approved_price'];
					        
					        $PO_method=$row121['PO_method'];
            				$ship_to_party=$row121['ship_to_party'];
            				$sold_to_party=$row121['sold_to_party'];
            				$material_number=$row121['material_number'];
            				$valid_to_date=$row121['valid_to_date'];
            				$customer_reference_date=$row121['customer_reference_date'];
            				$customer_reference_no=$row121['customer_reference_no'];
            				
            				$document_type=$row121['document_type'];
            				$distribution_channel=$row121['distribution_channel'];
            				$division=$row121['division'];
            				$sales_org=$row121['sales_org'];
            				
            				$quotation_provided=$row121['quotation_provided'];
            				$quotation_provided_date=$row121['quotation_provided_date'];
            				$mis_submission_date=$row121['mis_submission_date'];
            				$hos_submission_date=$row121['hos_submission_date'];
            				
            				
            	
				
						    $value=$value."\t".$sales_org."\t".$division."\t".$distribution_channel."\t".$document_type."\t".$customer_reference_no."\t".$customer_reference_date."\t".$valid_to_date."\t".$material_number."\t".$sold_to_party."\t".$ship_to_party."\t".$PO_method."\t".$quotation_provided."\t".$quotation_provided_date."\t".$mis_submission_date."\t".$hos_submission_date;
						...
						if($row_survey_output['row_id']=='RA326' || $row_survey_output['row_id']=='RA251' || $row_survey_output['row_id']=='RA327' || $row_survey_output['row_id']=='RA328' || $row_survey_output['row_id']=='RA325' || $row_survey_output['row_id']=='RA329' || $row_survey_output['row_id']=='RA330' || $row_survey_output['row_id']=='RA331' ||
						    $row_survey_output['row_id']=='RA585' || $row_survey_output['row_id']=='RA592')
						{
								if($value!='')
								{
									$imageval = $value;
			...
			{
			//$sql_get_menu = "SELECT layout_name,menu_id FROM survey_input_lead WHERE type='menu' ORDER BY display_order ASC";
			$sql_get_menu = "SELECT DISTINCT survey_sub_menu FROM survey_input_lead ORDER BY survey_sub_menu ASC";
			$res_get_menu = mysqli_query($link,$sql_get_menu);
			$count_menu=mysqli_num_rows($res_get_menu);
			$menu_no=1;
			//$menu_id_array=array();
			...
			$survey_sub_menu_array=array();
			 while($row_get_menu = mysqli_fetch_assoc($res_get_menu))
			 {
				...
				// ${'array_header'.$menu_id}=array();
					//$sql_get_display="SELECT display_name,row_id,action FROM survey_input_lead WHERE type!='menu' AND menu_id='".$menu_id."' ORDER BY display_order ASC";
					    $sql_get_display="SELECT display_name,row_id,action FROM survey_input_lead WHERE 
			...
			$sql_get_menu = "SELECT layout_name,menu_id FROM survey_input_lead WHERE type='menu' AND survey_type='".$survey_type."' ORDER BY display_order ASC";
			$res_get_menu = mysqli_query($link,$sql_get_menu);
			$count_menu=mysqli_num_rows($res_get_menu);
			$menu_no=1;
			$menu_id_array=array();
			$menu_name_array=array();
			 ...
			 while($row_get_menu = mysqli_fetch_assoc($res_get_menu))
			 {
				...
				// ${'array_header'.$menu_id}=array();
				$sql_layer_check = "SELECT row_id FROM survey_input_lead WHERE type = 'layer'";
				$res_layer_check = mysqli_query($link,$sql_layer_check);
				$total_layer_check = mysqli_num_rows($res_layer_check);
				
				
				 $sql_get_layer = "SELECT layout_name,row_id FROM survey_input_lead WHERE type='layer' AND 
				 ...
				 while($row_get_layer = mysqli_fetch_assoc($res_get_layer))
				 {
						...
						$sql_get_display="SELECT display_name,row_id,action FROM survey_input_lead WHERE layout_name='".$layout_name."' 
		...
		//$sql_get_menu = "SELECT layout_name,menu_id FROM survey_input_lead_bkup WHERE type='menu' AND survey_type='".$survey_type."' ORDER BY display_order ASC";die;
}
?>