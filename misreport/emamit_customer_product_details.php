...
else if($_REQUEST['condition_value'] == 3)
{
	...
	$condition = "AND (SUBSTRING(STL.sauda_no,8,8) BETWEEN ".$start_date." AND ".$end_date.")";
	$value = 3;
}
...
if($customer_name[1] != '')
{
	if($selected_date != '')
	{
		$sql_customer_product = "SELECT PM.prod_desc, SUM(STL.convert_qty_two) as mt_booked, SUM(STL.qty) as case_booked, STL.sale_rate, STL.freight_charge, STL.amount FROM sauda_transaction_log STL, product_master PM WHERE STL.customer_code='".$customer_name[1]."' ".$emp_hierarchy_condition_one." AND  DATE_FORMAT(SUBSTRING(STL.sauda_no,-14,8),'%d-%m-%Y') = '".$selected_date."' AND PM.$field_name1='".$product_group_code."' AND STL.prod_code=PM.prod_code GROUP BY STL.prod_code";
	}
	else
	{
		$sql_customer_product = "SELECT PM.prod_desc, SUM(STL.convert_qty_two) as mt_booked, SUM(STL.qty) as case_booked, STL.sale_rate, STL.freight_charge, STL.amount FROM sauda_transaction_log STL, product_master PM WHERE STL.customer_code='".$customer_code."' ".$condition." AND PM.$field_name1='".$product_code."' AND STL.prod_code=PM.prod_code GROUP BY STL.prod_code";
	}
}
else if($branch_name[1] != '')
{
	if($selected_date != '')
	{
		$sql_customer_product = "SELECT PM.prod_desc, SUM(STL.convert_qty_two) as mt_booked, SUM(STL.qty) as case_booked, STL.sale_rate, STL.freight_charge, STL.amount FROM sauda_transaction_log STL, product_master PM WHERE STL.branch_code='".$branch_name[1]."' ".$emp_hierarchy_condition_one." AND DATE_FORMAT(SUBSTRING(STL.sauda_no,-14,8),'%d-%m-%Y') = '".$selected_date."' AND PM.$field_name1='".$product_group_code."' AND STL.prod_code=PM.prod_code GROUP BY STL.prod_code";
	}
	else
	{
		$sql_customer_product = "SELECT PM.prod_desc, SUM(STL.convert_qty_two) as mt_booked, SUM(STL.qty) as case_booked, STL.sale_rate, STL.freight_charge, STL.amount FROM sauda_transaction_log STL, product_master PM WHERE STL.branch_code='".$branch_name[1]."' ".$emp_hierarchy_condition_one." ".$condition." AND PM.$field_name1='".$product_group_code."' AND STL.prod_code=PM.prod_code GROUP BY STL.prod_code";
	}
}
else
{
	$sql_customer_product ="SELECT PM.prod_desc, SUM(STL.convert_qty_two) as mt_booked, SUM(STL.qty) as case_booked, STL.sale_rate, STL.freight_charge, STL.amount FROM sauda_transaction_log STL, product_master PM 
							WHERE SUBSTRING(STL.sauda_no,3,5) IN (".$emp_wise_hierarchy_condition.") ".$condition." AND PM.$field_name1='".$product_code."' AND STL.prod_code=PM.prod_code GROUP BY STL.prod_code";
}
$res_customer_product = mysqli_query($link,$sql_customer_product);
while($row_customer_product = mysqli_fetch_assoc($res_customer_product))
{
	echo "<tr>
	<td>$count</td>
  	<td>$row_customer_product[prod_desc]</td>
    <td align=\"right\">".number_format($row_customer_product['mt_booked'],3)."</td>
