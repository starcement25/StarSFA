...
if($total_emp_rows>0)
{
    ...
    $sql_product_details = "SELECT PM.prod_desc, SMR.sale_rate, BM.branch_name FROM product_master PM, sauda_mrp SMR, branch_master BM, product_group_master PGM WHERE PM.product_group_code IN ($product_group_code) AND PM.branch_code IN ($branch_code) AND PM.prod_code = SMR.product_code AND PM.branch_code = SMR.branch_code AND PM.acedns='Y' AND SMR.sale_rate!='0' AND PM.branch_code=BM.branch_code AND PGM.product_group_code=PM.product_group_code  ORDER BY BM.branch_name ASC,PGM.product_group_name ASC";
...
{ ?>
	...
	$row_select_branch_name = mysqli_fetch_assoc($res_select_branch_name);*/
