...
if($current_month == '01' || $current_month == '02' || $current_month == '03'){
	...
	$months = array ('Apr-'.$previous_year.'','May-'.$previous_year.'','Jun-'.$previous_year.'','Jul-'.$previous1_year.'','Aug-'.$previous_year.'','Sep-'.$previous_year.'','Oct-'.$previous_year.'','Nov-'.$previous_year.'','Dec-'.$previous_year.'','Jan-'.$current_year.'','Feb-'.$current_year.'','Mar-'.$current_year.'');
	
	if(strtoupper($_SESSION['nick_name'])=='SKIPPER' || strtoupper($_SESSION['nick_name'])=='HALDIRAM' || strtoupper($_SESSION['nick_name'])=='RUPA' || strtoupper($_SESSION['nick_name'])=='PARLE' || strtoupper($_SESSION['nick_name'])=='MAGIK' || strtoupper($_SESSION['nick_name'])=='PALSONS')
	{
...
<table width="40%" class="border" style="border-collapse:collapse;" border="1" cellpadding="4">
  <tr  class="TDHEAD">
  	<td colspan="2" align="center">Monthly Adherence & Productivity Reports</td>
  </tr>
  ...
  <tr class="TDHEAD_SUB">
  	<td></td>
    <td align="left"><input name="submit" type="button" value="Submit" id="submitdata" onClick="show_result();" ></td>
  </tr>
</table>
<br />
<div id="display" style="max-height: 480px; width:97%; overflow-y: scroll; margin-left:10px;" align="center"></div>
</center>
</body>
...
function show_result()
{
	...
	GenericAjaxFunction('dashboard_sales_emp_month_wise_data.php?month='+month,'display',0);
}
...
function Popup(data) 
{
	var mywindow = window.open('', 'Dashboard Report', 'height=400,width=600');
	mywindow.document.write('<html><head><title>Dashboard Report</title>');
...
function exporttocsv(divid)
{
	...
	a.download = 'Dashboard Report Month Wise' + postfix + '.xls';
