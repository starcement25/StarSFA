...
$sql_get_empdetails = "SELECT LO.emp_code,SUM(CASE WHEN LO.trans_id LIKE 'O%' THEN 1 ELSE 0
						...
						SUM(CASE WHEN POCM.cust_type='D' AND POCM.order_no LIKE 'O%' THEN POCM.visit_qty ELSE 0 END ) AS primary_quantity,
						SUM(CASE WHEN POCM.cust_type='R' AND POCM.order_no LIKE 'O%' THEN POCM.visit_qty ELSE 0 END ) AS secondary_quantity,
						SUM(CASE WHEN POCM.cust_type='D' AND POCM.order_no LIKE 'O%' THEN POCM.amount ELSE 0 END ) AS primary_amount, 
SUM(CASE WHEN POCM.cust_type='R' AND POCM.order_no LIKE 'O%' THEN POCM.amount ELSE 0 END ) AS secondary_amount FROM location LO LEFT JOIN 
`prev_order_counting_master` POCM ON POCM.order_no=LO.trans_id WHERE SUBSTRING(LO.trans_id,1,1) IN('A','N','O') 
".$condition.$emp_hierarchy_condition."  GROUP BY LO.emp_code ORDER BY LO.emp_code ASC";
...
if($total_rows>0){
	...
	echo "<tr class='TDHEAD_SUB'>
			...
			$secondary_row".$stk_audit_customer_TR_secondary."
		  </tr>";
	$res_get_empdetails = mysqli_query($link,$sql_get_empdetails);
	while($row_get_empdetails = mysqli_fetch_assoc($res_get_empdetails)){
		...
		if($total_order_count>0){
			...
			if($retailer == 'true' && $distributor == 'true'){
				$table_column_data = "<td align='right'>".$primary_quantity."</td>
									  <td align='right'>".number_format($primary_amount,2)."</td>
			...
			if($retailer == 'true' && $distributor == 'true'){
				...
				$table_column_data = "<td align='right'>--</td><td align='right'>--</td>";
			}
		}
		...
		echo "<tr>
				...
				$table_column_data.$stk_audit_customer_TD."
			  </tr>";
				  
		$count++;
	}
	if($retailer == 'true' && $distributor == 'true'){
		$table_column_data = "<td align='right'>".$total_primary_quantity."</td>
							<td align='right'>".number_format($total_primary_amount,2)."</td>
		...
		$table_column_data = "<td align='right'>".$total_primary_quantity."</td><td align='right'>".number_format($total_primary_amount,2)."</td>";
	}
	...
	echo "<tr style='font-weight:bold;'>
			...
			$table_column_data".$stk_audit_customer_TD_total."
		  </tr>";
	echo "</table>";
	echo "<br>
