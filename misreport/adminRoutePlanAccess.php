	...
	<tr>
		<td valign="top" bgcolor="#FFFFFF" >
                ...
                style="display: '';height: 150px;overflow-y: scroll;" id="todayAttDisplay">
                    ...
                    <tr class="TDHEAD_SUB"> 
						...
						<?php
                            ...
                            while($rowfieldforce=mysqli_fetch_assoc($resfieldforce)){
								...
								$resaccessperiod=mysqli_query($link,$sqlaccessperiod) or die(mysqli_error()." Error in select access period: ".$sqlaccessperiod);
								$rowaccessperiod=mysqli_fetch_assoc($resaccessperiod);
			...
			if($_REQUEST['fjp_change_mode']=='changeFJPperiod'){
				...
				$resaccessperiod=mysqli_query($link,$sqlaccessperiod) or die(mysqli_error()." Error in select access period: ".$sqlaccessperiod);
				$cntaccessperiod=mysqli_num_rows($resaccessperiod);*/
				
				$sqldelfjp="DELETE FROM route_plan_access_period WHERE emp_code='".$emp_code."'";
				mysqli_query($link,$sqldelfjp);
				...
				$access_end_date_final=$access_end_date_ARR[2].'-'.$access_end_date_ARR[1].'-'.$access_end_date_ARR[0];
					...
					if(mysqli_query($link,$sqlfjpperiodchange)){
						$successval='1';
					}
					else
					{
