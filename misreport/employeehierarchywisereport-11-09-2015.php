...
<?php
  ...
  echo Populate_employee_hierarchy_html($reporting_to,$padding);
	echo "<tr style=\"font-weight:bold\">
		    ...
		    $res_total_qty_today = mysqli_query($link,$sql_total_qty_today);
		    $row_total_qty_today = mysqli_fetch_assoc($res_total_qty_today);
		    ...
		    $res_total_qty_MTD = mysqli_query($link,$sql_total_qty_MTD);
		    $row_total_qty_MTD = mysqli_fetch_assoc($res_total_qty_MTD);
		    ...
		    $res_total_qty_YTD = mysqli_query($link,$sql_total_qty_YTD);
		    $row_total_qty_YTD = mysqli_fetch_assoc($res_total_qty_YTD);
   ...
   $cntemphierarchy=mysqli_num_rows($rsemphierarchy);
	if($cntemphierarchy>0)
	{
		...
		while($rowemphierarchy=mysqli_fetch_assoc($rsemphierarchy))
		{
			...
			//echo $rowemphierarchy['emp_code'];
			/*$sql_emp_total_qty_today = "SELECT sum(qty) as total_emp_booked_today FROM vertical_branch_employeewise_details 
									 WHERE operation_date=CURDATE() AND emp_code IN(".$emp_hierarchy_condition.")";
		    $res_emp_total_qty_today = mysqli_query($link,$sql_emp_total_qty_today);
		    $row_emp_total_qty_today = mysqli_fetch_assoc($res_emp_total_qty_today);
		    $total_emp_booked_today = $row_emp_total_qty_today['total_emp_booked_today'];
			...
			/*$sql_emp_total_qty_today = "SELECT (CASE WHEN CM.cust_type='R' AND DATE_FORMAT(SUBSTRING(OH.order_no,-14,8),'%Y%-%m-%d')=CURDATE() THEN sum(OD.qty) END) as total_emp_booked_primary_today,
								(CASE WHEN CM.cust_type='D' AND DATE_FORMAT(SUBSTRING(OH.order_no,-14,8),'%Y%-%m-%d')=CURDATE() THEN sum(OD.qty) ELSE 0 END) as total_emp_booked_secondary_today,
								 (CASE WHEN CM.cust_type='R' AND YEAR(DATE_FORMAT(SUBSTRING(OH.order_no,-14,8),'%Y%-%m-%d'))=".$yearemp." AND MONTH(DATE_FORMAT(SUBSTRING(OH.order_no,-14,8),'%Y%-%m-%d'))=".$monthemp." THEN sum(OD.qty) ELSE 0 END) as total_emp_booked_primary_MTD,
								(CASE WHEN CM.cust_type='D' AND YEAR(DATE_FORMAT(SUBSTRING(OH.order_no,-14,8),'%Y%-%m-%d'))=".$yearemp." AND MONTH(DATE_FORMAT(SUBSTRING(OH.order_no,-14,8),'%Y%-%m-%d'))=".$monthemp." THEN sum(OD.qty) ELSE 0 END) as total_emp_booked_secondary_MTD,
								(CASE WHEN CM.cust_type='R' AND YEAR(DATE_FORMAT(SUBSTRING(OH.order_no,-14,8),'%Y%-%m-%d'))=".$yearemp." THEN sum(OD.qty) ELSE 0 END) as total_emp_booked_primary_YTD,
								(CASE WHEN CM.cust_type='D' AND YEAR(DATE_FORMAT(SUBSTRING(OH.order_no,-14,8),'%Y%-%m-%d'))=".$yearemp." THEN sum(OD.qty) ELSE 0 END) as total_emp_booked_secondary_YTD
								 FROM order_header OH,order_details OD,customer_master CM 
			...
			$yearemp = $mtddateemp[0];
			$monthemp = $mtddateemp[1];
			$sql_emp_total_qty_today = "SELECT SUM(CASE WHEN CM.cust_type='D' THEN OD.qty END) as total_emp_booked_primary_today,
								SUM(CASE WHEN CM.cust_type='R' THEN OD.qty ELSE 0 END) as total_emp_booked_secondary_today
								  FROM order_header OH,order_details OD,customer_master CM 
								WHERE OH.customer_code=CM.customer_code AND OH.order_no=OD.order_no AND  DATE_FORMAT(SUBSTRING(OH.order_no,-14,8),'%Y-%m-%d')=CURDATE()
								AND SUBSTRING(OH.order_no,2,5) IN(".$emp_hierarchy_condition.")";
		    $res_emp_total_qty_today = mysqli_query($link,$sql_emp_total_qty_today);
		    $row_emp_total_qty_today = mysqli_fetch_assoc($res_emp_total_qty_today);
		    $total_emp_booked_primary_today = $row_emp_total_qty_today['total_emp_booked_primary_today'];
			$total_emp_booked_secondary_today = $row_emp_total_qty_today['total_emp_booked_secondary_today'];
			...
			/*$sql_emp_total_qty_MTD = "SELECT sum(qty) as total_emp_booked_MTD FROM vertical_branch_employeewise_details WHERE 
							YEAR(operation_date)=".$yearemp." AND MONTH(operation_date)=".$monthemp." AND emp_code IN(".$emp_hierarchy_condition.")";
		    $res_emp_total_qty_MTD = mysqli_query($link,$sql_emp_total_qty_MTD);
		    $row_emp_total_qty_MTD = mysqli_fetch_assoc($res_emp_total_qty_MTD);
		    $total_emp_booked_MTD = $row_emp_total_qty_MTD['total_emp_booked_MTD'];*/
			
			$sql_emp_total_qty_MTD = "SELECT SUM(CASE WHEN CM.cust_type='D' THEN OD.qty ELSE 0 END) as total_emp_booked_primary_MTD,
								SUM(CASE WHEN CM.cust_type='R' THEN OD.qty ELSE 0 END) as total_emp_booked_secondary_MTD
								 FROM order_header OH,order_details OD,customer_master CM 
								WHERE OH.customer_code=CM.customer_code AND OH.order_no=OD.order_no AND  
							   YEAR(DATE_FORMAT(SUBSTRING(OH.order_no,-14,8),'%Y-%m-%d'))=".$yearemp." AND MONTH(DATE_FORMAT(SUBSTRING(OH.order_no,-14,8),'%Y-%m-%d'))=".$monthemp." 
							   AND SUBSTRING(OH.order_no,2,5) IN(".$emp_hierarchy_condition.")";
		    $res_emp_total_qty_MTD = mysqli_query($link,$sql_emp_total_qty_MTD);
		    $row_emp_total_qty_MTD = mysqli_fetch_assoc($res_emp_total_qty_MTD);
			$total_emp_booked_primary_MTD = $row_emp_total_qty_MTD['total_emp_booked_primary_MTD'];
			$total_emp_booked_secondary_MTD = $row_emp_total_qty_MTD['total_emp_booked_secondary_MTD'];

			
		  /* $sql_emp_total_qty_YTD = "SELECT sum(qty) as total_emp_booked_YTD FROM vertical_branch_employeewise_details WHERE 
							YEAR(operation_date)=".$yearemp." AND emp_code IN(".$emp_hierarchy_condition.")";
		    $res_emp_total_qty_YTD = mysqli_query($link,$sql_emp_total_qty_YTD);
		    $row_emp_total_qty_YTD = mysqli_fetch_assoc($res_emp_total_qty_YTD);
		    $total_emp_booked_YTD = $row_emp_total_qty_YTD['total_emp_booked_YTD']; */
			
			$sql_emp_total_qty_YTD = "SELECT SUM(CASE WHEN CM.cust_type='D' THEN OD.qty ELSE 0 END) as total_emp_booked_primary_YTD,
								SUM(CASE WHEN CM.cust_type='R' THEN OD.qty ELSE 0 END) as total_emp_booked_secondary_YTD
								 FROM order_header OH,order_details OD,customer_master CM 
								WHERE OH.customer_code=CM.customer_code AND OH.order_no=OD.order_no AND  
							   YEAR(DATE_FORMAT(SUBSTRING(OH.order_no,-14,8),'%Y-%m-%d'))=".$yearemp." AND SUBSTRING(OH.order_no,2,5) IN(".$emp_hierarchy_condition.")";
		    $res_emp_total_qty_YTD = mysqli_query($link,$sql_emp_total_qty_YTD);
		    $row_emp_total_qty_YTD = mysqli_fetch_assoc($res_emp_total_qty_YTD);
		    $total_emp_booked_primary_YTD = $row_emp_total_qty_YTD['total_emp_booked_primary_YTD'];
			$total_emp_booked_secondary_YTD = $row_emp_total_qty_YTD['total_emp_booked_secondary_YTD'];
