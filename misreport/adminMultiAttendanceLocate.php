...
function main()
{
	...
	$rsroutedetails=mysqli_query($link,$sqlroutedetails) or die(mysqli_error()." Error in select route details : ".$sqlroutedetails);	
	...
	$routeDetails='[';
	while($rowroutedetails=mysqli_fetch_assoc($rsroutedetails))
		{				
			...
			$sql_checkout = "SELECT SUBSTRING(date,12) AS checkout_time FROM location WHERE emp_code = '".$_REQUEST['emp_code']."' AND trans_id LIKE 'CH%'  ".$date_condition;
			//echo $sql_checkout;
