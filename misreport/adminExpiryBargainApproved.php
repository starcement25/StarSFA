...
<body onLoad="display_result();">
    ...
    <input name="export" type="button" value="Export" id="btnExport" onClick="exporttocsv();" >
			 ...
			 function display_result(){
				...
				GenericAjaxFunction('Expiry_Bargain_approved_data.php','display',0);
...
function exporttocsv()
{
	...
	e.preventDefault();
        ...
        <br />
 <?php
 if($_REQUEST['mode']=='expiryapprovebargain'){
    /*echo '<prev>';
		print_r($_POST);
		echo '</prev>';*/
	//exit();
	...
	for($i=0;$i<=count($prod_val);$i++)
	{
	  ...
	  if($bargain_status_val=='approved'){
		...
		$valid_upto_modified = date('Y-m-d',strtotime($curr_date. '+'.${validity_period.$saudano[$i]}.' days'));
		$sqlupdatebargainapprove="UPDATE DO_master SET is_approved='yes',qty='".$current_qty."',prev_qty='".$qty_val[$i]."',
									...
									expiry_settlement_date=CURRENT_TIMESTAMP(),
									expiry_settlement_by='".$_SESSION['admin_login']."',
     ...
     <table width="50%" align="center" border="0" cellpadding="5" cellspacing="1">
				<tr> 
					<td align="center" class="ERR">Expired Bargain approval successful</td>
					<td align="right">&nbsp;</td>
