...
$res_survey_row_id = mysqli_query($link,$sql_survey_row_id);
$row_survey_row_id = mysqli_fetch_assoc($res_survey_row_id);
...
if($_POST['submit'] == 'Save')
{
	...
	foreach($row_id_array as $index=>$value)
	{
		...
		$total_row = mysqli_num_rows($res_check);
		...
		if($total_row>0){
			...
			$res_insert_update = mysqli_query($link,$sql_insert_update);
...
<table width="100%" align="center" cellspacing="2" border="0" id="maintable" style="font-family:'Courier New', Courier, monospace;"  >
    ...
    <tr>
		...
		$res_survey_exist_check_surveyaudit = mysqli_query($link,$sql_survey_exist_check_surveyaudit);
		$row_survey_exist_check_surveyaudit = mysqli_fetch_assoc($res_survey_exist_check_surveyaudit);
        ...
        foreach($survey_layer_name as $layer_name)
        {
			...
			$layer_check_total_rows = mysqli_num_rows($res_select_survey_details);
