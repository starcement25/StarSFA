...
<script type="text/javascript" src="ajax1.js"></script>
	...
	if(strtoupper($_SESSION['nick_name'])=='RKBK')
	{
		if(modified_customer_emp_route == 'yes'){
			...
			$res_getroute_employeewise = mysqli_query($link,$sql_getroute_employeewise);
			while($row_getroute_employeewise = mysqli_fetch_assoc($res_getroute_employeewise)){
				$get_route .= "'".$row_getroute_employeewise['route_code']."',";
			}
			$get_route = rtrim($get_route,',');
			
			//$emp_route_relation_table = ", emp_route_relation ERR";
			$emp_hierarchy_condition = " CM.route_code IN(".$get_route.") ";
			
			$sql_count ="SELECT COUNT(DISTINCT CM.customer_code) FROM customer_master CM WHERE ".$emp_hierarchy_condition."";
		}
		else{
	...
	if($_REQUEST['search_mode']=='search')
	{
		...
		{
			if(modified_customer_emp_route == 'yes'){
				$sql_count.=" AND CM.route_code='".$_REQUEST['route_code']."'";}
			else{
	...
	if(strtoupper($_SESSION['nick_name'])=='RKBK')
	{
		if(modified_customer_emp_route == 'yes'){
			...
			$row=mysqli_fetch_assoc(mysqli_query($link,"SELECT COUNT(CM.customer_code) FROM customer_master CM WHERE  
			 ".$emp_hierarchy_condition_one.$sql_condition." ORDER BY CM.customer_name ASC"));
		}
		else{
			$sql="SELECT CM.customer_name,CM.customer_code,CM.acedns,CM.black_list,EM.emp_name FROM employee_master EM,customer_master CM WHERE CM.emp_code=EM.emp_code ".$emp_hierarchy_condition_one.$sql_condition." ORDER BY EM.emp_name,CM.customer_name ASC LIMIT ".$GLOBALS[start].",".$GLOBALS[show];
...
function get_route_name(emp_code){
	GenericAjaxFunction('get_route_name.php?emp_code='+emp_code,'route_name',0);
}
</script>
                        ...
                        <input type="hidden" name="search_mode" value="search">
                        	<tr>
                        		...
                        		<td align="left" width="" style="vertical-align:top;" >
                                    <select name="emp_name" id="emp_name" onchange="get_route_name(this.value);" >
							...
							if(modified_customer_emp_route == 'yes'){?>
                            <tr>
                            	<td align="right" width="25%">Route</td>
                                <td align="left" width="" style="vertical-align:top;">
