...
function showEmployeeWisedisplay(val,val1,val2)
{
	...
	var url="selectEmployeeWiseActivitySurvey.php?emp_code="+val+"&mode="+val1+"&page="+val2;
...
function showAttendaceActivitydisplay(val)
{
	...
	if(val=='T')
	{
	  var url="showAttendanceActivitySurvey.php?val="+val;
	}
	if(val=='MTD' || val=='YTD')
	{
	  var url="showAttendanceActivityMonthlySurvey.php?val="+val;
	}
...
function showDetailsEmployeeRoutePlan()
 ...
 function show_survey_details(survey_id){
	 alert(survey_id);
 }
		...
		<td valign="top" bgcolor="#FFFFFF" >
			...
			<br><br>
                ...
                style="display: '';height: 150px;overflow-y: scroll;" id="todayAttDisplay">
                    <tr class="TDHEAD" > 
                        <td colspan="4" align="center"><strong>MIS Report</strong></td>
                    </tr>
                    <tr class="TDHEAD_SUB"> 
                        ...
                        <td width="15%" align="left" style="padding-left:20px;">Survey Done</td>
                    ...
                    <?php
						...
						for($i=1;$i<=3;$i++)
						{
							...
							$rowpresent=mysqli_fetch_assoc($respresent);
							...
							$sqlcustomervisit="SELECT COUNT(LO.trans_id) AS tot_survey  FROM location LO WHERE LO.trans_id LIKE 'SU%' ".$emp_hierarchy_condition.$date_condition."";
							...
							$total_survey=$rowcustomervisit['tot_survey'];
					?>
							<tr> 
					...
					onClick="javascript:showAttendaceActivitydisplay('<?=$val?>')"  style="color:#930;font-weight:bold;"><?=$sl_value?></a></td>
									...
									<td align="right" valign="top" style="padding-left:20px;BORDER: #A92A61 1px solid;"><?php echo $total_survey;?></td>
                                    <?php /*if(route_plan=='yes'){
									?>
                                     <td valign="top" align="center" style="BORDER: #A92A61 1px solid;" ><a href="javascript:void(0)" 
    ...
    <tr class="TDHEAD" > 
     ...
     <tr class="TDHEAD_SUB"> 
        ...
        while($rowinformation=mysqli_fetch_assoc($resinformation))
        {
			...
			|| strtoupper($_SESSION['nick_name'])=='LTS' || strtoupper($_SESSION['nick_name'])=='EMARK' || strtoupper($_SESSION['nick_name'])=='OJBH' || strtoupper($_SESSION['nick_name'])=='JPHARMA' || strtoupper($_SESSION['nick_name'])=='STAR' || strtoupper($_SESSION['nick_name'])=='MINU') 
			&& $mode=='T'){
