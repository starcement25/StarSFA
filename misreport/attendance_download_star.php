...
ob_start();
	...
	if(($_SESSION['admin_login']=="E0674" || $_SESSION['admin_login']=="E1697")) require("adminUtils_accounts.php");
...
function exceldownloaddata($countbranches){
	...
	$sql_location_data = "SELECT DATE_FORMAT(SUBSTRING(LO.date,1,10),'%d-%m-%Y') AS date_selected, LO.emp_code, SUBSTRING(LO.date,12,16) AS atd_time FROM location LO WHERE LO.emp_code IN (".$emp_string.") AND (SUBSTRING(LO.date,1,10) BETWEEN '".$start_date."' AND '".$end_date."') AND (LO.trans_id LIKE 'A%' OR LO.trans_id LIKE 'CH%') ORDER BY SUBSTRING(LO.date,1,10) DESC, LO.emp_code ASC, SUBSTRING(LO.date,12) ASC";
	$res_location_data = mysqli_query($link,$sql_location_data);
	$total_rows = mysqli_num_rows($res_location_data);
	if($total_rows>0){
		$res_location_data = mysqli_query($link,$sql_location_data);
		while($row_location_data = mysqli_fetch_assoc($res_location_data)){
