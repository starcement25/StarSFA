...
function main()
{
   ...
   $sql_vertical = "SELECT DISTINCT SUBSTRING_INDEX(EM.vertical_value, ',', -1) as distinct_vertical_value FROM employee_master 
		...
		$total_rows = mysqli_num_rows($res_vertical);
		if($total_rows>0){
			$res_vertical = mysqli_query($link,$sql_vertical);
...
function catalogue_upload(){
	...
	if(file_exists($_FILES['pdf_file']['tmp_name']))
	{
		...
		$rsselcataloguedetails=mysqli_query($link,$sqlselcataloguedetails);
		$countselcataloguedetails=mysqli_num_rows($rsselcataloguedetails);
		if($countselcataloguedetails > 0){
			...
			mysqli_query($link,$sqlupdatecatalogueinfo);					
		}
		else
		{
			...
			mysqli_query($link,$sqlinsertcatalogueinfo);
			
		}
		$sqlmaxversion="SELECT MAX(file_version) as latest_version FROM catalogue_info WHERE vertical='".$vertical_value."'";
		$rsmaxversion=mysqli_query($link,$sqlmaxversion);
		$rowmaxversion=mysqli_fetch_assoc($rsmaxversion);
		...
		mysqli_query($link,$sqlupdatecatalogueinfoall);
	  }
	  else
	  {
