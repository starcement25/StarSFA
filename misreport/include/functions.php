...
function return_employee_hierarchy($emp_code) {
 ...
 if (!$link) {
    ...
    employee_hierarchy_details($emp_code, $emphierarchy, $visited);
...
function employee_hierarchy_details($emp_code, &$emphierarchy, &$visited) {
    global $link; 
 if (!$link) {
        $link = mysqli_connect(SERVER, USER, PASSWORD, DB);
        if (!$link) {
    ...
    while ($row = mysqli_fetch_assoc($res)) {
        ...
        employee_hierarchy_details($child, $emphierarchy, $visited);
    }
...
require("include/dbcon.php");
   ...
   $cntemphierarchy=mysqli_num_rows($rsemphierarchy);
	if($cntemphierarchy>0)
	{
		while($rowemphierarchy=mysqli_fetch_array($rsemphierarchy))
		{
