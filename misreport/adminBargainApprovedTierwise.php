...
<body onLoad="display_result();">
    ...
    <input name="export" type="button" value="Export" id="btnExport" onClick="exporttocsv();" >
			 ...
			 function display_result(){
				...
				GenericAjaxFunction('Bargain_approved_data_tierwise.php','display',0);
...
function exporttocsv()
{
 ...
 if($_REQUEST['mode']=='approvebargaintierwise'){
	...
	for($i=0;$i<=count($prod_val);$i++)
	{
     ...
     <table width="50%" align="center" border="0" cellpadding="5" cellspacing="1">
				<tr> 
					<td align="center" class="ERR">Bargain approval successful</td>
					<td align="right">&nbsp;</td>
 ...
 if($_REQUEST['mode']=='approvebargaintierwiseone'){
    /*echo '<prev>';
		print_r($_POST);
		echo '</prev>';
	//exit();*/
	  ...
	  if($bargain_status_val=='approved'){
		  foreach($subcat_val_array as $subcatval)
		  {
			...
			$sqlinsertapprovaltrans="INSERT INTO approval_trans_log SET approval_category='bargain',
									...
									approval_category_id='".$saudano[$i]."',
									approval_sub_category_id='".$prod_val[$i]."',
									approval_level_2='".$approval_level_1."',
									approval_authority='',
		  							approval_date=CURRENT_TIMESTAMP(),
									approved_by='".$_SESSION['admin_login']."',
									approval_status='ESCALATE2'";
		  	mysqli_query($link,$sqlinsertapprovaltrans);
		  }
	   }
	   if($bargain_status_val=='reject'){
   ...
   if($_REQUEST['mode']=='approvebargaintierwisetwo'){
    /*echo '<prev>';
		print_r($_POST);
		echo '</prev>';
	//exit();*/
	...
	for($i=0;$i<=count($prod_val);$i++)
	{
	  ...
	  if($bargain_status_val=='approved'){
		  foreach($subcat_val_array as $subcatval)
		  {
			  ...
			  $sqlprevdet="SELECT qty,valid_upto FROM DO_master WHERE sku_code='".$prod_val[$i]."' AND sauda_no='".$saudano[$i]."' AND qty > 0";
			  $rsprevdate=mysqli_query($link,$sqlprevdet);
			  $rowprevdet=mysqli_fetch_assoc($rsprevdate);
			  $prevqty=$rowprevdet['qty'];
			  $current_validity_days=$_REQUEST["validity_period_$saudano[$i]_$prod_val[$i]"];
			  $prev_valid_upto=$rowprevdet['valid_upto'];
			  $valid_upto=date('Y-m-d', strtotime($prev_valid_upto. " +  $current_validity_days days")); 
  
			$sqlupdatebargainapprove="UPDATE DO_master SET is_approved='yes',qty='".$_REQUEST["qty_$saudano[$i]_$prod_val[$i]"]."',
									prev_qty='0',
									TD='".$_REQUEST["TD_$saudano[$i]_$prod_val[$i]"]."',
									premium='".$_REQUEST["premium_$saudano[$i]_$prod_val[$i]"]."',
		  							is_approved_date_time=CURRENT_TIMESTAMP(),
									is_approved_by='".$_SESSION['admin_login']."',
									po_file='".$_REQUEST["po_no_$saudano[$i]_$prod_val[$i]"]."',
									valid_upto='".$valid_upto."'
									WHERE sku_code='".$prod_val[$i]."' AND sauda_no='".$saudano[$i]."'";
			  mysqli_query($link,$sqlupdatebargainapprove);
			  ...
			  mysqli_query($link,$sqlupdatechildbargainapprove);
	   ...
	   if($bargain_status_val=='reject'){
			...
			$sqlupdatebargainreject="UPDATE DO_master SET is_approved='reject',
									...
									is_approved_by='".$_SESSION['admin_login']."'
									WHERE sku_code='".$prod_val[$i]."' AND sauda_no='".$saudano[$i]."'";
		  mysqli_query($link,$sqlupdatebargainreject);
		  $sqlupdatechildbargainreject="UPDATE DO_master SET is_approved='reject',
		  							is_approved_date_time=CURRENT_TIMESTAMP(),
									is_approved_by='".$_SESSION['admin_login']."'
									WHERE mapped_sku_code='".$mapped_sku_code."' AND sauda_no='".$saudano[$i]."'";
		  mysqli_query($link,$sqlupdatechildbargainreject);
			  }
	   }
	 }
	 ?>
