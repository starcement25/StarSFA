...
function exporttocsv(){
	...
	a.download = 'Pending Bargain' + postfix + '.xls';
...
function show_data()
{
	...
	GenericAjaxFunction('pending_bargain_data.php?customer_code='+customer_code+'&start_date='+start_date+'&end_date='+end_date+'&state='+state+'&employee='+employee+'&prod_group_code='+prod_group_code+'&prod_code='+prod_code,'display',0);
}
function show_data_onload()
{
	document.getElementById("display").innerHTML = '<img src="ajax-loader.gif" id="ajaxloader">';
	GenericAjaxFunction('pending_bargain_data.php?mod=initial','display',0);
}
...
<table width="45%" class="border" style="border-collapse:collapse;" border="1" cellpadding="2">
  <tr class="TDHEAD">
  	<td colspan="2" align="center">Pending Bargain</td>
  </tr>
  <tr>
  ...
  <tr>
	...
	$sqlempbargain="SELECT DISTINCT EM.emp_code,EM.emp_name FROM customer_master CM,employee_master EM,DO_master DT WHERE 
				SUBSTRING(DT.sauda_no,3,5)=EM.emp_code AND DT.customer_code=CM.customer_code ORDER BY EM.emp_name ASC"; 					
...
<br />
<div id="display" style="max-height: 440px; width:97%; overflow-y: scroll;" align="center">
</div>
</center>
</body>
<?php } ?>