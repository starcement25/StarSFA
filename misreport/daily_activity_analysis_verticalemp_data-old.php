...
$employeeval=str_replace("'","",$_GET['empcode']);
$employeevalarray=explode(",",$employeeval);
$employee_upper_hierarchy_val='';
foreach($employeevalarray as $employeevalfinal)
{
$employee_upper_hierarchy=return_employee_upper_hierarchy($employeevalfinal);
$employee_upper_hierarchy_val.=$employee_upper_hierarchy.","."'".$employeevalfinal."'".",";
}
$employee_upper_hierarchy_val=substr($employee_upper_hierarchy_val,0,-1);
...
if($retailer == 'true' && $distributor == 'true'){
	...
	$table_column = "<td colspan=\"$colspan_vertical\" width=\"15%\">Primary</td>
					 <td colspan=\"$colspan_vertical\" width=\"15%\">Secondary</td>";
...
SUM(CASE WHEN POCM.cust_type='R' AND POCM.order_no LIKE 'O%' THEN POCM.amount ELSE 0 END ) AS secondary_amount FROM location LO 
LEFT JOIN `prev_order_counting_master` 
POCM ON POCM.order_no=LO.trans_id LEFT JOIN employee_master EM ON LO.emp_code=EM.emp_code WHERE SUBSTRING(LO.trans_id,1,1) IN('A','N','O') ".$condition." 
AND LO.emp_code IN(".$employee_upper_hierarchy_val.")  GROUP BY LO.emp_code ORDER BY EM.level ASC";
...
if($total_rows>0){
	...
	echo "<tr class='TDHEAD'><td colspan='".$colspan."' align='center'>Daily Activity Analysis - ".date('d-m-Y',strtotime($start_date))."</td></tr>";
	echo "<tr class='TDHEAD_SUB' align=\"center\">
			<td width=\"7%\">SI</td>
			<td width=\"12%\">Employee Name</td>
			<td width=\"9%\">".$table_columnname."</td>
			<td width=\"9%\">Total Calls</td>
			<td width=\"9%\">Productive Calls</td>
			<td width=\"9%\">Non Productive Calls</td>
			<td width=\"7%\">LPPC</td>
	...
	while($row_get_empdetails = mysqli_fetch_assoc($res_get_empdetails)){
		...
		/*----> Get employee name <----*/
		$sql_emp_name = "SELECT EM.emp_name,EM.designation,SUBSTRING(LO.date,12) AS attendance FROM employee_master EM,location LO 
			...
			if($retailer == 'true' && $distributor == 'true'){
				for($i=0;$i<2;$i++)
				{
					foreach($verticalarray as $verticalvalfinal)
					{
					  if($i==0)
					   {
					     $table_column_data_primary.= "<td align='right'>--</td>";
					   }
					   if($i==1)
					   {
						  $table_column_data_secondary.= "<td align='right'>--</td>";
					   }
					}
					if($i==0)
					   {
		...
		echo "<tr>
				<td>".$count."</td>
				<td style='background:$color_name'>".$emp_name." - ".$designation."</td>
