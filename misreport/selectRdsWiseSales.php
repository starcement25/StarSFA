...
<?php 
	...
	$date_condition_till_day=" AND DATE_FORMAT(SUBSTRING(OH.order_no,-14,8),'%Y%-%m-%d') <=DATE_FORMAT(NOW(),'%Y%-%m-%d')";
	//For  Today
	if($radio_type=='today')
	{
		$date=date('Y-m-d');
		if($date!='')
		{
			...
			/*$date_condition_opening_stock =" AND DATE_FORMAT(SUBSTRING(OH.order_no,-14,8),'%Y-%m-%d') <'".$date."' 
										AND DATE_FORMAT(SUBSTRING(OH.order_no,-14,8),'%Y%-%m-%d') >='".$fiinancial_year."'";*/
			$date_condition_opening_stock =" AND DATE_FORMAT(SUBSTRING(OH.order_no,-14,8),'%Y-%m-%d') <'".$date."'";							
		}
	...
	if($radio_type=='monthly')
	{
		...
		/*$date_condition_opening_stock =" AND DATE_FORMAT(SUBSTRING(OH.order_no,-14,8),'%Y-%m-%d') < '".$year.'-'.$month.'-01'."'
										AND DATE_FORMAT(SUBSTRING(OH.order_no,-14,8),'%Y%-%m-%d') >='".$fiinancial_year."'";*/
		$date_condition_opening_stock =" AND DATE_FORMAT(SUBSTRING(OH.order_no,-14,8),'%Y-%m-%d') < '".$year.'-'.$month.'-01'."'";													
	...
	if($radio_type=='yourchoice')
	{
	  ...
	  /*$date_condition_opening_stock =" AND DATE_FORMAT(SUBSTRING(OH.order_no,-14,8),'%Y-%m-%d') <'".$from_date."' 
	  									AND DATE_FORMAT(SUBSTRING(OH.order_no,-14,8),'%Y%-%m-%d') >='".$fiinancial_year."'";	*/	
		$date_condition_opening_stock =" AND DATE_FORMAT(SUBSTRING(OH.order_no,-14,8),'%Y-%m-%d') <'".$from_date."'";								
...
<table width="90%" align="center" border="0" cellpadding="5" cellspacing="2" class="border" style="height: 320px;overflow-y: scroll;display:block;">
   ...
   $cnt=$GLOBALS[start]+1;
	while($rowproduct=mysqli_fetch_assoc($resproduct))
	{
		...
		$opening_stk=0;
		// For purchase and stock receive
		 $sql_transaction_value="SELECT SUM(CASE WHEN OH.transaction_type = 'PB' ".$date_condition_till_day." THEN OD.qty ELSE 0 END) 
									AS 'total_purchase_till_day',
									SUM(CASE WHEN OH.transaction_type = 'SB' ".$date_condition_till_day." THEN OD.qty ELSE 0 END) 
									AS 'total_sale_till_day',
									SUM(CASE WHEN OH.transaction_type = 'ST' ".$date_condition_till_day." THEN OD.qty ELSE 0 END) 
									AS 'total_transfer_till_day',
									SUM(CASE WHEN OH.transaction_type = 'BT' ".$date_condition_till_day." THEN OD.qty ELSE 0 END) 
									AS 'total_receive_till_day',
									SUM(CASE WHEN OH.transaction_type IN('SH') ".$date_condition_till_day." THEN OD.qty ELSE 0 END) 
									AS 'total_shortage_till_day', 
									SUM(CASE WHEN OH.transaction_type = 'PB' ".$date_condition." THEN OD.qty ELSE 0 END) 
									AS 'total_purchase_value',
									SUM(CASE WHEN OH.transaction_type = 'SB' ".$date_condition." THEN OD.qty ELSE 0 END) 
									AS 'total_sale_value',
									SUM(CASE WHEN OH.transaction_type = 'ST'  ".$date_condition." THEN OD.qty ELSE 0 END) 
									AS 'total_transfer_value',
									SUM(CASE WHEN OH.transaction_type = 'BT' ".$date_condition." THEN OD.qty ELSE 0 END) 
									AS 'total_receive_value',
									SUM(CASE WHEN OH.transaction_type IN('SH') ".$date_condition." THEN OD.qty ELSE 0 END) 
									AS 'total_shortage_value',
									SUM(CASE WHEN OH.transaction_type = 'PB' ".$date_condition_opening_stock." THEN OD.qty ELSE 0 END) 
									AS 'total_purchase_op_stk',
									SUM(CASE WHEN OH.transaction_type = 'SB' ".$date_condition_opening_stock." THEN OD.qty ELSE 0 END) 
									AS 'total_sale_op_stk',
									SUM(CASE WHEN OH.transaction_type = 'ST' ".$date_condition_opening_stock." THEN OD.qty ELSE 0 END) 
									AS 'total_transfer_op_stk',
									SUM(CASE WHEN OH.transaction_type = 'BT' ".$date_condition_opening_stock." THEN OD.qty ELSE 0 END) 
									AS 'total_receive_op_stk',
									SUM(CASE WHEN OH.transaction_type IN('SH') ".$date_condition_opening_stock." THEN OD.qty ELSE 0 END) 
									AS 'total_shortage_op_stk'
									FROM 
									order_header OH,order_details OD
									WHERE 
									OH.order_no=OD.order_no AND OD.sku_code='".$prod_code."' AND SUBSTRING(OH.order_no,1,1)='O'
									AND SUBSTRING(OH.order_no,2,5)='".$emp_code."'";
	   $res_transaction_value=mysqli_query($link,$sql_transaction_value) or die(mysqli_error()." Error in select transaction value: ".$sql_transaction_value);
	   $row_transaction_value=mysqli_fetch_assoc($res_transaction_value);
	   $purchase_till_day=$row_transaction_value['total_purchase_till_day'];
	   $purchase_value=$row_transaction_value['total_purchase_value'];
	   $purchase_value_op_stk=$row_transaction_value['total_purchase_op_stk'];
	   $sale_till_day=$row_transaction_value['total_sale_till_day'];
	   $sale_value=$row_transaction_value['total_sale_value'];
	   $sale_value_op_stk=$row_transaction_value['total_sale_op_stk'];
	   $stock_transfer_till_day=$row_transaction_value['total_transfer_till_day'];
	   $stock_transfer_value=$row_transaction_value['total_transfer_value'];
	   $stock_transfer_value_op_stk=$row_transaction_value['total_transfer_op_stk'];
	   $stock_receive_till_day=$row_transaction_value['total_receive_till_day'];
	   $stock_receive_value=$row_transaction_value['total_receive_value'];
	   $stock_receive_value_op_stk=$row_transaction_value['total_receive_op_stk'];	
	   $shortage_till_day=$row_transaction_value['total_shortage_till_day'];
	   $shortage_value=$row_transaction_value['total_shortage_value'];
	   $shortage_value_op_stk=$row_transaction_value['total_shortage_op_stk'];		
