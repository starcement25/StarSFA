...
<table width="80%" align="center" cellpadding="2" cellspacing="2" border="0">
	<tr>
		<td width="100%" align="left" valign="middle" style="padding:10px;"><strong> Administrator >>Sauda Release Layout</strong></td>
	</tr>
...
class ZipFile
{
 ...
 //For sauda csv download
	$arr_header = array('Customer Code','Customer Name','Route Name','Broker Code','Broker Name','Material Description','Material Code','Material Qty(case)','PR00','FRC1','Sale Rate','Incoterms','App Contract No','App Contract Date','Contract Valid From','Contract Valid To','Material Qty(MT)','Product Group','Depot Code','Depot Name','State','Plant Name','Realization Per MT','UOM (case)','Pack Size','Material Cost','Primary Freight','Packing Cost','Honeycomb cost','Brokerage cost','Detention charges','Depot Cost','Margin Cost TP','Secondary Freight','TD','LTD','Premium','Total Value','Employee Name','Payment Due On','Remarks','Vertical','Realization Per case','Actual Packing Cost','Sauda Type','Margin Cost RA');
	$headersauda_header = "";
	...
	echo $zip->file();*/
	header("Content-type: application/octet-stream");
	header("Content-Disposition: inline; filename=Sauda Release layout.csv");
