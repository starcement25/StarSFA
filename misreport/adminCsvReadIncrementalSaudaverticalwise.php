	...
	<tr>
		<td valign="top" >
			<table width="70%" align="center" cellpadding="5" cellspacing="2">
            	 <tr> 
                    <td width="90%" align="center" class="ERR"><font size="+2"><u>Upload Master Data</u></font></td>
            	</tr>
            </table>
          </td>
    </tr>     
...
function csv_upload(){
		...
		foreach($lines as $line)
		{
			...
			{ 
			  ...
			  //print_r($data);
				...
				/*$dns_branch_code_prefix=substr($dns_branch_code,0,1);
				$dns_branch_code_length=strlen($dns_branch_code);
				$dns_branch_code_suffix=substr($dns_branch_code,2,($dns_branch_code_length-2));
				
				if($dns_branch_code_prefix !='BC')
				{
					$dns_branch_code='BC'.$dns_branch_code_suffix;
				}*/
				...
				if($countbranchnamechk<1)
				{
					...
					$sqlbranch .= " , HQ='".mysqli_real_escape_string($HQ)."'";
				...
				else
				{
					...
					$sqlbranch .= " , alternative_email_id='".mysqli_real_escape_string($alternative_email_id)."'";
					$sqlbranch .= " , download_time=CURRENT_TIMESTAMP()";
					$sqlbranch .= " , plant_name='".mysqli_real_escape_string($plant_name)."', HQ='".mysqli_real_escape_string($HQ)."' 
									WHERE branch_code='".addslashes($branch_code_db)."'";
				}
				...
				{ 
				  ...
				  //print_r($data);
					...
					if($countprodsubgroupnamechk<1){
						...
						mysqli_query($link,$sqlbrandform) or array_push($error_array,"mysqli_error().Duplicate key @row $csv_row_count on Brand code and Brand form code columns in Brand Form Master.csv.Please check.");
					}
					else
					{
						...
						if($product_group_name_existing!=$product_group_name || $vertical_value_existing!=$vertical_value)
						{
							...
							mysqli_query($link,$sqlupdatebrandform) or die(mysqli_error().".Internel error occurrs @row $csv_row_count on Brand Form Master.csv.Please check.");
						}
					}
				}
				 $rec_count++;
				...
				{ 
				  ...
				  //print_r($data);
					...
					if($countprodsubgroupnamechk<1){*/
						...
						mysqli_query($link,$sqlbrandsubform) or array_push($error_array,"mysqli_error().Duplicate key @row $csv_row_count on Brand code and Brand form code columns in Brand Sub Form Master.csv.Please check.");
						//exit();
					/*}
					else
					{
		...
		if(similar_file_exists("../csv/$folderName/Sku master.csv")!=false)
		{
			...
			foreach($lines as $line)
			{
				...
				{ 
					...
					$TD=trim($data[16]);
					$formulation=trim($data[17]);
					$pack_type=trim($data[18]);
					//echo $_SESSION['vertical_value'];
					if($_SESSION['admin_login']!='admin'){
					  if($_SESSION['vertical_value']!=$vertical_value){
						  $GLOBALS['msg'] = "You do not have access to upload sku of vertical ".$vertical_value;
				          disphtml("main();");
						  exit;
					...
					$branch_code=$rowbranchcode['branch_code'];
						...
						if($countprodgroupnamechk<1){
							...
							$sqlbrand .= " , formulation='".addslashes($formulation)."'";
							$sqlbrand .= " , download_time=CURRENT_TIMESTAMP()";
						...
						else
						{
							...
							if($vertical_value_db!=$vertical_value)
							{
								...
								$sqlupdatebrand .= " , download_time=CURRENT_TIMESTAMP(), formulation='".addslashes($formulation)."' 
					...
					if(no_of_filter > 3){
						...
						else
						{
							...
							$pack_type_db=$rowskunamechk['pack_type'];
							...
							if($acedns_db!=$acedns || $black_list_db!=$black_list  || $prod_desc_db!=$prod_desc 
								...
								|| $vertical_value_db!=$vertical_value || $conversion_factor_two_db!=$conversion_factor_two || $UOM3_db!=$UOM3 
								|| $pack_size_db!=$pack_size || $TD_db!=$TD || $pack_type_db!=$pack_type)
							{
		...
		if($cntproductgroupstatus>0)
		 {
			 ...
			 $sqlupdateproductgroupstatus="UPDATE product_group_master set acedns='N',download_time=CURRENT_TIMESTAMP() 
			 							WHERE product_group_code IN(".$groupcodestatus.")";
			 mysqli_query($link,$sqlupdateproductgroupstatus);
			 $sqlupdateproductgroupstatustoY="UPDATE product_group_master set acedns='Y',download_time=CURRENT_TIMESTAMP() 
			 									WHERE product_group_code NOT IN(".$groupcodestatus.") AND acedns='N'";
			 mysqli_query($link,$sqlupdateproductgroupstatustoY);
		 }
			...
			foreach($lines as $line)
			{
				...
				{ 
				 ...
				 //print_r($data);
					...
					while($rowproductsubgroupcode=mysqli_fetch_assoc($rsproductsubgroupcode))
					{
			...
			foreach($lines as $line)
			{
				...
				{ 
				  ...
				  $data[]=$value;
					...
					$vertical_value=trim($data[3]);
					if(strpos($vertical_value,';')!=false)
					 {
					...
					if($_SESSION['admin_login']!='admin'){
					  if($_SESSION['vertical_value']!=$vertical_value){
						  $GLOBALS['msg'] = "You do not have access to upload employee of varticel ".$vertical_value;
				          disphtml("main();");
						  exit;
					  }
					}				
					...
					while($rowbranchcode=mysqli_fetch_assoc($rsbranchcode))
					{
						...
						$branch_code_string=$branch_code_string."'".$rowbranchcode['branch_code']."'".',';
					}
					...
					if(providing_code=='yes'){
						$sqlempnamechk="SELECT emp_code,acedns,branch_code FROM employee_master WHERE dns_emp_code='".$dns_employee_code."' ".$vertical_condition."";
					}
					else{
						$sqlempnamechk="SELECT emp_code,acedns,branch_code FROM employee_master WHERE emp_name='".addslashes($employee_name)."' ".$vertical_condition."";
					}
					...
					else
					{
						...
						$sqlupdatedesignation="UPDATE TD_allocation_access SET designation='".$designation."' WHERE emp_code='".$emp_code_db."'";
						mysqli_query($link,$sqlupdatedesignation);
						$sqlproductupdate="UPDATE product_master SET download_time=CURRENT_TIMESTAMP() WHERE 
											branch_code IN(".$branch_code_string.") AND vertical_value='".$vertical_value."' AND acedns='Y'";
						mysqli_query($link,$sqlproductupdate);
						$sqlsaudamrpupdate="UPDATE sauda_mrp SET download_time=CURRENT_TIMESTAMP() WHERE 
											branch_code IN(".$branch_code_string.") AND vertical_value='".$vertical_value."'";
						mysqli_query($link,$sqlsaudamrpupdate);					
						array_push($lower_leaves_array,$emp_code_db);
						modifyempdatadownloadlog($emp_code_db,strtoupper($folderName));
					}
				}
				 $rec_count++;
			}
			//For Employee lower leaves data preperation
			foreach($lower_leaves_array as $lower_leaves_values)
			{
				$employee_hierarchy=return_employee_hierarchy($lower_leaves_values);
				$sqlupdateemp="UPDATE employee_master set lower_leaves='".addslashes($employee_hierarchy)."' WHERE emp_code='".$lower_leaves_values."'";
	...
	if(similar_file_exists("../csv/$folderName/Customer Master.csv")!=false)
	{
		...
		$err = "";
			...
			$lines = file($filename);
			$countroute=0;
			$not_exists_rds_string='';
			$not_exists_rds_array=array();
			foreach($lines as $line)
			{
				$i = 0;
				$char = substr($line, $i, 1);
				...
				{ 
				  ...
				  //print_r($data);
					...
					if($rds_tag!='' && $customer_type!='D')
					{
						...
						$rds_tag=str_replace('/ ',',',$rds_tag);
						$rds_tag=str_replace('/',',',$rds_tag);
						$rds_tag_array=explode(',',$rds_tag);
						foreach($rds_tag_array as $rds_tag_val)
						{
							if(providing_code=='yes'){
							  $sqlrdscode="SELECT customer_code FROM customer_master WHERE dns_customer_code='".addslashes($rds_tag_val)."' AND acedns='Y'";
							}
							else
							{
							  $sqlrdscode="SELECT customer_code FROM customer_master WHERE customer_name='".addslashes($rds_tag_val)."' AND acedns='Y'";
							}
							$rsrdscode=mysqli_query($link,$sqlrdscode);
							$countrdscode=mysqli_num_rows($rsrdscode);
							$rowrdscode=mysqli_fetch_assoc($rsrdscode);
							$rds_code=$rowrdscode['customer_code'];
							$rds_code_val=$rds_code_val.$rds_code.',';
							
							if($countrdscode==0){
								if(!in_array($rds_tag_val,$not_exists_rds_array))
								{
									$not_exists_rds_string=$not_exists_rds_string.$rds_tag_val.',';
								}
								array_push($not_exists_rds_array,$rds_tag_val);
							}
						}
						$rds_code_val=substr($rds_code_val,0,-1);
					}
					else
					{
						$rds_code_val='';
					...
					$csv_row_count=$rec_count+1;
					if($countcustomernamechk<1 && $route_code!='')
					{
						if(($rds_code_val!='' && $customer_type!='D') || $customer_type=='D'){
					...
					else
					{
						...
						if($route_code_db!=$route_code || $emp_code_db!=$emp_code || $current_balance_db!=$current_balance || $acedns_db!=$acedns || $black_list_db!=$black_list || $TD_db!=$TD || $customer_type_db!=$customer_type || $rds_tag_db!=$rds_code_val 
						|| $branch_code_db!=$branch_code || $sauda_validity_period_db!= $sauda_validity_period || $credit_days_db!= $credit_days 
						|| $customer_name_db!=$customer_name || $dns_customer_code_db!=$dns_customer_code || $phone_no_db!=$phone_no || $visit_day_db!=$visit_day || 
						$state_code_db!=$state || $email_db!=$email || $monthly_potential_db!=$monthly_potential || $sauda_limit_db!=$sauda_limit 
						|| $incoterms_db!=$incoterms || $loadability_ton_db!=$loadability_ton || $transport_mode_db!=$transport_mode || $sauda_type_db!=$sauda_type || $zone_db!=$zone)
						{
							if(($rds_code_val!='' && $customer_type!='D') || $customer_type=='D'){
								$sqlupdaterdstag=" , rds_tag='".$rds_code_val."'";
							}
							else
							{
								$sqlupdaterdstag="";
							...
							$sqlupdated .= " $sqlupdaterdstag,visit_day='".addslashes($visit_day)."',
											state_code='".addslashes($state)."',email='".addslashes($email)."',
											...
											zone='".$zone."',download_time=CURRENT_TIMESTAMP() 
											 WHERE  ".$update_condition." AND emp_code='".$emp_code."' AND route_code='".$route_code."' AND acedns='Y'";
					...
					//For customer sauda limit
					if($customer_type=='D'){
						$sqlcustomersaudalimit="SELECT customer_code,sauda_limit FROM customer_sauda_limit WHERE customer_code='".$dns_customer_code."'";
						...
						if($countcustomersaudalimit <1)
						{
							...
							$sqlinsertlimitlog=" INSERT INTO customer_sauda_limit_update_log ";
							$sqlinsertlimitlog .= " SET customer_code='".$dns_customer_code."'";
							$sqlinsertlimitlog .= " , previous_sauda_limit='0'";
							$sqlinsertlimitlog .= " , uploaded_sauda_limit='".$sauda_limit."'";
							$sqlinsertlimitlog .= " ,vertical_value='".$_SESSION['vertical_value']."'";
							$sqlinsertlimitlog .= " ,ip_address='".$_SERVER['REMOTE_ADDR']."'";
							$sqlinsertlimitlog .= " ,user_id='".$_SESSION['admin_login']."'";
							$sqlinsertlimitlog .= " ,datetime=CURRENT_TIMESTAMP()";
							mysqli_query($link,$sqlinsertlimitlog);
						}
						else
						{
							$rowcustomersaudalimit=mysqli_fetch_assoc($rscustomersaudalimit);
							$previuos_sauda_limit=$rowcustomersaudalimit['sauda_limit'];
							...
							if($previuos_sauda_limit!=$sauda_limit){
								$sqlinsertlimitlog=" INSERT INTO customer_sauda_limit_update_log ";
								$sqlinsertlimitlog .= " SET customer_code='".$dns_customer_code."'";
								$sqlinsertlimitlog .= " , previous_sauda_limit='".$previuos_sauda_limit."'";
								$sqlinsertlimitlog .= " , uploaded_sauda_limit='".$sauda_limit."'";
								$sqlinsertlimitlog .= " ,vertical_value='".$_SESSION['vertical_value']."'";
								$sqlinsertlimitlog .= " ,ip_address='".$_SERVER['REMOTE_ADDR']."'";
								$sqlinsertlimitlog .= " ,user_id='".$_SESSION['admin_login']."'";
				...
				if(sauda_allocation=='yes' && sauda_depot_wise=='yes')
				{
					...
					//End of customer branch relation
				}
				 $rec_count++;
			}
			//exit();

			//For Not exists rds code
			if($not_exists_rds_string!='' && count($not_exists_rds_array) >0)
			{
				$not_exists_rds_string='Distributor data '.$not_exists_rds_string.' Not exists in customer master';
				array_push($error_array,$not_exists_rds_string);
			}
			
			//Emp code checking start
				$sqlempcoderetail="SELECT emp_code FROM customer_master WHERE acedns='Y' and black_list='N' AND emp_code NOT IN(SELECT emp_code FROM employee_master)";
				...
				{ 
				  ...
				  //print_r($data);
					...
					if(providing_code=='yes'){
						...
						$rowempcode=mysqli_fetch_assoc($rsempcode);
						$emp_code=$rowempcode['emp_code'];
						$sqlcustomercode="SELECT customer_code,emp_code FROM customer_master WHERE dns_customer_code='".$customer_code_name."' AND emp_code='".$emp_code."' ";
					}
					else
					{
	...
	//For Sauda MRP CSV
	if(similar_file_exists("../csv/$folderName/Sauda mrp.csv")!=false)
	{
		$filename=similar_file_exists("../csv/$folderName/Sauda mrp.csv");
		$rec_count = 0;
		$ins_count = 0;
		$err = "";
			...
			foreach($lines as $line)
			{
				...
				//print_r($data);
					...
					$vertical_value=trim($data[5]);
					//$UOM=trim($data[6]);
					$basic_rate=trim($data[6]);
					$primary_freight=trim($data[7]);
					$depot_cost=trim($data[8]);
					
					if($_SESSION['admin_login']!='admin'){
					  if($_SESSION['vertical_value']!=$vertical_value){
						  $GLOBALS['msg'] = "You do not have access to upload mrp of varticel ".$vertical_value;
				          disphtml("main();");
						  exit;
					...
					if(providing_code=='yes'){
						if(branch_wise_product=='yes')
						{
							$sqlprodcode="SELECT prod_code,product_group_code FROM product_master WHERE dns_prod_code='".$prod_code_name."' 
											AND branch_code='".$branch_code."' AND acedns='Y' AND black_list='N'";
							/*$sqlprodcode="SELECT prod_code FROM product_master WHERE dns_prod_code='".$prod_code_name."' AND branch_code='".$branch_code."'";*/				
						}
						else
						{
							$sqlprodcode="SELECT prod_code,product_group_code FROM product_master 
										WHERE dns_prod_code='".$prod_code_name."' AND branch_code='".$branch_code."' AND acedns='Y' AND black_list='N'";
						}
					}
					else
					{
						$sqlprodcode="SELECT prod_code,product_group_code FROM product_master WHERE prod_desc='".addslashes($prod_code_name)."' AND acedns='Y' AND black_list='N'";
					}
					$rsprodcode=mysqli_query($link,$sqlprodcode);
					$rowprodcode=mysqli_fetch_assoc($rsprodcode);
					$prod_code=$rowprodcode['prod_code'];
					$product_group_code=$rowprodcode['product_group_code'];
					if(strtolower($_SESSION['vertical_value'])=='specialty fats' && strtoupper($_SESSION['nick_name'])=='EMAMIT')
					{
						$sqlselmargin="SELECT margin_cost FROM margin_cost WHERE dns_prod_code='".$prod_code_name."' 
										AND branch_code='".$branch_code."' AND vertical_value='".$_SESSION['vertical_value']."'";
						$rsselmargin=mysqli_query($link,$sqlselmargin);	
						$rowselmargin=mysqli_fetch_assoc($rsselmargin);
						$margin_cost=$rowselmargin['margin_cost'];			
					}
					if($margin_cost=='')		$margin_cost=0;
					
					$sqlisupload="SELECT is_upload,product_group_name FROM product_group_master WHERE product_group_code='".$product_group_code."'";
					$rsisupload=mysqli_query($link,$sqlisupload);
					$rowisupload=mysqli_fetch_assoc($rsisupload);
					$is_upload=$rowisupload['is_upload'];
					$product_group_name=$rowisupload['product_group_name'];
					if($is_upload=='no')
					{
						$GLOBALS['msg'] = "Uploading facility is blocked for the product '".$prod_code_name."' of the category '".$product_group_name."'";
						disphtml("main();"); 
						die;
					}					
					if(uom_wise_mrp=='yes' && branch_wise_mrp=='yes')
					{
						$sqlmrpchk="SELECT * FROM sauda_mrp WHERE product_code='".$prod_code."' AND UOM='".$UOM."' AND branch_code='".$branch_code."'";
					}
					else if(branch_wise_mrp=='yes')
					{
						$sqlmrpchk="SELECT SM.product_code,SM.mrp_code FROM sauda_mrp SM,product_master PM WHERE SM.product_code='".$prod_code."' 
									AND SM.branch_code='".$branch_code."' AND SM.product_code=PM.prod_code AND PM.acedns='Y' AND PM.black_list='N'";
						/*$sqlmrpchk="SELECT * FROM sauda_mrp WHERE product_code='".$prod_code."' AND branch_code='".$branch_code."'";*/
					...
					if($countmrpchk<1){
						...
						$sql  = "insert into sauda_mrp ";
						$sql .= " SET product_code='".$prod_code."'";
						$sql .= " , branch_code='".$branch_code."'";
						$sql .= " , mrp_code='".$max_mrp_code."'";
						$sql .= " , dns_mrp_code='".$dns_mrp_code."'";
						$sql .= " , mrp='".mysqli_real_escape_string($sale_rate)."'";
						$sql .= " , sale_rate='".mysqli_real_escape_string($sale_rate)."'";
						$sql .= " , vertical_value='".mysqli_real_escape_string($vertical_value)."'";
						$sql .= " , UOM='".mysqli_real_escape_string($UOM)."'";
						$sql .= " , basic_rate='".mysqli_real_escape_string($sale_rate)."'";
						$sql .= " , primary_freight='".mysqli_real_escape_string($primary_freight)."'";
						$sql .= " , depot_cost='".mysqli_real_escape_string($depot_cost)."'";
						$sql .= " , margin_cost='".mysqli_real_escape_string($margin_cost)."'";
						$sql .= " , download_time=CURRENT_TIMESTAMP()";
						mysqli_query($link,$sql)  or  array_push($error_array,"mysqli_error().Duplicate key @row $csv_row_count on Mrp code  columns in Sauda mrp.csv.Please check.");
						$insertflag=1;
						$suda_mrp_code=$max_mrp_code;
					}
					else
					{
						$rowmrpchk=mysqli_fetch_assoc($rsmrpchk);
						...
						if($mrp_db!=$mrp || $sale_rate_db!=$sale_rate){
							$sqlupdate  = "UPDATE sauda_mrp ";
							$sqlupdate .= " SET mrp='".mysqli_real_escape_string($sale_rate)."'";
							$sqlupdate .= " , sale_rate='".mysqli_real_escape_string($sale_rate)."'";
							$sqlupdate .= " , basic_rate='".mysqli_real_escape_string($sale_rate)."'";
							$sqlupdate .= " , primary_freight='".mysqli_real_escape_string($primary_freight)."'";
							$sqlupdate .= " , depot_cost='".mysqli_real_escape_string($depot_cost)."'";
							$sqlupdate .= " , margin_cost='".mysqli_real_escape_string($margin_cost)."'";
							$sqlupdate .= " , vertical_value='".mysqli_real_escape_string($vertical_value)."'";
							$sqlupdate .= " , download_time=CURRENT_TIMESTAMP() WHERE product_code='".$prod_code."' AND branch_code='".$branch_code."'";
							mysqli_query($link,$sqlupdate) or  array_push($error_array,".Internal error occurs @row $csv_row_count on Sauda mrp.csv.Please check.");
							$updateflag=1;
						}
						$suda_mrp_code=$mrp_code_db;
					}
					//For inserting on sauda mrp upload log
					$sqlinsertmrplog  = "insert into sauda_mrp_upload_log ";
					$sqlinsertmrplog .= " SET product_code='".$prod_code."'";
					$sqlinsertmrplog .= " , branch_code='".$branch_code."'";
					$sqlinsertmrplog .= " , mrp_code='".$suda_mrp_code."'";
					$sqlinsertmrplog .= " , dns_mrp_code='".$dns_mrp_code."'";
					$sqlinsertmrplog .= " , mrp='".mysqli_real_escape_string($sale_rate)."'";
					$sqlinsertmrplog .= " , sale_rate='".mysqli_real_escape_string($sale_rate)."'";
					$sqlinsertmrplog .= " , vertical_value='".mysqli_real_escape_string($vertical_value)."'";
					$sqlinsertmrplog .= " , UOM='".mysqli_real_escape_string($UOM)."'";
					$sqlinsertmrplog .= " , download_time=CURRENT_TIMESTAMP()";
					mysqli_query($link,$sqlinsertmrplog)  or  array_push($error_array,"mysqli_error().Error on inserting sauda mrp insert log.");
			...
			//Product code checking start
				$sqlprodcodeprice="SELECT product_code FROM sauda_mrp WHERE product_code <> '' AND product_code NOT IN
									(SELECT prod_code FROM product_master) GROUP BY product_code";
				$rsprodcodeprice=mysqli_query($link,$sqlprodcodeprice);
				$cntprodcodeprice=mysqli_num_rows($rsprodcodeprice);
				if($cntprodcodeprice>0)
				{
					...
					$prodcodeprice=substr($prodcodeprice,0,-1);
					$errorprodcodeprice=$prodcodeprice.' exists in Sauda mrp but not exists in Sku Master.';
					array_push($error_array,$errorprodcodeprice);
				}
			//Product code checking end		
			$successval=1;
				...
				{ 
				  ...
				  //print_r($data);
					...
					if($countbrokerchk<1)
					{
						...
						$sqlbroker .= " , brokerage_cost='".mysqli_real_escape_string($brokerage_cost)."'";
						$sqlbroker .= " , state_code='".mysqli_real_escape_string($state_code)."'";
					...
					else
					{
						...
						$sqlupdatebroker .= " , brokerage_cost='".mysqli_real_escape_string($brokerage_cost)."'";
						$sqlupdatebroker .= " , acedns='".mysqli_real_escape_string($acedns)."'
											  , state_code='".mysqli_real_escape_string($state_code)."',download_time=CURRENT_TIMESTAMP()";
	...
	//For Customer vertical csv
	if(similar_file_exists("../csv/$folderName/Customer Vertical.csv")!=false)
	{
		$filename=similar_file_exists("../csv/$folderName/Customer Vertical.csv");
		$rec_count = 0;
		$ins_count = 0;
		$err = "";
			...
			foreach($lines as $line)
			{
				...
				{ 
				 ...
				 //print_r($data);
				  	$csv_row_count=$rec_count+1;
					$customer_code_name=trim($data[0]);
				    $vertical_value=trim($data[1]);
					$TD=trim($data[2]);
					$credit_days=trim($data[3]);
					...
					$sqlcustomervertical="SELECT customer_code FROM customer_vertical_TD_relation WHERE customer_code='".$customer_code."' 
										AND vertical_value='".$vertical_value."'";
					$rscustomervertical=mysqli_query($link,$sqlcustomervertical);
					$countcustomervertical=mysqli_num_rows($rscustomervertical);
					if($countcustomervertical <1)
					{
						$sqlinsertcustomervertical="INSERT INTO customer_vertical_TD_relation ";
						$sqlinsertcustomervertical .= " SET customer_code='".$customer_code."'";
						$sqlinsertcustomervertical .= " ,vertical_value='".$vertical_value."'";
						$sqlinsertcustomervertical .= " ,TD='".$TD."'";
						$sqlinsertcustomervertical .= " ,credit_days='".$credit_days."'";
						$sqlinsertcustomervertical .= " ,download_time=CURRENT_TIMESTAMP()";
						mysqli_query($link,$sqlinsertcustomervertical)  or die(mysqli_error().".Internal error occurrs @row $csv_row_count in Customer Vertical.csv.Please check.");
					}
					else
					{
						$sqlupdatecustomervertical="UPDATE customer_vertical_TD_relation SET TD='".$TD."',credit_days='".$credit_days."',
												download_time=CURRENT_TIMESTAMP() WHERE customer_code='".$customer_code."' AND vertical_value='".$vertical_value."'";
						$rsupdateacednsstatus=mysqli_query($link,$sqlupdatecustomervertical)  or die(mysqli_error().".Internal error occurrs @row $csv_row_count in Customer Vertical.csv.Please check.");
					}
				}
				 $rec_count++;
			}
		...
		/*else
		{
			echo $successval="Naming convention for Customer vertical.csv is wrong.";
			exit();
		}*/
		
		//For Customer vertical credit limit csv
		if(similar_file_exists("../csv/$folderName/Customer vertical creditlimit.csv")!=false)
		{
			$filename=similar_file_exists("../csv/$folderName/Customer vertical creditlimit.csv");
			$rec_count = 0;
			$ins_count = 0;
			$err = "";
				...
				foreach($lines as $line)
				{
					...
					{ 
					 ...
					 //print_r($data);
						$csv_row_count=$rec_count+1;
						$branch_code_name=trim($data[0]);
						$customer_code_name=trim($data[1]);
						$vertical_value=trim($data[2]);
						$credit_limit=trim($data[3]);
						...
						$sqlcustomervertical="SELECT customer_code FROM customer_vertical_credit_limit WHERE customer_code='".$customer_code."' 
											AND vertical_value='".$vertical_value."' AND branch_code='".$branch_code."'";
						$rscustomervertical=mysqli_query($link,$sqlcustomervertical);
						$countcustomervertical=mysqli_num_rows($rscustomervertical);
						if($countcustomervertical <1)
						{
							$sqlinsertcustomervertical="INSERT INTO customer_vertical_credit_limit ";
							$sqlinsertcustomervertical .= " SET customer_code='".$customer_code."'";
							$sqlinsertcustomervertical .= " ,branch_code='".$branch_code."'";
							$sqlinsertcustomervertical .= " ,vertical_value='".$vertical_value."'";
							$sqlinsertcustomervertical .= " ,	credit_limit='".$credit_limit."'";
							$sqlinsertcustomervertical .= " ,download_time=CURRENT_TIMESTAMP()";
							mysqli_query($link,$sqlinsertcustomervertical)  or die(mysqli_error().".Internal error occurrs @row $csv_row_count in Customer vertical creditlimit.csv.Please check.");
						}
						else
						{
							$sqlupdatecustomervertical="UPDATE customer_vertical_credit_limit SET credit_limit='".$credit_limit."',
													download_time=CURRENT_TIMESTAMP() WHERE customer_code='".$customer_code."' AND vertical_value='".$vertical_value."' AND branch_code='".$branch_code."'";
							$rsupdatecustomervertical=mysqli_query($link,$sqlupdatecustomervertical)  or die(mysqli_error().".Internal error occurrs @row $csv_row_count in Customer vertical creditlimit.csv.Please check.");
						}
					}
					 $rec_count++;
				}
					 ...
					 //print_r($data);
						...
						if($customer_code!='' && $route_code!='')
						{
							...
							if($countdistributorroute <1)
							{
								...
								mysqli_query($link,$sqlinsertdistributorroute)  or die(mysqli_error().".Internal error occurrs @row $csv_row_count in Distributor route.csv.Please check.");
							}
							else
							{
								...
								if($acednsdb!=$acedns)
								{
									...
									$rsupdatedistributorroute=mysqli_query($link,$sqlupdatedistributorroute)  or die(mysqli_error().".Internal error occurrs @row $csv_row_count in Distributor route.csv.Please check.");
								}
							}
						}
					}
		...
		foreach($lines as $line)
		{
			...
			{ 
			  ...
			  //print_r($data);
				...
				$sqlbranchdestinationfreight="SELECT branch_code FROM branch_route_freight WHERE branch_code='".addslashes($branch_code)."' AND route_code='".$route_code."'";
	...
	//For State wise Depot wise TD csv
	if(similar_file_exists("../csv/$folderName/state depot wise TD.csv")!=false)
	{
		$filename=similar_file_exists("../csv/$folderName/state depot wise TD.csv");
		$rec_count = 0;
		$ins_count = 0;
		$err = "";
		...
		foreach($lines as $line)
		{
			...
			{ 
			  ...
			  //print_r($data);
				...
				$sate_code_name=trim($data[0]);
				$branch_code_name=trim($data[1]);
				$prod_code_name=trim($data[2]);
				$TD=trim($data[3]);
				...
				$sqlstate="SELECT state_code FROM state_master WHERE dns_state_code='".addslashes($sate_code_name)."'";
				$rsstate=mysqli_query($link,$sqlstate);
				$rowstate=mysqli_fetch_assoc($rsstate);
				$state_code=$rowstate['state_code'];
				
				$sqlproduct="SELECT prod_code FROM product_master WHERE dns_prod_code='".addslashes($prod_code_name)."' AND 
							branch_code='".$branch_code."' AND acedns='Y'";
				$rsproduct=mysqli_query($link,$sqlproduct);
				$rowproduct=mysqli_fetch_assoc($rsproduct);
				$prod_code=$rowproduct['prod_code'];

				$sqlbranchstateTD  = "insert into state_branch_product_wise_TD ";
				$sqlbranchstateTD .= " SET state_code='".$state_code."'";
				$sqlbranchstateTD .= " ,branch_code='".$branch_code."'";
				$sqlbranchstateTD .= " ,prod_code='".$prod_code."'";
				$sqlbranchstateTD .= " ,TD='".$TD."'";
				$sqlbranchstateTD .= " , download_time=CURRENT_TIMESTAMP()";
				mysqli_query($link,$sqlbranchstateTD) or  array_push($error_array,"mysqli_error().
								Internal DATA execution problem on state branch product wise TD table.PLease contact aceDNS admin.");
				$sqlupdateprodtime="UPDATE product_master SET download_time=CURRENT_TIMESTAMP() WHERE prod_code='".$prod_code."'";
				$rsupdateprodtime=mysqli_query($link,$sqlupdateprodtime);								
			   }
			 $rec_count++;
		}		
		$successval=1;
