...
function main(){
	...
	function display_result(){
		...
		if(survey_type == 'Site Lead and Conversion Tracking 2'){
			var site_status = document.getElementById("new_old").value;
			
			if(document.getElementById("new_old").value.search(/\S/) == -1){
		...
		//alert(url);
		//GenericAjaxFunction(''+url+'?employee='+employee+'&survey_type='+survey_type+'&start_date='+start_date+'&end_date='+end_date+'&zone='+zone+'&state='+state+'&branch='+branch+'&department='+department+'&technical_meet_type='+technical_meet_type+'&site_status='+site_status,'display',0);
		if(survey_type == 'Site Lead and Conversion Tracking 2'){
        // Store employee list in session first
        jQuery.ajax({
            url: 'store_employee_session.php',
            type: 'POST',
            dataType: 'json',
            data: { employee_list: employee },
            success: function(response) {
                if(response.status === 'success') {
                    // Call with session flag instead of employee parameter
                    GenericAjaxFunction(url+'?use_session_emp=1&survey_type='+encodeURIComponent(survey_type)+
                        '&start_date='+start_date+'&end_date='+end_date+
                        '&zone='+encodeURIComponent(zone)+'&state='+encodeURIComponent(state)+
                        '&branch='+encodeURIComponent(branch)+'&department='+encodeURIComponent(department)+
                        '&site_status='+encodeURIComponent(site_status),'display',0);
                } else {
                    alert('Error storing employee data');
        ...
        // For all other survey types, use the normal method
        GenericAjaxFunction(url+'?employee='+employee+'&survey_type='+survey_type+
            '&start_date='+start_date+'&end_date='+end_date+
            '&zone='+zone+'&state='+state+'&branch='+branch+
            '&department='+department+'&technical_meet_type='+technical_meet_type+
            '&site_status='+site_status,'display',0);
    }
		/*if(survey_type == 'KYC' )
    ...
    function UpdateElem(trans_id) {
		...
		var destination 		= document.getElementById('destination_'+trans_id).value;
		var company_constraint 	= document.getElementById('company_constraint_'+trans_id).value;
		var reason 				= document.getElementById('reason_'+trans_id).value;
		var nov 				= document.getElementById('nov_'+trans_id).value;
		var incoterms 			= document.getElementById('incoterms_'+trans_id).value;
		var serving_location 	= document.getElementById('serving_location_'+trans_id).value;
		var quoted_price 		= document.getElementById('quoted_price_'+trans_id).value;
		var tpc 				= document.getElementById('tpc_'+trans_id).value;
		var payment 			= document.getElementById('payment_'+trans_id).value;
		var last_price 			= document.getElementById('last_price_'+trans_id).value;
		var prev_last_price 	= document.getElementById('prev_last_price_'+trans_id).value;

		var lead_emp_code 		= document.getElementById('lead_emp_code_'+trans_id).value;
		var lead_party_name 	= document.getElementById('lead_party_name_'+trans_id).value;
		var lead_date 			= document.getElementById('lead_date_'+trans_id).value;
		var lead_exist 			= document.getElementById('lead_exist_'+trans_id).value;
		var product_packaging 	= document.getElementById('product_packaging_'+trans_id).value;
		var qty_req 			= document.getElementById('qty_req_'+trans_id).value;
		...
		if(company_constraint.trim()==""){
           alert("Enter company constraint");
           return false;
       	}
		if(reason.trim()==""){
           alert("Enter reason");
		...
		if(serving_location.trim()==""){
			alert("Enter Serving Location for EXW");
           return false;
       	}
		if(quoted_price.trim()==""){
			alert("Enter Quoted Price");
           return false;
       	}
		if(tpc.trim()==""){
			alert("Enter tpc");
		...
		if(last_price.trim()==""){
			alert("Enter last price");
           return false;
       	}
		if(prev_last_price.trim()==""){
			alert("Enter Prev. Last Price");
           return false;
       	}

		// let formData = new FormData();

		// // Collect all input fields manually
		// formData.append('trans_id', trans_id);
		// formData.append('destination', destination);
		// formData.append('company_constraint', company_constraint);
		// formData.append('reason', reason);
		// formData.append('nov', nov);
		// formData.append('incoterms', incoterms);
		// formData.append('serving_location', serving_location);
		// formData.append('quoted_price', quoted_price);
		// formData.append('tpc', tpc);
		// formData.append('payment', payment);
		// formData.append('last_price', last_price);
		// formData.append('prev_last_price', prev_last_price);
    ...
    //    }
        ...
        jQuery.ajax({
            ...
            // data: { trans_id: trans_id, aa: aa, bb:bb, cc:cc, dd:dd, ee:ee,ff:ff,gg:gg,hh:hh,ii:ii,jj:jj,kk:kk, ll:ll, mm:mm },
			 data: { trans_id: trans_id, destination: destination, company_constraint:company_constraint, reason:reason, nov:nov, incoterms:incoterms,serving_location:serving_location,quoted_price:quoted_price,tpc:tpc,payment:payment,last_price:last_price,prev_last_price:prev_last_price, lead_emp_code:lead_emp_code, lead_party_name:lead_party_name, lead_date:lead_date, lead_exist:lead_exist, product_packaging:product_packaging, qty_req:qty_req },
	...
	function show_option(survey_type){
		...
		else if(survey_type == 'Lead Generation New'){
			document.getElementById("date_div").hidden = false;
			document.getElementById("month_row").hidden = true;
			document.getElementById("technical_meet_row").hidden = true;
		}
   ...
   function activate_KYC(survey_id){
		...
		document.getElementById("display_details").innerHTML = '';
		document.getElementById("display").innerHTML = '<img src="ajax-loader.gif" id="ajaxloader">';
		GenericAjaxFunction(''+url+'?employee='+employee+'&survey_type='+survey_type+'&start_date='+start_date+'&end_date='+end_date+'&zone='+zone+'&state='+state+'&branch='+branch+'&department='+department+'&survey_id='+survey_id+'&site_status='+site_status+'&customer_code='+customer_code+'&opt_type=activateKYC','display',0);
		/*if(survey_type == 'KYC' )
		document.getElementById("print_export").hidden = false;*/
	}
	</script>
