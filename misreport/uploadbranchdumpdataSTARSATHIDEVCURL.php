<?php
	...
	$dump_code_array=array();
	foreach($_POST as $array_value) {
		foreach($array_value as $key=>$value) {
			if($key=='dump_code')   	 $dump_code=$value;
			if(!in_array($dump_code,$dump_code_array))
			{
				array_push($dump_code_array,$dump_code);
			}
 	 		if($key=='branch_code_name') $branch_code_name=$value;
			
			
			if(!isset(${'branch_code'.$dump_code}))
			{
				${'branch_code'.$dump_code}=array();
			}
			if(!in_array($branch_code_name,${'branch_code'.$dump_code}))
			{
			  if($branch_code_name!=''){
				array_push(${'branch_code'.$dump_code},$branch_code_name);
				}
			}
   ...
   foreach($dump_code_array as $dump_code_val)
	{
	   $sqldeldumpdata="UPDATE branch_dump SET acedns='N' WHERE dump_code='".$dump_code_val."'";
	   $rsdeldumpdata=mysqli_query($link,$sqldeldumpdata);
	   
	foreach(${'branch_code'.$dump_code_val} as $branch_code_val){
