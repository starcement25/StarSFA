...
<div id="display" style="max-height: 350px; max-width:1000px; overflow-y: scroll; overflow-x: scroll;" align="center">
...
function show_saudawise()
{
	...
	if(type != 'custom')
		GenericAjaxFunction('emamit_statewisedata.php?type='+type,'display',0);
	else 
	{
		if(response1 != 0 && response2 != 0)
			GenericAjaxFunction('emamit_statewisedata.php?type='+type+'&start_date='+start_date+'&end_date='+end_date,'display',0);
	}
...
function show_statewise_data(state,start_date,end_date,condition_value,product_group_code)
{
	...
	GenericAjaxFunction('emamit_zonewise_product_details.php?state='+state+'&product_group_code='+product_group_code+'&condition_value='+condition_value+'&start_date='+start_date+'&end_date='+end_date+'&mode=state','product',0);
}
