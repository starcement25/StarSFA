...
$ccc = $_REQUEST['ccc'];
	...
	$rs_route_code=mysqli_query($link,$sql_route_code);	
	$row_route_code=mysqli_fetch_assoc($rs_route_code);	
	...
	$res_ccc = mysqli_query($link,$sql_ccc);
	$count_ccc=mysqli_num_rows($res_ccc);
		$onclick = "show_others('kiosk_id',this.value);";
		echo "<select name=\"kiosk_id\" id=\"kiosk_id\" onchange=\"".$onclick."\">";
		echo "<option value=\"\">Select</option>";
		while($row_ccc = mysqli_fetch_assoc($res_ccc)){
