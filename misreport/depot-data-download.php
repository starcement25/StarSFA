...
<?php
$sqldepot= "SELECT * FROM (SELECT DC.dns_prod_code,DC.branch_code,PM.prod_desc,DATE_FORMAT(SUBSTRING(DC.datetime,1,10),'%d-%m-%Y') As last_updated_date,BM.plant_name,DC.depot_cost,BM.branch_name,BM.dns_branch_code,BM.branch_state,DC.depot_cost_ton FROM
				depot_cost DC,product_master PM,branch_master BM WHERE DC.dns_prod_code=PM.dns_prod_code AND 
				DC.branch_code=BM.branch_code AND PM.acedns='Y' AND 
				PM.vertical_value='".$_SESSION['vertical_value']."' ORDER BY DC.datetime DESC) AS SAT GROUP BY 1,2 ORDER BY 7";
...
if($total_depot>0){
    ...
    <thead>
     <tr>
        <td colspan="10" class="TDHEAD" align="center">Depot Cost</td>
      </tr>
      <tr class="TDHEAD_SUB" align="center">
        ...
        <td width="11%">State Name</td>
        <td width="10%">Depot cost (Rs/CASE)</td>
        <td width="">Depot cost (Rs/MT)</td>
      </tr>
      </thead>
      </table>
       ...
       <tbody>
    <?php
	while($row_depot = mysqli_fetch_assoc($rsdepot)){
		$dns_prod_code = $row_depot['dns_prod_code'];
		$prod_desc = str_replace(',','',$row_depot['prod_desc']);
		$branch_name = $row_depot['branch_name'];
		$branch_code = $row_depot['dns_branch_code'];
		$depot_cost = $row_depot['depot_cost'];
		$depot_cost_ton = $row_depot['depot_cost_ton'];
		$branch_state = $row_depot['branch_state'];
		$last_updated_date = $row_depot['last_updated_date'];
		...
		if($depot_cost >0)
		{
			echo "<tr>
					...
					<td width=\"8%\">".$branch_code."</td>
					<td width=\"10%\">".$branch_name."</td>
					<td width=\"10%\">".$dns_prod_code."</td>
					<td width=\"16%\">".$prod_desc."</td>
					<td width=\"10%\">".$state_code."</td>
					<td width=\"11%\">".$branch_state."</td>
					<td align=\"right\" width=\"10%\">".number_format($depot_cost,2)."</td>
					<td align=\"right\" width=\"\">".number_format($depot_cost_ton,2)."</td>
				  </tr>";	  
			$count++;
		}
	}
...
function exporttocsv()
{
	...
	a.download = 'Depot data' + postfix + '.xls';
...
function Popup(data) 
{
	...
	mywindow.document.write('<html><head><title>Depot Data</title>');
