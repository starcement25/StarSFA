	   ...
	   $cnt=$GLOBALS[start]+1;
		while($rowproduct=mysqli_fetch_assoc($resproduct))
		{
			...
			$sql_purchase_till_day=$sql_purchase." AND
									...
									AND DATE_FORMAT(SUBSTRING(OH.order_no,7,8),'%Y%-%m-%d') >='".$fiinancial_year."'";
		   $res_purchase_till_day=mysqli_query($link,$sql_purchase_till_day) or die(mysqli_error()." Error in select purchase till day: ".$sql_purchase_till_day);
		   ...
		   $res_purchase_query=mysqli_query($link,$sql_purchase_query) or die(mysqli_error()." Error in select purchase query: ".$sql_purchase_query);
		   $row_purchase_query=mysqli_fetch_assoc($res_purchase_query);
		   $purchase_value=$row_purchase_query['total_purchase_qty'];
		   
		   $sql_purchase_op_stk_query=$sql_purchase.$date_condition_opening_stock;
		   $res_purchase_op_stk_query=mysqli_query($link,$sql_purchase_op_stk_query) or die(mysqli_error()." Error in select 
		   						purchase query opening stock: ".$sql_purchase_op_stk_query);
		   $row_purchase_op_stk_query=mysqli_fetch_assoc($res_purchase_op_stk_query);
		  ...
		  $sale_till_day=$row_sale_till_day['total_sale_qty'];
		   ...
		   $res_sale_query=mysqli_query($link,$sql_sale_query) or die(mysqli_error()." Error in select sale query: ".$sql_sale_query);
		   $row_sale_query=mysqli_fetch_assoc($res_sale_query);
		   $sale_value=$row_sale_query['total_sale_qty'];
		   
		   $sql_sale_op_stk_query=$sql_sale.$date_condition_opening_stock;
		   $res_sale_op_stk_query=mysqli_query($link,$sql_sale_op_stk_query) or die(mysqli_error()." Error in select sale query opening stock: ".$sql_sale_op_stk_query);
		   $row_sale_op_stk_query=mysqli_fetch_assoc($res_sale_op_stk_query);
		  ...
		  $sql_stock_transfer_till_day=$sql_stock_transfer." AND
										DATE_FORMAT(SUBSTRING(OH.order_no,7,8),'%Y%-%m-%d') <=DATE_FORMAT(NOW(),'%Y%-%m-%d') 
										AND DATE_FORMAT(SUBSTRING(OH.order_no,7,8),'%Y%-%m-%d') >='".$fiinancial_year."'";
		$res_stock_transfer_till_day=mysqli_query($link,$sql_stock_transfer_till_day) or die(mysqli_error()." Error in select stock transfer till day: ".$sql_stock_transfer_till_day);
		...
		$stock_transfer_till_day=$row_stock_transfer_till_day['total_stocktransfer_qty'];
		 ...
		 $res_stock_transfer_query=mysqli_query($link,$sql_stock_transfer_query) or die(mysqli_error()." Error in select stock_transfer query: ".$sql_stock_transfer_query);
		 $row_stock_transfer_query=mysqli_fetch_assoc($res_stock_transfer_query);
		 ...
		 $res_stock_transfer_op_stk_query=mysqli_query($link,$sql_stock_transfer_op_stk_query) or die(mysqli_error()." Error in select stock_transfer query opening stock: ".$sql_stock_transfer_op_stk_query);
		 $row_stock_transfer_op_stk_query=mysqli_fetch_assoc($res_stock_transfer_op_stk_query);
		...
		$sql_stock_receive_till_day=$sql_stock_receive." AND
									DATE_FORMAT(SUBSTRING(OH.order_no,7,8),'%Y%-%m-%d') <=DATE_FORMAT(NOW(),'%Y%-%m-%d') 
									AND DATE_FORMAT(SUBSTRING(OH.order_no,7,8),'%Y%-%m-%d') >='".$fiinancial_year."'";
		$res_stock_receive_till_day=mysqli_query($link,$sql_stock_receive_till_day) or die(mysqli_error()." Error in select stock receive till day: ".$sql_stock_receive_till_day);
		...
		$stock_receive_till_day=$row_stock_receive_till_day['total_stockreceive_qty'];
		 ...
		 $res_stock_receive_query=mysqli_query($link,$sql_stock_receive_query) or die(mysqli_error()." Error in select stock_receive query: ".$sql_stock_receive_query);
		 $row_stock_receive_query=mysqli_fetch_assoc($res_stock_receive_query);
		 ...
		 $res_stock_receive_op_stk_query=mysqli_query($link,$sql_stock_receive_op_stk_query) or die(mysqli_error()." Error in select stock_receive query opening stock: ".$sql_stock_receive_op_stk_query);
		 $row_stock_receive_op_stk_query=mysqli_fetch_assoc($res_stock_receive_op_stk_query);
		...
		$sql_shortage_till_day=$sql_shortage." AND
									DATE_FORMAT(SUBSTRING(OH.order_no,7,8),'%Y%-%m-%d') <=DATE_FORMAT(NOW(),'%Y%-%m-%d') 
									AND DATE_FORMAT(SUBSTRING(OH.order_no,7,8),'%Y%-%m-%d') >='".$fiinancial_year."'";
		$res_shortage_till_day=mysqli_query($link,$sql_shortage_till_day) or die(mysqli_error()." Error in select shortage till day: ".$sql_shortage_till_day);
		...
		$shortage_till_day=$row_shortage_till_day['total_shortage_qty'];
		 ...
		 $res_shortage_query=mysqli_query($link,$sql_shortage_query) or die(mysqli_error()." Error in select shortage query: ".$sql_shortage_query);
		 $row_shortage_query=mysqli_fetch_assoc($res_shortage_query);
		 $shortage_value=$row_shortage_query['total_shortage_qty'];
		 
		 $sql_shortage_op_stk_query=$sql_shortage.$date_condition_opening_stock;
		 $res_shortage_op_stk_query=mysqli_query($link,$sql_shortage_op_stk_query) or die(mysqli_error()." Error in select shortage query opening stock: ".$sql_shortage_op_stk_query);
		 $row_shortage_op_stk_query=mysqli_fetch_assoc($res_shortage_op_stk_query);
