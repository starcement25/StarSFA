...
$sql_distinct_date = "SELECT DISTINCT survey_id, SUBSTRING(survey_id,3,5) AS emp_code,DATE_FORMAT(SUBSTRING(survey_id,-14,8),'%Y-%m-%d') AS survey_date,
					...
					AND SUBSTRING(survey_id,3,5) IN(".$employee.")  AND type = 'Counter Branding'  
...
if($total_rows>0){
	?>
        <form action="counter_branding_account.php" method="post">
    ...
    <table border="1" style="border-collapse:collapse;" class="border" width="90%">
      ...
      <tr class="TDHEAD">
        ...
        <td width="6%">Customer Typ</td>
        <td width="8%">Type of Branding</td>
        <td width="3%">L-R (ft.)</td>
        <td width="3%">T-B (ft.)</td>
        <td width="3%">Sq.Ft.</td>
	...
	while($row_survey_ouput = mysqli_fetch_assoc($res_survey_output)){
		...
		if($emp_code =='E0555')  $reporting_to='E0259';
		...
		while($row_survey_details = mysqli_fetch_assoc($res_survey_details)){
			...
			if($row_id == 'RA343'){
				$area=$survey_value;	
				$sql_branch = "SELECT branch_name FROM branch_master WHERE branch_code = '".$area."'";
				$res_branch = mysqli_query($link,$sql_branch);
				$row_branch = mysqli_fetch_assoc($res_branch);
				$branch_name = $row_branch['branch_name'];
				$area=$branch_name;
			}
			else if($row_id == 'RA344'){
				$route = $survey_value;
				$sql_route = "SELECT route_name FROM route_master WHERE route_code = '".$route."'";
				$res_route = mysqli_query($link,$sql_route);
				$row_route = mysqli_fetch_assoc($res_route);
				$route_name = $row_route['route_name'];
				$route=$route_name;
					}
			else if($row_id == 'RA345'){
				$customer_type = $survey_value;
					}
			else if($row_id == 'RA346'){
				$customer_code = $survey_value;
				$sql_customer = "SELECT customer_name,dns_customer_code FROM customer_master WHERE customer_code = '".$customer_code."'";
			...
			else if($row_id == 'RA347'){
				$type_of_branding =$survey_value;
			}
			else if($row_id == 'RA348'){
				$L_R = $survey_value;
			}
			else if($row_id == 'RA349'){
				$T_B = $survey_value;
			}
			else if($row_id == 'RA351'){
				$sq_ft = $survey_value;
			}
			else if($row_id == 'RA352'){
		...
		echo "<td >".$area."</td>
			...
			{
			echo "<input type=\"text\" name=\"PO_$survey_id\" id=\"PO_$survey_id\" value=\"\" /></td>";
			}
			echo "<td>".$PO_submitted_date."</td>
		  </tr>";
		  echo "<input type=\"hidden\" name=\"survey_id[]\" value=".$survey_id.">";
		}
		echo "<tr>
			<td colspan='19' align='right' height=\"30\"><input type='submit' name='SUBMIT1' value='SUBMIT' /></td>
			</tr>";
	}
	else{
