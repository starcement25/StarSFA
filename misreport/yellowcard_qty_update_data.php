...
if($yellowcard_row_check>0){
	...
	while($row_yellow_card_details = mysqli_fetch_assoc($res_yellow_card_details)){
		...
		$yellow_card_no=$row_yellow_card_details['yellow_card_no'];
		
		$sql_yc_emp_name = "SELECT emp_name FROM employee_master WHERE emp_code = '".$yc_emp_code."'";
		$res_yc_emp_name = mysqli_query($link,$sql_yc_emp_name);
		$row_yc_emp_name = mysqli_fetch_assoc($res_yc_emp_name);
		...
		echo "<td align=\"right\">".$qty."</td>
			  ...
			  <td align=\"center\"><a href=\"#\" style=\"color:blue;\" onclick=\"return edit_yellowcard_details('".$yellow_card_no."')\">Edit</a></td>
			  </tr>";
		$count++;
	}
	?>
