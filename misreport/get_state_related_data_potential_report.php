...
if($state_condition==""){
    $sql_emp = "SELECT emp_code FROM employee_master ORDER BY emp_name ASC";
}
...
if($type == 'branch'){
  ...
  else
  {
	...
	$branch_code_array = array();
	$branch_name_array = array();
	 $sql_branch = "SELECT DISTINCT branch_code FROM employee_master WHERE state IN (".$state.")".$emp_hierarchy_condition_one.$region_condition." AND branch_code != '' ".$zone_condition." ORDER BY branch_code ASC";
	 //echo"<pre>sk";print_r($sql_branch);die;
