...
//print_r($prod_groupcodename_array);	
	...
	$res_getdate = mysqli_query($link,$sql_getdate);
	while($row_getdate = mysqli_fetch_assoc($res_getdate)){
...
<?php
	...
	$res_distinct_prod = mysqli_query($link,$sql_distinct_prod);
	while($row_distinct_prod = mysqli_fetch_assoc($res_distinct_prod)){
    ...
    <?php
		foreach($product_group_name_array as $index=>$val){
			...
			while($row_getdate = mysqli_fetch_assoc($res_getdate)){
				...
				//$res_prod_quantity = mysqli_query($link,$sql_prod_quantity);
