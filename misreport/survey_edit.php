...
$res_survey_edit = mysqli_query($link,$sql_survey_edit);
while($row_survey_edit = mysqli_fetch_assoc($res_survey_edit))
{
	if($row_survey_edit['type'] != 'layer')
	{
	?>
       	<p class="text-box">
        <input type="hidden" name="row_id[]" value="<?php echo $row_survey_edit['row_id']; ?>" id="row_id<?php echo $count; ?>">
        <label for="box<?php echo $count; ?>">Info <span class="box-number">'<?php echo $count; ?>'</span></label><br><br>
        <input type="text" name="display_order[]" id="display_order<?php echo $count; ?>" value="<?php echo $row_survey_edit['display_order']; ?>" style="width:50px;"><br /><br />
         
        <input type="text" name="boxes[]"  id="box<?php echo $count; ?>" class="form-control" value="<?php echo $row_survey_edit['display_name'];?>" placeholder="Display name" onBlur="change_display_value(<?php echo $count; ?>,this.value);"/><br />
        <select name="layer_input[]" class="form-control" id="layer_input<?php echo $count; ?>">
			<?php 
            $sql_get_layer = "SELECT * FROM survey_layer";
        ...
        <?php
			if($row_survey_edit['type'] == '' || $row_survey_edit['type'] == ' ')
			{?>
        ...
        <span id="listview_div<?php echo $count; ?>" <?php if($row_survey_edit['type'] != 'radio' && $row_survey_edit['type'] != 'checkbox') echo " hidden" ?>>
            ...
            <?php } else if($row_survey_edit['type'] == 'radio'){ ?>
        ...
        <span id="action_type<?php echo $count; ?>" <?php if($row_survey_edit['action_id'] == '') echo "hidden"; ?>>
        ...
        <?php if($action_array[1] == 'checkbox') {?>
            ...
            <option value="checkbox" >checkbox</option>
            <option value="radio" selected>radio</option><?php } else{?>
            <option value="checkbox" selected>checkbox</option>
            <option value="radio">radio</option><?php } ?>
        </select>
        </span><br />
        ...
        <span id="action_mandatory<?php echo $count; ?>" <?php if($row_survey_edit['action_id'] == '') echo "hidden"; ?>>
        ...
        <?php if($action_array[2] == 'Y'){?>
            ...
            <option value="N">not mandatory</option><?php } else if($action_array[2] == 'N'){?>
            <option value="Y" >mandatory</option>
            <option value="N" selected>not mandatory</option><?php } else {?>
            <option value="Y" selected>mandatory</option>
            <option value="N">not mandatory</option><?php } ?>
        </select>
        </span>
        <br />
...
function add_action_boolean_data(k, select_option)
{
	if(select_option == 'listview')
	{
	...
	if($_REQUEST['mode']=='survey_post')
{
	...
	for($count=0;$count <count($array_box);$count++)
	{
		...
		array_push($dataArray,$array_row_id[$count]);
		array_push($dataArray, $array_display_order[$count]);
	}
	
	$survey_array_chunk = array_chunk($dataArray,15);
	...
	foreach($survey_array_chunk as $key=>$index)
	{
		...
		if($index[13] == '')
		{
			$sql_get_max_rowid = "SELECT (max(substring(row_id,3))+1) as new_row_id FROM survey_input";
			$res_get_max_rowid = mysqli_query($link,$sql_get_max_rowid);
			$row_get_max_rowid = mysqli_fetch_assoc($res_get_max_rowid);
			
			$new_row_id = $row_get_max_rowid['new_row_id'];
			
			if(strlen($new_row_id)>1)
			{
				$row_id = "RA0".$new_row_id;
				$index[13] = $row_id;
			}
			else
			{
				$row_id = "RA00".$new_row_id;
				$index[13] = $row_id;
			}
			$flag = 0;
		}
		...
		if($flag == 0)
		{
				$sql_check_display_order = "SELECT * FROM survey_input WHERE display_order = '".$index[14]."' AND layout_name = '".$index[1]."'";
				$res_check_display_order = mysqli_query($link,$sql_check_display_order);
				$total_rows = mysqli_num_rows($res_check_display_order);
				if($total_rows>0)
				{
					$sql_reset_display_order = "UPDATE survey_input SET display_order = (display_order+1) WHERE layout_name = '".$index[1]."' AND display_order>='".$index[14]."'";
					$res_reset_display_order = mysqli_query($link,$sql_reset_display_order);
				}
		...
		{
				$sql_select_survey_input = "SELECT * FROM survey_input WHERE row_id = '$index[13]' AND action_id = '$action_id' AND layout_name = '$index[1]' AND display_name = '$display_name' AND display_order = '$index[14]' AND type = '$type' AND display_table_name = '$display_table_name' AND mandatory = '$mandatory' AND validation = '$validation' AND `action` = '$action_value'";
				$res_select_survey_input = mysqli_query($link,$sql_select_survey_input);
				$select_total_rows = mysqli_num_rows($res_select_survey_input);
				if($select_total_rows == 0)
				{
					$sql_insert_survey_input = "UPDATE survey_input SET 
												 action_id = '".$action_id."', 
											   layout_name = '".$index[1]."', 
											  display_name = '".$display_name."',
											 display_order = '".$index[14]."', 
