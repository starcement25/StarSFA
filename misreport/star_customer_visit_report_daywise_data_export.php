...
$sql_office_plant = "SELECT customer_code FROM office_plant_master";
	...
	$header = "Date of Visit"."\t"."Customer Code"."\t"."Customer Name"."\t"."Route"."\t"."Type"."\t"."Branch"."\t"."Employee Code"."\t"."Employee Name"."\t"."Check In Time"."\t"."Check Out Time"."\t"."Duration"."\t"."Visit Status(Productive/Non productive)"."\t"."Remarks";
	...
	$sql_customer = "SELECT * FROM(SELECT DISTINCT CVD.emp_code,DATE_FORMAT(SUBSTRING(CVD.trans_id,-14,8),'%Y-%m-%d') AS visit_date,SUBSTRING(CVD.trans_id,-14,14) AS visit_date_parts,CVD.customer_code,CVD.customer_name,CVD.route_name,CVD.route_code,
					...
					CVD.cust_type,EM.dns_emp_code,EM.emp_name,CM.dns_customer_code,BM.branch_name,CVD.trans_id,CVD.hint_remarks 
	...
	if($total_customer>0){
		...
		while($row_customer = mysqli_fetch_assoc($res_customer)){
			...
			$table_data .= $visit_date."\t".$dns_customer_code."\t".$customer_name."\t".$route_name."\t".$cust_type."\t".$branch_name."\t".$dns_emp_code."\t".$emp_name."\t".$check_in_time."\t".$check_out_time."\t".$time_duration."\t".$visit_status."\t".$remark."\n";
