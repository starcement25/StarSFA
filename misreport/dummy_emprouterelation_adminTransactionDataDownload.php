                            ...
                            <tr>
								...
								?>	 
                                    ...
                                    <option value="0">SELECT</option>
									 ...
									 $resultrds = mysqli_query($link,$sqlqueryrds);
									 $countrds=mysqli_num_rows($resultrds);
									 if($countrds>0){
									while($rowrds = mysqli_fetch_assoc($resultrds))
									{
    ...
    function file()
    {
				 ...
				 //For Transaction csv download
					...
					if(modified_customer_emp_route == 'yes'){
						$sql_order_header_download = "SELECT DISTINCT BM.branch_name,RM.rds_name,EM.emp_name,EM.emp_code,
											DATE_FORMAT(SUBSTRING(LO.trans_id,-14,8),'%d-%m-%Y') AS transaction_date,
											...
											OH.transaction_type,OH.d_instruction,RM.rds_code,emp_route_relation ERR FROM order_details OD,customer_master CM,product_master PM, 
											...
											CM.rds_tag=RM.rds_code AND FIND_IN_SET(BM.branch_code,CM.branch_code) AND ERR.route_code=CM.route_code AND SUBSTRING(OH.order_no,2,5)=EM.emp_code AND CM.emp_code=EM.emp_code AND LO.trans_id=OH.order_no 
					...
					$sql_order_header_download .= " UNION ALL
									...
									AS transaction_date,OD.order_no, 
		...
		//exit();
			...
			{ 
				...
				${valueorderheader.$rds_code} .=${VAT.$rds_code}.",";
			...
			for($k=0;$k<count($emp_code_array);$k++)
			{
				$sql_query_order_header_SA="SELECT BM.branch_name,RM.rds_name,EM.emp_name,DATE_FORMAT(SUBSTRING(LO.trans_id,-14,8),'%d-%m-%Y') 
										...
										AND SUBSTRING(OH.order_no,2,5)=EM.emp_code AND OH.transaction_type='SA' 
				...
				$sql_query_freight="SELECT BM.branch_name,RM.rds_name,EM.emp_name,DATE_FORMAT(FE.date,'%d-%m-%Y') AS transaction_date,
									...
									AND EM.branch_code=BM.branch_code AND RM.emp_code=EM.emp_code AND FE.downld_transferred='NO'";
