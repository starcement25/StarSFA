...
if(stk_audit=='yes' || retailer_care=='yes'){
	$stk_audit_customer_TR='<td  width="8%" align="left" style="padding-left:20px;">No of Stock Audit</td>';
	$stk_audit_customer_TR_secondary='<td width="8%" align="left" style="padding-left:20px;"></td>';
}
else
{
...
/*----> Total Calls & Days Present <----*/
	...
	SUM(CASE WHEN POCM.order_no LIKE 'O%' AND POCM.cust_type='R' THEN POCM.amount ELSE 0 END) AS secondary_amount FROM location LO LEFT JOIN `order_header` OH ON OH.order_no=LO.trans_id LEFT JOIN order_details OD ON OD.order_no=LO.trans_id WHERE SUBSTRING(LO.trans_id,1,1) IN('A','N','O') AND SUBSTRING(LO.trans_id,-14,4)='".$year."' AND SUBSTRING(LO.trans_id,-10,2)='".$monthvalue."' ".$employee_select_condition." GROUP BY LO.emp_code";
		...
		{
	$sql_total_calls = "SELECT EM.emp_code,COUNT(CASE WHEN LO.trans_id LIKE 'A%' THEN 1 END) AS days_present,
	SUM(CASE WHEN LO.trans_id LIKE 'O%' THEN 1 ELSE 0 END ) AS total_order_count,
	COUNT(CASE WHEN POCM.order_no LIKE 'O%' THEN POCM.order_no END ) AS order_details_count,
	COUNT( DISTINCT (CASE WHEN POCM.order_no LIKE 'O%' THEN CONCAT(POCM.customer_code,'^',SUBSTRING(POCM.order_no,-14,8)) END )) AS productive_calls, 
	...
	SUM(CASE WHEN POCM.order_no LIKE 'O%' AND POCM.cust_type='D' THEN POCM.amount ELSE 0 END) AS primary_amount,
	SUM(CASE WHEN POCM.order_no LIKE 'O%' AND POCM.cust_type='R' THEN POCM.amount ELSE 0 END) AS secondary_amount,EM.designation,EM.emp_name,EM.acedns 
	FROM employee_master EM LEFT JOIN location LO ON LO.emp_code=EM.emp_code AND SUBSTRING(LO.trans_id,1,1) IN('A','N','O') 
	AND SUBSTRING(LO.date,1,10) ='".$start_date."' LEFT JOIN `prev_order_counting_master` POCM ON POCM.order_no=LO.trans_id 
	WHERE 
	".$employee_hierarchy_condition." GROUP BY EM.emp_code ORDER BY FIELD(EM.emp_code, $employee_hierarchy_val)";
		...
		if($total_rows>0){
			?><table width='100%' class='border' border='1' style='border-collapse:collapse;' cellpadding='6px'>
             <tr class='TDHEAD'><td colspan='20' align='center'>Daily Activity Analysis - <?php echo date('d-m-Y',strtotime($start_date));?></td></tr>
              <tr class="TDHEAD_SUB">
                <td>SL No</td>
                <td>Emp Name</td>
                <td>Designation</td>
                ...
                <td>NPC</td>
                <td>LPPC</td>
                <?php echo $table_column.$stk_audit_customer_TR;?>
              </tr>
              <tr class="TDHEAD_SUB">
                <td></td>
                <td></td>
                ...
                <?php echo $secondary_row.$secondary_row.$stk_audit_customer_TR_secondary;?>
              </tr>
            <?php
			$res_total_calls = mysqli_query($link,$sql_total_calls);
			$loop_count=0;
			while($row_total_calls = mysqli_fetch_assoc($res_total_calls)){
				...
				$total_order_count = $row_total_calls['total_order_count'];
				$acedns = $row_total_calls['acedns'];
				if($acedns=='N') $color_name='#FF0000';
				else 			 $color_name='';
				$attendance='ABSENT';
				if($days_present==1)
				{
					$sql_attendance = "SELECT SUBSTRING(date,12) AS attendance FROM location WHERE emp_code = '".$emp_code."' AND 
									SUBSTRING(date,1,10) ='".$start_date."'";
					$res_attendance = mysqli_query($link,$sql_attendance);
					$row_attendance = mysqli_fetch_assoc($res_attendance);
				    $attendance = $row_attendance['attendance'];
				}
				if($acedns=='Y' && $attendance=='ABSENT') $color_name_absent='#FF0000';
				else 			 $color_name_absent='';		
				...
				if($prod_call == 0 && $non_prod_call == 0){
					$color_name = 'FF0000';
					$non_active++;
				}
				else{
				...
				$LPPC=0;
				$order_details_count = $row_total_calls['order_details_count'];
				$LPPC = $order_details_count/$prod_call;
				//$LPPC_total += $LPPC;
			
				$total_order_header_count += $prod_call;
				...
				if(stk_audit=='yes' || retailer_care=='yes'){
				$stk_audit_customer_TD="<td align=\"right\" >".$rowstockaudit['total_stk_audit']."</td>";
				$total_stk_audit=$total_stk_audit+$rowstockaudit['total_stk_audit'];
				}
				...
				${total_order_details_count_RSM.$rsm_emp_code} += $order_details_count;
				${total_non_productive_call_RSM.$rsm_emp_code} += $non_prod_call;
				${total_call_RSM.$rsm_emp_code} +=$total_calls;
				${total_stk_audit_RSM.$rsm_emp_code} +=$rowstockaudit['total_stk_audit'];
				...
				for($i=0;$i<2;$i++)
				{
					foreach($verticalarray as $verticalvalfinal)
					{
						$sqlverticaltotal="SELECT SUM(CASE WHEN POCM.cust_type='D' AND POCM.order_no LIKE 'O%' THEN POCM.amount ELSE 0 END ) AS primary_amount, 
											...
											WHERE POCM.product_code=PM.prod_code AND SUBSTRING(POCM.visit_date,1,10)='".$start_date."' AND 
											SUBSTRING(POCM.order_no,2,5)='".$emp_code."' AND PM.vertical_value='".$verticalvalfinal."'";
				...
				if(($designation=='RSM' && $count > 1))
				{
					...
					<td align='right'>".${total_non_productive_call_RSM.$previous_rsm_emp_code}."</td>
					<td align='right'>".round((${total_order_details_count_RSM.$previous_rsm_emp_code}/${total_productive_call_RSM.$previous_rsm_emp_code}),2)."</td>".
					${table_column_data_primary.$previous_rsm_emp_code}.${table_column_data_secondary.$previous_rsm_emp_code}.
					"<td align=\"right\" >".${total_stk_audit_RSM.$previous_rsm_emp_code}."</td>
				  </tr>";
				}
				/*if($designation=='RSM')
				{
				...
				echo "<tr>
						...
						<td align=\"right\">".$non_prod_call."</td>
						<td align=\"right\">".number_format($LPPC,2)."</td>".$table_column_data_primary.$table_column_data_secondary.$stk_audit_customer_TD."
					  </tr>";
