...
$rscustomer=mysqli_query($link,$sqlcustomer) or die(mysqli_error()." Error in select rds name : ".$sqlcustomer);	
...
<table width="96%" align="center" border="0" cellpadding="5" cellspacing="2" class="border" style="height: 150px;overflow-y: scroll;">
   ...
   <tr class="TDHEAD_SUB"> 
		...
		$rsauditdetails=mysqli_query($link,$sqlauditdetails) or die(mysqli_error()." Error in select audit product details: ".$sqlauditdetails);
		
		 $cnt=$GLOBALS[start]+1;
        while($rowauditdetails=mysqli_fetch_assoc($rsauditdetails))
        {
