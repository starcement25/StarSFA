...
<br /><br />
<div id="display" style="max-height: 350px; width:55%; overflow-y: scroll; margin-left:10px;" align="center"></div>
<br><br>
<div id="collection" style="max-height: 350px; width:55%; overflow-y: scroll; margin-left:10px;" align="center"></div>
<br><br>
<div id="view" hidden></div>
<div style="width:60%; margin-left:10px;">
Today:<input type="radio" name="duration" value="today" id="today" <?php if($type == 'today') echo "checked"; ?> onClick="hide_date_div(); show_employeewise();" />
...
function show_employeewise(){
	...
	if(type != 'custom'){
		GenericAjaxFunction('collection_report_emp_data.php?type='+type+'&emp_name='+emp_name,'display',0);
	}
	else{
		if(response1 != 0 && response2 != 0)
			GenericAjaxFunction('collection_report_emp_data.php?type='+type+'&start_date='+start_date+'&end_date='+end_date+'&emp_name='+emp_name,'display',0);
	}
}
...
function PrintElem(elem)
{
	...
	var collectiondiv = document.getElementById("collection").innerHTML;
	var view = displaydiv+'<br>'+collectiondiv
	Popup(view);
   //Popup($(elem).html());
}
...
function Popup(data) 
{
	var mywindow = window.open('', 'Collection', 'height=400,width=600');
	mywindow.document.write('<html><head><title>Collection</title>');
...
function exporttocsv(divid)
{
	//alert(divid);
        ...
        a.download = 'Collection' + postfix + '.xls';
...
function show_invoice(receipt_id,customer_name){
	//alert(receipt_id);
	//var customer_name = encodeURI(customer_name);
	document.getElementById("collection").innerHTML = '<img src="ajax-loader.gif" id="ajaxloader">';
	GenericAjaxFunction('collection_report_invoice_data.php?receipt_id='+receipt_id+'&customer_name='+customer_name,'collection',0);
}
</script>
<?php
}
