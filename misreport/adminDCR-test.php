...
{	
	...
	function updateLatlong($emp_code)
	{
		...
		if($countselectlatlong>0)
		{
			...
			$rslastlatlong=mysqli_query($link,$sqlupdatelatlong) or die(mysqli_error()." Error in update zero latt longi: ".$sqlupdatelatlong);
		}
	}
	/*if(DCR_map=='no')
	{
	...
	$rowemployee=mysqli_fetch_assoc($rsemployee);
	 ...
	 if(check_in_out=='no')
	  {
		...
		if($countpoint>0)
		{
			...
			if(DCR_map=='yes'){	
			$body='<img src="https://maps.googleapis.com/maps/api/staticmap?size=700x600&path=color:0xff0000ff|weight:2|'.$pathpoints.'&sensor=false&'.$markers.'&key=AIzaSyAC5XJHC0k1ALyl5Bnelv3Nvuxpzr9nLdc" alt=""><br /> <br />';
			}
			...
			/*$sqlpoint="SELECT latt, longi,trans_id,DATE_FORMAT(date,'%H:%i:%s') as trantime FROM location WHERE emp_code ='".$emp_code."' AND 
			DATE LIKE '%".$dateprevious."%' ORDER BY DATE_FORMAT(DATE,'%Y-%m-%d %H:%i:%s') ASC ";*/
		$sqlpoint="(SELECT LO.latt, LO.longi,LO.trans_id,DATE_FORMAT(LO.date,'%H:%i:%s') as trantime,CIO.customer_code,CIO.check_in_time,
			...
			while($rowpoint=mysqli_fetch_assoc($rspoint))
			{
				...
				if($operation_type=='C')
				{
					...
					if(!in_array($customer_code,$check_in_out_customer_array))
					{
						...
						$sql_payment_details = "SELECT SUM(amount) as collection_amount,p_remark FROM payment_header WHERE customer_code = '".$customer_code."' 
				...
				if($operation_type=='O')
					...
					if($operation_type=='P')
					{
					   ...
					   $rschkpayment=mysqli_query($link,$sqlchkpayment);
					   $cntchkpayment=mysqli_num_rows($rschkpayment);
					   if($cntchkpayment > 0)
					   {
