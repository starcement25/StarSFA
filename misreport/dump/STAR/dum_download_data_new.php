...
if($attribute=='branch_wise_competitor'){
	...
	//exit();
	if($total_branch_competitor > 0){
		$header_branch_competitor = "COMPANY NAME".","."Product Name in SFA".","."Branch Code".","."Branch Name".","."Active (yes)".","."PRODUCT TYPE";
		//$res_prev_order_counting_master = mysql_query($sql_prev_order_counting_master);
		while($row_branch_competitor= mysql_fetch_array($res_branch_competitor)){
			$dns_branch_code = $row_branch_competitor['dns_branch_code'];
			$competitor_name = $row_branch_competitor['competitor_name'];
		...
		$fp = fopen("/home/acedns/public_html/misreport/dump/$foldername/BRANCH_WISE_COMPETITORS1.csv","wb");
		fwrite($fp,$datacontents);
		fclose($fp);
		echo "$foldername/BRANCH_WISE_COMPETITORS1.csv";
		//header("Content-type: application/octet-stream");
		//header("Content-Disposition: attachment; filename=Order_Download_Report.xls");
		//print "$datacontents";
		//$nick_name=strtoupper($_SESSION['nick_name']);
  ...
  if(modified_customer_emp_route=='yes'){
        ...
        $sql="SELECT DISTINCT CM.dns_customer_code,
				  ...
				  (SELECT route_no FROM route_master WHERE route_code = CRER.route_code) AS route_no,
				   ...
				   CM.pin,
				   CM.base_latt,
				   CM.base_longi,CM.retailer_app,CM.is_new_customer,CM.category_of_store,CM.is_nlp,CM.cluster FROM customer_master CM INNER JOIN customer_route_emp_relation CRER
 ...
 $res=mysql_query($sql);
  ...
  if($total_customer > 0){
	  $dns_customer_code_array=array();
		...
		"Email".","."Visit Day".","."State".","."Monthly Potential".","."Sauda limit".","."Incoterms".","."Loadability ton".","."Transport mode".","."Sauda type".","."Zone".","."Visit sequence".","."Appointment date".","."date_of_birth".","."date_of_anniversary".","."whatsapp_no".","."beneficiary_name".","."IFS_code".","."pin".","."Base lattitude".","."Base longitude".","."Retailer app".","."New Customer".","."Category of store".","."NLP".","."Area".","."Route No".","."Cluster";
		}
		//$res_prev_order_counting_master = mysql_query($sql_prev_order_counting_master);
		if(strtoupper($foldername)=='STAR')
		{
		...
		while($row_customer = mysql_fetch_array($res)){
		    ...
		    if(strtoupper($foldername)=='STAR')
			{
				...
				if(in_array('Y',$acednsparts))
				{
					...
					$base_latt.",".$base_longi.",".$retailer_app.",".$is_new_customer.",".$category_of_store.",".$is_nlp.",".$area.",".$route_no.",".$cluster.",".'Y'."\n";
				}
				else
				{
					$contents.=$dns_customer_code.",".$customer_name.",".$phone_no.",".$route_code.",".$route_name.",".$emp_code_name.",".$acedns.",".$credit_limit.",".$credit_days.",".$current_balance.",".$black_list.",".$TD.",".$branch_code.",".$cust_type.",".$rds_tag.",".$sauda_validity_period.",".$address.",".$landline_no.",".$owner_name.",".$owner_phone.",".$cust_class.",".$weekly_closing_day.",".$coverage_type.",".$TIN.",".$PAN.",".$district.",".$minimum_stock.",".$bank_name.",".$bank_account_number.",".$email.",".$visit_day.",".$state_code.",".$monthly_potential.",".$sauda_limit.",".$incoterms.",".$loadability_ton.",".$transport_mode.",".$sauda_type.",".$zone.",".$visit_sequence.",".$appointment_date.",".$date_of_birth.",".$date_of_anniversary.",".$whatsapp_no.",".$beneficiary_name.",".$IFS_code.",".$pin.","
					.$base_latt.",".$base_longi.",".$retailer_app.",".$is_new_customer.",".$category_of_store.",".$is_nlp.",".$area.",".$route_no.",".$cluster.",".'N'."\n";
				}
			}
			...
			{
				$contents.=$dns_customer_code.",".$customer_name.",".$phone_no.",".$route_code.",".$route_name.",".$emp_code_name.",".$acedns.",".$credit_limit.",".$credit_days.",".$current_balance.",".$black_list.",".$TD.",".$branch_code.",".$cust_type.",".$rds_tag.",".$sauda_validity_period.",".$address.",".$landline_no.",".$owner_name.",".$owner_phone.",".$cust_class.",".$weekly_closing_day.",".$coverage_type.",".$TIN.",".$PAN.",".$district.",".$minimum_stock.",".$bank_name.",".$bank_account_number.",".$email.",".$visit_day.",".$state_code.",".$monthly_potential.",".$sauda_limit.",".$incoterms.",".$loadability_ton.",".$transport_mode.",".$sauda_type.",".$zone.",".$visit_sequence.",".$appointment_date.",".$date_of_birth.",".$date_of_anniversary.",".$whatsapp_no.",".$beneficiary_name.",".$IFS_code.",".$pin.",".$base_latt.",".$base_longi.",".$retailer_app.",".$is_new_customer.",".$category_of_store.",".$is_nlp.",".$area.",".$route_no.",".$cluster."\n";
			}
...
if($attribute=='site'){
	...
	$total_site = mysql_num_rows($res_site);
	//exit();
	if($total_site > 0){
		...
		//$res_prev_order_counting_master = mysql_query($sql_prev_order_counting_master);
		while($row_site = mysql_fetch_array($res_site)){
...
if($attribute=='TA_DA_Master'){
	...
	$res_TA_DA = mysql_query($sql_TA_DA);
	$total_TA_DA = mysql_num_rows($res_TA_DA);
	//exit();
	if($total_TA_DA > 0){
		$header_TA_DA = "Employee Code".","."Employee Name".","."Per KM TA Amt".","."DA HQ".","."DA EX HQ".","."DA Outstation";
		//$res_prev_order_counting_master = mysql_query($sql_prev_order_counting_master);
		while($row_TA_DA = mysql_fetch_array($res_TA_DA)){
...
if($attribute=='PJP'){
	...
	$res_pjp= mysql_query($sql_pjp);
	$total_pjp = mysql_num_rows($res_pjp);
	//exit();
	if($total_pjp > 0){
		$header_pjp = "State".","."Date".","."Day".","."Employee Name".","."Area".","."Route Name".","."Route No.".","."Objective";
		//$res_prev_order_counting_master = mysql_query($sql_prev_order_counting_master);
		while($row_pjp = mysql_fetch_array($res_pjp)){
...
if($attribute=='Target'){
	...
	$res_target= mysql_query($sql_target);
	$total_target = mysql_num_rows($res_target);
	//exit();
	if($total_target > 0){
		...
		while($row_target = mysql_fetch_array($res_target)){
...
if($attribute=='dealer_target'){
	...
	if($total_target > 0){
		...
		$fp = fopen("/home/acedns/public_html/misreport/dump/$foldername/Star Dealer Target.csv","wb");
		fwrite($fp,$datacontentstarget);
		fclose($fp);
...
if($attribute=='vendor_details'){
	...
	$res_vendor_list = mysql_query($sql_vendor_list);
	$total_vendor_list = mysql_num_rows($res_vendor_list);
	//exit();
	if($total_vendor_list > 0){
		...
		//$res_prev_order_counting_master = mysql_query($sql_prev_order_counting_master);
		while($rowvendordetails = mysql_fetch_array($res_vendor_list)){
		...
		$fp = fopen("/home/acedns/public_html/misreport/dump/$foldername/OOH Site Master.csv","wb");
...
if($attribute=='warehouse_master'){
	...
	$res_warehouse = mysql_query($sql_warehouse);
	$total_warehouse = mysql_num_rows($res_warehouse);
	//exit();
	if($total_warehouse > 0){
		...
		//$res_prev_order_counting_master = mysql_query($sql_prev_order_counting_master);
		while($row_warehouse = mysql_fetch_array($res_warehouse)){
		...
		$fp = fopen("/home/acedns/public_html/misreport/dump/$foldername/warehouse master.csv","wb");
		fwrite($fp,$datacontentsweightage);
		fclose($fp);
...
if($attribute=='pop_product'){
	...
	$res_pop_product = mysql_query($sql_pop_product);
	$total_pop_product = mysql_num_rows($res_pop_product);
	//exit();
	if($total_pop_product > 0){
		$header_pop_product = "Dns Prod Code".","."Prod Desc".","."Prod Image".","."Min order qty".","."Price per piece".","."Gst rate".","."Status";
		//$res_prev_order_counting_master = mysql_query($sql_prev_order_counting_master);
		while($row_pop_product = mysql_fetch_array($res_pop_product)){
			...
			$contents_pop_product.=$dns_prod_code.",".$prod_desc.",".$prod_image.",".$min_order_qty.",".$price_per_piece.",".$GST_rate.",".$status."\n";
			$count++;
		}
		...
		$fp = fopen("/home/acedns/public_html/misreport/dump/$foldername/pop product master.csv","wb");
...
if($attribute=='branch_pop_product'){
	...
	if($total_pop_product_br > 0){
		...
		//$res_prev_order_counting_master = mysql_query($sql_prev_order_counting_master);
		while($row_pop_product_br = mysqli_fetch_array($res_pop_product_br)){
		...
		$fp = fopen("/home/acedns/public_html/misreport/dump/$foldername/branch pop product.csv","wb");
