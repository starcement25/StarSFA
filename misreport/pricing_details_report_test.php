...
function Popup(data) 
{
	var mywindow = window.open('', 'Oil Loose Rate Report', 'height=400,width=600');
	mywindow.document.write('<html><head><title>Oil Loose Rate Report</title>');
...
<center>
<br /><br />
<div id="display" style="max-height: 300px; width:45%; overflow-y: scroll; margin-left:10px;" align="center"></div>
...
<input name="click" id="click" type="button" value="click" onClick="append_data();">
...
function show_saudawise()
{
	...
	if(type != 'custom')
		GenericAjaxFunction('pricing_details_data_test.php?type='+type,'display',0);
	else 
	{
		if(response1 != 0 && response2 != 0)
			GenericAjaxFunction('pricing_details_data_test.php?type='+type+'&start_date='+start_date+'&end_date='+end_date,'display',0);
	}
}

function append_data(){
	alert('Hi');
	//$("#head_main").fadeIn(1000);
	$("#head_main").clone().prependTo("#display");
}
</script>
<?php
}
