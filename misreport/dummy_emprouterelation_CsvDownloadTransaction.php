...
if($branch_name!='' || $rds_name!='')
{
	...
	if($cntbranch >0 || $cntrds >0)
	{
		...
		if(modified_customer_emp_route == 'yes'){
			$sql_order_header_download = "SELECT DISTINCT BM.branch_name, EM.emp_name, EM.emp_code,DATE_FORMAT(SUBSTRING(LO.trans_id,-14,8),'%d-%m-%Y') AS transaction_date,LO.trans_id, OD.order_no,CM.customer_name,PM.prod_desc, OD.qty, OD.sale_rate,(OD.qty*OD.sale_rate) AS Amount, OD.amount AS input_amount,OH.VAT, OH.transaction_type,OH.d_instruction, RM.rds_code FROM order_details OD,customer_master CM,product_master PM, order_header OH,branch_master BM,employee_master EM,location LO, emp_route_relation ERR, rds_master RM WHERE OH.customer_code = CM.customer_code AND OD.sku_code = PM.prod_code and OD.order_no = OH.order_no AND SUBSTRING(OH.order_no,2,1)!='C' AND CM.rds_tag=RM.rds_code AND FIND_IN_SET(BM.branch_code,CM.branch_code) AND ERR.route_code=CM.route_code AND LO.emp_code=EM.emp_code AND LO.trans_id=OH.order_no AND PM.branch_code=BM.branch_code ".$condition." AND OH.downld_transferred='NO'";
		}
		else{
		...
		for($k=0;$k<count($emp_code_array);$k++)
		{
			...
			$rs_order_header_SA = mysqli_query($link,$sql_query_order_header_SA) or die(mysqli_error()." Error in order header SA download: ".$sql_order_header_download);
						
			while($rec_order_header_SA = mysqli_fetch_assoc($rs_order_header_SA)) 
			{ 
				...
				$rsupdateorderheader=mysqli_query($link,$sqlupdateorderheader)or die(mysqli_error()." Error in order header SA update: ".$sqlupdateorderheader);
			}
			...
			$rs_query_freight = mysqli_query($link,$sql_query_freight) or die(mysqli_error()." Error in freight download: ".$sql_query_freight);
						
			while($rec_query_freight = mysqli_fetch_assoc($rs_query_freight)) 
			{ 
				...
				$rsupdatefreight=mysqli_query($link,$sqlupdatefreight)or die(mysqli_error()." Error in  freight update: ".$sqlupdatefreight);
			}
		}
		$zip = new ZipFile();
