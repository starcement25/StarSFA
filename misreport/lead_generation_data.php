...
$sql_distinct_date = "SELECT DISTINCT survey_id,value, SUBSTRING(survey_id,3,5) AS emp_code,DATE_FORMAT(SUBSTRING(survey_id,-14,8),'%d-%m-%Y') AS survey_date,
					...
					AND SUBSTRING(survey_id,3,5) IN(".$employee.")  AND type = 'Lead Generation' AND row_id='RA486'  
...
if($total_rows>0){
    ...
    <table border="1" style="border-collapse:collapse;" class="border" width="90%">
      ...
      <tr class="TDHEAD">
        <td width="4%">Visit_Date</td>
        <td width="4%">Visit ID</td>
        <td width="3%">Employee Code</td>
        <td width="3%">Employee Name</td>
         <td width="3%">Party Name</td>
        <td width="3%">Site</td>
        <td width="3%">District</td>
        <td width="3%">STATE</td>
        <td width="3%">QTY REQ (MT)</td>
        <td width="3%">PRODUCT+PACKAGING</td>
        <td width="3%">Expected Rate Per Bag</td>
        <td width="3%">Contact person name</td>
        <td width="3%">Contact number</td>
        <td width="3%">Mail Id</td>
        <td width="3%">Mode</td>
        <td width="3%">Quotation</td>
        <td width="3%">PO</td>
        <td width="3%">Rejected</td>
        <td width="3%">Remarks</td>
        <td width="3%">Assigned to</td>
      </tr>
    <?php
	while($row_survey_ouput = mysqli_fetch_assoc($res_distinct_date)){
		...
		while($row_survey_details = mysqli_fetch_assoc($res_survey_details)){
			...
			if($row_id == 'RA486'){
				$party_name=$survey_value;	
			}
			else if($row_id == 'RA487'){
				$SITE  = $survey_value;
			}
			else if($row_id == 'RA488'){
				$DISTRICT =$survey_value;
			}
			else if($row_id == 'RA489'){
				$STATE = $survey_value;
			}
			else if($row_id == 'RA490'){
				$QTY_REQ  = $survey_value;
			}
			else if($row_id == 'RA491'){
				$product_packaging = $survey_value;
			}
			else if($row_id == 'RA492'){
				$expected_rate_bag = $survey_value;
			}
			else if($row_id == 'RA493'){
				$contact_person_name = $survey_value;
			}
			else if($row_id == 'RA494'){
				$contact_number = $survey_value;
			}
			else if($row_id == 'RA495'){
				$mail_id = $survey_value;
			}
			else if($row_id == 'RA496'){
				$mode = $survey_value;
			}
			else if($row_id == 'RA497'){
				$quotation = $survey_value;
			}
			else if($row_id == 'RA498'){
				$PO = $survey_value;
			}
			else if($row_id == 'RA499'){
				$rejected = $survey_value;
			}
			else if($row_id == 'RA500'){
				$remarks = $survey_value;
			}
			else if($row_id == 'RA501'){
				$assigned_to = $survey_value;
			}
		}
		echo "<td>".$party_name."</td>
			...
			<td >".$STATE."</td>
			<td >".$QTY_REQ."</td>
			<td >".$product_packaging."</td>
			<td >".$expected_rate_bag."</td>
