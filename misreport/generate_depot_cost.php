...
<form action="generate_depot_cost.php" name="depot_cost"  method="post" onsubmit="return validation();">
	   ...
	   if(flagplant==false)
		...
		for(i=0; i<document.depot_cost.elements.length; i++){
			if(document.depot_cost.elements[i].type=="checkbox" && document.depot_cost.elements[i].checked==true 
					&& document.depot_cost.elements[i].name=='product_group_code[]'){
				is_checked=true;
				break;
			}
		}
...
<?php
	if($_REQUEST['mode']=='generate_depot_cost')
	{
		...
		$sqlselectdistinctbranch="SELECT DISTINCT branch_code FROM product_master WHERE product_group_code IN(".$product_group_code.") 
									AND branch_code IN(".$branch_code.")";
		...
		{ 
			 ...
			 $sqlinsertdeptcostlog="INSERT INTO depot_cost_log 
								...
								datetime=CURRENT_TIMESTAMP";
			mysqli_query($link,$sqlinsertdeptcostlog);
		?><script language="JavaScript" type="text/javascript">alert('Depot cost generated successfully.');window.location.href='generate_depot_cost.php';</script>
		<?php }else{
			?><script language="JavaScript" type="text/javascript">alert('Depot cost generation unsuccessful.');window.location.href='generate_depot_cost.php';</script>
		<?php
		}
	}
}
