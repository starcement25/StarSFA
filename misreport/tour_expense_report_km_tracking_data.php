...
if($total_row_check>0){
	...
	while($row_emp_code = mysqli_fetch_assoc($res_emp_code)){
        ...
        <?php
			...
			$latest_tour_date=date('d-m-Y',strtotime($row_tour_details_latest['tour_date']));

		$sql_tour_details = "SELECT LO.latt,LO.longi,LO.trans_id,LO.TA_DA_mode,SUBSTRING(LO.date,1,10) AS tour_date FROM location LO 
							WHERE  LO.emp_code = '".$emp_code."' AND 
							(SUBSTRING(LO.date,1,10) >= '".$start_date."' AND SUBSTRING(LO.date,1,10) <='".$end_date."') 
							AND (LO.TA_DA_mode='OWN'  OR LO.trans_id LIKE 'A%')
		...
		while($row_tour_details = mysqli_fetch_assoc($res_tour_details)){
			...
			if(rtrim(${att_activities.${prev_tour_date.$emp_code}})=='HQ,') 			${DA_tot.${prev_tour_date.$emp_code}}=$DA_HQ;
			if(rtrim(${att_activities.${prev_tour_date.$emp_code}})=='EX HQ,') 	   ${DA_tot.${prev_tour_date.$emp_code}}=$DA_EX_HQ;
			
			$total_amount=$TA_tot+${DA_tot.${prev_tour_date.$emp_code}};
			...
			echo "<tr>
				...
				echo"<td>".$daily_assistance."</td>
					...
					{
						${latitudeTo.$emp_code.$tour_date}=$row_tour_details['latt'];
						//echo '<br />';
						${longitudeTo.$emp_code.$tour_date}=$row_tour_details['longi'];
						
						$distance=round((haversineGreatCircleDistance(${latitudeFrom.$emp_code.$tour_date}, ${longitudeFrom.$emp_code.$tour_date}, ${latitudeTo.$emp_code.$tour_date}, ${longitudeTo.$emp_code.$tour_date}, $earthRadius = 6371000))/1000,2);
						${distancefinal.$emp_code.$tour_date}=${distancefinal.$emp_code.$tour_date}+$distance;
			...
			if(rtrim(${att_activities.$latest_tour_date})=='HQ,') 		   ${DA_tot.$latest_tour_date}=$DA_HQ;
			if(rtrim(${att_activities.$latest_tour_date})=='EX HQ,') 	   ${DA_tot.$latest_tour_date}=$DA_EX_HQ;
			
			$total_amount=$TA_tot+${DA_tot.$latest_tour_date};
