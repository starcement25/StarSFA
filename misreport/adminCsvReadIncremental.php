...
function csv_upload(){
...

	{
		...
		//sk add condition start 09-09-25
        $baseName = pathinfo($file_name, PATHINFO_FILENAME); 
        $csvName  = $baseName . ".csv";
        $csvPath  = $upload_dir . $csvName;
		...
		//sk end add condition start 09-09-25
	 }
	...
	//echo"<pre>";print_r($_FILES['zip_file']['name']);die;
	if(similar_file_exists("../csv/$folderName/Branch master.csv")!=false && $_FILES['zip_file']['name']=='Branch master.zip')
		...
		if($double_coute_found && $char=="\"")
		{
...

			{
			  ...
			  //echo"<pre>";print_r($data);die;
				...
				$dns_branch_code = preg_replace('/[[:^print:]]/', '', $dns_branch_code);

				$branch_name=trim($data[1]);
				$branch_name = preg_replace('/[[:^print:]]/', '', $branch_name);

				$branch_location=trim($data[2]);
				$branch_location = preg_replace('/[[:^print:]]/', '', $branch_location);

				$comp_code=trim($data[3]);
				$comp_code = preg_replace('/[[:^print:]]/', '', $comp_code);

				$branch_state=trim($data[4]);
				$branch_state = preg_replace('/[[:^print:]]/', '', $branch_state);

				$branch_email_id=trim($data[5]);
				$branch_email_id = preg_replace('/[[:^print:]]/', '', $branch_email_id);

				$branch_accounts_email_id=trim($data[6]);
				$branch_accounts_email_id = preg_replace('/[[:^print:]]/', '', $branch_accounts_email_id);

				$alternative_email_id=trim($data[7]);
				$alternative_email_id = preg_replace('/[[:^print:]]/', '', $alternative_email_id);
...

				{
					...
					$sqlbranch .= " , dns_state_code='".$dns_state_code."'";
				...
				//echo"<pre>";print_r($sqlbranch);die;
				//echo $sqlbranch;
...

	{
		...
		if(similar_file_exists("../csv/$folderName/Sku master.csv")!=false && $_FILES['zip_file']['name']=='Sku master.zip')
           ...
           /* if (($handle = fopen($filename, "r")) !== false) {
...

			{
...

				{
					...
					$branch_code_name = preg_replace('/[[:^print:]]/', '', $branch_code_name);
					...
					$dns_prod_code = preg_replace('/[[:^print:]]/', '', $dns_prod_code);

					$prod_desc=trim($data[2]);
					$prod_desc = preg_replace('/[[:^print:]]/', '', $prod_desc);

					//$prod_desc=str_replace('~','"',$prod_desc);

					$product_group_code_name=trim($data[3]);
					$product_group_code_name = preg_replace('/[[:^print:]]/', '', $product_group_code_name);


					$product_sub_group_code_name=trim($data[4]);
					$product_sub_group_code_name = preg_replace('/[[:^print:]]/', '', $product_sub_group_code_name);

					$product_brand_code_name=trim($data[5]);
					$product_brand_code_name = preg_replace('/[[:^print:]]/', '', $product_brand_code_name);
					...
					$cl_stk=trim($data[6]);
					$cl_stk = preg_replace('/[[:^print:]]/', '', $cl_stk);
                    ...
                    //echo"<pre>";print_r($data);die;
					$acedns=trim($data[7]);
						$acedns = preg_replace('/[[:^print:]]/', '', $acedns);
					...
					$black_list = preg_replace('/[[:^print:]]/', '', $black_list);
					...
					$vertical_value = preg_replace('/[[:^print:]]/', '', $vertical_value);

					$UOM1=trim($data[10]);
					$UOM1 = preg_replace('/[[:^print:]]/', '', $UOM1);

					$UOM2=trim($data[11]);
					$UOM2 = preg_replace('/[[:^print:]]/', '', $UOM2);

					$conversion=trim($data[12]);
					$conversion = preg_replace('/[[:^print:]]/', '', $conversion);

					$pack_size=trim($data[13]);
					$pack_size = preg_replace('/[[:^print:]]/', '', $pack_size);

					$UOM3=trim($data[14]);
					$UOM3 = preg_replace('/[[:^print:]]/', '', $UOM3);

					$conversion_factor_two=trim($data[15]);
					$conversion_factor_two = preg_replace('/[[:^print:]]/', '', $conversion_factor_two);

					$conversion_factor_two=str_replace(',','',$conversion_factor_two);

					$TD=trim($data[16]);
					$TD = preg_replace('/[[:^print:]]/', '', $TD);

					$focus=trim($data[17]);
					$focus = preg_replace('/[[:^print:]]/', '', $focus);

					$vat=trim($data[18]);
					$vat = preg_replace('/[[:^print:]]/', '', $vat);

					$pack_unit=trim($data[19]);
					$pack_unit = preg_replace('/[[:^print:]]/', '', $pack_unit);

					$prod_size=trim($data[20]);
					$prod_size = preg_replace('/[[:^print:]]/', '', $prod_size);

					$lead_time=trim($data[21]);
					$lead_time = preg_replace('/[[:^print:]]/', '', $lead_time);

					$buffer_level=trim($data[22]);
					$buffer_level = preg_replace('/[[:^print:]]/', '', $buffer_level);

					$max_level_marketing=trim($data[23]);
					$max_level_marketing = preg_replace('/[[:^print:]]/', '', $max_level_marketing);

					$UOM4=trim($data[24]);
					$UOM4 = preg_replace('/[[:^print:]]/', '', $UOM4);

					$UOM5=trim($data[25]);
					$UOM5 = preg_replace('/[[:^print:]]/', '', $UOM5);


					$gross_weight=trim($data[26]);
					$gross_weight = preg_replace('/[[:^print:]]/', '', $gross_weight);

					$fg_rm=trim($data[27]);
					$fg_rm = preg_replace('/[[:^print:]]/', '', $fg_rm);

					$oil_category=trim($data[28]);
					$oil_category = preg_replace('/[[:^print:]]/', '', $oil_category);

					$alias=trim($data[29]);
					$alias = preg_replace('/[[:^print:]]/', '', $alias);

					$hsn_sac=trim($data[30]);
					$hsn_sac = preg_replace('/[[:^print:]]/', '', $hsn_sac);

					$packing_realization=trim($data[31]);
					$packing_realization = preg_replace('/[[:^print:]]/', '', $packing_realization);

					$state_name=trim($data[32]);
					$state_name = preg_replace('/[[:^print:]]/', '', $state_name);
					$prod_full_name=trim($data[33]);
					$prod_full_name = preg_replace('/[[:^print:]]/', '', $prod_full_name);
					$category=trim($data[34]);
					$category = preg_replace('/[[:^print:]]/', '', $category);
					$sp_login=trim($data[35]);
					$dealer_login=trim($data[36]);
				   ...
				   if(strtoupper($folderName)=='ASL')
					...
					{
					    	$sqlstatecode="SELECT state_code FROM state_master WHERE state_name LIKE '%".$state_name."%'";
					}
					...
					if(no_of_filter > 2){
						...
						if($countprodsubgroupnamechk<1){
							...
							$sqlbrandform .= "  product_sub_group_code='".mysqli_real_escape_string($link,$max_product_sub_group_code)."'";

							$sqlbrandform .= " , product_sub_group_name='".addslashes($product_sub_group_code_name)."'";

							$sqlbrandform .= " , product_group_code='".mysqli_real_escape_string($link,$product_group_code)."'";
					...
					if(no_of_filter > 3){
						...
						if($countprodbrandnamechk<1){
							...
							$sqlbrandsubform .= "  product_brand_code='".mysqli_real_escape_string($link,$max_product_brand_code)."'";

							$sqlbrandsubform .= " , product_sub_group_code='".mysqli_real_escape_string($link,$product_sub_group_code)."'";

							$sqlbrandsubform .= " , product_group_code='".mysqli_real_escape_string($link,$product_group_code)."'";
					...
					if($packing_realization ==""){$packing_realization="0";}
...

						{
							...
							$sql .= " , product_sub_group_code='".mysqli_real_escape_string($link,$product_sub_group_code)."'";

							$sql .= " , product_brand_code='".mysqli_real_escape_string($link,$product_brand_code)."'";

							$sql .= " , cl_stk='".mysqli_real_escape_string($link,$cl_stk)."'";
...

							{
...

								{
...

									{
										...
										$sqlinsertstk .= "  	branch_code='".mysqli_real_escape_string($link,$branch_code_cl_stk)."'";

										$sqlinsertstk .= " , product_code='".mysqli_real_escape_string($link,$max_prod_code)."'";
...

			{
			  ...
			  //print_r($data);
...

				{
					...
					$sqlinsertstk .= "  	branch_code='".mysqli_real_escape_string($link,$branch_code)."'";

					$sqlinsertstk .= " , product_code='".mysqli_real_escape_string($link,$product_code)."'";

					$sqlinsertstk .= " , closing_stk='".mysqli_real_escape_string($link,$cl_stk)."'";
...

				{
...

					{
						...
						$sqlupdatestk .= "  	closing_stk='".mysqli_real_escape_string($link,$cl_stk)."'";

						$sqlupdatestk .= ",  download_time=CURRENT_TIMESTAMP()";

						$sqlupdatestk .= "  WHERE branch_code='".mysqli_real_escape_string($link,$branch_code)."' AND product_code='".mysqli_real_escape_string($link,$product_code)."'";
	...
	if(similar_file_exists("../csv/$folderName/Employee master.csv")!=false && $_FILES['zip_file']['name']=='Employee master.zip')
...

			{
...

				{
				 ...
				 // print_r($data);
					...
					$dns_employee_code = preg_replace('/[[:^print:]]/', '', $dns_employee_code);

					$employee_name=trim($data[1]);
					$employee_name = preg_replace('/[[:^print:]]/', '', $employee_name);
					...
					$reporting_to = preg_replace('/[[:^print:]]/', '', $reporting_to);
					...
					$phone_no = preg_replace('/[[:^print:]]/', '', $phone_no);

					$sale_access=trim($data[7]);
					$sale_access = preg_replace('/[[:^print:]]/', '', $sale_access);

					$designation=trim($data[8]);
					$designation = preg_replace('/[[:^print:]]/', '', $designation);

					$HQ=trim($data[9]);
					$HQ = preg_replace('/[[:^print:]]/', '', $HQ);

					$state=trim($data[10]);
					$state = preg_replace('/[[:^print:]]/', '', $state);

					$zone=trim($data[11]);
					$zone = preg_replace('/[[:^print:]]/', '', $zone);

					$acedns=trim($data[12]);
                    $acedns = preg_replace('/[[:^print:]]/', '', $acedns);

					$district=trim($data[13]);
					$district = preg_replace('/[[:^print:]]/', '', $district);

					$functionality=trim($data[14]);
					$functionality = preg_replace('/[[:^print:]]/', '', $functionality);

					$functionality_rel_val=trim($data[15]);
					$functionality_rel_val = preg_replace('/[[:^print:]]/', '', $functionality_rel_val);

					$DOJ=trim($data[16]);
					$DOJ = preg_replace('/[[:^print:]]/', '', $DOJ);
					...
					$DOJ=$year.'-'.$months.'-'.$days;
//echo $DOJ;exit();
					$level=trim($data[17]);
					$level = preg_replace('/[[:^print:]]/', '', $level);
					$region=trim($data[18]);
...
//echo $sql;
				...
				if(strtoupper($folderName)=='START' || strtoupper($folderName)=='STAR')
			   {
				  $employee_ins_fields = array(

							'dns_employee_code' => $dns_employee_code,

							'employee_name' => $employee_name,

							'branch_code_name' => $ranch_code_name,
				  ...
				  $employee_in_final_array[]=$employee_ins_fields;
				  $employee_ins_fields_string = http_build_query($employee_in_final_array);
...

			{
				...
				echo $countempowncode=mysqli_num_rows($rsempowncode);

				echo $countempbosscode=mysqli_num_rows($rsempbosscode);
		...
		if(strtoupper($folderName)=='START' || strtoupper($folderName)=='STAR')

		  {
			...
			array_push($upload_master_table_array,'employee_master');
	...
	if(similar_file_exists("../csv/$folderName/Customer Master.csv")!=false && $_FILES['zip_file']['name']=='Customer Master.zip')
	{
		...
		$err = "";
			...
			//print_r($lines);
			//echo"<pre>";print_r($lines);die;
			//echo"<pre>";print_r($folderName);die;
			...
			{
				//sk comment not go hear 11-11-25 line 5144
					//echo"<pre>";print_r($lines);die;
...

			{
...

				{
				  ...
				  //print_r($data);
					...
					$dns_customer_code = preg_replace('/[[:^print:]]/', '', $dns_customer_code);
					...
					$dns_route_code = preg_replace('/[[:^print:]]/', '', $dns_route_code);

					$route_name	  =trim($data[4]);
					$route_name = preg_replace('/[[:^print:]]/', '', $route_name);

					$emp_code_name		=trim($data[5]);
					$emp_code_name = preg_replace('/[[:^print:]]/', '', $emp_code_name);
					...
					$credit_limit = preg_replace('/[[:^print:]]/', '', $credit_limit);
					...
					$customer_type = preg_replace('/[[:^print:]]/', '', $customer_type);

					$rds_tag   =trim($data[14]);
					$rds_tag = preg_replace('/[[:^print:]]/', '', $rds_tag);
                    ...
                    $sauda_validity_period = preg_replace('/[[:^print:]]/', '', $sauda_validity_period);
					$address  =trim($data[16]);
					$address = preg_replace('/[[:^print:]]/', '', $address);

					$landline_no  =trim($data[17]);
					$landline_no = preg_replace('/[[:^print:]]/', '', $landline_no);

					$owner_name  =trim($data[18]);
					$owner_name = preg_replace('/[[:^print:]]/', '', $owner_name);

					$owner_phone  =trim($data[19]);
					$owner_phone = preg_replace('/[[:^print:]]/', '', $owner_phone);

					$cust_class  =trim($data[20]);
					$cust_class = preg_replace('/[[:^print:]]/', '', $cust_class);
					...
					$bank_name  =trim(preg_replace( ['/[^\x20-\x7E]/', '/[\r\n]+/'], '',$data[27]));

					$bank_account_number  =trim(preg_replace( ['/[^\x20-\x7E]/', '/[\r\n]+/'], '',$data[28]));

					$email  =trim(preg_replace( ['/[^\x20-\x7E]/', '/[\r\n]+/'], '',$data[29]));

					$visit_day  =trim($data[30]);

					$state  =trim(preg_replace( ['/[^\x20-\x7E]/', '/[\r\n]+/'], '',$data[31]));
					$state = preg_replace('/[[:^print:]]/', '', $state);
					...
					$zone  =trim(preg_replace( ['/[^\x20-\x7E]/', '/[\r\n]+/'], '',$data[38]));
...

					{
						...
						if($monthly_potential==""){$monthly_potential="0";}
						if($loadability_ton==""){$loadability_ton="0";}
						if($visit_sequence==""){$visit_sequence="0";}
						if($base_latt==""){$base_latt="0";}
						if($base_longi==""){$base_longi="0";}
						if($activated_customer_code==""){$activated_customer_code="0";}
			...
			{
				//sk comment customer master start hear 11-11-25 
				//echo"<pre>";print_r($lines);die;
			  foreach($lines as $line)

			  {
				...
				$double_coute_found = false;
				//echo"<pre>";print_r($rec_count);die;
...

				{
				   ...
				   //$data[]=$value;
					...
					$customer_name	=trim(preg_replace( ['/[^\x20-\x7E]/', '/[\r\n]+/'], '',$data[1]));
					$customer_name = preg_replace('/[[:^print:]]/', '', $customer_name);

					$phone_no		=trim(preg_replace( ['/[^\x20-\x7E]/', '/[\r\n]+/'], '',$data[2]));

					$dns_route_code  =trim(preg_replace( ['/[^\x20-\x7E]/', '/[\r\n]+/'], '',$data[3]));
					$dns_route_code = preg_replace('/[[:^print:]]/', '', $dns_route_code);

					$route_name	  =trim(preg_replace( ['/[^\x20-\x7E]/', '/[\r\n]+/'], '',$data[4])); $route_name = preg_replace('/[[:^print:]]/', '', $route_name);

					$emp_code_name		=trim(preg_replace( ['/[^\x20-\x7E]/', '/[\r\n]+/'], '',$data[5]));
					$emp_code_name = preg_replace('/[[:^print:]]/', '', $emp_code_name);
					 if(strpos($emp_code_name,';')!=false)
					 {
						$emp_code_name=str_replace(';',',',$emp_code_name);
					...
					$customer_type   =trim(preg_replace( ['/[^\x20-\x7E]/', '/[\r\n]+/'], '',$data[13]));

					$rds_tag   =trim(preg_replace( ['/[^\x20-\x7E]/', '/[\r\n]+/'], '',$data[14]));
                    $rds_tag = preg_replace('/[[:^print:]]/', '', $rds_tag);
					//echo retailer_app;
					...
					$sauda_validity_period  =trim(preg_replace( ['/[^\x20-\x7E]/', '/[\r\n]+/'], '',$data[15]));

					$address  =trim(preg_replace( ['/[^\x20-\x7E]/', '/[\r\n]+/'], '',$data[16]));
                    $address = preg_replace('/[[:^print:]]/', '', $address);
					$landline_no  =trim(preg_replace( ['/[^\x20-\x7E]/', '/[\r\n]+/'], '',$data[17]));

					$owner_name  =trim(preg_replace( ['/[^\x20-\x7E]/', '/[\r\n]+/'], '',$data[18]));
                    $owner_name = preg_replace('/[[:^print:]]/', '', $owner_name);
					$owner_phone  =trim(preg_replace( ['/[^\x20-\x7E]/', '/[\r\n]+/'], '',$data[19]));

					$cust_class  =trim(preg_replace( ['/[^\x20-\x7E]/', '/[\r\n]+/'], '',$data[20]));

					$weekly_closing_day  =trim(preg_replace( ['/[^\x20-\x7E]/', '/[\r\n]+/'], '',$data[21]));

					$coverage_type  =trim(preg_replace( ['/[^\x20-\x7E]/', '/[\r\n]+/'], '',$data[22]));

					$TIN  =trim(preg_replace( ['/[^\x20-\x7E]/', '/[\r\n]+/'], '',$data[23]));

					$PAN  =trim(preg_replace( ['/[^\x20-\x7E]/', '/[\r\n]+/'], '',$data[24]));

					$district  =trim(preg_replace( ['/[^\x20-\x7E]/', '/[\r\n]+/'], '',$data[25]));
                    $district = preg_replace('/[[:^print:]]/', '', $district);
					$minimum_stock  =trim($data[26]);
					...
					//echo $dns_customer_code.'##'.count($appointment_dateArr).'##'.$appointment_date;
...

						{
							...
							$date_days=$appointment_dateArr[2];
							if($date_days=="00"){
							    $date_days = "01";
							}

						}else if(strlen($appointment_dateArr[2])==4)

						{
                            if($appointment_dateArr[2]=="0000"){
                                $year="1971";
                            }else{
							    $year=$appointment_dateArr[2];
                            }
                            $date_days=$appointment_dateArr[0];
							if($date_days=="00"){
							    $date_days = "01";
							}

						}if(strlen($appointment_dateArr[0])==4)

						{
                            if($appointment_dateArr[0]=="0000"){
                                $year="1971";
                            }else{
							    $year=$appointment_dateArr[0];
                            }
                            $date_days=$appointment_dateArr[2];
							if($date_days=="00"){
							    $date_days = "01";
                        ...
                        if($year=="1971"){
                            $appointment_date=$year.'-01-'.$date_days;
                        }else{
						$appointment_date=$year.'-'.$appointment_dateArr[1].'-'.$date_days;
                        }
					...
					if(strpos($date_of_birth,'-')!=false){
					 ...
					 $date_of_birthArr=explode('-',$date_of_birth);
...

						{
							...
							$date_days=$date_of_birthArr[2];
							if($date_days=="00") $date_days="01";

						}else if(strlen($date_of_birthArr[2])==4)

						{
                            if($date_of_birthArr[2]=="0000"){
                                $year="1971";
                            }else{
							    $year=$date_of_birthArr[2];
                            }

						}if(strlen($date_of_birthArr[0])==4)

						{
                            if($date_of_birthArr[0]=="0000"){
                                $year="1971";
                            }else{
							    $year=$date_of_birthArr[0];
                            }
                        ...
                        if($year=="1971" || $year=="0000" || $year=="00" || $date_of_birth[1]=="00"){
                            $date_of_birth='1971-01-01';
                        }else{
						$date_of_birth=$year.'-'.$date_of_birthArr[1].'-'.$date_days;
                        }
					}
					...
					if(strpos($date_of_anniversary,'-')!=false){
					 ...
					 $date_of_anniversaryArr=explode('-',$date_of_anniversary);
...

						{
							...
							$date_days=$date_of_anniversaryArr[2];
							if($date_days=="00"){
							    $date_days="01";
							}

						}else if(strlen($date_of_anniversaryArr[2])==4)
...

						{
							...
							$year=$date_of_anniversaryArr[0];

						}

                        if($year=="1971" || $year=="0000" || $year=="00" || $date_of_anniversaryArr[1]=="00"){
                            $date_of_anniversary='1971-01-01';
                        }else{
						$date_of_anniversary=$year.'-'.$date_of_anniversaryArr[1].'-'.$date_days;
                        }
					...
					$whatsapp_no=trim(preg_replace( ['/[^\x20-\x7E]/', '/[\r\n]+/'], '',$data[43]));

					$beneficiary_name=trim(preg_replace( ['/[^\x20-\x7E]/', '/[\r\n]+/'], '',$data[44]));

					$IFS_code=trim($data[45]);

					$pin=trim(preg_replace( ['/[^\x20-\x7E]/', '/[\r\n]+/'], '',$data[46]));
					...
					$category_of_store = preg_replace('/[[:^print:]]/', '', $category_of_store);
					$nlp=trim($data[52]);
					$area=trim($data[53]);
					$route_no=trim($data[54]);
					$cluster=trim($data[55]);
					$sap_customer_code=trim($data[56]);
                    $sap_customer_code = preg_replace('/[[:^print:]]/', '', $sap_customer_code);
					...
					if($retailer_app=='')  $retailer_app='no';
						if($is_new_customer=='')  $is_new_customer='no';
						if($category_of_store=='') $category_of_store='no';
						if($nlp=='') $nlp='no';
						if($need_location_update=='') 	$need_location_update='no';
...

					{
						...
						$sql .= " ,	is_nlp='".addslashes(strtolower($nlp))."'";
						...
						$sql .= " ,	activated_customer_code='".$activated_customer_code."'";
						$sql .= " ,	activated_by=''";
						$sql .= " ,	cust_class_update_by=''";
						$sql .= " , SAP_customer_code='".addslashes($sap_customer_code)."'";

						$sql .= " ,	cust_class_update_time='1971-01-01'";
						$sql .= " ,	activated_datetime='1971-01-01'";
						$sql .= " ,	download_time_credit_limit='1971-01-01'";
...

					{
						...
						$sap_customer_code_db=$rowcustomernamechk['sap_customer_code'];
						if(providing_code=='yes'){
						...
						|| $IFS_code_db!=$IFS_code || $pin_db!=$pin ||  $base_latt_db!=$base_latt || $base_longi_db!=$base_longi || $retailer_app_db!=$retailer_app || $is_new_customer_db!=$is_new_customer || $category_of_store_db!=$category_of_store || $is_nlp_db!=$nlp || $cluster_db!=$cluster || $sap_customer_code_db!=$sap_customer_code)
						...
						if($activated_customer_code==""){$activated_customer_code="0";}
							...
							$sqlupdated .= " , email='".mb_convert_encoding($email, 'UTF-8', 'ISO-8859-1')."'";
							...
							$sqlupdated .= " , rds_tag='".$rds_code."',visit_day='".addslashes($visit_day)."',
												...
												is_nlp='".addslashes(strtolower($nlp))."',
												cluster='".addslashes($cluster)."',
												SAP_customer_code='".addslashes($sap_customer_code)."',
												download_time=CURRENT_TIMESTAMP()
					...
					if(strtoupper($folderName)=='DURO')
					{
...

						{
							...
							$sqlupdatecustomerroute="UPDATE customer_route_emp_relation SET acedns='".$acedns."',download_time=CURRENT_TIMESTAMP() WHERE
													...
													customer_code='".$customer_code."' AND route_code='".$route_code."' AND emp_code='".$emp_code."'";
							//echo"<pre>1";print_r($sqlupdatecustomerroute);
					...
					else
					{
						...
						if($countcustomerroute==0)
						{
							$sqlinsertcustomerroute="INSERT INTO customer_route_emp_relation SET customer_code='".$customer_code."',
													...
													acedns='".$acedns."',
													download_time=CURRENT_TIMESTAMP()";
													//echo $sqlinsertcustomerroute;
							mysqli_query($link,$sqlinsertcustomerroute);
						}
						else
						{
							$sqlcustomermulticheck="SELECT customer_code FROM customer_route_emp_relation WHERE customer_code IN(SELECT customer_code FROM customer_master WHERE customer_name='".addslashes($customer_name)."'  AND dns_customer_code='".$dns_customer_code."' AND route_code='".$route_code."')";
							//echo"<pre>";print_r($sqlcustomermulticheck);die;

							$rscustomermulticheck=mysqli_query($link,$sqlcustomermulticheck);
							//echo $sqlcustomermulticheck;
							while($rowcustomermulticheck=mysqli_fetch_assoc($rscustomermulticheck))
							{
								...
								//add start line sk 27-01-25
								$sql01="SELECT COUNT(*) FROM customer_route_emp_relation WHERE
									   customer_code='".$customer_code."'  AND emp_code='".$emp_code."'";
							    $count_sql01=mysqli_query($link,$sql01);
								$rowcountsql01 = mysqli_fetch_row($count_sql01);
								

								if($rowcountsql01[0] > 0){
									$sql02="UPDATE customer_route_emp_relation SET
														acedns='N',download_time=CURRENT_TIMESTAMP() WHERE
														customer_code='".$customer_code_multiple."' AND emp_code='".$emp_code."' ";
														mysqli_query($link,$sql02);
									//echo"<pre>1";print_r($sql02);

								}
								//add end line sk 27-01-25

								$checkroutexists="SELECT COUNT(*) FROM customer_route_emp_relation WHERE
									   customer_code='".$customer_code."'  AND emp_code='".$emp_code."' AND route_code='".$route_code."'";
							    $count_checkroutexists=mysqli_query($link,$checkroutexists);
								$rowcount = mysqli_fetch_row($count_checkroutexists);
								if($rowcount[0] > 0){
								    $sqlupdatecustomerroute="UPDATE customer_route_emp_relation SET
														acedns='".$acedns."',download_time=CURRENT_TIMESTAMP() WHERE
														customer_code='".$customer_code_multiple."' AND emp_code='".$emp_code."' AND route_code='".$route_code."'";
								}else{
								    /*$sqlupdatecustomerroute="UPDATE customer_route_emp_relation SET route_code='".$route_code."',
														acedns='".$acedns."',download_time=CURRENT_TIMESTAMP() WHERE
														customer_code='".$customer_code_multiple."' AND emp_code='".$emp_code."'";*/

												//add line sk 21-01-25
                                                $sqlupdatecustomerroute = "
                                                INSERT INTO customer_route_emp_relation
                                                (customer_code, route_code, emp_code, acedns, download_time)
                                                VALUES
                                                ('$customer_code_multiple', '$route_code', '$emp_code', '$acedns', CURRENT_TIMESTAMP())
							...
							//echo"<pre>2";print_r($sqlupdatecustomerroute);

								//echo"<pre>";print_r($sqlupdatecustomerroute);die;


								try {
                                        mysqli_query($link,$sqlupdatecustomerroute);
                                }catch(mysqli_sql_exception $e) {
                                    //echo "data exist-".$route_code."-".$customer_code_multiple."-".$emp_code;//$sqlupdatecustomerroute;
				   ...
				   if(strtoupper($folderName)=='STAR')
				   {
					  ...
					  $cust_ins_fields = array(
								...
								'email' => $email,
						  		...
						  		'emp_code' => $emp_code_name_value_next,
							);
...
if(similar_file_exists("../csv/$folderName/Customer Master MAGIK.csv")!=false)
{
		  ...
		  foreach($lines as $line)
		  {
			...
			if($rec_count>=1)
			{
			   ...
			   $data[]=$value;
				...
				$customer_name	=trim(preg_replace( ['/[^\x20-\x7E]/', '/[\r\n]+/'], '',$data[1]));
				$route_name  =trim(preg_replace( ['/[^\x20-\x7E]/', '/[\r\n]+/'], '',$data[4]));
				$old_emp_name  =trim(preg_replace( ['/[^\x20-\x7E]/', '/[\r\n]+/'], '',$data[5]));
				$new_emp_name  =trim(preg_replace( ['/[^\x20-\x7E]/', '/[\r\n]+/'], '',$data[6]));
...

		{
...

			{
			  ...
			  //print_r($data);
				...
				$sqlvendor .= "  vendor_code='".mysqli_real_escape_string($link,$max_vendor_code)."'";

				$sqlvendor .= "  ,dns_vendor_code='".mysqli_real_escape_string($link,$dns_vendor_code)."'";

				$sqlvendor .= "  ,vendor_name='".mysqli_real_escape_string($link,$vendor_name)."'";

				$sqlvendor .= " , branch_code='".mysqli_real_escape_string($link,$branch_code)."'";

				$sqlvendor .= " , rds_code='".mysqli_real_escape_string($link,$rds_code)."'";

				$sqlvendor .= " , emp_code='".mysqli_real_escape_string($link,$emp_code)."'";
...

			{
				...
				$sqlprodcode="SELECT prod_code FROM product_master WHERE prod_desc='".addslashes($prod_code_name_parts[2])."' AND product_sub_group_code='".$product_sub_group_code."' AND product_group_code='".$product_group_code."'";
					...
					if($countmrpchk<1){
						...
						$sql .= " , mrp='".mysqli_real_escape_string($link,$mrp)."'";

						$sql .= " , sale_rate='".mysqli_real_escape_string($link,$sale_rate)."'";

						$sql .= " , vertical_value='".mysqli_real_escape_string($link,$vertical_value)."'";

						$sql .= " , UOM='".mysqli_real_escape_string($link,$UOM)."'";

						$sql .= " , acedns='".mysqli_real_escape_string($link,$acedns)."'";

						$sql .= " , state_code='".mysqli_real_escape_string($link,$state_code)."'";

						$sql .= " , ws_rate='".mysqli_real_escape_string($link,$ws_price)."'";

						$sql .= " , distributor_rate='".mysqli_real_escape_string($link,$distributor_price)."'";

						$sql .= " , ss_rate='".mysqli_real_escape_string($link,$ss_price)."'";

						$sql .= " , depot_rate='".mysqli_real_escape_string($link,$depot_price)."'";
...

					{
						...
						if($mrp_db!=$mrp || $sale_rate_db!=$sale_rate || $acedns_db!=$acedns || $ws_price_db!=$ws_price || $distributor_price_db!=$distributor_price || $ss_price_db!=$ss_price || $depot_price_db!=$depot_price || $vertical_value_db!=$vertical_value){
							...
							$sqlupdate .= " SET mrp='".mysqli_real_escape_string($link,$mrp)."'";

							$sqlupdate .= " , sale_rate='".mysqli_real_escape_string($link,$sale_rate)."'";

							$sqlupdate .= " , acedns='".mysqli_real_escape_string($link,$acedns)."'";

							$sqlupdate .= " , ws_rate='".mysqli_real_escape_string($link,$ws_price)."'";

							$sqlupdate .= " , distributor_rate='".mysqli_real_escape_string($link,$distributor_price)."'";

							$sqlupdate .= " , ss_rate='".mysqli_real_escape_string($link,$ss_price)."'";

							$sqlupdate .= " , depot_rate='".mysqli_real_escape_string($link,$depot_price)."'";

							$sqlupdate .= " , vertical_value='".mysqli_real_escape_string($link,$vertical_value)."'";
	...
	if(similar_file_exists("../csv/$folderName/Destination master.csv")!=false && $_FILES['zip_file']['name']=='Destination master.zip')
...

			{
			  ...
			  //print_r($data);
...

				{
					...
					$sqldestination .= "   destination_code='".mysqli_real_escape_string($link,$max_destination_code)."'";

					$sqldestination .= " , dns_destination_code='".mysqli_real_escape_string($link,$dns_destination_code)."'";

					$sqldestination .= " , destination_name='".mysqli_real_escape_string($link,$destination_name)."'";

					$sqldestination .= " , ex_for_type='".mysqli_real_escape_string($link,$ex_for_type)."'";
...

				{
					...
					$sqldestination .= "  dns_destination_code='".mysqli_real_escape_string($link,$dns_destination_code)."',
										...
										download_time=CURRENT_TIMESTAMP() WHERE destination_code='".mysqli_real_escape_string($link,$destination_code)."'";
					echo $sqldestination;
					mysqli_query($link,$sqldestination) or array_push($error_array,"mysqli_error().Duplicate key @row $csv_row_count in Destination master.csv.Please check.");
	...
	if((similar_file_exists("../csv/$folderName/Star Dealer Target.csv")!=false || similar_file_exists("../csv/$folderName/Dealer Target.csv")!=false) && ($_FILES['zip_file']['name']=='Star Dealer Target.zip' || $_FILES['zip_file']['name']=='Dealer Target.zip'))
...

		{
...

			{
			  ...
			  //print_r($data);
				...
				//echo"<pre>";print_r(providing_code);die;
				if(providing_code=='yes'){
					$dns_customer_code = trim($dns_customer_code);
					$dns_customer_code = preg_replace('/[^\x20-\x7E]/', '', $dns_customer_code);
				 $sqlchkcustomercode="SELECT customer_code FROM self_appraisal_customer_wise WHERE customer_code='".$dns_customer_code."'";
				$customer_code=$dns_customer_code;
				}
				else
				{
					$sqlcustomercode="SELECT customer_code FROM customer_master WHERE customer_name='".mysqli_real_escape_string($link,$customer_name)."'";
					//echo $customer_name."--". $sqlcustomercode;
					$rscustomercode=mysqli_query($link,$sqlcustomercode);
					$rowcustomercode=mysqli_fetch_assoc($rscustomercode);
					$customer_code=$rowcustomercode['customer_code'];
					$customer_code = trim($customer_code);
					$customer_code = preg_replace('/[^\x20-\x7E]/', '', $customer_code);
					 $sqlchkcustomercode="SELECT customer_code FROM self_appraisal_customer_wise WHERE customer_code='".$customer_code."'";
				}
				//sk add line 24-07-25
				$sqlcustomercode="SELECT SAP_customer_code FROM customer_master WHERE dns_customer_code='".$dns_customer_code."'";
					//echo $customer_name."--". $sqlcustomercode;
					$rscustomercode=mysqli_query($link,$sqlcustomercode);
					$rowcustomercode=mysqli_fetch_assoc($rscustomercode);
					$dns_customer_code_for_saathi=$rowcustomercode['SAP_customer_code'];
					//echo"<pre>";print_r($dns_customer_code_for_saathi);
				//sk add end line	24-07-25
				...
				if($countchkcustomercode ==0)
				{
					...
					$sqlselfappraisal .= "   customer_code='".mysqli_real_escape_string($link,$customer_code)."'";
				$sqlselfappraisal .= " , apr_30_target='".mysqli_real_escape_string($link,$april_target)."'";
				$sqlselfappraisal .= " , apr_30_achievement='".mysqli_real_escape_string($link,$april_achievement)."'";
				$sqlselfappraisal .= " , apr_prev_y_target='".mysqli_real_escape_string($link,$april_prev_target)."'";
				$sqlselfappraisal .= " , apr_prev_y_achievement='".mysqli_real_escape_string($link,$april_prev_achievement)."'";
				$sqlselfappraisal .= " , may_31_target='".mysqli_real_escape_string($link,$may_target)."'";
				$sqlselfappraisal .= " , may_31_achievement='".mysqli_real_escape_string($link,$may_achievement)."'";
				$sqlselfappraisal .= " , may_prev_y_target='".mysqli_real_escape_string($link,$may_prev_target)."'";
				$sqlselfappraisal .= " , may_prev_y_achievement='".mysqli_real_escape_string($link,$may_prev_achievement)."'";
				$sqlselfappraisal .= " , jun_30_target='".mysqli_real_escape_string($link,$june_target)."'";
				$sqlselfappraisal .= " , jun_30_achievement='".mysqli_real_escape_string($link,$june_achievement)."'";
				$sqlselfappraisal .= " , jun_prev_y_target='".mysqli_real_escape_string($link,$june_prev_target)."'";
				$sqlselfappraisal .= " , jun_prev_y_achievement='".mysqli_real_escape_string($link,$june_prev_achievement)."'";
				$sqlselfappraisal .= " , jul_31_target='".mysqli_real_escape_string($link,$july_target)."'";
				$sqlselfappraisal .= " , jul_31_achievement='".mysqli_real_escape_string($link,$july_achievement)."'";
				$sqlselfappraisal .= " , jul_prev_y_target='".mysqli_real_escape_string($link,$july_prev_target)."'";
				$sqlselfappraisal .= " , jul_prev_y_achievement='".mysqli_real_escape_string($link,$july_prev_achievement)."'";
				$sqlselfappraisal .= " , aug_31_target='".mysqli_real_escape_string($link,$august_target)."'";
				$sqlselfappraisal .= " , aug_31_achievement='".mysqli_real_escape_string($link,$august_achievement)."'";
				$sqlselfappraisal .= " , aug_prev_y_target='".mysqli_real_escape_string($link,$august_prev_target)."'";
				$sqlselfappraisal .= " , aug_prev_y_achievement='".mysqli_real_escape_string($link,$august_prev_achievement)."'";
				$sqlselfappraisal .= " , sep_30_target='".mysqli_real_escape_string($link,$september_target)."'";
				$sqlselfappraisal .= " , sep_30_achievement='".mysqli_real_escape_string($link,$september_achievement)."'";
				$sqlselfappraisal .= " , sep_prev_y_target='".mysqli_real_escape_string($link,$september_prev_target)."'";
				$sqlselfappraisal .= " , sep_prev_y_achievement='".mysqli_real_escape_string($link,$september_prev_achievement)."'";
				$sqlselfappraisal .= " , oct_31_target='".mysqli_real_escape_string($link,$october_target)."'";
				$sqlselfappraisal .= " , oct_31_achievement='".mysqli_real_escape_string($link,$october_achievement)."'";
				$sqlselfappraisal .= " , oct_prev_y_target='".mysqli_real_escape_string($link,$october_prev_target)."'";
				$sqlselfappraisal .= " , oct_prev_y_achievement='".mysqli_real_escape_string($link,$october_prev_achievement)."'";
				$sqlselfappraisal .= " , nov_30_target='".mysqli_real_escape_string($link,$november_target)."'";
				$sqlselfappraisal .= " , nov_30_achievement='".mysqli_real_escape_string($link,$november_achievement)."'";
				$sqlselfappraisal .= " , nov_prev_y_target='".mysqli_real_escape_string($link,$november_prev_target)."'";
				$sqlselfappraisal .= " , nov_prev_y_achievement='".mysqli_real_escape_string($link,$november_prev_achievement)."'";
				$sqlselfappraisal .= " , dec_31_target='".mysqli_real_escape_string($link,$december_target)."'";
				$sqlselfappraisal .= " , dec_31_achievement='".mysqli_real_escape_string($link,$december_achievement)."'";
				$sqlselfappraisal .= " , dec_prev_y_target='".mysqli_real_escape_string($link,$december_prev_target)."'";
				$sqlselfappraisal .= " , dec_prev_y_achievement='".mysqli_real_escape_string($link,$december_prev_achievement)."'";
				$sqlselfappraisal .= " , jan_31_target='".mysqli_real_escape_string($link,$january_target)."'";
				$sqlselfappraisal .= " , jan_31_achievement='".mysqli_real_escape_string($link,$january_achievement)."'";
				$sqlselfappraisal .= " , jan_prev_y_target='".mysqli_real_escape_string($link,$january_prev_target)."'";
				$sqlselfappraisal .= " , jan_prev_y_achievement='".mysqli_real_escape_string($link,$january_prev_achievement)."'";
				$sqlselfappraisal .= " , feb_28_target='".mysqli_real_escape_string($link,$february_target)."'";
				$sqlselfappraisal .= " , feb_28_achievement='".mysqli_real_escape_string($link,$february_achievement)."'";
				$sqlselfappraisal .= " , feb_prev_y_target='".mysqli_real_escape_string($link,$february_prev_target)."'";
				$sqlselfappraisal .= " , feb_prev_y_achievement='".mysqli_real_escape_string($link,$february_prev_achievement)."'";

				$sqlselfappraisal .= " , mar_31_target='".mysqli_real_escape_string($link,$march_target)."'";
				$sqlselfappraisal .= " , mar_31_achievement='".mysqli_real_escape_string($link,$march_achievement)."'";
				$sqlselfappraisal .= " , mar_prev_y_target='".mysqli_real_escape_string($link,$march_prev_target)."'";
				$sqlselfappraisal .= " , mar_prev_y_achievement='".mysqli_real_escape_string($link,$march_prev_achievement)."'";
				...
				else
				{
					...
					$sqlselfappraisalupdate .= " apr_30_target='".mysqli_real_escape_string($link,$april_target)."'";

					$sqlselfappraisalupdate .= " , apr_30_achievement='".mysqli_real_escape_string($link,$april_achievement)."'";

					$sqlselfappraisalupdate .= " , apr_prev_y_target='".mysqli_real_escape_string($link,$april_prev_target)."'";

					$sqlselfappraisalupdate .= " , apr_prev_y_achievement='".mysqli_real_escape_string($link,$april_prev_achievement)."'";

					$sqlselfappraisalupdate .= " , may_31_target='".mysqli_real_escape_string($link,$may_target)."'";

					$sqlselfappraisalupdate .= " , may_31_achievement='".mysqli_real_escape_string($link,$may_achievement)."'";

					$sqlselfappraisalupdate .= " , may_prev_y_target='".mysqli_real_escape_string($link,$may_prev_target)."'";

					$sqlselfappraisalupdate .= " , may_prev_y_achievement='".mysqli_real_escape_string($link,$may_prev_achievement)."'";

					$sqlselfappraisalupdate .= " , jun_30_target='".mysqli_real_escape_string($link,$june_target)."'";

					$sqlselfappraisalupdate .= " , jun_30_achievement='".mysqli_real_escape_string($link,$june_achievement)."'";

					$sqlselfappraisalupdate .= " , jun_prev_y_target='".mysqli_real_escape_string($link,$june_prev_target)."'";

				    $sqlselfappraisalupdate .= " , jun_prev_y_achievement='".mysqli_real_escape_string($link,$june_prev_achievement)."'";

					$sqlselfappraisalupdate .= " , jul_31_target='".mysqli_real_escape_string($link,$july_target)."'";

					$sqlselfappraisalupdate .= " , jul_31_achievement='".mysqli_real_escape_string($link,$july_achievement)."'";

					$sqlselfappraisalupdate .= " , jul_prev_y_target='".mysqli_real_escape_string($link,$july_prev_target)."'";

				    $sqlselfappraisalupdate .= " , jul_prev_y_achievement='".mysqli_real_escape_string($link,$july_prev_achievement)."'";

					$sqlselfappraisalupdate .= " , aug_31_target='".mysqli_real_escape_string($link,$august_target)."'";

					$sqlselfappraisalupdate .= " , aug_31_achievement='".mysqli_real_escape_string($link,$august_achievement)."'";

					$sqlselfappraisalupdate .= " , aug_prev_y_target='".mysqli_real_escape_string($link,$august_prev_target)."'";

				    $sqlselfappraisalupdate .= " , aug_prev_y_achievement='".mysqli_real_escape_string($link,$august_prev_achievement)."'";

					$sqlselfappraisalupdate .= " , sep_30_target='".mysqli_real_escape_string($link,$september_target)."'";

					$sqlselfappraisalupdate .= " , sep_30_achievement='".mysqli_real_escape_string($link,$september_achievement)."'";

					$sqlselfappraisalupdate .= " , sep_prev_y_target='".mysqli_real_escape_string($link,$september_prev_target)."'";

				    $sqlselfappraisalupdate .= " , sep_prev_y_achievement='".mysqli_real_escape_string($link,$september_prev_achievement)."'";

					$sqlselfappraisalupdate .= " , oct_31_target='".mysqli_real_escape_string($link,$october_target)."'";

					$sqlselfappraisalupdate .= " , oct_31_achievement='".mysqli_real_escape_string($link,$october_achievement)."'";

					$sqlselfappraisalupdate .= " , oct_prev_y_target='".mysqli_real_escape_string($link,$october_prev_target)."'";

				    $sqlselfappraisalupdate .= " , oct_prev_y_achievement='".mysqli_real_escape_string($link,$october_prev_achievement)."'";

					$sqlselfappraisalupdate .= " , nov_30_target='".mysqli_real_escape_string($link,$november_target)."'";

					$sqlselfappraisalupdate .= " , nov_30_achievement='".mysqli_real_escape_string($link,$november_achievement)."'";

					$sqlselfappraisalupdate .= " , nov_prev_y_target='".mysqli_real_escape_string($link,$november_prev_target)."'";

					$sqlselfappraisalupdate .= " , nov_prev_y_achievement='".mysqli_real_escape_string($link,$november_prev_achievement)."'";

					$sqlselfappraisalupdate .= " , dec_31_target='".mysqli_real_escape_string($link,$december_target)."'";

					$sqlselfappraisalupdate .= " , dec_31_achievement='".mysqli_real_escape_string($link,$december_achievement)."'";

					$sqlselfappraisalupdate .= " , dec_prev_y_target='".mysqli_real_escape_string($link,$december_prev_target)."'";

					$sqlselfappraisalupdate .= " , dec_prev_y_achievement='".mysqli_real_escape_string($link,$december_prev_achievement)."'";

					$sqlselfappraisalupdate .= " , jan_31_target='".mysqli_real_escape_string($link,$january_target)."'";

					$sqlselfappraisalupdate .= " , jan_31_achievement='".mysqli_real_escape_string($link,$january_achievement)."'";

					$sqlselfappraisalupdate .= " , jan_prev_y_target='".mysqli_real_escape_string($link,$january_prev_target)."'";

					$sqlselfappraisalupdate .= " , jan_prev_y_achievement='".mysqli_real_escape_string($link,$january_prev_achievement)."'";

					$sqlselfappraisalupdate .= " , feb_28_target='".mysqli_real_escape_string($link,$february_target)."'";

					$sqlselfappraisalupdate .= " , feb_28_achievement='".mysqli_real_escape_string($link,$february_achievement)."'";

					$sqlselfappraisalupdate .= " , feb_prev_y_target='".mysqli_real_escape_string($link,$february_prev_target)."'";

					$sqlselfappraisalupdate .= " , feb_prev_y_achievement='".mysqli_real_escape_string($link,$february_prev_achievement)."'";

					$sqlselfappraisalupdate .= " , mar_31_target='".mysqli_real_escape_string($link,$march_target)."'";

					$sqlselfappraisalupdate .= " , mar_31_achievement='".mysqli_real_escape_string($link,$march_achievement)."'";

					$sqlselfappraisalupdate .= " , mar_prev_y_target='".mysqli_real_escape_string($link,$march_prev_target)."'";

					$sqlselfappraisalupdate .= " , mar_prev_y_achievement='".mysqli_real_escape_string($link,$march_prev_achievement)."'";
					$sqlselfappraisalupdate .= " , download_time=CURRENT_TIMESTAMP() WHERE customer_code='".$customer_code."'";
				//	echo $sqlselfappraisalupdate;
			...
			if(strtoupper($folderName)=='START' || strtoupper($folderName)=='STAR')
			   {
				  $target_ins_fields = array(
							'customer_code' => $dns_customer_code_for_saathi,
				  ...
				  $target_ins_fields_string = json_encode($target_in_final_array);
				  //$target_ins_fields_string = $target_in_final_array;
				}
			 $rec_count++;
		}
		//echo"<pre>";print_r($target_in_final_array);die;
		if(strtoupper($folderName)=='START' || strtoupper($folderName)=='STAR')
		  {
			array_push($upload_master_table_array,'self_appraisal_customer_wise');
		  }
		$successval=1;
		//echo"<pre>";print_r($upload_master_table_array);die;
		//echo"<pre>";print_r($target_in_final_array);die;
   ...
   //For Product Wise Target Ach
	...
	if(similar_file_exists("../csv/$folderName/Depot Destination Freight.csv")!=false && $_FILES['zip_file']['name']=='Depot Destination Freight.zip')
	{
...
//$type=strtoupper(substr($_FILES['excel_file']['name'],(strrpos($_FILES['excel_file']['name'],".")+1)));
	...
	if(similar_file_exists("../csv/$folderName/customer destination.csv")!=false && $_FILES['zip_file']['name']=='customer destination.zip')
	{
...
//$type=strtoupper(substr($_FILES['excel_file']['name'],(strrpos($_FILES['excel_file']['name'],".")+1)));
	...
	if(similar_file_exists("../csv/$folderName/broker destination.csv")!=false && $_FILES['zip_file']['name']=='broker destination.zip')
	{
...
//$type=strtoupper(substr($_FILES['excel_file']['name'],(strrpos($_FILES['excel_file']['name'],".")+1)));
...

		{
...

			{
			  ...
			  //print_r($data);
				...
				$facilitator_name = preg_replace('/[[:^print:]]/', '', $facilitator_name);

				$emp_code_name =trim($data[1]);
				$emp_code_name = preg_replace('/[[:^print:]]/', '', $emp_code_name);

				$f_type =trim($data[2]);
				$f_type = preg_replace('/[[:^print:]]/', '', $f_type);

				$firm_name =trim($data[3]);
				$firm_name = preg_replace('/[[:^print:]]/', '', $firm_name);

				$f_address =trim($data[4]);
				$f_address = preg_replace('/[[:^print:]]/', '', $f_address);

				$f_pin =trim($data[5]);
				$f_pin = preg_replace('/[[:^print:]]/', '', $f_pin);

				$f_sub_area =trim($data[6]);
				$f_sub_area = preg_replace('/[[:^print:]]/', '', $f_sub_area);

				$f_area =trim($data[7]);
				$f_area = preg_replace('/[[:^print:]]/', '', $f_area);

				$mobile_no =trim($data[8]);
				$mobile_no = preg_replace('/[[:^print:]]/', '', $mobile_no);

				$email_id =trim($data[9]);
				$email_id = preg_replace('/[[:^print:]]/', '', $email_id);

				$dob =trim($data[10]);
				$dob = preg_replace('/[[:^print:]]/', '', $dob);

				$annniversary =trim($data[11]);
				$annniversary = preg_replace('/[[:^print:]]/', '', $annniversary);
...

	{
	  ...
	  //For Broker Master CSV
		...
		if(similar_file_exists("../csv/$folderName/Broker master.csv")!=false && $_FILES['zip_file']['name']=='Broker master.zip')

		{
...

			{
				...
				if($dns_broker_id!='' && $broker_name!='' && $acedns!=''){
				...
				//echo"<pre>";print_r($broker_final_array);die;
	...
	if(similar_file_exists("../csv/$folderName/Customer broker mapping.csv")!=false && $_FILES['zip_file']['name']=='Customer broker mapping.zip')
...

		{
...

			{
			  ...
			  //print_r($data);
				...
				$broker_code = preg_replace('/[^\x20-\x7E]/', '', $broker_code);
				$broker_name=trim($data[3]);
				$broker_name = preg_replace('/[^\x20-\x7E]/', '', $broker_name);
				$acedns=trim($data[4]);
				$acedns = preg_replace('/[^\x20-\x7E]/', '', $acedns);
				$mapped_broker=strtolower(trim($data[5]));
				$mapped_broker = preg_replace('/[^\x20-\x7E]/', '', $mapped_broker);
...

				{
					...
					$sqlupdatecustomerbroker="UPDATE customer_broker_relation SET acedns='".strtoupper($acedns)."',
			...
			if(strtoupper($folderName)=='START' || strtoupper($folderName)=='STAR')

			   {
				  ...
				  $customer_broker_ins_fields = array(
                    ...
                    if($customer_code !='' && $broker_code !=''){
                        $customer_broker_final_array[]=$customer_broker_ins_fields;


				  //echo '<pre>';print_r($customer_broker_final_array);
	...
	//For Depot Destination Freight
	//echo"<pre>";print_r($_FILES['zip_file']['name']);die;
	if(similar_file_exists("../csv/$folderName/branch dump.csv")!=false && $_FILES['zip_file']['name']=='branch dump.zip')
			  ...
			  //print_r($data);
				...
				if($countbranchdump<1 )

					{
						...
						$sqlinsertbranchdump .= " ,dump_name='".mb_convert_encoding($dump_name, 'UTF-8', 'UTF-8')."'";
...

				    {
					  ...
					  $branch_dump_ins_fields = array(
								...
								'dump_name' => mb_convert_encoding($dump_name, 'UTF-8', 'UTF-8'),
...
if(similar_file_exists("../csv/$folderName/pop product master.csv")!=false && $_FILES['zip_file']['name']=='pop product master.zip')
	{
		...
		foreach($lines as $line)
		{
			...
			if($rec_count>=1)
			{
			  ...
			  //print_r($data);
				...
				$status=trim($data[6]);
				$payment_gateway=trim($data[7]);
				$dealer_login=trim($data[8]);
				$sp_login=trim($data[9]);
				//$prod_desc = str_replace("\xC2\xA0", ' ', $prod_desc);
				//add condition sk 30-12-25
				$prod_desc = preg_replace('/[^\x20-\x7E]/', '', $prod_desc);
...
//echo"<pre>";print_r($pop_product_final_array);die;
	...
	if(similar_file_exists("../csv/$folderName/branch pop product.csv")!=false && $_FILES['zip_file']['name']=='branch pop product.zip')
	{
		...
		foreach($lines as $line)
		{
			...
			if($rec_count>=1)
			{
			  ...
			  //print_r($data);
				...
				$dns_branch_code=trim($data[0]);
				$dns_branch_code = preg_replace('/[[:^print:]]/', '', $dns_branch_code);
				$dns_branch_code = trim(preg_replace('/[^\x20-\x7E]/', '', $dns_branch_code));

				$branch_name=trim($data[1]);
				$branch_name = preg_replace('/[[:^print:]]/', '', $branch_name);
				$branch_name = trim(preg_replace('/[^\x20-\x7E]/', '', $branch_name));

				$dns_prod_code=trim($data[2]);
				$dns_prod_code = preg_replace('/[[:^print:]]/', '', $dns_prod_code);
				$dns_prod_code = trim(preg_replace('/[^\x20-\x7E]/', '', $dns_prod_code));

				$prod_desc=trim($data[3]);
				$prod_desc = preg_replace('/[[:^print:]]/', '', $prod_desc);
				$prod_desc = trim(preg_replace('/[^\x20-\x7E]/', '', $prod_desc));

				$status=trim($data[4]);
				$status = preg_replace('/[[:^print:]]/', '', $status);
				$status = trim(preg_replace('/[^\x20-\x7E]/', '', $status));
	...
	if(similar_file_exists("../csv/$folderName/beatewise_TA_DA.csv")!=false)
	{
		...
		foreach($lines as $line)
		{
...

			{
			  ...
			  //print_r($data);
				$sr_name=trim($data[0]);
								$sr_name = preg_replace('/[[:^print:]]/', '', $sr_name);

				$so_name=trim($data[1]);
				$so_name = preg_replace('/[[:^print:]]/', '', $so_name);
				$route_name=trim($data[2]);
				$route_name = preg_replace('/[[:^print:]]/', '', $route_name);
				$HQ_EX_SO=trim($data[3]);
				$HQ_EX_SO = preg_replace('/[[:^print:]]/', '', $HQ_EX_SO);
				$DA=trim($data[4]);
				$DA = preg_replace('/[[:^print:]]/', '', $DA);
				$distance=(trim($data[5]) == '') ? 0 : trim($data[5]);
				$distance = preg_replace('/[[:^print:]]/', '', $distance);
				$TA=(trim($data[6]) == '') ? 0 : trim($data[6]);
				$TA = preg_replace('/[[:^print:]]/', '', $TA);
	...
	if(similar_file_exists("../csv/$folderName/branch pgstatus.csv")!=false && $_FILES['zip_file']['name']=='branch pgstatus.zip')
	{
		...
		foreach($lines as $line)
		{
			...
			if($rec_count>=1)
			{
			  ...
			  //print_r($data);
				...
				$ooh_site_code = preg_replace('/[[:^print:]]/', '', $ooh_site_code);
				$state=trim($data[1]);
				$state = preg_replace('/[[:^print:]]/', '', $state);
				$area=trim($data[2]);
				$area = preg_replace('/[[:^print:]]/', '', $area);
				$city=trim($data[3]);
				$city = preg_replace('/[[:^print:]]/', '', $city);
				$location=trim($data[4]);
				$location = preg_replace('/[[:^print:]]/', '', $location);
				$facing=trim($data[5]);
				$facing = preg_replace('/[[:^print:]]/', '', $facing);
				$media=trim($data[6]);
				$media = preg_replace('/[[:^print:]]/', '', $media);
				$type=trim($data[7]);
				$type = preg_replace('/[[:^print:]]/', '', $type);
				$l_r=trim($data[8]);
				$l_r = preg_replace('/[[:^print:]]/', '', $l_r);
				$t_b=trim($data[9]);
				$t_b = preg_replace('/[[:^print:]]/', '', $t_b);
				$qty=trim($data[10]);
				$qty = preg_replace('/[[:^print:]]/', '', $qty);
				$fascia=trim($data[11]);
				$fascia = preg_replace('/[[:^print:]]/', '', $fascia);
				$sq_ft=trim($data[12]);
				$sq_ft = preg_replace('/[[:^print:]]/', '', $sq_ft);
				$vendor=trim($data[13]);
				$vendor = preg_replace('/[[:^print:]]/', '', $vendor);
				$vendor_code=trim($data[14]);
				$vendor_code = preg_replace('/[[:^print:]]/', '', $vendor_code);
				$location_facing=trim($data[15]);
				$location_facing = preg_replace('/[[:^print:]]/', '', $location_facing);
				$acedns=trim($data[16]);
				$acedns = preg_replace('/[[:^print:]]/', '', $acedns);
	...
	if(similar_file_exists("../csv/$folderName/MTL.csv")!=false)
	{
		$filename=similar_file_exists("../csv/$folderName/MTL.csv");
		$rec_count = 0;
		$ins_count = 0;
		$err = "";
		//$type=strtoupper(substr($_FILES['excel_file']['name'],(strrpos($_FILES['excel_file']['name'],".")+1)));
		...
		foreach($lines as $line)
		{
			...
			if($rec_count>=1)
			{
			   ...
			   //$data[]=$value;
				array_push($data,$value);
			  //print_r($data);
				$emp_code=$search = mb_convert_encoding(trim($data[0]), 'UTF-8', 'auto');;
				$mtl_no=trim($data[1]);
				//echo $emp_code;
				$sqlempcode="SELECT emp_code FROM employee_master WHERE dns_emp_code='".addslashes($emp_code)."'";
				$rsempcode=mysqli_query($link,$sqlempcode);
				$rowempcode=mysqli_fetch_assoc($rsempcode);
				//echo '<pre>';print_r($rowempcode);
				$emp_code=$rowempcode['emp_code'];

				$sqlmtlchk="SELECT mtl_no FROM emp_mtl_mapping  WHERE emp_code='".addslashes($emp_code)."' and mtl_no='".$mtl_no."'";
				$rsmtlchk=mysqli_query($link,$sqlmtlchk);
				$countmtlchk=mysqli_num_rows($rsmtlchk);
				$csv_row_count=$rec_count+1;
				if($countmtlchk==0)
					{
						$sql  = "insert into emp_mtl_mapping ";
						$sql .= " SET emp_code='".$emp_code."'";
						$sql .= " , mtl_no='".addslashes($mtl_no)."'";
						$sql .= " , upload_datetime=CURRENT_TIMESTAMP()";
					mysqli_query($link,$sql) or die(mysqli_error().".Internal error occurrs @row $csv_row_count in MTL.csv.Please check.");
					}
			...
			/*	$headers .= "From: ".FROMTAG."<".FROMEMAIL."> \r\n" .
				...
				*/
				...
				{*/
					...
					//STAR CURL
					//sk add line start 25-07-25
					// Detect protocol (HTTP or HTTPS)
							$protocol = (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off') ? "https://" : "http://";

							// Get domain name and port (if any)
							$host = $_SERVER['HTTP_HOST'];

							// Get base path (optional: if you want to remove the filename)
							$scriptPath = dirname($_SERVER['SCRIPT_NAME']);

							// Final dynamic base URL
							$baseURL = $protocol . $host . $scriptPath;

							// Optional: Add trailing slash
							$baseURL = rtrim($baseURL, '/') . '/';
							//echo"<pre>";print_r($baseURL);die;
						//sk add line end 25-07-25
					if(strtoupper($folderName)=='STAR')

					{
...

						{
						   ...
						   $sqlupdatetableinfo="UPDATE master_tables_update_info set need_update='yes',download_time_yes=CURRENT_TIMESTAMP()
							...
							$url=$baseURL . "uploadbranchdataSTARSATHICURL.php";
						...
						if(in_array('employee_master',$upload_master_table_array))
						{
						   ...
						   $sqlupdatetableinfo="UPDATE master_tables_update_info set need_update='yes',download_time_yes=CURRENT_TIMESTAMP()
							...
							curl_setopt($ch,CURLOPT_POSTFIELDS, "$employee_ins_fields_string");
...

						{
						   ...
						   mysqli_query($link,$sqlupdatetableinfo);
							...
							$url=$baseURL ."uploadskudataSTARSATHICURL.php";
...

						{
						   ...
						   $sqlupdatetableinfo="UPDATE master_tables_update_info set need_update='yes',download_time_yes=CURRENT_TIMESTAMP()
							...
							$url=$baseURL ."uploaddataSTARSATHICURL.php";
...

						{
						   ...
						   // mysqli_query($link,$sqlupdatetableinfo);
							...
							$url=$baseURL ."uploaddestinationdataSTARSATHICURL.php";
						...
						{
						   // echo"<pre>";print_r($target_ins_fields_string);die;
						   ...
						   $sqlupdatetableinfo="UPDATE master_tables_update_info set need_update='yes',download_time_yes=CURRENT_TIMESTAMP()
							...
							//$postData = json_encode($cust_in_final_array);
							 ...
							 $url=$baseURL ."uploadtargetdataSTARSATHICURL.php"; 
							...
							curl_setopt($ch, CURLOPT_HTTPHEADER, array(
									...
									'Content-Length: ' . strlen($target_ins_fields_string)
								));
							
						//	curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
                           // curl_setopt($ch, CURLOPT_TIMEOUT, 300);           // allow long requests
                          //  curl_setopt($ch, CURLOPT_POSTFIELDS, $target_ins_fields_string); // without quotes!
							...
							if (curl_errno($ch)) {
                                ...
                                echo "Response:\n";
                                echo htmlspecialchars($result); // Show HTML response safely
                            }
                                
				// 			echo "Test Res:".$result;
...

						{
						   ...
						   $sqlupdatetableinfo="UPDATE master_tables_update_info set need_update='yes',download_time_yes=CURRENT_TIMESTAMP()
							...
							$url=$baseURL ."uploadbrokerdataSTARSATHICURL.php";
						...
						if(in_array('pop_product',$upload_master_table_array))
						{
						   $sqlupdatetableinfo="UPDATE master_tables_update_info set need_update='yes',download_time_yes=CURRENT_TIMESTAMP()
							...
							$url=$baseURL ."upload_pop_product_data_curl.php";
						...
						if(in_array('branch_pop_product',$upload_master_table_array))
						{
						   $sqlupdatetableinfo="UPDATE master_tables_update_info set need_update='yes',download_time_yes=CURRENT_TIMESTAMP()
							...
							$url=$baseURL ."upload_pop_branch_product_data_curl.php";
...

						{
						   ...
						   $sqlupdatetableinfo="UPDATE master_tables_update_info set need_update='yes',download_time_yes=CURRENT_TIMESTAMP()
							...
							$url=$baseURL ."uploadcustomerbrokerdataSTARSATHICURL.php";
						...
						if(in_array('branch_destination_freight',$upload_master_table_array))
						{
						   ...
						   $sqlupdatetableinfo="UPDATE master_tables_update_info set need_update='yes',download_time_yes=CURRENT_TIMESTAMP()
							...
							$url=$baseURL ."uploadbranchdestinationdataSTARSATHICURL.php";
						...
						if(in_array('customer_destination',$upload_master_table_array))
						{
						   ...
						   // mysqli_query($link,$sqlupdatetableinfo);
							//$postData = json_encode($cust_in_final_array);
							$url=$baseURL ."uploadcustomerdestinationdataSTARSATHICURL.php";
							$ch = curl_init();
							//set the url, number of POST vars, POST data
							curl_setopt($ch,CURLOPT_URL, $url);
							curl_setopt($ch,CURLOPT_POST, 1);
						...
						if(in_array('broker_destination',$upload_master_table_array))
						{
						   $sqlupdatetableinfo="UPDATE master_tables_update_info set need_update='yes',download_time_yes=CURRENT_TIMESTAMP() where table_name='broker_destination'";
						    //mysqli_query($link,$sqlupdatetableinfo);
							//$postData = json_encode($cust_in_final_array);
							$url=$baseURL ."uploadbrokerdestinationdataSTARSATHICURL.php";
							$ch = curl_init();
							//set the url, number of POST vars, POST data
							curl_setopt($ch,CURLOPT_URL, $url);
							curl_setopt($ch,CURLOPT_POST, 1);
						...
						if(in_array('branch_dump',$upload_master_table_array))
						{
						   ...
						   $sqlupdatetableinfo="UPDATE master_tables_update_info set need_update='yes',download_time_yes=CURRENT_TIMESTAMP()
							...
							//$postData = json_encode($cust_in_final_array);
							 ...
							 $url=$baseURL ."uploadbranchdumpdataSTARSATHICURL.php";
						...
						if(in_array('branch_pgstatus',$upload_master_table_array))
						{
						   $sqlupdatetableinfo="UPDATE master_tables_update_info set need_update='yes',download_time_yes=CURRENT_TIMESTAMP()
							...
							//mysqli_query($link,$sqlupdatetableinfo);
							//$postData = json_encode($cust_in_final_array);
							$url=$baseURL ."uploadbranchpgstatusSTARSATHICURL.php";
							$ch = curl_init();
							//set the url, number of POST vars, POST data
							curl_setopt($ch,CURLOPT_URL, $url);
							curl_setopt($ch,CURLOPT_POST, 1);
					...
					{
							$url=$baseURL ."uploaddataSUPERSHAKTICURL.php";
							$ch = curl_init();
							//set the url, number of POST vars, POST data
							curl_setopt($ch,CURLOPT_URL, $url);
							curl_setopt($ch,CURLOPT_POST, 1);
					...
					{
							$url=$baseURL ."uploaddataGOLDSTONECURL.php";
							$ch = curl_init();
							//set the url, number of POST vars, POST data
							curl_setopt($ch,CURLOPT_URL, $url);
							curl_setopt($ch,CURLOPT_POST, 1);
