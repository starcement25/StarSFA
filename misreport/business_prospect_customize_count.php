...
while($row_prospect = mysqli_fetch_assoc($res_prospect))
{
	echo "<tr>
			<td>$count</td>";
			if($value==4){
			echo "<td><a href=\"#\" id=\"emp$count\" onClick=\"GenericAjaxFunction('business_prospect_customize_data.php?emp_code=$row_prospect[emp_code]&value=$value&start_date=$start_date&end_date=$end_date','prospect_detail',0); clearprospectdiv(); changecolor(id);\" style=\"color:blue;\">$row_prospect[emp_name]</a></td>";
		  }
			else{
				echo "<td><a href=\"#\" id=\"emp$count\" onClick=\"GenericAjaxFunction('business_prospect_customize_data.php?emp_code=$row_prospect[emp_code]&value=$value','prospect_detail',0); clearprospectdiv(); changecolor(id);\" style=\"color:blue;\">$row_prospect[emp_name]</a></td>";
			}
