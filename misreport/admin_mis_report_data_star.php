...
function main(){
	...
	?>
    <div id="report_tag" align="right" style="font-size:9px;" >* Total Field Activity = No of Customer visited + KYC + Brand Activity +Technical Meet + Site Visit</div><br />
	...
	if($mode == 'datewise'){
		...
		function get_query_result($column_name,$start_date,$end_date){
			...
			$row_result = mysqli_fetch_assoc($res_result);
			$return_result = $row_result['get_result'];
			return $return_result;
		}
		
		$sqlfieldforce="SELECT COUNT(EM.emp_code) AS available_field_force FROM employee_master EM,changepassword CH 					WHERE CH.emp_code=EM.emp_code AND CH.is_licensed='1' AND SUBSTRING(EM.emp_code,1,1)!='C' AND EM.emp_code IN(".$employee_arg.")";
		$resfieldforce=mysqli_query($link,$sqlfieldforce);
		...
		(".$employee_arg.") AND (operation_date BETWEEN '".$start_date."' AND '".$end_date."') AND present>0";
		$res_present = mysqli_query($link,$sql_present);
		$row_present = mysqli_fetch_assoc($res_present);
		...
		$res_customer_visit = mysqli_query($link,$sql_customer_visit);
						$customer_not_duplicate_array=array();
		$customer_not_duplicate_string='';				

		while($row_customer_visit = mysqli_fetch_assoc($res_customer_visit)){
			...
			if($customer_visited != ''){
				...
				foreach($customer_array  as $customer_value){
					if(!in_array($customer_value,$customer_not_duplicate_array))
					{
						array_push($customer_not_duplicate_array,$customer_value);
						$customer_not_duplicate_string=$customer_not_duplicate_string."'".$customer_value."'".',';
					}
					if($customer_value != ''){
		...
		//print_r($customer_not_duplicate_array);
		//echo $customer_not_duplicate_string;
	...
	for($i=1;$i<=3;$i++){
		...
		while($row_order_header_customer = mysqli_fetch_assoc($res_order_header_customer)){
			...
			if($emp_code != '' && $order_header_customer_code != '' && $location_date != ''){
		...
		while($row_stock_audit_customer = mysqli_fetch_assoc($res_stock_audit_customer)){
			...
			if($emp_code != '' && $stock_audit_customer != '' && $location_date != ''){
		...
		while($row_market_feedback = mysqli_fetch_assoc($res_market_feedback)){
			...
			if($emp_code != '' && $market_feedback_customer != '' && $market_feedback_date != ''){
		...
		$res_present_data = mysqli_query($link,$sql_present_data);
		$row_present_data = mysqli_fetch_assoc($res_present_data);
