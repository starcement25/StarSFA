...
if($total_rows>0){
	...
	/*---------------------------------> Count of non-productive call <--------------------------------*/
		...
		$res_order_no = mysqli_query($link,$sql_order_no);
		while($row_order_no = mysqli_fetch_assoc($res_order_no)){
			...
			$res_OPPC = mysqli_query($link,$sql_OPPC);
			$row_OPPC = mysqli_fetch_assoc($res_OPPC);
