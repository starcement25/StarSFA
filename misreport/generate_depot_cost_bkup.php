...
<form action="generate_depot_cost.php" name="depot_cost" onSubmit="return validation();" method="post">
...
<table cellpadding="4px">
     ...
     <tr>
        ...
        <td align="left">Depot Cost:<input type="text" name="depot_cost" id="depot_cost" style="height:20px;" value="<?php echo $depot_cost;?>"/></td>
    </tr>
    <tr>
...
function select_depot()
{
	...
	var url="returnbranchdepotcost.php?plant_name="+plant_name;
	...
	if($_REQUEST['mode']=='generate_depot_cost')
	{
		...
		while($rowselectdistinctbranch=mysqli_fetch_assoc($rsselectdistinctbranch))
		{
			...
			while($rowselectdistinctdnsprod=mysqli_fetch_assoc($rsselectdistinctdnsprod))
			{
				...
				$depot_cost_case_prodwise=round(($depot_cost/${conversion_factor_two.$distinct_dnsprod_code}),2);
