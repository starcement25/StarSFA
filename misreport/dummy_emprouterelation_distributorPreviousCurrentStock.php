...
function notmain()
{
  if($_SESSION['admin_login']=="admin"){
	...
	if(modified_customer_emp_route == 'yes'){
		if($_SESSION['admin_login']!="admin"){
			$sql_get_route = "SELECT DISTINCT ERR.route_code as route_code FROM emp_route_relation ERR WHERE ERR.emp_code IN(".$emp_hierarchy.")";
                                ...
                                <td align="left" width="" style="vertical-align:top;" colspan="2">
                                   <?php $customer_code=$_REQUEST['customer_code'];?>
                                    ...
                                    <option value="0">SELECT</option>
									 ...
									 $rsquerycustomer=mysqli_query($link,$sqlquerycustomer);					
                                     $count=mysqli_num_rows($rsquerycustomer);
                                     $cnt=1;
                                        if($count>0){
                                        while($rowquerycustomer = mysqli_fetch_assoc($rsquerycustomer))
                                        {
                                      ?>
...
if($_REQUEST['mode'] =='stock_details_display'){
    ...
    $rsstockauditcurrent=mysqli_query($link,$sqlstockauditcurrent) or die(mysqli_error()." Error in stock audit fetch: ".$sqlstockauditcurrent);
	while($rowstockauditcurrent=mysqli_fetch_assoc($rsstockauditcurrent))
	{
    ...
    $rsstockauditprevious=mysqli_query($link,$sqlstockauditprevious) or die(mysqli_error()." Error in stock audit previous fetch: ".$sqlstockauditprevious);
	while($rowstockauditprevious=mysqli_fetch_assoc($rsstockauditprevious))
	{
