...
$rslatestuploaddate=mysqli_query($link,$sqllatestuploaddate);
$rowlatestuploaddate=mysqli_fetch_assoc($rslatestuploaddate);
$packing_upload_date_latest=$rowlatestuploaddate['packing_upload_date'];*/

$sql_packing= "SELECT * FROM (SELECT PC.dns_prod_code, PM.prod_desc, PC.packing_cost,PC.plant_name,PC.labour_cost,PC.extra_cost,
				PM.acedns,DATE_FORMAT(SUBSTRING(PC.datetime,1,10),'%d-%m-%Y') As last_updated_date,PM.product_group_code,PM.pack_size,
				PGM.product_group_name,PC.packing_realization,PM.packing_realization AS  packing_realization_percent FROM
				packing_master PC,product_master PM,product_group_master PGM WHERE PC.dns_prod_code=PM.dns_prod_code 
				AND PM.product_group_code=PGM.product_group_code AND PC.plant_name!='' ".$product_condition."  
				ORDER BY PC.datetime DESC) AS SAT GROUP BY 1,4 ORDER BY 2";
...
if($total_packing>0){
    ...
    <table width="90%" border="1" style="border-collapse:collapse;" class="border" cellpadding="4">
     <tr>
      ...
      <tr class="TDHEAD_SUB" align="center">
        ...
        <td width="8%">Product Group</td>
        <td width="7%">Material Code</td>
        <td width="11%">Material Description</td>
        <td width="7%">Pack size (BP/CP)</td>
        <td width="9%">Packing Cost <br />(Rs/case)</td>
        <td width="9%">Labour Cost <br />(Rs/case)</td>
        <td width="9%">Extra Cost <br />(Rs/case)</td>
        <td width="9%">Packing Realization</td>
        <td width="9%">Active/Inactive</td>
      </tr>
    <?php
	while($row_packing = mysqli_fetch_assoc($res_packing)){
		...
		$extra_cost = $row_packing['extra_cost'];
		$plant = $row_packing['plant_name'];
		$labour_cost = $row_packing['labour_cost'];
		$packing_realization = $row_packing['packing_realization'];
		$packing_realization_percent = $row_packing['packing_realization_percent'];
		...
		echo "<tr>
				...
				<td align=\"right\" style=\"background:$color;\">".number_format($labour_cost,2)."</td>
				<td align=\"right\" style=\"background:$color;\">".number_format($extra_cost,3)."</td>
				<td align=\"right\" style=\"background:$color;\">".number_format($packing_realization_percent,2)."</td>
