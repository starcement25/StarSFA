...
$res_row_id = mysqli_query($link,$sql_row_id);
$row_row_id = mysqli_fetch_assoc($res_row_id);
$row_id = $row_row_id['row_id'];

$count = 1;
$sql_survey_id = "SELECT DISTINCT survey_id FROM survey_output WHERE row_id = '".$row_id."' AND type = 'NEW'";
$res_survey_id = mysqli_query($link,$sql_survey_id);
$total_row_check = mysqli_num_rows($res_survey_id);
if($total_row_check>0){
    ...
    <?php
	$res_survey_id = mysqli_query($link,$sql_survey_id);
	while($row_survey_id = mysqli_fetch_assoc($res_survey_id)){
