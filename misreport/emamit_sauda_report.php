...
if($_GET['type'] == 'today'){
	...
	$order_condition = "AND SUBSTRING(STL.sauda_no,-14,8) LIKE '".str_replace("-","",$today)."'";
	$value = 1;
	...
	$order_condition = "AND SUBSTRING(STL.sauda_no,8,4) =$year AND SUBSTRING(STL.sauda_no,12,2) =$month";
	$value = 2;
	...
	$order_condition = "AND (SUBSTRING(STL.sauda_no,-14,8) BETWEEN ".$start_date." AND ".$end_date.")";
	$value = 3;
...
echo "<table border=\"1\" width=\"100%\" style=\"border-collapse:collapse;\" class=\"border\">
		...
		if($_GET['type'] == 'today' || $_GET['type'] == '')
		{
			if($_SESSION['admin_login'] == 'admin' || $_SESSION['admin_login'] == 'supervisor' || $_SESSION['admin_login'] == 'system')
			{
				$sql_sauda_alloted = "SELECT $group_code, $group_name,SUM(SAL.qty) as qty FROM $sauda_table_value $acronym LEFT JOIN sauda_allocation SAL 
									ON $group_code = SAL.product_filter_code AND SAL.emp_code IN(SELECT emp_code FROM employee_master WHERE 
									reporting_to IN(SELECT emp_code FROM employee_master WHERE reporting_to='' AND acedns!='N') AND acedns!='N') GROUP BY $group_code ORDER BY $group_name ASC";
			}
			else
			{ 
				$sql_sauda_alloted = "SELECT $group_code, $group_name,SUM(SAL.qty) as qty FROM $sauda_table_value $acronym LEFT JOIN sauda_allocation SAL ON $group_code = SAL.product_filter_code AND SAL.emp_code IN(SELECT emp_code FROM employee_master 
								WHERE reporting_to='".$_SESSION['admin_login']."' AND acedns!='N')".$condition_one." GROUP BY $group_code ORDER BY $group_name ASC";
			}
		}
		else
		{
			if($_SESSION['admin_login'] == 'admin' || $_SESSION['admin_login'] == 'supervisor' || $_SESSION['admin_login'] == 'system')
			{
				$sql_sauda_alloted = "SELECT * FROM (SELECT $group_code, $group_name,SAL.qty,SUBSTRING(SAL.allocation_date,1,10) AS allocation_date,SAL.emp_code FROM $sauda_table_value $acronym, sauda_allocation_log SAL 
									WHERE $group_code = SAL.product_filter_code AND SAL.emp_code IN(SELECT emp_code FROM employee_master WHERE 
									reporting_to IN(SELECT emp_code FROM employee_master WHERE reporting_to='' AND acedns!='N') AND acedns!='N')".$condition." ORDER BY SAL.allocation_date DESC) AS SAT GROUP BY 4,5,1 ORDER BY 2 ASC";
			}
			else
			{ 
				$sql_sauda_alloted = "SELECT * FROM (SELECT $group_code, $group_name,SAL.qty,SUBSTRING(SAL.allocation_date,1,10) AS allocation_date,SAL.emp_code FROM $sauda_table_value $acronym, sauda_allocation_log SAL WHERE 
									$group_code = SAL.product_filter_code AND SAL.emp_code IN(SELECT emp_code FROM employee_master 
									WHERE reporting_to='".$_SESSION['admin_login']."' AND acedns!='N'".$condition_three.")".$condition." ORDER BY SAL.allocation_date DESC) AS SAT GROUP BY 4,5,1 ORDER BY 2 ASC";
			}
		}
	$res_sauda_alloted = mysqli_query($link,$sql_sauda_alloted);
	
	while($row_sauda_alloted = mysqli_fetch_assoc($res_sauda_alloted)){
		...
		$sql_quantity_booked = "SELECT SUM(STL.convert_qty_two) AS mt_booked, SUM(STL.qty) AS case_booked FROM sauda_transaction_log STL, product_master PM 
							WHERE PM.$field_name1='$row_sauda_alloted[$field_name1]' $order_condition $emp_hierarchy_condition_one AND STL.prod_code=PM.prod_code";
