	...
	while($row_yellow_card_details = mysqli_fetch_assoc($res_yellow_card_details)){
		...
		if($yellowcard_row_check==$count)
		{
			...
			/*echo "<tr>
				...
				<td align=\"center\"></td>
				";	*/
			if((${'dealer_ach'.$dealer_code}*20)*100 > 0){
			$deler_ach_total=(${'grand_total'.$prev_dealer_code}/(${'dealer_ach'.$dealer_code}*20)*100);
			}
			else $deler_ach_total=0;
			$table_data_dealer_final= ''."\t".''."\t".''."\t".$dealer_name." TOTAL \t".''."\t".''."\t".''."\t".''."\t".''."\t".''."\t".''."\t".''."\t"
				.(${'dealer_ach'.$dealer_code}*20)."\t".round($deler_ach_total,2)."%\t".''."\t".''."\t".''."\t".''."\n";
