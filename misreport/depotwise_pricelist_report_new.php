  ...
  <form name ="frmSearch" method="post" action="<?=$_SERVER['PHP_SELF']?>" >
      ...
      <tr class="TDHEAD_SUB">
		<td align="center">
			<select name="branch" id="branch">
				...
				<?php
					$sql_select_depot = "SELECT * FROM branch_master WHERE acedns='Y' ORDER BY branch_name ASC";
		...
		&nbsp;&nbsp;
			<select name="product_group" id="product_group">
				...
				<?php
					$sql_select_product_group = "SELECT $group_code, $group_name FROM $sauda_table_value $acronym WHERE PGM.acedns='Y' 
	...
	function return_result(branch_code, product_group_code,from_date)
	{
		...
		else
		{
			...
			GenericAjaxFunction('depotwise_pricelist_data_new.php?branch_code='+branch_code+'&product_group_code='+product_group_code+'&from_date='+from_date,'display',0);
		}
	}
