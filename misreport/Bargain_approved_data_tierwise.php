...
if(strtoupper($_SESSION['admin_login'])!='ADMIN')
{
	...
	$rowlevel=mysqli_fetch_assoc($rslevel);
...
if($totalbargaidetails >0){
       ...
       <form name="frm_bargainapprove" method="post" action="adminBargainApprovedTierwise.php" enctype="multipart/form-data" />
		...
		while($rowbargaindetails = mysqli_fetch_assoc($resbargaidetails)){
			...
			$rsbargainstatustwo=mysqli_query($link,$sqlbargainstatustwo);
			$cntbargainstatustwo=mysqli_num_rows($rsbargainstatustwo);
			...
			$resqty = mysqli_query($link,$sqlqty);	
			...
			$resTD = mysqli_query($link,$sqlTD);	
			$rowTD = mysqli_fetch_assoc($resTD);	
			...
			$respremium = mysqli_query($link,$sqlpremium);	
			$rowpremium = mysqli_fetch_assoc($respremium);	
			...
			$respo = mysqli_query($link,$sqlpo);	
			$rowpo = mysqli_fetch_assoc($respo);	
			...
			$resvalidity = mysqli_query($link,$sqlvalidity);	
			$rowvalidity = mysqli_fetch_assoc($resvalidity);	
