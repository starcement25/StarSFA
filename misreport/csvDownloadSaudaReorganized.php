...
<table width="80%" align="center" cellpadding="2" cellspacing="2" border="0">
	<tr>
		<td width="100%" align="left" valign="middle" style="padding:10px;"><strong> Administrator >>Daily Margin Report</strong></td>
	</tr>
...
class ZipFile
{
 ...
 //For sauda csv download
	$arr_header = array('Product Group','Material Description','Material Code','Material Qty(case)','UOM (case)','Material Qty(MT)','Pack Size','State','Customer Code','Customer Name','Route Name','Plant Name','Incoterms','Depot Code','Depot Name','Broker Code','Broker Name','App Contract No','App Contract Time','App Contract Date','Contract Valid From','Contract Valid To','Material Cost','Premium','TD','LTD','Margin Cost','Packing Cost','Honeycomb cost','Brokerage cost','Primary Freight','Secondary Freight','Depot Cost','Detention charges','PR00','FRC1','Sale Rate','Total Value','Payment Due On','CM-Credit Limit','Vertical','Actual Packing Cost','Employee Name','Remarks','Realization Per case','Realization Per MT','Sauda Type','Margin Cost RA');
	$headersauda_header = "";
	...
	$sql_sauda_header_download = "SELECT *,DATE_FORMAT(sauda_date,'%d/%m/%Y') AS sauda_date,DATE_FORMAT(contract_valid_from,'%d/%m/%Y') 
								 ...
								 ".$date_condition.$state_condition.$vertical_condition." AND sauda_type <> 'RAA' 
								 ORDER BY DATE_FORMAT(SUBSTRING(sauda_no,-14,14),'%Y-%m-%d %H:%i:%s') ASC,product_group_name,state  ASC";
	...
	while($rec_sauda_header_download = mysqli_fetch_assoc($rs_sauda_header_download))
	{
		...
		if($margin_cost_RA=='')       $margin_cost_RA=0;
		...
		$rspacksize=mysqli_query($link,$sqlpacksize);
		$rowpacksize=mysqli_fetch_assoc($rspacksize);
		...
		/*$arr_header = array('Product Group','Material Description','Material Code','Material Qty(case)','UOM (case)','Material Qty(MT)','Pack Size','State','Customer Code','Customer Name','Route Name','Plant Name','Incoterms','Depot Code','Depot Name','Broker Code','Broker Name','App Contract No','App Contract Time','App Contract Date','Contract Valid From','Contract Valid To','Material Cost','Premium','TD','LTD','Margin Cost','Packing Cost','Honeycomb cost','Brokerage cost','Primary Freight','Secondary Freight','Depot Cost','Detention charges','PR00','FRC1','Sale Rate','Total Value','Payment Due On','CM-Credit Limit','Vertical','Actual Packing Cost','Employee Name','Remarks','Realization Per case','Realization Per MT','Sauda Type');*/
		$final_amount=$amount;
		...
		$valuesaudaheader .= $margin_cost_RA.",";
	...
	echo $zip->file();*/
	header("Content-type: application/octet-stream");
	header("Content-Disposition: inline; filename=Daily Margin Report.csv");
