...
$res_nicknames = mysqli_query($link,$sql_nicknames);
while($row_nicknames = mysqli_fetch_assoc($res_nicknames)){
...
function star_db_function(){
	...
	$conn = mysqli_connect(SERVERREMOTE,USERREMOTE,PASSWORDREMOTE);
}
function lipl_db_function(){
	...
	$conn = mysqli_connect(SERVERLIPL,USERLIPL,PASSWORDLIPL);
	$val = 'fmpowera_LIPL';
}
...
function insert_update_transaction_count($atd_col_name,$trans_col_name,$total_attend,$total_trans,$n_name,$year_val){
	...
	$res_check_data_exist = mysqli_query($link,$sql_check_data_exist);
	$total_check_rows = mysqli_num_rows($res_check_data_exist);
	...
	mysqli_query($link,$sql_insert_atd);
}
...
foreach($nick_names as $val)
{
	...
	if (mysqli_select_db($val)){
		...
		$res_location_year = mysqli_query($link,$sql_location_year);
		while($row_location_year = mysqli_fetch_assoc($res_location_year)){
			...
			$res_location_month = mysqli_query($link,$sql_location_month);
			while($row_location_month = mysqli_fetch_assoc($res_location_month)){
				...
				$res_total_attend = mysqli_query($link,$sql_total_attend);
				$row_total_attend = mysqli_fetch_assoc($res_total_attend);
				...
				$res_total_trans = mysqli_query($link,$sql_total_trans);
				$row_total_trans = mysqli_fetch_assoc($res_total_trans);
