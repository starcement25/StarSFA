	...
	{*/
		...
		while($row_select_date = mysqli_fetch_assoc($res_select_date))
		{
			...
			while($row_product = mysqli_fetch_assoc($res_product))
			{
				...
				$sql_quantity_booked = "SELECT SUM(STL.convert_qty_two) as mt_booked, SUM(STL.qty) as case_booked FROM sauda_transaction_log STL, product_master PM WHERE STL.customer_code='".$customer_code."' AND STL.prod_code=PM.prod_code AND PM.$field_name1='".$row_product[$field_name1]."' AND DATE_FORMAT(SUBSTRING(STL.sauda_no,-14,8),'%d-%m-%Y') = '".$sauda_date."'";
