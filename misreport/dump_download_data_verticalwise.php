...
$foldername=substr(strtoupper($_SESSION['nick_name']),0,5);

	/*$sqllowerleaves="SELECT lower_leaves FROM employee_master WHERE emp_code='".$_SESSION['admin_login']."'";
	$rslowerleaves=mysqli_query($link,$sqllowerleaves);
	$rowlowerleaves=mysqli_fetch_assoc($rslowerleaves);
...
CM.incoterms,CM.loadability_ton,CM.transport_mode,CM.sauda_type,CM.zone FROM customer_master CM WHERE CM.acedns='Y' AND CM.emp_code IN(".$lowerleaves.") AND CM.cust_type='D' ORDER BY CM.dns_customer_code ASC";
	...
	if($total_customer > 0){
		...
		"Loadability".","."Transport mode".","."Sauda Type".","."Zone";
		//$res_prev_order_counting_master = mysqli_query($link,$sql_prev_order_counting_master);
		while($row_customer = mysqli_fetch_assoc($res_customer)){
			...
			.",".$transport_mode.",".$sauda_type.",".$zone."\n";
			$count++;
		}
		if ( !file_exists("../dump/$foldername")){
			mkdir("../dump/$foldername");
...
if($attribute=='freight')
{
	...
	FIND_IN_SET(BM.branch_code,'".$branchcodelist."') AND BRF.vertical_value='".$_SESSION['vertical_value']."' ORDER BY BRF.date DESC) alias GROUP BY 1,2,3 ORDER BY 3 DESC";
	$resfreight = mysqli_query($link,$sqlfreight);
	$total_freight = mysqli_num_rows($resfreight);
	if($total_freight > 0){
		$header_freight = "Depot code".","."Route".","."Freight".","."acedns".","."date".","."Transport mode".","."Capacity".","."State code";
		//$res_prev_order_counting_master = mysqli_query($link,$sql_prev_order_counting_master);
		while($row_freight = mysqli_fetch_assoc($resfreight)){
