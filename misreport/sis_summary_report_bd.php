...
function main(){
	...
	$sql_month_selection = "SELECT 
    SUBSTRING(trans_id, -14, 6) AS distinct_datetime
FROM customer_visit_details
WHERE DATE_FORMAT(SUBSTRING(trans_id, -14, 8), '%Y-%m-%d') > '".$previous_year_date."'
GROUP BY distinct_datetime
ORDER BY ANY_VALUE(SUBSTRING(trans_id, -14, 8)) ASC";
	...
	echo "<span style=\"font-weight:bold; font-size:14px;\">SIS SUMMARY REPORT FOR BD</span><br><br>";
	...
	function display_result(){
		...
		GenericAjaxFunction('sis_summary_report_bd_data.php?employee='+employee+'&month_data='+month_data+'&zone='+zone+'&state='+state+'&branch='+branch+'&department='+department,'display',0);
	...
	function Popup(data) 
	{
		var mywindow = window.open('', 'SIS Summary Report FOR BD', 'height=400,width=600');
		mywindow.document.write('<html><head><title>SIS Report FOR BB</title>');
	...
	function exporttocsv(divid)
	{
		...
		a.download = 'SIS Summary Report FOR BD' + postfix + '.xls';
