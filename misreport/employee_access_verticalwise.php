...
//ob_start();
	...
	|| strtoupper($_SESSION['admin_login'])=='SYSTEM' || strtoupper($_SESSION['admin_login'])=='E1093' || strtoupper($_SESSION['admin_login'])=='E0076' || strtoupper($_SESSION['admin_login'])=='GMSFATS' ||  strtoupper($_SESSION['admin_login'])=='E0042'){
...
<table width="100%" style="border-collapse:collapse;">
  <tr style="font-weight:bold;" class="TDHEAD_SUB">
    ...
    <td style="width:10%;" align="center">SAP Emp Code</td>
    <td style="width:10%;" align="center">Emp Code</td>
    <td style="width:13%;" align="center">Emp Name</td>
     <td style="width:10%;" align="center">Phone No</td>
    <td style="width:10%;" align="center">Headquarter</td>
    <td style="width:10%;" align="center">Location</td>
    <td style="width:10%;" align="center">Designation</td>
    <td style="width:13%; word-wrap:break-word;" align="center">Provide Access</td>
    <td style="width:12%;" align="center">Clear Allocation</td>
  </tr>
</table>
</div>
</div>
...
while($row_vertical = mysqli_fetch_assoc($res_vertical))
{
	...
	if(!in_array($vertical_name,$vertical_array))
	{
		...
		echo "<tr class=\"TDHEAD_SUB\" align=\"center\"><td colspan=\"10\">".$vertical_name."</td></tr>";
		$excel_header .= "<tr><td colspan=\"10\" align=\"center\" style=\"font-weight:bold;\">".$vertical_name."</td></tr>";
	}
	
	$sql_employee_access = "SELECT EM.emp_code, EM.dns_emp_code, EM.emp_name, EM.HQ, EM.designation, EM.branch_code,EM.phone_no FROM employee_master EM 
							WHERE FIND_IN_SET('".$distinct_vertical_value."',EM.vertical_value) AND EM.acedns = 'Y' ".$emp_hierarchy_condition."  ORDER BY EM.emp_code ASC";
	$res_employee_access = mysqli_query($link,$sql_employee_access);
	while($row_employee_access = mysqli_fetch_assoc($res_employee_access))
	{
		...
		$res_device_id = mysqli_query($link,$sql_device_id);
		$row_device_id = mysqli_fetch_assoc($res_device_id);
		...
		<td style=\"width:10%\">$row_employee_access[dns_emp_code]</td>
		<td style=\"width:10%\">$row_employee_access[emp_code]</td>
		<td style=\"width:13%\">$row_employee_access[emp_name]<a href='change_emp_name.php?emp_code=$row_employee_access[emp_code]'>&nbsp;&nbsp;<img src='fileedit.png'></a></td>
		<td style=\"width:10%\">$row_employee_access[phone_no]</td>
		<td style=\"width:10%\">$row_employee_access[HQ]</td>
		<td style=\"width:10%\">$branch_name</td>
		<td style=\"width:10%\">$row_employee_access[designation]</td>
		<td style=\"width:13%\"><a href=\"employee_access_verticalwise.php?emp_code=$row_employee_access[emp_code]&emp_name=$row_employee_access[emp_name]&provide=access\" style=\"color:blue;\">Provide Access</a></td>
		<td style=\"width:12%\">$deviceid</td>
	  </tr>";
	  $excel_header .= "
		<tr>
		<td>$count</td>
		...
		<td>$row_employee_access[phone_no]</td>
...
function getdata()
{
	...
	if($_GET['emp_code'] && $_GET['allocation'] == "clear")
	{
		...
		if(mysqli_query($link,$sql_clear_allocation))
		{
			main();
	...
	else if($_GET['emp_code'] && $_GET['emp_name'] && $_GET['provide'] == 'access')
	{
		...
		$res_provideaccess = mysqli_query($link,$sql_provideaccess);
		$row_provideaccess = mysqli_fetch_assoc($res_provideaccess);
...
function postdata()
{
	...
	if($_POST['submit'] == 'Change')
	{
		...
		if(mysqli_query($link,$sql_update_access))
			{
