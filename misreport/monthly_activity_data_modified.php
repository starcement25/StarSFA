...
/*----> Total Calls & Days Present <----*/
	...
	SUM(CASE WHEN POCM.order_no LIKE 'O%' AND POCM.cust_type='D' THEN POCM.amount ELSE 0 END) AS primary_amount,
	SUM(CASE WHEN POCM.order_no LIKE 'O%' AND POCM.cust_type='R' THEN POCM.amount ELSE 0 END) AS secondary_amount  
	FROM location LO LEFT JOIN `prev_order_counting_master` POCM ON POCM.order_no=LO.trans_id WHERE SUBSTRING(LO.trans_id,1,1) IN('A','N','O') AND SUBSTRING(LO.trans_id,-14,4)='".$year."' AND SUBSTRING(LO.trans_id,-10,2)='".$monthvalue."' ".$employee_select_condition." GROUP BY LO.emp_code";
		}
		//exit();
		...
		if($total_rows>0){
			if(stk_audit=='yes' || retailer_care=='yes'){
				...
				$stk_audit_customer_TR_sub='<td></td>';
			}
			else
			{
			...
			?><table width='100%' class='border' border='1' style='border-collapse:collapse;' cellpadding='6px'>
              <tr class="TDHEAD_SUB">
                ...
                <td>Productive <br />Calls %</td>
                <td>LPPC</td>
                <td colspan="2" align="center">Value</td><?php echo $stk_audit_customer_TR;?>
              </tr>
              <tr class="TDHEAD_SUB">
                <td></td>
                <td></td>
                ...
                <td></td>
                <td></td>
                <td>Primary</td><td>Secondary</td><?php echo $stk_audit_customer_TR_sub;?>
              </tr>
            <?php
			$res_total_calls = mysqli_query($link,$sql_total_calls);
			while($row_total_calls = mysqli_fetch_assoc($res_total_calls)){
				...
				echo "<tr>
						...
						<td align=\"right\">".number_format($primary_amount,2)."</td><td align=\"right\">".number_format($secondary_amount,2)."</td>".$stk_audit_customer_TD."
					  </tr>";
				$count++;
			}
			?>
