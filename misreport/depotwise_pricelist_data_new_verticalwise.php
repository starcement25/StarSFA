...
$product_group_code_single=str_replace("'","",$product_group_code);
...
//$from_date_final=$from_date_check;
...
$res_employee_branch_code = mysqli_query($link,$sql_employee_branch_code);
$total_emp_rows = mysqli_num_rows($res_employee_branch_code);
}*/
//else
$total_emp_rows = 1;
...
?>
	<table class="border" width="100%" border="1" style="border-collapse:collapse;" cellpadding="5px" align="center">
      ...
      <?php
		...
		$res_select_branch_name = mysqli_query($link,$sql_select_branch_name);
		while($row_select_branch_name = mysqli_fetch_assoc($res_select_branch_name))
		{
			...
			product_group_code IN ($product_group_code) AND branch_code='".$branch_code."' AND acedns='Y'  ORDER BY prod_desc ASC";
	 		$res_dnsproduct_details = mysqli_query($link,$sql_dnsproduct_details);
			while($row_dnsproduct_details=mysqli_fetch_assoc($res_dnsproduct_details))
			{
		...
		{
			$branch_code='';
			if($product_group_code_single!='BR9')
			{
			...
			$rspricingbranch=mysqli_query($link,$sqlpricingbranch);
			$cntpricingbranch=mysqli_num_rows($rspricingbranch);
			...
			{
				while($rowpricingbranch=mysqli_fetch_assoc($rspricingbranch))
				{
			...
			else{ //For formulation yes
			$sqlpricingbranch="SELECT branch_code,branch_name,plant_name FROM branch_master WHERE plant_name IN(SELECT DISTINCT plant_name FROM 
							pricing_detials_formulation WHERE SUBSTRING(datetime,1,10)='".$from_date_check."')";
			$rspricingbranch=mysqli_query($link,$sqlpricingbranch);
			$cntpricingbranch=mysqli_num_rows($rspricingbranch);
			...
			else 
			{
				while($rowpricingbranch=mysqli_fetch_assoc($rspricingbranch))
				{
					...
					$plant_name=$rowpricingbranch['plant_name'];
					$sql_dnsproduct_details = "SELECT prod_desc,dns_prod_code,product_group_code FROM product_master WHERE 
					product_group_code IN (SELECT DISTINCT product_group_code FROM pricing_detials_formulation 
					WHERE SUBSTRING(datetime,1,10)='".$from_date_check."' 
					AND plant_name='".$plant_name."' AND product_group_code IN ($product_group_code)) 
					AND branch_code='".$branch_code."' ORDER BY prod_desc ASC";
					$res_dnsproduct_details = mysqli_query($link,$sql_dnsproduct_details);
					while($row_dnsproduct_details=mysqli_fetch_assoc($res_dnsproduct_details))
					{
						$dns_prod_code=$row_dnsproduct_details['dns_prod_code'];
						...
						$sqlprocesscost="SELECT process_cost FROM process_cost WHERE dns_prod_code='".$dns_prod_code."' 
						...
						${process_cost.$dns_prod_code}=$rowprocesscost['process_cost'];
						...
						$sqlprodwiseformulation="SELECT * FROM(SELECT prod_code,formulation,oils FROM loose_oilrate_formulation 
										WHERE product_group_code='".$product_group_code_val."' AND plant_name='".$plant_name."' 
										AND prod_code='".$dns_prod_code."' AND datetime <='".$from_date_final."' ORDER BY datetime DESC) AS SAT GROUP BY 3 ";
						...
						{					
							$sqllooserate="SELECT oils_rate FROM pricing_detials_formulation WHERE product_group_code='".$product_group_code_val."' AND 
						 ...
						 ${loosrate_calc_val.$dns_prod_code}=(substr($rowprodwiseformulation['formulation'],0,-1)*$oils_rate_ton)/100;
						 ${loosrate_final_val.$dns_prod_code}=${loosrate_final_val.$dns_prod_code}+${loosrate_calc_val.$dns_prod_code};
						}
						${loosrate_final_val.$dns_prod_code}=${loosrate_final_val.$dns_prod_code}+${process_cost.$dns_prod_code};
						if(!in_array($dns_prod_code, $dns_prod_code_array)){
							array_push($dns_prod_code_array,$dns_prod_code);
							array_push($prod_desc_array,$prod_desc);
							array_push($loose_rate_ton_array,${loosrate_final_val.$dns_prod_code});
						}
						if(!in_array($branch_code, $branch_code_array)){
						echo "<td  align=\"left\" style=\"	FONT-FAMILY: Verdana;FONT-SIZE : 11px;FONT-WEIGHT: bold;COLOR: #000000;BACKGROUND-COLOR: #DCE6F1;\">".$rowpricingbranch['branch_name']."</td>";
							array_push($branch_code_array,$branch_code);
		...
		{
			if($product_group_code_single!='BR9'){ //For formulation no
			...
			{
				$rowpricingbranch=mysqli_fetch_assoc($rspricingbranch);
				$branch_code=$rowpricingbranch['branch_code'];
				$plant_name=$rowpricingbranch['plant_name'];
				$sql_dnsproduct_details = "SELECT prod_desc,dns_prod_code,product_group_code FROM product_master WHERE 
						product_group_code IN (SELECT DISTINCT product_group_code FROM pricing_detials WHERE SUBSTRING(datetime,1,10)='".$from_date_check."' 
			...
			{
			$sqlpricingbranch="SELECT branch_code,plant_name,branch_name FROM branch_master WHERE branch_code IN(".$branch_code.") AND plant_name 
								IN(SELECT plant_name FROM pricing_detials_formulation WHERE SUBSTRING(datetime,1,10)='".$from_date_check."')";
			$rspricingbranch=mysqli_query($link,$sqlpricingbranch);
			$cntpricingbranch=mysqli_num_rows($rspricingbranch);
			if($cntpricingbranch==0)
			{
			...
			else
			{
				while($rowpricingbranch=mysqli_fetch_assoc($rspricingbranch))
				{
					...
					while($row_dnsproduct_details=mysqli_fetch_assoc($res_dnsproduct_details))
					{
						...
						//echo ${loosrate_final_val.$dns_prod_code};
						${loosrate_final_val.$dns_prod_code}=${loosrate_final_val.$dns_prod_code}+${process_cost.$dns_prod_code};
						if(!in_array($dns_prod_code, $dns_prod_code_array)){
							array_push($dns_prod_code_array,$dns_prod_code);
							array_push($prod_desc_array,$prod_desc);
		...
		{
			echo "<tr><td>".$prod_desc_array[$i]." - ".$dns_prod_code_array[$i]."</td>";
			foreach($branch_code_array as $branch_code_values){
				
				$sqlplantname="SELECT plant_name FROM branch_master WHERE branch_code ='".$branch_code_values."'";
				$rsplantname=mysqli_query($link,$sqlplantname);
				$rowplantname=mysqli_fetch_assoc($rsplantname);
				$plant_name=$rowplantname['plant_name'];
				$sql_product_details = "SELECT prod_code,conversion_factor,conversion_factor_two,acedns FROM product_master WHERE 
										dns_prod_code='".$dns_prod_code_array[$i]."' AND branch_code='".$branch_code_values."' 
										ORDER BY prod_code DESC LIMIT 0,1";
				$res_product_details = mysqli_query($link,$sql_product_details);
				$row_product_details = mysqli_fetch_assoc($res_product_details);
				...
				if($from_date=='')
				{
					...
					$rs_rate=mysqli_query($link,$sql_rate);
					$row_rate=mysqli_fetch_assoc($rs_rate);
					$rate=$row_rate['sale_rate'];
					//${final_rate.$dns_prod_code_array[$i]}=$rate/$no_pc_one_case;
					${final_rate.$dns_prod_code_array[$i]}=$rate;
				}
				else
				{
					$sqlpackingprodwise="SELECT packing_cost,no_pc_one_case FROM packing_master WHERE dns_prod_code='".$dns_prod_code_array[$i]."' 
										AND plant_name='".$plant_name."' AND datetime <='".$from_date_final."' ORDER BY datetime DESC LIMIT 0,1";
					$rspackingprodwise=mysqli_query($link,$sqlpackingprodwise);
					$rowpackingprodwise=mysqli_fetch_assoc($rspackingprodwise);
					...
					${mrp_prodwise.$dns_prod_code_array[$i]}=round(${mrp_prodwise.$dns_prod_code_array[$i]},2);
					//${final_rate.$dns_prod_code_array[$i]}=${mrp_prodwise.$dns_prod_code_array[$i]}/${no_pc_one_case.$dns_prod_code_array[$i]};
					${final_rate.$dns_prod_code_array[$i]}=${mrp_prodwise.$dns_prod_code_array[$i]};
				}
				//if(${final_rate.$dns_prod_code_array[$i]} > 0){
	...
	echo "</table>";
	/*if(count($branch_code_array) > 1)
	{
		echo "<table><tr><td align=\"left\"><b>N Denotes For Discontinued Product</b></td></tr></table>";
	}*/
/*else
{
