...
function main()
{
	$hidden = " hidden";
	echo "<center>";
	echo "<span style=\"font-weight:bold; font-size:14px;\">MIS REPORT</span><br><br>";
	...
	attribute_selection_emphierarchy($hidden);
	echo "</td><td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td></tr></table>";
	echo "<br>"
?>
<script language="javascript" type="text/javascript">
function display_result_emphierarchy(){
	//alert("Break Free");
	...
	/*if(document.getElementById("employee_lev_two").value.search(/\S/) == -1){
		alert("Please select TSI.");
		return false;
	}
	if(document.getElementById("employee_lev_three").value.search(/\S/) == -1){
		alert("Please select DSM.");
		return false;
	}*/
	
	document.display_mis_report.submit();
}

</script>
<?php
	if($_REQUEST['modehierarchy']=='displayreport')
	{	
	...
	$emp_hierarchy_value=return_employee_hierarchy_sate_commaseperated($employee_lev_one,$state);
	$emp_hierarchy_value_condition = " WHERE emp_code IN(".$emp_hierarchy_value.") ";
	$emp_hierarchy_fieldforce=str_replace(",'".$_SESSION['admin_login']."'",'',$emp_hierarchy_value);
	$emp_hierarchy_condition=' AND LO.emp_code IN('.$emp_hierarchy_value.')';
	$emp_hierarchy_condition_one=' AND EM.emp_code IN('.$emp_hierarchy_value.')';
	$emp_hierarchy_condition_two=' AND SUBSTRING(order_no,-19,5) IN('.$emp_hierarchy_value.')';
	$emp_hierarchy_condition_three=' AND SUBSTRING(receipt_id,-19,5) IN('.$emp_hierarchy_value.')';
	$emp_hierarchy_condition_four=' AND EM.emp_code IN('.$emp_hierarchy_value.')';
?>	
...
{
	document.getElementById('loader').style.display='';
	var empHierarchyVal="<?=$emp_hierarchy_value?>";
	var state=document.getElementById('state').value;
	var emp_type=document.getElementById('emp_type').value;
	var employee_lev_one=document.getElementById('employee_lev_one').value;
	
	//alert(empHierarchyVal);
	...
	if(val=='T')
	{
	 // var url="showAttendanceActivity.php?val="+val+"&emphierarchyval="+empHierarchyVal+"&state="+state+"&emp_type="+emp_type+"&employee_lev_one="+employee_lev_one+"&page=misreporthierarchy&modehierarchy=displayreport";
	 var url="showAttendanceActivity.php";
	  var params = "val="+val+"&emphierarchyval="+empHierarchyVal+"&state="+state+"&emp_type="+emp_type+"&employee_lev_one="+employee_lev_one+"&page=misreporthierarchy&modehierarchy=displayreport";
	}
	if(val=='MTD' || val=='YTD')
	{
	  var url="showAttendanceActivityMonthly.php";
	 // var url="showAttendanceActivityMonthly.php?val="+val+"&emphierarchyval="+empHierarchyVal+"&state="+state+"&emp_type="+emp_type+"&employee_lev_one="+employee_lev_one+"&page=misreporthierarchy&modehierarchy=displayreport";
	  var params = "val="+val+"&emphierarchyval="+empHierarchyVal+"&state="+state+"&emp_type="+emp_type+"&employee_lev_one="+employee_lev_one+"&page=misreporthierarchy&modehierarchy=displayreport";
	}
	xmlHttpNext.open("POST", url, true);
	//Send the proper header information along with the request
	xmlHttpNext.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
	
	xmlHttpNext.onreadystatechange = function() {//Call a function when the state changes.
                    ...
                    <?php
						...
						for($i=1;$i<=3;$i++)
						{
							...
							if($i==2)
							{
								...
								$payment_header_date_cond = " SUBSTRING(receipt_id,-14,6) = '".$current_month."' ";
							}
    ...
    <tr class="TDHEAD" > 
     ...
     <tr class="TDHEAD_SUB"> 
		...
		$sqlinformation="SELECT EM.emp_name,EM.emp_code,LO.trans_id,count(LO.trans_id) AS no_of_visit FROM 
						...
						('O','P','S') OR SUBSTRING(trans_id,1,2) IN('NO','NC')) AND SUBSTRING(trans_id,1,2) NOT IN('PA')  
        ...
        while($rowinformation=mysqli_fetch_assoc($resinformation))
        {
			...
			|| strtoupper($_SESSION['nick_name'])=='LTS' || strtoupper($_SESSION['nick_name'])=='EMARK' || strtoupper($_SESSION['nick_name'])=='OJBH' || strtoupper($_SESSION['nick_name'])=='JPHARMA' || strtoupper($_SESSION['nick_name'])=='STAR' || strtoupper($_SESSION['nick_name'])=='MINU' || strtoupper($_SESSION['nick_name'])=='PARLE' || strtoupper($_SESSION['nick_name'])=='HALDIRAM' || strtoupper($_SESSION['nick_name'])=='SKIPPER') 
			&& $mode=='T'){
				  ...
				  if($mode!='T' && !in_array($rowtrans['date'],$date_array))
					{
						...
						|| strtoupper($_SESSION['nick_name'])=='JPHARMA' || strtoupper($_SESSION['nick_name'])=='STAR' || strtoupper($_SESSION['nick_name'])=='MINU')
						{
