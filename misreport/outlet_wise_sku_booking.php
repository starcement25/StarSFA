...
<center>
	<span style="font-weight:bold; font-size:14px;">Outlet Wise SKU Booking</span><br><br>
    <table class="border" width="40%" style="border-collapse:collapse;" cellpadding="6px">
      <tr class="TDHEAD_SUB">
        ...
        <select id="product" >
        	<option value="">Select</option>
        <?php
		$sql_product = "SELECT prod_code, prod_desc FROM product_master WHERE acedns = 'Y' AND prod_code 
		IN(SELECT DISTINCT product_code FROM prev_order_counting_master) ORDER BY prod_desc ASC";
	...
	function display_result(){
		...
		GenericAjaxFunction('outlet_wise_sku_booking_data.php?employee_id='+employee_id+'&product_id='+product_id,'display',0);
	}
	...
	function Popup(data) 
	{
		var mywindow = window.open('', 'Outlet Wise SKU Booking', 'height=400,width=600');
...
function exporttocsv(){
	...
	a.download = 'Outlet Wise SKU Booking' + postfix + '.xls';
