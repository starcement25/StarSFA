...
function main()
{
	...
	if(strtoupper($_SESSION['nick_name'])=='RKBK')
	{
		...
		(EM.emp_code IN(SELECT DISTINCT reporting_to FROM employee_master WHERE reporting_to <> '') OR EM.emp_code IN(SELECT emp_code FROM customer_master GROUP BY emp_code)) ".$emp_hierarchy_condition_one." ORDER BY EM.emp_name ASC LIMIT ".$GLOBALS[start].",".$GLOBALS[show];
		$row=mysqli_fetch_assoc(mysqli_query($link,"SELECT COUNT(EM.emp_code) FROM employee_master EM,changepassword CH WHERE CH.emp_code=EM.emp_code 
				AND (EM.emp_code IN(SELECT DISTINCT reporting_to FROM employee_master WHERE reporting_to <> '') OR EM.emp_code IN(SELECT emp_code FROM customer_master GROUP BY emp_code)) ".$emp_hierarchy_condition_one." ORDER BY EM.emp_name ASC"));
	}
	else
	{
	$sql="SELECT EM.* FROM employee_master EM,changepassword CH WHERE CH.emp_code=EM.emp_code AND 
			...
			ORDER BY EM.emp_name ASC LIMIT ".$GLOBALS[start].",".$GLOBALS[show];
			
	$row=mysqli_fetch_assoc(mysqli_query($link,"SELECT COUNT(EM.emp_code) FROM employee_master EM,changepassword CH WHERE CH.emp_code=EM.emp_code 
		AND (FIND_IN_SET(EM.emp_code,(SELECT GROUP_CONCAT(reporting_to) FROM employee_master WHERE reporting_to <> '')) OR EM.emp_code IN(SELECT emp_code FROM customer_master GROUP BY emp_code)) AND EM.emp_code<>'C0007' ORDER BY EM.emp_name ASC"));
	}
	$count=$row[0];
	//echo $sql;
	...
	<tr>
		<td valign="top" bgcolor="#FFFFFF">		
			...
			<table width="70%" align="center" border="0" cellpadding="5" cellspacing="2" class="border">
				<tr class="TDHEAD" > 
					<td colspan="8">Employee Information</td>
				</tr>
			<? 
			if($count == 0)
			{ 
...
{
		$sqlemp="SELECT emp_name FROM employee_master WHERE emp_code = '".$row_id."'";
		...
		<td valign="top" bgcolor="#FFFFFF">
			...
			<table width="50%" align="center" class="border" cellpadding="5" cellspacing="2">
                ...
                <tr>
                    ...
                    <option value="true"  <?php if($status=='true'){ echo 'selected';}?>>unblocked</option>
                    <option value="false" <?php if($status=='false'){ echo 'selected';}?>>blocked</option>
                    </select>
                    </td>
				</tr>
				<tr>
...
{
	$emp_code = $_REQUEST['row_id'];
	$upd_sql="UPDATE changepassword SET deviceid=''
			  WHERE emp_code = '" .$emp_code."'";
