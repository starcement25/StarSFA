...
function Popup(data) 
{
	var mywindow = window.open('', 'Productgroupwise Sauda Report', 'height=400,width=600');
	mywindow.document.write('<html><head><title>Productgroupwise Sauda Report</title>');
...
function exporttocsv(divid)
{
	//alert(divid);
        ...
        a.download = 'Productgroupwise Sauda Report' + postfix + '.xls';
...
<br>
<div style="width:40%;" align="right"><input name="print" type="button" value="Print" id="print" onClick="PrintElem('#display');">&nbsp;
...
function show_employeewise()
{
	...
	if(type != 'custom')
		GenericAjaxFunction('product_groupwise_report_data.php?type='+type,'display',0);
	else 
	{
		if(response1 != 0 && response2 != 0)
			GenericAjaxFunction('product_groupwise_report_data.php?type='+type+'&start_date='+start_date+'&end_date='+end_date,'display',0);
	}
}
