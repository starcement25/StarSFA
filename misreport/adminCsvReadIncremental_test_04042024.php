...

						{
...

							{
...

								{
									...
									modifyempdatadownloadlog1($emp_code_branchwise,strtoupper($folderName));
...

			  {
				...
				{ 
				   ...
				   //$data[]=$value;
					...
					$appointment_date=trim($data[40]);
					$appointment_date = preg_replace('/[[:^print:]]/', '', $appointment_date);
...
//echo "y1-". $year;
					...
					//echo "-apd-".$appointment_date;exit();
...
echo "dob--". $year;//exit();
					...
					if(strpos($date_of_anniversary,'-')!=false){
					 ...
					 $date_of_anniversaryArr=explode('-',$date_of_anniversary);
...

						{
							...
							$date_days=$date_of_anniversaryArr[0];
						...
						//echo "gg". $date_of_anniversary;exit();
                        }
					...
					else
					{
						...
						//$sqlempcode = str_replace("�"," ",$sqlempcode);
						//$sqlempcode = str_replace(" "," ",$sqlempcode);
						//echo removeSpecialChars($sqlempcode);
						//echo html_entity_decode($sqlempcode);
						//$sqlempcode = htmlspecialchars($sqlempcode, ENT_COMPAT, 'utf-8');
						$sqlempcode = preg_replace('/[[:^print:]]/', '', $sqlempcode);
						echo "--".$sqlempcode;
						//echo filter_var($sqlempcode, FILTER_SANITIZE_STRING);
						//echo htmlspecialchars($sqlempcode,ENT_COMPAT,'UTF-8',TRUE);
						$rsempcode=mysqli_query($link,$sqlempcode);
						$rowempcode=mysqli_fetch_assoc($rsempcode);
						$emp_code=$rowempcode['emp_code'];
...
echo "qqqqq".$sqlroutechk;
                    ...
                    $customer_name = html_entity_decode($customer_name, ENT_QUOTES, 'UTF-8');
                    //$customer_name = strToHex($customer_name);
                    echo $customer_name;
