...
<table width="100%" border="1" style="border-collapse:collapse;">
  <tr>
  	<td colspan="<?php echo (($total_product*2)+2); ?>" class="TDHEAD" align="center"><b>Plantwise <?php echo $sauda_duration; ?></b></td>
  </tr>
  <tr class="TDHEAD_SUB">
  	<td align="center"><b>SI</b></td>
    <td align="center"><b>Plant Name</b></td>
    <?php
	$res_count_product = mysqli_query($link,$sql_count_product);
	while($row_count_product = mysqli_fetch_assoc($res_count_product))
...
{
  $sql_depot = "SELECT BM.branch_code, BM.branch_name, BM.plant_name FROM branch_master BM, sauda_details SD, sauda_header SH WHERE SD.sauda_no=SH.sauda_no AND BM.branch_code=SH.branch_code AND substring(SD.sauda_no,3,5)='".$row_emp['emp_code']."' ".$sauda_booked_condition." GROUP BY BM.plant_name";
  $res_depot = mysqli_query($link,$sql_depot);
  while($row_depot = mysqli_fetch_assoc($res_depot))	
  {
	  $branch_name = $row_depot['branch_name'];
	  //$branch_code = $row_depot['branch_code'];
	  $plant_name = $row_depot['plant_name'];
	  
	  $sql_branch_code = "SELECT branch_code FROM branch_master WHERE plant_name LIKE '%".$plant_name."%'";
	  $res_branch_code = mysqli_query($link,$sql_branch_code);
	  while($row_branch_code = mysqli_fetch_assoc($res_branch_code)){
		  $branch_code = $row_branch_code['branch_code'];
	  				
		  $sql_product = "SELECT $group_name, $group_code FROM $sauda_table_value $acronym ".$emp_cond." AND PGM.product_group_code 
		  				IN(SELECT DISTINCT PM.product_group_code FROM product_master PM,sauda_details SD WHERE PM.prod_code=SD.sku_code 
						   $vertical_val_condition_one  $sauda_booked_condition)ORDER BY $group_name ASC";
		  ...
		  while($row_product = mysqli_fetch_assoc($res_product))
		  {
			   ...
			   if($row_quantity_booked['case_booked'] != '')
				...
				$quantity_booked_case = 0;
					...
					$branchwise[$plant_name][$product_group_value] += $quantity_booked;
					$branchwise_case_booked[$plant_name][$product_group_value] += $quantity_booked_case;
...
foreach($branchwise as $branchwiseindex=>$branchwisevalue)
{
	...
	echo "<tr>
			<td>$count</td>
			<td><a href=\"#\" style=\"color:blue;\" onclick=\"show_plant_branch('$branchwiseindex', '$value', '$start_date','$end_date');\">".$branchwiseindex."</a></td>";
	foreach($branchwisevalue as $index=>$val)
	{
		...
		{
		   echo "
				<td align=\"right\" style=\"background:$color;\">".$val."</td>";
		   echo "<td align=\"right\" style=\"background:$color;\">".$branchwise_case_booked[$branchwiseindex][$index]."</td>";
		}
		else
		{
		   echo "
