...
ob_start();
	...
	|| strtoupper($_SESSION['nick_name']) == 'GOLDSTONET')
	{
		require("adminUtils.php");
	}
	else
...
<script type="text/javascript" src="ajax1.js"></script>
	...
	$sql="SELECT CM.customer_code,CM.customer_name,EM.emp_name,CM.cust_type,CM.rds_tag,CM.phone_no,CRR.route_code,CM.state_code,CM.image,RM.route_name,(SELECT customer_name FROM customer_master CMB WHERE CMB.customer_code = CM.rds_tag) AS rds_tag,(SELECT COUNT(phone_no) FROM customer_master CMD WHERE CMD.phone_no = CM.phone_no) AS phone_no_count
	 ...
	 /*$row=mysqli_fetch_assoc(mysqli_query($link,"SELECT COUNT(DISTINCT CM.customer_code) FROM customer_master CM,customer_route_emp_relation CRR,employee_master EM 
		...
		<td valign="top" bgcolor="#FFFFFF">		
			...
			?>
				<tr class="TDHEAD_SUB"> 
                    ...
                    <?php if((strpos(geo_fencing_menu,'add_customer')!=false) || add_customer_image_creation == 'yes') {?>
				...
				while($rec=mysqli_fetch_assoc($rs))
				{
					...
					$rsselphonecount=mysqli_query($link,$sqlselphonecount);
					$cntselphonecount=mysqli_num_rows($rsselphonecount);
...
function activate_customer($row_id = '')
{
	...
	mysqli_query($link,$sqlupdatecustomer) or die(mysqli_error()." Error in customer activation.");
	$sqlupdatecustomerroute="UPDATE customer_route_emp_relation SET acedns='Y',download_time=CURRENT_TIMESTAMP() 
							WHERE customer_code='".$row_id."'";
	mysqli_query($link,$sqlupdatecustomerroute) or die(mysqli_error()." Error in customer route emp activation.");
...
require("include/config-setup.php");
   ...
   if($rowcustomerdetails['minimum_stock']==""){$minimum_stock="0";}
		...
		mysqli_query($link,$sqldeletecustomer) or die(mysqli_error()." Error in customer deactivation.");
		$sqldeletecustomerroute="DELETE FROM customer_route_emp_relation WHERE customer_code='".$row_id."'";
		mysqli_query($link,$sqldeletecustomerroute) or die(mysqli_error()." Error in customer route emp deactivation.");
