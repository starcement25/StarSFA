...
$requisition_type = $_REQUEST['requisition_type'];
...
$count = 1;
	...
	$res_get_display_menunew = mysqli_query($link,$sql_get_display_menunew);
	$count_display_menunew=mysqli_num_rows($res_get_display_menunew);
	$menu_id_new_array=array();
	while($row_get_display_menunew = mysqli_fetch_assoc($res_get_display_menunew))
	{
	...
	$sql_get_display_menuexisting="SELECT display_name,row_id,action,display_order FROM survey_input WHERE  type!='menu' AND menu_id='RA002'  AND acedns='Y'
					   ORDER BY display_order ASC";
	$res_get_display_menuexisting = mysqli_query($link,$sql_get_display_menuexisting);
	$count_display_menuexisting=mysqli_num_rows($res_get_display_menuexisting);
	$menu_id_existing_array=array();
	while($row_get_display_menuexisting = mysqli_fetch_assoc($res_get_display_menuexisting))
	{
		array_push($menu_id_existing_array,$row_get_display_menuexisting['row_id']);
	}
	
	$survey_site_menu_id_array=array('RA046');
	  ...
	  while($row_survey_output=mysqli_fetch_assoc($rs_survey_output))
	  {
		...
		if(count($survey_site_menu_id_array)>0){
			?><table width='100%' class='border' border='1' style='border-collapse:collapse;' cellpadding='6px'>
              <tr class="TDHEAD_SUB">
                <td width="4%">SL No</td>
                 <td width="10%">Date Done</td>
                <td width="10%">Executive Name</td>
                 <td width="5%">Designation</td>
                  <td width="5%">GSB Size</td>
                <td width="5%">NLB Size</td>
                <td width="5%">In Shop</td>
                <td width="5%">Pole Kisoks</td>
                <td width="5%">Dealer Wall painting</td>
                <td width="5%">Dealer Shop painting(Sq Ft)</td>
                <td width="5%">Dealer Shop painting</td>
                <td width="5%">Dealer Certificate</td>
                <td width="5%">Product display unit</td>
                <td width="5%">Price list stand</td>
                <td width="5%">Speical  Request</td>
