...
function main()
{
	if($_REQUEST['mode']=="submit_stock_return")
	{
		...
		$dns_customer_code_from=$_POST['dns_customer_code_from'];
		$dns_customer_code_to=$_POST['dns_customer_code_to'];
		$IMEI=$_POST['IMEI'];
		for($i=0;$i<count($IMEI);$i++)
		{	  
			$sqlselcustomercode="SELECT customer_code FROM customer_master WHERE dns_customer_code='".addslashes($dns_customer_code_from[$i])."'";
			$rsselcustomercode=mysqli_query($link,$sqlselcustomercode);
			$rowcustomercode=mysqli_fetch_assoc($rsselcustomercode);
			$customer_code_from=$rowcustomercode['customer_code'];
			if($dns_customer_code_to[$i]!='-')
			{
				$sqlselcustomercode="SELECT customer_code FROM customer_master WHERE dns_customer_code='".addslashes($dns_customer_code_to[$i])."'";
				$rsselcustomercode=mysqli_query($link,$sqlselcustomercode);
				$rowcustomercode=mysqli_fetch_assoc($rsselcustomercode);
				$customer_code_to=$rowcustomercode['customer_code'];
				$sqlupdatebilling="UPDATE customer_product_billing SET 
								customer_code='".$customer_code_to."',stock_out_date='0000d-00-00',download_time=CURRENT_TIMESTAMP() 
								WHERE  IMEI='".$IMEI[$i]."'";
				$rsupdatebilling=mysqli_query($link,$sqlupdatebilling);					
			}
			else
			{
				$sqldeletebilling="DELETE FROM customer_product_billing WHERE IMEI='".$IMEI[$i]."'";
				$rsdeletebilling=mysqli_query($link,$sqldeletebilling);
				$customer_code_to='-';
			}
			$sqlinertlog="INSERT INTO stock_return_log SET customer_code_from='".$customer_code_from."',
							customer_code_to='".$customer_code_to."',IMEI='".$IMEI[$i]."',return_date_time=CURRENT_TIMESTAMP()";
			mysqli_query($link,$sqlinertlog);				
	...
	<tr>
		<td valign="top" >
			<table width="70%" align="center" cellpadding="5" cellspacing="2">
            	 <tr> 
                    <td width="90%" align="center" class="ERR"><font size="+2"><u>IMEI Return</u></font></td>
            	</tr>
            </table>
          </td>
    </tr>       
...
if($_REQUEST['mode']=="xls_upload"){
	...
	if(file_exists($_FILES['zip_file']['tmp_name']))
	{
	  ...
	  //For IMEI return xls
	   if(similar_file_exists("../csv/$folderName/IMEI return.xlsx")!=false || similar_file_exists("../csv/$folderName/IMEI return.xls")!=false)
	    {
		$rec_count = 0;
		$ins_count = 0;
		$err = "";
		...
		//$lines = file($filename);
		if(similar_file_exists("../csv/$folderName/IMEI return.xlsx")!=false)
		{
			$inputfilename = "../csv/$folderName/IMEI return.xlsx";
		}
		if(similar_file_exists("../csv/$folderName/IMEI return.xls")!=false)
		{
			$inputfilename = "../csv/$folderName/IMEI return.xls";
		}
		...
		$tabledata='<form name="depot_cost" method="post" action=""><table border="1" style="border-collapse:collapse;" class="border" width="50%" cellpadding="4" align="center" >
		  <tr class="TDHEAD" align="center" id="head_main">
			<td colspan="4" class="TDHEAD" align="center">IMEI return</td>
		  </tr>
		  <tr class="TDHEAD_SUB" align="center" id="head_main">
		   <td>SI</td><td>IMEI</td><td>Return From</td><td>Return To</td></tr>';
				...
				{ 
					...
					{ 
						...
						$dns_customer_code_to = trim($rowData[0][2]);
						
					    $sqlselcustomercode_from="SELECT CRR.customer_code FROM customer_master CM,customer_route_emp_relation CRR WHERE 
				  					CRR.customer_code=CM.customer_code AND CM.dns_customer_code='".addslashes($dns_customer_code_from)."' and CRR.acedns='Y'";
						$rsselcustomercode_from=mysqli_query($link,$sqlselcustomercode_from);
						$countselcustomercode_from=mysqli_num_rows($rsselcustomercode_from); 
						if($countselcustomercode_from==0)
						{
							array_push($error_array,"Error @Row (".$csv_row_count.") Column : (Return From)");
						}
						else
						{
							$rowselcustomercode_from=mysqli_fetch_assoc($rsselcustomercode_from);
							$customer_code_from=$rowselcustomercode_from['customer_code'];
						}
						if($dns_customer_code_to!='-')
						{
							$sqlselcustomercode_to="SELECT CRR.customer_code FROM customer_master CM,customer_route_emp_relation CRR WHERE 
										CRR.customer_code=CM.customer_code AND CM.dns_customer_code='".addslashes($dns_customer_code_to)."' and CRR.acedns='Y'";
							$rsselcustomercode_to=mysqli_query($link,$sqlselcustomercode_to);
							$countselcustomercode_to=mysqli_num_rows($rsselcustomercode_to); 
							if($countselcustomercode_to==0)
							{
								array_push($error_array,"Error @Row (".$csv_row_count.") Column : (Return TO)");
							}
							else
							{
								$rowselcustomercode_to=mysqli_fetch_assoc($rsselcustomercode_to);
								$customer_code_to=$rowselcustomercode_to['customer_code'];
							}
						}
					  ...
					  $tabledatacsv.="<input type=\"hidden\" name=\"dns_customer_code_from[]\" value=\"$dns_customer_code_from\">
									<input type=\"hidden\" name=\"dns_customer_code_to[]\" value=\"$dns_customer_code_to\">
									<input type=\"hidden\" name=\"IMEI[]\" value=\"$IMEI\">
									<tr id=\"tab\">
									<td>".$count."</td>
									<td>".$IMEI."</td>
									<td>".$dns_customer_code_from."</td>
									<td>".$dns_customer_code_to."</td>
									</tr>";
						$count++;				
					}
					$rec_count++;
		...
		if(count($error_array) >0){
			 echo "<tr> 
					<td width=\"90%\" align=\"center\"  colspan=\"7\"><font size=\"+2\"><u>IMEI Return</u></font></td></tr><br />";
		   ...
		   {
			echo $tabledatafinal=$tabledata.$tabledatacsv."<tr><td colspan='7' align='center'><input type='hidden' name='mode' value='submit_stock_return' /><input type='submit' name='submit1' value='Final Upload' /><input type='button' name='button3' value='Cancel' onclick=\"javascript:window.location='http://salesmpower.acedns.in/misreport/adminExcelReadStockReturn.php'\"/></td></tr></table></form>";
			die;
			$successval=1;
		   }
	}
