...
$sql_skuwise = "SELECT PM.prod_desc, STL.prod_code, STL.convert_qty_two FROM sauda_transaction_log STL, product_master PM WHERE STL.prod_code=PM.prod_code AND PM.product_group_code='".$prodgrcode."' AND ".$condition;
$res_skuwise = mysqli_query($link,$sql_skuwise);
while($row_skuwise = mysqli_fetch_assoc($res_skuwise)){
	$prod_code = $row_skuwise['prod_code'];
	$prod_name = $row_skuwise['prod_desc'];
	$convert_qty_two = $row_skuwise['convert_qty_two'];
	
	$sku_array[$prod_code] += $convert_qty_two;
	$sku_name_array[$prod_code] = $prod_name;
}
  ...
  <tr>
    ...
    <br>
    	<table width="95%">
          ...
          <tr>
          	<td align="center">
            	...
            	<table width="50%" style="border-collapse:collapse;">
                  ...
                  <?php
				  $count = 1;
				  foreach($sku_array as $index=>$val){
					  /*$sql_prod_name = "SELECT prod_desc FROM product_master WHERE prod_code = '".$index."'";
					  ...
					  echo "<tr style=\"background:$color;\">
					  			...
					  			<td style=\"padding:4px; border-left:1px solid; border-left-color:#CDC9C9;\">".$sku_name_array[$index]."</td>
								<td align=\"right\" style=\"padding:4px; border-left:1px solid; border-left-color:#CDC9C9;\">".$val."</td>
					  		</tr>";
					  $booked_total += $val;
					  $count++;
				  }
          ...
          <tr>
          	<td align="center"><a href="#" onClick="window.history.back();" style="font-weight:bold; color:blue;">Back</a></td>
          </tr>
          <!--<tr>
