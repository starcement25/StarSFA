...
?>
	...
	if (strtoupper($_SESSION['nick_name']) != 'SAI' && strtoupper($_SESSION['nick_name']) != 'PROPELLO') {
		$table_data .= "<option value=\"branch\">Branch</option>";
	}
	if (strtoupper($_SESSION['nick_name']) == 'STAR' || strtoupper($_SESSION['nick_name']) == 'START') {
		//$table_data .="<option value=\"branch_competitor\">Branch Competitor</option>";
		$table_data .= "<option value=\"branch_wise_competitor\">Branch Wise Competitor</option>";
	}
	...
	if (strtoupper($_SESSION['nick_name']) == 'STAR') {
		$table_data .= "<option value=\"destination\">Destination</option>";
		$table_data .= "<option value=\"manager_list\">Manager List</option>";
	}
	...
	if (strtoupper($_SESSION['nick_name']) == 'STAR' || strtoupper($_SESSION['nick_name']) == 'START') {
		$table_data .= "<option value=\"route\">Route</option>";
	}

	if (
		strtoupper($_SESSION['nick_name']) == 'ARCHITA' || strtoupper($_SESSION['nick_name']) == 'EDIBLE'  || strtoupper($_SESSION['nick_name']) == 'EDIBLEH' || strtoupper($_SESSION['nick_name']) == 'AJANTA' || strtoupper($_SESSION['nick_name']) == 'ASL'
		|| strtoupper($_SESSION['nick_name']) == 'DNVFOODS' || strtoupper($_SESSION['nick_name']) == 'GOLDSTONE'
	) {
		$table_data .= "<option value=\"distributor_route_emp\">Distributor route emp</option>";
	}
	if (strtoupper($_SESSION['nick_name']) == 'EMAMI') {
		//$table_data .="<option value=\"packing\">Packing</option>";
	}
	if (strtoupper($_SESSION['nick_name']) == 'ASL') {
		$table_data .= "<option value=\"conversion\">Conversion</option>";
	}
	if (strtoupper($_SESSION['nick_name']) == 'NIMBUS') {
		$table_data .= "<option value=\"state\">State</option>";
	}
	if (strtoupper($_SESSION['nick_name']) == 'DURO' || strtoupper($_SESSION['nick_name']) == 'SYLVAN') {
		$table_data .= "<option value=\"facilitator\">Facilitator</option>";
		if (strtoupper($_SESSION['nick_name']) == 'SYLVAN') {
			$table_data .= "<option value=\"site\">Site</option>";
		}
	}
	if (strtoupper($_SESSION['nick_name']) == 'STAR' || strtoupper($_SESSION['nick_name']) == 'START') {
		$table_data .= "<option value=\"fecilitator\">Fecilitator</option>";
		$table_data .= "<option value=\"customer_broker\">Customer Broker mapping</option>";
		$table_data .= "<option value=\"vendor_details\">OOH Site Master</option>";
		$table_data .= "<option value=\"dealer_target\">Dealer Target</option>";
		$table_data .= "<option value=\"warehouse_master\">Warehouse Master</option>";
		$table_data .= "<option value=\"branch_pop_product\">Branch pop product</option>";
		
		$table_data .= "<option value=\"pop_product\">Pop product</option>";
	}
	if (strtoupper($_SESSION['nick_name']) == 'PALSONS') {
		$table_data .= "<option value=\"beatewise_TA_DA\">Beat wise TA DA</option>";
	}
	if (strtoupper($_SESSION['nick_name']) == 'ABDOS') {
		$table_data .= "<option value=\"weightage\">Weightage</option>";
		$table_data .= "<option value=\"TA_DA_Master\">TA DA Master</option>";
		$table_data .= "<option value=\"PJP\">PJP</option>";
		$table_data .= "<option value=\"Target\">Target Master</option>";
	}
	...
	<script>
		...
		function display_result() {
			...
			var url = "dum_download_data_new.php";
			var params = "attribute=" + attribute;
			
			if(attribute=='branch_pop_product'){
			    var url = "dum_download_data_new_branch_pop_product.php";
			}
			//alert(url);
			//alert(params);
			...
			xmlHttp.onreadystatechange = function() { //Call a function when the state changes.
				if (xmlHttp.readyState == 4 && xmlHttp.status == 200) {
					...
					if (val != "") {
						...
						window.location = 'https://salesmpower.acedns.in/misreport/dump/' + val;
					}
				}
			}
			//alert(params);
