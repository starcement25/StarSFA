...
$sql_location_date = "SELECT DISTINCT SUBSTRING(date,1,10) AS distinct_date FROM `location` WHERE (SUBSTRING(date,1,10) BETWEEN '2017-01-01' AND '".$today."') ORDER BY SUBSTRING(date,1,10) ASC";
...
while($row_location_date = mysqli_fetch_assoc($res_location_date)){
	...
	while($row_emp_code = mysqli_fetch_assoc($res_emp_code)){
		...
		if($present_check>0){
			...
			$sql_insert_emp_details = "UPDATE mis_details_emp_datewise SET 
											`customer_visited` = '".$customer_string."'
											 WHERE emp_code = '".$emp_code."' AND operation_date = '".$location_date."'";
