...
function main(){
	...
	$sql_survey_type = "SELECT DISTINCT survey_sub_menu FROM survey_input_lead ORDER BY survey_sub_menu ASC";
	...
	while($row_survey_type = mysqli_fetch_assoc($res_survey_type)){
		...
		if($survey_type != ''){
		    if($survey_type=="Lead Generation"){
		        $create_control .= "<option>".$survey_type."</option>";
		        $create_control .= "<option>Lead Generation New</option>";
		    }else{
		        $create_control .= "<option>".$survey_type."</option>";
		    }
		}
	...
	$sql_technical_type = "SELECT display_name,row_id FROM survey_input_lead WHERE type='menu' and survey_sub_menu='Technical Meets' ORDER BY display_order ASC";
	...
	function display_result(){
		...
		if(survey_type == 'Branding'){
			...
			var url = 'lead_generation_data.php';
		}
		else if(survey_type == 'Lead Generation New'){
			var url = 'lead_generation_data_new.php';
		}
