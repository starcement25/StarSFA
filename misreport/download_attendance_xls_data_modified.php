...
$sql_checkout = "SELECT EM.emp_code,SUBSTRING(LO.date,12) AS checkout_time,LO.trans_id,DATE_FORMAT(LO.date,'%Y-%m-%d') AS check_out_date 
					FROM employee_master EM LEFT JOIN location LO ON LO.emp_code=EM.emp_code WHERE (LO.trans_id LIKE 'CH%')  
					AND DATE_FORMAT(LO.date,'%Y-%m-%d') >='" . $start_date_captured . "' AND DATE_FORMAT(LO.date,'%Y-%m-%d') <='" . $end_date . "'
					AND EM.emp_code IN(" . $emp_code . ") order BY state ASC,zone ASC,emp_name ASC,DATE_FORMAT(LO.date,'%Y-%m-%d') ASC";

$res_checkout = mysqli_query($link, $sql_checkout) or die(mysqli_error() . " Error in select check out employee list: " . $sql_checkout);
//echo $sql_checkout;die;
while ($row_checkout = mysqli_fetch_assoc($res_checkout)) {
	$check_out_date = $row_checkout['check_out_date'];
	$emp_code_val = $row_checkout['emp_code'];
	$checkout_time = $row_checkout['checkout_time'];
	${'check_out_time' . $emp_code_val . $check_out_date} = $checkout_time;
}
...
$cntemployeelist = mysqli_num_rows($resemployeelist);
if ($cntemployeelist > 0) {
	...
	while ($rowemployeelist = mysqli_fetch_assoc($resemployeelist)) {
		...
		/*$sql_checkout = "SELECT SUBSTRING(`date`,12) AS checkout_time,trans_id FROM location  WHERE emp_code = '".$emp_code."' 
							AND trans_id LIKE 'CH%' AND DATE_FORMAT(`date`,'%Y-%m-%d')='".$att_date."'";
	...
	foreach ($emp_code_array as $emp_code_val) {
		...
		for ($count_number_days = 0; $count_number_days <= $number_days; $count_number_days++) {
			...
			//$value.=${'check_out_time'.$emp_code_val.$start_date}." \t";
			////////////
			$checkout_key = 'check_out_time' . $emp_code_val . $start_date;
			$checkin_key  = 'time' . $emp_code_val . $start_date;

			if (!empty($$checkout_key)) {
			
				$value .= $$checkout_key . " \t";
			} elseif (!empty($$checkin_key)) {
	...
	if (file_exists("/var/www/html/misreport/$filename.xls")) {
		unlink("/var/www/html/misreport/$filename.xls");
	}
	$fp = fopen("/var/www/html/misreport/$filename.xls", "wb");
