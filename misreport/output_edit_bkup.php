...
while($row_select_survey_details = mysqli_fetch_assoc($res_select_survey_details))
{
	...
	else if($row_select_survey_input['type'] == 'radio' || $row_select_survey_input['type'] == 'checkbox')
	{
		...
		if(strstr($row_select_survey_details['value'],';',true))
		{
			...
			$res_category_table = mysqli_query($link,$sql_category_table);
			while($row_category_table = mysqli_fetch_assoc($res_category_table))
			{
...
if($_POST['submit'] == 'Submit')
{
	...
	foreach($survey_output_chunk as $survey_chunk_index=>$survey_chunk_value)
	{
		...
		$res_record_exist_check = mysqli_query($link,$sql_record_exist_check);
		$total_record_exist_check = mysqli_num_rows($res_record_exist_check);
		...
		if($total_record_exist_check==0)
		{
			...
			$res_update_survey_output = mysqli_query($link,$sql_update_survey_output);
		}
	}
