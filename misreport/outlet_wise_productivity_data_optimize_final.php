...
//if(!empty($date_array)){
    ...
    <tr class='TDHEAD'><td colspan='17' align='center'>Customer Visit & Productivity report Details</td></tr>
	 ...
	 $result = mysqli_query($link,"SELECT DISTINCT  SUBSTRING(`visit_date`,1,7) AS month FROM prev_order_counting_master WHERE DATE_FORMAT(SUBSTRING(order_no,-14,8),'%Y-%m-%d') BETWEEN '".$start_date."' AND '".$end_date."'");
		while($row = mysqli_fetch_assoc($result)) {
		   $monthval = date("F", strtotime($row['month']));
    ...
    <?
	$sql_outlet_sku="SELECT DISTINCT EM.emp_name,EM.emp_code,CM.customer_name,CM.cust_class,CM.category_of_store,CM.is_nlp,RM.route_name,
	DATE_FORMAT(SUBSTRING(POCM.order_no,-14,8),'%d') AS visit_date ,  IF( POCM.order_no LIKE 'O%', DATE_FORMAT(SUBSTRING(POCM.order_no,-14,8),'%d'), NULL ) 
	AS productive_date, SUBSTRING(`visit_date`,1,7)  AS month,POCM.customer_code FROM customer_master CM,employee_master EM,prev_order_counting_master POCM,
	route_master RM WHERE POCM.customer_code=CM.customer_code AND CM.route_code=RM.route_code AND SUBSTRING(POCM.order_no,-19,5)=EM.emp_code 
	AND DATE_FORMAT(SUBSTRING(POCM.order_no,-14,8),'%Y-%m-%d') > '2020-12-01' ".$emp_hierarchy_value_condition." 
	AND DATE_FORMAT(SUBSTRING(POCM.order_no,-14,8),'%Y-%m-%d') BETWEEN '".$start_date."' AND '".$end_date."' 
	ORDER BY EM.emp_name ASC,CM.customer_name ASC,MONTH(POCM.visit_date) ASC,POCM.visit_date ASC";
	/*$sql_outlet_sku = "SELECT DISTINCT EM.emp_name,CM.customer_name,RM.route_name,
					...
					GROUP_CONCAT( DISTINCT IF( POCM.order_no LIKE 'O%', DATE_FORMAT(SUBSTRING(POCM.order_no,-14,8),'%d'), NULL ) SEPARATOR ',')  AS productive_date,MONTH(POCM.visit_date) AS month,POCM.customer_code
	...
	while($row_outlet_sku = mysqli_fetch_assoc($res_outlet_sku)){
		...
		$cust_class = $row_outlet_sku['cust_class'];
		$category_of_store = $row_outlet_sku['category_of_store'];
		$emp_name = $row_outlet_sku['emp_name'];
		$emp_code = $row_outlet_sku['emp_code'];
		...
		$is_nlp = $row_outlet_sku['is_nlp'];
		$customer_emp_string=$customer_code.'-'.$emp_code;
		if(!in_array($customer_emp_string,$customer_code_array))
		{
			array_push($customer_code_array,$customer_emp_string);
			array_push($customer_name_array,$customer_name);
			array_push($customer_class_array,$cust_class);
			array_push($customer_category_array,$category_of_store);
			array_push($emp_name_array,$emp_name);
			array_push($emp_code_array,$emp_code);
			array_push($route_name_array,$route_name);
			array_push($is_nlp_array,$is_nlp);
		}
		if($visit_date!='')
		{
			${'visit_date'.$emp_code.$customer_code.$month}=${'visit_date'.$emp_code.$customer_code.$month}.$visit_date.",";
		}
		if($productive_date!='')
		{
