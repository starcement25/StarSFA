...
if($totalbargaidetails >0){
       ...
       <form name="frm_bargainapprove" method="post" action="adminBargainApproved.php" enctype="multipart/form-data" />
		...
		while($rowbargaindetails = mysqli_fetch_assoc($resbargaidetails)){
			...
			$rsremarks=mysqli_query($link,$sqlremarks);
			$rowremarks=mysqli_fetch_assoc($rsremarks);
			...
			$rsbargainstatus=mysqli_query($link,$sqlbargainstatus);
			$cntbargainstatus=mysqli_num_rows($rsbargainstatus);
			...
			else
			{
				...
				$rsbargainstatusone=mysqli_query($link,$sqlbargainstatusone);
				$cntbargainstatusone=mysqli_num_rows($rsbargainstatusone);
