...
$res_distinct_survey_id = mysqli_query($link,$sql_distinct_survey_id);
while($row_distinct_survey_id = mysqli_fetch_assoc($res_distinct_survey_id)){
	...
	$res_vtype = mysqli_query($link,$sql_vtype);
	$row_vtype = mysqli_fetch_assoc($res_vtype);
	//$vtype = $row_vtype['value'];
	$vtype = str_replace('#',':',$row_vtype['value']);
	
	if(strtolower(substr($vtype,0,7))=='counter') 
	{
	...
	while($row_survey_details = mysqli_fetch_assoc($res_survey_details)){
		...
		if($row_id == 'RA046')
			...
			foreach($pic_array as $pic_val){
				...
				$pic_string .= "<a href=\"https://starcement1-sbinfo-upload.s3.ap-south-1.amazonaws.com/STAR/".$pic_val."\" target=\"_blank\" style=\"color:brown;\">View</a><br>";
			}
