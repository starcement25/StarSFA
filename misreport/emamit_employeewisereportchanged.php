...
function PrintElem(elem)
{
	...
	var customerdiv = document.getElementById("customer").innerHTML;
    var productdiv = document.getElementById("product").innerHTML;
    var view = displaydiv+'<br>'+customerdiv+'<br>'+productdiv;
   Popup(view);
   //Popup($(elem).html());
}
...
function Popup(data) 
{
	var mywindow = window.open('', 'Employeewise Sauda Report', 'height=400,width=600');
	mywindow.document.write('<html><head><title>Employeewise Sauda Report</title>');
...
function exporttocsv(divid)
{
	//alert(divid);
        ...
        a.download = 'Employeewise Sauda Report' + postfix + '.xls';
...
function show_employeewise()
{
	...
	if(type != 'custom')
		GenericAjaxFunction('emamit_employeewisedatachanged.php?type='+type,'display',0);
	else 
	{
		if(response1 != 0 && response2 != 0)
			GenericAjaxFunction('emamit_employeewisedatachanged.php?type='+type+'&start_date='+start_date+'&end_date='+end_date,'display',0);
	}
...
function show_customer(product_code, emp_code, emp_name, product_name, condition_value, start_date, end_date)
{
	...
	GenericAjaxFunction('emamit_customer_product_details.php?emp_code='+emp_code+'&emp_name='+emp_name+'&product_code='+product_code+'&product_name='+product_name+'&condition_value='+condition_value+'&start_date='+start_date+'&end_date='+end_date,'product',0);
}
