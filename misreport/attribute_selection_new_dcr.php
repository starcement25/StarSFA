...
function attribute_selection($hidden,$get_control){
	...
	$emp_access=1;
	if($emp_access>0){
	    $table_data .= "<tr><td align=\"right\">Employee Access:</td>";
		/*if($zone_total>0 || $state_total>0 || $branch_total>0){
			$table_data .= "<td><div id=\"saleaccess_select_div\"></div></td></tr>";
		}
		else{*/
			...
			$table_data .= "<td>";
			$saleaccess_select_control = "<select name=\"emp_access\" id=\"emp_access\" onchange=\"".$onclick."\" style=\"display:none\">";
			$saleaccess_select_control .= "<option value=\"\">Select</option>";
			$saleaccess_select_control .= "<option value=\"Y\">Active</option>";
			$saleaccess_select_control .= "<option value=\"N\">In Active</option>";
			$saleaccess_select_control .= "</select>";
			$table_data .= $saleaccess_select_control;
			$table_data .= "</td></tr>";
		//}
...
Date:<input type=\"date\" name=\"start_date\" id=\"start_date\" style=\"height:15px;\" />
</div></td></tr>";
	...
	function branch_saleaccess(branch){
		...
		document.getElementById("saleaccess_select_div").innerHTML = '<img src="ajax-loader.gif" id="ajaxloader">';
		GenericAjaxFunction('get_branch_related_data_new.php?branch='+branch+'&zone='+zone+'&state='+state+'&type=sale_access','saleaccess_select_div',0);
	...
	function saleaccess_emp_type(sale_access){
	   if(document.getElementById("sale_access").value.search(/\S/) == -1)
			return false;
	   document.getElementById("emp_access").style.display="block";
	}
	...
	function saleaccess_emp(sale_access){
		...
		var state = encodeURIComponent(document.getElementById("state").value);
		var zone = encodeURIComponent(document.getElementById("zone").value);
		var useractive = encodeURIComponent(document.getElementById("emp_access").value);
		
		document.getElementById("emp_select_div").innerHTML = '<img src="ajax-loader.gif" id="ajaxloader">';
		GenericAjaxFunction('get_saleaccess_related_data_new.php?sale_access='+sale_access+'&branch='+branch+'&state='+state+'&zone='+zone+'&useractive='+useractive+'&showall=0&type=emp','emp_select_div',0);
