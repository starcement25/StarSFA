...
$sql_competitor_name = "SELECT DISTINCT competitor_name,display_name FROM competitor_group_master $branchcondition 
						and acedns='yes' and branch_code <> '' ORDER BY FIELD(competitor_name, 'STAR PSC','STAR PPC','STAR') DESC,competitor_name ASC";
...
while($row_competitor_name = mysqli_fetch_assoc($res_competitor_name)){
	...
	$display_name = $row_competitor_name['display_name'];
	if($display_name !='') $display_competitor_name=$display_name;
	else				    $display_competitor_name=$competitor_name;
	
	echo "<td align=\"center\">".$display_competitor_name."</td>";
