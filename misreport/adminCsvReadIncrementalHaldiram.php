...
function csv_upload(){
			...
			foreach($lines as $line)
			{
				...
				{ 
					...
					if(no_of_filter > 3){
						...
						if(providing_code=='yes' || $folderName=='HALDIRAM' || $folderName=='PRABHUJI'){
			...
			foreach($lines as $line)
			{
				...
				{ 
				  ...
				  $data[]=$value;
					...
					if(providing_code=='yes'){
						$sqlchkphone="SELECT emp_code FROM employee_master WHERE dns_emp_code='".addslashes($dns_employee_code)."' AND 
										phone_no='".addslashes($phone_no)."'";
						$rschkphone=mysqli_query($link,$sqlchkphone);
						$cntchkphone=mysqli_num_rows($rschkphone);
						if($cntchkphone > 0){
							  $GLOBALS['msg'] = "Improper Employee name and Phone no @row $csv_row_count in Employee master.csv";
							  disphtml("main();");
							  exit;
						  }
					}
					...
					{
					$sqlchkphone="SELECT emp_code FROM employee_master WHERE emp_name <> '".addslashes($employee_name)."' AND 
								phone_no='".addslashes($phone_no)."'";
					$rschkphone=mysqli_query($link,$sqlchkphone);
					$cntchkphone=mysqli_num_rows($rschkphone);
					if($cntchkphone > 0){
	...
	//For Customer master haldiram CSV
	if(similar_file_exists("../csv/$folderName/Customer Master Haldiram.csv")!=false)
	{
		$filename=similar_file_exists("../csv/$folderName/Customer Master Haldiram.csv");
		$rec_count = 0;
		$ins_count = 0;
		$err = "";
					...
					foreach($emp_code_name_array as $emp_code_name_value_next)
					{
					 ...
					 //$emp_code=$emp_code_name;
						...
						if($countroutechk<1 && $route_name!='')
						{
							...
							$sqlroute  = "insert into route_master ";
							$sqlroute .= " SET route_code='".$max_route_code."'";
							$sqlroute .= " ,dns_route_code='".addslashes($dns_route_code)."'";
			  ...
			  foreach($lines as $line)
			  {
				...
				{ 
				   ...
				   $data[]=$value;
					...
					$sqlstatecode="SELECT state_code FROM state_master WHERE statename='".addslashes($state)."'";
					$rsstatecode=mysqli_query($link,$sqlstatecode);
					$cntstatecode=mysqli_num_rows($rsstatecode);
					if($cntstatecode==0){
						  $GLOBALS['msg'] = "Improper state @row $csv_row_count in Customer Master Haldiram.csv";
				          disphtml("main();");
						  exit;
					  }
					...
					else
					{
						...
						if($cntempcode==0){
							  $GLOBALS['msg'] = "Improper Employee name @row $csv_row_count in Customer Master Haldiram.csv";
							  disphtml("main();");
							  exit;
						  }
						$rowempcode=mysqli_fetch_assoc($rsempcode);
					...
					if($rds_tag=='' && $customer_type=='R')
					{
						$GLOBALS['msg'] = "Improper rds tag @row $csv_row_count in Customer Master Haldiram.csv";
				        disphtml("main();");
						exit;
					}
					if($rds_tag!='')
					{
						if(providing_code=='yes'){
							$sqlrdscode="SELECT customer_code FROM customer_master WHERE dns_customer_code='".addslashes($rds_tag)."' AND acedns='Y'";
						}
						else
						{
						...
						$rsrdscode=mysqli_query($link,$sqlrdscode);
						$cntrdscode=mysqli_num_rows($rsrdscode);
						if($cntrdscode==0 ){
							  $GLOBALS['msg'] = "Improper rds tag @row $csv_row_count in Customer Master Haldiram.csv";
							  disphtml("main();");
							  exit;
					...
					//For validation
					$sqlchkcustomerphone="SELECT customer_code FROM customer_master WHERE customer_name <> '".addslashes($customer_name)."' 
											AND phone_no='".$phone_no."'";
					$rschkchkcustomerphone=mysqli_query($link,$sqlchkcustomerphone);
					$cntchkchkcustomerphone=mysqli_num_rows($rschkchkcustomerphone);
					if($cntchkchkcustomerphone > 0)
					{
					  $GLOBALS['msg'] = "Duplicate Phone no @row $csv_row_count in Customer Master Haldiram.csv";
					  disphtml("main();");
					  exit;
					}
					$sqlchkemplevel="SELECT emp_code FROM employee_master WHERE reporting_to='".$emp_code."'";
					$rschkemplevel=mysqli_query($link,$sqlchkemplevel);
					$cntchkemplevel=mysqli_num_rows($rschkemplevel);
					if($customer_type=='R' || $customer_type=='D'){
						if($cntchkemplevel > 0)
						{
						  $GLOBALS['msg'] = "Improper employee mapping @row $csv_row_count in Customer Master Haldiram.csv";
				          disphtml("main();");
						  exit;
						}
					}
					if($cntchkemplevel==0){
						if($customer_type!='R'){
							if(providing_code=='yes'){
								$sqlchkmultiplecust="SELECT DISTINCT CM.dns_customer_code FROM customer_master CM,customer_route_emp_relation CRR WHERE 
													CM.customer_code=CRR.customer_code AND CRR.emp_code='".$emp_code."' AND CM.cust_type!='R'";
								$rschkmultiplecust=mysqli_query($link,$sqlchkmultiplecust);
								$cntchkmultiplecust=mysqli_num_rows($rschkmultiplecust);
								if($cntchkmultiplecust > 0)
								{
									$rowchkmultiplecust=mysqli_fetch_assoc($rschkmultiplecust);
									$existing_dns_customer_code=$rowchkmultiplecust['dns_customer_code'];
									if($existing_dns_customer_code!=$dns_customer_code)
									{
									  $GLOBALS['msg'] = "DSM cannot mapped with two D OR SS OR DEPOT @row $csv_row_count in Customer Master Haldiram.csv";
									  disphtml("main();");
									  exit;
									}
								}
							...
							{
								$sqlchkmultiplecust="SELECT DISTINCT CM.customer_name FROM customer_master CM,customer_route_emp_relation CRR WHERE 
													CM.customer_code=CRR.customer_code AND CRR.emp_code='".$emp_code."' AND CM.cust_type!='R'";
								$rschkmultiplecust=mysqli_query($link,$sqlchkmultiplecust);
								$cntchkmultiplecust=mysqli_num_rows($rschkmultiplecust);
								if($cntchkmultiplecust > 0)
								{
									$rowchkmultiplecust=mysqli_fetch_assoc($rschkmultiplecust);
									$existing_customer_name=$rowchkmultiplecust['customer_name'];
					...
					/*else
					{
						$sqlchkempsaleaccess="SELECT sale_access FROM employee_master WHERE emp_code='".$emp_code."'";
						$rschkempsaleaccess=mysqli_query($link,$sqlchkempsaleaccess);
						$rowchkempsaleaccess=mysqli_fetch_assoc($rschkempsaleaccess);
						$sale_access=$rowchkempsaleaccess['sale_access'];
						if(strtoupper($sale_access)!='PRIMARY')
						{
							$GLOBALS['msg'] = "Improper Sale access for mapped employee @row $csv_row_count in Customer Master Haldiram.csv";
							  disphtml("main();");
							  exit;
						}
					}*/
					...
					if($customer_type!='R'){
						if(providing_code=='yes'){
							$sqlroutechk="SELECT RM.* FROM route_master RM,customer_master CM 
									WHERE CM.route_code=RM.route_code AND RM.dns_route_code='".addslashes($dns_route_code)."' AND CM.state_code='".$state."'";
						}
						else
						{
							$sqlroutechk="SELECT RM.* FROM route_master RM,customer_master CM WHERE CM.route_code=RM.route_code
								AND RM.route_name='".addslashes($route_name)."' AND CM.state_code='".$state."'";
						}
						$rsroutechk=mysqli_query($link,$sqlroutechk);
						$countroutechk=mysqli_num_rows($rsroutechk);
						if($countroutechk<1 && $route_name!='' )
					...
					{
						$sqlfetchroute="SELECT RM.route_code FROM distributor_route_relation DRR,route_master RM WHERE 
									DRR.route_code=RM.route_code AND DRR.distributor_code='".$rds_code."' AND DRR.state='".$state."' 
									AND RM.route_name='".$route_name."'";
						$rsfetchroute=mysqli_query($link,$sqlfetchroute);
						$countfetchroute=mysqli_num_rows($rsfetchroute);
						if($countfetchroute<1)
						{
							 $GLOBALS['msg'] = "Improper Route mapping For Customer type=R @row $csv_row_count in Customer Master Haldiram.csv";
							  disphtml("main();");
							  exit;
						}
						else
						{
							$rowfetchroute=mysqli_fetch_assoc($rsfetchroute);
							$route_code=$rowfetchroute['route_code'];
						}
					}
					//For customer
					if(providing_code=='yes'){
					...
					else
					{
					 if($customer_type!='R'){	
						 $sqlcustomernamechk="SELECT * FROM customer_master WHERE customer_name='".addslashes($customer_name)."' AND 
						 						state_code='".$state."' AND phone_no='".$phone_no."'";
						 //$sqlcustomernamechk="SELECT * FROM customer_master WHERE customer_name='".addslashes($customer_name)."'";
					 }
					 else
					 {
					   $sqlcustomernamechk="SELECT * FROM customer_master WHERE customer_name='".addslashes($customer_name)."' AND state_code='".$state."' 
					   						AND route_code='".$route_code."' AND phone_no='".$phone_no."'";
					 }
					}
					$rscustomernamechk=mysqli_query($link,$sqlcustomernamechk);
					$countcustomernamechk=mysqli_num_rows($rscustomernamechk);
					...
					if($countcustomernamechk<1)
					{
						...
						$sql .= " , email='".$email."'";
						$sql .= " , visit_day='".addslashes($visit_day)."'";
						$sql .= " , state_code='".addslashes(strtoupper($state))."'";
						$sql .= " , monthly_potential='".addslashes($monthly_potential)."'";
						$sql .= " , coverage_type='".addslashes($coverage_type)."'";
					...
					else
					{
						...
						if($customer_type_db!=$customer_type)
						{
						  $GLOBALS['msg'] = "Duplicate customer @row $csv_row_count in Customer Master Haldiram.csv";
				          disphtml("main();");
						  exit;
						}
						else
						{
						if(providing_code=='yes'){
							$update_condition=" dns_customer_code='".addslashes($dns_customer_code)."' AND state_code='".$state."'";
						}
						else
						{
							if($customer_type!='R'){				
								$update_condition=" customer_name='".addslashes($customer_name)."' AND state_code='".addslashes($state)."' AND phone_no='".$phone_no."'";
							}
							else
							{
							$update_condition=" customer_name='".addslashes($customer_name)."' AND state_code='".addslashes($state)."' 
												AND route_code='".$route_code."' AND phone_no='".$phone_no."'";
							}
						}
						...
						|| $incoterms_db!=$incoterms || $loadability_ton_db!=$loadability_ton || $transport_mode_db!=$transport_mode || $sauda_type_db!=$sauda_type || $zone_db!=$zone || $visit_sequence_db!=$visit_sequence || $state_code_db!=$state)
						{
							$sqlupdated  = "update customer_master ";
							$sqlupdated .= " SET route_code='".$route_code."'";
							$sqlupdated .= " , dns_customer_code='".$dns_customer_code."'";
							...
							$sqlupdated .= " , rds_tag='".$rds_code."',visit_day='".addslashes($visit_day)."',
											coverage_type='".addslashes($coverage_type)."',
											state_code='".addslashes(strtoupper($state))."',
											monthly_potential='".addslashes($monthly_potential)."',
					...
					$sqlselcustomerroute="SELECT customer_code,route_code,emp_code,acedns FROM customer_route_emp_relation WHERE 
					...
					{
						//No place to route updation
						$rowselcustomerroute=mysqli_fetch_assoc($rsselcustomerroute);
						$emp_code_relation_db=$rowselcustomerroute['emp_code'];
						$acdns_relation_db=$rowselcustomerroute['acedns'];
						if($emp_code_relation_db!=$emp_code || $acdns_relation_db!=$acedns)
						{
							$sqlupdatecustomerroute="UPDATE customer_route_emp_relation SET acedns='N',download_time=CURRENT_TIMESTAMP() WHERE 
													customer_code='".$customer_code."' AND acedns='Y'";
		...
		$err = "";
			...
			foreach($lines as $line)
			{
				...
				//print_r($data);
					...
					else if($folderName=='HALDIRAM' || $folderName=='SKIPPER' || $folderName=='PRABHUJI')
					{
						$sqlprodcode="SELECT prod_code FROM product_master WHERE dns_prod_code='".$prod_code_name."'";
					}
					else
		...
		foreach($lines as $line)
		{
			...
			{ 
			  ...
			  //print_r($data);
				...
				$cntstatecode=mysqli_num_rows($rsstatecode);
				if($cntstatecode==0){
					  $GLOBALS['msg'] = "Improper state @row $csv_row_count in distributor_emp_route_relation.csv";
					  disphtml("main();");
					  exit;
				  }
				$emp_code_name_array=explode(',',$emp_code_name);
				...
				if($countroutecode<1 )
					{
						...
						$sqlroute  = "insert into route_master ";
						$sqlroute .= " SET route_code='".$max_route_code."'";
						$sqlroute .= " ,dns_route_code='".addslashes($route_code_name)."'";
						$sqlroute .= " ,route_name='".addslashes($route_code_name)."'";
						$sqlroute .= " , download_time=CURRENT_TIMESTAMP()";
						mysqli_query($link,$sqlroute) or  array_push($error_array,"mysqli_error().
										Internal DATA execution problem on route table.PLease contact aceDNS admin.");				
						//modifyempdatadownloadlog($emp_code,strtoupper($folderName));
				...
				if(providing_code=='yes'){
					$sqlcustomercode="SELECT customer_code FROM customer_master WHERE dns_customer_code='".addslashes($customer_code_name)."' AND 
					...
					{*/
					$sqlcustomercode="SELECT customer_code FROM customer_master WHERE customer_name='".addslashes($customer_code_name)."' AND 
					state_code='".addslashes($state)."'";
					//}
				}
				$rscustomercode=mysqli_query($link,$sqlcustomercode);
				...
				if($cntcustomercode==0){
					  $GLOBALS['msg'] = "Improper Customer name @row $csv_row_count in distributor_emp_route_relation.csv";
					  disphtml("main();");
					  exit;
				  }
				$rowcustomercode=mysqli_fetch_assoc($rscustomercode);
				...
				foreach($emp_code_name_array as $emp_code_name_value_next)
					{
						...
						$rsempnamechk=mysqli_query($link,$sqlempnamechk);
						$cntempcode=mysqli_num_rows($rsempnamechk);
						if($cntempcode==0){
							  $GLOBALS['msg'] = "Improper Employee name @row $csv_row_count in distributor_emp_route_relation.csv";
							  disphtml("main();");
							  exit;
						  }
						$rowempnamechk=mysqli_fetch_assoc($rsempnamechk);
					...
					//For validation
					if($folderName=='HALDIRAM' || $folderName=='PRABHUJI')
					{
						$sqlchkemplevel="SELECT emp_code FROM employee_master WHERE reporting_to='".$emp_code."'";
						$rschkemplevel=mysqli_query($link,$sqlchkemplevel);
						$cntchkemplevel=mysqli_num_rows($rschkemplevel);
						if($cntchkemplevel > 0)
						{
						  $GLOBALS['msg'] = "Employee should belong to the lowest level @row $csv_row_count in Distributor_emp_route_relation.csv";
						  disphtml("main();");
						  exit;
						}
						$sqlchkmultipledist="SELECT DISTINCT distributor_code FROM distributor_route_relation WHERE emp_code='".$emp_code."'";
						$rschkmultipledist=mysqli_query($link,$sqlchkmultipledist);
						$cntchkmultipledist=mysqli_num_rows($rschkmultipledist);
						if($cntchkmultipledist > 0)
						{
							$rowchkmultipledist=mysqli_fetch_assoc($rschkmultipledist);
							$existing_distributor_code=$rowchkmultipledist['distributor_code'];
							if($existing_distributor_code!=$customer_code)
							{
							  $GLOBALS['msg'] = "DSM cannot mapped with two distributors @row $csv_row_count in Distributor_emp_route_relation.csv";
							  disphtml("main();");
							  exit;
							}
						}
						...
						if(strtoupper($sale_access)!='PRIMARY')
						{
							$GLOBALS['msg'] = "Improper Sale access for mapped employee @row $csv_row_count in distributor_emp_route_relation.csv";
							disphtml("main();");
							exit;
						}*/
					}
				   ...
				   $countchkdistributorroute=mysqli_num_rows($rschkdistributorroute);
					   ...
					   {						
						   $sqlinsertdistributorroute="INSERT INTO distributor_route_relation SET distributor_code='".$customer_code."',
													route_code='".$route_code."',emp_code='".$emp_code."',state='".addslashes($state)."',download_time=CURRENT_TIMESTAMP()";
						   $rsinsertdistributorroute=mysqli_query($link,$sqlinsertdistributorroute);
					   }
					}
			   }
