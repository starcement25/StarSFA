...
<table border="1" width="600" style="border-collapse:collapse;" class="border" cellpadding="5px">
	<tr class="TDHEAD">
    	<td colspan="2" align="center"><b>Sauda Allocation</b><div style="float:right;"><a href="sauda_main.php"><img src="close.png" width="25" height="25" /></a></div></td>
    </tr>
    <tr>
    ...
    <tr>
        ...
        <td>
        	<select name="product_group" id="product_group" <?php if($_GET) echo "onchange=\"call_ajax();\""; ?>>
            	<option value="">Select</option>
        	<?php
				$sql_select_product_group = "SELECT $field_name1, $field_name2 FROM $sauda_table_value WHERE 1 AND acedns='Y' ".$condition_one." ORDER BY $field_name2 ASC";
    ...
    <tr>
		...
		{
		?>	
			<input type="submit" name="update" value="Update" id="update" class="inplogin"  <?php if(strtoupper($_SESSION['admin_login'])!='ADMIN' && $_SESSION['admin_login']!=$top_emp_code){?>onclick="javascript:check_sauda_qty('<?php echo $_SESSION['admin_login'];?>');"<?php }else{?>onclick="updatedata();"<?php }?> />
        <?php   
