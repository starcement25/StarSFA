...
?>
  ...
  <form name ="frmSearch" method="post" action="<?=$_SERVER['PHP_SELF']?>" >
	<table border="1" width="40%" class="border" style="border-collapse:collapse;" cellpadding="5px;">
      ...
      <?php
		...
		$sql_state = "SELECT DISTINCT SM.dns_state_code,SM.state FROM state_master SM,customer_master CM WHERE SM.state!='' 
						AND CM.state_code=SM.dns_state_code AND CM.acedns='Y' ORDER BY SM.state ASC";
	...
	/*function show_transportmode(state)
	{
		if(document.getElementById("state").value.search(/\S/) == -1)
			return false;
		var state = encodeURIComponent(state);
		...
		GenericAjaxFunction('get_state_related_data.php?state='+state+'&type=statetransport','showtransportmode',0);
	}*/
...
function exporttocsv(product_group_code,from_date,state,route,transport_mode,capacity)
{
	//alert(divid);
       ...
       /* var dt = new Date();
        ...
        e.preventDefault();*/
		window.open('sale-rate-data-download-modified-export.php?from_date='+from_date+'&product_group_code='+product_group_code+'&state='+state+'&route='+route+'&transport_mode='+transport_mode+'&capacity='+capacity,'mywindow')	;
}
...
function load_capacity()
{
	...
	var url="returntransportmodedetails.php?transport_mode="+transport_mode+"&state="+state+"&type=loadcapacitystate";
