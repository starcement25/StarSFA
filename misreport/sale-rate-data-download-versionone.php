...
<center>
  ...
  <form name ="frmSearch" method="post" action="<?=$_SERVER['PHP_SELF']?>" >
	<table border="1" width="40%" class="border" style="border-collapse:collapse;" cellpadding="5px;">
    	<tr class="TDHEAD_SUB">
    	<td align="center">Detail Costing Report</td>
       </tr>
    	<tr class="TDHEAD_SUB">
	   ...
	   <tr class="TDHEAD_SUB" >
        	...
        	<!--option value="" selected>Select</option-->
			<?php 
                $sqlratecreatedate="SELECT DISTINCT DATE_FORMAT(SUBSTRING(create_date,1,19),'%d-%m-%Y %H:%i:%s') AS create_date  FROM 
									sauda_mrp ORDER BY DATE_FORMAT(SUBSTRING(create_date,1,19),'%Y-%m-%d %H:%i:%s') DESC";
	...
	function return_result(from_date,plant_name)
	{
		...
		else
		{
			...
			GenericAjaxFunction('sale-rate-data-download-modified-versionone.php?from_date='+from_date+'&plant_name='+plant_name,'display',0);
...
function exporttocsv(divid)
{
	//alert(divid);
        ...
        a.download = 'Bargain Rate' + postfix + '.xls';
