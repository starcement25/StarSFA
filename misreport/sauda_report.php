...
else if($_SESSION['vertical_value']=='HBC:Rasoi:BIB')
{
	...
	$today = date('Y-m-d');
	$condition = " AND substring(SAL.allocation_date,1,10) LIKE '%$today%'";
	$order_condition = "AND substring(SD.sauda_no,-14,8) ='".str_replace("-","",$today)."'";
}
else if($_GET['type'] == 'mtd'){
	@$current_date = date('Y-m-d');
	$month = explode("-",$current_date);
	...
	echo "<table border=\"1\" width=\"100%\" style=\"border-collapse:collapse;\" class=\"border\">
		<tr>
			<td align=\"center\" class=\"TDHEAD\" colspan=\"5\">Today's Sauda Booking Status</td>
		</tr>
		<tr align=\"center\" style=\"font-weight:bold;\" class=\"TDHEAD_SUB\">
			<td>Alloted</td>
			<td colspan=\"2\">Booked</td>
		...
		$sqlcustomrdetals="SELECT SUM(sauda_limit) AS total_sauda_limit,SUM(pending_qty) AS total_pending_qty FROM customer_sauda_limit";
		$rscustomerdetails=mysqli_query($link,$sqlcustomrdetals);
		$rowcustomerdetails=mysqli_fetch_assoc($rscustomerdetails);
		$total_sauda_limit=$rowcustomerdetails['total_sauda_limit'];
		$total_pending_qty=$rowcustomerdetails['total_pending_qty'];
		$final_allotted=($total_sauda_limit-$total_pending_qty);
		
		$sql_quantity_booked = "SELECT sum(SD.convert_qty_two) as mt_booked, sum(SD.qty) as case_booked FROM sauda_details SD, product_master PM 
							WHERE SD.sku_code=PM.prod_code $order_condition AND PM.vertical_value='HBC:Rasoi:BIB'";
		$res_quantity_booked = mysqli_query($link,$sql_quantity_booked);
		$row_quantity_booked = mysqli_fetch_assoc($res_quantity_booked);

		$mt_booked=$row_quantity_booked['mt_booked'];
		$case_booked=$row_quantity_booked['case_booked'];
		
		$balance=($final_allotted -$mt_booked);
		
		echo "<tr>
			<td align=\"right\">$final_allotted</td>
			<td align=\"right\">".$mt_booked."</td>
			<td align=\"right\">".$case_booked."</td>
