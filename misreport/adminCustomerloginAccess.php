...
<script type="text/javascript" src="ajax1.js"></script>
	...
	else
	{
	 ...
	 $sql_count = "SELECT COUNT(CM.customer_code) FROM customer_master CM,changepassword CH WHERE CH.emp_code=CM.customer_code AND CM.acedns='Y'";
	...
	if($GLOBALS[start] < 0)		 $GLOBALS[start] = 0;
	 ...
	 $sql="SELECT CM.* FROM customer_master CM,changepassword CH WHERE CH.emp_code=CM.customer_code AND CM.acedns='Y' 
ORDER BY CH.emp_code ASC";
	...
	/*$row=mysqli_fetch_assoc(mysqli_query($link,"SELECT COUNT(EM.emp_code) FROM employee_master EM,changepassword CH WHERE CH.emp_code=EM.emp_code
	 ...
	 $row=mysqli_fetch_assoc(mysqli_query($link,"SELECT COUNT(CM.customer_code) FROM customer_master CM,changepassword CH WHERE CH.emp_code=CM.customer_code
		AND CM.acedns='Y' ORDER BY CH.emp_code ASC"));
	$count=$row[0];
	//echo $sql;
...
<table width="100%" align="center" cellpadding="2" cellspacing="2" border="0" id="display_table">
		...
		<td valign="top" bgcolor="#FFFFFF">
			...
			?>
				<tr class="TDHEAD_SUB">
                    ...
                    <td width="15%" align="left" style="padding-left:20px;">DNS Customer code</td>
				...
				while($rec=mysqli_fetch_assoc($rs))
				{
					...
					$deviceid_sql="SELECT deviceid FROM changepassword WHERE  emp_code = '".$rec[customer_code]."' AND deviceid <>''";
					...
					$sqlroute="SELECT route_name FROM route_master WHERE route_code='".$rec['route_code']."'";
				...
				<tr onMouseOver="this.bgColor='<?=SCROLL_COLOR;?>'" onMouseOut="this.bgColor=''" class="body">
                    ...
                    <td align="left" valign="top" style="padding-left:20px;"><?=stripslashes($rec[customer_code]);?></td>
                    ...
                    <td align="left" valign="top" style="padding-left:20px;"><?=stripslashes($route_name);?></td>
                    ...
                    <td align="center"><?php if($countdeviceid>0){?><a href="javascript:clear_allocation('<?=$rec[customer_code];?>','<?=$GLOBALS[start]?>');" title=" Clear Allocation "
	...
	<br>
	<form name="frm_opts" action="adminCustomerloginAccess.php" method="post" >
		<input type="hidden" name="mode" value="<?=$_REQUEST['mode']?>">
		<input type="hidden" name="pageNo" value="<?=$_REQUEST[pageNo]?>">
		<input type="hidden" name="url" value="adminCustomerloginAccess.php">
...
function clear_allocation()
{
	...
	$GLOBALS['err_msg']="Customer device allocation has been cleared Successfully.";
	disphtml("main();");
}
?>
