...
class RABidRateUploadControllerV2 extends Controller{
    ...
    public function operationdbuploadbidrate(Request $request){
        ...
        //if($isverify==1){
					...
					foreach ($xml as $RA_bid_rate) {
						...
						foreach ($RA_bid_rate->RA_bid_rate_data as $RA_bid_rate_data) {
							foreach($RA_bid_rate_data->RA_bid_rate_details as $RA_bid_rate_details){
									...
									$margin_cost_RA=(float)$RA_bid_rate_details->margin_cost;
									$honeycomb_cost_RA=(float)$RA_bid_rate_details->honeycomb_cost;
									$detention_cost_RA=(float)$RA_bid_rate_details->detention_cost;
									
									$upper_limit_percent=0;
									$upper_limit_bid_rate=0;
									
									$without_freightdepot_bid_rate=$bid_rate-$primary_freight-$secondary_freight-$depot_cost-$GST_value-$margin_cost_RA-$honeycomb_cost_RA-$detention_cost_RA;
									...
									if($without_freightdepot_bid_rate >=$base_rate)
									{
									  ...
									  ->first();
										...
										$bid_status='ACCEPT';
										$counter_bid_rate='';
										$counter_bid_rate_GST_val='';
										$counter_bid_threshold='';
										
										/*if($bid_rate > $app_indicative_rate)
										{*/
											$sqlselectupperlimitdetails=$CUTDB->select("SELECT upper_limit FROM plant_product_wise_RA_rate 
																	WHERE plant_name= '".$plant_name."' AND 	prod_code='".$prod_code."' AND acedns='Y'");		
											if(count($sqlselectupperlimitdetails) >0){
												foreach ($sqlselectupperlimitdetails as $key => $upperlimitdetails) {
													$upper_limit=$upperlimitdetails ->upper_limit;
												}
											}
											else
											{
											...
											if($upper_limit > 0)
											{
												$upper_limit_percent=$upper_limit;
												$base_rate_four_percent=($base_rate+(($base_rate*$upper_limit_percent)/100));
												if($without_freightdepot_bid_rate > $base_rate_four_percent)
												{
													$upper_limit_bid_rate=$bid_rate;
										$bid_rate=($base_rate_four_percent+$primary_freight+$secondary_freight+$depot_cost+$margin_cost_RA+$honeycomb_cost_RA+$detention_cost_RA);
												$bid_rate=($bid_rate+(($bid_rate*$GST_percent)/100));
													$bid_rate=(float)round($bid_rate,0);
													$GST_value=(($bid_rate*$GST_percent)/(100+$GST_percent));
													$GST_value=(float)round($GST_value,2);
												}
												else
												{
													$upper_limit_percent=0;
								...
								$realization_per_case=$material_cost+$packing_cost+$margin_cost+$margin_cost_RA+$premium-$TD-$liquid_TD-$packing_realization;
									...
									else
									{
										...
										if($without_freightdepot_bid_rate >=$counter_bid_threshold)
										{
											...
											$counter_bid_jump_val=(float)(($base_rate*$counter_bid_jump)/100);
											//$margin_cost_RA=(float)$margin_cost_RA;
											$counter_bid_rate=$base_rate+$counter_bid_jump_val+$primary_freight+$secondary_freight+$depot_cost+$margin_cost_RA+$honeycomb_cost_RA+$detention_cost_RA;
											if($GST_percent >0){
												$counter_bid_rate_GST_val=($counter_bid_rate*$GST_percent)/100;
											...
											if(strpos($counter_bid_rate,'.')!=false)
					 						{
											   $counter_bid_rate=round($counter_bid_rate,2);
											   $counter_bid_rate_array=explode('.',$counter_bid_rate);
											   if(count($counter_bid_rate_array) >1){
												   if(strlen($counter_bid_rate_array[1])==1)
												   {
													  $counter_bid_rate_array[1]= $counter_bid_rate_array[1].'0';
												   }
												   else
												   {
													   $counter_bid_rate_array[1]=$counter_bid_rate_array[1];
												   }
	
												   if($counter_bid_rate_array[1]<=24)
												   {
														$counter_bid_rate_right_parts='00';
														$counter_bid_rate=$counter_bid_rate_array[0];
												   }
												   if($counter_bid_rate_array[1] > 24 && $counter_bid_rate_array[1]<=49)
												   {
														$counter_bid_rate_right_parts='50';
														$counter_bid_rate=$counter_bid_rate_array[0].'.'.$counter_bid_rate_right_parts;
												   }
												   if($counter_bid_rate_array[1] > 49 && $counter_bid_rate_array[1] <=74)
												   {
														$counter_bid_rate_right_parts='50';
														$counter_bid_rate=$counter_bid_rate_array[0].'.'.$counter_bid_rate_right_parts;
												   }
												   if($counter_bid_rate_array[1] > 74)
												   {
														$counter_bid_rate_right_parts='00';
														$counter_bid_rate=$counter_bid_rate_array[0]+1;
												   }
											   }
											   else
											   {
									  ...
									  ->first();
						   			if(count($sqlselectbidrate)==0){
										$sqlInsertbidrate=$CUTDB->table('RA_bid_rate_details')
											...
											'qty'=>$qty,
											'bid_rate'=>$bid_rate,
											'bid_rate_w_all'=>$without_freightdepot_bid_rate,
											...
											'GST_value'=>$GST_value,
											'GST_value_counter_bid'=>$counter_bid_rate_GST_val,
											'branch_code'=>$branch_code,
											'vertical_value'=>$vertical_value,
											'incoterms'=>$incoterms,
											'margin_cost'=>$margin_cost_RA,
											'honeycomb_cost'=>$honeycomb_cost_RA,
											'detention_cost'=>$detention_cost_RA,
											'upper_limit_percent'=>$upper_limit_percent,
											'upper_limit_bid_rate'=>$upper_limit_bid_rate,
											'download_time'=>$location_date,
											));
									  if($sqlInsertbidrate){
											$flag=5;
        	...
        	$url = url('/api/v2/RAbidrateuploadV2?nick_name='.$nick_name.'&emp_code='.$emp_code);
