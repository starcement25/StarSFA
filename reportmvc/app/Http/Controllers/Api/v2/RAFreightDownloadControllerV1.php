...
class RAFreightDownloadControllerV1 extends Controller
{
    /**
    ...
    public function freightdownload(Request $request){
       ...
       // echo count($sqlquery);
		if(count($sqlquery)>0){
			$contentsrowcolumn  =count($sqlquery).'##'.'8';
			foreach($sqlquery as $rowfreightdownload){
				...
				$contents  .= (($rowfreightdownload->branch_code!='')?$rowfreightdownload->branch_code: ' ');
          		$linecontents  .= $contents."\n";
			}
			$datacontents = Apicommonfunction::encrypt($contentsrowcolumn."\n".str_replace("\r","",$linecontents));
            $datetime = gmdate('Y-m-d H:m:s',strtotime('+330 minute'));
            $url =url('/api/v2/RAfreightdownloadV1?nick_name='.$nick_name.'&emp_code='.$emp_code);
