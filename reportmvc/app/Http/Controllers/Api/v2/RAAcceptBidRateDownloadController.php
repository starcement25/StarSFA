...
class RAAcceptBIdRateDownloadController extends Controller
{
    ...
    public function acceptbidratedownload(Request $request){
		...
		$login_condition=" AND UNIX_TIMESTAMP(NOW()) > UNIX_TIMESTAMP(DATE_FORMAT(SUBSTRING(RAD.bid_id,-14,14),'%Y%-%m-%d %H:%i:%s') + INTERVAL 15 MINUTE)";				 
...
$smsstring="Dear Customer,\nThank you for participating in the Emami bidding process\nCongratulations! Your bid has been accepted\n".$skustring."\nTeam HBC";
            ...
            $url =url('/api/v2/acceptbidratedownload?nick_name='.$nick_name.'&emp_code='.$emp_code.'&last_update_time='.$last_update_time);
