...
class RABidRateUploadControllerV1 extends Controller{
					...
					foreach ($xml as $RA_bid_rate) {
						...
						foreach ($RA_bid_rate->RA_bid_rate_data as $RA_bid_rate_data) {
							foreach($RA_bid_rate_data->RA_bid_rate_details as $RA_bid_rate_details){
									...
									$without_freightdepot_bid_rate=$bid_rate-$primary_freight-$secondary_freight-$depot_cost-$GST_value-$margin_cost_RA-$honeycomb_cost_RA;
									...
									if($without_freightdepot_bid_rate >=$base_rate)
									{
										...
										$bid_rate=($base_rate_four_percent+$primary_freight+$secondary_freight+$depot_cost+$margin_cost_RA+$honeycomb_cost_RA);
										...
										$realization_per_case=$material_cost+$packing_cost+$margin_cost+$margin_cost_RA+$premium-$TD-$liquid_TD-$packing_cost;
									...
									else
									{
										...
										if($without_freightdepot_bid_rate >=$counter_bid_threshold)
										{
											...
											$counter_bid_rate=$base_rate+$counter_bid_jump_val+$primary_freight+$secondary_freight+$depot_cost+$margin_cost_RA+$honeycomb_cost_RA;
											if($GST_percent >0){
