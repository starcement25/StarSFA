...
class RAMarginDownloadController extends Controller{
  /**
  ...
  public function margindownloadincremental(Request $request){
	...
	/*echo "SELECT * FROM (SELECT MC.dns_prod_code,DATE_FORMAT(SUBSTRING(MC.datetime,1,10),'%d-%m-%Y') As last_updated_date,
      ...
      $sqlquery=$CUTDB->select("SELECT * FROM (SELECT MC.dns_prod_code,DATE_FORMAT(SUBSTRING(MC.datetime,1,10),'%d-%m-%Y') As last_updated_date,
				MC.margin_cost,MC.state_code,MC.zone FROM margin_cost_RA MC WHERE 1 ".$login_condition.$condition_one." ORDER BY MC.datetime DESC) AS SAT GROUP BY 1,4,5 ORDER BY 2 DESC");
      $count=count($sqlquery);
      	...
      	if($count>0){
			...
			$contentsdatetime =$year.'-'.$month.'-'.$date.'€'.$hour.':'.$minute.':'.$second."\n";
      	 	foreach($sqlquery as $rowmargin){
				
				
				$contents  = (($rowmargin->dns_prod_code!='')?$rowmargin->dns_prod_code: ' ')."^";
      			$contents  .= (($rowmargin->state_code!='')?$rowmargin->state_code: ' ')."^";
      			$contents  .= (($rowmargin->margin_cost!='')?$rowmargin->margin_cost: '0')."^";
      			$contents  .= (($rowmargin->zone!='')?$rowmargin->zone: ' ');
			...
			$url = url('/api/v2/margindownload?nick_name='.$nick_name.'&emp_code='.$emp_code.'&last_update_time='.$last_update_time);
