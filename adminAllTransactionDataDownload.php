		...
		<td valign="top" bgcolor="#FFFFFF" >
			...
			<br><br>
				...
				if($_REQUEST['mode']=='data_download'){
					...
					{
						$date_condition=" AND DATE_FORMAT(SUBSTRING(OH.order_no,7,8),'%Y-%m-%d') >='".$from_date."' 
										AND DATE_FORMAT(SUBSTRING(OH.order_no,7,8),'%Y-%m-%d') <='".$to_date."'";
				 ...
				 //For Transaction csv download
					...
					$sql_order_header_download = "select BM.branch_name,RM.rds_name,EM.emp_name,EM.emp_code,
											DATE_FORMAT(SUBSTRING(LO.trans_id,7,8),'%d-%m-%Y') AS transaction_date,LO.trans_id,
											...
											".$condition.$date_condition." 
																	UNION ALL
											select BM.branch_name,RM.rds_name,EM.emp_name,EM.emp_code,DATE_FORMAT(SUBSTRING(LO.trans_id,7,8),'%d-%m-%Y') 
											...
											ORDER BY rds_name,DATE_FORMAT(SUBSTRING(trans_id,7,8),'%Y-%m-%d'),transaction_type,prod_desc";
		//exit();
			...
			{
				$sql_query_order_header_SA="SELECT BM.branch_name,RM.rds_name,EM.emp_name,DATE_FORMAT(SUBSTRING(LO.trans_id,7,8),'%d-%m-%Y') 
										AS transaction_date,LO.trans_id,
										...
										AND LO.trans_id=OH.order_no ".$date_condition." ORDER BY DATE_FORMAT(SUBSTRING(trans_id,7,8),'%Y-%m-%d')";
				...
				$sql_query_freight="SELECT BM.branch_name,RM.rds_name,EM.emp_name,DATE_FORMAT(FE.date,'%d-%m-%Y') AS transaction_date,FE.date,
									...
									AND EM.branch_code=BM.branch_code AND RM.emp_code=EM.emp_code ".$date_condition_freight." ORDER BY DATE_FORMAT(SUBSTRING(FE.date,8,8),'%Y-%m-%d')";
