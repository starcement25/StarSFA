...
//exit();
/*$body="<?xml version='1.0' encoding='UTF-8'?><root><stock_audit><location><emp_code><![CDATA[E0002]]></emp_code><trans_id><![CDATA[SE000220210127185658]]></trans_id><latt><![CDATA[37.421998333333335]]></latt><longi><![CDATA[-122.08400000000002]]></longi><date><![CDATA[2021-01-27 18:56:58]]></date></location><stock_audit_details><transaction_id><![CDATA[SE000220210127185658]]></transaction_id><customer_code><![CDATA[C/0030564]]></customer_code><product_code><![CDATA[12002]]></product_code><prod_mrp><![CDATA[]]></prod_mrp><prod_details><![CDATA[]]></prod_details><remarks><![CDATA[test]]></remarks><HINT_REMARKS><![CDATA[]]></HINT_REMARKS><mfd_date><![CDATA[]]></mfd_date><UOM><![CDATA[CASE]]></UOM><quantity><![CDATA[12]]></quantity></stock_audit_details><stock_audit_details><transaction_id><![CDATA[SE000220210127185658]]></transaction_id><customer_code><![CDATA[C/0030564]]></customer_code><product_code><![CDATA[12025]]></product_code><prod_mrp><![CDATA[]]></prod_mrp><prod_details><![CDATA[]]></prod_details><remarks><![CDATA[test]]></remarks><HINT_REMARKS><![CDATA[]]></HINT_REMARKS><mfd_date><![CDATA[]]></mfd_date><UOM><![CDATA[CASE]]></UOM><quantity><![CDATA[12]]></quantity></stock_audit_details><stock_audit_details><transaction_id><![CDATA[SE000220210127185658]]></transaction_id><customer_code><![CDATA[C/0030564]]></customer_code><product_code><![CDATA[12003]]></product_code><prod_mrp><![CDATA[]]></prod_mrp><prod_details><![CDATA[]]></prod_details><remarks><![CDATA[test]]></remarks><HINT_REMARKS><![CDATA[]]></HINT_REMARKS><mfd_date><![CDATA[]]></mfd_date><UOM><![CDATA[PC]]></UOM><quantity><![CDATA[500]]></quantity></stock_audit_details></stock_audit></root>";*/
...
$audit_TA_DA_mode="*ROOT*STOCK_AUDIT*LOCATION*TA_DA_MODE";
...
$stockaudit_UOM = "*ROOT*STOCK_AUDIT*STOCK_AUDIT_DETAILS*UOM";
$stockaudit_weightage = "*ROOT*STOCK_AUDIT*STOCK_AUDIT_DETAILS*WEIGHTAGE";
...
class xml_audit{
	var $audit_emp_code,$audit_trans_id,$audit_latt,$audit_longi,$audit_trans_date,$audit_TA_DA_mode;	
}
class xml_stock_audit{
	...
	$stockaudit_remarks,$stockaudit_hint_remarks,$stockaudit_mfd_date,$stockaudit_UOM,$stockaudit_weightage;
}
...
function contents($parser, $data){
	...
	$audit_emp_code,$audit_trans_id,$audit_latt,$audit_longi,$audit_trans_date,$audit_TA_DA_mode,$stockaudit_trans_id,$stockaudit_customer_code,$stockaudit_product_code,$stockaudit_qty,$stockaudit_prod_mrp,$stockaudit_prod_details,$stockaudit_remarks,$stockaudit_hint_remarks,$stockaudit_mfd_date,$stockaudit_UOM,$stockaudit_weightage;
	//echo $current_tag.'<br />';
	//echo $data;
	if(substr($current_tag,0,17)=='*ROOT*STOCK_AUDIT')
	{
		...
		switch($current_tag){
			...
			case $audit_trans_date:
				...
				break;
			case $audit_TA_DA_mode:
				$audit_array[$counteraudit]->audit_TA_DA_mode = $data;
				$counteraudit++;
				break;	
		}
	}
	if(substr($current_tag,0,37)=='*ROOT*STOCK_AUDIT*STOCK_AUDIT_DETAILS')
		{
			...
			switch($current_tag){
				...
				case $stockaudit_mfd_date:
					 ...
					 break; 
				case $stockaudit_UOM:
					$stock_audit_array[$counterstockaudit]->stockaudit_UOM = $data;
					break;	
				case $stockaudit_qty:
					$stock_audit_array[$counterstockaudit]->stockaudit_qty = $data;
					break;
				case $stockaudit_weightage:
					$stock_audit_array[$counterstockaudit]->stockaudit_weightage = $data;
					$counterstockaudit++;
					break;	
...
if(count($audit_array)>0)
{
	for($x=0;$x<count($audit_array);$x++){
		...
		$audit_TA_DA_mode=$audit_array[$x]->audit_TA_DA_mode;
		...
		else
		{
			...
			{
				$TA_DA_condiion=" TA_DA_mode	='".$audit_TA_DA_mode."',";
			}
			else
			{
				$TA_DA_condiion='';
			...
			$sqlinsertauditlocation="INSERT INTO location SET emp_code='".$audit_emp_code."',
								  ...
								  latt='".$audit_latt."',
								  longi='".$audit_longi."',
								  date='".$audit_trans_date."',".$TA_DA_condiion."
	...
	//For Insert into the audit transaction table for new trans id
		if(count($stock_audit_array)>0)
		{
			for($i=0;$i<count($stock_audit_array);$i++){
				...
				$stockaudit_UOM=$stock_audit_array[$i]->stockaudit_UOM;
				$stockaudit_weightage=$stock_audit_array[$i]->stockaudit_weightage;
					...
					{
						$sqlinsertUOMcond=" UOM='".$stockaudit_UOM."',";
						$sqlinsertweightagecond=" weightage='".$stockaudit_weightage."',";
					}
					else { $sqlinsertUOMcond=''; $sqlinsertweightagecond='';}
					
					if($stockaudit_prod_mrp==""){
					    $stockaudit_prod_mrp="0";
					}
					...
					if(!in_array($stockaudit_trans_id,$audit_array_receipt_id_exists))
					{
						$sqlinsertstockaudit="INSERT INTO stock_audit SET transaction_id ='".$stockaudit_trans_id."',
											  ...
											  hint_remarks			='".$stockaudit_hint_remarks."',".$sqlinsertcond.$sqlinsertUOMcond.$sqlinsertweightagecond."
											  remarks 				='".addslashes($stockaudit_remarks)."'";											//echo  $sqlinsertstockaudit;
					   ...
					   //Start for STAR mis data details stock audit qty updation
					  	if($nick_name=='STAR')
						{
							...
							$trans_type='S';
							$trans_sub_type='';
							update_transaction_STAR($emp_code,$stockaudit_trans_id,$qty,$trans_type,$trans_sub_type,$link);
					  ...
					  if($flag==5)
					  {
						  ...
						  if($cust_type != 'R')
						  {
							...
							if($countselectprevstock>0)
							{
								...
								if($visit3_date==""){
								    $visit3_date = date("Y-m-d H:i:s");;
								}
								
								if($visit2_date==""){
								    $visit2_date = date("Y-m-d H:i:s");;
								}
							...
							else
							{
								...
								$sqlinsertprevstock="INSERT INTO prev_stock_counting_master SET 
													visit_1 ='0',
													visit1_date='1971-01-01 00:00:01',
													visit_2 ='0',
													visit2_date='1971-01-01 00:00:01',
													visit_3 ='".$stockaudit_qty."',
													visit3_date='".$audit_trans_date."',
													customer_code='".$stockaudit_customer_code."',
													product_code='".$stockaudit_product_code."',
