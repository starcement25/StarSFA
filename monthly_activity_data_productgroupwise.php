...
/*----> Total Calls & Days Present <----*/
	...
	$sql_total_calls = "SELECT  SUM(CASE WHEN POCM.order_no LIKE 'O%' AND POCM.cust_type='D' THEN POCM.visit_qty ELSE 0 END) AS primary_val,
						SUM(CASE WHEN POCM.order_no LIKE 'O%' AND POCM.cust_type='R' THEN (POCM.visit_qty/PM.conversion_factor_two) ELSE 0 END) 
						AS secondary_val,
						EM.designation,EM.emp_name,PM.product_group_code,EM.emp_code
		...
		if($total_rows>0){
			?><table width='100%' class='border' border='1' style='border-collapse:collapse;' cellpadding='6px'>
             <tr class='TDHEAD'><td colspan='14' align='center'>SKU Wise Report - From <?php echo date('d-m-Y',strtotime($start_date));?> To <?php echo date('d-m-Y',strtotime($end_date));?></td></tr>
              <tr class="TDHEAD_SUB">
                <td>SL No</td>
                <td>Emp Name</td>
                <td>Designation</td>
