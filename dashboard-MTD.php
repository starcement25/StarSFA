...
//From KSM
/*select ISNULL(a.TAG,'NA') TAG,SUM(a.QTYSHIPPED) AS tot_qty,SUM(a.TBASE1) from ( select IH.CUSTOMER,ID.*,(select ISNULL(VALUE,'NA') from ARCUSO where IDCUST=IH.CUSTOMER AND OPTFIELD='ARZONE') TAG FROM OEINVD ID INNER JOIN OEINVH IH ON ID.INVUNIQ=IH.INVUNIQ --and ACCTSET='FG' and SUBSTRING(CAST(IH.INVDATE AS varchar(20)),1,6)='202001' AND (IH.location LIKE '1%' or IH.location LIKE 'RUIYA%' or IH.location LIKE '3004%') UNION select IH.CUSTOMER,ID.*,c.zone TAG FROM OEINVD ID INNER JOIN OEINVH IH ON ID.INVUNIQ=IH.INVUNIQ INNER JOIN sales_hierarchy_new c ON c.SUBZONE=SUBSTRING(CAST(IH.location AS varchar(20)),1,4) --and ACCTSET='FG' and SUBSTRING(CAST(IH.INVDATE AS varchar(20)),1,6)='202001' AND (IH.location LIKE '3%' and IH.location not LIKE '3004%') ) a group by a.TAG 

select ISNULL(SEGMENT,'NA') SEGMENT,SUM(a.QTYSHIPPED) AS tot_qty,SUM(a.TBASE1) from ( select IH.CUSTOMER,ID.*,(select ISNULL(VALUE,'NA') from ARCUSO where IDCUST=IH.CUSTOMER AND OPTFIELD='ARZONE') TAG, (select ISNULL(VDESC,'NA') from CSOPTFD where value in(select value from ICITEMO where OPTFIELD='ITEMSEG' and ITEMNO=ID.ITEM) and OPTFIELD='ITEMSEG' ) SEGMENT FROM OEINVD ID INNER JOIN OEINVH IH ON ID.INVUNIQ=IH.INVUNIQ --and ACCTSET='FG' and SUBSTRING(CAST(IH.INVDATE AS varchar(20)),1,6)='202001' AND (IH.location LIKE '1%' or IH.location LIKE 'RUIYA%' or IH.location LIKE '3004%') UNION select IH.CUSTOMER,ID.*,c.zone TAG,(select ISNULL(VDESC,'NA') from CSOPTFD where value in(select value from ICITEMO where OPTFIELD='ITEMSEG' and ITEMNO=ID.ITEM) and OPTFIELD='ITEMSEG' ) SEGMENT FROM OEINVD ID INNER JOIN OEINVH IH ON ID.INVUNIQ=IH.INVUNIQ INNER JOIN sales_hierarchy_new c ON c.SUBZONE=SUBSTRING(CAST(IH.location AS varchar(20)),1,4) --and ACCTSET='FG' and SUBSTRING(CAST(IH.INVDATE AS varchar(20)),1,6)='202001' AND (IH.location LIKE '3%' and IH.location not LIKE '3004%') ) a group by a.SEGMENT*/
