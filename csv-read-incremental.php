...
function csv_upload(){
				...
				{ 
				  ...
				  //print_r($data);
					...
					else
					{
						...
						if($vertical_value_existing!=$vertical_value)
						{
			...
			foreach($lines as $line)
			{
				...
				{ 
				  ...
				  //print_r($data);
					...
					$csv_row_count=$rec_count+1;
					$sqlprodbrandnamechk="SELECT product_brand_name,product_sub_group_code FROM product_brand_master 
										 WHERE product_brand_name='".addslashes($product_brand_name)."' 
										 AND product_sub_group_code='".$product_sub_group_code."'";
					...
					if($countprodbrandnamechk<1){
						...
						$sqlbrandsubform .= "  product_brand_code='".mysqli_real_escape_string($product_brand_name)."'";
						...
						$sqlbrandsubform .= " , product_brand_name='".mysqli_real_escape_string($product_brand_name)."'";
					...
					else
					{
						...
						$vertical_value_existing=$rowprodbrandnamechk['vertical_value'];
						if($product_sub_group_name_existing!=$product_group_name || $vertical_value_existing!=$vertical_value)
						{
							$sqlupdatebrandsubform  = "UPDATE product_brand_master SET ";
							$sqlupdatebrandsubform .= " vertical_value='".mysqli_real_escape_string($vertical_value)."'";
							$sqlupdatebrandsubform .= " , download_time=CURRENT_TIMESTAMP()";
							$sqlupdatebrandsubform .= "  WHERE product_brand_name='".addslashes($product_sub_group_name)."' AND product_sub_group_code='".$product_sub_group_code."'";
							mysqli_query($link,$sqlupdatebrandsubform) or die(mysqli_error().".Internel error occurrs @row $csv_row_count on Brand SUB Form Master.csv.Please check.");
						}
					}
				}
				 $rec_count++;
			...
			foreach($lines as $line)
			{
				...
				//print_r($data);
					...
					if(no_of_filter > 2){
						...
						$sqlprodsubgroupnamechk="SELECT product_sub_group_code FROM product_sub_group_master WHERE product_sub_group_name='".addslashes($product_sub_group_code_name)."' AND product_group_code='".$product_group_code."'";
						$rsprodsubgroupnamechk=mysqli_query($link,$sqlprodsubgroupnamechk);
						$countprodsubgroupnamechk=mysqli_num_rows($rsprodsubgroupnamechk);
						if($countprodsubgroupnamechk<1){
					...
					{*/
					$sqlskunamechk="SELECT * FROM product_master WHERE prod_desc='".$prod_desc."' AND branch_code='".$branch_code."' AND dns_prod_code='".$dns_prod_code."' 
			  ...
			  //print_r($data);
				...
				if(providing_code=='yes'){
					...
					$sqlproductcode="SELECT prod_code FROM product_master WHERE dns_prod_code='".$prod_code_name."' AND 
									product_group_code='".addslashes($brand_code_name)."' AND product_sub_group_code='".addslashes($brand_form_code_name)."' 
									AND product_brand_code='".addslashes($brand_sub_form_code_name)."'";
				}
				else
				{
				...
				$sqlclstkchk="SELECT branch_code FROM branch_product_wise_stock WHERE branch_code='".$branch_code."' 
	...
	if(similar_file_exists("csv/$folderName/Customer Master.csv")!=false)
	{
				...
				{ 
				   ...
				   $data[]=$value;
					...
					else
					{
						...
						if($route_code_db!=$route_code || $emp_code_db!=$emp_code || $current_balance_db!=$current_balance || $acedns_db!=$acedns 
						|| $black_list_db!=$black_list || $TD_db!=$TD || $customer_type_db!=$customer_type || $rds_tag_db!=$rds_code 
						|| $branch_code_db!=$branch_code || $sauda_validity_period_db!= $sauda_validity_period || $credit_days_db!= $credit_days 
						|| $customer_name_db!=$customer_name || $dns_customer_code_db!=$dns_customer_code || $phone_no_db!=$phone_no || $address_db!=$address || $owner_name_db!=$owner_name || $owner_phone_db!=$owner_phone || $cust_class_db!=$cust_class || $weekly_closing_day_db!=$weekly_closing_day || $TIN_db!=$TIN || $PAN_db!=$PAN || $district_db!=$district || $zone_db !=$zone)
						{
		...
		//For customer master csv of PARLE
		if(similar_file_exists("csv/$folderName/Customer Master-parle.csv")!=false)
		{
		$filename=similar_file_exists("csv/$folderName/Customer Master-parle.csv");
		$rec_count = 0;
		$ins_count = 0;
		$err = "";
				...
				{ 
				   ...
				   $data[]=$value;
					...
					else
					{
						...
						|| $black_list_db!=$black_list || $TD_db!=$TD || $customer_type_db!=$customer_type || $rds_tag_db!=$rds_code 
						|| $branch_code_db!=$branch_code || $sauda_validity_period_db!= $sauda_validity_period || $credit_days_db!= $credit_days 
						|| $customer_name_db!=$customer_name || $dns_customer_code_db!=$dns_customer_code || $phone_no_db!=$phone_no || $address_db!=$address || $owner_name_db!=$owner_name || $owner_phone_db!=$owner_phone || $cust_class_db!=$cust_class || $weekly_closing_day_db!=$weekly_closing_day || $TIN_db!=$TIN || $PAN_db!=$PAN)
						{
							$sqlupdated  = "update customer_master ";
							$sqlupdated .= " SET route_code='".$route_code."'";
							$sqlupdated .= " , dns_customer_code='".$dns_customer_code."'";
				...
				{ 
				  ...
				  //print_r($data);
					...
					else
					{
						...
						$sqlcustomercode="SELECT customer_code FROM customer_master WHERE customer_name='".addslashes($customer_code_name)."' AND route_code='".$route_code."'";*/
						$sqlcustomercode="SELECT customer_code,emp_code FROM customer_master WHERE customer_name='".addslashes($customer_code_name)."' and download_time=(SELECT MAX(download_time) FROM customer_master WHERE customer_name='".addslashes($customer_code_name)."')";
					}
	...
	if(similar_file_exists("csv/$folderName/MRP.csv")!=false)
	{
			...
			foreach($lines as $line)
			{
				...
				//print_r($data);
					...
					if($destination_code_name!='')
					{
						...
						{
							$sqldestinationcode="SELECT destination_code FROM destination_master WHERE 
												destination_name='".addslashes($destination_code_name)."'";
						}
					}
			...
			foreach($lines as $line)
			{
				...
				{ 
				  ...
				  //print_r($data);
					...
					if(providing_code=='yes')
					{
						$sqlbrokerchk="SELECT broker_id FROM broker_master WHERE broker_id='".addslashes($dns_broker_id)."'";
					}
					else
					{
					...
					else
					{
						...
						mysqli_query($link,$sqlupdatevendor) or die(mysqli_error().".Internal error occurrs @row $csv_row_count in Broker master.csv.Please check.");
					}
				}
				 $rec_count++;
			}		
		...
		if(similar_file_exists("csv/$folderName/mall_histreet_names(1).csv")!=false){
			$filename=similar_file_exists("csv/$folderName/mall_histreet_names(1).csv");
			$rec_count = 0;
			$ins_count = 0;
			$err = "";
			...
			foreach($lines as $line)
			{
				...
				{ 
				  ...
				  //print_r($data);
					...
					$replacing_mall=trim($data[0]);
					$actual_mall=trim($data[1]);
						$sqlupdatebroker  = "UPDATE survey_header SET ";
						$sqlupdatebroker .= "  mall_hs_name='".mysqli_real_escape_string($actual_mall)."'";
						$sqlupdatebroker .= "  ,download_time=CURRENT_TIMESTAMP()";
						$sqlupdatebroker .= " WHERE mall_hs_name='".mysqli_real_escape_string($replacing_mall)."'";
						mysqli_query($link,$sqlupdatebroker) or die(mysqli_error().".Internal error occurrs @row $csv_row_count in mall master.csv.Please check.");
				}
				 $rec_count++;
			}		
			$successval=1;
	 ...
	 //For Employee target achievement CSV
		...
		//For Customer CSV
	if(similar_file_exists("csv/$folderName/Customer_Master3.csv")!=false)
	{
		$filename=similar_file_exists("csv/$folderName/Customer_Master3.csv");
		$rec_count = 0;
		$ins_count = 0;
		$err = "";
				...
				{ 
				   ...
				   $data[]=$value;
					...
					if($countroutechk<1 && $route_name!='')
					 ...
					 echo $sqlcustomernamechk="SELECT * FROM customer_master WHERE customer_code='".addslashes($dns_customer_code)."'";
					/*$sqlcustomernamechk="SELECT * FROM customer_master WHERE customer_name='".addslashes($customer_name)."'";*/
					$rscustomernamechk=mysqli_query($link,$sqlcustomernamechk);
					$countcustomernamechk=mysqli_num_rows($rscustomernamechk);
					...
					if($countcustomernamechk>0)
					{
						...
						$dns_customer_code_db=$rowcustomernamechk['dns_customer_code'];*/
							...
							echo $sqlupdated .= " , rds_tag='".$rds_code."',download_time=CURRENT_TIMESTAMP() 
											 WHERE   customer_code='".addslashes($dns_customer_code)."'";
							mysqli_query($link,$sqlupdated) or array_push($error_array,".Internel error occurrs @row $csv_row_count on Customer Master.csv.Please check.");					
							}
							//modifyempdatadownloadlog($emp_code,strtoupper($folderName));
						$customer_code=$customer_code_db;
	...
	//For Branch Master CSV
	if(similar_file_exists("csv/$folderName/Branch code update.csv")!=false)
	{
		$filename=similar_file_exists("csv/$folderName/Branch code update.csv");
		$rec_count = 0;
		$ins_count = 0;
		$err = "";
		...
		foreach($lines as $line)
		{
			...
			{ 
			  ...
			  //print_r($data);
				...
				$old_branch_code=trim($data[1]);
				$new_branch_code=trim($data[2]);

					$sqlbranch  = "UPDATE branch_master ";
					$sqlbranch .= "  SET	dns_branch_code='".mysqli_real_escape_string($new_branch_code)."'";
					$sqlbranch .= " , download_time=CURRENT_TIMESTAMP()";
					echo $sqlbranch .= " WHERE branch_name='".addslashes($branch_name)."' AND dns_branch_code='".$old_branch_code."'";
	...
	/*else
	{
		...
		if(similar_file_exists("csv/$folderName/Customer Master ABOS.csv")!=false)
		{
		$filename=similar_file_exists("csv/$folderName/Customer Master ABOS.csv");
		$rec_count = 0;
		$ins_count = 0;
		$err = "";
			...
			foreach($lines as $line)
			{
				...
				{ 
				  ...
				  //print_r($data);
					...
					$random_no_length=7-strlen($folderName);//7 is the maximum length of the company nick name
					$foldernamerand=$folderName.rand(pow(10, $random_no_length-1), pow(10, $random_no_length)-1);*/

					//------------------------------------------For customer route creation------------------------------------------------------------------
					$sqlcustomersel="SELECT * FROM customer_master WHERE customer_name='D. HARBOUR' AND customer_code='C/0000003' order by customer_name ASC";
					...
					while($rowcustomersel=mysqli_fetch_assoc($rscustomersel))
					{
						...
						echo $sqlroutechk="SELECT * FROM route_master WHERE route_name=(SELECT route_name FROM route_master WHERE 
						...
						{	
							...
							while($rowroutechk=mysqli_fetch_assoc($rsroutechk))
							{
								if($countroutechk==1)
								{
									...
									echo $sqlupdateroute="UPDATE route_master SET emp_code='',flag='1',download_time=CURRENT_TIMESTAMP() 
							...
							echo $sqlselcustomerprevroute="SELECT customer_code,customer_name,emp_code FROM customer_master WHERE 
							...
							while($rowselcustomerprevroute=mysqli_fetch_assoc($rsselcustomerprevroute))
							{
								...
								if(in_array($customer_name_previous,$customer_name_prev_array))
								{
									echo 'duplicate exists';
									array_push($customer_code_del_prev_array,$customer_code_previous);
									array_push($emp_code_del_prev_array,$emp_code_previous);
									$key_val=array_search($customer_name_previous,$customer_name_prev_array);
									array_push($key_val_array,$key_val);
								}
								else
								{
									array_push($customer_code_prev_array,$customer_code_previous);
									array_push($emp_code_prev_array,$emp_code_previous);
									array_push($customer_name_prev_array,$customer_name_previous);
								}
								//$customer_code_prev_string.="'".$rowselcustomerprevroute['customer_code']."'".',';
							}
							for($k=0;$k<count($customer_code_prev_array);$k++)
							{
								echo $sqlinsertcustomerroute="INSERT INTO customer_route_emp_relation SET customer_code='".$customer_code_prev_array[$k]."',
														 	route_code='".$route_code_present."',
															emp_code='".$emp_code_prev_array[$k]."',
															acedns='Y',
															download_time=CURRENT_TIMESTAMP()";
								...
								$sqlupdatepevcustomer="UPDATE customer_master SET route_code='".$route_code_present."',emp_code='' 
													WHERE customer_code='".$customer_code_prev_array[$k]."'";
								mysqli_query($link,$sqlupdatepevcustomer);
							}
							print_r($key_val_array);
							for($i=0;$i<count($key_val_array);$i++)
							{
								$key_val_present=$key_val_array[$i];
								$sqlupdateorder="UPDATE order_header SET customer_code='".$customer_code_prev_array[$key_val_present]."' 
													WHERE customer_code='".$customer_code_del_prev_array[$i]."'";
								mysqli_query($link,$sqlupdateorder);
								$sqlupdateprevordercounting="UPDATE prev_order_counting_master SET 
														customer_code='".$customer_code_prev_array[$key_val_present]."' 
													WHERE customer_code='".$customer_code_del_prev_array[$i]."'";
								mysqli_query($link,$sqlupdateprevordercounting);					
								$sqlupdatepaymentheader="UPDATE payment_header SET customer_code='".$customer_code_prev_array[$key_val_present]."' 
													WHERE customer_code='".$customer_code_del_prev_array[$i]."'";
								mysqli_query($link,$sqlupdatepaymentheader);
								
								$sqlupdatestockaudit="UPDATE stock_audit SET customer_code='".$customer_code_prev_array[$key_val_present]."' 
													WHERE customer_code='".$customer_code_del_prev_array[$i]."'";
								mysqli_query($link,$sqlupdatestockaudit);
								
								$sqlupdateroutecustomerplan="UPDATE route_customer_plan SET customer_code='".$customer_code_prev_array[$key_val_present]."' 
													WHERE customer_code='".$customer_code_del_prev_array[$i]."'";
								mysqli_query($link,$sqlupdateroutecustomerplan);
								
								$selcustomerroute="SELECT customer_code FROM customer_route_emp_relation WHERE 
													customer_code='".$customer_code_prev_array[$key_val_present]."' AND route_code='".$route_code_present."' 
								...
								echo $sqldelpevcustomer="DELETE FROM customer_master WHERE customer_code='".$customer_code_del_prev_array[$i]."'";
								mysqli_query($link,$sqldelpevcustomer);
							}
							echo $sqldelprevroute="DELETE FROM route_master WHERE route_code IN(".$route_code_prev_string.")";
			...
			//
	if(similar_file_exists("csv/$folderName/retailer_distributor_mapping1.csv")!=false)
	{
		$filename=similar_file_exists("csv/$folderName/retailer_distributor_mapping1.csv");
		$rec_count = 0;
		$ins_count = 0;
		$err = "";
		...
		foreach($lines as $line)
		{
			...
			{ 
			  ...
			  //print_r($data);
				...
				$sqlroute="SELECT route_code FROM route_master WHERE route_name='".addslashes($route_name)."'";
				$rsroute=mysqli_query($link,$sqlroute);
				$rowroute=mysqli_fetch_assoc($rsroute);
				$route_code=$rowroute['route_code'];
				echo $sqlselcustomercode="SELECT customer_code FROM customer_master WHERE customer_name='".addslashes($rds_tag)."' AND cust_type='D'";
				$rsselcustomercode=mysqli_query($link,$sqlselcustomercode);
				while($rowselcustomercode=mysqli_fetch_assoc($rsselcustomercode)){
					$customer_code_string.="'".$rowselcustomercode['customer_code']."'".',';
				}
				$customer_code_string=substr($customer_code_string,0,-1);
				$sqlempcodename="SELECT emp_code FROM employee_master WHERE emp_name='".addslashes($emp_name)."'";
				$rsempcodename=mysqli_query($link,$sqlempcodename);
				$rowempcodename=mysqli_fetch_assoc($rsempcodename);
				$emp_code=$rowempcodename['emp_code'];
				
				echo $selectdistcode="SELECT customer_code FROM customer_route_emp_relation WHERE emp_code='".$emp_code."' AND 
								customer_code IN(".$customer_code_string.")";
				$rsdistcode=mysqli_query($link,$selectdistcode);
				$rowdistcode=mysqli_fetch_assoc($rsdistcode);
				$distributorcode=$rowdistcode['customer_code'];				
				
					if($cust_type=='R')
					{
						$sqlupdate  = "UPDATE customer_master ";
						$sqlupdate .= "  SET	rds_tag='".mysqli_real_escape_string($distributorcode)."'";
						$sqlupdate .= " , download_time=CURRENT_TIMESTAMP()";
						echo $sqlupdate .= " WHERE customer_name='".addslashes($customer_name)."' AND route_code='".$route_code."'";
						exit();
