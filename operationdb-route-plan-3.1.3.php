...
class xml_route_plan{
	var $route_plan_trans_id,$route_plan_emp_code,$route_plan_route_code,$route_plan_route_name,$route_plan_visit_date,$route_plan_create_date;
}
...
function contents($parser, $data){
    global $current_tag,$route_plan_trans_id,$route_plan_emp_code,$route_plan_route_code,$route_plan_route_name,$route_plan_visit_date,
...
if(count($route_plan_array)>0)
{
   ...
   //print_r($route_plan_visit_date_event_array);	
	if($flag==5){
		...
		$sqlempname="SELECT emp_name FROM employee_master WHERE emp_code='".$route_plan_emp_code_array[$y]."'";
...
//echo $routeplanmailbody;
	  ...
	  $headers .= "From: ".FROMTAG."<".FROMEMAIL."> \r\n" .
					...
					'X-Mailer: PHP/' . phpversion();
		//echo 	$routeplanmailbody;		
		if(mail(ROUTEPLANMAILRECIPENTS, $routeplanmailsubj, $routeplanmailbody, $headers,'-facedns@acedns.in'))
		{
			$flag=6;
		}
		else
