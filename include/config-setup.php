...
//require_once(__DIR__ . '/../sfa_connection.php');
header('Content-Type: application/json');

// $localDB = new sfa_connection();
// $linksetup = $localDB->conn;
...
if (!is_array($roworderformdetails)) {
    $roworderformdetails = []; 
}
...
$sauda_rate_dependent_on_despatch_point = isset($rowsaudadetails['sauda_rate_dependent_on_despatch_point']) ? ['sauda_rate_dependent_on_despatch_point'] : '';
$state_branch_wise_TD = isset($rowsaudadetails['state_branch_wise_TD']) ? $rowsaudadetails['state_branch_wise_TD'] : '';
$incoterms_vertical = isset($rowsaudadetails['incoterms_vertical']) ? $rowsaudadetails['incoterms_vertical'] : '';
...
define("BASE_URL", 'https://sfa.starcement.co.in/');
define("DEV_BASE_URL", 'https://sfa.starcement.co.in/');
define("col1", $col1);
define("col2", $col2);
define("col3", $col3);
define("col4", $col4);
...
$incremental_download = $_REQUEST['incremental_download'] ?? null;
$last_update_time = $_REQUEST['last_update_time'] ?? null;

if (is_string($last_update_time) && $last_update_time !== '') {
    $last_update_time = str_replace('€', ' ', $last_update_time);
}
