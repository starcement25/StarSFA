...
//End for DSM
//For RSM
$sqlselzsmemail="SELECT email FROM employee_master WHERE level='5'";
$rsselzsmemail=mysqli_query($link,$sqlselzsmemail);
$rowselzsmemail=mysqli_fetch_assoc($rsselzsmemail);
$zsmemail=$rowselzsmemail['email'];
$sqlselrsmlevel="SELECT emp_code,emp_name,email,designation FROM employee_master WHERE level='4'";
...
while($rowselrsmlevel=mysqli_fetch_assoc($rsselrsmlevel))
{
	...
	//${dsm_avl_ff.$dsm_emp_code}=count(explode(',',${employee_hierarchy.$dsm_emp_code}))-1;
	$sql_get_empdetails_dsm="SELECT emp_code,emp_name FROM employee_master WHERE reporting_to='".$rsm_emp_code."' ORDER BY emp_name ASC";					
...
*/
	...
	$email_to_rsm=$rsm_emp_email.','.$zsmemail.','.'vikram.singh1@parle.biz';
	...
	@mail($email_to_rsm, $rsm_emp_mailsubj, ${table.$rsm_emp_code}, $rsm_emp_mail_headers);
	//@mail('dipankarc@coral.in', $rsm_emp_mailsubj, ${table.$rsm_emp_code}, $rsm_emp_mail_headers);
	}
}
//End for RSM
//For ZSM
$sqlselzsmlevel="SELECT emp_code,emp_name,email,designation FROM employee_master WHERE level='5'";
...
while($rowselzsmlevel=mysqli_fetch_assoc($rsselzsmmlevel))
{
	...
	$sql_get_empdetails_rsm="SELECT emp_code,emp_name FROM employee_master WHERE reporting_to='".$zsm_emp_code."' ORDER BY emp_name ASC";					
...
*/
	...
	$zsm_emp_mail_headers .= "From: ".FROMTAG."<".FROMEMAIL."> \r\n" .
				...
				"Bcc: ".BCCEMAIL." \r\n".
				'X-Mailer: PHP/' . phpversion();
	//$email_to_zsm	='dipankarc@coral.in';			
