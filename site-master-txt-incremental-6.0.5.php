...
$sqlquery="SELECT site_id,emp_code,site_name,address,city,location,sub_area,state,pin,contact_person,phone_no,site_reffered_by,
		...
		sub_dealer_involved,stage_of_construction,material_sold_value,desc_thickness_qty,veneer_species,veneer_qty,remarks,scope_of_teak,scope_of_NTD,
		follow_up_date,expected_month_maturity,escalation_clause,auth_retailer_involved,architect_involved,contractor_involved,area
				FROM site_master WHERE 
...
$count=mysqli_num_rows($result);
	...
	$url = APICALLLOGURL."/site-master-txt-incremental-6.0.5.php?nick_name=$nick_name&emp_code=$emp_code&last_update_time=$last_update_time&incremental_download=$incremental_download";
