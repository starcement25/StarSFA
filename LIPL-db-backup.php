...
$link = mysqli_connect('89.107.61.190','fmpowera_dnsprod','acednsprod1234'); 
mysqli_select_db('fmpowera_LIPL', $link); 
...
//cycle through 
//array_push($tables,'survey_header');
//array_push($tables,'survey_publish');
...
$file = 'db-backup-'.$date.$month.$year.$hour.$minute.$second.'.sql'; 
$zipfile = 'db-backup-'.$date.$month.$year.$hour.$minute.$second.'.zip';
$destination = "ftp://fmpoweracedns:iBf8d)u@]c[h@fmpower.acedns.in/public_html/lipl-bkup/$zipfile"; 
/*$handle = fopen($file,'w+'); 
...
$contents=$zip->file();
//$destination = "lipl-bkup/$zipfile";
//@unlink($destination);
//For deletion of file older than 7 days
/*$files = scandir("lipl-bkup");
$now   = time();
