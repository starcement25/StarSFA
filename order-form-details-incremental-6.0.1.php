...
$count=mysqli_num_rows($result);
	...
	if($count>0){
		...
		while($rowsorderformdetails = mysqli_fetch_assoc($result))
		{
				...
				$contents .='<order_form_id><![CDATA['.mb_convert_encoding($rowsorderformdetails['order_form_id'], 'UTF-8', 'UTF-8').']]></order_form_id>
							...
							<sauda_allocation_carry_forward><![CDATA['.mb_convert_encoding($rowsorderformdetails['sauda_allocation_carry_forward'], 'UTF-8', 'UTF-8').']]></sauda_allocation_carry_forward>
							<sauda_depot_wise><![CDATA['.mb_convert_encoding($rowsorderformdetails['sauda_depot_wise'], 'UTF-8', 'UTF-8').']]></sauda_depot_wise>
							<sauda_rate_variable><![CDATA['.mb_convert_encoding($rowsorderformdetails['sauda_rate_variable'], 'UTF-8', 'UTF-8').']]></sauda_rate_variable>
							<sauda_rate_variable_value><![CDATA['.mb_convert_encoding($rowsorderformdetails['sauda_rate_variable_value'], 'UTF-8', 'UTF-8').']]></sauda_rate_variable_value>
							';	
				$contents.="</data>";
				//echo $cnt++;
		}
	...
	$url = "http://www.acedns.in/acednsproduct/order-form-details-incremental-6.0.0.php?nick_name=$nick_name&last_update_time=$last_update_time&incremental_download=$incremental_download&mode=$mode";
	insertapilog($datetime,$emp_code,$url,$nick_name);
	/*$config = 'api_calllog.txt';
	$file=fopen($config,"r+");
	$date = date("F j, Y");
	$time = date("H:i:s");
	$newuser ="[$date $time]"."http://www.acedns.in/acednsproduct/order-form-details-incremental-6.0.0.php?nick_name=$nick_name&last_update_time=$last_update_time&incremental_download=$incremental_download&mode=$mode"."\r\n";
