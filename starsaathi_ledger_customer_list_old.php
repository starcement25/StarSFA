...
$emp_code=$_REQUEST['emp_code'];
	...
	$sqlcustlist="SELECT DISTINCT CC.dns_customer_code, CC.SAP_customer_code FROM customer_route_emp_relation CRR,customer_master CC WHERE CC.customer_code=CRR.customer_code 
					AND CRR.acedns='Y' $emp_hierarchy_condition";
		//echo"<pre>";print_r($sqlcustlist);die;			
	$rscustlist=mysqli_query($link,$sqlcustlist);
	$customer_string='';
	$SAP_customer_code_string='';
	while($rowcustlist=mysqli_fetch_assoc($rscustlist))
	{
		$customer_string=$customer_string."'".$rowcustlist['dns_customer_code']."'".',';
		if($rowcustlist['SAP_customer_code']!=''){
		$SAP_customer_code_string=$SAP_customer_code_string."'".$rowcustlist['SAP_customer_code']."'".',';
		}
	}
	$customer_string=substr($customer_string,0,-1);
	$SAP_customer_code_string=substr($SAP_customer_code_string,0,-1);
...
$sqlquerycustlist="SELECT DISTINCT CM.customer_code,CM.dns_customer_code, CM.customer_name,CM.cust_type
				FROM  customer_master CM WHERE  (CM.dns_customer_code IN(".$customer_string.") OR CM.customer_id IN(".$SAP_customer_code_string."))
				ORDER BY CM.cust_type DESC,CM.customer_name ASC LIMIT 0,1500";		
				//echo"<pre>";print_r($sqlquerycustlist);die;
