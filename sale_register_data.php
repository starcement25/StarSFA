...
OH.order_no LIKE 'O%' AND ".$date_condition." AND OH.customer_code = CM.customer_code 
...
if($total_row_check>0){
	...
	while($row_order_header = mysqli_fetch_assoc($res_order_header)){
		...
		while($row_order_details = mysqli_fetch_assoc($res_order_details)){
			...
			$sql_prod_details = "SELECT prod_desc, product_group_code, product_sub_group_code, product_brand_code FROM product_master 
