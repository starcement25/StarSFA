	...
	if(modified_customer_emp_route=='yes')
	{
		...
		if(strtoupper(substr($emp_code,0,1))=='C')
		{
			$sqlquery="SELECT DISTINCT c.customer_code,c.recid, cc.customer_name, c.invoice_id, c.date, c.invoice_amount, c.due_amount 
				...
				c.customer_code = cc.customer_code AND  cc.customer_code='".$emp_code."'
				ORDER BY c.date ASC";
		}
		if(strtoupper(substr($emp_code,0,1))=='B')
		{
		$sqlquery="SELECT DISTINCT c.customer_code,c.recid, cc.customer_name, c.invoice_id, c.date, c.invoice_amount, c.due_amount 
				FROM outstanding c, customer_master cc,customer_broker_relation CRER WHERE 
				c.customer_code = cc.customer_code AND CRER.customer_code=cc.customer_code AND CRER.broker_code='".$emp_code."' 
				ORDER BY c.date ASC";
		}
	}
	else
...
$count=mysqli_num_rows($result);
	...
	if($count>0){
		...
		while($rowoutstanding = mysqli_fetch_assoc($result))
		{
			/*$contents.="<data>";
