...
,TAP.QTY,TAP.STATUS,TAP.freight,TAP.destination_code,TAP.destination_name,TAP.destination_address,TAP.phone_no,TAP.dump_status,TAP.dump_code,TAP.dump_name,TAP.dealer_truck,TAP.approval_status,TAP.order_by,TAP.consignee_address FROM T_APPERPDO_APPROVAL TAP,customer_route_emp_relation CM WHERE CM.customer_code=TAP.customer_code ".$login_condition.$emp_hierarchy_condition." AND TAP.approval_status='PENDING'";	
...
$count=mysqli_num_rows($result);
	...
	if($count>0){
		...
		while($rowsorderapproval = mysqli_fetch_assoc($result))
		{
			...
			$consignee_address=$rowsorderapproval['consignee_address'];
			${order_by.$APPORDERNO}=$rowsorderapproval['order_by'];
			
			if(${order_by.$APPORDERNO}!='')
			{
				$sqlbroker="SELECT broker_name FROM broker_master WHERE dns_broker_id='".${order_by.$APPORDERNO}."'";
			...
			$contents  .= (($broker_name!='')?$broker_name: ' ')."^";
			$contents  .= (($consignee_address!='')?$consignee_address: ' ');
	...
	$url = APICALLLOGURL."/order-approval-details-incremental-6.0.2.php?nick_name=$nick_name&emp_code=$emp_code&last_update_time=$last_update_time&incremental_download=$incremental_download";
