...
if(strtoupper(substr($emp_code,0,1))=='E')
  {
    $sqlquery="SELECT customer_code,destination,DO_no,vehicle_no,trans_response_date,arrival_date_gate,trans_response_id,arrival_gate_id
			...
			exit_approval_id,exit_approval_date,security_chk_date,security_chk_id FROM DO_tracking WHERE 
			customer_code IN(SELECT customer_code FROM customer_route_emp_relation WHERE acedns='Y' AND emp_code='".$emp_code."')";
  }
  ...
  if(strtoupper(substr($emp_code,0,1))=='B')
  {
    $sqlquery="SELECT customer_code,destination,DO_no,vehicle_no,trans_response_date,arrival_date_gate,trans_response_id,arrival_gate_id
			,despatch_approval_date,despatch_approval_id,gate_vehicle_in_id,gate_vehicle_in_date,wb_in_id,wb_in_date,tare_weight,
			loading_id,loading_date,bay_no,GRN_attachment,GRN_attached_date,wb_out_id,wb_out_date,despatch_trans_id,invoice_date,
			exit_approval_id,exit_approval_date,security_chk_date,security_chk_id FROM DO_tracking WHERE SUBSTRING(DO_no,3,6)='".$emp_code."'";
  }
	...
	if($count>0){
		...
		{
			$sqlquery="SELECT dns_DO_no FROM DO_transaction WHERE DO_no='".$rowvehicle['DO_no']."'";
			$rsquery=mysqli_query($link,$sqlquery);
			$rowquery=mysqli_fetch_assoc($rsquery);
			$dns_DO_no=str_replace('DO//','DO/BHD/',$rowquery['dns_DO_no']);
