...
if(survey=='yes')
{
	...
	$sqlquery="SELECT DISTINCT BM.branch_code,BM.branch_name,BM.comp_code,BM.HQ,BM.plant_name FROM 
				branch_master BM,employee_master EM WHERE FIND_IN_SET(BM.branch_code,EM.branch_code) AND ".$emp_hierarchy_condition.$login_condition." 
				ORDER BY BM.branch_name ASC";			
}
else
{
	$sqlquery="SELECT BM.branch_code,BM.branch_name,BM.comp_code,BM.HQ,BM.plant_name FROM branch_master BM WHERE 
				1 ".$login_condition." ORDER BY BM.branch_name ASC";
}
	...
	$url = "http://www.acedns.in/acednsproduct/branch-master-txt-6.0.2.php?nick_name=$nick_name&emp_code=$emp_code&last_update_time=$last_update_time&data_download_time=$data_download_time&incremental_download=$incremental_download";
