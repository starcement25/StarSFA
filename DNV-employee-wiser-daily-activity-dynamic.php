...
$current_date=date('Y-m-d', strtotime("-6 days,$curdateserver "));
echo $current_date_fromat=date('d-m-Y',strtotime($current_date));
...
$sql="SELECT DISTINCT level FROM employee_master WHERE acedns='Y' AND level >1 ORDER BY level ASC";
$rs=mysqli_query($link,$sql);
while($row=mysqli_fetch_assoc($rs))
{
$level_emp=$row['level'];	
//For ASM
$sqlselbaselevel="SELECT emp_code,emp_name,email,designation,reporting_to,acedns,level FROM employee_master WHERE level='".$level_emp."' AND acedns='Y'";
...
while($rowbaselevel=mysqli_fetch_assoc($rsselbaselevel))
{
	...
	${employee_hierarchy.$base_emp_code}=return_employee_hierarchy_active($base_emp_code);
	...
	$base_emp_level=$rowbaselevel['level'];
	...
	if($base_emp_level ==2)
	{						
	$sql_get_empdetails ="SELECT emp_code,emp_name,level FROM employee_master WHERE ".${emp_hierarchy_condition.$base_emp_code}." AND acedns='Y' 
						AND emp_code <>'".$base_emp_code."' ORDER BY emp_name ASC";
	}
	else
	{
	$sql_get_empdetails ="SELECT emp_code,emp_name,level FROM employee_master WHERE reporting_to='".$base_emp_code."' AND acedns='Y' ORDER BY emp_name ASC";
	}
	...
	while($row_get_empdetails = mysqli_fetch_assoc($res_get_empdetails)){
		...
		$emp_level_level1 = $row_get_empdetails['level'];
		echo ${employee_hierarchy.$emp_code_level1}=return_employee_hierarchy_active($emp_code_level1);
		
		${total_hierarchy.$base_emp_code} += (count(explode(',',${employee_hierarchy.$emp_code_level1})));
		
		$sql_get_locdetails = "SELECT SUM(CASE WHEN LO.trans_id LIKE 'A%'  THEN 1 ELSE 0 END ) AS attendance,SUM(CASE WHEN LO.trans_id LIKE 'O%' THEN 1 ELSE 0
						END ) AS total_order_count FROM location LO WHERE  (LO.trans_id LIKE 'A%' OR LO.trans_id LIKE 'O%' )".$datecondition ."
						AND LO.emp_code IN(".${employee_hierarchy.$emp_code_level1}.") ORDER BY LO.date ASC";
		...
		${total_att.$base_emp_code} += $attendance_level1;
		...
		$sql_productive_nonproductive_call = "SELECT SUM(CASE WHEN (LO.trans_id LIKE 'O%' OR LO.trans_id LIKE 'S%' OR LO.trans_id LIKE 'P%') AND LO.trans_id 
									...
									THEN 1 ELSE 0 END ) AS non_productive_call  FROM location LO WHERE LO.emp_code 
									IN(".${employee_hierarchy.$emp_code_level1}.")".$datecondition;
		...
		if($total_order_count_level1>0){
			...
			$sql_total_primary_secondary = "SELECT COUNT(DISTINCT order_no) AS order_header_count, COUNT(order_no) AS order_details_count,
											SUM(amount) AS amount FROM prev_order_counting_master WHERE SUBSTRING(order_no,2,5) IN(".${employee_hierarchy.$emp_code_level1}.") 
		...
		${table.$base_emp_code}.="<tr>
					<td>".$count."</td>
					<td align='left'>".$emp_name_level1."(level ".$emp_level_level1.")</td>
					<td>".(count(explode(',',${employee_hierarchy.$emp_code_level1}))-1)."</td>
	...
	${table.$base_emp_code}.="<tr style='font-weight:bold;background:yellow;'>
			...
			<td align='right'>".${total_hierarchy.$base_emp_code}."</td>
			<td align='right'>".${total_att.$base_emp_code}."</td>
	...
	//echo "</table>";
	echo $base_emp_name."(level ".$base_emp_level.")";
	echo '<br />';
