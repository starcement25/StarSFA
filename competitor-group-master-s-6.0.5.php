...
if($nick_name=='START' || $nick_name=='STAR')
{
	...
	$sqlquery="SELECT DISTINCT competitor_name,group_name,UOM,display_name,branch_code,product_type FROM competitor_group_master 
	WHERE acedns='yes' ".$condition_branch." AND branch_code <>'' AND product_type <>''";  
//echo $sqlquery; exit();
	/*$sqlquery="SELECT DISTINCT competitor_name,group_name,UOM,display_name,branch_code FROM competitor_group_master 
	WHERE acedns='yes' ".$condition_branch." AND branch_code <>''  
	UNION ALL SELECT DISTINCT competitor_name,group_name,UOM,display_name,branch_code 
	FROM competitor_group_master WHERE acedns='yes' AND group_name='WSP' ORDER BY competitor_name ASC";*/
...
$count=mysqli_num_rows($result);
	...
	if($count>0){
		...
		while($rowcompetitorgroup = mysqli_fetch_assoc($result))
		{
			...
			$contents  .= (($rowcompetitorgroup['display_name']!='')?$rowcompetitorgroup['display_name']: ' ')."^";
			$contents  .= (($rowcompetitorgroup['branch_code']!='')?$rowcompetitorgroup['branch_code']: ' ')."^";
			$contents  .= (($rowcompetitorgroup['product_type']!='')?$rowcompetitorgroup['product_type']: ' ');
