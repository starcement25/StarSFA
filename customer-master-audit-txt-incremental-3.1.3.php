...
$countbranches=mysqli_num_rows($rsbranches);
	 ...
	 {
	  $sqlquery="SELECT DISTINCT c1.customer_code, c1.customer_name, c1.route_code,c1.emp_code,c1.current_balance,c1.credit_limit,c1.black_list,c1.acedns,
				c1.TD,c1.cust_type FROM customer_master c1,employee_master em WHERE ".$emp_hierarchy_condition." ".$login_condition." ORDER BY c1.customer_name ASC"; 
	 }
...
$count=mysqli_num_rows($result);
	...
	$url = "http://www.acedns.in/acednsproduct/customer-master-audit-txt-incremental-3.1.3.php?nick_name=$nick_name&emp_code=$emp_code&last_update_time=$last_update_time&data_download_time=$data_download_time&incremental_download=$incremental_download";
	insertapilog($datetime,$emp_code,$url,$nick_name);
	/*$config = 'api_calllog.txt';
	$file=fopen($config,"r+");
	$date = date("F j, Y");
	$time = date("H:i:s");
	$newuser ="[$date $time]"."http://www.acedns.in/acednsproduct/customer-master-audit-txt-incremental-3.1.3.php?nick_name=$nick_name&emp_code=$emp_code&last_update_time=$last_update_time&data_download_time=$data_download_time&incremental_download=$incremental_download"."\r\n";
