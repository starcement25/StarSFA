...
{
   $sqlquery="SELECT CM.customer_name,CM.customer_code,DM.DO_no,DM.DO_qty AS qty, DM.DO_amount AS amount,PM.prod_desc,PM.prod_code,DM.DO_status	 FROM customer_master CM,DO_transaction DM,product_master PM where DM.sku_code=PM.prod_code and DM.customer_code=CM.customer_code AND DM.DO_no='".$DO_no."' ORDER BY CM.customer_name ASC
";
}
...
if(strtoupper(substr($emp_code,0,1))=='B')
{
	...
	if($count>0){
		...
		while($rowDOtrans = mysqli_fetch_assoc($result))
		{
			...
			$DO_status=$rowDOtrans['DO_status'];
	...
	$url = APICALLLOGURL."/DO-downlod-mode-sku-wise-6.0.0.php?nick_name=$nick_name&emp_code=$emp_code&mode=$mode&DO_no=$DO_no&from_date=$from_date&to_date=$to_date";

	insertapilog($datetime,$emp_code,$url,$nick_name);
	header("Content-type: application/text"); 
	header("Content-Disposition: attachment; filename=DO_transaction_mode_sku_wise.txt");
	print "$datacontents"; 
	mysqli_close($link);		
?>