...
$sqlquery="SELECT DISTINCT SAPW.product_group_code,SAPW.customer_code, SAPW.jan_31_target,SAPW.jan_31_achievement,SAPW.feb_28_target,SAPW.feb_28_achievement,SAPW.mar_31_target,
...
FROM self_appraisal_cust_productgroup_wise SAPW,customer_route_emp_relation CRER WHERE SAPW.customer_code = CRER.customer_code  AND ".$emp_hierarchy_condition."";
...
$count=mysqli_num_rows($result);
	...
	if($count>0){
		...
		while($rowsappraisal = mysqli_fetch_assoc($result))
		{
			for($i=1;$i<=12;$i++)
			{
				...
				$contents  .= (($rowsappraisal['customer_code']!='')?trim(preg_replace('/[\r\n]+/', '',$rowsappraisal['customer_code'])): ' ')."^";
				$contents  .= ''."^";
				$contents  .= $i."^";
	...
	$url = APICALLLOGURL."/productgroup-cust-wise-target-achievement-txt.php?nick_name=$nick_name&emp_code=$emp_code";
	insertapilog($datetime,$emp_code,$url,$nick_name);

	header("Content-type: application/text"); 
	header("Content-Disposition: attachment; filename=productgroup_cust_wise_target_ach.txt");
	print "$datacontents"; 
	mysqli_close($link);		
?>
