...
if($emp_code!='C0007'){
	$sqlquery="SELECT emp_code,emp_name,month,district,customer_code,customer_name,volume_target,volume_achievement,collection_target,
					collection_achievement,cust_type FROM emp_target_achievement WHERE 1 ".$emp_hierarchy_condition;
}
else
{
	$sqlquery="SELECT emp_code,emp_name,month,district,customer_code,customer_name,volume_target,volume_achievement,collection_target,
			collection_achievement,cust_type FROM emp_target_achievement WHERE 1";
}
...
$count=mysqli_num_rows($result);
	...
	if($count>0){
		...
		while($rowemptarget = mysqli_fetch_assoc($result))
		{
			...
			$contents  .= (($rowemptarget['cust_type']!='')?$rowemptarget['cust_type']: ' ');
