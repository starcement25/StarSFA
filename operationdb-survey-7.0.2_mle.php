...
$last_git_master_update_time = $last_git_master_update_time = $last_loyalty_purchase_update_time = '';
...
//$xmlObj = simplexml_load_string($body1);*/
...
function contents($parser, $data)
{
	...
	if (substr($current_tag, 0, 16) == '*ROOT*ATTENDANCE') {
		...
		switch ($current_tag) {
			case $attendance_emp_code:
				//$attendacne_array[$counter] = new xml_attendance();
	...
	if (substr($current_tag, 0, 12) == '*ROOT*SURVEY') {
		//echo $countersurvey.$current_tag.'<br />';
		//echo $countersurvey;
		//echo $data.'<br />';
...
//print_r($survey_array);die;
// echo '<pre>' . count($survey_array);
// echo '<pre>' . count($survey_details_array);
// echo '<pre>' . count($attendance_array);
// print_r($survey_array);
//print_r($survey_details_array);
//print_r($attendance_array);
//die;
//echo"<pre>";print_r($data);
//echo"<pre>";print_r($xml_parser);die;
...
//add line sk 26-08-25
			foreach ($survey_details_array as $key => $item) {
				if ($item->row_id === "RA601") {
					// Save the object
					$ra601 = $item;

					// Remove from current position
					unset($survey_details_array[$key]);

					// Reindex and put at start
					$survey_details_array = array_values($survey_details_array);
					array_unshift($survey_details_array, $ra601);

					break;
				}
			}
		//end line sk 26-08-25	
...
if (count($survey_array) > 0) {
	...
	for ($x = 0; $x < count($survey_array); $x++) {
		...
		if ($survey_emp_code != '' && $survey_trans_id != '') {
			...
			if ($countchkorlocation > 0) {
				...
				$survey_date = !empty($survey_date) ? $survey_date : date('Y-m-d H:i:s');
				...
				if ($surveyheader_survey_id != '') {
					...
					//	echo"<pre>";print_r($sql_insert_survey_header);die;
	...
	//echo count($survey_details_array);die;
		...
		for ($i = 0; $i < count($survey_details_array); $i++) {
			...
			if (!in_array($survey_id, $survey_array_trans_id)) {
				...
				if ($insert_table_detail != '') {
					...
					//print_r($insert_table_detail_array);die;
			...
			if (strtoupper($nick_name) == 'DURO' && (${'insert_table_name' . $survey_id} == 'facilitator_master' || ${'insert_table_name' . $survey_id} == 'site_master') && ($row_id == 'RA164' || $row_id == 'RA024' || $row_id == 'RA197')) {
				...
				if (${'insert_table_name' . $survey_id} == 'facilitator_master') {
					${'update_col_name' . $survey_id} = 'f_code';
				}
				if (${'insert_table_name' . $survey_id} == 'site_master') {
					${'update_col_name' . $survey_id} = 'site_id';
				}
	...
	for ($countarr = 0; $countarr < count($surveytansid_array_mailbody); $countarr++) {
		...
		$survey_email = 'atanu.sahoo@sbinfowaves.com';
		...
		/*$headers .= "MIME-Version: 1.0\r\n";			
			...
			$headers .= $strContent1."\n\n";*/
		//echo '<pre>';print_r(array('survey_email'=> $survey_email, 'surveyemailsubj'=> $surveyemailsubj, 'surveyemailbody'=>$surveyemailbody, 'headers'=>$headers, 'spam_filter'=>$spam_filter));
		//this mail funtion not working thats why comment out 14.05.2025
