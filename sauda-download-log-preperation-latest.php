...
mysqli_select_db(DB);
	...
	$sql_sauda_header_download = "SELECT CM.dns_customer_code,CM.customer_code,CM.customer_name,CM.credit_days,CM.state_code,SH.broker_id,SH.sauda_no,DATE_FORMAT(LO.date,'%d-%m-%Y') AS sauda_date,DATE_FORMAT(LO.date,'%H:%i:%s') AS sauda_time,
								 ...
								 PGM.product_group_name,PGM.is_upload,PGM.formulation,SH.d_instruction,RM.route_name FROM sauda_header SH,location LO,sauda_details SD,product_master PM,customer_master CM,branch_master BM,employee_master EM, product_group_master PGM,route_master RM
								 ...
								 SH.customer_code=CM.customer_code AND SH.branch_code=BM.branch_code AND PM.product_group_code=PGM.product_group_code AND CM.route_code=RM.route_code
	...
	while($rec_sauda_header_download = mysqli_fetch_assoc($rs_sauda_header_download))
	{
		...
		$PR00=$material_cost+$packing_cost+$honeycomb_cost+$brokerage_cost+$margin_cost+$premium-$TD-$liquid_TD;
		...
		$realization_per_case=$material_cost+$packing_cost+$margin_cost+$premium-$TD-$liquid_TD-$packing_realization;
		...
		/*else
		{
			$sqlupdatesaudadownloadlog="UPDATE sauda_download_log SET 
										realization_per_case='".$realization_per_case."',
									   realization_per_MT='".$realization_per_MT."' WHERE  sauda_no='".$sauda_no."' AND prod_code='".$dns_prod_code."'";
