...
{
   $sqlquery="SELECT DISTINCT MRP.* FROM mrp MRP,employee_master EM WHERE EM.branch_code=MRP.branch_code 
...
$count=mysqli_num_rows($result);
	...
	$url = "http://www.acedns.in/acednsproduct/mrp-txt-incremental-5.0.7.php?nick_name=$nick_name&emp_code=$emp_code&last_update_time=$last_update_time&data_download_time=$data_download_time&incremental_download=$incremental_download";
	insertapilog($datetime,$emp_code,$url,$nick_name);
	/*$config = 'api_calllog.txt';
	$file=fopen($config,"r+");
	$date = date("F j, Y");
	$time = date("H:i:s");
	$newuser ="[$date $time]"."http://www.acedns.in/acednsproduct/mrp-txt-incremental-5.0.7.php?nick_name=$nick_name&emp_code=$emp_code&last_update_time=$last_update_time&data_download_time=$data_download_time&incremental_download=$incremental_download"."\r\n";
