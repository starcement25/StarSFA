...
$count=mysqli_num_rows($result);
	...
	$contents  .= (($rowsite['petty_contractor_head_mason_name']!='')?$rowsite['petty_contractor_head_mason_name']: '')."^";
			...
			$contents  .= (($rowsite['engineer_name']!='')?trim(preg_replace('/[\r\n]+/', '',$rowsite['engineer_name'])): ' ')."^";
			...
			$contents  .= (($rowsite['site_segment']!='')?trim(preg_replace('/[\r\n]+/', '',$rowsite['site_segment'])): ' ')."^";
			...
			$contents  .= (($rowsite['reason_for_not_delivery']!='')?trim(preg_replace('/[\r\n]+/', '',$rowsite['reason_for_not_delivery'])): ' ')."^";
			$contents  .= (($rowsite['reasons_for_non_conversion']!='')?trim(preg_replace('/[\r\n]+/', '',$rowsite['reasons_for_non_conversion'])): ' ')."^";
			
			$contents  .= (($rowsite['other_ remarks']!='')?trim(preg_replace('/[\r\n]+/', '',$rowsite['other_ remarks'])): ' ')."^";
			$contents  .= (($rowsite['overall_remarks']!='')?trim(preg_replace('/[\r\n]+/', '',$rowsite['overall_remarks'])): ' ')."^";
			$contents  .= (($rowsite['branch']!='')?trim(preg_replace('/[\r\n]+/', '',$rowsite['branch'])): ' ')."^";
			$contents  .= (($rowsite['district']!='')?trim(preg_replace('/[\r\n]+/', '',$rowsite['district'])): ' ')."^";
			$contents  .= (($rowsite['price_rsp_bags']!='')?$rowsite['price_rsp_bags']: ' ');
...
$contents = preg_replace('/\s+/', ' ', $contents);
	...
	$url = APICALLLOGURL."/site-lead-conversion-txt-incremental-6.0.1.php?nick_name=$nick_name&emp_code=$emp_code&last_update_time=$last_update_time&incremental_download=$incremental_download";
