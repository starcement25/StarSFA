...
$sql = "select ISNULL(SAT.MONTHVAL,'NA') MONTHVAL,SUM(SAT.QTYSHIPPED) AS tot_qty,SUM(SAT.TBASE1) AS tot_amount from ( select ID.*,SUBSTRING(CAST(IH.INVDATE AS varchar(20)),1,6) AS MONTHVAL FROM OEINVD ID INNER JOIN OEINVH IH ON ID.INVUNIQ=IH.INVUNIQ AND (IH.location LIKE '1%' or IH.location LIKE 'RUIYA%' or IH.location LIKE '3004%') UNION select ID.*,SUBSTRING(CAST(IH.INVDATE AS varchar(20)),1,6) AS MONTHVAL FROM OEINVD ID INNER JOIN OEINVH IH ON ID.INVUNIQ=IH.INVUNIQ INNER JOIN sales_hierarchy_new c ON c.SUBZONE=SUBSTRING(CAST(IH.location AS varchar(20)),1,4) AND (IH.location LIKE '3%' and IH.location not LIKE '3004%') ) SAT group by SAT.MONTHVAL";
...
while( $row = sqlsrv_fetch_array( $stmt, SQLSRV_FETCH_NUMERIC) ) {
   ...
   $json_array['saleDetailsYTD'] = $response;
