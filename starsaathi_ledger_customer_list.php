...
$emp_code=$_REQUEST['emp_code'];
	...
	//$sqlcustlist="SELECT DISTINCT CC.dns_customer_code FROM customer_route_emp_relation CRR,customer_master CC WHERE CC.customer_code=CRR.customer_code 
					...
					$sqlcustlist="SELECT DISTINCT CC.customer_code FROM customer_route_emp_relation CRR,customer_master CC WHERE CC.customer_code=CRR.customer_code 
					AND CRR.acedns='Y' $emp_hierarchy_condition";
	...
	while($rowcustlist=mysqli_fetch_assoc($rscustlist))
	{
		...
		$customer_code_string=$customer_code_string."'".$rowcustlist['customer_code']."'".',';
	}
...
$sqlquerycustlist="SELECT DISTINCT CM.customer_code,CM.dns_customer_code, CM.customer_name,CM.cust_type
				FROM  customer_master CM WHERE  SUBSTR(lower(CM.cust_type),1,1) IN ('d','e') AND CM.customer_code IN(".$customer_code_string.")
				ORDER BY CM.cust_type DESC,CM.customer_name ASC ";	
