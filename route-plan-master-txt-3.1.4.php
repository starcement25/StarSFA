...
$sqlaccessperiod="select DATE_FORMAT(access_start_date,'%d-%m-%Y') AS access_start_date,DATE_FORMAT(access_end_date,'%d-%m-%Y') AS access_end_date 
				FROM route_plan_access_period where emp_code='".$emp_code."'";
...
if($countaccessperiod>0)
{
	...
	$contentsaccessperiod=$rowaccessperiod['access_start_date'].'µ'.$rowaccessperiod['access_end_date'];
}
else
{
