...
{
	//$sqlquery="SELECT * FROM sauda_mrp WHERE 1  ".$login_condition."";
	$sqlquery="SELECT * FROM sauda_mrp WHERE acedns='Y' ORDER BY parent_child ASC";
}
...
$count=mysqli_num_rows($result);
	...
	if($count>0){
		...
		while($rowprice = mysqli_fetch_assoc($result))
		{
			...
			if(!in_array($rowprice['product_code'],$productarr))
			{
			...
			array_push($productarr,$rowprice['product_code']);
			$countmrp++;
			}
		}
		$contentsrowcolumn  =$countmrp.'¥'.'7';
	...
	$url = APICALLLOGURL."/bargain-rate-txt-incremental-6.0.0.php?nick_name=$nick_name&emp_code=$emp_code&last_update_time=$last_update_time&data_download_time=$data_download_time&incremental_download=$incremental_download";
