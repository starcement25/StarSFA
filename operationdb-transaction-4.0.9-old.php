				...
				$mrp=$recmrpdetails['mrp'];
					...
					if(!in_array($orderdatadetails_order_no,$order_array_trans_id))
					{
						...
						if(mysqli_query($link,$sqlinsertorderdetails))
						{
							...
							if($orderdataheader_transaction_type=='BT' || $orderdataheader_transaction_type=='ST' || $orderdataheader_transaction_type=='SA')
							{
								if($orderdataheader_transaction_type=='ST')
								{
								...
								if($orderdataheader_transaction_type=='BT' || $orderdataheader_transaction_type=='SA')
								{
						...
						if(mrp=='yes' && TD=='yes' && TD_type=='sku wise'){
						 ...
						 if(VAT=='yes')
						 {
							 if(strpos($orderdatadetails_VAT,'.')!=false){
								$orderemailbody_VAT_VAL="<td style='width:50px;text-align:right;min-height:21px;background-color:white'>
								<span style='font-family:Arial CE;font-size:10pt'>".$orderdatadetails_VAT."</span>&nbsp;</td>";
							 }
							 else
							 {
								$orderemailbody_VAT_VAL="<td style='width:50px;text-align:right;min-height:21px;background-color:white'>
						 ...
						 if(amount=='yes')
						 {
							 ...
							 {
							 	$amount_mail=$orderdatadetails_amount;
							 }
							 else
							 {
							 ...
							 {
								 $sale_rate_mail=$orderdatadetails_sale_rate;
							 }
							 else
							 {
								 $sale_rate_mail=number_format($orderdatadetails_sale_rate,2);
							 }
						 }
						 else
						 {
						...
						${a.$orderdatadetails_order_no} .="
								...
								<td style='width:50px;text-align:right;min-height:21px;background-color:white'>
								<span style='font-family:Arial CE;font-size:10pt'>".$sale_rate_mail."</span>&nbsp;</td>".$orderemailbody_TD_VAL.$orderemailbody_VAT_VAL."
								<td style='width:50px;text-align:right;min-height:21px;background-color:white'>
								<span style='font-family:Arial CE;font-size:10pt'>".$amount_mail."</span>&nbsp;</td>
								</tr>";
					}
			}//End for loop
		}
...
$url = "http://www.acedns.in/acednsproduct/operationdb-transaction-4.0.9.php?nick_name=$nick_name&emp_code=$emp_code&last_update_time=$last_update_time&last_git_master_update_time=$last_git_master_update_time";
insertapilog($datetime,$emp_code,$url,$nick_name);
	/*$config = 'api_calllog.txt';
	$file=fopen($config,"r+");
	$date = date("F j, Y");
	$time = date("H:i:s");
	$newuser ="[$date $time]"."http://www.acedns.in/acednsproduct/operationdb-transaction-4.0.9.php?nick_name=$nick_name&emp_code=$emp_code&last_update_time=$last_update_time&last_git_master_update_time=$last_git_master_update_time"."\r\n";
