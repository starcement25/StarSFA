...
$count=mysqli_num_rows($result);
	...
	if($count>0){
		...
		while($rowselfappraisaldetails = mysqli_fetch_assoc($result))
		{
				$contents.="<data>";
				$contents .='<self_appraisal_id><![CDATA['.mb_convert_encoding($rowselfappraisaldetails['self_appraisal_id'], 'UTF-8', 'UTF-8').']]></self_appraisal_id>
							...
							<previous_year><![CDATA['.mb_convert_encoding($rowselfappraisaldetails['previous_year'], 'UTF-8', 'UTF-8').']]></previous_year>
							<vertical_wise><![CDATA['.mb_convert_encoding($rowselfappraisaldetails['vertical_wise'], 'UTF-8', 'UTF-8').']]></vertical_wise>
							';
				$contents.="</data>";
				//echo $cnt++;
		}
	...
	$url = APICALLLOGURL."/self-appraisal-details-incremental-6.0.2.php?nick_name=$nick_name&last_update_time=$last_update_time&incremental_download=$incremental_download&mode=$mode";
