...
/*$sqlquery="SELECT f_code,emp_code,facilitator_name,f_type,firm_name,f_address,f_pin,f_area,f_sub_area,mobile_no,email_id,dob,
		annniversary,acedns,branch_code,check_in_date,f_category FROM facilitator_master WHERE 
			...
			//f_sub_area,branch_code,check_in_date, f_category,f_city,f_state,comment table ,nature_of_work, branch_code='".$branch_code."' sk 07-05-25
$sqlquery="SELECT f_code,emp_code,facilitator_name,f_type,firm_name,f_address,f_pin,f_area,mobile_no,email_id,dob,
		annniversary,acedns,designation,visit_count,f_status,architect_name,type_of_work,running_sites,product_required,sample_required,sample_status,f_conversion,f_product,f_product_qty,f_product_value,f_product_thickness,f_history,next_follow_up_date 
		FROM facilitator_master WHERE 1
			".$login_condition." ORDER BY facilitator_name ASC";	
					
$result = mysqli_query($link,$sqlquery);
$count=mysqli_num_rows($result);
	$cnt=1;
	$contentsrowcolumn  =$count.'¥'.'36';
	if($count>0){
		...
		while($rowfaclitator = mysqli_fetch_assoc($result))
		{
			...
			$contents  .= (($rowfaclitator['visit_count']!='')?trim(preg_replace('/[\r\n]+/', '',$rowfaclitator['visit_count'])): ' ')."^";
			$contents  .= (($rowfaclitator['f_status']!='')?trim(preg_replace('/[\r\n]+/', '',$rowfaclitator['f_status'])): ' ')."^";
			$contents  .= (($rowfaclitator['architect_name']!='')?trim(preg_replace('/[\r\n]+/', '',$rowfaclitator['architect_name'])): ' ')."^";
			$contents  .= (($rowfaclitator['type_of_work']!='')?trim(preg_replace('/[\r\n]+/', '',$rowfaclitator['type_of_work'])): ' ')."^";
			$contents  .= (($rowfaclitator['running_sites']!='')?trim(preg_replace('/[\r\n]+/', '',$rowfaclitator['running_sites'])): ' ')."^";
			$contents  .= (($rowfaclitator['product_required']!='')?trim(preg_replace('/[\r\n]+/', '',$rowfaclitator['product_required'])): ' ')."^";
			$contents  .= (($rowfaclitator['sample_required']!='')?trim(preg_replace('/[\r\n]+/', '',$rowfaclitator['sample_required'])): ' ')."^";
			$contents  .= (($rowfaclitator['sample_status']!='')?trim(preg_replace('/[\r\n]+/', '',$rowfaclitator['sample_status'])): ' ')."^";
			$contents  .= (($rowfaclitator['f_conversion']!='')?trim(preg_replace('/[\r\n]+/', '',$rowfaclitator['f_conversion'])): ' ')."^";
			$contents  .= (($rowfaclitator['f_product']!='')?trim(preg_replace('/[\r\n]+/', '',$rowfaclitator['f_product'])): ' ')."^";
			$contents  .= (($rowfaclitator['f_product_qty']!='')?trim(preg_replace('/[\r\n]+/', '',$rowfaclitator['f_product_qty'])): ' ')."^";
			$contents  .= (($rowfaclitator['f_product_value']!='')?trim(preg_replace('/[\r\n]+/', '',$rowfaclitator['f_product_value'])): ' ')."^";
			$contents  .= (($rowfaclitator['f_product_thickness']!='')?trim(preg_replace('/[\r\n]+/', '',$rowfaclitator['f_product_thickness'])): ' ')."^";
			$contents  .= (($rowfaclitator['f_history']!='')?trim(preg_replace('/[\r\n]+/', '',$rowfaclitator['f_history'])): ' ')."^";
			$contents  .= (($rowfaclitator['next_follow_up_date']!='')?trim(preg_replace('/[\r\n]+/', '',$rowfaclitator['next_follow_up_date'])): ' ');
