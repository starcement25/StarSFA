...
if(strtoupper($nick_name)=='PALSONS'){
	$sqlquery="SELECT POCM.order_no,POCM.customer_code,POCM.product_code,POCM.visit_qty,POCM.d_instruction,POCM.rate,POCM.amount,
			SUBSTRING(POCM.visit_date,1,10) as entry_date,POCM.creation_type
			...
			".$login_condition."  AND POCM.status='pending' AND (POCM.order_no LIKE 'O%' OR POCM.order_no LIKE 'NO%')";
}
else
{
$sqlquery="SELECT POCM.order_no,POCM.customer_code,POCM.product_code,POCM.visit_qty,POCM.d_instruction,POCM.rate,POCM.amount,
...
$count=mysqli_num_rows($result);
	...
	if($count>0){
		...
		while($rowsorderstatus = mysqli_fetch_assoc($result))
		{
			...
			$creation_type=$rowsorderstatus['creation_type'];
			$remarks='';
			$flag='1';
			...
			$contents  .= (($d_instruction!='')?trim(preg_replace('/[\r\n]+/', '',$d_instruction)): ' ')."^";
			$contents  .= (($creation_type!='')?$creation_type: ' ');
	...
	$url = APICALLLOGURL."/customer-product-info-txt.6.0.1.php?nick_name=$nick_name&emp_code=$emp_code&last_update_time=$last_update_time&data_download_time=$data_download_time&incremental_download=$incremental_download";
