...
$count=mysqli_num_rows($resultlocation);
	if($count>0){
		...
		while($rowlocation = mysqli_fetch_assoc($resultlocation))
		{
		   ...
		   if(strtoupper($nick_name)=='CORAL'){
		       $total_customer_visit=0;
		       $cust_order_sql = "SELECT `trans_id` FROM `location` WHERE `emp_code` = '$emp_code_lower' AND `date` LIKE '$currentdate%'";
                ...
                while ($cust_row = mysqli_fetch_assoc($cust_query)) {
                    ...
                    if (substr($cust_trans_id, 0, 3) == 'NOE' || substr($cust_trans_id, 0, 2) == 'OE') {
                        $total_customer_visit = $total_customer_visit+1;
                    }
                }
    
		   }
		   //echo $total_customer_visit;exit();
		   
		   
			if(strtoupper($nick_name)=='SYLVAN')
		   {
			  $sqlsurvey="SELECT COUNT(survey_id) AS tot_survey FROM survey_header WHERE 
			  					SUBSTRING(survey_id,-19,5)='".$emp_code_lower."' 
							AND DATE_FORMAT(SUBSTRING(survey_id,-14,14),'%Y-%m-%d %H:%i:%s') <='".$currentdatetime."' AND DATE_FORMAT(SUBSTRING(survey_id,-14,8),'%Y-%m-%d')='".$currentdate."'";
			   $rssurvey=mysqli_query($link,$sqlsurvey);
			   $rowsurvey=mysqli_fetch_assoc($rssurvey);
			   $tot_survey=$rowsurvey['tot_survey'];
			...
			$contents .= (($tot_survey!='')?$tot_survey: 0);
			}
			else
			{
			$contents = (($emp_code_lower!='')?$emp_code_lower: ' ')."^";
	...
	$url = "https://sfa.starcement.co.in/manager-activity-download-6.0.2.php?nick_name=$nick_name&emp_code=$emp_code&dateval=$currentdate&timeval=$current_time";
