...
if($branch_code !='')
{
	...
	$sqlquery="SELECT * FROM (SELECT PDF_file_name,branch_code,acedns,start_date,end_date,scheme_name FROM branch_schemes_PDF WHERE 
			FIND_IN_SET(branch_code,'".$branch_code."') AND UNIX_TIMESTAMP(download_time) > UNIX_TIMESTAMP('".$last_update_time."')
	...
	if($count>0){
		while($rowschemePDF = mysqli_fetch_assoc($result))
		{
			...
			$contents  .= (($rowschemePDF['scheme_name']!='')?$rowschemePDF['scheme_name']: ' ');
...
$datetime = gmdate('Y-m-d H:m:s',strtotime('+330 minute'));
	$url = APICALLLOGURL."/branchwise-scheme-download-txt-6.0.2.php?nick_name=$nick_name&emp_code=$emp_code&last_update_time=$last_update_time";
